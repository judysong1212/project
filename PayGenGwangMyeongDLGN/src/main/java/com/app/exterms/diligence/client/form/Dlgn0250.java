 ...
 **/
	...
	//  private FormData formData; 
      ...
      private Button btnDlgn0250Print1;
      /**
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       * 검색변수 선언 
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
//                      }
	   ...
	   //신규 
        ...
        btnDlgn0250Del.addListener(Events.Select, new Listener<ButtonEvent>() {
	  	...
	  	btnDlgn0250Print1 = new Button("근무상황부");
	  	btnDlgn0250Print1.setIcon(MSFMainApp.ICONS.print16());
	  	topDlgn0250Bar.add(btnDlgn0250Print1);
	  	btnDlgn0250Print1.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
