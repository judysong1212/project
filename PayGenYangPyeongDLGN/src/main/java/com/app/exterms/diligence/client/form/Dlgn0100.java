...
//				creSystemkey.setValue("");	//근태 개별 생성 후 일괄 생성할 경우 systemkey 들어가면 안되기 때문에
				...
				//엣지변환
				//MSFFormWindows msFwDlgnP010001= new MSFFormWindows("복무관리 일괄생성 (기본근무, 시간외근무 생성)" ,dlgnP010001Form,"닫기","520px", "340px",true);
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(year) || "".equals(month)){
						...
						if("".equals(businCd) && "".equals(mangeDeptCd) ){
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(), "사업명을 선택하십시요.", null);
	                         ...
	                         dto.setMangeDeptCd(mangeDeptCd);					//관리부서
