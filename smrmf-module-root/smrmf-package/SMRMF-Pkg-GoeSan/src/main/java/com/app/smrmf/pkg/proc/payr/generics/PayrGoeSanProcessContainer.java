...
import com.app.smrmf.pkg.proc.payr.goesan.a002001001.PayrA002001001_B0150000CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001001.PayrA002001001_B0150001CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001001.PayrA002001001_B0150002CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001001.PayrA002001001_B0150003CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001001.PayrA002001001_B0150004CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001001.PayrA002001001_B0150005CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001001.PayrA002001001_B0150006CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001002.PayrA002001002_B0150000CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001002.PayrA002001002_B0150001CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001002.PayrA002001002_B0150002CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001002.PayrA002001002_B0150003CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001002.PayrA002001002_B0150004CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001002.PayrA002001002_B0150005CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002001002.PayrA002001002_B0150006CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.a002002001.PayrA002002001_B0150000CalcProc;
import com.app.smrmf.pkg.proc.payr.goesan.comm.PayrCommCalcProc;
 ...
 */
@Controller 
public class PayrGoeSanProcessContainer  extends AbstractCustomServiceImpl {
	

	private static final Logger logger = LoggerFactory.getLogger(PayrGoeSanProcessContainer.class);
 ...
 */
	...
	//  PagingLoadResult<ShowMessageBM> retval = null; 
      List<ShowMessageBM>  bmResult = new ArrayList<ShowMessageBM>();
  	  ...
  	  // 전역 개념의 공통 메서드 처리 함수 
      PayrCommCalcProc mtdPayrCommCalcProc = new PayrCommCalcProc();
     ...
     //  int iDducCnt = 0;
      // boolean boolInsert = false;
       ...
       InfcPkgPayr0417VO  payr0417Vo = new InfcPkgPayr0417VO();  //일용직형태 공무직처리 객체
       ...
       //급여관련 처리 정보 객체 
       PkgPayrAmntVO   pkgPayrAmntVo = new PkgPayrAmntVO();
      ...
      //  BigDecimal pymtA0150003 = BigDecimal.ZERO;
        //과세합계
     ...
     //  boolInsert = true;  //payr0300 급여마스터 인서트 여부 
       ...
       pkgPayrAmntVo.setBoolInsert(true);  //payr0300 급여마스터 인서트 여부 
   ...
   //    DlgnProcessContainer dlgnProcessContainer = new DlgnProcessContainer();
       Map<String, Object> mapPayr = new HashMap<String, Object>();
        ...
        //payr0250 테이블 필드 맵핑
        payr0250Vo = mtdPayrCommCalcProc.call_MappingPayr0250(dtoPayr0250,payr0250Vo); 
                       ...
                       if (psnlList.size() > 0) { 
                               ...
                               for (int icnt = 0; icnt < psnlList.size();icnt++) { 
                                   ...
                                   if ("Y".equals(payCalculationVo.getPayRcptYn())) {     
                                	   ...
                                	   /***********************************************************************
		                        	    *  급여 마스터 테이블 저장 PAYR0300
		                        	    ***********************************************************************/
		                        	   if (pkgPayrAmntVo.isBoolInsert()) {
		                                   //직종 // 근속년수 에따른 급여 항목 정보 추출 및 단가표에서 값을 가지고 온다. 
		                             	   ...
		                             	   * 급여 마스터 테이블에 인서트 처리 
		                             	   ***************************************************************************/  
		                             	   mtdPayrCommCalcProc.call_funcPayr0300(request,payr0250Vo,sessionUser); 
		                             	  
		                                   pkgPayrAmntVo.setBoolInsert(false);
		                                   bmResult.add(new ShowMessageBM(PropPayGenConfs.PAYGEN_INFO,"Payr","[기간제근로자]급여마스터저장" ,"급여마스터저장완료","")); 
		                               }
		                        	   /***********************************************************************
		                        	    *  급여 마스터 테이블 저장 PAYR0300
		                        	    ***********************************************************************/
		                        	   ...
		                        	   /*************************************************************** 
		                                 4대보험 이력 테이블 추가 
		                               ****************************************************************/ 
		                               mtdPayrCommCalcProc.call_funcPayr0309(request,payr0250Vo,payCalculationVo,sessionUser); 
                                	   ...
                                	   pkgPayrAmntVo.setTpFamyExtpySum(BigDecimal.ZERO);  //가족수당 합 
                                	   pkgPayrAmntVo.setTpFamyFreeExtpySum(BigDecimal.ZERO);            //가족수당 비과세합계  
                                	   pkgPayrAmntVo.setTpSchlExpnAdmclExtpySum(BigDecimal.ZERO);//학비보조금함계 
                                	   pkgPayrAmntVo.setTpStdPymtTotAmnt(BigDecimal.ZERO);   //기본
                                	   pkgPayrAmntVo.setTpPymtTotAmnt(BigDecimal.ZERO); //pymtTotAmnt 지급총액 
                                	   pkgPayrAmntVo.setTpExtpyTotAmnt(BigDecimal.ZERO);  //extpyTotAmnt 수당총액 
                                           // pkgPayrAmntVo.setTpTxtnTotAmnt(BigDecimal.ZERO);//txtnTotAmnt 과세총액  
                                	   pkgPayrAmntVo.setTpFreeDtyTotAmnt(BigDecimal.ZERO); //freeDtyTotAmnt 비과세총액  
                                	   pkgPayrAmntVo.setTpDducTotAmnt(BigDecimal.ZERO); //dducTotAmnt  공제총액 
                                           ...
                                           pkgPayrAmntVo.setChkMonth("N");//월 출력체크 
                                        ...
                                        if ((payCalculationVo.getExPayr0490() != null)  && (!payCalculationVo.getExPayr0490().isEmpty())) { 
  		                            	  ...
  		                            	  /***************************************************************************************************
  		                                   * 급여대상자 마스터 테이블 Payr0304 인서트 업데이트  
  		                                   ***************************************************************************************************/ 
  		                                  payr0304Vo = mtdPayrCommCalcProc.call_funcPayr0304(request,payr0250Vo,payCalculationVo,sessionUser);  
  		                              	  mtdPayrCommCalcProc.call_funcPayr0500AddIncmTx(request,payCalculationVo,payr0304Vo,sessionUser); 
  		                              
  		                                  bmResult.add(new ShowMessageBM(PropPayGenConfs.PAYGEN_INFO,"Payr","[기간제근로자]월급여내역인서" ,"월급여내역저장완료","대상자 : " + payCalculationVo.getHanNm())); 
  		                                  /**@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  		                                   *  월급여내역개인 인서트  종료
  		                                   @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
                                            ...
                                            if (ExtermsProps.getProps("PAY_CD_01").equals(payr0250Vo.getPayCd())) {
                                            	 ...
                                            	 /**+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                                           * @@@@@@@@@@@@@@@@@@@@@@ 월급여 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ 
		                                   	    ...
		                                   	    PayrA002002001_B0150000CalcProc callPayrB015000Calc = new PayrA002002001_B0150000CalcProc();
		                                     	callPayrB015000Calc.callProcMethedPayrA002002001_B0150000(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo);
        ...
        //                                      payr0301Vo.setRevnAddr(payr0250Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
                                                ...
                                                while (iterpayr0530Set.hasNext() ) {
                                                           ...
                                                           if ("Y".equals(payr0530.getFamyAllwPymtYn())) {
                                                        	   if(("B0260030".equals(payr0530.getFamyExtpyDivCd())) && (!PayrServerUtilsGS.getAgeYn(MSFSharedUtils.allowNulls(payr0530.getPayFamyResnRegnNum()).replaceAll("-", ""))) && ("N".equals(payr0306Vo.getDabpnDducYn()))){
                                                           		payr0306Vo.setFamyExtpySum(new BigDecimal("0"));
                                                           	}
                                                           	else if(("B0260045".equals(payr0530.getFamyExtpyDivCd())) && (!PayrServerUtilsGS.getAgeYn(MSFSharedUtils.allowNulls(payr0530.getPayFamyResnRegnNum()).replaceAll("-", ""))) && ("N".equals(payr0306Vo.getDabpnDducYn()))){
                                                           		payr0306Vo.setFamyExtpySum(new BigDecimal("0"));
                                                           	}
                                                           	else if(("B0260060".equals(payr0530.getFamyExtpyDivCd())) && (!PayrServerUtilsGS.getAgeYn(MSFSharedUtils.allowNulls(payr0530.getPayFamyResnRegnNum()).replaceAll("-", ""))) && ("N".equals(payr0306Vo.getDabpnDducYn()))){
                                                           		payr0306Vo.setFamyExtpySum(new BigDecimal("0"));
                                                           	}
                                                           	else{
...
//		                                                           }
                                                           ...
                                                           if (ExtermsProps.getProps("PYMT_ITEM_01").equals(payr0490.getPayItemCd()) && (pkgPayrAmntVo.getTpFamyExtpySum().compareTo(BigDecimal.ZERO) > 0)) {
                                                               ...
                                                               payr0302Vo.setPayItemSeilNum(BigDecimal.ZERO);    /** column 급여항목일련번호 : payItemSeilNum */ 
                                                               payr0302Vo.setPymtDducDivCd(payr0490.getPymtDducDivCd());    /** column 지급공제구분코드 : pymtDducDivCd */ 
                                                               payr0302Vo.setPymtDducFreeDtySum(payr0490.getFreeDtySum().add(pkgPayrAmntVo.getTpFamyFreeExtpySum()));    /** column 지급공제비과세금액 : pymtDducFreeDtySum */
                                                               payr0302Vo.setPymtDducSum(pkgPayrAmntVo.getTpFamyExtpySum());   /** column 지급공제금액 : pymtDducSum */
                                                               payr0302Vo.setPayPymtDducPrcsFlag("N");   /** column 급여지급공제처리플래그 : payPymtDducPrcsFlag */
                                                               payr0302Vo.setPayReattyAdmntVal("0");    /** column 급여지급공제소급조정값 : payReattyAdmntVal */
                                                               payr0302Vo.setPymtDducTxtnAmnt(pkgPayrAmntVo.getTpFamyExtpySum().subtract(payr0490.getFreeDtySum().add(pkgPayrAmntVo.getTpFamyFreeExtpySum())));    /** column 지급공제과세금액 : pymtDducTxtnAmnt */ 
        //                                                       payr0302Vo.setKybdr(payr0302Vo.getKybdr());    /** column 입력자 : kybdr */
        //                                                       payr0302Vo.setInptDt(payr0302Vo.getInptDt());    /** column 입력일자 : inptDt */
        //                                                       payr0302Vo.setInptAddr(payr0302Vo.getInptAddr());    /** column 입력주소 : inptAddr */
        //                                                       payr0302Vo.setIsmt(payr0302Vo.getIsmt());    /** column 수정자 : ismt */
        ...
        //                                                       payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */  
                                                               ...
                                                               pkgPayrAmntVo.setTpPymtTotAmnt(pkgPayrAmntVo.getTpPymtTotAmnt().add(pkgPayrAmntVo.getTpFamyExtpySum()));
                                                               pkgPayrAmntVo.setTpExtpyTotAmnt(pkgPayrAmntVo.getTpExtpyTotAmnt().add(pkgPayrAmntVo.getTpFamyExtpySum()));
                                                               pkgPayrAmntVo.setTpFreeDtyTotAmnt(pkgPayrAmntVo.getTpFreeDtyTotAmnt().add(payr0490.getFreeDtySum().add(pkgPayrAmntVo.getTpFamyFreeExtpySum())));
                                                           ...
                                                           } else if (ExtermsProps.getProps("PYMT_ITEM_02").equals(payr0490.getPayItemCd()) && (pkgPayrAmntVo.getTpSchlExpnAdmclExtpySum().compareTo(BigDecimal.ZERO) > 0)) {
                                                               //학비보조수당 C9060100
                                                               ...
                                                               payr0302Vo.setPayPymtDducPrcsFlag("N");   /** column 급여지급공제처리플래그 : payPymtDducPrcsFlag */
                                                               payr0302Vo.setPayReattyAdmntVal("0");    /** column 급여지급공제소급조정값 : payReattyAdmntVal */
                                                               payr0302Vo.setPymtDducTxtnAmnt(pkgPayrAmntVo.getTpSchlExpnAdmclExtpySum().subtract(payr0490.getFreeDtySum()));    /** column 지급공제과세금액 : pymtDducTxtnAmnt */ 
        //                                                       payr0302Vo.setKybdr(payr0302Vo.getKybdr());    /** column 입력자 : kybdr */
        //                                                       payr0302Vo.setInptDt(payr0302Vo.getInptDt());    /** column 입력일자 : inptDt */
        //                                                       payr0302Vo.setInptAddr(payr0302Vo.getInptAddr());    /** column 입력주소 : inptAddr */
        //                                                       payr0302Vo.setIsmt(payr0302Vo.getIsmt());    /** column 수정자 : ismt */
        ...
        //                                                       payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                               ...
                                                               if ("B0050002".equals(payr0490.getTxtnDivCd())) {
                                                                   ...
                                                                   payr0302Vo.setPymtDducFreeDtySum(payr0490.getFreeDtySum());    /** column 지급공제비과세금액 : pymtDducFreeDtySum */
                                                                   payr0302Vo.setPymtDducTxtnAmnt(pkgPayrAmntVo.getTpSchlExpnAdmclExtpySum().subtract(payr0490.getFreeDtySum()));    /** column 지급공제과세금액 : pymtDducTxtnAmnt */ 
                                                                   pkgPayrAmntVo.setTpFreeDtyTotAmnt(pkgPayrAmntVo.getTpFreeDtyTotAmnt().add(payr0490.getFreeDtySum()));
                                                               } else {
                                                            	   payr0302Vo.setPymtDducFreeDtySum(BigDecimal.ZERO);    /** column 지급공제비과세금액 : pymtDducFreeDtySum */
                                                                   payr0302Vo.setPymtDducTxtnAmnt(pkgPayrAmntVo.getTpSchlExpnAdmclExtpySum());    /** column 지급공제과세금액 : pymtDducTxtnAmnt */ 
                                                                   pkgPayrAmntVo.setTpFreeDtyTotAmnt(BigDecimal.ZERO);
...
//		                                                                tpFreeDtyTotAmnt = tpFreeDtyTotAmnt.add(payr0490.getFreeDtySum());
                                                               ...
                                                               if (ExtermsProps.getProps("PAY_EMYMT_PYMT_19").equals(payr0490.getPayItemCd()) && (ExtermsProps.getProps("PAY_EMYMT_DYMNTH_01").equals(payr0490.getDayMnthAmntDivCd())) 
                                                                   ...
                                                                   pkgPayrAmntVo.setTpStdPymtTotAmnt(pkgPayrAmntVo.getTpStdPymtTotAmnt().add(tpEmymtAmnt));
...
//		                                                                  } 
        ...
        //                                                       payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                               ...
                                                               pkgPayrAmntVo.setTpPymtTotAmnt(pkgPayrAmntVo.getTpPymtTotAmnt().add(tpEmymtAmnt)); 
                                                               pkgPayrAmntVo.setTpFreeDtyTotAmnt(pkgPayrAmntVo.getTpFreeDtyTotAmnt().add(payr0490.getFreeDtySum()));
                                                                
                                                                if (!ExtermsProps.getProps("PYMT_DDUC_DIV_CD_01").equals(payr0490.getPymtDducDivCd())) {
                                                                	pkgPayrAmntVo.setTpExtpyTotAmnt(pkgPayrAmntVo.getTpExtpyTotAmnt().add(tpEmymtAmnt));
                                                                }
...
//                                        //	public static final String PAY_DDUC_D02201 = "D0220100";   // 	D0220100	B0080030	B0250260	환급소득세
		...
		//    			                                        	 //	public static final String PAY_DDUC_D02201 = "D0220100";   // 	D0220100	B0080030	B0250260	환급소득세
		            ...
		            //                                                payr0302Vo.setRevnAddr(payr0307Dto.getRevnAddr());    /** column 수정주소 : revnAddr */
		                                                            payrCommCalcService.insertPayr0302(payr0302Vo); 
		                                                            pkgPayrAmntVo.setTpDducTotAmnt(pkgPayrAmntVo.getTpDducTotAmnt().add(subtEtax)); //공제합계 
		...
		//    			                                        	//	public static final String PAY_DDUC_D02301 = "D0230100";   // 	D0230100	B0080030	B0250270	환급주민세
		    	        ...
		    	        //                                                payr0302Vo.setRevnAddr(payr0307Dto.getRevnAddr());    /** column 수정주소 : revnAddr */
		    	                                                    payrCommCalcService.insertPayr0302(payr0302Vo); 
		    	                                                    pkgPayrAmntVo.setTpDducTotAmnt(pkgPayrAmntVo.getTpDducTotAmnt().add(subtInhb)); //공제합계 
...
//		                                                 payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
	        ...
	        //                                     payr0301Vo.setRevnAddr(payr0250Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
              ...
              //                                                   payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                                 ...
                                                                 pkgPayrAmntVo.setTpPymtTotAmnt(pkgPayrAmntVo.getTpPymtTotAmnt().add(payr0490.getPymtDducSum().setScale(-1,BigDecimal.ROUND_DOWN)));
                                                                 pkgPayrAmntVo.setTpExtpyTotAmnt(pkgPayrAmntVo.getTpExtpyTotAmnt().add(payr0490.getPymtDducSum().setScale(-1,BigDecimal.ROUND_DOWN)));
                                                                 pkgPayrAmntVo.setTpFreeDtyTotAmnt(pkgPayrAmntVo.getTpFreeDtyTotAmnt().add(payr0490.getFreeDtySum().setScale(-1,BigDecimal.ROUND_DOWN))); 
              ...
              //                                  payr0301Vo.setRevnAddr(payr0250Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
                                                 ...
                                                 payrCommCalcService.updatePayr0301(payr0301Vo); //명절 휴가비 
  	 ...
  	 private List<ShowMessageBM>  procEmymtDivCd01(HttpServletRequest request,InfcPkgPayr0250VO dtoPayr0250,InfcPkgPsnl0100VO voPsnl0100,boolean boolInsert)  throws Exception {
	  	...
	  	//	  PagingLoadResult<ShowMessageBM> retval = null; 
	         ...
	         //  InfcPkgPayr0520SrhVO payr0520SrhVo = new InfcPkgPayr0520SrhVO();
	           ...
	           //가족수당 합
	           pkgPayrAmntVo.setTpFamyExtpySum(BigDecimal.ZERO); 
	           pkgPayrAmntVo.setTpFamyFreeExtpySum(BigDecimal.ZERO); //비과세 함계 
	           //학비보조금함계
	           pkgPayrAmntVo.setTpSchlExpnAdmclExtpySum(BigDecimal.ZERO); 
	           pkgPayrAmntVo.setTpStdPymtTotAmnt(BigDecimal.ZERO);   //기본
	           //pymtTotAmnt 지급총액
	           pkgPayrAmntVo.setTpPymtTotAmnt(BigDecimal.ZERO);
	           //extpyTotAmnt 수당총액
	           pkgPayrAmntVo.setTpExtpyTotAmnt(BigDecimal.ZERO);
	           //txtnTotAmnt 과세총액 
	           pkgPayrAmntVo.setTpTxtnTotAmnt(BigDecimal.ZERO);
	           //freeDtyTotAmnt 비과세총액 
	           pkgPayrAmntVo.setTpFreeDtyTotAmnt(BigDecimal.ZERO);
	           //dducTotAmnt  공제총액 
	           pkgPayrAmntVo.setTpDducTotAmnt(BigDecimal.ZERO);
	            //기본급정보 
	           pkgPayrAmntVo.setBasePymtAmnt(BigDecimal.ZERO);
	            ...
	            //과세합계
	            pkgPayrAmntVo.setTxTotAmnt(BigDecimal.ZERO);
	         ...
	         try {
			             ...
			             //사회보험요율가져오기 
//			             List insureList = pkgInsrCtl.getSelect4InsureDataList(request,payr0250Vo);  
//			             
		                                   ...
		                                   *  월급여내역개인 인서트  종료
		                                   @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
		                                    ...
		                                    if (ExtermsProps.getProps("PAY_CD_01").equals(payr0250Vo.getPayCd())) {
		                                   	   ...
		                                   	   PayrA002001001_B0150000CalcProc callPayrB015000Calc = new PayrA002001001_B0150000CalcProc();
		                                   	   callPayrB015000Calc.callProcMethedPayrA0020010_B0150000(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo);
			                                	   ...
			                                	   PayrA002001001_B0150001CalcProc callPayrB015001Calc = new PayrA002001001_B0150001CalcProc();
			                                	   callPayrB015001Calc.callProcMethedPayrA0020010_B0150001(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo);
		                                         ...
		                                         */  
		                                  	   ...
		                                  	   PayrA002001001_B0150002CalcProc callPayrB015002Calc = new PayrA002001001_B0150002CalcProc();
		                                  	   callPayrB015002Calc.callProcMethedPayrA0020010_B0150002(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
			                                        ...
			                                        */  
			                                        //C9110100  명절휴가비
			                                  	  PayrA002001001_B0150003CalcProc callPayrB015003Calc = new PayrA002001001_B0150003CalcProc();
			                                    	   callPayrB015003Calc.callProcMethedPayrA0020010_B0150003(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                         ...
		                                         */  
		                                   	   ...
		                                   	   PayrA002001001_B0150004CalcProc callPayrB015004Calc = new PayrA002001001_B0150004CalcProc();
		                                    	   callPayrB015004Calc.callProcMethedPayrA0020010_B0150004(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                          ...
		                                          */
		                                    	  ...
		                                    	  PayrA002001001_B0150005CalcProc callPayrB015005Calc = new PayrA002001001_B0150005CalcProc();
		                                    	  callPayrB015005Calc.callProcMethedPayrA0020010_B0150005(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                         ...
		                                         * @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		                                         */
		                                   	  PayrA002001001_B0150006CalcProc callPayrB015006Calc = new PayrA002001001_B0150006CalcProc();
		                                   	  callPayrB015006Calc.callProcMethedPayrA0020010_B0150006(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                   	   
		                                    }   
		                                    bmResult.add(new ShowMessageBM(PropPayGenConfs.PAYGEN_INFO,"Payr","[무기계약직]월급여" ,"처리완료","대상자 : " + payCalculationVo.getHanNm() + "명처리완료 "));
		                              
		                              } else if ((payCalculationVo.getExPayr0417() != null)  && (!payCalculationVo.getExPayr0417().isEmpty())) {
		                            	  /**************************************************************************************************
		                            	   * 일용직 형태의 공무직 정액제 급여처리 루틴 처리 부 
		                            	   **************************************************************************************************/
		                                   ...
		                                   *  월급여내역개인 인서트  종료
		                                   @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
		                                    ...
		                                    if (ExtermsProps.getProps("PAY_CD_01").equals(payr0250Vo.getPayCd())) {
		                                 	   ...
		                                 	   PayrA002001002_B0150000CalcProc callPayrB01500002Calc = new PayrA002001002_B0150000CalcProc();
		                                 	   callPayrB01500002Calc.callProcMethedPayrA0020010_B0150000(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo);
		                                 	   ...
		                                 	   PayrA002001002_B0150001CalcProc callPayrB015001Calc = new PayrA002001002_B0150001CalcProc();
		                                 	   callPayrB015001Calc.callProcMethedPayrA0020010_B0150001(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo);
		                                         ...
		                                         */ 
		                                  	   ...
		                                  	   PayrA002001002_B0150002CalcProc callPayrB01500202Calc = new PayrA002001002_B0150002CalcProc();
		                                  	   callPayrB01500202Calc.callProcMethedPayrA0020010_B0150002(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                         ...
		                                         */  
		                                         //C9110100  명절휴가비
		                                   	  PayrA002001002_B0150003CalcProc callPayrB01500302Calc = new PayrA002001002_B0150003CalcProc();
		                                     	   callPayrB01500302Calc.callProcMethedPayrA0020010_B0150003(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                         ...
		                                         */  
		                                   	   ...
		                                   	   PayrA002001002_B0150004CalcProc callPayrB01500402Calc = new PayrA002001002_B0150004CalcProc();
		                                    	   callPayrB01500402Calc.callProcMethedPayrA0020010_B0150004(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                   	  
		                                    } else if (ExtermsProps.getProps("PAY_CD_06").equals(payr0250Vo.getPayCd())) {
		                                    	
		                                    	 /**  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		                                          * @@@@@@@@@@@@@@@@@@@@@@ 연가보상비 C9410600 연월차 c9410400  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		                                          * @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		                                          */
		                                    	  
		                                    	  PayrA002001002_B0150005CalcProc callPayrB01500502Calc = new PayrA002001002_B0150005CalcProc();
		                                    	  callPayrB01500502Calc.callProcMethedPayrA0020010_B0150005(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
		                                         ...
		                                         * @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		                                         */
		                                   	  PayrA002001002_B0150006CalcProc callPayrB01500602Calc = new PayrA002001002_B0150006CalcProc();
		                                   	  callPayrB01500602Calc.callProcMethedPayrA0020010_B0150006(request,payr0250Vo,payCalculationVo,pkgPayrAmntVo,pkgInsrWorkVo); 
