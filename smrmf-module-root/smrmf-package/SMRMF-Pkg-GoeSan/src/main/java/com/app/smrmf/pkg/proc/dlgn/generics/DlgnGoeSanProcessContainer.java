...
public class DlgnGoeSanProcessContainer {
	

	private static final Logger logger = LoggerFactory.getLogger(DlgnGoeSanProcessContainer.class);
	   
    @Resource(name = "ManageCommuteService")
    protected ManageCommuteService manageCommuteService;

    public DlgnGoeSanProcessContainer() {
    ...
    MSFSysm0100BM sessionUser = MSFServerUtils.getLoggedUser(RemoteServiceUtil.getThreadLocalRequest());
        ...
        try {
            ...
            while ( iter.hasNext() ) {
                ...
                BigDecimal fndtnTm = BigDecimal.ZERO;
                BigDecimal totTmRstDutyTm = BigDecimal.ZERO;
...
//                upInfcDlgn0200Vo.setDilnlazExceDutyEndDt(infcDlgn0200Vo.getDilnlazExceDutyEndDt());    /** column 근태_초과근무종료일자 : dilnlazExceDutyEndDt */
                ...
                if (ExtermsProps.getProps("EMYMT_DIVCD_02").equals(infcDlgn0200Vo.getEmymtDivCd())) {
                    ...
                    /******************** 휴일근무 일수 괴산군청은 무조건 전 공무직은 적용 함 **********************************************************/ 
                      ...
                      /******************** 기본시간외근무시간 괴산군청은 무조건 전 공무직은 적용 함 **********************************************************/ 
                      fndtnTm =(new BigDecimal(MSFSharedUtils.defaultNulls(mapInfcDlgn0200Tot.get("dilnlazFndtnTmRstDutyTm"),"0"))); 
                      /******************** 기본시간외근무시간 괴산군청은 무조건 전 공무직은 적용 함  20시간 무조건 적용해서 처리 **********************************************************/   
                      
                } else {
                	 /******************** 휴일근무 일수 괴산군청은 무조건 전 공무직은 적용 함 **********************************************************/ 
                      hodyYnTm =(new BigDecimal(MSFSharedUtils.defaultNulls(mapInfcDlgn0200Tot.get("dilnlazHodyYnTm"),"0"))).add(new BigDecimal("1")); 
                    
                      /******************** 휴일근무 일수 괴산군청은 무조건 전 공무직은 적용 함 **********************************************************/ 
                      hodyYnTT =(new BigDecimal(MSFSharedUtils.defaultNulls(mapInfcDlgn0200Tot.get("dilnlazPubcHodyDutyTm"),"0"))).add(new BigDecimal("8")); 
               
                      /******************** 기본시간외근무시간 괴산군청은 무조건 전 공무직은 적용 함 **********************************************************/ 
                      fndtnTm =(new BigDecimal(MSFSharedUtils.defaultNulls(mapInfcDlgn0200Tot.get("dilnlazFndtnTmRstDutyTm"),"0"))).add(new BigDecimal("20")); 
                      /******************** 기본시간외근무시간 괴산군청은 무조건 전 공무직은 적용 함  20시간 무조건 적용해서 처리 **********************************************************/ 
...
//                upInfcDlgn0200Vo.setDilnlazFndtnTmRstDutyTm("0"); /** column 근태_기본시간외근무시간 : dilnlazFndtnTmRstDutyTm */
                upInfcDlgn0200Vo.setDilnlazFndtnTmRstDutyTm(MSFSharedUtils.defaultNulls(String.valueOf(fndtnTm),"0")); 				/** column 근태_기본시간외근무시간 : dilnlazFndtnTmRstDutyTm */
...
//                upInfcDlgn0200Vo.setDilnlazTotTmRstDutyTm(MSFSharedUtils.allowNulls(mapInfcDlgn0200Tot.get("dilnlazTmRstDutyTm")));   /** column 근태_총시간외근무시간 : dilnlazTotTmRstDutyTm */
                totTmRstDutyTm = (new BigDecimal(MSFSharedUtils.defaultNulls(mapInfcDlgn0200Tot.get("dilnlazTmRstDutyTm"),"0"))).add(fndtnTm); 
                upInfcDlgn0200Vo.setDilnlazTotTmRstDutyTm(MSFSharedUtils.defaultNulls(String.valueOf(totTmRstDutyTm),"0"));   			/** column 근태_총시간외근무시간 : dilnlazTotTmRstDutyTm */
...
//                upInfcDlgn0200Vo.setDilnlazExceDutyEndDt(infcDlgn0200Vo.getDilnlazExceDutyEndDt());    /** column 근태_초과근무종료일자 : dilnlazExceDutyEndDt */
                ...
                if (ExtermsProps.getProps("EMYMT_DIVCD_02").equals(infcDlgn0200Vo.getEmymtDivCd())) {
                   ...
                   hodyYnTm =(new BigDecimal(MSFSharedUtils.defaultNulls(mapInfcDlgn0200Tot.get("dilnlazHodyYnTm"),"0"))).add(new BigDecimal("1")); 
                   /******************** 휴일근무 일수 괴산군청은 무조건 전 공무직은 적용 함  1일 무조건 적용해서 처리 **********************************************************/ 
                   hodyYnTT =(new BigDecimal(MSFSharedUtils.defaultNulls(mapInfcDlgn0200Tot.get("dilnlazPubcHodyDutyTm"),"0"))).add(new BigDecimal("480")); 
