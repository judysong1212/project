package com.app.smrmf.pkg.proc.payr.goesan.a002001001;
                               ...
                               //   if ((new BigDecimal(payCalculationVo.getDilnlazAbnceNumDys())).compareTo(BigDecimal.ZERO) > 0) {
                                 if (abnceNumDysCnt.compareTo(BigDecimal.ZERO) > 0) {
                                     ...
                                     //  payCalculationVo.getDilnlazAbnceNumDys()   /** column 근태_결근일수 : dilnlazAbnceNumDys */
                                      abnceNumDysAmt = abnceNumDysCnt.multiply(payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal(ConstGoeSan.PAYR_BASS_WKTM_GH_01)));  // USALY_AMNT_HRWG_QNTY 시급통상임금 
                                  ...
                                  //결근은 가족수당을 제외하지 않음.
                                  	 ...
                                  	 if ((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0")).compareTo((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))) > 0)) {
                                  		  ...
                                  		  if (pkgPayrAmntVo.getTpFamyExtpySum().compareTo(BigDecimal.ZERO) > 0) { 
                                         		...
                                         		dayFmyPayAmt = (pkgPayrAmntVo.getTpFamyExtpySum().divide((new BigDecimal(payCalculationVo.getDilnlazDutyNumDys())),5, BigDecimal.ROUND_UP)) 
                                         				.multiply((new BigDecimal(payCalculationVo.getDilnlazTotDutyNumDys()))).setScale(-1, 1);
                                         				//.add(new BigDecimal(payCalculationVo.getDilnlazAbnceDutyRcgtnDys()))
                                         		...
                                         		if (pkgPayrAmntVo.getTpFamyFreeExtpySum().compareTo(BigDecimal.ZERO) > 0) { 
                                         			...
                                         			dayFreeFmyPayAmt = (pkgPayrAmntVo.getTpFamyFreeExtpySum().divide((new BigDecimal(payCalculationVo.getDilnlazDutyNumDys())),5, BigDecimal.ROUND_UP))
                                         					            .multiply((new BigDecimal(payCalculationVo.getDilnlazTotDutyNumDys()))).setScale(-1, 1);
                                         					         //   .add(new BigDecimal(payCalculationVo.getDilnlazAbnceDutyRcgtnDys()))
                                         					         //   .add((new BigDecimal(payCalculationVo.getDilnlazAbnceNumDys())))
...
//                                                       payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                          	   ...
                          	   /***********************************************************************/
                          	   /* 환경미화원    00200010A0060000' = AA.DTIL_OCCU_INTTN_CD   */
                          	   /************************************************************************/
                          	   //반장수당        
                                 //직책별 수당  
                                   if ("00200050A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) && "A0150003".equals(payCalculationVo.getOdtyCd())) {
...
//                                                   payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                  ...
                                  // InfcPkgPayr0302VO tpPayr0302vo = new InfcPkgPayr0302VO();
                                   ...
                                   if ("00200050A0060000".equals(payCalculationVo.getDtilOccuInttnCd())) {    
...
//환경미화원 
                                                  	  ...
                                                  	  // BigDecimal ucstSum =  (((payr0410.getMnthUsalyAmntWag().divide(new BigDecimal("209"), 5))).multiply(new BigDecimal("2"))).setScale(0,BigDecimal.ROUND_DOWN);
                                                          ucstSum = ((payr0410.getMnthUsalyAmntWag().divide(new BigDecimal("226"), 5)).multiply(new BigDecimal("2"))).setScale(5,BigDecimal.ROUND_DOWN);
                                                  	  ...
                                                  	  // BigDecimal ucstSum =  (((payr0410.getMnthUsalyAmntWag().divide(new BigDecimal("209"), 5))).multiply(new BigDecimal("2"))).setScale(0,BigDecimal.ROUND_DOWN);
                                                          ucstSum =  (payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum())).multiply(new BigDecimal("0.5"));
                                                        if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
                                                      	//  ucstSum = (((payr0410.getMnthUsalyAmntWag().divide(new BigDecimal("209"), 5)).multiply(new BigDecimal("2")))).setScale(0,BigDecimal.ROUND_DOWN);
                                                      	  ucstSum =  (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum()).multiply(new BigDecimal("0.5"));
                                                        } 
                                                      }
                                              ...
                                              // 휴일수당        -- 일수 C9410200
                                          	  ...
                                          	  //의료급여관리사 인경우 출장여비처리 일할 계산 처리로 일단 하여 최대 금액범위 안에서 준다. 
                                          	  if ("00400060A0060000".equals(payCalculationVo.getDtilOccuInttnCd())) {     
                                                    
                                          		  BigDecimal tmSumAmt = BigDecimal.ZERO;
                                          		  
                                          		  tmSumAmt = ((new BigDecimal(payCalculationVo.getDilnlazWkdDutyNumDys())).multiply(ucstSum)).setScale(-1, BigDecimal.ROUND_DOWN);  //출장여비 
                                          		  
                                          		  if ((new BigDecimal("260000")).compareTo(tmSumAmt) > 0) {
                                          			 
                                          			  totTmSum = tmSumAmt;
                                          			  
                                          		  } else {
                                          			  
                                          			  totTmSum = (new BigDecimal("260000"));
                                            ...
                                            } else  if (ExtermsProps.getProps("PAY_EMYMT_PYMT_48").equals(payr0410.getPayrImcd())) {
                                            ...
                                            } else  if (ExtermsProps.getProps("PAY_EMYMT_PYMT_41").equals(payr0410.getPayrImcd())) {
