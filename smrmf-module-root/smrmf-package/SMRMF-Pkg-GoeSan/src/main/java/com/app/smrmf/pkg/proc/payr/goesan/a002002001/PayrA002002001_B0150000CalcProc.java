package com.app.smrmf.pkg.proc.payr.goesan.a002002001;
 ...
 */
@Controller 
public class PayrA002002001_B0150000CalcProc  extends AbstractCustomServiceImpl {
	 

	private static final Logger logger = LoggerFactory.getLogger(PayrA002002001_B0150000CalcProc.class);
   ...
   protected PayrCommCalcService payrCommCalcService;
     ...
     * 생성자 
     */
     public PayrA002002001_B0150000CalcProc () { 
        
     }
                 
 	public void callProcMethedPayrA002002001_B0150000(HttpServletRequest request, InfcPkgPayr0250VO payr0250Vo,
      ...
      try {   
					...
					//                                      payr0301Vo.setRevnAddr(payr0250Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
					       ...
					       /*########################################################################################*/     
					         ...
					         *  급여 항목에 따른 인서트 및 계산 처리 시작 
					          @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
					         /*****************************  반장수당 처리 를 위해 추가 된 부분  **************************************/
					         //기본급가져오기위해 추가된 부분 처리 해야함. 
					            Set<InfcPkgPayr0490VO> payr0490Base = payCalculationVo.getExPayr0490();
					            Iterator<InfcPkgPayr0490VO> iterpayr0490Base = payr0490Base!=null ? payr0490Base.iterator()  : null;
					            
					            while ( iterpayr0490Base.hasNext() ) { 
					                
					         	     InfcPkgPayr0490VO payr0490Be = new InfcPkgPayr0490VO();
					         	     payr0490Be = iterpayr0490Base.next(); 
					                  
					         	        
					         	    	 //반장수당인경우 
					                  if (ExtermsProps.getProps("PYMT_ITEM_10").equals(payr0490Be.getPayItemCd())) {
					                 	 //주휴수당 계산을 위한 처리 부분 
					                 	 if ("446011500".equals(payCalculationVo.getCurrAffnDeptCd()) && "A0150003".equals(payCalculationVo.getOdtyCd())) {
					                      	// logger.debug("값이 넘어오는가 :" + payCalculationVo.getHanNm() + ":" +payr0410Be.getUcstSum() );
					                 		 pymtA0150003  = payr0490Be.getPymtDducSum();
					                 	 }	 
					                  } 
					            }
					...
					//                                 tpFreeDtyTotAmnt = tpFreeDtyTotAmnt.add(payr0490.getFreeDtySum());
					                    ...
					                    } else if (ExtermsProps.getProps("PYMT_ITEM_10").equals(payr0490.getPayItemCd())) {
					                 	   /***********************************************************************/
					                 	   /* 반장수당 처리  임시 TODO 확인 처리 해야 함   446011500 산림녹지과 임시 처리  */
					                 	   /************************************************************************/
					                 	   //반장수당        
					                        //직책별 수당  
					                          if ("446011500".equals(payCalculationVo.getCurrAffnDeptCd()) && "A0150003".equals(payCalculationVo.getOdtyCd())) {
					                         	 
					                         	 BigDecimal pymtDdumSum = BigDecimal.ZERO;
					                         	 BigDecimal pymtSum = BigDecimal.ZERO;
					                              ...
					                              //실 근무일수 
					                             if (new BigDecimal(payCalculationVo.getDilnlazTotDutyNumDys()).compareTo(BigDecimal.ZERO) > 0) {
					                             	
					                                 	//반장수당 단가  
					                              	 pymtDdumSum  = payr0490.getPymtDducSum();
					                             	 
					                              	 pymtSum = (pymtDdumSum.multiply(new BigDecimal(payCalculationVo.getDilnlazTotDutyNumDys()))).setScale(-1,BigDecimal.ROUND_DOWN);
					                              	
					                             	 payr0302Vo.setPymtDducSum(pymtSum);   /** column 지급공제금액 : pymtDducSum */
					                             	 payr0302Vo.setPymtDducTxtnAmnt(pymtSum.subtract(payr0490.getFreeDtySum()).setScale(-1,BigDecimal.ROUND_DOWN) );    /** column 지급공제과세금액 : pymtDducTxtnAmnt */ 
					                                   
					                             	 
					                             	 pkgPayrAmntVo.setTpPymtTotAmnt(pkgPayrAmntVo.getTpPymtTotAmnt().add( pymtSum));
					                                  pkgPayrAmntVo.setTpExtpyTotAmnt(pkgPayrAmntVo.getTpExtpyTotAmnt().add(pymtSum));
					                      ...
					                      //  BigDecimal tpEmymtC9410700Amnt = BigDecimal.ZERO;
					                       ...
					                       // BigDecimal tpEmymtC9410500Amnt = BigDecimal.ZERO;
					                        // 주휴수당       --일수산정
					                        if (ExtermsProps.getProps("PAY_EMYMT_PYMT_08").equals(payr0490.getPayItemCd())) {
					                     	   
					                               if ((ExtermsProps.getProps("PAY_EMYMT_DYMNTH_01").equals(payr0490.getDayMnthAmntDivCd())) 
					                                ...
					                                } else  if ((ExtermsProps.getProps("PAY_EMYMT_DYMNTH_04").equals(payr0490.getDayMnthAmntDivCd())) 
					                                        && (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazWklyHldyNumDys(),"0")).compareTo(BigDecimal.ZERO) > 0)) {
					                              	 ...
					                              	 if ("446011500".equals(payCalculationVo.getCurrAffnDeptCd()) && "A0150003".equals(payCalculationVo.getOdtyCd())) {
					                                   	 BigDecimal pymtSum = BigDecimal.ZERO;
					                                   	 if (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazWklyHldyNumDys(),"0")).compareTo(BigDecimal.ZERO) > 0) {
					                                            
					                                   		 pymtSum = (pymtA0150003.multiply(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazWklyHldyNumDys(),"0")))).setScale(-1,BigDecimal.ROUND_DOWN);;
					                              		
					                                   		 tpEmymtAmnt = tpEmymtAmnt.add(pymtSum);
					                                   	 }	 
					                              	 }	 
					                     ...
					                     // 휴일수당        -- 일수  ExtermsProps.getProps("PAY_EMYMT_PYMT_21") = "C9410200";  
					                        if (ExtermsProps.getProps("PAY_EMYMT_PYMT_21").equals(payr0490.getPayItemCd())) { 
					                            	 ...
					                            	 //TODO 휴일수당 계산을 위한 처리 부분 
					                              	 if ("446011500".equals(payCalculationVo.getCurrAffnDeptCd()) && "A0150003".equals(payCalculationVo.getOdtyCd())) {
					                                   	 ...
					                                   	 if (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazPubcHodyDutyNumDys(),"0")).compareTo(BigDecimal.ZERO) > 0) {
					                                   		 ...
					                                   		 pymtSum = ((pymtA0150003.multiply((new BigDecimal("1.5")))).multiply(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazPubcHodyDutyNumDys(),"0")))).setScale(-1,BigDecimal.ROUND_DOWN);;
					...
					//                                   } 
					                     ...
					                     // BigDecimal tpEmymtC9410300Amnt = BigDecimal.ZERO; 
					                        ...
					                        } else  if (ExtermsProps.getProps("PAY_EMYMT_PYMT_32").equals(payr0490.getPayItemCd())) {
					                         	//public static final String PAY_EMYMT_PYMT_32 = "C9310300";    //		간식비
					                                 
					                             BigDecimal ucstSum =  payr0490.getPymtDducSum();
					                             BigDecimal totTmSum = BigDecimal.ZERO;
					                             
					                             // 15일이상 근무 여부에 따른 작업 
					                             //if ((new BigDecimal(payCalculationVo.getDilnlazTotDutyNumDys())).compareTo((new BigDecimal("15"))) >= 0 ) {
					                             	totTmSum =  ucstSum.multiply((new BigDecimal(payCalculationVo.getDilnlazTotDutyNumDys()))).setScale(-1,BigDecimal.ROUND_DOWN);
					                           //  } else {
					                        ...
					                        } else  if (ExtermsProps.getProps("PAY_EMYMT_PYMT_48").equals(payr0490.getPayItemCd())) {
                                                // PAY_EMYMT_PYMT_48 = "C9430100";    // 의정활동비  
                                            ...
                                            } else  if (ExtermsProps.getProps("PAY_EMYMT_PYMT_41").equals(payr0490.getPayItemCd())) {
                                                // PAY_EMYMT_PYMT_41 = "C9440100";    // 월정수당   
                                       	    	BigDecimal ucstSum =  BigDecimal.ZERO;
                                       	    	BigDecimal totTmSum = BigDecimal.ZERO; 
