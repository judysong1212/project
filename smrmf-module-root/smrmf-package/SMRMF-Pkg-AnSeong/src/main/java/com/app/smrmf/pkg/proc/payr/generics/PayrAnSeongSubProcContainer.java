 ...
 */
@Controller 
public class PayrAnSeongSubProcContainer  extends AbstractCustomServiceImpl {
	

	private static final Logger logger = LoggerFactory.getLogger(PayrAnSeongSubProcContainer.class);
...
//	 	        	                  payr0302Vo.setRevnDt(payr0302Vo.getRevnDt());    /** column 수정일자 : revnDt */
                       ...
                       // 급여구분의 월급여가 아닐 때	
                           ...
                           BigDecimal tpDducAmntsumB001 = BigDecimal.ZERO;  //공제금액  
                           BigDecimal tpDducAmntsumD9500400 = BigDecimal.ZERO;  //기본금액  
                          ...
                          //ucstSumD9500400 노조회비 공제 대상금액UCST_SUM_D9500400
                           ...
                           tpDducAmntsumD9500400 = new BigDecimal(String.valueOf(tempMap.get("sumD9500400"))); //이전공제 금액  
                           ...
                           if (MSFSharedUtils.paramNotNull(payr0410)) {
	                           if (PayrPkgUtil.funcMonthCheck(payr0250Vo, payr0410, null)) {  
                                   ...
                                   //노조회비_급여조정시_계산식수정_20211028_ssh
                                    
                                   /*logger.debug("노조회비4-1 = " + dducSum); //0
*/                                   //바꿔야할부분
                                   //필요한것 = 기본금,정액급식비,위험수당,조정수당
                                   //tpDducAmntsumB001 = 기본급
                                   //
                                   ...
                                   /*logger.debug("노조회비1 = " + dducSumRate); //노조회비 요율
                                   logger.debug("노조회비2 = " + ucstSumD9500400); //수당합계
                                   logger.debug("노조회비3 = " + tpDducAmntsumB001); //기본급
                                   logger.debug("노조회비4-2 = " + dducSum); //최종노조회비
*/                                   
                                  ...
                                  if ( payr0410.getPymtDducRate().compareTo(BigDecimal.ZERO) > 0 && "Y".equals(payCalculationVo.getUnnMbspSttYn())) {
	                               	   if (BigDecimal.ZERO.compareTo(payr0302Vo.getPymtDducSum()) < 0)  {  
	                               		     ...
	                               		     tpPernSubPymtSum = (tpPernPymtSum.add(tpDducAmntsumD9500400)).subtract(dducSum);  
	                               		     
	                               		     
	                                         tpDducTotAmnt = (tpDducTotAmnt.subtract(tpDducAmntsumD9500400)).add(dducSum);
