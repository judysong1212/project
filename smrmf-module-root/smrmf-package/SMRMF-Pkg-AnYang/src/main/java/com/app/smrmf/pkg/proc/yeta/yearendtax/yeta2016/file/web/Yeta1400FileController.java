	   ...
	   /********************************************************암호화 처리 를 위해 반드시 해야 함................................*/
        ...
        InfcPkgYeta1000VO infcPkgYeta1000Vo = new InfcPkgYeta1000VO();
         ...
         A_Record = ("A" + "20" + yeta1000Vo.getTaxaCode() + String.format("%08d", Long.parseLong(StringUtils.replace(yeta1000Vo.getEdacPrdt(),"-","")))
                       ...
                       +  PkgFuncUtils.getRHanPad( PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName() ,38),40," ")  
                       +  PkgFuncUtils.getRHanPad( PkgFuncUtils.subStringBytes(yeta1000Vo.getCharDept() ,28),30," ") 
                       +  PkgFuncUtils.getRHanPad( PkgFuncUtils.subStringBytes(yeta1000Vo.getCharName(),28),30," ")    
...
//           Exit;
          ...
          mTempData01 = 0L ;
              ...
              B_RecStr = "B" + "20" + yeta1000Vo.getTaxaCode() +  String.format("%06d", B_Cnt)   +   String.format("%-10s", StringUtils.replace(yeta1000Vo.getBusiNumb(),"-",""))
                           + PkgFuncUtils.getRHanPad( PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName() ,38),40," ")  
                           + PkgFuncUtils.getRHanPad( PkgFuncUtils.subStringBytes(yeta1000Vo.getReprName() ,28),30," ")  
                           + RSNOSTR ;
           ...
           Map<String, Object> mapYetaC = (Map<String, Object>) iter.next();  
               ...
               //  oraQrySelect01.FieldByName('CITY_SEQN').AsString; Format('%.11d', [])
                          ...
                          // 주민번호(복호화)
                             ...
                             C_RecStr = C_RecStr   + PkgFuncUtils.getRHanPad( PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName() ,38),40," ") ;                                     //주현근무처-근무처명
...
//        TOT_Record.SaveToFile('C:\EOSDATA\' + DEFIN + leftStr(defBUNS_LINS,7) + '.' + rightStr(defBUNS_LINS,3) + '.denc');
        ...
        //  BufferedWriter bw = new BufferedWriter(fw);
          //TODO 확인 cp949  / Cp1252  CP949
          BufferedWriter bw = new BufferedWriter(new OutputStreamWriter( new FileOutputStream(fileDir), "CP949"));
    		...
    		if  (listYetaCA.size() > 0) { 
    		  ...
    		  //	for (int i = 0 ; i < listYeta1.size() ; i++) {
    				...
    				//	infcPkgYeta3140Vo = (InfcPkgYeta3140VO)listYeta2.get(j);
						...
						A_RecStr = A_RecStr + PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName() ,38),40," ") ; // 10 상호
 	 ...
 	 /********************************************************암호화 처리 를 위해 반드시 해야 함................................*/
	    // 1. 암호화 객체 생성
	     ...
	     /********************************************************암호화 처리 를 위해 반드시 해야 함................................*/   
 	        ...
 	        strTempDataA = strTempDataA  +  PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName(),38),40," "); //String.format("%-40s", yeta1000Vo.getJuriName()); //  Format('%-40s', [edPROF_RSNM.Text])   ; //법인명
 	        strTempDataA = strTempDataA  + PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getCharDept(),28),30," "); //String.format("%-30s", yeta1000Vo.getCharDept());  //Format('%-30s', [edSELF_DEPT.Text]) ;  //담당자부서
 	        strTempDataA = strTempDataA  + PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getCharName(),28),30," "); 
...
// 	      end;
           ...
           else
           {
	         ...
	         strTempDataB  = strTempDataB +  PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName(),38),40," ")  ;
...
//
// 	        strTempDataA = strTempDataA  + String.format("%-6s", MSFSharedUtils.defaultNulls(yeta1000Vo.getTaxaDnum(),""));   //Format('%-6s', [edTXOF_NUMB.Text]) ; //세무대리인번호(7)
// 	        strTempDataA = strTempDataA  + PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getHomeTxid(),18),20," "); // String.format("%-20s", yeta1000Vo.getHomeTxid()); //Format('%-20s', [edHOME_TXID.Text]) ;  //홈텍스아이디 
// 	        strTempDataA = strTempDataA  + String.format("%-4s", yeta1000Vo.getTaxaPrcd()) ;//Format('%-4s', [edTAXP_CODE.Text])  ; //세무프로그램코드
// 	        strTempDataA = strTempDataA  + String.format("%-10s", StringUtils.replace(yeta1000Vo.getBusiNumb(),"-","")) ; //Format('%-10s', [StringReplace(edBUNS_LINS.Text,'-','',[rfReplaceALL])]) ; //사업자등록번호
// 	        strTempDataA = strTempDataA  +  PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName(),38),40," "); //String.format("%-40s", yeta1000Vo.getJuriName()); //  Format('%-40s', [edPROF_RSNM.Text])   ; //법인명
// 	        strTempDataA = strTempDataA  + PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getCharDept(),28),30," "); //String.format("%-30s", yeta1000Vo.getCharDept());  //Format('%-30s', [edSELF_DEPT.Text]) ;  //담당자부서
...
// 	         strTempDataB   = strTempDataB  + String.format("%06d", B_Cnt);   //일련번호 
//	         strTempDataB  = strTempDataB +  String.format("%-10s", StringUtils.replace(yeta1000Vo.getBusiNumb(),"-",""));  //사업자등록번호 10
//	         strTempDataB  = strTempDataB +  PkgFuncUtils.getRHanPad(PkgFuncUtils.subStringBytes(yeta1000Vo.getJuriName(),38),40," ");   //상호(40)
