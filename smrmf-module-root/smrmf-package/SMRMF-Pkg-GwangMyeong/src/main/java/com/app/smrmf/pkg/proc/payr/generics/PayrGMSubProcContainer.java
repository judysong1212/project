 ...
 */
@Controller 
public class PayrGMSubProcContainer  extends AbstractCustomServiceImpl {
	
	private static final Logger logger = LoggerFactory.getLogger(PayrGMSubProcContainer.class);
	private static final String calledClass = PayrGMSubProcContainer.class.getName();  
	...
	private int funcPayr4200ToPayrCalc(HttpServletRequest request,InfcPkgPayr0302VO voPayr0302)  throws Exception  {
		...
		BigDecimal tpPymtTotAmnt = BigDecimal.ZERO;		// pymtTotAmnt 지급총액
		BigDecimal tpExtpyTotAmnt = BigDecimal.ZERO;	// extpyTotAmnt 수당총액
		// BigDecimal tpTxtnTotAmnt = BigDecimal.ZERO;	// txtnTotAmnt 과세총액
		BigDecimal tpFreeDtyTotAmnt = BigDecimal.ZERO;	// freeDtyTotAmnt 비과세총액
		BigDecimal tpDducTotAmnt = BigDecimal.ZERO;		// dducTotAmnt 공제총액
		...
		//
		/**
		 * WHERE DPOB_CD = ' ' AND PYMT_YR_MNTH = '201501' AND PAY_CD =
		 * 'B0150000' AND PAYR_SEIL_NUM = 6 AND SYSTEMKEY = '19910001'
		 */
		// 소득세/지방소득세 합계
...
//			if (ExtermsProps.getProps("PAY_CD_01").equals(voPayr0302.getPayCd()) || ExtermsProps.getProps("PAY_CD_03").equals(voPayr0302.getPayCd()) ) { 
        ...
        //                    }
				...
				tpPymtTotAmnt = new BigDecimal(String.valueOf(tempMap.get("tpPymtTotAmnt")));					// pymtTotAmnt 지급총액
				tpExtpyTotAmnt = new BigDecimal(String.valueOf(tempMap.get("tpExtpyTotAmnt")));					// extpyTotAmnt 수당총액
				pkgInsrWorkVo.setTpTxtnTotAmnt(new BigDecimal(String.valueOf(tempMap.get("tpTxtnTotAmnt"))));	// txtnTotAmnt 과세총액
				tpFreeDtyTotAmnt = new BigDecimal(String.valueOf(tempMap.get("tpFreeDtyTotAmnt")));				// freeDtyTotAmnt 비과세총액
				tpDducTotAmnt = new BigDecimal(String.valueOf(tempMap.get("tpDducTotAmnt")));					// dducTotAmnt 공제총액
