 ...
 */
@Controller 
public class PayrYongInProcessContainer  extends AbstractCustomServiceImpl {
	

	private static final Logger logger = LoggerFactory.getLogger(PayrYongInProcessContainer.class);
...
//                          }
                       ...
                       for (int icnt = 0; icnt < psnlList.size();icnt++) { 
                          //사람데이타 처리  
                           ...
                           if ("Y".equals(payCalculationVo.getPayRcptYn())) {
                                   ...
                                   bmResult.add(new ShowMessageBM(PropPayGenConfs.PAYGEN_INFO,"Payr","[무기계약직]대상자" ,"급여작업중","4대보험처리작업")); 
                                   /*************************************************************** 
                                    4대보험 이력 테이블 추가 
                                   ****************************************************************/
            ...
            //                                  payr0301Vo.setRevnAddr(payr0250Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
                                                System.out.println("systemkey : " + payCalculationVo.getSystemkey());
...
//                                                               }
            ...
            //                                                 payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                               ...
                                                               if (abnceNumDysCnt.compareTo(BigDecimal.ZERO) > 0) {
                                                                   ...
                                                                   if ((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0")).compareTo((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))) > 0)) {
                                                                        ...
                                                                        // dayPaySum = dayPayAmt.multiply((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")).add(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceDutyRcgtnDys(),"0")))));
                                                                         ...
                                                                         dayPaySum = (mothPayAmt.divide((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0"))),5, BigDecimal.ROUND_UP))
                                                                        		 ...
                                                                        		 .add(abnceNumDysCnt))).setScale(-1, 1);
                                                                        			...
                                                                        			//	 .add((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceNumDys(),"0"))))
                                                                   ...
                                                                   abnceNumDysAmt = abnceNumDysCnt.multiply(payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal(ExtermsProps.PAYR_BASS_WKTM_201401_01)));  // USALY_AMNT_HRWG_QNTY 시급통상임금 
                                                                	  ...
                                                                	  if ((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0")).compareTo((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))) > 0)) {
                                                                         ...
                                                                         // dayPaySum = dayPayAmt.multiply((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")).add(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceDutyRcgtnDys(),"0")))));
                                                                          ...
                                                                          dayPaySum = (mothPayAmt.divide((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0"))),5, BigDecimal.ROUND_UP))
                                                                        		  .multiply((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))).setScale(-1, 1);
                                                                        		 // .add(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceDutyRcgtnDys(),"0")))
                                                              ...
                                                              //  if ((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceNumDys(),"0"))).compareTo(BigDecimal.ZERO) > 0) {
                                                                if (abnceNumDysCnt.compareTo(BigDecimal.ZERO) > 0) {
                                                                //결근은 가족수당을 제외하지 않음..add((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceNumDys(),"0"))))
                                                                	  if ((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0")).compareTo((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))) > 0)) {
                                                                          	
                                                                		  if (tpFamyExtpySum.compareTo(BigDecimal.ZERO) > 0) { 
		                                                               			
		                                                               		dayFmyPayAmt = (tpFamyExtpySum.divide((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0"))),5, BigDecimal.ROUND_UP)) 
		                                                               				.multiply((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))).setScale(-1, 1);
		                                                               				//.add(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceDutyRcgtnDys(),"0")))
		                                                               		...
		                                                               		if (tpFamyFreeExtpySum.compareTo(BigDecimal.ZERO) > 0) { 
		                                                               			...
		                                                               			dayFreeFmyPayAmt = (tpFamyFreeExtpySum.divide((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0"))),5, BigDecimal.ROUND_UP))
		                                                               					            .multiply((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))).setScale(-1, 1);
		                                                               					         //   .add(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceDutyRcgtnDys(),"0")))
		                                                               					         //   .add((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceNumDys(),"0"))))
            ...
            //                                                       payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                        	   ...
                                                        	   /***********************************************************************/
                                                        	   /* 환경미화원   = AA.DTIL_OCCU_INTTN_CD   */
                                                        	   /************************************************************************/
                                                        	   //반장수당        
                                                               //직책별 수당  
                                                                 if ("00300010A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) && "A0150003".equals(payCalculationVo.getOdtyCd())
                                                                		 || "00300020A0060000".equals(payCalculationVo.getDtilOccuInttnCd())) {
	                                                                      ...
	                                                                      if (ExtermsProps.getProps("PAY_EMYMT_PYMT_07").equals(payr0410.getPayrImcd())) {
	                                                                          ...
	                                                                          if (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotTmRstDutyTm(),"0")).compareTo(BigDecimal.ZERO) > 0) {
	                                                                        	  ...
	                                                                        	  if ("00300010A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) && "A0150003".equals(payCalculationVo.getOdtyCd())
	                                                                        		     ...
	                                                                        		     ucstSum = payr0410.getBefUcstSum02().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum02() : (payr0410.getUcstSum02().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum02() : payr0410.getPymtDducSum());
		                                                                                
		                                                                                  if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
		                                                                                	  ucstSum = payr0410.getUcstSum02().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum02() : payr0410.getPymtDducSum();
		                                                                                  } 
	                                                                        	  } else {
	                                                                                    ucstSum = payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum());
                                                                                  ...
                                                                                  //결근은 정액급식비 을 제외 급여모자랄시  
                                                                                	 // if ((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0")).compareTo((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0"))).add((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceNumDys(),"0"))))) > 0)) {
                                                                                	  if ((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0")).compareTo((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))) > 0)) {
	                                                                                	   ...
	                                                                                	   dayPaySum = (ucstSum.divide((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazDutyNumDys(),"0"))),5, BigDecimal.ROUND_UP))
	                                                                                			     .multiply((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyNumDys(),"0")))).setScale(-1, 1);
	                                                                                			             //.add(new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceDutyRcgtnDys(),"0")))
	                                                                                			    		// .add((new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazAbnceNumDys(),"0"))))
      ...
      //                                          payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
	            ...
	            //                                          payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
	                                                        if (BigDecimal.ZERO.compareTo(payr0302Vo.getPymtDducSum()) != 0)  {
	                                                        	...
	                                                        	//TODO 소득세/주민세 추가시 처리 루틴 추가 해야 함. 
...
//                                                                tpFreeDtyTotAmnt = tpFreeDtyTotAmnt.add(payr0490.getFreeDtySum());
                                                               ...
                                                               /** 각수당별로 처리하는 로직을구현한다. */ 
                                                                //용인은 실적을 합해서 처리하므로 생각한후 처리 완료 해야 함. 
                                                               ...
                                                               if (ExtermsProps.getProps("PAY_EMYMT_PYMT_19").equals(payr0490.getPayItemCd()) && (ExtermsProps.getProps("PAY_EMYMT_DYMNTH_01").equals(payr0490.getDayMnthAmntDivCd())) 
                                                                 ...
                                                                 //  tpStdPymtTotAmnt = tpStdPymtTotAmnt.add(tpEmymtAmnt);
                                                                       ...
                                                                       && (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyTm(),"0")).compareTo(BigDecimal.ZERO) > 0)) {
                                                                   //총 근무일수  * 본봉 -시급처리 추가 
                                                                   tpEmymtAmnt = (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotDutyTm(),"0")).multiply(payr0490.getPymtDducSum())).setScale(-1,BigDecimal.ROUND_DOWN); 
                                                                   tpStdPymtTotAmnt = tpStdPymtTotAmnt.add(tpEmymtAmnt);
                                                               }  
...
//                                                   payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
         ...
         //                                                   payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                            ...
                                                            if (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazLvsgNumDys(),"0")).compareTo(BigDecimal.ZERO) > 0) {
                                                               ...
                                                               ucstSum =  payr0490.getPymtDducSum();
                                                                ...
                                                                payr0302Vo.setPymtDducSum(totTmSum);   /** column 지급공제금액 : pymtDducSum */
                                                                payr0302Vo.setPymtDducTxtnAmnt(totTmSum.subtract(payr0490.getFreeDtySum()) );    /** column 지급공제과세금액 : pymtDducTxtnAmnt */ 
