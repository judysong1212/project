package com.app.smrmf.pkg.proc.yeta.yearendtax.yeta2015.file.web;
...
@Controller
public class Yeta0400FileController {


    private static final Logger logger = LoggerFactory.getLogger(Yeta0400FileController.class);  
 
    
    @Resource(name = "YetaPayr6540Service")
    private YetaPayr6540Service yetaPayr6540Service;

    /** EgovPropertyService */
    @Resource(name = "propertiesService")
    protected EgovPropertyService propertiesService; 
    
    @RequestMapping(value = "/exp/FileYeta0400Export.do")
    public String expYeta0400Export(@ModelAttribute("InfcPkgYeta1000VO") InfcPkgYeta1000VO yeta1000VO,final HttpServletRequest request,final HttpServletResponse response
        ...
        if  (yetaPayr6540Service == null) { 
            WebApplicationContext wac = WebApplicationContextUtils.
                    getRequiredWebApplicationContext( request.getSession().getServletContext());

            yetaPayr6540Service = (YetaPayr6540Service) wac.getBean("YetaPayr6540Service" ); 
...
//	  	psnl0100SrhVO.setPyspGrdeCd(MSFSharedUtils.allowNulls(MSFServerUtils.getFilterValue(columnFilters, "pyspGrdeCd"))); //호봉등급코드
		...
		yeta1000VO.setBusinCd(MSFSharedUtils.allowNulls(yeta1000VO.getBusinCd()));   //사업코드
		       	 ...
		       	 if (yeta1000VO.getCreFile().equals("Y0090001")) {
		       		 //근로소득지급명세서 호출 
		       		  state = fnPayr06540_ReTax_Payr425_Create(yeta1000VO,response) ;
		//       		 if state = 0 then
		//			          begin
		...
		//			          end;
		       		 ...
		       		 //의료비지급명세서 
		       		state = fnPayr06540_ReTax_Payr430_Create(yeta1000VO,response) ;
		...
		//	          end;
		       		 ...
		       		 //기부금지급명세서 
		       		state = fnPayr06540_ReTax_Payr440_Create(yeta1000VO,  response) ;
    ...
    * 2. 처리내용 : 
    * </pre>
    * @Method Name : fnPayr06540_ReTax_Payr425_Create
    * @date : Feb 11, 2016
    * @author : leeheuisung
    * @history : 
    *	-----------------------------------------------------------------------
    ...
    * @param yeta1000Vo
    */
  private int fnPayr06540_ReTax_Payr425_Create(InfcPkgYeta1000VO yeta1000Vo,HttpServletResponse response) throws Exception  {
...
//    resultString : string;
  ...
  //{근로소득지급명세서}
        ...
        InfcPkgYeta1000VO yetaPrc1000Vo = new InfcPkgYeta1000VO();
         ...
         A_Record = ("A" + "20" + yeta1000Vo.getTaxaCode() + String.format("%08d", Long.parseLong(StringUtils.replace(yeta1000Vo.getEdacPrdt(),"-","")))
                       ...
                       +  getRPad( subStringBytes(yeta1000Vo.getHomeTxid() ,20),20," ")  
                       +  String.format("%-4s", yeta1000Vo.getTaxaPrcd()) 
                       +  String.format("%-10s", StringUtils.replace(yeta1000Vo.getBusiNumb(),"-",""))
                       +  getRPad( subStringBytes(yeta1000Vo.getJuriName() ,38),40," ")  
                       +  getRPad( subStringBytes(yeta1000Vo.getCharDept() ,28),30," ") 
                       +  getRPad( subStringBytes(yeta1000Vo.getCharName(),28),30," ")    
                       +  getRPad( subStringBytes(yeta1000Vo.getCharTele() ,15),15," ")  
      ...
      try {
    	  ...
    	  Map mapYetaB = yetaPayr6540Service.fnPayrB_ReTax_Total_Select(yeta1000Vo);
...
//           Exit;
          ...
          mTempData01 = 0L ;
              ...
              B_RecStr = "B" + "20" + yeta1000Vo.getTaxaCode() +  String.format("%06d", B_Cnt)   +   String.format("%-10s", StringUtils.replace(yeta1000Vo.getBusiNumb(),"-",""))
                           + getRPad( subStringBytes(yeta1000Vo.getJuriName() ,38),40," ")  
                           + getRPad( subStringBytes(yeta1000Vo.getReprName() ,28),30," ")  
                           + RSNOSTR ;
              ...
              B_RecStr =  B_RecStr + String.format("%014d", mTempData01);   //총급여 총계
              mTempData01 =   Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaB.get("dcsnIncm"),"0"));  
              B_RecStr =  B_RecStr   +  String.format("%013d", mTempData01);    //결정세액 소득세 총계
            ...
            //  B_RecStr =  B_RecStr  + FormatFloat('0000000000000', mTempData01);
              ...
              B_RecStr =  B_RecStr  + String.format("%013d", mTempData01);   //결정세액 지방소득세 총계
              mTempData01 =   Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaB.get("dcsnFafv"),"0"));  
              B_RecStr =  B_RecStr   + String.format("%013d", mTempData01);    //결정세액 농특세 총계
              ...
              B_RecStr =  B_RecStr  + "1" +  String.format("%1424s", "");    //체출기간 /공란 
               
              TOT_Record.add(B_RecStr);

           C_Cnt =  1;
           
            //fnPayrC_ReTax_Total_Select
           List listYetaC = yetaPayr6540Service.fnPayrC_ReTax_Total_Select(yeta1000Vo);
          ...
          while ( iter.hasNext() ) { 
           ...
           Map<String, Object> mapYetaC = (Map<String, Object>) iter.next(); 
               ...
               C_RecStr = "C" + "20" + yeta1000Vo.getTaxaCode();
                             C_RecStr = C_RecStr + String.format("%06d", C_Cnt);    
                             ...
                             mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("resdFlag"),"0"));        //거주자구분코드
                             ...
                             mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("frnrSgyn"),"0"));                        //단일세율적용여부
                             C_RecStr = C_RecStr  + String.format("%01d", mTempData02) ;

                             C_RecStr = C_RecStr  + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaC.get("kornName"),"") ,28),30," ") ;    //성명
                             mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("frnrYeno"),"0"));                                 //내외국인구부노드
                             C_RecStr = C_RecStr  +  String.format("%01d",  mTempData02) ;
                             C_RecStr = C_RecStr  + String.format("%-13s", MSFSharedUtils.defaultNulls(mapYetaC.get("resnRegnNum"),"") );     //주민등록번호
                             ...
                             C_RecStr = C_RecStr   + getRPad( subStringBytes(yeta1000Vo.getJuriName() ,38),40," ") ;                                     //주현근무처-근무처명
                            ...
                            C_RecStr = C_RecStr   + String.format("%011d",  mTempData02);
                             ...
                             C_RecStr = C_RecStr  + "0000000000000000000000";
                     ...
                     //       C_RecStr = C_RecStr   + FormatFloat('0000000000', mTempData02);
                           ...
                           mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("taxmSi01"),"0"));  
                            ...
                            C_RecStr = C_RecStr   +  String.format("%010d",  mTempData02);
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("payrTemp61"),"0"));    //비과세계
                            C_RecStr = C_RecStr   +  String.format("%010d",  mTempData02);
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("payrTemp62"),"0"));   //감면소득계
                            C_RecStr = C_RecStr   +  String.format("%010d",  mTempData02);
                            ...
                            C_RecStr = C_RecStr   + String.format("%08d",mTempData02)  ;
                            mTempData02 =  Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("addrCt70"),"0"));  /*(27) column 추가공제_경로우대_70세이상수 : ADDR_CT70 */       
                            C_RecStr = C_RecStr   + String.format("%02d",mTempData02);
                            mTempData02 =  Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("addrRpt70"),"0"));  /*(27) column 추가공제_경로우대70 : ADDR_RPT70 */   
                            C_RecStr = C_RecStr   + String.format("%08d", mTempData02) ;
                            mTempData02 =  Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("addrHdrc"),"0"));  /*(28) column 추가공제_장애인수 : ADDR_HDRC */    
                            C_RecStr = C_RecStr   + String.format("%02d",mTempData02);
                            mTempData02 =  Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("addrHdam"),"0"));   /*(28) column 추가공제_장애인 : ADDR_HDAM */     
                            C_RecStr = C_RecStr   + String.format("%08d", mTempData02);
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("addrFame"),"0")); /*(29) column 추가공제_부녀자 : ADDR_FAME */      
                            C_RecStr = C_RecStr   + String.format("%08d", mTempData02);
                         ...
                         //   C_RecStr = C_RecStr   + FormatFloat('00000000', mTempData02)  ;
                          ...
                          //  C_RecStr = C_RecStr   + FormatFloat('00000000000',mTempData02);
                            ...
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("subtEram"),"0"));       /* (37) column 차감소득금액 : SUBT_ERAM */  
                            C_RecStr = C_RecStr  + String.format("%011d", mTempData02);
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etcpPsnv"),"0"));     /* (38)column 개인연금저축불입액_2000년이전 : ETCP_PSNV */  
                            C_RecStr = C_RecStr   + String.format("%08d", mTempData02);
                       //     mTempData02 = oraQrySelect02.FieldByName('ETCP_NSAV').AsFloat;                 //연금저축소득공제
                        //    C_RecStr = C_RecStr  + FormatFloat('00000000', mTempData02) ;
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etchPrep"),"0"));    /* (39) column 기타공제_소기업공제불입금 : ETCH_PREP */    
                            C_RecStr = C_RecStr  + String.format("%010d", mTempData02);

                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etcsComp"),"0"));   /*(40)-가 column 기타공제_주택마련저축_청약저축 : ETCS_COMP */       
                            C_RecStr = C_RecStr  + String.format("%010d",  mTempData02) ;
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etwkSbam"),"0"));    /* (40)-나column 기타공제_주택마련저축_주택청약종합저축 : ETWK_SBAM */       
                            C_RecStr = C_RecStr   + String.format("%010d", mTempData02) ;
                          //  mTempData02 = oraQrySelect02.FieldByName('ETWK_LGAM').AsFloat ;                  //장기주택마련저축
                          //  C_RecStr = C_RecStr   + FormatFloat('0000000000',mTempData02) ;
                            mTempData02 =Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etwkSvam"),"0"));   /*(40)-다 column 기타공제_주택마련저축_근로자주택마련저축 : ETWK_SVAM */
                            C_RecStr = C_RecStr   + String.format("%010d",  mTempData02);
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etgdDdam"),"0"));        /* (41) column 기타공제_투자조합출자공제액계 : ETGD_DDAM */   
                            C_RecStr = C_RecStr   + String.format("%010d", mTempData02) ;
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etccCard"),"0"));     /*(42) column 기타공제_신용카드등사용액소득공제 : ETCC_CARD */ 
                            C_RecStr = C_RecStr   + String.format("%08d", mTempData02) ;
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("etckUnon"),"0"));       /*(43) column 기타공제_우리사주출연금공제 : ETCK_UNON */      
                            C_RecStr = C_RecStr   + String.format("%010d",  mTempData02);
                      ...
                      //      C_RecStr = C_RecStr  + FormatFloat('00000000', mTempData02);
                            ...
                            C_RecStr = C_RecStr   + String.format("%010d", mTempData02) ;
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("spedEtar"),"0"));                 //특별세액공제_교육비_공제대상금액
                            C_RecStr = C_RecStr   + String.format("%010d", mTempData02) ;
                            ...
                            C_RecStr = C_RecStr   + String.format("%011d", mTempData02) ;
                            mTempData02 = Long.parseLong( MSFSharedUtils.defaultNulls(mapYetaC.get("spcirfam"),"0"));    /*(65)-다 column 특별공제_기부금_우리사주조합2015이후 : SPCI_RFAM */
                            C_RecStr = C_RecStr   + String.format("%010d", mTempData02) ;
                        ...
                        //    C_RecStr = C_RecStr   + FormatFloat('0000000000', mTempData02);
                            C_RecStr = C_RecStr   + String.format("%-5s", "") ;
               ...
               List listYetaD = yetaPayr6540Service.fnPayrD_ReTax_Total_Select(yeta1000Vo);
               ...
               if (listYetaD.size() > 0) { 
                   ...
                   while ( iter01.hasNext() ) { 
                     ...
                     D_RecStr = "D" + "20" + yeta1000Vo.getTaxaCode();
                                 ...
                                 D_RecStr = D_RecStr + "2" ;
                                 D_RecStr = D_RecStr + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaD.get("siteCtnt"),"") ,38),40," ") ;
                              ...
                              //   D_RecStr = D_RecStr +  FormatFloat('0000000000', mTempData02);
                               ...
                               //  D_RecStr = D_RecStr  +  FormatFloat('0000000000',mTempData02);
                                 ...
                                 D_RecStr = D_RecStr + String.format("%981s", ""); 
              ...
              yeta1000Vo.setSystemkey(MSFSharedUtils.allowNulls(mapYetaC.get("systemkey")));
               ...
               List listYetaE = yetaPayr6540Service.fnPayrE_ReTax_Total_Select(yeta1000Vo);
               ...
               if ( listYetaE.size() > 0) { 
                   ...
                   while ( iter03.hasNext() ) { 
                     ...
                     Map<String, Object> mapYetaE = (Map<String, Object>) iter03.next(); 
                      ...
                      E_RecStr =  "E" + "20" + yeta1000Vo.getTaxaCode() +  String.format("%06d", C_Cnt)
                                 +  String.format("%-10s", StringUtils.replace(yeta1000Vo.getBusiNumb(),"-",""))
                                 + String.format("%-13s", StringUtils.replace(MSFSharedUtils.defaultNulls(mapYetaC.get("resnRegnNum"),""),"-",""));
                 ...
                 if (listYetaE.size() + 1 > E_RowCnt)  {
                      ...
                      E_RecStr =  E_RecStr + MSFSharedUtils.defaultNulls(mapYetaE.get("relhCode")," ");  
                                  ...
                                  E_RecStr = E_RecStr + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaE.get("kornName"),"") ,18),20," ")   ;
                                  E_RecStr = E_RecStr+ MSFSharedUtils.defaultNulls(mapYetaE.get("rsnoNumb"),"");  
...
//                             	 }      
              ...
              //F레코드
               ...
               List listYetaF = yetaPayr6540Service.fnPayrF_ReTax_Total_Select(yeta1000Vo);
               ...
               if ( listYetaF.size() > 0) { 
                 ...
                 if (listYetaF.size() + 1 > F_RowCnt)  {
                                ...
                                F_RecStr = F_RecStr + String.format("%-2s", MSFSharedUtils.defaultNulls(mapYetaF.get("imdeTxcd"),""))   
                                   ...
                                   + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaF.get("finaName"),""),48),50," ")  
                                   + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaF.get("acntNumb"),""),18),20," ") ; 
...
//              	 }     
      ...
      else
      {
             ...
             F_RowCnt =  F_RowCnt + 1;
                 ...
                 F_RecStr = F_RecStr + String.format("%-150s","");
               ...
               List listYetaG = yetaPayr6540Service.fnPayrG_ReTax_Total_Select(yeta1000Vo);
                   ...
                   while ( iter05.hasNext() ) { 
                     ...
                     ||  MSFSharedUtils.paramNotNull( mapYetaG.get("cnclName")) || MSFSharedUtils.paramNotNull( mapYetaG.get("leasName"))))  {
                       ...
                       if (listYetaG.size() + 1 > G_RowCnt)  {
                                    ...
                                    G_RecStr = G_RecStr + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaG.get("mnrnName"),""),18),20," ")  
                                       +  String.format("%-13s",  MSFSharedUtils.defaultNulls(mapYetaG.get("mnrnRsno"),""));
                                       ...
                                       G_RecStr = G_RecStr +  getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaG.get("mnrnAddr"),"") ,98),100," ")  ;
                                       ...
                                       G_RecStr = G_RecStr +  String.format("%010d",mTempData02) ;
                                        ...
                                        G_RecStr = G_RecStr + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaG.get("cnclName"),"") ,18),20," ") 
                                      ...
                                      G_RecStr = G_RecStr + getRPad( subStringBytes( MSFSharedUtils.defaultNulls(mapYetaG.get("leasName"),"") ,18),20," ")  
                                       ...
                                       G_RecStr = G_RecStr +  getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaG.get("leasAddr"),"") ,98),100," ") ;
                     ...
                     else
                     {
                       ...
                       if (listYetaG.size() + 1 > G_RowCnt)  {
                           ...
                           G_RecStr = G_RecStr + getRPad( subStringBytes(MSFSharedUtils.defaultNulls(mapYetaG.get("mnrnName"),"") ,18),20," ")   
                              +  String.format("%-13s",  MSFSharedUtils.defaultNulls(mapYetaG.get("mnrnNsno"),""));
...
//                    	 }     
          ...
          File dir = new File("C:/EOSDATA/2015/");
          //디렉토리가 없으면 생성
          if(!dir.isDirectory()){
           dir.mkdirs();
          }
...
//        TOT_Record.SaveToFile('C:\EOSDATA\' + DEFIN + leftStr(defBUNS_LINS,7) + '.' + rightStr(defBUNS_LINS,3) + '.denc');
          String fileDir = "C:/EOSDATA/2015/" + DEFIN + StringUtils.left(defBUNS_LINS,7) + "." + StringUtils.right(defBUNS_LINS,3) + ".denc";
          ...
          File objFile = new File("C:/EOSDATA/2015/" + DEFIN + StringUtils.left(defBUNS_LINS,7) + "." + StringUtils.right(defBUNS_LINS,3) + ".denc");
...
//	   	   retval =  new BasePagingLoadResult<ShowMessageBM>(bmResult, 0, bmResult.size());   
      ...
      * 2. 처리내용 : 
      * </pre>
      * @Method Name : fnPayr06540_ReTax_Payr430_Create
      * @date : Feb 11, 2016
      * @author : leeheuisung
      * @history : 
      *	-----------------------------------------------------------------------
      ...
      * @param yeta1000Vo
      */
    private int fnPayr06540_ReTax_Payr430_Create(InfcPkgYeta1000VO yeta1000Vo,HttpServletResponse response) throws Exception  {
    	...
    	try {
    		...
    		List listYetaCA = yetaPayr6540Service.fnPayrPSNL_ReTax_CA_Total_Select(yeta1000Vo);
    		...
    		if  (listYetaCA.size() > 0) { 
    		  ...
    		  //	for (int i = 0 ; i < listYeta1.size() ; i++) {
    	            ...
    	            List listCAYeta = yetaPayr6540Service.fnPayrCA_ReTax_Total_Select(yeta1000Vo);
    				...
    				//	yetaPrc3140Vo = (InfcPkgYeta3140VO)listYeta2.get(j);
						...
						A_RecStr = A_RecStr + getRPad(subStringBytes(yeta1000Vo.getJuriName() ,38),40," ") ; // 10 상호

						/*이렇게 구성하는것이 맞는지 모르겠니다만
						10번까지는 화면에서 받아온 vo (yeta1000Vo) 이용
						이후는 select 결과 vo (yetaPrc3140Vo) 이용*/
						
						A_RecStr = A_RecStr +    String.format("%-13s", StringUtils.replace(MSFSharedUtils.defaultNulls(mapCAData.get("resnRegnNum"),""),"-",""));     // 11 소득자 주민번호 
						A_RecStr = A_RecStr + String.format("%-1s", MSFSharedUtils.defaultNulls(mapCAData.get("frnrYeno"),"1")); // 12 내,외국인 코드
						A_RecStr = A_RecStr + getRPad(subStringBytes(MSFSharedUtils.defaultNulls(mapCAData.get("kornName"),""),28),30 ," "); // 13 성명
						...
						A_RecStr = A_RecStr + getRPad(subStringBytes( MSFSharedUtils.defaultNulls(mapCAYeta.get("vendName"),"") ,38),40," "); // 15 지급처 상호
						...
						//A_RecStr = A_RecStr + String.format("%05d", yetaPrc3140Vo.getMediCfcd()); // 17 건수
						...
						//A_RecStr = A_RecStr + String.format("%011d", yetaPrc3140Vo.getCashUeam()); // 18 금액
						
						A_RecStr = A_RecStr + String.format("%-1s", MSFSharedUtils.defaultNulls(mapCAYeta.get("ctifTrea"),""));   // 19 난임시술비 해당 여부
						A_RecStr = A_RecStr + String.format("%-13s", StringUtils.replace(MSFSharedUtils.defaultNulls(mapCAYeta.get("rsnoNumb"),""),"-","")); // 20 주민번호 번호						
						...
						A_RecStr = A_RecStr + "1"; // 23 제출대상기간 코드
...
//					 
//					    System.out.println("vvvv[" + getRPad(subStringBytes(bbuffer.toString() ,38),40," ") + "]");
//						
...
//					 
//					    System.out.println("aaaaa[" + getRPad(subStringBytes(bbuffer1.toString() ,38),40," ") + "]");
//						
...
//					    System.out.println("ccccc[" + getRPad(subStringBytes(decode(hex_euckr, "EUC-KR") ,38),40," ") + "]");
//					    System.out.println( changeCharset(source, "UTF-8"));
//					    System.out.println("aaaaa[" + getRPad(subStringBytes(changeCharset(source, "UTF-8") ,38),40," ") + "]");
//						        
//					    System.out.println( changeCharset(source, "EUC-KR"));
//					    System.out.println("vvvvc[" + getRPad(subStringBytes(changeCharset(source, "EUC-KR") ,38),40," ") + "]");
...
//	   	   retval =  new BasePagingLoadResult<ShowMessageBM>(bmResult, 0, bmResult.size());   
   ...
   // 기부금지급명세서  
 private int fnPayr06540_ReTax_Payr440_Create(InfcPkgYeta1000VO yeta1000Vo,HttpServletResponse response) throws Exception { 
...
// 	  mStream: TMemoryStream;
 	...
 	//{기부금지급명세서} 
 	        ...
 	        strTempDataA = strTempDataA  + getRPad(subStringBytes(yeta1000Vo.getHomeTxid(),18),20," "); // String.format("%-20s", yeta1000Vo.getHomeTxid()); //Format('%-20s', [edHOME_TXID.Text]) ;  //홈텍스아이디 
 	        ...
 	        strTempDataA = strTempDataA  +  getRPad(subStringBytes(yeta1000Vo.getJuriName(),38),40," "); //String.format("%-40s", yeta1000Vo.getJuriName()); //  Format('%-40s', [edPROF_RSNM.Text])   ; //법인명
 	        strTempDataA = strTempDataA  + getRPad(subStringBytes(yeta1000Vo.getCharDept(),28),30," "); //String.format("%-30s", yeta1000Vo.getCharDept());  //Format('%-30s', [edSELF_DEPT.Text]) ;  //담당자부서
 	        strTempDataA = strTempDataA  + getRPad(subStringBytes(yeta1000Vo.getCharName(),28),30," "); 
 	        	//	String.format("%-30s", yeta1000Vo.getCharName());  //Format('%-30s', [edSELF_NAME.Text])   ; //담당자성명
 	        strTempDataA = strTempDataA  + getRPad(subStringBytes(yeta1000Vo.getCharTele(),15),15," ")  ;  //Format('%-15s', [edSELF_TELE.Text]) ;   //담당자전화번호
 	        mTempDataA = 1L;
 	        ...
 	        strTempDataA = strTempDataA  + String.format("%2s", "");  //Format('%2s', ['']);  //공란
 	    ...
 	    try {
 	       ...
 	       Map mapHb = yetaPayr6540Service.fnPayrHB_ReTax_Total_Select(yeta1000Vo);
...
// 	      end;
           ...
           else
           {
	         ...
	         strTempDataB  = strTempDataB +  getRPad(subStringBytes(yeta1000Vo.getJuriName(),38),40," ")  ;
	         ...
	         strTempDataB = strTempDataB + "1" ;   //제출대상기간코드
	         strTempDataB = strTempDataB + String.format("%77s", "");     //공란
 	         ...
 	         List listTotData01 = yetaPayr6540Service.fnPayrPSNL_ReTax_Total_Select(yeta1000Vo);
...
// 	             Exit;
	          ...
	          while ( iter.hasNext() ) { 
	            ...
	            List listHCData = yetaPayr6540Service.fnPayrHC_ReTax_Total_Select(yeta1000Vo);  
...
// 	                end; }
 		          ...
 		          while ( iter01.hasNext() ) { 
 		            ...
 		            Map<String, Object> mapHCData = (Map<String, Object>) iter01.next(); 
                        ...
                        strTempDataC = strTempDataC  +  String.format("%-13s", StringUtils.replace(MSFSharedUtils.defaultNulls(mapTotData01.get("resnRegnNum"),""),"-",""));     //소득자주민등록번호
                        ...
                        strTempDataC = strTempDataC  +   getRPad(subStringBytes(MSFSharedUtils.defaultNulls(mapTotData01.get("kornName"),""),28),30," "); 
                        ...
                        strTempDataC = strTempDataC  + String.format("%25s", "");      //공란 -25
 	              ...
 	              List listHDData = yetaPayr6540Service.fnPayrHD_ReTax_Total_Select(yeta1000Vo); 
		          ...
		          while ( iter02.hasNext() ) { 
		            ...
		            Map<String, Object> mapHDData = (Map<String, Object>) iter02.next();
                          ...
                          strTempDataD = strTempDataD +  String.format("%-13s", StringUtils.replace(MSFSharedUtils.defaultNulls(mapTotData01.get("resnRegnNum"),""),"-",""));      //소득자주민등록번호
                          ...
                          strTempDataD = strTempDataD + getRPad(subStringBytes(MSFSharedUtils.defaultNulls(mapHDData.get("ctrbName"),""),28),30," ");
                          ...
                          strTempDataD = strTempDataD +  getRPad(subStringBytes(MSFSharedUtils.defaultNulls(mapHDData.get("cbtgName"),""),18),20," "); 
                          //String.format("%-20s", MSFSharedUtils.defaultNulls(mapHDData.get("cbtgName"),""));   //성명 - 20
                          strTempDataD = strTempDataD +  String.format("%-13s", StringUtils.replace(MSFSharedUtils.defaultNulls(mapHDData.get("rsnoNumb"),""),"-",""));    //기부자주민등록번호 
                          mTempDataD =   Long.parseLong(MSFSharedUtils.defaultNulls(mapHDData.get("ctrbCtnt"),"1"));   
                          ...
                          strTempDataD = strTempDataD +  String.format("%42s", "");    //공란 42
...
//	   	   retval =  new BasePagingLoadResult<ShowMessageBM>(bmResult, 0, bmResult.size());   
		 ...
		 * @return  
		 */
		private static String getCPad(String str, int size, String strFillText) {
		    int intPadPos = 0;
		 ...
		 * @return  
		 */
		private static String getLPad(String str, int size, String strFillText) {
		    for(int i = (str.getBytes()).length; i < size; i++) {
		        str = strFillText + str;
		    }
		    return str;
		 ...
		 * @return  
		 */
		private static String getRPad(String str, int size, String strFillText) {
		    for(int i = (str.getBytes()).length; i < size; i++) {
		        str += strFillText;
		    }
		    return str;
		...
		private static  String subStringBytes(String str, int byteLength) {   
