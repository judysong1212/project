package com.app.smrmf.pkg.proc.yeta.yearendtax.yeta2017.file.web;
...
import com.app.smrmf.pkg.yeta.yeta2017.file.Yeta2400PkgFileController;
import com.app.smrmf.sysm.server.service.vo.SysIfBass0350VO;

@Controller
public class Yeta2400FileController {


    private static final Logger logger = LoggerFactory.getLogger(Yeta2400FileController.class);  
 
     
    
    @RequestMapping(value = "/exp/FileYeta2400Export.do")
    public String expYeta2400Export(@ModelAttribute("InfcPkgYe161010VO") InfcPkgYe161010VO  ye161010VO,final HttpServletRequest request,final HttpServletResponse response
        ...
        Yeta2400PkgFileController yete2400 = new Yeta2400PkgFileController();
...
//		ye161010VO.setEdacPrdt(MSFSharedUtils.allowNulls(commandMap.get("edacPrdt")));				//원천신고부서
...
//     	listTypCd = SysifPkgServiceUtils.funcArrBeforTypOccuCd(RemoteServiceUtil.getThreadLocalRequest(),sysIfBass0320Vo);
     ...
     //	 List<String> listTyInttnCd = new ArrayList<String>(); 
    	...
    	sysIfBass0350Vo.setDtilOccuInttnCd(ye161010VO.getDtilOccuInttnCd());
     	...
     	/************************과거 직종세에 해당하는 사람들 데이타 가져오기 **********************************************************/
		       	 ...
		       	 if (ye161010VO.getCreFile().equals("Y0090001")) {
		       		 //근로소득지급명세서 호출 
		       		  state = yete2400.fnYeta2400_Tax_Ye161010_File_Create(request,ye161010VO,response) ;
		//       		 if state = 0 then
		//			          begin
		...
		//			          end;
		       		 ...
		       		 //의료비지급명세서 
		       		state = yete2400.fnYeta2400_Tax_Ye164010_File_Create(request,ye161010VO,response) ;
		...
		//	          end;
		       		 ...
		       		 //기부금지급명세서 
		       		state = yete2400.fnYeta2400_Tax_Ye165010_File_Create(request,ye161010VO,  response) ;
 ...
 public String yeta2400Verifiction(@ModelAttribute("InfcPkgYe161010VO") InfcPkgYe161010VO  ye161010VO,final HttpServletRequest request,final HttpServletResponse response
         , Map commandMap , Model model) throws Exception {
     ...
     //	 List<String> listTyInttnCd = new ArrayList<String>(); 
    	...
    	sysIfBass0350Vo.setDtilOccuInttnCd(ye161010VO.getDtilOccuInttnCd());
     	...
     	/************************과거 직종세에 해당하는 사람들 데이타 가져오기 **********************************************************/
		       	 ...
		       	 if (ye161010VO.getCreFile().equals("Y0090001")) {
		       		 //근로소득지급명세서 검증  
		       		rtnResult = yete2400.fnYeta2400_Tax_Ye161010_File_Check(request,ye161010VO,response) ;
		 
		       	 
		       	 } else if (ye161010VO.getCreFile().equals("Y0090002")) {
		       		 //의료비지급명세서 검증 
		       		rtnResult = yete2400.fnYeta2400_Tax_Ye164010_File_Check(request,ye161010VO,response) ;
		       	    
		       		
		       	 } else if (ye161010VO.getCreFile().equals("Y0090003")) {
		       		 //기부금지급명세서 검증 
		       		rtnResult = yete2400.fnYeta2400_Tax_Ye165010_File_Check(request,ye161010VO,  response) ;
