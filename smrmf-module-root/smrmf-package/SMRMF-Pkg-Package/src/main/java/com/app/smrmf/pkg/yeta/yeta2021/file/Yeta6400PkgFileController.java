package com.app.smrmf.pkg.yeta.yeta2021.file;
...
import com.app.smrmf.pkg.yeta.yeta2021.file.service.Yeta6400PayrService;

import egovframework.rte.fdl.property.EgovPropertyService;

@Controller
public class Yeta6400PkgFileController {

    private static final Logger logger = LoggerFactory.getLogger(Yeta6400PkgFileController.class);  
    
    @Resource(name = "Yeta6400PayrService")
    private Yeta6400PayrService yeta6400PayrService;
    ...
    public Yeta6400PkgFileController() {
    ...
    * @param ye161010VO
    */
	public int fnYeta6400_Tax_Ye161010_File_Create(HttpServletRequest request,InfcPkgYe161010_2021_VO  ye161010VO,HttpServletResponse response) throws Exception  {

		if  (yeta6400PayrService== null) {
			WebApplicationContext wac = WebApplicationContextUtils.
			getRequiredWebApplicationContext( request.getSession().getServletContext());

			yeta6400PayrService = (Yeta6400PayrService) wac.getBean("Yeta6400PayrService");
		}
...
//    resultString : string;
       ...
       // InfcPkgye161010VO infcPkgye161010VO = new InfcPkgye161010VO(); 
		...
		infcPkgYe161005Vo  = yeta6400PayrService.selectYe161005(ye161010VO);
		...
		try {
			Map mapYetaB = yeta6400PayrService.fnPayrB_ReTax_Total_Select(infcPkgYe161020Vo); 
...
//           Exit;
			...
			B_RecStr = "B" + "20" + infcPkgYe161005Vo.getTxOffcCd() +  String.format("%06d", B_Cnt)  //레코드구분 : B, 1BYTE + 자료구분 : 20, 2BYTE + 세무서코드 : 3BYTE + 일련번호 : 6BYTE = 12BYTE
                        ...
                        + String.format("%-13s", StringUtils.replace(RSNOSTR,"-","")) + "2021"; //주민번호 : 13BYTE + 귀속년도 : 4BYTE = 17BYTE
			...
			List listYetaC = yeta6400PayrService.fnPayrC_ReTax_Total_Select(infcPkgYe161020Vo);
			...
			while ( iter.hasNext() ) { 
               ...
               //  oraQrySelect01.FieldByName('CITY_SEQN').AsString; Format('%.11d', [])
				...
				//2020연말정산_추가
				C_RecStr = C_RecStr   + String.format("%010d", 0); //외국인기술자 소득세감면 70% : 10BYTE
				...
				//2020연말정산_추가
				/*logger.debug("실효세율1 = " + Double.parseDouble(MSFSharedUtils.defaultNulls
						  (mapYetaC.get("yetaC159"),"0.00")));
				logger.debug("실효세율2 = " + Double.parseDouble(MSFSharedUtils.defaultNulls
      				  (mapYetaC.get("yeta0c63"),"0.00")));
				logger.debug("실효세율3 = " + (Double.parseDouble(MSFSharedUtils.defaultNulls
						  (mapYetaC.get("yetaC159"),"0.00")))/(Double.parseDouble(MSFSharedUtils.defaultNulls
		        				  (mapYetaC.get("yeta0c63"),"0.00"))));
				logger.debug("실효세율4 = " + ((Double.parseDouble(MSFSharedUtils.defaultNulls
						  (mapYetaC.get("yetaC159"),"0.00")))/(Double.parseDouble(MSFSharedUtils.defaultNulls
		        				  (mapYetaC.get("yeta0c63"),"0.00"))))*100);
				logger.debug("실효세율5 = " + String.format("%04.1f",((Double.parseDouble(MSFSharedUtils.defaultNulls
								  (mapYetaC.get("yetaC159"),"0.00")))/(Double.parseDouble(MSFSharedUtils.defaultNulls
				        				  (mapYetaC.get("yeta0c63"),"0.00"))))*100));
				logger.debug("실효세율6 = " + StringUtils.replace(String.format("%04.1f",((Double.parseDouble(MSFSharedUtils.defaultNulls
								  (mapYetaC.get("yetaC159"),"0.00")))/(Double.parseDouble(MSFSharedUtils.defaultNulls
				        				  (mapYetaC.get("yeta0c63"),"0.00"))))*100),".",""));*/
				C_RecStr = C_RecStr +  
				...
				List listYetaD = yeta6400PayrService.fnPayrD_ReTax_Total_Select(infcPkgYe161020Vo);
				//D 레코드 처리 화면  
				if (listYetaD.size() > 0) { 
					...
					while ( iter01.hasNext() ) { 
						...
						D_RecStr = D_RecStr   +  String.format("%010d",  0);	//중소기업청년근로자및핵심인력성과보상기금수령액 : 10BYTE
									
						//2021연말정산_추가
						D_RecStr = D_RecStr   +  String.format("%010d",  0); //중견기업청년근로자 및 핵심인력 성과보상기금 소득세감면 : 10BYTE
						...
						D_RecStr = D_RecStr +  String.format("%020d",0); //공란 : 10BYTE + 공란 : 10BYTE = 20BYTE
				...
				List listYetaE = yeta6400PayrService.fnPayrE_ReTax_Total_Select(infcPkgYe161020Vo);
				...
				if ( listYetaE.size() > 0) { 
					...
					while ( iter03.hasNext() ) { 
						...
						if (listYetaE.size() + 1 > E_RowCnt)  {
							...
							E_RecStr = E_RecStr+   String.format("%010d",mTempData02)  ;	//소비증가분_2020년 전체사용분 : 10BYTE
						...
						E_RecStr = E_RecStr +  String.format("%683s",""); //공란
				...
				List listYetaF = yeta6400PayrService.fnPayrF_ReTax_Total_Select(infcPkgYe161020Vo);
				...
				F_RecStr = F_RecStr + String.format("%-323s",""); //공란 : 323BYTE
               ...
               List listYetaG = yeta6400PayrService.fnPayrG_ReTax_Total_Select(infcPkgYe161020Vo);
     	  ...
     	  //      DEFIN = "H"; //기부금  
	        ...
	        //    Map<String, Object> mapTotData01 = (Map<String, Object>) iter.next();  
	            ...
	            List listHCData = yeta6400PayrService.fnPayrHC_ReTax_Total_Select(infcPkgYe161020Vo);  
	            ...
	            if (listHCData.size() <= 0) { 
		          ...
		          infcPkgYe161020Vo.setSystemkey(MSFSharedUtils.allowNulls(mapYetaC.get("systemkey")));
	              //I레코드 - 해당연도 기부명세 레코드  
	              List listHDData = yeta6400PayrService.fnPayrHD_ReTax_Total_Select(infcPkgYe161020Vo); 
               ...
               // infcPkgYeta17FileDAO.updateYeta6400ToYe161010_S
              ye161010VO.setSystemkey(MSFSharedUtils.allowNulls(mapYetaC.get("systemkey"))); 
              yeta6400PayrService.updateYeta6400ToYe161010(ye161010VO); 

          } 
          
       
          
          File dir = new File("C:/EOSDATA/2021/");
          //디렉토리가 없으면 생성
          if(!dir.isDirectory()){
           dir.mkdirs();
          }
...
//        TOT_Record.SaveToFile('C:\EOSDATA\' + DEFIN + leftStr(defBUNS_LINS,7) + '.' + rightStr(defBUNS_LINS,3) + '.denc');
          String fileDir = "C:/EOSDATA/2021/" + DEFIN + StringUtils.left(defBUNS_LINS,7) + "." + StringUtils.right(defBUNS_LINS,3) + ".denc";
         // FileWriter fw = new FileWriter(fileDir); // 절대주소 경로 가능
          ...
          File objFile = new File("C:/EOSDATA/2021/" + DEFIN + StringUtils.left(defBUNS_LINS,7) + "." + StringUtils.right(defBUNS_LINS,3) + ".denc");
  ...
  public int fnYeta6400_Tax_Ye1611010_File_Create(HttpServletRequest request,InfcPkgYe161010_2021_VO  ye161010VO,HttpServletResponse response) throws Exception  {
      ...
      * 2. 처리내용 : 
      * </pre>
      * @Method Name : fnYeta1400_ReTax_Yeta6140_Create
      * @date : Feb 11, 2016
      * @author : leeheuisung
      * @history : 
      *	-----------------------------------------------------------------------
      ...
      * @param ye161010VO
      */
  public int fnYeta6400_Tax_Ye164010_File_Create(HttpServletRequest request,InfcPkgYe161010_2021_VO  ye161010VO,HttpServletResponse response) throws Exception  {
    	...
    	try {
    		...
    		// A레코드 - 의료비지급명세서 레코드
    		List listYetaCA = yeta6400PayrService.fnPayrPSNL_ReTax_CA_Total_Select(infcPkgYe161020Vo);
    		...
    		if  (listYetaCA.size() > 0) { 
    		  ...
    		  //	for (int i = 0 ; i < listYeta1.size() ; i++) { 
    	            ...
    	            List listCAYeta = yeta6400PayrService.fnPayrCA_ReTax_Total_Select(infcPkgYe161020Vo);
    	            ...
    	            Iterator<Map<String, Object>> iter01 = listCAYeta.iterator(); 
      	            ...
      	            Map<String, Object> mapCAYeta = (Map<String, Object>) iter01.next(); 
						...
						A_RecStr = A_RecStr + "2021"; //귀속년도 : 4BYTE
						A_RecStr = A_RecStr + defBUNS_LINS; //사업자등록번호 : 10BYTE
						...
						이후는 select 결과 vo (infcPkgYeta6140Vo) 이용*/
  ...
  * 2. 처리내용 : 
  * </pre>
  * @Method Name : fnYeta1400_ReTax_Yeta6180_Create
  * @date : 2017. 1. 20.
  * @author : atres-pc
  * @history : 
  *	-----------------------------------------------------------------------
  ...
  * @throws Exception
  */
  public int fnYeta6400_Tax_Ye165010_File_Create(HttpServletRequest request,InfcPkgYe161010_2021_VO  ye161010VO,HttpServletResponse response) throws Exception { 
...
// 	       
// 	    	//B레코드 - 원천징수의무자별 집계레코드 
// 	       Map mapHb = yeta6400PayrService.fnPayrHB_ReTax_Total_Select(ye161010VO);
// 	        
...
//	          mTempDataB =    Long.parseLong( MSFSharedUtils.defaultNulls(mapHb.get("yeta6180Cnt"),"0"));     
...
// 	          
// 	         //C 레코드 - 기부금 조정명세 레코드  
// 	         List listTotData01 = yeta6400PayrService.fnPayrPSNL_ReTax_Total_Select(ye161010VO);
// 	          
//
...
// 	              //SYSTEMKEY 가지고 데이타 가져오기 
//	            //C 레코드 - 기부금 조정명세 레코드  YETA3170
//	            List listHCData = yeta6400PayrService.fnPayrHC_ReTax_Total_Select(ye161010VO);  
//	             
...
// 	             
// 	              //D레코드 - 해당연도 기부명세 레코드  
// 	              List listHDData = yeta6400PayrService.fnPayrHD_ReTax_Total_Select(ye161010VO); 
// 	               
// 	             if (listHCData.size() > 0) { 
//	              
//	              Iterator<Map<String, Object>> iter02 = listHDData.iterator(); 
...
//      } 
  ...
  public List<YetaMessageVO> fnYeta6400_Tax_Ye161010_File_Check(HttpServletRequest request,InfcPkgYe161010_2021_VO  ye161010VO,HttpServletResponse response) throws Exception  {
    ...
    Long mTempData02 = 0L;
      ...
      * @param ye161010VO
      */
  public List<YetaMessageVO> fnYeta6400_Tax_Ye164010_File_Check(HttpServletRequest request,InfcPkgYe161010_2021_VO  ye161010VO,HttpServletResponse response) throws Exception  {
  ...
  * @throws Exception
  */
 public List<YetaMessageVO> fnYeta6400_Tax_Ye165010_File_Check(HttpServletRequest request,InfcPkgYe161010_2021_VO  ye161010VO,HttpServletResponse response) throws Exception { 
