package com.app.smrmf.infc.yearendtax.server.vo.yeta2023;

import java.io.Serializable;

/**
 * @Class Name : Ye166020VO.java
 * @Description : Ye166020 VO class
 * @Modification Information
 *
 * @author ATRES
 * @since 2017.01.07
 * @version 1.0
 * @see
 *  
 *  Copyright (C)  All right reserved.
 */
public class InfcPkgYe166020_2023_VO   implements Serializable {
    private static final long serialVersionUID = 1L;
    
    /** DPOB_CD */
    private java.lang.String dpobCd;
    
    /** YRTX_BLGG_YR */
    private java.lang.String yrtxBlggYr;
    
    /** CLUT_SEPT_CD */
    private java.lang.String clutSeptCd;
    
    /** SYSTEMKEY */
    private java.lang.String systemkey;
    
    /** FAM_RESNO_ENC_CNTN */
    private java.lang.String famResnoEncCntn;
    
    /** DATA_CD */
    private java.lang.String dataCd;
    
    /** NNF_CL */
    private java.lang.String nnfCl;
    
    /** SUPT_FMLY_RLT_CL */
    private java.lang.String suptFmlyRltCl;
    
    /** SUPT_FMLY_FNM */
    private java.lang.String suptFmlyFnm;
    
    /** SUPT_FMLY_BHDT */
    private java.lang.String suptFmlyBhdt;
    
    /** CRDC_USE_AMT_SUM */
    private java.math.BigDecimal crdcUseAmtSum;
    
    /** CRDC_USE_AMT */
    private java.math.BigDecimal crdcUseAmt;
    
    /** CSHPT_USE_AMT */
    private java.math.BigDecimal cshptUseAmt;
    
    /** DRTP_CARD_USE_AMT */
    private java.math.BigDecimal drtpCardUseAmt;
    
    /** ZRTD_USE_AMT */
    private java.math.BigDecimal zrtdUseAmt;
    
    /** TDMR_USE_AMT */
    private java.math.BigDecimal tdmrUseAmt;
    
    /** BOOK_SHOW_AMT */
    private java.math.BigDecimal bookShowAmt;

    /** ETC_USE_AMT */
    private java.math.BigDecimal etcUseAmt;
    
    /** KYBDR */
    private java.lang.String kybdr;
    
    /** INPT_DT */
    private java.lang.String inptDt;
    
    /** INPT_ADDR */
    private java.lang.String inptAddr;
    
    /** ISMT */
    private java.lang.String ismt;
    
    /** REVN_DT */
    private java.lang.String revnDt;
    
    /** REVN_ADDR */
    private java.lang.String revnAddr;
    
    
    //2021연말정산_추가
    //CRDC_PRE_USE_AMT_SUM
    private java.math.BigDecimal crdcPreUseAmtSum;
    
    private java.math.BigDecimal crdcCurrUseAmtSum;
    
    
    //2023연말정산_추가-------------------------------
    
    //전년도 전통시장 합계금액
    private java.math.BigDecimal tdmrPreUseAmtSum;
    
    //이번년도 전통시장 합계금액
    private java.math.BigDecimal tdmrCurrUseAmtSum;
    
    //이번년도 상반기 대중교통 합계금액
    private java.math.BigDecimal tfhyPbtUseAmt;
    
    //이번년도 하반기 대중교통 합계금액
    private java.math.BigDecimal shfyPbtUseAmt;
    

    //2023연말정산_추가-------------------------------
    
    
    
    
    
    
    
    
    public java.math.BigDecimal getCrdcCurrUseAmtSum() {
		return crdcCurrUseAmtSum;
	}

	public java.math.BigDecimal getTdmrPreUseAmtSum() {
		return tdmrPreUseAmtSum;
	}

	public void setTdmrPreUseAmtSum(java.math.BigDecimal tdmrPreUseAmtSum) {
		this.tdmrPreUseAmtSum = tdmrPreUseAmtSum;
	}

	public java.math.BigDecimal getTdmrCurrUseAmtSum() {
		return tdmrCurrUseAmtSum;
	}

	public void setTdmrCurrUseAmtSum(java.math.BigDecimal tdmrCurrUseAmtSum) {
		this.tdmrCurrUseAmtSum = tdmrCurrUseAmtSum;
	}


	public java.math.BigDecimal getTfhyPbtUseAmt() {
		return tfhyPbtUseAmt;
	}

	public void setTfhyPbtUseAmt(java.math.BigDecimal tfhyPbtUseAmt) {
		this.tfhyPbtUseAmt = tfhyPbtUseAmt;
	}


	public java.math.BigDecimal getShfyPbtUseAmt() {
		return shfyPbtUseAmt;
	}

	public void setShfyPbtUseAmt(java.math.BigDecimal shfyPbtUseAmt) {
		this.shfyPbtUseAmt = shfyPbtUseAmt;
	}


	public void setCrdcCurrUseAmtSum(java.math.BigDecimal crdcCurrUseAmtSum) {
		this.crdcCurrUseAmtSum = crdcCurrUseAmtSum;
	}

	public java.math.BigDecimal getCrdcPreUseAmtSum() {
		return crdcPreUseAmtSum;
	}

	public void setCrdcPreUseAmtSum(java.math.BigDecimal crdcPreUseAmtSum) {
		this.crdcPreUseAmtSum = crdcPreUseAmtSum;
	}

	public java.lang.String getDpobCd() {
        return this.dpobCd;
    }
    
    public void setDpobCd(java.lang.String dpobCd) {
        this.dpobCd = dpobCd;
    }
    
    public java.lang.String getYrtxBlggYr() {
        return this.yrtxBlggYr;
    }
    
    public void setYrtxBlggYr(java.lang.String yrtxBlggYr) {
        this.yrtxBlggYr = yrtxBlggYr;
    }
    
    public java.lang.String getClutSeptCd() {
        return this.clutSeptCd;
    }
    
    public void setClutSeptCd(java.lang.String clutSeptCd) {
        this.clutSeptCd = clutSeptCd;
    }
    
    public java.lang.String getSystemkey() {
        return this.systemkey;
    }
    
    public void setSystemkey(java.lang.String systemkey) {
        this.systemkey = systemkey;
    }
    
    public java.lang.String getFamResnoEncCntn() {
        return this.famResnoEncCntn;
    }
    
    public void setFamResnoEncCntn(java.lang.String famResnoEncCntn) {
        this.famResnoEncCntn = famResnoEncCntn;
    }
    
    public java.lang.String getDataCd() {
        return this.dataCd;
    }
    
    public void setDataCd(java.lang.String dataCd) {
        this.dataCd = dataCd;
    }
    
    public java.lang.String getNnfCl() {
        return this.nnfCl;
    }
    
    public void setNnfCl(java.lang.String nnfCl) {
        this.nnfCl = nnfCl;
    }
    
    public java.lang.String getSuptFmlyRltCl() {
        return this.suptFmlyRltCl;
    }
    
    public void setSuptFmlyRltCl(java.lang.String suptFmlyRltCl) {
        this.suptFmlyRltCl = suptFmlyRltCl;
    }
    
    public java.lang.String getSuptFmlyFnm() {
        return this.suptFmlyFnm;
    }
    
    public void setSuptFmlyFnm(java.lang.String suptFmlyFnm) {
        this.suptFmlyFnm = suptFmlyFnm;
    }
    
    public java.lang.String getSuptFmlyBhdt() {
        return this.suptFmlyBhdt;
    }
    
    public void setSuptFmlyBhdt(java.lang.String suptFmlyBhdt) {
        this.suptFmlyBhdt = suptFmlyBhdt;
    }
    
    public java.math.BigDecimal getCrdcUseAmtSum() {
        return this.crdcUseAmtSum;
    }
    
    public void setCrdcUseAmtSum(java.math.BigDecimal crdcUseAmtSum) {
        this.crdcUseAmtSum = crdcUseAmtSum;
    }
    
    public java.math.BigDecimal getCrdcUseAmt() {
        return this.crdcUseAmt;
    }
    
    public void setCrdcUseAmt(java.math.BigDecimal crdcUseAmt) {
        this.crdcUseAmt = crdcUseAmt;
    }
    
    public java.math.BigDecimal getCshptUseAmt() {
        return this.cshptUseAmt;
    }
    
    public void setCshptUseAmt(java.math.BigDecimal cshptUseAmt) {
        this.cshptUseAmt = cshptUseAmt;
    }
    
    public java.math.BigDecimal getDrtpCardUseAmt() {
        return this.drtpCardUseAmt;
    }
    
    public void setDrtpCardUseAmt(java.math.BigDecimal drtpCardUseAmt) {
        this.drtpCardUseAmt = drtpCardUseAmt;
    }
    
    public java.math.BigDecimal getTdmrUseAmt() {
        return this.tdmrUseAmt;
    }
    
    public void setTdmrUseAmt(java.math.BigDecimal tdmrUseAmt) {
        this.tdmrUseAmt = tdmrUseAmt;
    }
    
    public java.math.BigDecimal getBookShowAmt() {
        return this.bookShowAmt;
    }
    
    public void setBookShowAmt(java.math.BigDecimal bookShowAmt) {
        this.bookShowAmt = bookShowAmt;
    }
    
    public java.math.BigDecimal getEtcUseAmt() {
        return this.etcUseAmt;
    }
    
    public void setEtcUseAmt(java.math.BigDecimal etcUseAmt) {
        this.etcUseAmt = etcUseAmt;
    }
    
    public java.lang.String getKybdr() {
        return this.kybdr;
    }
    
    public void setKybdr(java.lang.String kybdr) {
        this.kybdr = kybdr;
    }
    
    public java.lang.String getInptDt() {
        return this.inptDt;
    }
    
    public void setInptDt(java.lang.String inptDt) {
        this.inptDt = inptDt;
    }
    
    public java.lang.String getInptAddr() {
        return this.inptAddr;
    }
    
    public void setInptAddr(java.lang.String inptAddr) {
        this.inptAddr = inptAddr;
    }
    
    public java.lang.String getIsmt() {
        return this.ismt;
    }
    
    public void setIsmt(java.lang.String ismt) {
        this.ismt = ismt;
    }
    
    public java.lang.String getRevnDt() {
        return this.revnDt;
    }
    
    public void setRevnDt(java.lang.String revnDt) {
        this.revnDt = revnDt;
    }
    
    public java.lang.String getRevnAddr() {
        return this.revnAddr;
    }
    
    public void setRevnAddr(java.lang.String revnAddr) {
        this.revnAddr = revnAddr;
    }

	public java.math.BigDecimal getZrtdUseAmt() {
		return zrtdUseAmt;
	}

	public void setZrtdUseAmt(java.math.BigDecimal zrtdUseAmt) {
		this.zrtdUseAmt = zrtdUseAmt;
	}
    
}
