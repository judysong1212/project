package com.app.smrmf.pkg.yeta.yeta2023.file;
...
@Controller
public class Yeta8400PkgFileController {

    private static final Logger logger = LoggerFactory.getLogger(Yeta8400PkgFileController.class);  
    
    @Resource(name = "Yeta8400PayrService")
    private Yeta8400PayrService yeta8400PayrService;
    ...
    public Yeta8400PkgFileController() {
    ...
    * @param ye161010VO
    */
	public int fnYeta8400_Tax_Ye161010_File_Create(HttpServletRequest request,InfcPkgYe161010_2023_VO  ye161010VO,HttpServletResponse response) throws Exception  {

		if  (yeta8400PayrService== null) {
			WebApplicationContext wac = WebApplicationContextUtils.
			getRequiredWebApplicationContext( request.getSession().getServletContext());

			yeta8400PayrService = (Yeta8400PayrService) wac.getBean("Yeta8400PayrService");
		}
...
//    resultString : string;
       ...
       // InfcPkgye161010VO infcPkgye161010VO = new InfcPkgye161010VO(); 
		...
		infcPkgYe161005Vo  = yeta8400PayrService.selectYe161005(ye161010VO);
		...
		try {
			Map mapYetaB = yeta8400PayrService.fnPayrB_ReTax_Total_Select(infcPkgYe161020Vo); 
...
//           Exit;
			...
			List listYetaC = yeta8400PayrService.fnPayrC_ReTax_Total_Select(infcPkgYe161020Vo);
			...
			while ( iter.hasNext() ) { 
               ...
               //  oraQrySelect01.FieldByName('CITY_SEQN').AsString; Format('%.11d', [])
				...
				//2023연말정산_추가 : 청년형장기집합투자증권저축 추가필요
				...
				List listYetaD = yeta8400PayrService.fnPayrD_ReTax_Total_Select(infcPkgYe161020Vo);
				//D 레코드 처리 화면  
				if (listYetaD.size() > 0) { 
				...
				List listYetaE = yeta8400PayrService.fnPayrE_ReTax_Total_Select(infcPkgYe161020Vo);
				...
				if ( listYetaE.size() > 0) { 
					...
					while ( iter03.hasNext() ) { 
						...
						if (listYetaE.size() + 1 > E_RowCnt)  {
							...
							//2023연말정산_추가 : 21년 전통시장 사용금액 추가 필요
							...
							E_RecStr = E_RecStr+   String.format("%010d",mTempData02)  ;	//소비증가분_2023년 전체사용분 : 10BYTE
							
							//2023연말정산_추가 : 22년 전통시장 사용금액 추가 필요
				...
				List listYetaF = yeta8400PayrService.fnPayrF_ReTax_Total_Select(infcPkgYe161020Vo);
				...
				if ( listYetaF.size() > 0) { 
					...
					while ( iter04.hasNext() ) { 
						...
						if (listYetaF.size() + 1 > F_RowCnt)  { //1
							...
							//2023연말정산_추가 : 청년형장기집합투자증권저축 가입일,계약기간 추가 필요
               ...
               List listYetaG = yeta8400PayrService.fnPayrG_ReTax_Total_Select(infcPkgYe161020Vo);
     	  ...
     	  //      DEFIN = "H"; //기부금  
	        ...
	        //    Map<String, Object> mapTotData01 = (Map<String, Object>) iter.next();  
	            ...
	            List listHCData = yeta8400PayrService.fnPayrHC_ReTax_Total_Select(infcPkgYe161020Vo);  
	            ...
	            if (listHCData.size() <= 0) { 
		          ...
		          infcPkgYe161020Vo.setSystemkey(MSFSharedUtils.allowNulls(mapYetaC.get("systemkey")));
	              //I레코드 - 해당연도 기부명세 레코드  
	              List listHDData = yeta8400PayrService.fnPayrHD_ReTax_Total_Select(infcPkgYe161020Vo); 
               ...
               // infcPkgYeta17FileDAO.updateYeta8400ToYe161010_S
              ye161010VO.setSystemkey(MSFSharedUtils.allowNulls(mapYetaC.get("systemkey"))); 
              yeta8400PayrService.updateYeta8400ToYe161010(ye161010VO); 
...
//        TOT_Record.SaveToFile('C:\EOSDATA\' + DEFIN + leftStr(defBUNS_LINS,7) + '.' + rightStr(defBUNS_LINS,3) + '.denc');
  ...
  public int fnYeta8400_Tax_Ye1611010_File_Create(HttpServletRequest request,InfcPkgYe161010_2023_VO  ye161010VO,HttpServletResponse response) throws Exception  {
      ...
      * 2. 처리내용 : 
      * </pre>
      * @Method Name : fnYeta1400_ReTax_Yeta8140_Create
      * @date : Feb 11, 2016
      * @author : leeheuisung
      * @history : 
      *	-----------------------------------------------------------------------
      ...
      * @param ye161010VO
      */
  public int fnYeta8400_Tax_Ye164010_File_Create(HttpServletRequest request,InfcPkgYe161010_2023_VO  ye161010VO,HttpServletResponse response) throws Exception  {
    	...
    	try {
    		...
    		// A레코드 - 의료비지급명세서 레코드
    		List listYetaCA = yeta8400PayrService.fnPayrPSNL_ReTax_CA_Total_Select(infcPkgYe161020Vo);
    		...
    		if  (listYetaCA.size() > 0) { 
    		  ...
    		  //	for (int i = 0 ; i < listYeta1.size() ; i++) { 
    	            ...
    	            List listCAYeta = yeta8400PayrService.fnPayrCA_ReTax_Total_Select(infcPkgYe161020Vo);
    	            ...
    	            Iterator<Map<String, Object>> iter01 = listCAYeta.iterator(); 
      	            ...
      	            Map<String, Object> mapCAYeta = (Map<String, Object>) iter01.next(); 
						...
						이후는 select 결과 vo (infcPkgYeta8140Vo) 이용*/
  ...
  * 2. 처리내용 : 
  * </pre>
  * @Method Name : fnYeta1400_ReTax_Yeta8180_Create
  * @date : 2017. 1. 20.
  * @author : atres-pc
  * @history : 
  *	-----------------------------------------------------------------------
  ...
  * @throws Exception
  */
  public int fnYeta8400_Tax_Ye165010_File_Create(HttpServletRequest request,InfcPkgYe161010_2023_VO  ye161010VO,HttpServletResponse response) throws Exception { 
...
// 	       
// 	    	//B레코드 - 원천징수의무자별 집계레코드 
// 	       Map mapHb = yeta8400PayrService.fnPayrHB_ReTax_Total_Select(ye161010VO);
// 	        
...
//	          mTempDataB =    Long.parseLong( MSFSharedUtils.defaultNulls(mapHb.get("yeta8180Cnt"),"0"));     
...
// 	          
// 	         //C 레코드 - 기부금 조정명세 레코드  
// 	         List listTotData01 = yeta8400PayrService.fnPayrPSNL_ReTax_Total_Select(ye161010VO);
// 	          
//
...
// 	              //SYSTEMKEY 가지고 데이타 가져오기 
//	            //C 레코드 - 기부금 조정명세 레코드  YETA3170
//	            List listHCData = yeta8400PayrService.fnPayrHC_ReTax_Total_Select(ye161010VO);  
//	             
...
// 	             
// 	              //D레코드 - 해당연도 기부명세 레코드  
// 	              List listHDData = yeta8400PayrService.fnPayrHD_ReTax_Total_Select(ye161010VO); 
// 	               
// 	             if (listHCData.size() > 0) { 
//	              
//	              Iterator<Map<String, Object>> iter02 = listHDData.iterator(); 
...
//      } 
  ...
  public List<YetaMessageVO> fnYeta8400_Tax_Ye161010_File_Check(HttpServletRequest request,InfcPkgYe161010_2023_VO  ye161010VO,HttpServletResponse response) throws Exception  {
    ...
    Long mTempData02 = 0L;
      ...
      * @param ye161010VO
      */
  public List<YetaMessageVO> fnYeta8400_Tax_Ye164010_File_Check(HttpServletRequest request,InfcPkgYe161010_2023_VO  ye161010VO,HttpServletResponse response) throws Exception  {
  ...
  * @throws Exception
  */
 public List<YetaMessageVO> fnYeta8400_Tax_Ye165010_File_Check(HttpServletRequest request,InfcPkgYe161010_2023_VO  ye161010VO,HttpServletResponse response) throws Exception { 
