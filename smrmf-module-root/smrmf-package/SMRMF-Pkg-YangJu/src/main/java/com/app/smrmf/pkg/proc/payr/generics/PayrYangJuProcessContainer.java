...
import com.app.smrmf.constc.yangju.ConstYangJu;
 ...
 */
@Controller 
public class PayrYangJuProcessContainer  extends AbstractCustomServiceImpl {
	

	private static final Logger logger = LoggerFactory.getLogger(PayrYangJuProcessContainer.class);
...
//                          }
                       ...
                       //--
                       List<PkgPayrCommCalcVO>  psnlList = payCalculationService.selectYangJuPayrInfoList(payr0250Vo);  
...
//                                                               }
                                                                ...
                                                                if ((new BigDecimal(payCalculationVo.getDilnlazAbnceNumDys())).compareTo(BigDecimal.ZERO) > 0) {
	                                                               ...
	                                                               //  payCalculationVo.getDilnlazAbnceNumDys()   /** column 근태_결근일수 : dilnlazAbnceNumDys */
	                                                                abnceNumDysAmt = (new BigDecimal(payCalculationVo.getDilnlazAbnceNumDys()))
	                                                                                .multiply(payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal(ConstYangJu.PAYR_BASS_WKTM_YG_01)));  // USALY_AMNT_HRWG_QNTY 시급통상임금 
            ...
            //                                                       payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                        	   ...
                                                        	   /***********************************************************************/
                                                        	   /* C0020010공원관리원 00800003A0060000
																	C0020010도로보수 00800001A0060000
																	C0020010하수도준설및검침 00800002A0060000 */
                                                        	   /************************************************************************/
                                                        	   //반장수당        
                                                               //직책별 수당  
                                                                 if (("00800003A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) ||
                                                                		 "00800001A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) ||
                                                                				 "00800002A0060000".equals(payCalculationVo.getDtilOccuInttnCd())
                                                                		 ) && "A0150003".equals(payCalculationVo.getOdtyCd())) {
	                                                                      ...
	                                                                      if (ExtermsProps.getProps("PAY_EMYMT_PYMT_07").equals(payr0410.getPayrImcd())) {
	                                                                       ...
	                                                                       // 야간수당       -- 시간산정C9410300 MNTH_USALY_AMNT_WAG
	                                                                          if (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotNtotTm(),"0")).compareTo(BigDecimal.ZERO) > 0) {
	                                                                        	  ...
	                                                                        	  BigDecimal ucstSum =  BigDecimal.ZERO; 
                                                                                  ucstSum = (payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal("2"))).setScale(5,BigDecimal.ROUND_DOWN);  //payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum());
                                                                              
                                                                                if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
                                                                              	  ucstSum =  (payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal("2"))).setScale(5,BigDecimal.ROUND_DOWN);   //payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum();
                                                                                } 
                     ...
                     //권한적용 루틴 추가 
                      ...
                      // payr0250Vo.setDpobCd(sessionUser.getDpobCd());  //사업장코드
                       ...
                       if (psnlList.size() > 0) { 
                                   ...
                                   //기간제근로자 
                                   payr0300Vo.setDpobCd(payr0250Vo.getDpobCd()); //사업장코드 z
                                   payr0300Vo.setPayCd(payr0250Vo.getPayCd());  //급여구분코드 
                                   payr0300Vo.setPayrMangDeptCd(payr0250Vo.getPayrMangDeptCd()) ;//단위기관코드 
                                   payr0300Vo.setPymtYrMnth(payr0250Vo.getJobYrMnth()); 
                                   payr0300Vo.setPayrSeilNum(payr0250Vo.getPayScduSeilNum()); //일련번호 
