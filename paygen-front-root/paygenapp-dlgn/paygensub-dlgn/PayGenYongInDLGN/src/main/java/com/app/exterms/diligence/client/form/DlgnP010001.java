		...
		MessageBox.confirm("근태 생성", "복무, 시간외 근무 기간을 동일하게 생성합니다.<br>(등록되어있는 근태가 있을시 삭제 처리 됩니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					final String year = DlgnUtils.getSelectedComboValue(crePayYr,"year");
					final String month = DlgnUtils.getSelectedComboValue(crePayMonth,"month");
					final String payCd = DlgnUtils.getSelectedComboValue(crePayCd,"commCd");
					final String workDayS01 = DlgnUtils.getConvertDateToString(creWorkDayS01, "yyyyMMdd");
					final String workDayE01 = DlgnUtils.getConvertDateToString(creWorkDayE01, "yyyyMMdd"); 
					final String workDayS02 = DlgnUtils.getConvertDateToString(creWorkDayS02, "yyyyMMdd");
					final String workDayE02 = DlgnUtils.getConvertDateToString(creWorkDayE02, "yyyyMMdd"); 
					...
					final int weekDifference = endWeek - startWeek;
					//주휴일수 계산 끝
					
					final String deptCd = DlgnUtils.getStrValToBMMultiCombo(lsDeptCd,creDeptCd.getValue(),"deptNmRtchnt","deptCd");  
					final String payrMangDeptCd = DlgnUtils.getSelectedComboValue(crePayrMangDeptCd,"payrMangDeptCd");
					
					final String typOccuCd = DlgnUtils.getStrValToBMMultiCombo(lsTypOccuCd,creTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
					final String dtilOccuInttnCd = DlgnUtils.getStrValToBMMultiCombo(lsDtilOccuInttnCd,creDtilOccuInttnCd.getValue(),"dtilOccuClsNm","dtilOccuInttnCd");  
					...
					}else if(GregorianCalendar.compareDate(creWorkDayS01.getValue(), creWorkDayE01.getValue()) > 31
							&& GregorianCalendar.compareDate(creWorkDayS02.getValue(), creWorkDayE02.getValue()) <= 31){
						
						MessageBox.confirm("근태생성", "복무기간이 31일(1개월)을 초과합니다.<br>그래도 생성하시겠습니까?",new Listener<MessageBoxEvent>(){
							@Override
							public void handleEvent(MessageBoxEvent be) {
								if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
...
//												MessageBox.info("", "근태가 생성 되었습니다.", null);
					...
					MessageBox.confirm("근태생성", "시간외근무기간이 31일(1개월)을 초과합니다.<br>그래도 생성하시겠습니까?",new Listener<MessageBoxEvent>(){
						@Override
						public void handleEvent(MessageBoxEvent be) {
							if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
								Dlgn0100DTO dlgn0100Dto = new Dlgn0100DTO();
...
//											MessageBox.info("", "근태가 생성 되었습니다.", null);
					...
					MessageBox.confirm("근태생성", "복무기간, 시간외근무기간이 31일(1개월)을 초과합니다.<br>그래도 생성하시겠습니까?",new Listener<MessageBoxEvent>(){
						@Override
						public void handleEvent(MessageBoxEvent be) {
							if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
								Dlgn0100DTO dlgn0100Dto = new Dlgn0100DTO();
