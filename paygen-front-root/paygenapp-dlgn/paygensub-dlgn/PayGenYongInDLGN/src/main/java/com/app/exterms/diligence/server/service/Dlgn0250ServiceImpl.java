        ...
        try {  
           ...
           if  (MSFSharedUtils.paramNull(dlgn0110DAO)) {
            ...
            for(int iPayrCnt=0;iPayrCnt < listDlgn0250dto.size();iPayrCnt++) {
                    ...
                    dlgn0200Vo.setDilnlazPubcHodyDutyNumDys(MSFSharedUtils.defaultNulls(dlgn0250Dto.getDilnlazPubcHodyDutyNumDys(),"0"));    /** column 근태_휴일근무일수 : dilnlazPubcHodyDutyNumDys */
...
//					infcDlgn0100Vo.setResnRegnNum(rrnDecCrypt);
				...
				infcDlgn0100Vo.setDpobCd(dlgn0100Vo.getDpobCd());								/** column 사업장코드 : dpobCd */
				infcDlgn0100Vo.setSystemkey(dlgn0100Vo.getSystemkey());							/** column SYSTEMKEY : systemkey */
				infcDlgn0100Vo.setDilnlazYrMnth(dlgn0100Vo.getDilnlazYrMnth());					/** column 근태년월 : dilnlazYrMnth */
				infcDlgn0100Vo.setPayCd(dlgn0100Vo.getPayCd());									/** column 급여구분코드 : payCd */
				infcDlgn0100Vo.setEmymtDivCd(dlgn0100Vo.getEmymtDivCd());						/** column 고용구분코드 : emymtDivCd */
				infcDlgn0100Vo.setDeptCd(dlgn0100Vo.getDeptCd());								/** column 부서코드 : deptCd */
				infcDlgn0100Vo.setBusinCd(dlgn0100Vo.getBusinCd());								/** column 사업코드 : businCd */
				infcDlgn0100Vo.setTypOccuCd(dlgn0100Vo.getTypOccuCd());							/** column 직종코드 : typOccuCd */
				infcDlgn0100Vo.setPyspGrdeCd(dlgn0100Vo.getPyspGrdeCd());						/** TODO 추가후 수정 column 호봉등급코드 : pyspGrdeCd */
				infcDlgn0100Vo.setDtilOccuInttnCd(dlgn0100Vo.getDtilOccuInttnCd());				/** column 직종세구분코드 : dtilOccuInttnCd */
				infcDlgn0100Vo.setDilnlazWklyHldyNumDys(dlgn0100Vo.getDilnlazWklyHldyNumDys()); // 주휴일수
				 ...
				 ****************************************************************************************************/
				//TODO 2017.04.07
				// 근무실적관리로 근태 생성 시 기간제근로자도 근무일수가 총일수로 생성됨.
				// 해당 모듈 적용 하면 기간제도 맞게 생성되는데, 적용되어 있지 않음.
				// 확인 후 적용하는 것이 맞다면 주석 풀고 반영 필요.
...
//				Long iResult = dlgnAdapter.setUpdateManageDlgn0100CommuteService(RemoteServiceUtil.getThreadLocalRequest(), infcDlgn0100Vo);
			...
			throw MSFServerUtils.getOperationException(
					"createDlgn0250WorkPerfm", ex, logger);
		} finally {
