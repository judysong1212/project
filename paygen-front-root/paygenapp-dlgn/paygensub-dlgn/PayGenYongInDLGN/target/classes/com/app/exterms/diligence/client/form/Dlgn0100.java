...
public class Dlgn0100 extends MSFPanel {
 ...
 // --------- gird,Def 선언 시작 ----------
 	private MSFGridPanel Dlgn0100Grid0110;	//근태관리대상자정보
 	private Dlgn0100Def Dlgn0100Def = new Dlgn0100Def("DLGN0100"); // 근무내역등록 GRID
 	// --------- grid,Def 선언 종료 ----------
...
//							ArrayList<Dlgn0100DTO> changeDataList = new ArrayList<Dlgn0100DTO>();
//							
//							for ( int i = 0; i < storeDlgn0110.getCount(); i++){
//								
//								if(dlgn0110Grid.getStore().getRecord(storeDlgn0110.getAt(i)).isDirty()){
//									
//									changeDataList.add(storeDlgn0110.getAt(i));
//									if (storeDlgn0110.getAt(i).getDilnlazAbnceYnboolean() ) {
//										storeDlgn0110.getAt(i).setDilnlazDutyTm("0");
//									}
//									else if (!storeDlgn0110.getAt(i).getDilnlazAbnceYnboolean() && !storeDlgn0110.getAt(i).getDilnlazAbnceDutyRcgtnYnboolean() 
//											&& !storeDlgn0110.getAt(i).getDilnlazLvsgYnboolean() && !storeDlgn0110.getAt(i).getDilnlazSpclHodyYnboolean() 
//											&& !storeDlgn0110.getAt(i).getDilnlazSckleaYnboolean() ){
//										//storeDlgn0110.getAt(i).setDilnlazDutyTm("8");
//									}
//									
//									storeDlgn0110.getAt(i).setDilnlazWklyHldyNumDys(storeDlgn0110.getAt(i).getDilnlazWklyHldyNumDys()); //기존 휴일수
//									storeDlgn0110.getAt(i).setInptDilnlazWklyHldyNumDys(detailWeekHdayCnt.getValue()); //입력받은 휴일수
//									
...
//					            
//					            MessageBox.alert("기본근무정보", "저장할 기본근무정보가 존재하지 않습니다.", null);
//					            return;
//					         }
//							
//							if(changeDataList.size() > 0){
//								dlgn0100Service.saveWorkTime(changeDataList, new AsyncCallback<Integer>() {
//									@Override
//									public void onFailure(Throwable caught) {
//										caught.printStackTrace();
...
//		buttonBar.add(topBtn);
		...
		plFrmDlgn0100.setSize("990px", "690px");  // ******************** 이전로직 : 분단위일경우 멘트 추가 : 705px로 수정
	}
...
//	 			dlgn0110dto.setPayCd(DlgnUtils.getSelectedComboValue(crePayCd,"commCd"));			/** column 급여구분 : payCd */
	 			...
	 			dlgn0110dto.setEmymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("emymtDivCd")));    /** column 고용구분코드 : emymtDivCd */
...
//		layoutContainer.add(lc4,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.27));
		...
		Button btnDlgn0100Upload = new Button("실적업로드");
		...
		Button btnDlgn0100DownLoad = new Button("양식다운로드");
...
//				"* 근무시간 입력방식( ex : 8시간일 경우 8:00 , 5시간 35분일 경우 5:35 )와 같이 입력해주세요.");
...
//		cp01.setSize(320, 380);
		...
		dlgn0100Grid.getSelectionModel().addSelectionChangedListener(new SelectionChangedListener<Dlgn0100DTO>() {
			...
			@Override
			public void selectionChanged(SelectionChangedEvent<Dlgn0100DTO> se) {
...
//					Dlgn0100Grid0110.getTableDef().addColumnFilter("offday", MSFSharedUtils.allowNulls(bllc.get("getOffdayKubn")), SimpleColumnFilter.OPERATOR_EQUALS);
