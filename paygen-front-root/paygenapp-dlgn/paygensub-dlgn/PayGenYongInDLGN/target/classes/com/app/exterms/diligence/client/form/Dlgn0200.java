...
//	//중간 근태생성 부분 끝
	...
	// --------- gird,Def 선언 시작 ----------
	private MSFGridPanel Dlgn0200Grid0210;	//근태관리대상자정보
	private Dlgn0200Def Dlgn0200Def = new Dlgn0200Def("DLGN0200"); //시간외근무관리 근무내역등록 GRID
	// --------- grid,Def 선언 종료 ----------
...
//			dto.setResnRegnNum(searchResnRegnNum.getValue());
	...
	private Grid<Dlgn0200DTO> dlgn0200Grid;
	...
	private EditorGrid<Dlgn0200DTO> dlgn0210Grid;
...
//			});
	...
	private void createDlgn0200Form() {
		...
		plFrmDlgn0200.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 초과근무관리"));
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				 //로그 뷰화면 호출 메서드 
				  funcLogMessage("초과근무관리","DLGN0200");
				}
			});
...
//							}
		...
		topBtn.addListener(Events.Select, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				MessageBox.confirm("복무관리대상자 삭제", "선택하시 초과근무관리 대상자를 삭제 하시겠습니까? [기본근무내역도 삭제됩니다.]",new Listener<MessageBoxEvent>(){
					@Override
					public void handleEvent(MessageBoxEvent be) {
					//	if("Yes".equals(be.getButtonClicked().getText())){
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
...
//		buttonBar.add(topBtn);
		...
		plFrmDlgn0200.setSize("990px", "690px"); // ******************** 이전로직 : 분단위일경우 멘트 추가 : 730px로 수정
	}
...
//		layoutContainer.add(lc4,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.27));
		...
		*/
		...
		Button btnDlgn0200Upload = new Button("실적업로드");
...
//				Dlgn0200Grid0210.reload(); //TODO 그리드 reload 위치 다시.
		...
		Button btnDlgn0200DownLoad = new Button("양식다운로드");
...
//				"* 근무내역등록 수정 할 때 시간은 무조건 09:00 형식으로 입력하세요. 9:00 형식은 수정 안됩니다.");
...
//	             return "<div style=\"overflow: visible; white-space:normal !important;\">"+ val +" </div>";
	...
	private LayoutContainer occupationalRBottom() {
		...
		Dlgn0200Grid0210.getGrid().getColumnModel().addHeaderGroup(0, 6, new HeaderGroupConfig("야근근무(시:분)", 1, 2));
...
//	   GridCellRenderer<BaseModel> cellRender = new GridCellRenderer<BaseModel>() {   // 기존것 보고 안에 수정
//			public Object render(Dlgn0200DTO model, String property,
//					ColumnData config, int rowIndex, int colIndex,
//					ListStore<Dlgn0200DTO> store, Grid<Dlgn0200DTO> grid) {
...
//		column.setRenderer(new GridCellRenderer<Dlgn0200DTO>() {
//
//			@Override
//			public Object render(Dlgn0200DTO model, String property,
//					ColumnData config, int rowIndex, int colIndex,
...
//		
//		
//		CheckColumnConfig checkColumn = new CheckColumnConfig("dilnlazDutyYnboolean", "연장", 50);
...
//		
//		
//		MSFTextField  msfTxtDilnlazDutyTm = new MSFTextField();
//		msfTxtDilnlazDutyTm.setSelectOnFocus(true);
//		
//		column = new ColumnConfig();
//		column.setId("dilnlazDutyTm");
//		column.setHeaderText("연장근무(시:분)");
//		column.setAlignment(HorizontalAlignment.CENTER);
//		column.setWidth(90);
//		column.setEditor(new CellEditor(msfTxtDilnlazDutyTm));
//		column.setMenuDisabled(true);
//		column.setSortable(false);
//		configs.add(column);
//
//		// 콤보로처리 결근/결근인정/무단결근
//		checkColumn = new CheckColumnConfig("dilnlazNtotYnboolean", "야간", 50);
//		checkColumn.setAlignment(HorizontalAlignment.CENTER);
//		checkColumn.setMenuDisabled(true);
//		checkColumn.setSortable(false);
//		checkColumn.setHidden(true);
...
//		configs.add(checkColumn);
//		
//		MSFTextField  msfTxtDilnlazNtotTm = new MSFTextField();
//		msfTxtDilnlazNtotTm.setSelectOnFocus(true);
//		
//		column = new ColumnConfig();
//		column.setId("dilnlazNtotTm");
//		column.setHeaderText("야간근무(시:분)");
//		column.setAlignment(HorizontalAlignment.CENTER);
//		column.setWidth(90);
//		column.setEditor(new CellEditor(msfTxtDilnlazNtotTm));
//		column.setMenuDisabled(true);
//		column.setSortable(false);
//		configs.add(column);
//		
//
//		//------------------------------------
//		 checkColumn = new CheckColumnConfig("dilnlazHodyDutyYnBoolean", "휴일", 50);
//		checkColumn.setAlignment(HorizontalAlignment.CENTER);
//		checkColumn.setMenuDisabled(true);
//		checkColumn.setSortable(false);
//		checkColumn.setHidden(true);
...
//		configs.add(checkColumn);
//		
//		MSFTextField  msfTxtDilnlazHodyDutyTm = new MSFTextField();
//		msfTxtDilnlazHodyDutyTm.setSelectOnFocus(true);
//		
//		
//		column = new ColumnConfig();
//		column.setId("dilnlazHodyDutyTm");
//		column.setHeaderText("휴일근무(시:분)");
//		column.setAlignment(HorizontalAlignment.CENTER);
//		column.setWidth(90);
//		column.setEditor(new CellEditor(msfTxtDilnlazHodyDutyTm));
//		column.setMenuDisabled(true);
//		column.setSortable(false);
//		configs.add(column);
//		//------------------------
//		
//		//------------------------------------ TODO 휴일연장 변수명 재정의
//		checkColumn = new CheckColumnConfig("dilnlazHodyDutyYnBooleanA", "휴일연장", 50);
//		checkColumn.setAlignment(HorizontalAlignment.CENTER);
//		checkColumn.setMenuDisabled(true);
//		checkColumn.setSortable(false);
//		checkColumn.setHidden(true);
...
//		configs.add(checkColumn);
//		
//		MSFTextField  msfTxtDilnlazHodyDutyTmA = new MSFTextField();
//		msfTxtDilnlazHodyDutyTmA.setSelectOnFocus(true);
//		
//		
//		column = new ColumnConfig();
//		column.setId("dilnlazHodyDutyTmA");
//		column.setHeaderText("휴일연장근무(시:분)");
//		column.setAlignment(HorizontalAlignment.CENTER);
//		column.setWidth(120);
//		column.setEditor(new CellEditor(msfTxtDilnlazHodyDutyTmA));
//		column.setMenuDisabled(true);
//		column.setSortable(false);
//		configs.add(column);
//		//------------------------
...
//		
//		
//		MSFTextField  msfTxtField = new MSFTextField();
//		msfTxtField.setReadOnly(true);
//		
//		
//		column = new ColumnConfig();
//		column.setId("msfTxtField");
//		column.setHeaderText("");
//		column.setAlignment(HorizontalAlignment.CENTER);
//		column.setWidth(200);
//		column.setEditor(new CellEditor(msfTxtField));
//		column.setMenuDisabled(true);
//		column.setSortable(false);
//		configs.add(column);
//		
...
//		cp01.setHeaderVisible(false);
//		cp01.setLayout(new FitLayout());
////		cp01.setSize(600, 255);
//		cp01.setSize(610, 255);
//
//		dlgn0210Grid = new EditorGrid<Dlgn0200DTO>(storeDlgn0210, cm);
//		dlgn0210Grid.setStateful(false);
//		//dlgn0210Grid.setSelectionModel(sm);
//		dlgn0210Grid.setBorders(true);
//		//dlgn0210Grid.setColumnReordering(true);
//		dlgn0210Grid.setLoadMask(true);
//		dlgn0210Grid.addPlugin(checkColumn);
//		dlgn0210Grid.setAutoExpandColumn("dilnlazApptnDt");
//		dlgn0210Grid.addListener(Events.AfterEdit, new Listener<GridEvent<Dlgn0200DTO>>() {
//			@Override
//			public void handleEvent(GridEvent<Dlgn0200DTO> be) {
//				/*
//				if(be.getColIndex() == 2){ //근무시간 수정시
//					String val = be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).getDilnlazDutyTm();
//					double changVal = 0;
//					try{
//						changVal = Double.parseDouble(val);
//					}catch(Exception e){
//						changVal = 0;
//					}
//					be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).setDilnlazDutyTm(changVal+"");
//				}else if(be.getColIndex() == 3 || be.getColIndex() == 4 || be.getColIndex() == 5){
//					
//					System.out.println(be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).getDilnlazAbnceYnboolean());
//					if(be.getColIndex() == 3 && be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).getDilnlazAbnceYnboolean()){
//						be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).setDilnlazDutyTm("0");
//					}
//					if(be.getColIndex() == 4 && be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).getDilnlazLvsgYnboolean()){
//						be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).setDilnlazDutyTm("0");
//					}
//					if(be.getColIndex() == 5 && be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).getDilnlazSckleaYnboolean()){
//						be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).setDilnlazDutyTm("0");
//					}
//				}
//				
//				be.getGrid().getStore().commitChanges(); //그리드 업데이트
//				//업데이트 한 그리드 수정했다는 내용 처리
//				be.getGrid().getStore().getRecord(be.getGrid().getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem()))).setDirty(true);
//				*/
//			}
//		});
//		cp01.add(dlgn0210Grid);
//
//		lcTabFormLayer.add(cp01);
//	
...
//					Dlgn0200Grid0210.getTableDef().addColumnFilter("detailYm", MSFSharedUtils.allowNulls(DateTimeFormat.getFormat("yyyyMM").parse(detailYm.getValue())), SimpleColumnFilter.OPERATOR_EQUALS);
					...
					Dlgn0200Grid0210.getTableDef().addColumnFilter("detailEmymtDivCd",MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd, "commCd"),SimpleColumnFilter.OPERATOR_EQUALS);
