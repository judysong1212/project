...
//				dto.setPayrMangDeptCd(DlgnUtils.getSelectedComboValue(srhPayrMangDeptCd,"mangeDeptCd"));
...
//			});
			...
			/*
			dlgn0100Service.getPayrMangDeptCd(new Dlgn0100DTO(), new AsyncCallback<List<BaseModel>>() {
				@Override
				public void onSuccess(List<BaseModel> result) {
					listStoreBass0400All.add(result);
				}
...
//			 			dlgn0110dto.setEmymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("detailEmymtDivCd")));    /** column 고용구분코드 : emymtDivCd */
								...
								dlgn0110dto.setTypOccuCd(MSFSharedUtils.allowNulls(bmMapModel.get("typOccuCd")));    					/** column 직종코드 : typOccuCd */
			 					...
			 					dlgn0110dto.setTypOccuBusinVal(MSFSharedUtils.allowNulls(bmMapModel.get("typOccuBusinVal"))); 			/** column 복무통합구분값 : typOccuBusinVal */
			 					dlgn0110dto.setDilnlazWklyHldyNumDys(weekDifference+""); 												/** column 주휴일수 dilnlazWklyHldyNumDys */
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							if(Dlgn0100Grid0110.getMsfGrid().getGrid().getStore().getModifiedRecords().size() < 1 ) {
								...
								dlgn0110dto.setDilnlazYrMnth(detailYm.getValue().replaceAll("\\.",""));									/** column 근무년월 : detailYm */
								dlgn0110dto.setDilnlazWklyHldyNumDys(MSFSharedUtils.allowNulls(detailWeekHdayCnt.getValue())); 			/** column 주휴일수 */
								
								
								String typOccuBusinVal = "";
								// 무기계약직(A0020010)일 경우 직종세 통합코드, 기간제(A0020020)일 경우 부서 + 사업코드로 넣어줌.
								if(MSFConfiguration.EMYMT_DIVCD01.equals(MSFSharedUtils.allowNulls(detailEmymtDivCd.getValue()))) {
									typOccuBusinVal =  MSFSharedUtils.allowNulls(dlgn0110dto.getDtilOccuInttnCd());
									dlgn0110dto.setTypOccuBusinVal(typOccuBusinVal); 			/** column 복무통합구분값 : typOccuBusinVal */
								}else if(MSFConfiguration.EMYMT_DIVCD02.equals(MSFSharedUtils.allowNulls(detailEmymtDivCd.getValue()))) {
									typOccuBusinVal =  MSFSharedUtils.allowNulls(dlgn0110dto.getDeptCd()) + MSFSharedUtils.allowNulls(dlgn0110dto.getBusinCd());
									dlgn0110dto.setTypOccuBusinVal(typOccuBusinVal); 			/** column 복무통합구분값 : typOccuBusinVal */
								}
...
//		layoutContainer.add(lc4,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.27));
		...
		layoutContainer.add(lc1,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.3));	// 급여구분
        ...
        lcSchCol3.add(lc4,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.27));				// 출력조건
...
//					Dlgn0100Grid0110.getTableDef().addColumnFilter("detailEmymtDivCd",MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd, "commCd"),SimpleColumnFilter.OPERATOR_EQUALS);
					...
					// 무기계약직(A0020010)일 경우 직종세 통합코드, 기간제(A0020020)일 경우 부서 + 사업코드로 넣어줌.
					if(MSFConfiguration.EMYMT_DIVCD01.equals(MSFSharedUtils.allowNulls(detailEmymtDivCd.getValue()))) {
						typOccuBusinVal =  MSFSharedUtils.allowNulls(se.getSelectedItem().getDtilOccuInttnCd());
						Dlgn0100Grid0110.getTableDef().addColumnFilter("typOccuBusinVal", typOccuBusinVal, SimpleColumnFilter.OPERATOR_EQUALS);
					}else if(MSFConfiguration.EMYMT_DIVCD02.equals(MSFSharedUtils.allowNulls(detailEmymtDivCd.getValue()))) {
						typOccuBusinVal =  MSFSharedUtils.allowNulls(se.getSelectedItem().getDeptCd()) + MSFSharedUtils.allowNulls(se.getSelectedItem().getBusinCd());
						Dlgn0100Grid0110.getTableDef().addColumnFilter("typOccuBusinVal", typOccuBusinVal, SimpleColumnFilter.OPERATOR_EQUALS);
					}
...
//					Dlgn0100Grid0110_1.getTableDef().addColumnFilter("detailEmymtDivCd",MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd, "commCd"),SimpleColumnFilter.OPERATOR_EQUALS);
					...
					if(MSFConfiguration.EMYMT_DIVCD01.equals(MSFSharedUtils.allowNulls(detailEmymtDivCd.getValue()))) {
						typOccuBusinVal =  MSFSharedUtils.allowNulls(se.getSelectedItem().getDtilOccuInttnCd());
						Dlgn0100Grid0110_1.getTableDef().addColumnFilter("typOccuBusinVal", typOccuBusinVal, SimpleColumnFilter.OPERATOR_EQUALS);
					}else if(MSFConfiguration.EMYMT_DIVCD02.equals(MSFSharedUtils.allowNulls(detailEmymtDivCd.getValue()))) {
						typOccuBusinVal =  MSFSharedUtils.allowNulls(se.getSelectedItem().getDeptCd()) + MSFSharedUtils.allowNulls(se.getSelectedItem().getBusinCd());
						Dlgn0100Grid0110_1.getTableDef().addColumnFilter("typOccuBusinVal", typOccuBusinVal, SimpleColumnFilter.OPERATOR_EQUALS);
					}
...
//	                         dto.setSystemkey(srhSystemkey.getValue());			
							 ...
							 // 무기계약직(A0020010)일 경우 직종세 통합코드, 기간제(A0020020)일 경우 부서 + 사업코드로 넣어줌.
							 if(MSFConfiguration.EMYMT_DIVCD01.equals(dto.getEmymtDivCd())) {
								 dto.setTypOccuBusinVal(dto.getDtilOccuInttnCd());
							 }else if(MSFConfiguration.EMYMT_DIVCD02.equals(dto.getEmymtDivCd())) {
								 dto.setTypOccuBusinVal( dto.getDeptCd() + dto.getBusinCd());
							 }
...
//	            	   dto.setSystemkey(srhSystemkey.getValue());
	 ...
	 **/
	  private void xlsExportData() {
			...
			param.put("payrMangDeptCd",  DlgnUtils.getSelectedComboValue(crePayrMangDeptCd, "payrMangDeptCd")); 				//단위기관
