...
public class Dlgn0100ServiceImpl extends AbstractCustomServiceImpl implements Dlgn0100Service {
	...
	private String detailYmKubn = DiligenceCommonConstants.GWANGMYEONG_DLGN_DT_BUNGI;
...
//	        	}  
	        	...
	        	logger.debug("무기,기간제generateWorkTime메소드");
	...
	//		            dlgn0100Dao.`Dlgn0100(dto);
			            if((detailYmKubn.compareTo(dto.getDilnlazYrMnth().replaceAll("\\.", "")) >= 0 )){ //기준일 전
				            ...
				            logger.debug("무기계약직,기간제insert1");
				            if("A0020010".equals(map.get("emymtDivCd"))){ //무기계약직일 경우
				            	
				            	logger.debug("무기계약직insert1");
				            	dlgn0100Dao.insertDlgn0110Pre(dto);
				            	
				            }else{ //기간제일경우
				            	logger.debug("기간제insert");
				            	dlgn0100Dao.insertDlgn0110Pre2(dto);
				          ...
				          //인서트 처리 
				            dlgn0100Dao.insertDlgn0100(dto);
				            logger.debug("무기계약직,기간제insert2");
				            if("A0020010".equals(map.get("emymtDivCd"))){ //무기계약직일 경우
				            	logger.debug("무기계약직insert2");
				            	dlgn0100Dao.insertDlgn0110(dto);
				            	
				            }else{ //기간제일경우
				            	logger.debug("기간제insert2");
