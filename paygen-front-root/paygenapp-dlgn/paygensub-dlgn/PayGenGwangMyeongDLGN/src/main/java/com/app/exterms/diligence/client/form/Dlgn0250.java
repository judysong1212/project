 ...
 **/
	...
	//  private FormData formData; 
      ...
      private Button btnDlgn0251Print;
      /**
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       * 검색변수 선언 
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
//                      }
	   ...
	   //신규 
        ...
        btnDlgn0250Del.addListener(Events.Select, new Listener<ButtonEvent>() {
	  	...
	  	btnDlgn0251Print = new Button("근로명부");
	  	btnDlgn0251Print.setIcon(MSFMainApp.ICONS.print16());
	  	topDlgn0250Bar.add(btnDlgn0251Print);
	  	btnDlgn0251Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
