...
import com.app.exterms.diligence.client.service.Dlgn0230ServiceAsync;
 ...
 * =====================================================
 **/
public class Dlgn0230 extends MSFPanel {
    ...
    private MSFDateField creDilnlazDutyBgnnDt;      		//근태_근무시작일자
    private MSFDateField creDilnlazDutyEndDt;      			//근태_근무종료일자
    private MSFDateField creDilnlazExceDutyBgnnDt;      	//근태_초과근무시작일자
    private MSFDateField creDilnlazExceDutyEndDt;      		//근태_초과근무종료일자
    ...
    private ComboBox<BaseModel> creDlgnType;				//복무구분
    // -- 중간 근태생성 부분 종료
...
//	private TextField<String> detailDilnlazDutyBgnnDt;		//근태 시작기간
//	private TextField<String> detailDilnlazDutyEndDt;		//근태 종료기간
	private DateField detailDilnlazDutyBgnnDt;				//근태 시작기간
	private DateField detailDilnlazDutyEndDt;				//근태 종료기간
    private DateField detailDilnlazExceDutyBgnnDt;			//근태_초과근무시작일자
    private DateField detailDilnlazExceDutyEndDt;			//근태_초과근무종료일자
...
//  	private String detailYmKubn = "201703"; //복무관리 기준일 property에서 받을 부분.
	...
	private TabPanel tabsDlgn0230 = new TabPanel();		// 전체
...
//	private Dlgn0200ServiceAsync 		dlgn0210Service 		= Dlgn0200Service.Util.getInstance();
	private Dlgn0230ServiceAsync 		dlgn0230Service 		= Dlgn0230Service.Util.getInstance();
//	private Dlgn0200ServiceAsync 		dlgn0200Service 		= Dlgn0200Service.Util.getInstance();
	...
	// --------- gird,Def 선언 시작 ----------
 	...
 	private MSFGridPanel Dlgn0100MemberGrid;									//근태관리대상자정보
 	private Dlgn0100Def Dlgn0100MemDef = new Dlgn0100Def("DLGN0230_Member"); 	//근태관리대상자정보
	
 	private MSFGridPanel Dlgn0100Grid0110;										//근태관리대상자정보(기준일 후)
 	private Dlgn0100Def Dlgn0100Def = new Dlgn0100Def("DLGN0230"); 				// 근무내역등록 GRID(기준일 후)
	
	private MSFGridPanel Dlgn0200Grid0210;										//근태관리대상자정보(기준일 후)
	private Dlgn0200Def Dlgn0200Def = new Dlgn0200Def("DLGN0230");				//시간외근무관리 근무내역등록 GRID (기준일 후)
	// --------- grid,Def 선언 종료 ----------
	...
	//  private static MessageBox authBox = MessageBox.wait("화면로딩", "설정 초기화 작업 진행 중입니다....", "설정 작업 진행 중..."); 
	   ...
	   * AuthAction 
	   */
	  private void checkDlgn0230Auth(String authAction, ListStore<BaseModel> bm) {   
                 ...
                 setInitDate();
	              ...
	              /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	                ...
	                * 값순서 : 단위기관 ,고용구분, 부서, 직종, 사업 순으로  없으면 ""
	                ...
	                // GWTAuthorization.formAuthPopConfig(plFrmDlgn0230, "srhPayrMangDeptCd","srhEmymtDivCd",strArrDeptCd,"srhTypOccuCd","srhBusinCd");
	                // GWTAuthorization.formAuthBtnConfig(btnSrhDeptCd); 
			    //  gwtAuthorization.formAuthConfig(plFrmDlgn0230, "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhDtilOccuInttnCd","srhBusinCd");
	               /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	                ...
	                srhDeptCd.getListView().fireEvent(Events.CheckChanged); 
	                  ...
	                  /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	                     ...
	                     * 값순서 : 단위기관 ,고용구분, 부서, 직종, 사업 순으로  없으면 ""
	                     ...
	                     // GWTAuthorization.formAuthPopConfig(plFrmDlgn0230, "crePayrMangDeptCd","creEmymtDivCd",strArrDeptCd,"creTypOccuCd","creBusinCd");
	                     // GWTAuthorization.formAuthBtnConfig(btnCreCurrDeptCd); 
	             //  gwtAuthorization.formAuthConfig(plFrmDlgn0230,  "crePayrMangDeptCd","creEmymtDivCd","creDeptCd","creTypOccuCd","creDtilOccuInttnCd","creBusinCd" );
...
//					 authMapDef.put("midCurrDeptCd",Boolean.FALSE); 
	...
	private FormPanel plFrmDlgn0230;
	...
	public ContentPanel getViewPanel() {
		if (panel == null) {
			...
			uploadDivCdBaseModel.set("commCd", "DLGN01");
			uploadDivCdBaseModel.set("commCdNm", "기본");
			lsUploadDivCd.add(uploadDivCdBaseModel);
			
			uploadDivCdBaseModel = new BaseModel(); 
			uploadDivCdBaseModel.set("commCd", "DLGN02");
			uploadDivCdBaseModel.set("commCdNm", "초과");
			lsUploadDivCd.add(uploadDivCdBaseModel);
			//--------------------업로드구분 코드 생성 --------------------------------------------------	
			...
			vp.setSpacing(10);
			createDlgn0230Form();	// 화면 기본정보를 설정
	...
	public Dlgn0230(String txtForm) {
		this.txtForm = txtForm;
	}

	private void createDlgn0230Form() {
		...
		plFrmDlgn0230 = new FormPanel();
		plFrmDlgn0230.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 복무통합관리"));
		plFrmDlgn0230.setPadding(2);
		plFrmDlgn0230.setIcon(Resources.APP_ICONS.text());
		plFrmDlgn0230.setBodyStyleName("pad-text");
		plFrmDlgn0230.setFrame(true);
		...
		plFrmDlgn0230.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmDlgn0230.getHeader().addTool(btnlogView); 
		/************************************************************************
		...
		//멀티콤보박스 닫기 
		plFrmDlgn0230.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		topBtnDel.addListener(Events.Select, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				MessageBox.confirm("복무 삭제", "선택하시 근로자의 복무 내역을 삭제 하시겠습니까?[기본 및 초과 근무 전체 삭제 처리 됩니다.]",new Listener<MessageBoxEvent>(){
					@Override
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							if(Dlgn0100MemberGrid.getGrid().getSelectionModel().getSelectedItems().size() > 0){
								
								
								Iterator<BaseModel> itBm = Dlgn0100MemberGrid.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
						        ...
						        List<Dlgn0100DTO> listDlgn0100Dto = new ArrayList<Dlgn0100DTO>(); 
						        ...
						        while (iterRecords.hasNext()) {
									...
									dlgn0100Dto.setDpobCd(MSFSharedUtils.allowNulls(bmMapModel.get("dpobCd")));    				/** column 사업장코드 : dpobCd */
									dlgn0100Dto.setSystemkey(MSFSharedUtils.allowNulls(bmMapModel.get("systemkey")));    		/** column SYSTEMKEY : systemkey */
									dlgn0100Dto.setDilnlazYrMnth(MSFSharedUtils.allowNulls(bmMapModel.get("dilnlazYrMnth")));	/** column 근태년월 : dilnlazYrMnth */
									dlgn0100Dto.setPayCd(MSFSharedUtils.allowNulls(bmMapModel.get("payCd")));    				/** column 급여구분코드 : payCd */
									
									listDlgn0100Dto.add(dlgn0100Dto);			
								}  
								
								
								dlgn0230Service.deleteWorkTime(listDlgn0100Dto, new AsyncCallback<Integer>() {
									@Override
									public void onSuccess(Integer result) {
										MessageBox.info("", "삭제 되었습니다.", null);
										dlgn0100MemberReload();
...
//		buttonBar.add(topBtn);
		...
		plFrmDlgn0230.add(buttonBar);

		vp.add(plFrmDlgn0230);
		plFrmDlgn0230.setSize("990px", "750px"); 
	}
...
//			savePayr4200();
	...
	private void saveDlgn0230() { 
		
		if("itemDlgn01".equals(tabsDlgn0230.getSelectedItem().getId())){
			
			MessageBox.confirm("저장", "기본근무 내역을 저장하시겠습니까?",new Listener<MessageBoxEvent>(){
		...
		}else if("itemDlgn02".equals(tabsDlgn0230.getSelectedItem().getId())) {
			
			MessageBox.confirm("저장", "초과근무 내역을 저장하시겠습니까?",new Listener<MessageBoxEvent>(){
			  ...
			  MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(),  "저장하실 탭을 선택해 주세요.", null);
		}
	} 
	...
	private void createSearchForm() {  
		...
		plFrmDlgn0230.setLayout(new FlowLayout());
	    		...
	    		EventType type = be.getType();
	    			if (type == Store.Add) { 
			             ...
			             * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
							++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
	    					checkDlgn0230Auth("srhPayYr", lsPayYrStore); 
	    			}
	    		}
	    	});
	    srhPayYr.addSelectionChangedListener(new SelectionChangedListener<BaseModel>() {
	       ...
	       // srhPayMonth.setEmptyText("--월선택--");
	        ...
	        srhPayMonth.getStore().addStoreListener( new StoreListener<BaseModel>() {   
	        	public void handleEvent(StoreEvent<BaseModel> be) {  
	        		checkDlgn0230Auth("srhPayMonth", lsPayMonth); 
	        	}
	        });
	        ...
	        lsEmymtDivCd.addStoreListener( new StoreListener<BaseModel>() {   
	        	public void handleEvent(StoreEvent<BaseModel> be) {  
	        		...
	        		if (type == Store.Add) { 
	        			 ...
	        			 * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
	  						++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
	        				checkDlgn0230Auth("srhEmymtDivCd", lsEmymtDivCd ); 
	        		} 
	            }
	        });    
	        ...
	        lsPayrMangDeptCd.addStoreListener( new StoreListener<BaseModel>() {   
	        	public void handleEvent(StoreEvent<BaseModel> be) {  
	        		...
	        		if (type == Store.Add) { 
	        			 ...
	        			 * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
			 				++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
	        				checkDlgn0230Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
	        		}
	            }
	        });    
	        ...
	        srhPayCd.getStore().addStoreListener( new StoreListener<BaseModel>() {   
	            public void handleEvent(StoreEvent<BaseModel> be) {  
		    	   ...
		    	   if (type == Store.Add) { 
             		  ...
             		  * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		    			++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		    		   checkDlgn0230Auth("srhPayCd", lsPayCd); 
		    	   }
	            }
	        });    
	        	...
	        	public void handleEvent(StoreEvent<ModelData> be) {  
	        		...
	        		if (type == Store.Add) { 
	        			 ...
	        			 * 값순서 : 단위기관 ,고용구분, 부서, 직종, 사업 순으로  없으면 ""
			         	 * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
			     			++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
	        				checkDlgn0230Auth("srhDeptCd", lsDeptCd); 
	        		}
	        	}
	        });    
	        ...
	        srhTypOccuCd.getStore().addStoreListener( new StoreListener<ModelData>() {   
	            public void handleEvent(StoreEvent<ModelData> be) {  
		    		  ...
		    		  if (type == Store.Add) { 
		    			   ...
		    			   * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		         			++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	//checkDlgn0230Auth("srhTypOccuCd", lsTypOccuCd); 
		    		  }
	            }
	        });     
     	...
     	creYear.getStore().addStoreListener( new StoreListener<BaseModel>() {   
     		public void handleEvent(StoreEvent<BaseModel> be) {  
     			...
     			if (type == Store.Add) { 
     				 ...
     				 * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
			  			++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
     						checkDlgn0230Auth("creYear", lscrPayYr); 
     			}
     		}
     	});
     	...
     	creMonth.setEmptyText("--월--");
      	creMonth.setSelectOnFocus(true); 
       	...
       	creMonth.getStore().addStoreListener( new StoreListener<BaseModel>() {   
       		public void handleEvent(StoreEvent<BaseModel> be) {   
       			checkDlgn0230Auth("creMonth", lscrPayMonth); 
		...
		lscrPayrMangDeptCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkDlgn0230Auth("crePayrMangDeptCd", lscrPayrMangDeptCd);
				}
			}
		});
		...
		lscrEmymtDivCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkDlgn0230Auth("creEmymtDivCd", lscrEmymtDivCd);
				}
			}
		});
...
//	    frmlytSch.setLabelWidth(0); 
       ...
       // srhRepbtyBusinDivCd.setFieldLabel("고용구분");  
		...
		LayoutContainer lc41_1 = new LayoutContainer();
     	...
     	creDilnlazDutyBgnnDt = new MSFDateField();
     	creDilnlazDutyBgnnDt.setName("creDilnlazDutyBgnnDt");
     	new DateFieldMask(creDilnlazDutyBgnnDt, "9999.99.99"); 
     	creDilnlazDutyBgnnDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
     	creDilnlazDutyBgnnDt.setFieldLabel("기본기간");
         
     	lc41_1.add(creDilnlazDutyBgnnDt, new FormData("100%"));
     	...
     	fl41_2.setLabelAlign(LabelAlign.RIGHT);
      	...
      	creDilnlazDutyEndDt = new MSFDateField();
      	creDilnlazDutyEndDt.setName("creDilnlazDutyEndDt");
     	new DateFieldMask(creDilnlazDutyEndDt, "9999.99.99"); 
    	creDilnlazDutyEndDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
      	creDilnlazDutyEndDt.setLabelSeparator("~");
      	lc41_2.add(creDilnlazDutyEndDt, new FormData("100%"));
		...
		crePayCd.getStore().addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkDlgn0230Auth("crePayCd", lscrPayCd);
				}
			}
		});
		...
		creDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkDlgn0230Auth("creDeptCd", lscrDeptCd);
				}
			}
		});
...
//		creTypOccuCd.setFieldLabel("직종");
		creTypOccuCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				 ...
				 * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				 	++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
					// checkDlgn0230Auth("creTypOccuCd", lscrTypOccuCd);
				}
			}
		});
...
//		frmlytSch.setLabelWidth(0);
		...
		creDilnlazExceDutyBgnnDt = new MSFDateField();
		creDilnlazExceDutyBgnnDt.setName("creDilnlazExceDutyBgnnDt");
     	new DateFieldMask(creDilnlazExceDutyBgnnDt, "9999.99.99"); 
     	creDilnlazExceDutyBgnnDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
     	creDilnlazExceDutyBgnnDt.setFieldLabel("초과기간");
         
     	layoutContainer_16_1.add(creDilnlazExceDutyBgnnDt, new FormData("100%"));
              
     	LayoutContainer layoutContainer_16_2 = new LayoutContainer();
     	frmlytSch = new FormLayout();
     	frmlytSch.setLabelWidth(7);
     	frmlytSch.setLabelAlign(LabelAlign.RIGHT);
     	layoutContainer_16_2.setLayout(frmlytSch);
              
     	creDilnlazExceDutyEndDt = new MSFDateField();
     	creDilnlazExceDutyEndDt.setName("creDilnlazExceDutyEndDt");
     	new DateFieldMask(creDilnlazExceDutyEndDt, "9999.99.99"); 
     	creDilnlazExceDutyEndDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
     	creDilnlazExceDutyEndDt.setLabelSeparator("~");
     	layoutContainer_16_2.add(creDilnlazExceDutyEndDt, new FormData("100%"));
              
      	layoutContainer_16.add(layoutContainer_16_1, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.61));
      	layoutContainer_16.add(layoutContainer_16_2, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.39));
        ...
        lcSchCol3.add(layoutContainer_16,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.27));	// 초과기간
		...
		layoutContainer_2.setLayout(frmlytSch);
         ...
         layoutContainer_1.add(layoutContainer_22,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.57));
         ...
         creDlgnType = new ComboBox<BaseModel>();
         creDlgnType.setName("creDlgnType");
         creDlgnType.setForceSelection(true);
         creDlgnType.setMinChars(1);
         creDlgnType.setDisplayField("commCdNm");
         creDlgnType.setValueField("commCd");
         creDlgnType.setTriggerAction(TriggerAction.ALL);
         creDlgnType.setEmptyText("-구분선택-");
         creDlgnType.setSelectOnFocus(true); 
         creDlgnType.setReadOnly(false);
         creDlgnType.setEnabled(true); 
         creDlgnType.setStore(lsUploadDivCd);
//         creDlgnType.setLabelSeparator("");
         creDlgnType.setFieldLabel("복무구분");
         creDlgnType.addSelectionChangedListener(new SelectionChangedListener<BaseModel>() {
			public void selectionChanged(SelectionChangedEvent<BaseModel> se) {
//				// 부서가 변경되는 경우 해당하는 사업직종관련처리를 위한 이벤트처리
//				BaseModel bmPayCd = se.getSelectedItem();
//				if (bmPayCd != null&& MSFConfiguration.EMYMT_DIVCD02.equals(bmPayCd.get("commCd"))) {
...
//				}
         ...
         layoutContainer_222.add(creDlgnType, new FormData("100%"));
         layoutContainer_1.add(layoutContainer_222, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.13));
         

         
 		ButtonBar buttonBar = new ButtonBar();
 		buttonBar.setAlignment(HorizontalAlignment.RIGHT);
 		buttonBar.setStyleAttribute("paddingLeft", "10px");
		...
		btnDlgn0200Upload.addListener(Events.Select,new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				if(MSFSharedUtils.getSelectedComboValue(creDlgnType, "commCd") == null || "".equals(MSFSharedUtils.getSelectedComboValue(creDlgnType, "commCd"))) {
					MessageBox.info("경고", "업로드 할 복무구분을 선택해 주세요.", null);
				}else {
					xlsImportData();
				}
			}
		});
		buttonBar.add(btnDlgn0200Upload);
		...
		btnDlgn0200DownLoad = new Button("양식다운");
		btnDlgn0200DownLoad.addListener(Events.Select,new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				
				if(MSFSharedUtils.getSelectedComboValue(creDlgnType, "commCd") == null || "".equals(MSFSharedUtils.getSelectedComboValue(creDlgnType, "commCd"))) {
					MessageBox.info("경고", "양식다운로드 할 복무구분을 선택해 주세요.", null);
				}else {
					xlsExportData();
				}
			}
		});
		buttonBar.add(btnDlgn0200DownLoad);
		
		
		
		Button btnDlgn0200CreateSingle = new Button("개별생성");
...
//				dlgn0200Grid.getStore().removeAll();
				...
				String workDayS = DlgnUtils.getConvertDateToString(creDilnlazDutyBgnnDt, "yyyyMMdd");
				String workDayE = DlgnUtils.getConvertDateToString(creDilnlazDutyEndDt, "yyyyMMdd"); 
				...
				}else if(GregorianCalendar.compareDate(creDilnlazDutyBgnnDt.getValue(), creDilnlazDutyEndDt.getValue()) < 0){
		...
		buttonBar.add(btnDlgn0200CreateSingle);
		...
		Button btnDlgn0200Create = new Button("일괄생성");
		...
		buttonBar.add(btnDlgn0200Create);
		...
		fldstNewFieldset.setHeadingHtml("근태생성");

		plFrmDlgn0230.add(fldstNewFieldset);
...
//		detailDilnlazDutyBgnnDt = new TextField<String>();
//		detailDilnlazDutyBgnnDt.setFieldLabel("근태기간");
//		detailDilnlazDutyBgnnDt.setReadOnly(true);
//		layoutContainer_3.add(detailDilnlazDutyBgnnDt, new FormData("100%"));
//		
//		detailDilnlazExceDutyBgnnDt = new TextField<String>();
//		detailDilnlazExceDutyBgnnDt.setFieldLabel("근태기간");
//		detailDilnlazExceDutyBgnnDt.setReadOnly(true);
//		layoutContainer_3.add(detailDilnlazExceDutyBgnnDt, new FormData("100%"));
//		detailDilnlazExceDutyBgnnDt.hide();
		
		
		detailDilnlazDutyBgnnDt = new DateField();
		detailDilnlazDutyBgnnDt.setName("detailDilnlazDutyBgnnDt");
        new DateFieldMask(detailDilnlazDutyBgnnDt, "9999.99.99"); 
        detailDilnlazDutyBgnnDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
        detailDilnlazDutyBgnnDt.setFieldLabel("근태기간");
        detailDilnlazDutyBgnnDt.show();
     	layoutContainer_3.add(detailDilnlazDutyBgnnDt, new FormData("100%"));
		
		
     	detailDilnlazExceDutyBgnnDt = new DateField();
     	detailDilnlazExceDutyBgnnDt.setName("detailDilnlazExceDutyBgnnDt");
     	new DateFieldMask(detailDilnlazExceDutyBgnnDt, "9999.99.99"); 
     	detailDilnlazExceDutyBgnnDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
     	detailDilnlazExceDutyBgnnDt.setFieldLabel("근태기간");
     	detailDilnlazExceDutyBgnnDt.hide();
     	layoutContainer_3.add(detailDilnlazExceDutyBgnnDt, new FormData("100%"));
...
//		detailDilnlazDutyEndDt= new TextField<String>();
//		detailDilnlazDutyEndDt.setLabelSeparator("~");
//		detailDilnlazDutyEndDt.setReadOnly(true);
//		layoutContainer_4.add(detailDilnlazDutyEndDt, new FormData("100%"));
//		
//		detailDilnlazExceDutyEndDt= new TextField<String>();
//		detailDilnlazExceDutyEndDt.setLabelSeparator("~");
//		detailDilnlazExceDutyEndDt.setReadOnly(true);
//		layoutContainer_4.add(detailDilnlazExceDutyEndDt, new FormData("100%"));
//		detailDilnlazExceDutyEndDt.hide();
		
		detailDilnlazDutyEndDt = new DateField();
		detailDilnlazDutyEndDt.setName("detailDilnlazDutyEndDt");
     	new DateFieldMask(detailDilnlazDutyEndDt, "9999.99.99"); 
     	detailDilnlazDutyEndDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
     	detailDilnlazDutyEndDt.setLabelSeparator("~");
     	detailDilnlazDutyEndDt.show();
		layoutContainer_4.add(detailDilnlazDutyEndDt, new FormData("100%"));
     	
		detailDilnlazExceDutyEndDt = new DateField();
		detailDilnlazExceDutyEndDt.setName("detailDilnlazExceDutyEndDt");
     	new DateFieldMask(detailDilnlazExceDutyEndDt, "9999.99.99"); 
     	detailDilnlazExceDutyEndDt.getPropertyEditor().setFormat(DateTimeFormat.getFormat(MSFMainApp.MSFCONSTANTS.DateFormat()));
     	detailDilnlazExceDutyEndDt.setLabelSeparator("~");
     	detailDilnlazExceDutyEndDt.hide();
		layoutContainer_4.add(detailDilnlazExceDutyEndDt, new FormData("100%"));
		...
		layoutContainer.add(layoutContainer_10, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.35));
...
//			detailRegnNum.setFieldLabel("주민번호");
		...
		layoutContainer_5.add(lcTypOccuNm, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.3));
...
//	        			}
		...
		layoutContainer_89.add(detailWeekHdayCnt, new FormData("100%"));
		layoutContainer_5.add(layoutContainer_89, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.18));
		...
		fieldRightForm.add(lcTabFormLayer);
		
		rightGrid.add(fieldRightForm);
		
		
		
		LayoutContainer tapPanel = new LayoutContainer();
		
		tapPanel.setStyleAttribute("paddingLeft", "5px");
		
		tabsDlgn0230 = new TabPanel(); // 인사탭정보설정을 위한 tab
		
		//하위탭을 선택하였을 경우 관련된 정보를 가져온다.
		tabsDlgn0230.addListener(Events.Select, new Listener<TabPanelEvent>() {
			public void handleEvent(TabPanelEvent e) {
				...
				if("itemDlgn01".equals(tabsDlgn0230.getSelectedItem().getId())){
					...
					detailDilnlazDutyBgnnDt.hide();
					detailDilnlazDutyEndDt.hide();
					
					detailDilnlazExceDutyBgnnDt.show();
					detailDilnlazExceDutyEndDt.show();
		...
		tabsDlgn0230.setAutoWidth(true);
		// tabsPsnl.setResizeTabs(true);
		tabsDlgn0230.setAnimScroll(true);
		tabsDlgn0230.setTabScroll(true);
		tabsDlgn0230.setPlain(true);
		tabsDlgn0230.setSize(620, 460);

		
		// 기본근무
		TabItem itemDlgn01= new TabItem();
		itemDlgn01.setId("itemDlgn01");
		//itemPsnl0111.setStyleAttribute("padding", "10px");
		itemDlgn01.setText("기본근무");
		//itemPsnl0111.addStyleName("pad-text");
		itemDlgn01.setLayout(new FormLayout());
		itemDlgn01.add(dlgnData01(), new FormData("100%"));
		tabsDlgn0230.add(itemDlgn01);	
		
		// 초과근무
		TabItem itemDlgn02= new TabItem();
		itemDlgn02.setId("itemDlgn02");
		//itemPsnl0111.setStyleAttribute("padding", "10px");
		itemDlgn02.setText("초과근무");
		//itemPsnl0111.addStyleName("pad-text");
		itemDlgn02.setLayout(new FormLayout());
		itemDlgn02.add(dlgnData02(), new FormData("100%"));
		tabsDlgn0230.add(itemDlgn02);
		
		tapPanel.add(tabsDlgn0230);
		rightGrid.add(tapPanel);
		...
		*/
		...
		plFrmDlgn0230.add(lcButtom);
		
	}
	
	// 기본근무관리 탭 시작
	private LayoutContainer dlgnData01() {
...
//		fieldRightForm.setSize("625px", "120px");
		fieldRightForm.setHeadingHtml("근태상세정보");
		fieldRightForm.setCollapsible(false);
//		fieldRightForm.add(occupationalRTop01(), new FormData("100%"));
//		rightGrid.add(fieldRightForm);
		...
		fieldRightGrid.setStyleAttribute("paddingRight", "5px");
		fieldRightGrid.setCollapsible(false);
		fieldRightGrid.add(occupationalRBottom01(), new FormData("100%"));
		rightGrid.add(fieldRightGrid);

		
		lcTabFormLayer.add(rightGrid);
	...
	// 초과근무 탭 시작
	private LayoutContainer dlgnData02() {
...
//		fieldRightForm.setSize("625px", "120px");
		fieldRightForm.setHeadingHtml("근태상세정보");
		fieldRightForm.setCollapsible(false);
//		fieldRightForm.add(occupationalRTop02(), new FormData("100%"));
//		rightGrid.add(fieldRightForm);
		...
		fieldRightGrid.setStyleAttribute("paddingRight", "5px");
		fieldRightGrid.setCollapsible(false);
		fieldRightGrid.add(occupationalRBottom02(), new FormData("100%"));
		rightGrid.add(fieldRightGrid);
	...
	// 기본근무 : 근태상세정보 시작
//	private LayoutContainer occupationalRTop01() {
//	    
//	    detailEmymtDivCd = new HiddenField<String>();
//
//		LayoutContainer lcTabFormLayer = new LayoutContainer();
...
//	}
	...
	// 기본근무 : 근태상세정보 시작
//	private LayoutContainer occupationalRTop02() {
//	    
//	    detailEmymtDivCd = new HiddenField<String>();
//
//		LayoutContainer lcTabFormLayer = new LayoutContainer();
...
//	}
	...
	// 기본근무 : 근무내역등록 그리드 
	private LayoutContainer occupationalRBottom01() {
	    ...
	    Dlgn0100Grid0110.setBodyBorder(false);
...
//		Dlgn0100Grid0110.getBottomComponent().setVisible(false);
		...
		final Grid dlgn0200Grid = Dlgn0100Grid0110.getMsfGrid().getGrid();
...
//            	 // 선택한 근로자 정보를 우측해 보여준다.
//            	 leftDlgn0100Detail(Dlgn0100Grid0110.getCurrentlySelectedItem());
	...
	private LayoutContainer occupationalRBottom02() {
		...
		Dlgn0200Grid0210.setBodyBorder(false);
	...
	private LayoutContainer occupationalLeft() {
	    ...
	    cpGrid.setHeaderVisible(false);   
	    cpGrid.setLayout(new FitLayout());      
		cpGrid.setSize(320, 440);
		
	    MSFCustomForm msfCustomForm = new MSFCustomForm(Dlgn0100MemDef, 0, 0, false, false, false);
	    msfCustomForm.setHeaderVisible(false);
	    Dlgn0100MemberGrid = new MSFGridPanel(Dlgn0100MemDef, false, false, false, false, false);
	    Dlgn0100MemberGrid.setHeaderVisible(false);  
	    Dlgn0100MemberGrid.setBodyBorder(true);
	    Dlgn0100MemberGrid.setBorders(true);
	    //Set the CustomForm to be used by the GridPanel
	    Dlgn0100MemberGrid.setMSFFormPanel(msfCustomForm);
        
	    final Grid dlgn0100Grid = Dlgn0100MemberGrid.getMsfGrid().getGrid();
	    dlgn0100Grid.addListener(Events.RowClick, new Listener<BaseEvent>() {
	    	public void handleEvent(BaseEvent be) {  
	    		if (Dlgn0100MemberGrid.getCurrentlySelectedItem() != null) {      
            	 
	    			// def 에서 그리드 제어를 위해 값을 담아줌.
	    			Dlgn0100MemberGrid.getMSFFormPanel().bind(Dlgn0100MemberGrid.getCurrentlySelectedItem());
            	 
	    			// 근로자정보에서 선택한 대상자의 근태 상세정보를 우측에 보여줌.
	    			leftDlgn0100Detail(Dlgn0100MemberGrid.getCurrentlySelectedItem());
	    			
	    		} 
            }
        }); 
	    cpGrid.add(Dlgn0100MemberGrid); 
...
//		    Dlgn0200Grid0210.getMsfGrid().clearData();
			...
			detailDilnlazDutyBgnnDt.clear();	//근태시작일자
			detailDilnlazDutyEndDt.clear();		//근태종료일자
			detailDilnlazExceDutyBgnnDt.clear();//근태시작일자
			detailDilnlazExceDutyEndDt.clear();	//근태종료일자
...
//			detailEmymtEndDt.setValue("");		//종료일자
	...
	private void setInitDate() {
		...
		int maxDays = 0;
			...
			creDilnlazDutyBgnnDt.setValue(intiCal.getTime());
			creDilnlazExceDutyBgnnDt.setValue(intiCal.getTime());
...
//	        }
	      ...
	      // GWT.log("일수" + DateTimeFormat.getFormat("yyyy.MM.dd").format(intiCal.getTime()));
			creDilnlazDutyEndDt.setValue(intieCal.getTime());
			creDilnlazExceDutyEndDt.setValue(intieCal.getTime());
	}	  
	...
	/** 근무실적생성  */
	private void createDlgnAllData() {
				

		final Tracker tracker = new Tracker();
	   	tracker.setStatus(false);
	   	   
		MessageBox.confirm("근태 생성", "기본 및 초과 근무 근태를 생성하시겠습니까?<br>(기 등록되어있는 근태가 있을시 삭제 처리 됩니다.)",new Listener<MessageBoxEvent>(){
			@Override
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					String year = DlgnUtils.getSelectedComboValue(creYear,"year");															//년도
					String month = DlgnUtils.getSelectedComboValue(creMonth,"month");														//월
					String payrMangDeptCd = DlgnUtils.getSelectedComboValue(crePayrMangDeptCd,"payrMangDeptCd"); 							//단위기관
					String emymtDivCd = DlgnUtils.getSelectedComboValue(creEmymtDivCd,"commCd");											//고용구분
					String payCd = DlgnUtils.getSelectedComboValue(crePayCd,"commCd");														//급여구분
					
					String deptCd = DlgnUtils.getStrValToBMMultiCombo(lscrDeptCd,creDeptCd.getValue(),"deptNmRtchnt","deptCd"); 			//부서
					String typOccuCd = DlgnUtils.getStrValToBMMultiCombo(lscrTypOccuCd,creTypOccuCd.getValue(),"typOccuNm","typOccuCd");	//직종 
					String dtilOccuInttnCd = DlgnUtils.getStrValToBMMultiCombo(lscrDtilOccuInttnCd,creDtilOccuInttnCd.getValue()
																										,"dtilOccuClsNm","dtilOccuInttnCd");//직종세  
					String businCd = DlgnUtils.getSelectedComboValue(creBusinCd, "businCd"); 												//사업
					
					String dilnlazDutyBgnnDt = DlgnUtils.getConvertDateToString(creDilnlazDutyBgnnDt, "yyyyMMdd");							//근태시작기간
					String dilnlazDutyEndDt = DlgnUtils.getConvertDateToString(creDilnlazDutyEndDt, "yyyyMMdd"); 							//근태종료기간
					
					String dilnlazExceDutyBgnnDt = DlgnUtils.getConvertDateToString(creDilnlazExceDutyBgnnDt, "yyyyMMdd");					//근태_초과근무시작일자
					String dilnlazExceDutyEndDt = DlgnUtils.getConvertDateToString(creDilnlazExceDutyEndDt, "yyyyMMdd"); 					//근태_초과근무종료일자
					
					if("".equals(year) || "".equals(month)){
						MessageBox.info("", "작업년월은 필수 입니다.", null);
					}else if("".equals(dilnlazDutyBgnnDt) || "".equals(dilnlazDutyEndDt)){
						MessageBox.info("", "기본기간은 필수 입니다.", null);
					}else if("".equals(dilnlazExceDutyBgnnDt) || "".equals(dilnlazExceDutyEndDt)){
						MessageBox.info("", "초과기간은 필수 입니다.", null);
					}else if("".equals(payrMangDeptCd) || "".equals(payrMangDeptCd)){
						...
						dlgn0100Dto.setDilnlazYrMnth(year+months);				
						dlgn0100Dto.setDilnlazDutyBgnnDt(dilnlazDutyBgnnDt);				
						dlgn0100Dto.setDilnlazDutyEndDt(dilnlazDutyEndDt);		
						
						dlgn0200Dto.setDilnlazExceDutyYrMnth(year+months);
						dlgn0200Dto.setDilnlazExceDutyBgnnDt(dilnlazExceDutyBgnnDt);
						dlgn0200Dto.setDilnlazExceDutyEndDt(dilnlazExceDutyEndDt);
						
						dlgn0100Dto.setPayCd(payCd);
						dlgn0100Dto.setEmymtDivCd(emymtDivCd);
						dlgn0100Dto.setCurrAffnDeptCd(deptCd);
						dlgn0100Dto.setTypOccuCd(typOccuCd);
						dlgn0100Dto.setDtilOccuInttnCd(dtilOccuInttnCd);
						dlgn0100Dto.setBusinCd(businCd);
						dlgn0100Dto.setPayrMangDeptCd(payrMangDeptCd);		//단위기관
						dlgn0100Dto.setSystemkey(creSystemkey.getValue());
						
						  final MessageBox box = MessageBox.wait("근태일괄생성", "기본 및 초과근무 일괄 생성 중 입니다...", "기본 및 초과근무 생성(삭제) 중...");
			                ...
			                dlgn0230Service.generateAllWorkTime(dlgn0100Dto, dlgn0200Dto, new AsyncCallback<PagingLoadResult<ShowMessageBM>>() {
		...
		MessageBox.confirm("근태 생성", "근태생성을 하시겠습니까?<br>(등록되어있는 근태가 있을시 삭제 처리 됩니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					String dlgn01 = "imp/XlsDlgn0100Import.do";	// 기본근무
					String dlgn02 = "imp/XlsDlgn0200Import.do";	// 초과근무
					String dlgnType = "";
						
					if(MSFSharedUtils.getSelectedComboValue(creDlgnType, "commCd") == "DLGN01" || "DLGN01".equals(MSFSharedUtils.getSelectedComboValue(creDlgnType, "commCd"))) {
						dlgnType = dlgn01;
					}else {
						dlgnType = dlgn02;
					}
	            	...
	            	if("".equals(year) || "".equals(month)){
	            		...
	            		if("".equals(businCd)){
	            			...
	            			if (filename.length() == 0) {
	            				...
	            				//submit the form
	            				plFrmDlgn0230.mask("엑셀 데이타 업데이트 중입니다. 업로드 중..."); 
	                    			
	            				//파일업로드 처리
	                    		plFrmDlgn0230.setAction(GWT.getHostPageBaseURL() + dlgnType);
	                    		plFrmDlgn0230.setEncoding(Encoding.MULTIPART);
	                    		plFrmDlgn0230.setMethod(Method.POST);   
//	                                     FormElement frmEl=FormElement.as(plFrmDlgn0250.getElement()); 
//	                                     frmEl.setAcceptCharset("UTF-8"); 
	                    		plFrmDlgn0230.submit();  
	                    		plFrmDlgn0230.onFrameLoad();  
	                            picBoolFile = true;
	                    	}               
	                                  
	                    	// reset and unmask the form 
	                    	// after file upload 
	                    	plFrmDlgn0230.addListener(Events.Submit, new Listener<FormEvent>() {
	                    		public void handleEvent(FormEvent evt) {   
	                    			if ("success".endsWith(evt.getResultHtml()) && picBoolFile) {
	                    				//업로드가  성공했으면 인서트 모듈을 태운다. 
	                    				plFrmDlgn0230.unmask();  
...
//	                               frmEl.setAcceptCharset("UTF-8"); 
	...
	private void xlsExportData() {
		...
		String dlgn01 = "exp/XlsDlgn0100Export.do";	// 기본근무
		String dlgn02 = "exp/XlsDlgn0200Export.do";	// 초과근무
		String dlgnType = "";
...
//			 param.put("pyspGrdeCd", pyspGrdeCd);
		...
		if ("".equals(year) || "".equals(month)) {
			...
			if ("".equals(businCd)) {
			  ...
			  GWTUtils.xlsExportData(GWT.getHostPageBaseURL() + dlgnType,"extgwtFrame", JsUtil.toJavaScriptObject(param)); 
	...
	/** 근로자정보 조회 **/
	public void dlgn0100MemberReload() {
		...
		Dlgn0100MemberGrid.getTableDef().setTableColumnFilters(filters);
		
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("dilnlazYrMnth", yearMonth, SimpleColumnFilter.OPERATOR_EQUALS);
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		String strDeptCd = DlgnUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);
		           
		String strTypOccuCd = DlgnUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	//	psnl0126GridPanel.getTableDef().addColumnFilter("pyspGrdeCd", strPyspGrdeCd, SimpleColumnFilter.OPERATOR_EQUALS);  
		...
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		       
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
		Dlgn0100MemberGrid.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   
		
		Dlgn0100MemberGrid.reload();
	}
	
	
	
	/** 기본근무관리 조회 **/
	public void dlgn0100Reload() {
		
		IColumnFilter filters = null;
		
		Dlgn0100Grid0110.getTableDef().setTableColumnFilters(filters);
		Dlgn0100Grid0110.getTableDef().addColumnFilter("detailYm"			, detailYm.getValue()											, SimpleColumnFilter.OPERATOR_EQUALS);
		...
		Dlgn0100Grid0110.getTableDef().addColumnFilter("detailEmymtBgnnDt"	, MSFSharedUtils.allowNulls(creDilnlazDutyBgnnDt.getValue())	, SimpleColumnFilter.OPERATOR_EQUALS);
		Dlgn0100Grid0110.getTableDef().addColumnFilter("detailEmymtEndDt"	, MSFSharedUtils.allowNulls(creDilnlazDutyEndDt.getValue())		, SimpleColumnFilter.OPERATOR_EQUALS);
		...
		Dlgn0100Grid0110.getTableDef().addColumnFilter("systemkey"			, MSFSharedUtils.allowNulls(systemkeyStr.getValue())			, SimpleColumnFilter.OPERATOR_EQUALS);
	...
	/** 초과근무관리 조회 **/
	public void dlgn0200Reload() {
		
		IColumnFilter filters = null;
		
		Dlgn0200Grid0210.getTableDef().setTableColumnFilters(filters);
		Dlgn0200Grid0210.getTableDef().addColumnFilter("detailYm"			, detailYm.getValue()												, SimpleColumnFilter.OPERATOR_EQUALS);
		...
		Dlgn0200Grid0210.getTableDef().addColumnFilter("detailEmymtBgnnDt"	, MSFSharedUtils.allowNulls(creDilnlazExceDutyBgnnDt.getValue())	, SimpleColumnFilter.OPERATOR_EQUALS);
		Dlgn0200Grid0210.getTableDef().addColumnFilter("detailEmymtEndDt"	, MSFSharedUtils.allowNulls(creDilnlazExceDutyEndDt.getValue())		, SimpleColumnFilter.OPERATOR_EQUALS);
		...
		Dlgn0200Grid0210.getTableDef().addColumnFilter("systemkey"			, MSFSharedUtils.allowNulls(systemkeyStr.getValue())				, SimpleColumnFilter.OPERATOR_EQUALS);
		Dlgn0200Grid0210.getTableDef().addColumnFilter("payCd"				, MSFSharedUtils.getSelectedComboValue(srhPayCd,"commCd")			, SimpleColumnFilter.OPERATOR_EQUALS);
	...
	/** 선택한 근로자 정보를 보여줌**/
    private void leftDlgn0100Detail(final BaseModel selRecord) {
    	
		dpobCdStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("dpobCd")));						//사업장코드
		payCdStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("payCd")));						//급여구분코드
		systemkeyStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("systemkey")));				//시스템키	
		deptCdStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("deptCd")));  					//부서코드
		businCdStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("businCd")));					//사업명	
		typOccudStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("typOccuCd")));				//직종명
		dtilOccuInttnCdStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("dtilOccuInttnCd")));	//직종세명
		emymtDivCdStr.setValue(MSFSharedUtils.allowNulls(selRecord.get("emymtDivCd")));				//고용구분
		
		
		// ==========================================================
		// =============== DB에 입력된 정보 가져옴(근태상세정보)
		// ==========================================================
		detailYm.setValue(MSFSharedUtils.allowNulls(selRecord.get("dilnlazYrMnth")));
		
		detailDilnlazDutyBgnnDt.setValue(GWTUtils.getDateFromString(MSFSharedUtils.allowNulls(selRecord.get("dilnlazDutyBgnnDt")), "yyyyMMdd"));
		detailDilnlazDutyEndDt.setValue(GWTUtils.getDateFromString(MSFSharedUtils.allowNulls(selRecord.get("dilnlazDutyEndDt")), "yyyyMMdd"));
		
		detailDilnlazExceDutyBgnnDt.setValue(GWTUtils.getDateFromString(MSFSharedUtils.allowNulls(selRecord.get("dilnlazExceDutyBgnnDt")), "yyyyMMdd"));
		detailDilnlazExceDutyEndDt.setValue(GWTUtils.getDateFromString(MSFSharedUtils.allowNulls(selRecord.get("dilnlazExceDutyEndDt")), "yyyyMMdd"));
		
		
		detailDeptNm.setValue(MSFSharedUtils.allowNulls(selRecord.get("deptNm")));
		detailName.setValue(MSFSharedUtils.allowNulls(selRecord.get("hanNm")));
		detailRegnNum.setValue(MSFSharedUtils.allowNulls(selRecord.get("resnRegnNum")));
		
		detailEmymtDivCd.setValue(MSFSharedUtils.allowNulls(selRecord.get("emymtDivCd")));
		
		if("A0020010".equals(detailEmymtDivCd.getValue())) {
			detailTypOccuNm.setValue(MSFSharedUtils.allowNulls(selRecord.get("typOccuNm")) + "(" + MSFSharedUtils.allowNulls(selRecord.get("dtilOccuInttnNm")) + ")");
		}else {
			detailTypOccuNm.setValue(MSFSharedUtils.allowNulls(selRecord.get("businNm")));
		}
		// ==========================================================
		// =============== DB에 입력된 정보 가져옴(근태상세정보)
		// ==========================================================
		...
		dlgn0100Dto.setDilnlazYrMnth(MSFSharedUtils.allowNulls(selRecord.get("dilnlazYrMnth")));
		dlgn0100Dto.setPayCd(MSFSharedUtils.allowNulls(selRecord.get("payCd")));
		dlgn0100Dto.setDeptCd(MSFSharedUtils.allowNulls(selRecord.get("payrMangDeptCd")));
//		dlgn0100Dto.setHanNm(srhHanNm.getValue());
//		dlgn0100Dto.setResnRegnNum(detailRegnNum.getValue());
		dlgn0100Dto.setSystemkey(MSFSharedUtils.allowNulls(selRecord.get("systemkey")));
		dlgn0100Dto.setEmymtDivCd(MSFSharedUtils.allowNulls(selRecord.get("emymtDivCd")));
		dlgn0100Dto.setCurrAffnDeptCd(MSFSharedUtils.allowNulls(selRecord.get("deptCd")));	//부서
		dlgn0100Dto.setTypOccuCd(MSFSharedUtils.allowNulls(selRecord.get("typOccuCd"))); 
		dlgn0100Dto.setDtilOccuInttnCd(MSFSharedUtils.allowNulls(selRecord.get("dtilOccuInttnCd")));
		dlgn0100Dto.setBusinCd(MSFSharedUtils.allowNulls(selRecord.get("businCd")));
		
		
		//service 구현
		dlgn0230Service.getMemberInfo(dlgn0100Dto, new AsyncCallback<Dlgn0100DTO>(){
			@Override 
			public void onFailure(Throwable caught) {
			// 해당 서비스 오류날 경우 기존 값 넣어줌.	
				detailWeekHdayCnt.setValue(MSFSharedUtils.allowNulls(selRecord.get("DilnlazWklyHldyNumDys")));
				detailWeekHdayCntStr = MSFSharedUtils.allowNulls(selRecord.get("DilnlazWklyHldyNumDys"));
    ...
    private void saveDlgn0100() {
		...
		if ((Dlgn0100Grid0110.getMsfGrid().getGrid().getStore().getModifiedRecords().size() <= 0)){ 
			...
			if(Dlgn0100Grid0110.getMsfGrid().getGrid().getStore().getModifiedRecords().size() < 1 ) {
				...
				dlgn0110dto.setDilnlazWklyHldyNumDys(MSFSharedUtils.allowNulls(detailWeekHdayCnt.getValue()));	/** column 주휴일수 :  dilnlazWklyHldyNumDys */
				
				dlgn0230Service.updateDlgn0100_DILNLAZ_WKLY_HLDY_NUM_DYS(dlgn0110dto, new AsyncCallback<Long>(){
					@Override 
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
...
// 					dlgn0110dto.setDilnlazHlthCreYn(MSFSharedUtils.allowNulls(bmMapModel.get("dilnlazHlthCreYn")));			/** column 보건여부 : dilnlazHlthCreYn */
 					...
 					dlgn0110dto.setDilnlazDutyList(MSFSharedUtils.allowNulls(bmMapModel.get("dilnlazDutyList$commCd")));	/** column 근무여부 : dilnlazDutyYn */
				...
				//service 구현
				dlgn0230Service.updateDlgn0110Grid(listDlgn0110_1dto, new AsyncCallback<Long>(){
					@Override 
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
    ...
    private void saveDlgn0200() {
		...
		//	 			}
				 		...
				 		//service 구현
				 		dlgn0230Service.updateDlgn0210Grid(listDlgn0210_1dto, new AsyncCallback<Long>(){
				 			@Override 
				 			public void onFailure(Throwable caught) {
				 				MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
				                         MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnDlgn0210(업데이트오류) : " + caught)
