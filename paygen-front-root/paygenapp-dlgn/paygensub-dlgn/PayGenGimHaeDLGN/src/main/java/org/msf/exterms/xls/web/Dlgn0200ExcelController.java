...
//    	String xmlConfig = "/upXmlConfig/DLGN/GimHae/Dlgn0200Mapping.xml";
    	String xmlConfig = "/upXmlConfig/DLGN/"+extGlobals.get("PayGen.MunicipalitiesName").toLowerCase() +"/Dlgn0200Mapping.xml";
    	...
    	try {
    		...
    		if(xlsReadStatus.isStatusOK()) {
				...
				for(int i = 0 ; i<  dlgn0210List.size()  ; i++){
					...
					String dlgnFlagStr = MSFSharedUtils.defaultNulls(dlgn0210List.get(i).getDlgnFlag(),"02");
					...
					chkTime1 = MSFSharedUtils.defaultNulls(dlgn0210List.get(i).getDilnlazTmRstDutyTm(),"");  	//연장(평일)
					...
					if("01".equals(dlgnFlagStr) || dlgnFlagStr == "01") {
...
//						chkTime4 = MSFSharedUtils.defaultNulls(dlgn0210List.get(i).getDilnlazHodyNtotTm(),""); 		//연장(휴일)
						...
						if(!"".equals(chkTime1) ){
							if(chkTime1.length() ==1 &&(  Integer.parseInt(chkTime1)>0&& Integer.parseInt(chkTime1)<10 )){
								dlgn0210List.get(i).setDilnlazTmRstDutyTm("0"+chkTime1+":00");
							}else if(chkTime1.length() ==2 &&(  Integer.parseInt(chkTime1)>0&& Integer.parseInt(chkTime1)<10 )){
								dlgn0210List.get(i).setDilnlazTmRstDutyTm(chkTime1+":00");
							}else{
						...
						if(!"".equals(chkTime1) ){
							if(chkTime1.contains(":")){
								chkFormCnt++;
							}
						}
						
						if(!"".equals(chkTime2) ){
							if(chkTime2.contains(":")){
								chkFormCnt++;
							}
						}
						
						if(!"".equals(chkTime3) ){
							if(chkTime3.contains(":")){
								chkFormCnt++;
							}
						}
						
						if(!"".equals(chkTime4) ){
							if(chkTime4.contains(":")){
								chkFormCnt++;
							}
						}
				...
				if(chkFormCnt > 0){
					...
					long excelCount = dlgn0210List.size();
	    			...
	    			if(lResult > 0) {
						...
						if(lResult == excelCount){
			            	PrgmComUtils.responseAlert(response, "업로드가 완료되었습니다." );
			            }else{
			            	PrgmComUtils.responseAlert(response, "엑셀업로드 "+ excelCount+"중에  총"+ lResult +"건 입력되었습니다. 엑셀데이타를 확인해주세요." );
			            }
