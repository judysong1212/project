...
//
//          dlgn0250SrhVo.setJobYrMnth(dlgn0250SrhVo.getJobYrMnth());
...
//	        
//	        dlgn0250SrhVo.setPayCd(MSFSharedUtils.allowNulls(dlgn0250SrhVo.getPayCd()));
...
//			 */
//			if (dlgn0250SrhVo.getPayrMangDeptCd().equals(
//					MSFSharedUtils.allowNulls(dlgn0250SrhVo.getDeptCdAuth()))|| MSFSharedUtils.allowNulls(dlgn0250SrhVo.getDeptCdAuth()).contains(dlgn0250SrhVo.getPayrMangDeptCd())) {
//				// 김해로직
...
//	        HashMap<String, String> extGlobals = ExtConf.getPayGenConfigInfo(RemoteServiceUtil.getThreadLocalConfig() , RemoteServiceUtil.getThreadLocalRequest()); ;
			...
			if (dlgn0100SrhVo.getPayrMangDeptCd().equals(
				...
				// 김해로직
				// 호봉제구분코드 포함해서 가져 오기 REPBTY_BUSIN_DIV_CD
				dlgn0100SrhVo.setRepbtyBusinDivCd("A0480010");
			}
...
//    }   
    ...
    public String handleFormUpload(@RequestParam("dlgn0100flUp") MultipartFile file, Model model, final HttpServletRequest request, final HttpServletResponse response) throws InvalidFormatException, IOException, SAXException {
    	
    	HashMap<String, String> extGlobals = MSFServerUtils.getLocalidedMessages(RemoteServiceUtil.getThreadLocalConfig() , RemoteServiceUtil.getThreadLocalRequest());	    	
    	String realPath = request.getSession().getServletContext().getRealPath(""); 
//    	String xmlConfig = "/upXmlConfig/DLGN/GimHae/Dlgn0100Mapping.xml";
...
//    	String xmlConfig = File.separator+"upXmlConfig"+File.separator+"DLGN"+File.separator+"Dlgn0100Mapping.xml";
    	...
    	try {
    		...
    		if(xlsReadStatus.isStatusOK()) {
    			...
    			if(lResult >= 1) {
    				PrgmComUtils.responseAlert(response, "기본근무관리 업로드가 완료되었습니다. 결과를 확인해주세요." );
					sResult =  "success";
				} else {
					sResult =  "error";
				}
