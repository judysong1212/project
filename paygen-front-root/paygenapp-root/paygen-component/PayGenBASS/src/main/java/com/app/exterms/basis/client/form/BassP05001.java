...
import com.app.exterms.basis.client.service.BassP05001Service;
import com.app.exterms.basis.client.service.BassP05001ServiceAsync;
...
=====================================================**/
public  class BassP05001   extends MSFFormPanel {  
...
//	private Bass0500Def bass0500Def  = new Bass0500Def();
   ...
   */
	...
	private BassP05001ServiceAsync bassP05001Service = BassP05001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
 ...
 **/ 
	 ...
	 **/ 		    
		 ...
		 */
		 private void checkBassP05001Auth( String authAction, ListStore<BaseModel>bm) {
...
//				 authMapDef.put("srhEmymtDivCd",Boolean.FALSE);
	...
	public BassP05001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//						GregorianCalendar.compareDate(businStDt.getValue(), businStDt.getValue()) < 0 || 
						BasisUtil.getConvertDateToString(businStDt, "yyyyMMdd").replace(".", "")==null ||
						"".equals(BasisUtil.getConvertDateToString(businEdDt, "yyyyMMdd").replace(".", ""))) {
				
					MessageBox.alert("경고", "사업 시작 기간이 이상합니다. 확인하십시요.", null);  
					return;
				} else if(
//						GregorianCalendar.compareDate(businEdDt.getValue(), businEdDt.getValue()) < 0 || 
						BasisUtil.getConvertDateToString(businEdDt, "yyyyMMdd").replace(".", "") == null ||
						"".equals(BasisUtil.getConvertDateToString(businEdDt, "yyyyMMdd").replace(".", ""))){
...
//		actionDatabase = ActionDatabase.READ;
	...
	public void trnrBass0500() {
        ...
        while (iterRecords.hasNext()) {
			...
			bass0500Dto.setBusinStdt(MSFSharedUtils.allowNulls(BasisUtil.getConvertDateToString(businStDt, "yyyyMMdd")));    	/** column 사업시작기간 : businStdt */
			bass0500Dto.setBusinEddt(MSFSharedUtils.allowNulls(BasisUtil.getConvertDateToString(businEdDt, "yyyyMMdd")));    	/** column 사업종료기간 : businEddt */
