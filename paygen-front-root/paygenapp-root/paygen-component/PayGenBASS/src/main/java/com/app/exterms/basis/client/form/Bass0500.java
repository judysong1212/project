...
import com.app.exterms.basis.client.form.defs.Bass0500Def;
import com.app.exterms.basis.client.languages.BasisConstants;
import com.app.exterms.basis.client.service.Bass0500Service;
import com.app.exterms.basis.client.service.Bass0500ServiceAsync;
...
import com.clipsoft.clipreport.document.Document;
import com.clipsoft.org.apache.poi.ss.formula.functions.Column;
 ...
 * =========================================================================
 **/
public class Bass0500 extends MSFPanel {
	/**
	 * ########################################################################## 
	 * { 시스템 기본설정파일 선언부 시작}
	 * ##########################################################################
	...
	private FormPanel plFrmBass0500;
	...
	private MSFGridPanel msfBass0500GridPanel;
	// -------------- grid 선언 종료  ---------------
	 ...
	 */	
	// -- 상단 버튼 시작
	private ButtonBar topBass0500Bar;				
	private Button btnBass0500New;						//신규 버튼
	private Button btnBass0500Save;						//저장 버튼 
	private Button btnBass0500Del;						//삭제 버튼
	private Button btnBass0500Sreach;					//조회 버튼
	private Button btnBass0500Print;					//인쇄 버튼
	private Button btnBass0500Excel;					//엑셀 버튼	

	private Button btnBass0500Trnr;						//이관 버튼
	...
	private TextField<String> srhDeptNm; 					//부서코드
	 ...
	 */
	// -------------- 상세정보 시작 -------------- 
	private TabPanel tabsBass0500; // 하위 탭 정보
	...
	private MSFTextField deptNm; 					//부서코드
	...
	private MSFTextField dilnlazTmRstDutyTm; 				//근무인정시간
// --------------  상세정보 끝 -------------- 
	...
	// -------------- 서비스 호출 시작 --------------
	//	private InfcBass0500ServiceAsync infcBass0500Service = InfcBass0500Service.Util.getInstance();
	private Bass0500ServiceAsync bass0500Service = Bass0500Service.Util.getInstance();
	 ...
	 * AuthAction 
	 */
	private void checkBass0500Auth( String authAction, ListStore<BaseModel> bm) {   
	...
	private void authExecEnabled() { 
		 ...
		 * 값순서 : 단위기관 ,고용구분, 부서, 직종, 사업 순으로  없으면 ""
           ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		Field<?>[] fldArrField = {srhDeptCd, srhDeptNm};
		...
		//gwtAuthorization.formAuthConfig(plFrmBass0500, "","","srhDeptCd","","",""); 
		/**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	//		   * AuthAction 
	//		   */
	//		  private void checkBass0500Auth( AuthAction authAction, ListStore<BaseModel> bm, int authChk) {   
	//	    	   //MSFMainApp.maskMainPage("[화면로딩] 초기화 진행 중"); 
	//			 
	...
	private SelectionListener<ButtonEvent> selectionListener = new SelectionListener<ButtonEvent>() {
		public void componentSelected(ButtonEvent ce) {
			if (ce.getButton() == btnBass0500Save) {
				doAction(ActionDatabase.INSERT);
			}else if (ce.getButton() == btnBass0500Save) {
				doAction(ActionDatabase.UPDATE);
			}else if (ce.getButton() == btnBass0500Del) {
	...
	public ContentPanel getViewPanel() {
		if (panel == null) {
			...
			//			infcBass0500Service.getYearList(new AsyncCallback<List<BaseModel>>() {
			//				@Override
			...
			vp.setSpacing(10);
			createBass0500Form(); 	// 화면 기본정보를 설정
	...
	private void createStdSubFrom() {
		...
		plFrmBass0500.add(fldstNewFieldset, new FormData("100%"));
	}
	/**
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	 * 서브 입력 폼 종료
	...
	public Bass0500(String txtForm) {
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createBass0500Form() {

		plFrmBass0500 = new FormPanel();
		plFrmBass0500.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 부서별사업관리"));
		plFrmBass0500.setIcon(Resources.APP_ICONS.text());
		plFrmBass0500.setBodyStyleName("pad-text");
		...
		plFrmBass0500.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmBass0500.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/
		plFrmBass0500.setPadding(2);

		plFrmBass0500.setFrame(true);
		plFrmBass0500.setLayout(new FormLayout());
		...
		btnBass0500Trnr = new Button("이전사업이관");
		btnBass0500Trnr.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {

				BassP05001 bassP05001Form = new BassP05001(ActionDatabase.READ, getThis());   

				MSFFormWindows msFwBass05001 = new MSFFormWindows("이전사업이관", bassP05001Form, "닫기", "820px", "710px", true);
				msFwBass05001.show();
				bassP05001Form.setMSFFormWindows(msFwBass05001);

			}
		});

		btnBar.add(btnBass0500Trnr);
		lc1.add(btnBar);
		...
		topBass0500Bar = new ButtonBar();
		topBass0500Bar.setAlignment(HorizontalAlignment.RIGHT);


		//Button topBtn = new Button("신규");
		btnBass0500New = new Button("신규");
		btnBass0500New.setIcon(Resources.APP_ICONS.new16());
		btnBass0500New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//신규 버튼 클릭시 
				mainFormInit();
				subFormInit();
		...
		/**
		btnBass0500New.addSelectionListener(new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				dpobCd.setValue("");
				//2014.08.27 신규버튼 클릭시 상세정보 사업년도를 현재년도로 설정
				BasisUtil.setSelectedComboValue(businApptnYr, DateTimeFormat.getFormat("yyyy").format(new Date()), "commCd");
		 ...
		 */
		topBass0500Bar.add(btnBass0500New);

		//topBtn = new Button("저장");
		btnBass0500Save = new Button("저장");
		btnBass0500Save.setIcon(Resources.APP_ICONS.save16());
		btnBass0500Save.addSelectionListener(selectionListener);
		/**
		btnBass0500Save.addSelectionListener(new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				save(ActionDatabase.UPDATE);
			}
		});*/
		topBass0500Bar.add(btnBass0500Save);

		//topBtn = new Button("삭제");
		btnBass0500Del = new Button("삭제");
		btnBass0500Del.setIcon(Resources.APP_ICONS.delete16());
		btnBass0500Del.addSelectionListener(selectionListener);
		/**
		btnBass0500Del.addSelectionListener(new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				save(ActionDatabase.DELETE);
			}
		});*/
		topBass0500Bar.add(btnBass0500Del);


		//topBtn = new Button("조회");
		btnBass0500Sreach = new Button("조회");
		btnBass0500Sreach.setIcon(Resources.APP_ICONS.search16());
		btnBass0500Sreach.addListener(Events.Select,new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 조회버튼 클릭시 처리
				reload();
			}
		});
		topBass0500Bar.add(btnBass0500Sreach);


		//TODO 기능없으므로 닫아놓음
		//topBtn = new Button("인쇄");
		//		btnBass0500Print = new Button("인쇄");
		//		btnBass0500Print.setIcon(Resources.APP_ICONS.print16());
		//		topBass0500Bar.add(btnBass0500Print);


		//TODO 기능없으므로 닫아놓음
		//topBtn = new Button("엑셀");
		//		btnBass0500Excel = new Button("엑셀");
		//		btnBass0500Excel.setIcon(Resources.APP_ICONS.excel16());
		//		topBass0500Bar.add(btnBass0500Excel);



		//lc.add(buttonBarLeft, new com.extjs.gxt.ui.client.widget.layout.ColumnData(0.3));
		lc2.add(topBass0500Bar);
		...
		plFrmBass0500.add(lc);

		// 파일업로드 처리 - 사업장 직인 정보
		plFrmBass0500.setAction("myurl"); // url 정보 추가 할것.
		plFrmBass0500.setEncoding(Encoding.MULTIPART);
		plFrmBass0500.setMethod(Method.POST);

		vp.add(plFrmBass0500);
		//엣지변환
		//plFrmBass0500.setSize("990px", "680px");
		plFrmBass0500.setSize("990px", "730px");
	}
	...
	public void save(final ActionDatabase actionDatabase){
		...
		if("".equals(businApptnYr.getValue()) || businApptnYr.getValue() == null){
			...
			businEddt.focus();
		} else if("".equals(dilnlazTmRstDutyTm.getValue()) || dilnlazTmRstDutyTm.getValue() == null){
			MessageBox.info("", "근무시간을 입력하여주세요.", null);
		}else{
			...
			dto.setBusinEmymtTypOccuCd(BasisUtil.getSelectedComboValue(businEmymtTypOccuCd));	//사업고용직종코드(산재보험직종)
			...
			dto.setBusinStdt(BasisUtil.getConvertDateToString(businStdt, "yyyyMMdd"));			//시작일자
			dto.setBusinEddt(BasisUtil.getConvertDateToString(businEddt, "yyyyMMdd"));			//종료일자
			
			//개발_시간외근무 최대 인정시간_20201026
			dto.setDilnlazTmRstDutyTm(dilnlazTmRstDutyTm.getValue());   						//시간외근무 최대인정시간
						...
						bass0500Service.saveBass0500(dto, actionDatabase, new AsyncCallback<String>() {
							...
							public void onFailure(Throwable caught) {
								...
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
										MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnBass0500(" + actionDatabase.name() + ") : " + caught)
										, null);
							}
						});
					}
	...
	private void createSearchForm() {
		...
		plFrmBass0500.add(srhDeptCd);

		LayoutContainer lccmCurrAffnDeptNm = new LayoutContainer();
		frmlytSchLeft = new FormLayout();
		frmlytSchLeft.setLabelWidth(85);
		frmlytSchLeft.setLabelAlign(LabelAlign.RIGHT);
		lccmCurrAffnDeptNm.setLayout(frmlytSchLeft);
		...
		//		                    * 값순서 : 단위기관 ,고용구분, 부서, 직종, 사업 순으로  없으면 ""
		//		                    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		//	                    checkBass0500Auth("srhDeptCd", listStoreSrhDeptCd);  
		//		    	   }   
		...
		srhDeptNm.addKeyListener(new KeyListener() {
			public void componentKeyDown(ComponentEvent event) {
				...
				if(event.getKeyCode() == KeyCodes.KEY_ENTER) {
					...
					fnPopupCommP140_Curr(srhDeptNm.getValue());
				}
		...
		lccmCurrAffnDeptNm.add(srhDeptNm, new FormData("100%"));

		lc1.add(lccmCurrAffnDeptNm, new com.extjs.gxt.ui.client.widget.layout.ColumnData(0.91));
		lc1.add(btnCurrAffnDeptCd, new com.extjs.gxt.ui.client.widget.layout.ColumnData(0.09));
	...
	private void createTabForm(){
		tabsBass0500 = new TabPanel(); // 인사탭정보설정을 위한 tab

		// tabsBass0350.setMinTabWidth(80);
		tabsBass0500.setAutoWidth(false);
		// tabsBass0350.setResizeTabs(true);
		tabsBass0500.setAnimScroll(true);
		tabsBass0500.setTabScroll(true);
		tabsBass0500.setPlain(true);
		tabsBass0500.setSize(970, 173);
		...
		tabsBass0500.add(itemBass0351);

		plFrmBass0500.add(tabsBass0500, new FormData("100%"));
	}
	...
	private LayoutContainer detailImfo01(){
		...
		LayoutContainer lc1_2 = new LayoutContainer();
		lc1_2.setLayout(new ColumnLayout());

		LayoutContainer lccmCurrAffnDeptNm = new LayoutContainer();
		frmlytStd1 = new FormLayout();
		frmlytStd1.setLabelWidth(85);
		frmlytStd1.setLabelAlign(LabelAlign.RIGHT);
		lccmCurrAffnDeptNm.setLayout(frmlytStd1);

		deptCd = new MSFTextField();
		deptCd.setName("deptCd");
		deptCd.setVisible(false);
		plFrmBass0500.add(deptCd);
		...
		deptNm.addKeyListener(new KeyListener() {
			public void componentKeyDown(ComponentEvent event) {
				...
				if(event.getKeyCode() == KeyCodes.KEY_ENTER) {
					...
					fnPopupCommP150_Curr(deptNm.getValue());
				}
				super.componentKeyDown(event);
			}
		});
		...
		lccmCurrAffnDeptNm.add(deptNm, new FormData("100%"));

		lc1_2.add(lccmCurrAffnDeptNm, new com.extjs.gxt.ui.client.widget.layout.ColumnData(0.91));
		lc1_2.add(btnCurrAffnDeptCd, new com.extjs.gxt.ui.client.widget.layout.ColumnData(0.09));
		...
		FormLayout fl4_2 = new FormLayout();
		fl4_2.setLabelWidth(85);
		fl4_2.setLabelAlign(LabelAlign.RIGHT);
		lc4_2.setLayout(fl4_2);
		dilnlazTmRstDutyTm = new MSFTextField();
		dilnlazTmRstDutyTm.setFieldLabel("근무시간");
		//숫자만 입력_20230412
		new TextFieldMask<String>(dilnlazTmRstDutyTm, "9");
		//"숫자만 입력하세요" 표시_20230412
		dilnlazTmRstDutyTm.setEmptyText("※숫자만 입력하세요");
		
		lc4_2.add(dilnlazTmRstDutyTm, new FormData("100%"));
				
		lc4.add(lc4_2, new com.extjs.gxt.ui.client.widget.layout.ColumnData(0.33));
	...
	//		deptCd.setFieldLabel("부서명");
	//		deptCd.setName("deptCd");
	//		deptCd.addKeyListener(new KeyListener() {
	//            public void componentKeyDown(ComponentEvent event) {
	//          	  
	//          	  if (MSFSharedUtils.allowNulls(deptCd.getValue()).trim().equals("")) {
	//          		deptCd.setValue("");
	//                }
	//                  if (MSFSharedUtils.paramNull(deptCd.getValue())) {
	//                	  deptCd.setValue("");
	//                  }  
	//                if(event.getKeyCode() == KeyCodes.KEY_ENTER) {
	...
	//                	deptCd.setValue("");
	//                    //}
	//                    fnPopupCommP150_Curr(deptCd.getValue());
	//                }
	//                super.componentKeyDown(event);
	//            }
	//        });
	...
	private void createListGrid() {
		...
		msfBass0500GridPanel = new MSFGridPanel(bass0500Def, false, false, false, false);
		msfBass0500GridPanel.setHeaderVisible(false);

		final Grid<BaseModel> grid = msfBass0500GridPanel.getMsfGrid().getGrid();
		grid.addListener(Events.RowClick, new Listener<GridEvent<BaseModel>>() {
			...
			public void handleEvent(GridEvent<BaseModel> be) {
				...
				//BasisUtil.setSelectedComboValue(businApptnYr, grid.getSelectionModel().getSelectedItem().get("businApptnYr")+"");//단위기관
				businApptnYr.setValue(listStoreYr.findModel("year", grid.getSelectionModel().getSelectedItem().get("businApptnYr")));
				deptCd.setValue(grid.getSelectionModel().getSelectedItem().get("deptCd") == null ? "" : grid.getSelectionModel().getSelectedItem().get("deptCd")+"");
				deptNm.setValue(grid.getSelectionModel().getSelectedItem().get("deptNm") == null ? "" : grid.getSelectionModel().getSelectedItem().get("deptNm")+"");
				BasisUtil.setSelectedComboValue(businEmymtTypOccuCd, grid.getSelectionModel().getSelectedItem().get("businEmymtTypOccuCd")+"");
				...
				businStdt.setValue(BasisUtil.getConvertStringToDate((String)grid.getSelectionModel().getSelectedItem().get("businStdt"), "yyyyMMdd"));
				businEddt.setValue(BasisUtil.getConvertStringToDate((String)grid.getSelectionModel().getSelectedItem().get("businEddt"), "yyyyMMdd"));
				
				//개발_시간외근무 최대 인정시간_20201026
				dilnlazTmRstDutyTm.setValue(grid.getSelectionModel().getSelectedItem().get("dilnlazTmRstDutyTm") == null ? "" : grid.getSelectionModel().getSelectedItem().get("dilnlazTmRstDutyTm")+"");
				...
				//				BasisUtil.setSelectedComboValue(businApptnYr, grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("businApptnYr")+"");//단위기관
				//				BasisUtil.setSelectedComboValue(deptCd, grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("deptCd")+"");
				//				BasisUtil.setSelectedComboValue(businEmymtTypOccuCd, grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("businEmymtTypOccuCd")+"");
				//				
				...
				//				businStdt.setValue(BasisUtil.getConvertStringToDate((String)grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("businStdt"), "yyyyMMdd"));
				//				businEddt.setValue(BasisUtil.getConvertStringToDate((String)grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("businEddt"), "yyyyMMdd"));
				//				
		...
		cpGrid.add(msfBass0500GridPanel);
	...
	public void reload() {
		...
		msfBass0500GridPanel.getTableDef().setTableColumnFilters(filters);
		msfBass0500GridPanel.getTableDef().addColumnFilter("srhBusinApptnYr", MSFSharedUtils.getSelectedComboValue(srhBusinApptnYr, "year")  , SimpleColumnFilter.OPERATOR_LIKE);
		msfBass0500GridPanel.getTableDef().addColumnFilter("srhDeptCd",srhDeptCd.getValue(), SimpleColumnFilter.OPERATOR_LIKE);
		msfBass0500GridPanel.getTableDef().addColumnFilter("srhBusinNm", srhBusinNm.getValue(), SimpleColumnFilter.OPERATOR_LIKE);
		msfBass0500GridPanel.reload();
	...
	private void formBass0500Init() {

		dpobCd.setValue("");
		//2014.08.27 신규버튼 클릭시 상세정보 사업년도를 현재년도로 설정
		//BasisUtil.setSelectedComboValue(businApptnYr, DateTimeFormat.getFormat("yyyy").format(new Date()), "commCd");	//사업년도
		businApptnYr.setValue(listStoreYr.findModel("year", DateTimeFormat.getFormat("yyyy").format(new Date()) ));
		//businApptnYr.setValue(new BaseModel());
		deptCd.setValue("");				//부서코드
		deptNm.setValue("");				//부서코드
		businEmymtTypOccuCd.setValue(new BaseModel());	//사업고용직종코드
	...
	//소속부서
	private void fnPopupCommP150_Curr(String popupDeptNm) {
		//검색폼을 만들어 팝업으로 넘김. msfgrigpanel 내용을 이용함.
		//넘긴데이터는 폼에 검색조건을 필터링에 넣은 작업을한다.
		//검색.처리하면됨. 
		MSFFormPanel popCom0140 = PrgmComPopupUtils.lovPopUpPrgmCom0140Form(popupDeptNm);  //부서
		...
		popBindingCom0140.addListener(Events.Change, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {
				...
				if (!"".equals(mapModel.get("deptCd"))) {  
					if("".equals(deptCd.getValue()) || "".equals(deptNm.getValue())
							|| deptCd.getValue() == null || deptNm.getValue() == null){
	...
	//소속부서
	private void fnPopupCommP140_Curr(String popupDeptNm) {
		//검색폼을 만들어 팝업으로 넘김. msfgrigpanel 내용을 이용함.
		//넘긴데이터는 폼에 검색조건을 필터링에 넣은 작업을한다.
		//검색.처리하면됨. 
		MSFFormPanel popCom0140 = PrgmComPopupUtils.lovPopUpPrgmCom0140Form(popupDeptNm);  //부서
		...
		popBindingCom0140.addListener(Events.Change, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {
				BaseModel mapModel = (BaseModel)be.getSource();
				if (!"".equals(mapModel.get("deptCd"))) {  
					if("".equals(srhDeptCd.getValue()) || "".equals(srhDeptNm.getValue())
							|| srhDeptCd.getValue() == null || srhDeptNm.getValue() == null){
