...
import com.app.exterms.basis.client.form.defs.Bass0100Def;
import com.app.exterms.basis.client.languages.BasisConstants;
import com.app.exterms.basis.client.service.Bass0100Service;
import com.app.exterms.basis.client.service.Bass0100ServiceAsync;
 ...
 * =========================================================================
 **/
public class Bass0100 extends MSFPanel {
...
//	private MSFComboBox<BaseModel> retryCllnDebrDivCd; 	// 징수의무자 
	...
	private Bass0100ServiceAsync bass0100Service = Bass0100Service.Util.getInstance();
	private PrgmComBass0300ServiceAsync sysComBass0300Service = PrgmComBass0300Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 
	
	
	// -------------- grid 시작 -------------- 
	private Bass0100Def bass0100Def = new Bass0100Def(); // 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel;
	
	
	private Bass0100Def mBass0100Def = new Bass0100Def("BASS0100_01"); // 주 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel01;
	
	private Bass0100Def sBass0100Def = new Bass0100Def("BASS0100_02"); // 종 그리드 테이블 컬럼 define
	...
	public void save(final ActionDatabase actionDatabase) {
		...
		final Bass0100DTO dto = new Bass0100DTO();
		


		dto.setTplvlDpobCd(MSFSharedUtils.allowNulls(BasisUtil.getSelectedComboValue(tplvlDpobCd))); 			// 사업장 상위구분
...
//		dto.setRetryCllnDebrDivCd(BasisUtil.getSelectedComboValue(retryCllnDebrDivCd));	// 징수의무자
		...
		dto.setSysDpobCd(MSFSharedUtils.allowNulls(
				BasisUtil.getSelectedComboValue(sysDpobCd, "commCd")));											// 시스템사업장코드
...
//		dto.setTplvlDpobCd(BasisUtil.getSelectedComboValue(tplvlDpobCd)); // 사업장 상위구분
...
//		dto.setSysDivCd(BasisUtil.getSelectedComboValue(sysDivCd)); // 적용시스템
...
//						});
	...
	public ContentPanel getViewPanel() {
		...
		lsRetryCllnDebrDivCd = PrgmComComboUtils.getSingleBass0300ComboData(sysComBass0300Dto);
		// -------------------------------------------------------------------------------------------
...
//			createStdSubFrom(); 		// 사업장 관리번호
	...
	// 생성자 함수에 변수을 받아 처리
	public Bass0100(String txtForm) {
	...
	private void createBass0100Form() {
		...
		plFrmBass0100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - " + lblBasisConst.title_Bass0100()));
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				 //로그 뷰화면 호출 메서드 
				  funcLogMessage(lblBasisConst.title_Bass0100(),"BASS0100");
				}
			});
...
//		btnBass0100Save.addSelectionListener(selectionListener);
		...
		plFrmBass0100.setSize("990px", "730px");
	}
...
//				dpobCd.setReadOnly(true);
//				BasisUtil.setSelectedComboValue(tplvlDpobCd, grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("tplvlDpobCd")+ ""); // 사업자 상위구분
...
//				}
//				
//				BasisUtil.setSelectedComboValue(sysDivCd, grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("sysDivCd") + "", "sysDivNm"); // 적용시스템
//				
...
////				BasisUtil.setSelectedComboValue(retryCllnDebrDivCd, grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("retryCllnDebrDivCd") + "");	//징수의무자
...
//		});
		 ...
		 * 
		 * BasisUtil.setSelectedComboValue(tplvlDpobCd,
...
//		BasisUtil.setSelectedComboValue(sysDivCd,(String)selRecord.get("commCd"),"commCd");		// 적용시스템
		...
		BasisUtil.setSelectedComboValue(sysDpobCd 
...
//		dpobFndtnUseYn.setValue("Y".eq?uals(grid.getStore().getAt(grid.getStore().indexOf(grid.getSelectionModel().getSelectedItem())).get("dpobFndtnUseYn")) ? true : false); //사용여부
    ...
    private void saveBass0100(Bass0100DTO dto, final ActionDatabase actionDatabase, final String fileName) {
