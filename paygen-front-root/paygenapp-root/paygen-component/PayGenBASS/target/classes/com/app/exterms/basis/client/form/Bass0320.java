...
import com.app.exterms.basis.client.form.defs.Bass0320Def;
import com.app.exterms.basis.client.languages.BasisConstants;
import com.app.exterms.basis.client.service.Bass0320Service;
import com.app.exterms.basis.client.service.Bass0320ServiceAsync;
 ...
 * =====================================================
 **/
public class Bass0320 extends MSFPanel {
	/**
	 * ########################################################################## 
	 * { 시스템 기본설정파일 선언부 시작}
	 * ##########################################################################
	...
	private Bass0320Def bass0320Def = new Bass0320Def(); // 그리드 테이블 컬럼 define
	// private MSFCustomForm msfCustomForm;O
	private MSFGridPanel msfBass0320GridPanel;
	
	//파일업로드처리 부분  
    private FileUploadField bass0320flUp;
	...
	// 입력수정삭제처리 rpc 처리부
	private Bass0320ServiceAsync bass0320Service = Bass0320Service.Util.getInstance();
...
//			dto.setRevnAddr(MSFSharedUtils.allowNulls(revnAddr.getValue()));    /** column 수정주소 : revnAddr */
			...
			dto.setChngTypOccuCd(BasisUtil.getSelectedComboValue(chngTypOccuCd,"typOccuCd"));	/** column 개편전 직종명 : chngTypOccuCd */
			dto.setTypOccuStdt(BasisUtil.getConvertDateToString(typOccuStdt, "yyyyMMdd"));	/** column 시작일자 : typOccuStdt */
			dto.setTypOccuEddt(BasisUtil.getConvertDateToString(typOccuEddt, "yyyyMMdd"));	/** column 종료일자 : typOccuEddt */
			...
			MessageBox.confirm("","DELETE".equals(actionDatabase.name()) ? "삭제 하시겠습니까?" : "저장 하시겠습니까?",new Listener<MessageBoxEvent>(){
				...
				public void handleEvent(MessageBoxEvent be) {
					...
					if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
						bass0320Service.saveBass0320(dto, actionDatabase, new AsyncCallback<Long>() {
							@Override
...
//			});
	...
	public Bass0320(String txtForm) {
	...
	private void createPsnl0350Form() {
		...
		plFrmBass0320.setSize("990px", "730px"); 
...
//		stTypOccuCd = PrgmComComboUtils.getSingleTypOccuCdBass0320ComboData(sysComBass0320Dto);
	...
	private void createListGrdForm() {
		...
		msfBass0320GridPanel = new MSFGridPanel(bass0320Def, false, false, false, true);
...
//	        		Window.alert(""+(String)grid.getSelectionModel().getSelectedItem().get("chngTypOccuCd"));
	        		...
	        		typOccuStdt.setValue(BasisUtil.getConvertStringToDate((String)grid.getSelectionModel().getSelectedItem().get("typOccuStdt"), "yyyyMMdd"));
	        		typOccuEddt.setValue(BasisUtil.getConvertStringToDate((String)grid.getSelectionModel().getSelectedItem().get("typOccuEddt"), "yyyyMMdd"));
	...
	/**
	  ...
	  private void createSubForm() {
	    ...
	    bass0320flUp = new FileUploadField();
	    bass0320flUp.setName("bass0320flUp");
	    bass0320flUp.setValidator(new Validator() {  
		...
		layoutContainer_2_1.add(bass0320flUp, new FormData("100%"));
		bass0320flUp.setFieldLabel("직종");
		  ...
		  MessageBox.confirm("직종 생성", "직종을 생성하시겠습니까?<br>",new Listener<MessageBoxEvent>(){
		    	...
		    	public void handleEvent(MessageBoxEvent be) {
		      		...
		      		if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
		      			...
		      			String filename = MSFSharedUtils.allowNulls(bass0320flUp.getValue());
...
//                                  MessageBox.alert("", "엑셀 업로드가 정상적으로 완료되었습니다.", null); 
	  ...
	  private void xlsExportData() {
		  ...
		  param.put("chngTypOccuCd",BasisUtil.getSelectedComboValue(chngTypOccuCd,"typOccuCd"));	/** column 개편전 직종명 : chngTypOccuCd */
		  param.put("typOccuStdt",BasisUtil.getConvertDateToString(typOccuStdt, "yyyyMMdd"));	/** column 시작일자 : typOccuStdt */
		  param.put("typOccuEddt",BasisUtil.getConvertDateToString(typOccuEddt, "yyyyMMdd"));	/** column 종료일자 : typOccuEddt */
