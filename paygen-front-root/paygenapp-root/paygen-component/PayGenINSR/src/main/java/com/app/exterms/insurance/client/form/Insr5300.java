...
=====================================================**/
public class Insr5300  extends MSFPanel { 
 ...
 **/
   // private RemtConstants lblRemtConst = RemtConstants.INSTANCE; 
    ...
    private FormPanel plFrmInsr5300; 
 ...
 **/	  
  ...
  // private AuthAction authAction;  
  		...
  		//  private static MessageBox authBox = MessageBox.wait("화면로딩", "설정 초기화 작업 진행 중입니다....", "설정 작업 진행 중..."); 
  		   ...
  		   * AuthAction 
  		   */
  		  private void checkInsr5300Auth( String authAction, ListStore<BaseModel> bm) {   
  		   ...
  		   //------------------
               ...
               /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	                ...
	                * 값순서 : 단위기관 ,고용구분, 부서, 직종, 직종세 ,사업 순으로  없으면 ""
	                ...
	                // GWTAuthorization.formAuthPopConfig(plFrmInsr5300, "srhPayrMangDeptCd","srhEmymtDivCd",strArrDeptCd,"srhTypOccuCd","srhDtilOccuInttnCd","srhBusinCd");
...
//   	   * AuthAction 
//   	   */
//   	  private void checkInsr5300Auth( AuthAction authAction, ListStore<BaseModel> bm, int authChk) {   
//       	   //MSFMainApp.maskMainPage("[화면로딩] 초기화 진행 중"); 
//   		 
...
public Insr5300(String txtForm) {
...
private void createPsnl0100Form() {
    ...
    plFrmInsr5300 = new FormPanel();
   
   // plFrmInsr5300.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" -  " + lblRemtConst.title_Insr4300());
    plFrmInsr5300.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 휴퇴직자정산관리"));
    plFrmInsr5300.setIcon(MSFMainApp.ICONS.text());
    //plFrmInsr5300.setBodyStyleName("pad-text");
    plFrmInsr5300.setFrame(true); 
	...
	btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
		public void handleEvent(ButtonEvent e) {
			 //로그 뷰화면 호출 메서드 
			  funcLogMessage("휴퇴직자정산관리","INSR5300");
			}
		});
	plFrmInsr5300.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
	plFrmInsr5300.getHeader().addTool(btnlogView); 
	/************************************************************************
    ...
    plFrmInsr5300.add(buttonBar);       
    
    //파일업로드 처리
    plFrmInsr5300.setAction("myurl");
    plFrmInsr5300.setEncoding(Encoding.MULTIPART);
    plFrmInsr5300.setMethod(Method.POST);
     
        
    vp.add(plFrmInsr5300);
    //엣지변환
    //plFrmInsr5300.setSize("990px", "680px");
    plFrmInsr5300.setSize("990px", "730px");
  }
...
//	     		plFrmInsr5300.add(typOccuCdGrid);
...
//	  		plFrmInsr5300.add(payYrGrid); 

	  
	    plFrmInsr5300.setLayout(new FlowLayout());
...
//	    lcSchLeft.setStyleAttribute("paddingRight", "10px");
	   ...
	   // srhPayYr.setEmptyText("--년도선택--");
	    ...
	    srhPayYr.getStore().addStoreListener( new StoreListener<BaseModel>() {   
	        public void handleEvent(StoreEvent<BaseModel> be) {  
	     	   checkInsr5300Auth("srhPayYr",lsPayYrStore);  
	   ...
	   // srhPayMonth.setEmptyText("--월선택--");
	    ...
	    srhPayMonth.getStore().addStoreListener( new StoreListener<BaseModel>() {   
	        public void handleEvent(StoreEvent<BaseModel> be) {  
	     	   checkInsr5300Auth("srhPayMonth",lsPayMonth);  
		    	   ...
		    	   if (type == Store.Add) { 
               		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkInsr5300Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
               	 } 
            }
        });    
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkInsr5300Auth("srhEmymtDivCd", lsEmymtDivCd); 
              	 }
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkInsr5300Auth("srhDeptCd", lsDeptCd); 
              	 }
        ...
        srhTypOccuCd.getStore().addStoreListener( new StoreListener<ModelData>() {   
            ...
            // srhEmymtDivCd.setValue(lsEmymtDivCd.getAt(0)); 
           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	                     ...
	                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
	                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
	                	//checkInsr5300Auth("srhTypOccuCd", lsTypOccuCd); 
	    		   }
	    		   } 
...
//		   }); 
	        ...
	        srhPayCd.getStore().addStoreListener( new StoreListener<BaseModel>() {   
	            public void handleEvent(StoreEvent<BaseModel> be) {  
	            	...
	            	checkInsr5300Auth("srhPayCd",lsPayCd);   
	            }
	        });    
...
//    fieldSet.setHeadingHtml("");
  ...
  //  layoutContainer_16.setBorders(false);  
    ...
    fldstNewFieldset.setSize("958px", "510px");
...
private LayoutContainer createCheckBoxGrid() {    
	    ...
	    column = new ColumnConfig();   
	    column.setId("last");   
	    column.setHeaderText("고용보험료");   
	    column.setAlignment(HorizontalAlignment.CENTER);   
	    column.setWidth(100);   
	    column.setRenderer(gridNumber);   
	    configs.add(column);   
	      
	    column = new ColumnConfig("change", "산정보험료", 100);   
	    column.setAlignment(HorizontalAlignment.CENTER);    
	    column.setRenderer(change);   
	    configs.add(column);    

	    column = new ColumnConfig("change", "경감보험료", 100);   
	    column.setAlignment(HorizontalAlignment.CENTER);    
	    column.setRenderer(change);   
	    configs.add(column);    
