...
import com.app.exterms.insurance.client.form.defs.Insr3150Def;
...
=====================================================**/
public class Insr4400  extends MSFPanel { 
 ...
 **/
   // private RemtConstants lblRemtConst = RemtConstants.INSTANCE; 
    ...
    private FormPanel plFrmInsr4400; 
...
//	  private MSFGridPanel typOccuCdGridPanel;
	  ...
	  private Insr3150Def insr3150Def  = new Insr3150Def("INSR4400");   //그리드 테이블 컬럼 define  
	    // private MSFCustomForm msfCustomForm; 
	      private MSFGridPanel insr3150GridPanel;
      ...
      private ButtonBar topInsr4400Bar;
      private Button btnInsr4400Init;
    //  private Button btnInsr4400New;
    //  private Button btnInsr4400Save;
    //  private Button btnInsr4400Del;
      private Button btnInsr4400Sreach;
      private Button btnInsr4400File;
   //   private Button btnInsr4400Print;
      private Button btnInsr4400Excel;
      /**
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       * 검색변수 선언 
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      ...
      /** column  년도 : year */
      private ComboBox<BaseModel> srhHlthInsrYr; 
...
//      private TextField<String> srhMangeDeptNm; //관리부서 수정  
      ...
      // --------------- 대상자 업로드 ---------------
      private FileUploadField insr4400flUp;
 ...
 **/	  
  ...
  // private AuthAction authAction;  
  		...
  		//  private static MessageBox authBox = MessageBox.wait("화면로딩", "설정 초기화 작업 진행 중입니다....", "설정 작업 진행 중..."); 
  		   ...
  		   * AuthAction 
  		   */
  		  private void checkInsr4400Auth( String authAction, ListStore<BaseModel> bm) {   
  		 ...
  		 private void authExecEnabled() { 
            ...
            // srhPayMonth.setValue(lsPayMonth.findModel("month",DateTimeFormat.getFormat("MM").format(new Date())));
              ...
              * 값순서 : 단위기관 ,고용구분, 부서, 직종, 직종세 ,사업 순으로  없으면 ""
              ...
              // GWTAuthorization.formAuthPopConfig(plFrmInsr4400, "srhPayrMangDeptCd","srhEmymtDivCd",strArrDeptCd,"srhTypOccuCd","srhDtilOccuInttnCd","srhBusinCd");
		   ...
		   final Timer tmMask = new Timer()
  		  ...
  		  private void initLoad() { 
  			    ...
  			    // 권한처리 부분 추가 전역이 아니 각 화면별로 객체 생성시 처리를 위해 변경함.
  			     ...
  			     authMapDef.put("srhHlthInsrYr",Boolean.FALSE);
...
//   	   * AuthAction 
//   	   */
//   	  private void checkInsr4400Auth( AuthAction authAction, ListStore<BaseModel> bm, int authChk) {   
//       	   //MSFMainApp.maskMainPage("[화면로딩] 초기화 진행 중"); 
//   		 
...
public Insr4400(String txtForm) {
...
private void createPsnl0100Form() {
    ...
    plFrmInsr4400 = new FormPanel();
   
   // plFrmInsr4400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" -  " + lblRemtConst.title_Insr4400());
    plFrmInsr4400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 건강보험연말정산"));
    plFrmInsr4400.setIcon(MSFMainApp.ICONS.text());
    //plFrmInsr4400.setBodyStyleName("pad-text");
    plFrmInsr4400.setPadding(2);
    plFrmInsr4400.setFrame(true); 
	...
	btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
		public void handleEvent(ButtonEvent e) {
			 //로그 뷰화면 호출 메서드 
			  funcLogMessage("건강보험연말정산","INSR4400");
			}
		});
	plFrmInsr4400.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
	plFrmInsr4400.getHeader().addTool(btnlogView); 
	/************************************************************************
    ...
    //멀티콤보박스 닫기 
    plFrmInsr4400.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
        @Override
...
//                    }
    ...
    topInsr4400Bar = new ButtonBar();    
    topInsr4400Bar.setAlignment(HorizontalAlignment.RIGHT);
 
  
    btnInsr4400Init = new Button("초기화");  
    btnInsr4400Init.setIcon(MSFMainApp.ICONS.new16());
    topInsr4400Bar.add(btnInsr4400Init);
    btnInsr4400Init.addListener(Events.Select, new Listener<ButtonEvent>() {
        public void handleEvent(ButtonEvent e) {
            //초기화 버튼 클릭시 처리     
        	formInit(); 
...
//    buttonBar.add(topBtn);
    ...
    btnInsr4400Sreach = new Button("조회"); 
    btnInsr4400Sreach.setIcon(MSFMainApp.ICONS.search16());
    topInsr4400Bar.add(btnInsr4400Sreach);
    btnInsr4400Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
		public void handleEvent(ButtonEvent e) {
			//조회버튼 클릭시 처리 
		    reload();  
		}
    ...
    btnInsr4400File = new Button("보수총액신고파일"); 
    btnInsr4400File.setIcon(MSFMainApp.ICONS.excel16()); 
    topInsr4400Bar.add(btnInsr4400File);   
    btnInsr4400File.addListener(Events.Select, new Listener<ButtonEvent>() {
		public void handleEvent(ButtonEvent e) {
		   //Window.alert("엑셀저장을 이용하세요.");
		   excTxtFileExport();
		}
	});
    
    btnInsr4400Excel = new Button("엑셀"); 
    btnInsr4400Excel.setIcon(MSFMainApp.ICONS.excel16()); 
    topInsr4400Bar.add(btnInsr4400Excel);       
    btnInsr4400Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
		public void handleEvent(ButtonEvent e) {
			excelFileExport();
		}
	});
    
    plFrmInsr4400.add(topInsr4400Bar);       
    
    //파일업로드 처리
    plFrmInsr4400.setAction("myurl");
    plFrmInsr4400.setEncoding(Encoding.MULTIPART);
    plFrmInsr4400.setMethod(Method.POST);
     
        
    vp.add(plFrmInsr4400);
    //엣지변환
    //plFrmInsr4400.setSize("990px", "680px");
    plFrmInsr4400.setSize("990px", "730px");
  }
...
//	     		plFrmInsr4400.add(typOccuCdGrid);
...
//	  		    
//	  		   //년도 콤보처리 
//	  		   yearGridPanel = new MSFGridPanel(lkPayYr, false, false, false, true); 
...
//	  			          public void handleEvent(StoreEvent<BaseModel> be) {  
//	  			        	 lsPayYrStore.add(yearGridPanel.getMsfGrid().getStore().getModels()); 
//	  			        	 
//	  			        	//  Date today = new Date(); 
//	  			        	    // A custom date format
//	  			        	//  DateTimeFormat fmt = DateTimeFormat.getFormat("yyyy"); 
//	  			        	 
//	  			      	     // BaseModel  bm  =  srhHlthInsrYr.getStore().findModel("apptnYr", fmt.format(today) ); 
//	  			      	     // srhHlthInsrYr.setValue(bm);
//	  			        	srhHlthInsrYr.setValue(lsPayYrStore.getAt(0));
//	  			        	Date today = new Date(); 
...
//	  		plFrmInsr4400.add(yearGrid); 

	  
	    plFrmInsr4400.setLayout(new FlowLayout());
	   ...
	   // srhHlthInsrYr.setEmptyText("--년도선택--");
	    ...
	    srhHlthInsrYr.getStore().addStoreListener( new StoreListener<BaseModel>() {   
	        public void handleEvent(StoreEvent<BaseModel> be) {  
	     	   checkInsr4400Auth("srhHlthInsrYr",lsPayYrStore);  
...
//	        public void handleEvent(StoreEvent<BaseModel> be) {  
//	     	   checkInsr4400Auth("srhPayMonth",lsPayMonth);  
//	          
//	        }
//	    });
		    	   ...
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkInsr4400Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
              	 } 
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkInsr4400Auth("srhEmymtDivCd", lsEmymtDivCd); 
              	 }
            ...
            public void handleEvent(StoreEvent<ModelData> be) {  
		    	 ...
		    	 if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkInsr4400Auth("srhDeptCd", lsDeptCd); 
              	 }
		    	   ...
		    	   if (type == Store.Add) { 
		    		 //2015.11.30 추가 
              		 ...
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	//checkInsr4400Auth("srhTypOccuCd", lsTypOccuCd); 
		    		   }
		    		   }
...
//	   }); 
     ...
     //   srhHdofcCodtnCd.setHeight(100);
        ...
        srhHdofcCodtnCd.getStore().addStoreListener( new StoreListener<ModelData>() {   
            public void handleEvent(StoreEvent<ModelData> be) {  
            	checkInsr4400Auth("srhHdofcCodtnCd",lsHdofcCodtnCd); 
            }
        }); 
...
//    fieldSet.setHeadingHtml("");
  ...
  //  layoutContainer_16.setBorders(false);  
    ...
    fldstNewFieldset.setSize("958px", "455px");
...
//	  
//	    
//	    column = new ColumnConfig("change", "사용자부담금(건+장)", 100);   
//	    column.setAlignment(HorizontalAlignment.CENTER);    
...
//	    lcStdGrid.add(cpGrid);   
	  ...
	  //  frmlytStd.setLabelWidth(85); 
	   // frmlytStd.setLabelAlign(LabelAlign.RIGHT);   
	    ...
	    cpGrid.setHeaderVisible(false);   
	    cpGrid.setLayout(new FitLayout());      
	    cpGrid.setSize(945, 425);  
	    
	    //msfCustomForm.setHeaderVisible(false);
	    //Instantiate the GridPanel
	    //insr3150GridPanel = new MSFGridPanel(insr3150Def, false, false, false, false,MSFGridType.GROUPBY,"deptNm");
	    insr3150GridPanel = new MSFGridPanel(insr3150Def, false, false, false, false,false);
	    insr3150GridPanel.getMsfGrid().setCellModel(true);
	    insr3150GridPanel.setHeaderVisible(false);  
	    insr3150GridPanel.setBodyBorder(true);
	    insr3150GridPanel.setBorders(true);
	    insr3150GridPanel.getBottomComponent().setVisible(true);
	     
	    
		//   insr3150GridPanel.setSize(308, 466); 
	 //   msfGridPanel.setMSFFormPanel(msfCustomForm);  //폼을 그리드에 넣은다. 
	   // GWTUtils.findColumnConfig(insr3150GridPanel, Insr3150BM.ATTR_NATPENNGRDE).setStyle("background-color:#F5F6CE;");
	  //  GWTUtils.findColumnConfig(insr3150GridPanel, Insr3150BM.ATTR_NATPENNSTDINCMMNTHAMNT).setStyle("background-color:#F5F6CE;");
	  //  GWTUtils.findColumnConfig(insr3150GridPanel, Insr3150BM.ATTR_NATPENNAQTNDT).setStyle("background-color:#F5F6CE;");
...
//	});
	    ...
	    //2줄파싱 
	    insr3150GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 6, new HeaderGroupConfig("전년도보험료납부총액", 1, 3));
	  //  insr3150GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 0, new HeaderGroupConfig("Stock Information", 1, 2));
		  //  final ListStore<BaseModel> store = new ListStore<BaseModel>();  
		   // store.add(dlgn0300GridPanel.getGrid().getStore().getModels());  
		     
		  //  AggregationRowConfig<BaseModel> insr3150SummaryRow = new AggregationRowConfig<BaseModel>();  
		 
		   // insr3150SummaryRow.setCellStyle(Insr3150BM.ATTR_EMYMTDIVNM,"summary_color");
		   // insr3150SummaryRow.setCellStyle(Insr3150BM.ATTR_NATPENNSTDINCMMNTHAMNT,"summary_color");
		   //  String htmlAttr = "<span style='background-color:yellow; background-repeat: no-repeat;'>합 계</span>" ;
		    
		   // insr3150SummaryRow.setHtml(Insr3150BM.ATTR_EMYMTDIVNM, "합 계");   
		    
		    // with summary type and format  
		   // insr3150SummaryRow.setSummaryType(Insr3150BM.ATTR_NATPENNSTDINCMMNTHAMNT, SummaryType.SUM);   
		   // insr3150SummaryRow.setSummaryFormat(Insr3150BM.ATTR_NATPENNSTDINCMMNTHAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
		    
		   // insr3150SummaryRow.setSummaryType(Insr3150BM.ATTR_NATPENNMNTHAMNT, SummaryType.SUM);   
		  //  insr3150SummaryRow.setSummaryFormat(Insr3150BM.ATTR_NATPENNMNTHAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
...
//		    });  
		  ...
		  // insr3150GridPanel.getGrid().getColumnModel().addAggregationRow(insr3150SummaryRow);  
		    
	    cpGrid.add(insr3150GridPanel); 
	    lcStdGrid.add(cpGrid);    
...
private void createBottom() {
	...
	FieldSet fldstNewFieldset = new FieldSet();	
	fldstNewFieldset.setHeadingHtml("보수총액대상자업로드");
...
//	layoutContainer_1.setLayout(new HBoxLayout()); 
	...
	insr4400flUp = new FileUploadField();
	insr4400flUp.setName("insr4400flUp");
	insr4400flUp.setValidator(new Validator() {  
   		@Override  
	...
	layoutContainer_2.add(insr4400flUp, new FormData("100%"));
	insr4400flUp.setFieldLabel("보수총액대상자");
...
//	layoutContainer_1.add(layoutContainer_2, hbld_layoutContainer_2);
	...
	Button btnXlsInsr4400Upload = new Button("엑셀업로드");
	btnXlsInsr4400Upload.addListener(Events.Select, new Listener<ButtonEvent>() {
 		public void handleEvent(ButtonEvent e) {
 			xlsImportData();	//대상자 업로드
 			//TO DO 건강보험 관리공단 대상자 업로드시 Method 호출(취득일자 Null인것)
 		}
	});
	layoutContainer_4.add(btnXlsInsr4400Upload, new FormData("100%"));
	layoutContainer_4.setBorders(false);
	...
	Button btnTxtInsr4400Upload = new Button("텍스트업로드");
	btnTxtInsr4400Upload.addListener(Events.Select, new Listener<ButtonEvent>() {
 		public void handleEvent(ButtonEvent e) {
 			Window.alert("현재 텍스트 업로드는 지원하지 않습니다."); 
 		}
	});
	layoutContainer_5.add(btnTxtInsr4400Upload, new FormData("100%"));
	layoutContainer_5.setBorders(false);
	...
	Button btnInsr4400down = new Button("양식다운");
	btnInsr4400down.addListener(Events.Select, new Listener<ButtonEvent>() {
		public void handleEvent(ButtonEvent e) {
			xlsExportData();
		}
	}); 
	layoutContainer_3.add(btnInsr4400down, new FormData("100%"));
...
//			    
//			  MSFFormWindows msFwInsr31501 = new MSFFormWindows("예외건강보험",insrP41001Form,"닫기","820px", "710px",true);
//			  msFwInsr31501.show();
//			  insrP41001Form.setMSFFormWindows(msFwInsr31501);
...
// 			    
// 			  MSFFormWindows msFwInsr31502 = new MSFFormWindows("월별납부실적",insrP41002Form,"닫기","820px", "710px",true);
// 			  msFwInsr31502.show();
// 			  insrP41002Form.setMSFFormWindows(msFwInsr31502);
...
// 			    
// 			  MSFFormWindows msFwInsr31503 = new MSFFormWindows("개별납부실적",insrP41003Form,"닫기","820px", "710px",true);
// 			  msFwInsr31503.show();
// 			  insrP41003Form.setMSFFormWindows(msFwInsr31503);
...
public void reload() {
	...
	insr3150GridPanel.getTableDef().setTableColumnFilters(filters);
	
	insr3150GridPanel.getTableDef().addColumnFilter("hlthInsrYr",  MSFSharedUtils.getSelectedComboValue(srhHlthInsrYr,"year"), SimpleColumnFilter.OPERATOR_EQUALS);  
	
	insr3150GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
	insr3150GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
	insr3150GridPanel.getTableDef().addColumnFilter("repbtyBusinDivCd", MSFSharedUtils.getSelectedComboValue(srhRepbtyBusinDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
	//insr3150GridPanel.getTableDef().addColumnFilter("payCd", MSFSharedUtils.getSelectedComboValue(srhPayCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
	 

    String strDeptCd = InsrUtils.getStrValToBMMultiCombo(lsDeptCd,srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
    insr3150GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);
   
    //20180305추가 
    insr3150GridPanel.getTableDef().addColumnFilter("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
  		
    String strTypOccuCd = InsrUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
    insr3150GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
//    String strPyspGrdeCd = InsrUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","pyspGrdeCd");  
//    insr3150GridPanel.getTableDef().addColumnFilter("pyspGrdeCd", strPyspGrdeCd, SimpleColumnFilter.OPERATOR_EQUALS);  
    ...
    insr3150GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 
  
	//insr3150GridPanel.getTableDef().addColumnFilter("deptCd", MSFSharedUtils.allowNulls(srhDeptCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
	//insr3150GridPanel.getTableDef().addColumnFilter("typOccuCd", MSFSharedUtils.getSelectedComboValue(srhTypOccuCd,"typOccuCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
	//insr3150GridPanel.getTableDef().addColumnFilter("pyspGrdeCd",  MSFSharedUtils.getSelectedComboValue(srhTypOccuCd,"pyspGrdeCd") , SimpleColumnFilter.OPERATOR_EQUALS); 
	//insr3150GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", MSFSharedUtils.getSelectedComboValue(srhDtilOccuInttnCd,"dtilOccuInttnCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
  
	insr3150GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
	insr3150GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getRawValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
	insr3150GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getRawValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
	insr3150GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getRawValue()), SimpleColumnFilter.OPERATOR_EQUALS);  
   ...
   if (MSFSharedUtils.paramNotNull(srhHdofcCodtnCd.getValue())) {
    ...
    insr3150GridPanel.getTableDef().addColumnFilter("hdofcCodtnCd", strHdofcCodtnCd, SimpleColumnFilter.OPERATOR_EQUALS); 
      
	 
	insr3150GridPanel.reload();
...
// actionDatabase = ActionDatabase.READ;
   	...
   	private void xlsExportData() {
   		...
   		param.put("hlthInsrYr",  MSFSharedUtils.getSelectedComboValue(srhHlthInsrYr,"year")); 
   		...
   		if (MSFConfiguration.EMYMT_DIVCD02.equals(MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"))) {
   			...
   			if("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))){ 
 		    	...
 		    	insr3150GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsInsr4400Export.do","extgwtFrame" ,param);   
 		     }
        }else{ 
      	  insr3150GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsInsr4400Export.do","extgwtFrame" ,param);  
        }
	...
	private void xlsImportData() { 
		MessageBox.confirm("대상자 생성", "보수총액 대상자를 생성하시겠습니까?<br>(등록되어있는 대상자가 있을시 삭제 처리 됩니다.)",new Listener<MessageBoxEvent>(){
	      	@Override
...
//	      			HashMap<String, String> param = new HashMap<String, String>();
	            	...
	            	String year = MSFSharedUtils.getSelectedComboValue(srhHlthInsrYr, "year");
	            	
	            	if("".equals(year)){
	            		MessageBox.info("", "귀속년도는 필수 입니다.", null);
	             	}else{ 
	              		//get the filename to be uploaded
	              		String filename = MSFSharedUtils.allowNulls(insr4400flUp.getValue());
	              		if (filename.length() == 0) {
	              			Window.alert("선택된 파일이 없습니다.");
	                	} else {
	                		//submit the form
	                		plFrmInsr4400.mask("엑셀 데이타 업데이트 중입니다. 업로드 중..."); 
	                		//파일업로드 처리
	                		
	                		plFrmInsr4400.setAction(GWT.getHostPageBaseURL() + "imp/XlsReadInsr4400Import.do");
	                		plFrmInsr4400.setEncoding(Encoding.MULTIPART);
	                		plFrmInsr4400.setMethod(Method.POST);   
	                		plFrmInsr4400.submit();  
	                		plFrmInsr4400.onFrameLoad();  
	                    	picBoolFile = true;
	                	}               
	                       
	                    // reset and unmask the form 
	                    // after file upload 
	              		plFrmInsr4400.addListener(Events.Submit, new Listener<FormEvent>() {
	                		public void handleEvent(FormEvent evt) {   
	                			if ("success".endsWith(evt.getResultHtml()) && picBoolFile) {
	                				//업로드가  성공했으면 인서트 모듈을 태운다. 
	                				plFrmInsr4400.unmask();  
				           ...
				           param.put("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue())); 
		                      ...
		                      if (MSFConfiguration.EMYMT_DIVCD02.equals(MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"))) {
		                    	  ...
		                    	  if("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))){ 
		               		    	...
		               		    	insr3150GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/TxtFileInsr4400Export.do","extgwtFrame" ,param);   
		               		     }
		                      }else{ 
				           ...
				           param.put("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue())); 
		                      ...
		                      if (MSFConfiguration.EMYMT_DIVCD02.equals(MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"))) {
		                    	  ...
		                    	  if("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))){ 
		               		    	...
		               		    	insr3150GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsFileInsr4400Export.do","extgwtFrame" ,param);   
		               		     }
		                      }else{ 
