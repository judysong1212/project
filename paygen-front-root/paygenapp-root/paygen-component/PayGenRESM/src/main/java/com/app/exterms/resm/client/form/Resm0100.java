...
import com.app.exterms.resm.client.form.defs.Bass0300Def;
import com.app.exterms.resm.client.languages.ResmConstants;
import com.app.exterms.resm.client.service.Resm0100Service;
import com.app.exterms.resm.client.service.Resm0100ServiceAsync;
 ...
 * =====================================================
 **/
public class Resm0100 extends MSFPanel {
	/**
	 * ########################################################################## 
	 * { 시스템 기본설정파일 선언부 시작}
	 * ##########################################################################
	...
	private FormPanel plFrmResm0100;
	...
	// -------------- 서비스 호출 시작 --------------
	private Resm0100ServiceAsync resm0100Service = Resm0100Service.Util.getInstance();	// 입력수정삭제처리 rpc 처리부
	// -------------- 서비스 호출 시작 --------------
	...
	public void save(final ActionDatabase actionDatabase){
		...
		if( "".equals(ResmUtils.getSelectedComboValue(rpsttvCd,"rpsttvCd")) ){
			...
			if("DELETE".equals(actionDatabase.name())){
				resm0100Service.activityOnDeleteBass0300(dto, new AsyncCallback<String>() {
				...
				resm0100Service.activityOnSaveBass0300(dto, actionDatabase, new AsyncCallback<String>() {
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
...
//			});
			...
			vp.setSpacing(10);
			createResm0100Form(); 	// 화면 기본정보를 설정
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createResm0100Form() {

		plFrmResm0100 = new FormPanel();
		plFrmResm0100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - ") + lblPayrConst.title_Resm0100());
		plFrmResm0100.setIcon(Resources.APP_ICONS.text());
		plFrmResm0100.setPadding(2);
		plFrmResm0100.setBodyStyleName("pad-text");
		plFrmResm0100.setFrame(true);
		plFrmResm0100.setSize("990px", "680px");
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				 //로그 뷰화면 호출 메서드 
				  funcLogMessage(lblPayrConst.title_Resm0100(),"PAYR5010");
				}
			});
		plFrmResm0100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmResm0100.getHeader().addTool(btnlogView); 
		/************************************************************************
		...
		*/
		...
		plFrmResm0100.add(topBass0300Bar);

		vp.add(plFrmResm0100);
	}
	...
	private void createSearchForm() {
		...
		plFrmResm0100.setLayout(new FlowLayout());
		...
		ResmUtils.setSelectedComboValue(srhRpsttvCd, "C002", "rpsttvCd");
		...
		plFrmResm0100.add(fieldSetSrhFrm);
	}
	/**
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	 * 검색 함수 선언부 종료
	...
	private void createStandardForm() {
		...
		ResmUtils.setSelectedComboValue(rpsttvCd, "C002", "rpsttvCd");
	...
	private void createListGrdForm() {
		...
		grid.addListener(Events.RowClick, new Listener<GridEvent<BaseModel>>() {
			...
			public void handleEvent(GridEvent<BaseModel> be) {
				...
				if (msfBass0300GridPanel.getCurrentlySelectedItem() != null) {  
					ResmUtils.setSelectedComboValue(rpsttvCd, (String) msfBass0300GridPanel.getCurrentlySelectedItem().get("rpsttvCd"), "rpsttvCd");
		...
		plFrmResm0100.add(fieldSetGrd, new FormData("0% -475"));
