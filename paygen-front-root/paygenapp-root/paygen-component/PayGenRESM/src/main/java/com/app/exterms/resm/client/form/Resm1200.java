 ...
 * =====================================================
 **/
public class Resm1200 extends MSFPanel {
	/**
	 * ########################################################################## 
	 * { 시스템 기본설정파일 선언부 시작}
	 * ##########################################################################
	...
	// -------------- 서비스 호출 시작 --------------
	private Resm1200ServiceAsync resm1200Service = Resm1200Service.Util.getInstance();	// 입력수정삭제처리 rpc 처리부
	// -------------- 서비스 호출 시작 --------------
...
//			    		  	  ResmUtils.setSelectedComboValue(chngDtilOccuInttnCd, (String)bmData.get("chngDtilOccuInttnCd"),"dtilOccuInttnCd");
//			    		  			    		  	 
//                		  }
//                	  }); 
//                }
...
//	};
	...
	public void save(final ActionDatabase actionDatabase){
		//|| "".equals(ResmUtils.getConvertDateToString(dtilOccuInttnEddt, "yyyyMMdd"))
		final Bass0350DTO dto = new Bass0350DTO();
		if( "".equals(ResmUtils.getSelectedComboValue(typOccuCd,"typOccuCd")) ){
...
//			dto.setDpobCd(dpobCd.getValue());									
			...
			dto.setTypOccuCd(ResmUtils.getSelectedComboValue(typOccuCd,"typOccuCd"));	//직종코드
			dto.setDtilOccuClsDivCd(dtilOccuClsDivCd.getValue());						//직종세구분코드
			dto.setPyspGrdeCd(ResmUtils.getSelectedComboValue(pyspGrdeCd));				//호봉등급코드
			
			dto.setDtilOccuJbfmlDivCd(ResmUtils.getSelectedComboValue(dtilOccuJbfmlDivCd));	//직군
			dto.setBggLogSvcYrNumCd(ResmUtils.getSelectedComboValue(bggLogSvcYrNumCd));		//최대근속년수
...
//			dto.setDtilOccuInttnCd(dtilOccuClsDivCd.getValue() + ResmUtils.getSelectedComboValue(pyspGrdeCd));
			
			dto.setChngTgtYn(chngTgtYn.getValue() ? "Y" : "N");							//전환대상여부
			
			dto.setChngTypOccuCd(ResmUtils.getSelectedComboValue(chngTypOccuCd,"typOccuCd"));	/** column 개편전 직종 : chngTypOccuCd */
			dto.setChngDtilOccuInttnCd(ResmUtils.getSelectedComboValue(chngDtilOccuInttnCd,"dtilOccuInttnCd"));	/** column 개편전 직종세 : chngDtilOccuInttnCd */
			dto.setDtilOcclsApptnUcstCd(ResmUtils.getSelectedComboValue(dtilOcclsApptnUcstCd,"commCd"));	/** column 직종세 전용단가코드 : dtilOcclsApptnUcstCd */
			dto.setDtilOcclsApptnUcstSum((Long)dtilOcclsApptnUcstSum.getValue());	/** column 직종세 전용단가 : dtilOcclsApptnUcstSum */
			dto.setDtilOccuInttnStdt(ResmUtils.getConvertDateToString(dtilOccuInttnStdt, "yyyyMMdd"));	/** column 시작일자 : dtilOccuInttnStdt */
			dto.setDtilOccuInttnEddt(ResmUtils.getConvertDateToString(dtilOccuInttnEddt, "yyyyMMdd"));	/** column 종료일자 : dtilOccuInttnEddt */
					
			// 회계코드, 기간제 사용여부, 단가일수 우선 숨김
//			dto.setDtilOccuInttnAccCd(ResmUtils.getSelectedComboValue(dtilOccuInttnAccCd,"commCd")); /** column 직종제회계코드 : dtilOccuInttnAccCd */
			...
			MessageBox.confirm("","DELETE".equals(actionDatabase.name()) ? "삭제 하시겠습니까?" : "저장 하시겠습니까?",new Listener<MessageBoxEvent>(){
				...
				public void handleEvent(MessageBoxEvent be) {
					...
					if(Dialog.YES.equals(be.getButtonClicked().getItemId())){						
						resm1200Service.saveBass0350(dto, actionDatabase, new AsyncCallback<String>() {
							@Override
...
//			});
	...
	public Resm1200(String txtForm) {
	...
	private void createPsnl0350Form() {
		...
		plFrmBass0350.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - ") + lblBasisConst.title_Resm1200());
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				 //로그 뷰화면 호출 메서드 
				  funcLogMessage(lblBasisConst.title_Resm1200(),"BASS0350");
				}
			});
		...
		btnBass0350Trnr.addListener(Events.Select, new Listener<ButtonEvent>() {
    		public void handleEvent(ButtonEvent e) {
    			...
    			ResmP120001 bassP03501Form = new ResmP120001(ActionDatabase.READ, getThis());   
		...
		grid.addListener(Events.RowClick, new Listener<BaseEvent>() {
			...
			public void handleEvent(BaseEvent be) {				
				if (msfBass0350GridPanel.getCurrentlySelectedItem() != null){
	        		...
	        		ResmUtils.setSelectedComboValue(chngDtilOccuInttnCd, (String)bmData.get("chngDtilOccuInttnCd"),"dtilOccuInttnCd");/** column 개편전 직종세 : chngDtilOccuInttnCd */
	        		...
	        		pyspGrdeCd.setEnabled(false);
					//ResmUtils.setSelectedComboValue(typOccuCd, grid.getSelectionModel().getSelectedItem().get("typOccuCd")+"");
	        		//ResmUtils.setSelectedComboValue(pyspGrdeCd, grid.getSelectionModel().getSelectedItem().get("pyspGrdeCd")+"");
...
//					Window.alert(""+(String)bmData.get("dtilOcclsApptnUcstCd"));
					...
					ResmUtils.setSelectedComboValue(chngTypOccuCd, (String)bmData.get("chngTypOccuCd"),"typOccuCd");/** column 개편전 직종 : chngTypOccuCd */
					dtilOcclsApptnUcstCd.setEnabled(false);
					ResmUtils.setSelectedComboValue(dtilOcclsApptnUcstCd, (String)bmData.get("dtilOcclsApptnUcstCd"),"commCd");/** column 직종세 전용단가코드 : dtilOcclsApptnUcstCd */
					dtilOcclsApptnUcstSum.setValue((Long)bmData.get("dtilOcclsApptnUcstSum"));/** column 직종세 전용단가 : dtilOcclsApptnUcstSum */
					dtilOccuInttnStdt.setValue(ResmUtils.getConvertStringToDate((String)grid.getSelectionModel().getSelectedItem().get("dtilOccuInttnStdt"), "yyyyMMdd"));/** column 시작일자 : dtilOccuInttnStdt */
					dtilOccuInttnEddt.setValue(ResmUtils.getConvertStringToDate((String)grid.getSelectionModel().getSelectedItem().get("dtilOccuInttnEddt"), "yyyyMMdd"));/** column 종료일자 : dtilOccuInttnEddt */
					
					// 회계코드, 기간제 사용여부, 단가일수 우선 숨김
//					ResmUtils.setSelectedComboValue(dtilOccuInttnAccCd, (String)bmData.get("dtilOccuInttnAccCd"),"commCd"); /** column 직종제회계코드 : dtilOccuInttnAccCd */
...
//                                MessageBox.alert("", "엑셀 업로드가 정상적으로 완료되었습니다.", null); 
	  ...
	  private void xlsExportData() {
		  ...
		  param.put("typOccuCd", ResmUtils.getSelectedComboValue(srhTypOccuCd, "typOccuCd"));	//직종코드
	...
	public void reload() {
		...
		msfBass0350GridPanel.getTableDef().addColumnFilter("srhTypOccuCd", ResmUtils.getSelectedComboValue(srhTypOccuCd, "typOccuCd"), SimpleColumnFilter.OPERATOR_LIKE);	//직종명
