...
import com.app.exterms.payroll.client.service.PayrP525001ServiceAsync;
...
=====================================================**/
public class PayrP525001  extends MSFFormPanel { 

	private ContentPanel payrP525001;
	private MSFPanel caller;

	private Payr0416Def pPayr5250Def  = new Payr0416Def("payrP525001");   //그리드 테이블 컬럼 define
	private MSFGridPanel pPayr525001GridPanel;
	...
	private void setpayrP525001FormBinding() {
	}      

	private PayrP525001ServiceAsync payrP525001Service = PayrP525001Service.Util.getInstance();	 
	
	
	
	public PayrP525001(ActionDatabase actionDatabase, MSFPanel caller) {
		// super();
		...
		payrP525001 = new ContentPanel();
		createpayrP525001Form();
		createSearchForm();
		createStandardForm();

		payrP525001.setBodyBorder(false);
		payrP525001.setBorders(false);
		payrP525001.setHeaderVisible(false);

		this.add(payrP525001);
	...
	private  void createpayrP525001Form() { 
		payrP525001.setSize("600px", "400px");
	}
...
//							bm.set("deptCd", MSFSharedUtils.getSelectedComboValue(createYr,"year"));
//							bm.set("typOccuCd", MSFSharedUtils.getSelectedComboValue(createYr,"year"));
//							bm.set("dtilOccuInttnCd", MSFSharedUtils.getSelectedComboValue(createYr,"year"));
//							bm.set("pyspGrdeCd", MSFSharedUtils.getSelectedComboValue(createYr,"year"));
							
							
							if(pPayr525001GridPanel.getGrid().getSelectionModel().getSelectedItems().size() > 0){							
//								payrP525001Service.activityOnPayr0415Copy(pPayr525001GridPanel.getGrid().getSelectionModel().getSelectedItems(), bm, new AsyncCallback<Long>() {
//									@Override
...
//								});
	...
	private  void createStandardForm() {
		...
		payrP525001.add(layoutContainer);
	...
	private LayoutContainer createCheckBoxGrid() {    
		...
		pPayr525001GridPanel = new MSFGridPanel(pPayr5250Def, false, false, false, false, false);
		pPayr525001GridPanel.setHeaderVisible(false);  
		pPayr525001GridPanel.setBodyBorder(true);
		pPayr525001GridPanel.setBorders(true);
		//pPayr525001GridPanel.getBottomComponent().setVisible(false);

		


		final Grid pPayr150001Grid = pPayr525001GridPanel.getMsfGrid().getGrid(); 
		pPayr150001Grid.addListener(Events.CellClick,  new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (pPayr525001GridPanel.getCurrentlySelectedItem() != null) {  
					
					
					if(MSFSharedUtils.allowNulls(pPayr525001GridPanel.getCurrentlySelectedItem().get("transferYn")).equals("true")){
						//Window.alert("직종세이관 완료된 필드");

						//pPayr525001GridPanel.getMsfGrid().getTableDef().getColumnModel().getColumn(0).getId();
						//pPayr525001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		cpGrid.add(pPayr525001GridPanel); 			    
	...
	public void reload() {
		...
		pPayr525001GridPanel.getMsfGrid().clearData();
		pPayr525001GridPanel.getTableDef().setTableColumnFilters(filters);			
		pPayr525001GridPanel.getTableDef().addColumnFilter("payYr", MSFSharedUtils.getSelectedComboValue(targetYr,"payYr"), SimpleColumnFilter.OPERATOR_EQUALS);
		pPayr525001GridPanel.getTableDef().addColumnFilter("createYr", MSFSharedUtils.getSelectedComboValue(createYr,"year"), SimpleColumnFilter.OPERATOR_EQUALS);
		pPayr525001GridPanel.reload();
