...
=====================================================**/
public class Payr4550  extends MSFPanel { 
 ...
 **/
	  ...
	  private FormPanel plFrmPayr4550;
	  //private FormData formData; 
	  private String txtForm = "";
	  private XTemplate detailTp; 
		   
	 
	  private final Payr0301Def payr0301Def  = new Payr0301Def("PAYR4550");   //그리드 테이블 컬럼 define  
       ...
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       */
//      private ButtonBar topPayr4550Bar;
    //  private Button btnPayr4550New;
    //  private Button btnPayr4550Save;
    //  private Button btnPayr4550Del;
//      private Button btnPayr4550Sreach;
      /**
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       * 검색변수 선언 
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 **/	  
   ...
   // private AuthAction authAction;  
   			...
   			//  private static MessageBox authBox = MessageBox.wait("화면로딩", "설정 초기화 작업 진행 중입니다....", "설정 작업 진행 중..."); 
   			   ...
   			   * AuthAction 
   			   */
   			  private void checkPayr4550Auth( String authAction, ListStore<BaseModel> bm) {   
	        	 ...
	        	 srhPayYr.setValue(lsPayYrStore.findModel("payYr", fmt.format(today)) );
					...
					/**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	                    ...
	                    * 값순서 : 단위기관 ,고용구분, 부서, 직종, 사업 순으로  없으면 ""
	                    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
					//gwtAuthorization.formAuthConfig(plFrmPayr4550, "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd");
 ...
 **/
	...
	private void Payr4550Print(String fileName, String repType) {
	...
	private void RdaPrint(String fileName) {
 	 ...
 	 //권한 설정으로 인해 추가된 부분 
	    ...
	    if (payrMangDeptCd.equals(deptCdAuth)) { 
        	...
        	serarchParam += "["+ PayrUtils.getPrintStrToMultiData(lsDeptCd,MSFSharedUtils.allowNulls(srhDeptCd.getValue()),"deptNmRtchnt","deptCd")+"]"; //$5
        }  
	    //20151212-추가 끝
 	    serarchParam += "["+ PayrUtils.getPrintStrToMultiData(lsTypOccuCd,MSFSharedUtils.allowNulls(srhTypOccuCd.getValue()),"typOccuNm","typOccuCd")+"]";//$6
...
//        }
	...
	private void RexPrint(String fileName) {
	    ...
	    //20151212-추가 끝  	    
 	    ...
 	    serarchParam += ""+PayrUtils.getPrintStrToMultiData(lsTypOccuCd,MSFSharedUtils.allowNulls(srhTypOccuCd.getValue()),"typOccuNm","typOccuCd")+"⊥";	//$6
 	    serarchParam += ""+MSFSharedUtils.allowNulls(PayrUtils.getPrintStrToMultiData(lsDtilOccuInttnCd,MSFSharedUtils.allowNulls(srhDtilOccuInttnCd.getValue()),"dtilOccuClsNm","dtilOccuInttnCd"))+"⊥"; //$7
 ...
 **/
	...
	public ContentPanel getViewPanel() {
		if (panel == null) {
			...
			vp.setSpacing(10);
			createPayr4550Form(); 		// 화면 기본정보를 설정
 ...
 **/ 	
	...
	public Payr4550(String txtForm) {
 ...
 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
 **/	  
	private void createPayr4550Form() {

		plFrmPayr4550 = new FormPanel();
		plFrmPayr4550.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" -  "+ lblBasisConst.title_Payr4550()));
		plFrmPayr4550.setIcon(MSFMainApp.ICONS.text());
		plFrmPayr4550.setBodyStyleName("pad-text");
		plFrmPayr4550.setPadding(2);
		plFrmPayr4550.setFrame(true); 
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				 //로그 뷰화면 호출 메서드 
				  funcLogMessage(lblBasisConst.title_Payr4550(),"PAYR4550");
				}
			});
		plFrmPayr4550.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmPayr4550.getHeader().addTool(btnlogView); 
		/************************************************************************
		 ...
		 ************************************************************************/
		   ...
		   //멀티콤보박스 닫기 
		plFrmPayr4550.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
            @Override
		...
		Button btnPayr4550Sreach = new Button("조회");
		btnPayr4550Sreach.setIcon(MSFMainApp.ICONS.search16());
		buttonBar.add(btnPayr4550Sreach);
		btnPayr4550Sreach.addListener(Events.Select,new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 조회버튼 클릭시 처리
				reload();
			}
		...
		Button btnPayr4550Print = new Button("인쇄");
		btnPayr4550Print.setIcon(MSFMainApp.ICONS.print16());
		buttonBar.add(btnPayr4550Print);
		btnPayr4550Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// Print버튼 클릭시 처리
				// Payr4550Print("PAYRT4550");
				Payr4550Print("PAYRT4550", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
			}
		});
		
		Button btnPayr4550Exl = new Button("엑셀");
		btnPayr4550Exl.setIcon(MSFMainApp.ICONS.excel16());
		buttonBar.add(btnPayr4550Exl);
		btnPayr4550Exl.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//엑셀저장  
    			excelFileExport();
			}
	  	...
	  	plFrmPayr4550.add(buttonBar);    
	  	 
	
		vp.add(plFrmPayr4550);
		
		//엣지변환
		//plFrmPayr4550.setSize("990px", "680px");
		plFrmPayr4550.setSize("990px", "730px");
	  }
 /**
 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
 ...
 **/ 
	  private void createSearchForm() {   
		...
		plFrmPayr4550.add(payYrGrid);
		  
		plFrmPayr4550.setLayout(new FlowLayout());
				...
				if (type == Store.Add) {
	                		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			                     ...
			                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
			                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
					checkPayr4550Auth("srhPayYr", lsPayYrStore);
				}
			}
		});
                	 ...
                	 EventType type = be.getType();
			    	   if (type == Store.Add) { 
	                		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			                     ...
			                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
			                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
			                	checkPayr4550Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
	                	 }
                }
            });  
                	 ...
                	 EventType type = be.getType();
			    	   if (type == Store.Add) { 
	                		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			                     ...
			                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
			                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
			                	checkPayr4550Auth("srhEmymtDivCd", lsEmymtDivCd); 
	                	 }    
		...
		srhDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
						++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
					checkPayr4550Auth("srhDeptCd", lsDeptCd);
				}
			}
		}); 
			...
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					...
					if (!MSFConfiguration.EMYMT_DIVCD02.equals(MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd, "commCd"))) {
						 ...
						 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
						 */
						// checkPayr4550Auth("srhTypOccuCd", lsTypOccuCd);
					}
				}
			});
 ...
 **/   
   ...
   **/ 	 
	private void createCheckBoxGrid() {
		...
		FieldSet fieldSet = new FieldSet();
		fieldSet.setHeadingHtml("원청징수부출력대상자정보");
   ...
   **/ 	  
		public void reload() {
			...
			payr0301GridPanel.getTableDef().addColumnFilter("payYr", MSFSharedUtils.getSelectedComboValue(srhPayYr,"payYr"), SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	private void excelFileExport() {
		...
		param.put("deptCd", strDeptCd); 
         ...
         payr0301GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsPayr4550Export.do","extgwtFrame" ,param);                      
