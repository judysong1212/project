...
import com.app.exterms.payroll.client.service.Payr1400ServiceAsync;
...
=====================================================**/
public class Payr1400  extends MSFPanel { 
 ...
 **/
	 ...
	 // private FormData formData; 
	  ...
	  private Payr0470Def payr0470Def  = new Payr0470Def();   //그리드 테이블 컬럼 define  
 ...
 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
 **/
	      private Payr1400ServiceAsync payr1400Service = Payr1400Service.Util.getInstance();  
		   //공통 코드  
...
//	        if (isValid()) {
	            ...
	            if (payr0470GridPanel.getMsfGrid().getGrid().getStore().getModifiedRecords().size() <= 0) {
	            ...
	            MessageBox.confirm("저장", "데이터를 저장하시겠습니까?",new Listener<MessageBoxEvent>(){
	                ...
	                public void handleEvent(MessageBoxEvent be) {
	                	...
	                	if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
	                        ...
	                        Iterator<Record> iterRecords = payr0470GridPanel.getMsfGrid().getGrid().getStore().getModifiedRecords().iterator();
	                           
                            List<Payr0470DTO> listPayr0470dto = new ArrayList<Payr0470DTO>();  
                            ...
                            while (iterRecords.hasNext()) {
                                ...
                                payr0470Dto.setDpobCd(MSFSharedUtils.allowNulls(bmMapModel.get("dpobCd")));    /** column 사업장코드 : dpobCd */
                                payr0470Dto.setPayDducCd(MSFSharedUtils.allowNulls(bmMapModel.get("payDducCd")));    /** column 급여공제코드 : payDducCd */
                                payr0470Dto.setPymtDducDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("pymtDducDivCd$commCd")));    /** column 지급공제구분코드 : pymtDducDivCd */
                                payr0470Dto.setPayDducGrpCd(MSFSharedUtils.allowNulls(bmMapModel.get("payDducGrpCd$commCd")));    /** column 급여공제그룹코드 : payDducGrpCd */
                                payr0470Dto.setPayDducNm(MSFSharedUtils.allowNulls(bmMapModel.get("payDducNm")));    /** column 급여공제명 : payDducNm */
                                payr0470Dto.setPayrTermDducYn((Boolean)bmMapModel.get("payrTermDducYn"));    /** column 기간제공제사용여부 : payrTermDducYn */
                                payr0470Dto.setPayDducUseYn((Boolean)bmMapModel.get("payDducUseYn"));    /** column 급여공제사용여부 : payDducUseYn */
                                payr0470Dto.setPayDducSum((Long)bmMapModel.get("payDducSum"));    /** column 급여공제금액 : payDducSum */
                                payr0470Dto.setPayDducRto((Double)bmMapModel.get("payDducRto"));    /** column 급여공제비율 : payDducRto */
                                payr0470Dto.setYrtxDducDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("yrtxDducDivCd$commCd")));    /** column 연말정산_공제구분코드 : yrtxDducDivCd */
                                payr0470Dto.setGrpApptnDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("grpApptnDivCd$commCd")));    /** column 그룹적용구분코드 : grpApptnDivCd */
                                payr0470Dto.setPayDducBgnnDt(GWTUtils.getStringFromDate((Date)bmMapModel.get("payDducBgnnDt"),"yyyyMMdd"));    /** column 급여공제시작일자 : payDducBgnnDt */
                                payr0470Dto.setPayDducEndDt(GWTUtils.getStringFromDate((Date)bmMapModel.get("payDducEndDt"),"yyyyMMdd"));    /** column 급여공제종료일자 : payDducEndDt */
...
//                                payr0470Dto.setRevnAddr(MSFSharedUtils.allowNulls(bmMapModel.get("revnAddr")));    /** column 수정주소 : revnAddr */
                                ...
                                listPayr0470dto.add(payr0470Dto);
                              
                            }   
                            
	                         payr1400Service.activityOnPayr1400Save(listPayr0470dto,  
...
//	        if (isValid()) { 
	            ...
	            if(payr0470GridPanel.getGrid().getSelectionModel().getSelectedItems().size() > 0){
	                    ...
	                    public void handleEvent(MessageBoxEvent be) {
	                     //   if("Yes".equals(be.getButtonClicked().getText())){
	                    	if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
	                            ...
	                            payr1400Service.activityOnPayr1400Delete(payr0470GridPanel.getGrid().getSelectionModel().getSelectedItems() , new AsyncCallback<Long>() {
 ...
 **/ 	
	  ...
	  public Payr1400(String txtForm) {
 ...
 **/	  
	  private void createPayr1400Form() {
			...
			plFrmPayr1400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" -  ") + lblPayrConst.title_Payr1400());
			...
			btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 //로그 뷰화면 호출 메서드 
					  funcLogMessage(lblPayrConst.title_Payr1400(),"PAYR1400");
					}
				});
...
//		  	topPayr1400Bar.add(topBtn);  
			...
			btnPayr1400Excel.addSelectionListener(new SelectionListener<ButtonEvent>() {
		  		public void componentSelected(ButtonEvent ce) {
		  		  payr0470GridPanel.callExportServlet(""); 
		  		}
		  	});
 ...
 **/  
	  ...
	  private void createListGrid() { 
		  ...
		  //  frmlytStd.setLabelWidth(85); 
		   // frmlytStd.setLabelAlign(LabelAlign.RIGHT);   
		    ...
		    payr0470GridPanel = new MSFGridPanel(payr0470Def, false, false, false, false,false);
   ...
   **/ 	  
	public void reload() {
			...
			payr0470GridPanel.getTableDef().addColumnFilter("payDducGrpCd", MSFSharedUtils.getSelectedComboValue(srhPayDducGrpCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
			payr0470GridPanel.getTableDef().addColumnFilter("payDducNm", MSFSharedUtils.defaultNulls(srhPayDducNm.getRawValue(),""), SimpleColumnFilter.OPERATOR_LIKE);
   ...
   public void formNewRows() {
       ...
       //TODO 그리드를 초기화 한후  ROW을 인서트 한다. 
        ...
        BaseModel bmData = payr0470GridPanel.getTableDef().getDefaultValues(); 
...
//payr0470Dto.setRevnAddr(MSFSharedUtils.allowNulls(record.get("revnAddr")));    /** column 수정주소 : revnAddr */
        ...
        payr0470GridPanel.getMsfGrid().getGrid().stopEditing();      
        payr0470GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
        payr0470GridPanel.getMsfGrid().getGrid().startEditing(payr0470GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
        payr0470GridPanel.setSize(935, 490);
