...
import com.app.exterms.payroll.client.form.defs.Payr0100Def;
import com.app.exterms.payroll.client.languages.PayrConstants;
import com.app.exterms.payroll.client.lookup.LookupPayr0100ApptnStdBgnnDt;
import com.app.exterms.payroll.client.lookup.LookupPayr0100ApptnYr;
import com.app.exterms.payroll.client.service.Payr1100Service;
import com.app.exterms.payroll.client.service.Payr1100ServiceAsync;
...
=====================================================**/
public class Payr1100  extends MSFPanel { 
 ...
 **/
	  ...
	  private FormPanel plFrmPayr1100;
	//  private FormData formData; 
	  private String txtForm = "";  
	  
	  private Payr0100Def payr0100Def  = new Payr0100Def();   //그리드 테이블 컬럼 define  
      // private MSFCustomForm msfCustomForm; 
      private MSFGridPanel payr0100GridPanel;
      
      private LookupPayr0100ApptnYr lkApptnYr = new LookupPayr0100ApptnYr();  
      private MSFGridPanel apptnYrGridPanel;
      
      //일자처리콤보 
      private LookupPayr0100ApptnStdBgnnDt lkApptnStdBgnnDt = new LookupPayr0100ApptnStdBgnnDt();  
       ...
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       */
      private ButtonBar topPayr1100Bar;
      private Button btnPayr1100New;
      private Button btnPayr1100Save;
      private Button btnPayr1100Del;
      private Button btnPayr1100Sreach;
      /**
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       * 검색변수 선언 
       * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      ...
      private FileUploadField payr0100flUp;
 ...
 **/
    //입력수정삭제처리 rpc 처리부     
  	private Payr1100ServiceAsync payr1100Service = Payr1100Service.Util.getInstance();	    
...
//	            if (ce.getButton() == btnPayr1100Save) {
//	                doAction(ActionDatabase.INSERT);
//	            } else if (ce.getButton() == btnPayr1100Save) {
//	                doAction(ActionDatabase.UPDATE);
//	            } else if (ce.getButton() == btnPayr1100Del) {
		...
		MessageBox.confirm("알림", "해당 간이세액표를 " + msg + "하시겠습니까?",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					Payr0100DTO payr0100Dto = new Payr0100DTO();
					
					payr0100Dto.setApptnYr(MSFSharedUtils.getSelectedComboValue(srhApptnYr,"apptnYr"));	//적용년도
					payr0100Dto.setApptnStdBgnnDt(MSFSharedUtils.getSelectedComboValue(srhApptnStdBgnnDt,"apptnStdBgnnDt"));//기준일자
					
			    	payr1100Service.activityOnPayr1100(payr0100Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
			                          MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnPayr1100(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//	    vp.setSpacing(10);
//	    createPayr0100Form();  //화면 기본정보를 설정
 ...
 **/ 	
	  ...
	  public Payr1100(String txtForm) {
 ...
 **/	  
	  ...
	  private void createPayr0100Form() {
		  
		plFrmPayr1100 = new FormPanel();
		plFrmPayr1100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" -  ") + lblPayrConst.title_Payr1100());
		//plFrmPayr1100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 간이소득세액표관리");
		plFrmPayr1100.setIcon(MSFMainApp.ICONS.text());
		plFrmPayr1100.setBodyStyleName("pad-text");
		plFrmPayr1100.setPadding(2);
		plFrmPayr1100.setFrame(true); 
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				 //로그 뷰화면 호출 메서드 
				  funcLogMessage(lblPayrConst.title_Payr1100(),"PAYR1100");
				}
			});
		plFrmPayr1100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmPayr1100.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		
		 topPayr1100Bar = new ButtonBar();    
		 topPayr1100Bar.setAlignment(HorizontalAlignment.RIGHT);
...
//	      }));    
	  	...
	  	btnPayr1100Sreach = new Button("조회"); 
	  	btnPayr1100Sreach.setIcon(MSFMainApp.ICONS.search16());
	  	topPayr1100Bar.add(btnPayr1100Sreach);
	  	btnPayr1100Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
    		public void handleEvent(ButtonEvent e) {
    			//조회버튼 클릭시 처리 
    		    reload();  
    		}
	  	...
	  	btnPayr1100Del = new Button("삭제"); 
	  	btnPayr1100Del.setIcon(MSFMainApp.ICONS.delete16());
	  	topPayr1100Bar.add(btnPayr1100Del);
	  	btnPayr1100Del.addListener(Events.Select, new Listener<ButtonEvent>() {
    		public void handleEvent(ButtonEvent e) {
...
//	  	buttonBar.add(topBtn);  
	  	...
	  	plFrmPayr1100.add(topPayr1100Bar);    	
	  	
	  //파일업로드 처리
	  	plFrmPayr1100.setAction("myurl");
	  	plFrmPayr1100.setEncoding(Encoding.MULTIPART);
  		plFrmPayr1100.setMethod(Method.POST);
	  	
		vp.add(plFrmPayr1100);
		plFrmPayr1100.setSize("990px", "690px");
	  }
 ...
 */
		...
		plFrmPayr1100.setLayout(new FlowLayout());
...
//           }); 
			...
			payr0100flUp = new FileUploadField();
			payr0100flUp.setFieldLabel("간이조건세액표");
			payr0100flUp.setName("payr0100flUp");
			payr0100flUp.setValidator(new Validator() {  
	       		@Override  
			...
			layoutContainer_2.add(payr0100flUp, new FormData("100%"));
			...
			Button btnPayr0100Save = new Button("저장");
			layoutContainer_4.add(btnPayr0100Save, new FormData("100%"));
			btnPayr0100Save.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {	
					// 엑셀 업로드
					xlsImportData();  
				}
			...
			Button btnPayr0100down = new Button("양식다운로드");
			btnPayr0100down.addListener(Events.Select, new Listener<ButtonEvent>() {
	  	    	public void handleEvent(ButtonEvent e) {
//	  	    		payr0100GridPanel.callExportServlet("");
	  	    		xlsExportData();
	  	    	}
	  	    });
			layoutContainer_3.add(btnPayr0100down, new FormData("100%"));
			...
			layoutContainer.setBorders(false);
		  	plFrmPayr1100.add(fldstNewFieldset);
   ...
   **/ 	 
	  ...
	  //  frmlytStd.setLabelWidth(85); 
	   // frmlytStd.setLabelAlign(LabelAlign.RIGHT);   
	    ...
	    payr0100GridPanel = new MSFGridPanel(payr0100Def, false, false, false, false, false);
	    payr0100GridPanel.setHeaderVisible(false);  
	    payr0100GridPanel.setBodyBorder(true);
	    payr0100GridPanel.setBorders(true);
		...
		MessageBox.confirm("간이소득세액이관", "간이소득세액표를 이관하시겠습니까?<br>(등록되어있는 세액표가 있을시 삭제 처리 됩니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					payr0100Dto.setApptnYr(MSFSharedUtils.getSelectedComboValue(creApptnYr,"apptnYr"));	//적용년도
					payr0100Dto.setApptnStdBgnnDt(MSFSharedUtils.getSelectedComboValue(crdApptnStdBgnnDt, "apptnStdBgnnDt"));//적용일자
					payr0100Dto.setCreTransferYr(MSFSharedUtils.getSelectedComboValue(creTransferYr, "year"));//이관년도
					payr0100Dto.setCreTransferStdBgnnDt(GWTUtils.getStringFromDate(creTransferStdBgnnDt.getValue(), "yyyyMMdd"));//이관일자
					
					
					payr1100Service.payr0100Transfer(payr0100Dto,  new AsyncCallback<Long>() {
...
//					param.put("apptnStdBgnnDt", PayrUtils.getConvertDateToString(createApptnStdBgnnDt, "yyyyMMdd"));
					...
					if("".equals(createApptnYr)){
						...
						String filename = MSFSharedUtils.allowNulls(payr0100flUp.getValue());
				       	if (filename.length() == 0) {
				       		Window.alert("선택된 파일이 없습니다.");
				       	} else {
				       		//submit the form
				       		plFrmPayr1100.mask("엑셀 데이타 업데이트 중입니다. 업로드 중..."); 
				                
				       		//파일업로드 처리
				            plFrmPayr1100.setAction(GWT.getHostPageBaseURL() + "imp/XlsReadPayr0100Import.do");
							plFrmPayr1100.setEncoding(Encoding.MULTIPART);
							plFrmPayr1100.setMethod(Method.POST);
							plFrmPayr1100.submit();
							plFrmPayr1100.onFrameLoad();
							picBoolFile = true;
				       	}               
				       	// reset and unmask the form 
				        // after file upload 
				        plFrmPayr1100.addListener(Events.Submit, new Listener<FormEvent>() {
				        	public void handleEvent(FormEvent evt) {   
				        		if ("success".endsWith(evt.getResultHtml()) && picBoolFile) {
				        			
				        			//업로드가  성공했으면 인서트 모듈을 태운다. 
				                  	plFrmPayr1100.unmask();  
...
//				                   	apptnStdBgnnDtGridPanel.reload();
	...
	private void xlsExportData() {
		...
		if("".equals(createApptnYr)){
			...
			payr0100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsPayr0100Export.do","extgwtFrame", param);   
		}
	}	
	...
	public void reload() {
			...
			IColumnFilter filters = null;
			payr0100GridPanel.getTableDef().setTableColumnFilters(filters);
			
			payr0100GridPanel.getTableDef().addColumnFilter("apptnYr", MSFSharedUtils.allowNulls(srhApptnYr.getRawValue()), SimpleColumnFilter.OPERATOR_LIKE); 
			payr0100GridPanel.getTableDef().addColumnFilter("apptnStdBgnnDt", MSFSharedUtils.getSelectedComboValue(srhApptnStdBgnnDt,"apptnStdBgnnDt"), SimpleColumnFilter.OPERATOR_LIKE); 
		  
			payr0100GridPanel.reload();
		}
