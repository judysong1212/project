...
=====================================================**/
public class PayrP150002  extends MSFFormPanel { 

	private ContentPanel payrP150002;
	...
	private Payr0410Def pPayr150002LDef  = new Payr0410Def("PAYRP150002");   //그리드 테이블 컬럼 define
	private MSFGridPanel pPayr150002LGridPanel;
	

	private Payr0410Def pPayr150002RDef  = new Payr0410Def("PAYRP150002TARGET");   //그리드 테이블 컬럼 define
	private MSFGridPanel pPayr150002RGridPanel;
	...
	private void setpayrP150002FormBinding() {
	}        
...
/*	private void checkPayr1500Auth( String authAction, ListStore<BaseModel> bm) {   
	...
	public PayrP150002(ActionDatabase actionDatabase, MSFPanel caller) {
		//super();
		...
		payrP150002 = new ContentPanel();
		createPayrP150002Form();
		createSearchForm();
		createStandardForm();

		payrP150002.setBodyBorder(false);
		payrP150002.setBorders(false);
		payrP150002.setHeaderVisible(false);

		this.add(payrP150002);
		formBinding = new FormBinding(this, true);
		//엣지변환
		//this.setSize("996px", "640px");  
		this.setSize("996px", "670px");  

		formBinding = new FormBinding(this, true);

	}
	private  void createPayrP150002Form() { 
		//엣지변환
		//payrP150002.setSize("976px", "640px");
		payrP150002.setSize("976px", "670px");
	}
    ...
    private void createSearchForm() {   
    	...
    	btnHanNm.addListener(Events.Select, new Listener<ButtonEvent>() {
    		public void handleEvent(ButtonEvent e) {
    			pPayr150002LGridPanel.getMsfGrid().clearData();
    			fnPopupCommP600();
    		}
    	});
...
/*		LayoutContainer layoutContainer = new LayoutContainer();
		...
		payrP150002.add(layoutContainer);
        ...
        fldstNewFieldset1.setSize(976, 170);
        ...
        payrP150002.add(layoutContainer1);
        ...
        layoutContainer1.setBorders(false);
		 ...
		 payrP150002.add(layoutContainer2);
...
//			fldstNewFieldset3.add(layoutContainer2_2);
		 ...
		 payrP150002.add(layoutContainer3);
	   ...
	   private  LayoutContainer payrLeft01() {
		  ...
		  //  frmlytStd.setLabelWidth(85); 
		   // frmlytStd.setLabelAlign(LabelAlign.RIGHT);   
		    ...
		    pPayr150002LGridPanel = new MSFGridPanel(pPayr150002LDef, false, false, false, false, false);
		    pPayr150002LGridPanel.setHeaderVisible(false);  
		    pPayr150002LGridPanel.setBodyBorder(true);
		    pPayr150002LGridPanel.setBorders(true);
		// 	 payrL0540GridPanel.setGridType(MSFGridType.GROUPBY);
		  	//payrL0540GridPanel.setSize(598, 184);   
			    final Grid payr0540Grid = pPayr150002LGridPanel.getMsfGrid().getGrid(); 
			   payr0540Grid.addListener(Events.Select,  new Listener<BaseEvent>() {
		             public void handleEvent(BaseEvent be) {  
		           	  if (pPayr150002LGridPanel.getCurrentlySelectedItem() != null) {  
			  ...
			  // ================전체선택버튼=========================
			     ...
			     btnMove.addSelectionListener(new SelectionListener<ButtonEvent>() {
			        ...
			        //	 BaseModel bm = new BaseModel();
			        	// pPayr150002LGridPanel.getGrid().getStore().remove(0);
			        	 
			   
			       // 	 pPayr150002LGridPanel.getMsfGrid().getGrid().getStore().commitChanges();
			        //	 bm =  pPayr150002LGridPanel.getGrid().getSelectionModel().getSelectedItem();
			        	 
			        	 
			        	 if(pPayr150002LGridPanel.getGrid().getSelectionModel().getSelectedItems().size() > 0){

			        		// pPayr150002LGridPanel.getGrid().getSelectionModel().getSelectedItem().get
			        		 
			        		 
				        	 List<BaseModel> list = pPayr150002LGridPanel.getGrid().getSelectionModel().getSelectedItems();
				        	 for(BaseModel bm : list){
									//bm.get("typOccuGrdeMppgSeilNum");
									pPayr150002LGridPanel.getGrid().getStore().remove(bm);
								}
				        	// pPayr150002LGridPanel.getMsfGrid().getStore().rejectChanges();
				        	 pPayr150002RGridPanel.getGrid().getStore().add(list);
			        		// pPayr150002LGridPanel.getGrid().getStore().remove( pPayr150002LGridPanel.getGrid().getSelectionModel().getSelectedItem());
			     ...
			     //====================================================admi
			     pPayr150002LGridPanel.setBottomComponent(bottomToolbar2);
			   
			   cpGridPayrR0540.add(pPayr150002LGridPanel);     
		 ...
		 //   lcStdGrid.setStyleAttribute("paddingRight", "10px");
		  ...
		  //  frmlytStd.setLabelWidth(85); 
		   // frmlytStd.setLabelAlign(LabelAlign.RIGHT);   
		    ...
		    pPayr150002RGridPanel = new MSFGridPanel(pPayr150002RDef, false, false, false, false, false);
		    pPayr150002RGridPanel.setHeaderVisible(false);  
		    pPayr150002RGridPanel.setBodyBorder(true);
		    pPayr150002RGridPanel.setBorders(true);
		// 	 payrL0540GridPanel.setGridType(MSFGridType.GROUPBY);
		  	//payrL0540GridPanel.setSize(598, 184);   
			    final Grid payr150002Target = pPayr150002RGridPanel.getMsfGrid().getGrid(); 
			    payr150002Target.addListener(Events.Select,  new Listener<BaseEvent>() {
		             public void handleEvent(BaseEvent be) {  
		           	  if (pPayr150002RGridPanel.getCurrentlySelectedItem() != null) {  
			  ...
			  // ================전체선택버튼=========================
			     ...
			     btnDelect.addSelectionListener(new SelectionListener<ButtonEvent>() {
			         public void componentSelected(ButtonEvent ce) {
			        	 ...
			        	 if(pPayr150002RGridPanel.getGrid().getSelectionModel().getSelectedItems().size() > 0){
			        		 
				        	 List<BaseModel> list = pPayr150002RGridPanel.getGrid().getSelectionModel().getSelectedItems();
				        	 for(BaseModel bm : list){
									//bm.get("typOccuGrdeMppgSeilNum");
				        		 pPayr150002RGridPanel.getGrid().getStore().remove(bm);
								}
				        	// pPayr150002LGridPanel.getMsfGrid().getStore().rejectChanges();
				        	 pPayr150002LGridPanel.getGrid().getStore().add(list);
			        		// pPayr150002LGridPanel.getGrid().getStore().remove( pPayr150002LGridPanel.getGrid().getSelectionModel().getSelectedItem());
			     ...
			     bottomToolbar2.add(new SeparatorMenuItem());
			     //====================================================admi
			     pPayr150002RGridPanel.setBottomComponent(bottomToolbar2);
		    
			   cpGrid.add(pPayr150002RGridPanel);  
...
/*	         cp01.addButton(new Button("저장", new SelectionListener<ButtonEvent>() {   
	...
	public void reload() {
		...
		pPayr150002LGridPanel.getMsfGrid().clearData();
		pPayr150002LGridPanel.getTableDef().setTableColumnFilters(filters);			
		pPayr150002LGridPanel.getTableDef().addColumnFilter("payYr", srhYear.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
		pPayr150002LGridPanel.getTableDef().addColumnFilter("payrImcd", srhPayrImcd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);		
		pPayr150002LGridPanel.getTableDef().addColumnFilter("itemNm", srhItemNm.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
		pPayr150002LGridPanel.getTableDef().addColumnFilter("payCd", MSFSharedUtils.getSelectedComboValue(srhPayCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		pPayr150002LGridPanel.reload();
	}
	...
	public void reload2() {
		...
		IColumnFilter filters = null;
		pPayr150002RGridPanel.getMsfGrid().clearData();
		pPayr150002RGridPanel.getTableDef().setTableColumnFilters(filters);	
		pPayr150002RGridPanel.reload();
	}
   ...
   public void detailFormSave() {
	   ...
	   if(pPayr150002RGridPanel.getMsfGrid().getGrid().getStore().getModels().size() > 0){
		   ...
		   payr0410Dto.setPayYr(srhYear.getValue());
		   ...
		   payr1500Service.activityOnPayr150002ToPayr0410Insert( pPayr150002RGridPanel.getMsfGrid().getGrid().getStore().getModels(), payr0410Dto, new AsyncCallback<Long>() {
			   ...
			   public void onSuccess(Long result) { 
				   if (result == 0) {
					   ...
					   List<BaseModel> list = pPayr150002RGridPanel.getMsfGrid().getGrid().getStore().getModels();
					   for(BaseModel bm : list){
						   pPayr150002RGridPanel.getGrid().getStore().remove(bm);
					   }
				   } 
