...
=====================================================**/
public class Payr4400  extends MSFPanel { 
 ...
 **/
	...
	private FormPanel plFrmPayr4400;
	//private FormData formData; 
	private String txtForm = "";
	private XTemplate detailTp; 
	 
	private Payr0301Def payr0301Def  = new Payr0301Def("PAYR4400");   //그리드 테이블 컬럼 define  
...
//	private MSFGridPanel typOccuCdGridPanel;
	...
	/**
	  ...
	  * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  	  */
	private ButtonBar topPayr4400Bar;
	private Button btnPayr4400Init;
     // private Button btnPayr4400Save;
    //  private Button btnPayr4400Del;
	private Button btnPayr4400Sreach;
	private Button btnPayr4400Print;
	private Button btnPayr4400Excel;
	/**
	 * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     * 검색변수 선언 
     * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 **/	  
	...
	// private AuthAction authAction;  
				...
				//  private static MessageBox authBox = MessageBox.wait("화면로딩", "설정 초기화 작업 진행 중입니다....", "설정 작업 진행 중..."); 
				   ...
				   * AuthAction 
				   */
				  private void checkPayr4400Auth( String authAction, ListStore<BaseModel> bm) {    
                  ...
                  //  GWTAuthorization.formAuthPopConfig(plFrmPayr4400, "srhPayrMangDeptCd","srhEmymtDivCd",strArrDeptCd,"srhTypOccuCd","srhDtilOccuInttnCd","srhBusinCd");
 ...
 **/
   // 검색 폼 바인딩 처리 함수 
	...
	//화면 폼 바인딩 처리 
	private void setPayr4400FormBinding() {
 ...
 **/
	...
	//입력수정삭제처리 rpc 처리부     
	//private Payr4400ServiceAsync payr4400Service = Payr4400Service.Util.getInstance();	 
     ...
     * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     */  
//    private void readPayr4400(){
//    	payr0301GridPanel.mask(MSFMainApp.MSFMESSAGES.LoadingData());
//    	
//    	//검색조건 넘김 
//    	bass0200Dto.setPubcHodyCtnt(srhPubcHodyCtnt.getValue());
//        //LOOP QUESTIONARI
//    	bass0200Service.selectPayr4400List(bass0200Dto, new AsyncCallback<List<Payr4400BM>>(){
//            public void onFailure(Throwable caught) {
//                MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
//                        MSFMainApp.ADMINMESSAGES.ExceptionMessageService("selectPayr4400(): "+caught), null);
//                msfPayr4400GridPanel.unmask(); 
//            }
//
//            public void onSuccess(List<Payr4400BM> result) {
//            	composedPayr4400List(result);
//            }
//        });
//    }
    
    //조회처리부
//    private void composedPayr4400List(List<Payr4400BM> listaMenu) {
...
//	            if (ce.getButton() == btnPayr4400Save) {
//	                doAction(ActionDatabase.INSERT);
//	            } else if (ce.getButton() == btnPayr4400Save) {
//	                doAction(ActionDatabase.UPDATE);
//	            } else if (ce.getButton() == btnPayr4400Del) {
 ...
 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
 **/
	private void Payr4400Print(String fileName, String repType) {
	...
	private void RdaPrint(String fileName) {
		...
		serarchParam += "["+ PayrUtils.getPrintStrToMultiData(lsDtilOccuInttnCd,MSFSharedUtils.allowNulls(srhDtilOccuInttnCd.getValue()),"dtilOccuClsNm","dtilOccuInttnCd") +"]";  //$8
		...
		//20151212-변경추가 시작 
	    serarchParam += "[" + MSFMainApp.get().getUser().getUsrId() + "]";  //$11
		serarchParam += "["+MSFMainApp.get().getUser().getPayrMangDeptYn()+"]"; //$12
		  
		//권한처리를위해 넘기는 변수 2개추가 
		serarchParam += "["+deptCdAuth+"]"; //$13
		serarchParam += "["+dtilOccuInttnCdAuth+"]"; //$14
		//20151212-추가 끝 
	...
	private void RexPrint(String fileName) {
		...
		serarchParam += ""+ PayrUtils.getPrintStrToMultiData(lsDtilOccuInttnCd,MSFSharedUtils.allowNulls(srhDtilOccuInttnCd.getValue()),"dtilOccuClsNm","dtilOccuInttnCd") +"⊥";  //parameter8
		serarchParam += ""+MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd")+"⊥";          //parameter9
		serarchParam += ""+MSFSharedUtils.allowNulls(srhSystemkey.getValue())+"⊥";      //$parameter10
...
//		} else {
			...
			//직종 권한에 대한 처리 추가 기본 처리  $9로해서 처리 할것...$12
...
//	private void Payr4400Print(String fileName) {
// 
//		// 출력물 디렉토리 패스경로 인사 : PSNL
//		String strDirPath = "PAYR";
//		// mrd 출력물
	  ...
	  public ContentPanel getViewPanel(){
			if(panel == null){
			   ...
			   // formData = new FormData("-650");
			    ...
			    vp.setSpacing(10);
			    createPayr4400Form();  //화면 기본정보를 설정
 ...
 **/ 	
	  ...
	  public Payr4400(String txtForm) {
 ...
 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
 **/
	  private void createPayr4400Form() {
		  
		plFrmPayr4400 = new FormPanel();
		plFrmPayr4400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" -  " + lblPayrConst.title_Payr4400()));
		//plFrmPayr4400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 급여지로이체명세서");
		plFrmPayr4400.setIcon(MSFMainApp.ICONS.text());
		plFrmPayr4400.setBodyStyleName("pad-text");
		plFrmPayr4400.setPadding(2);
		plFrmPayr4400.setFrame(true);  
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				 //로그 뷰화면 호출 메서드 
				  funcLogMessage(lblPayrConst.title_Payr4400(),"PAYR4400");
				}
			});
		plFrmPayr4400.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmPayr4400.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

	    //멀티콤보박스 닫기 
		plFrmPayr4400.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
            @Override
...
//                    }
		  ...
		  topPayr4400Bar = new ButtonBar();    
		  topPayr4400Bar.setAlignment(HorizontalAlignment.RIGHT);  
	  
		  btnPayr4400Init = new Button("초기화");  
		  btnPayr4400Init.setIcon(MSFMainApp.ICONS.new16());
		  topPayr4400Bar.add(btnPayr4400Init);
		  btnPayr4400Init.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			 //초기화 
	    			formInit();
	    		}
		   ...
		   btnPayr4400Sreach = new Button("조회"); 
		    btnPayr4400Sreach.setIcon(MSFMainApp.ICONS.search16());
		    topPayr4400Bar.add(btnPayr4400Sreach);
		  	btnPayr4400Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
		  		public void handleEvent(ButtonEvent e) {
	    			//조회버튼 클릭시 처리 
	    		    reload();  
	    		}
		  	...
		  	btnPayr4400Print = new Button("인쇄"); 
		  	btnPayr4400Print.setIcon(MSFMainApp.ICONS.print16()); 
		  	topPayr4400Bar.add(btnPayr4400Print); 
		  	btnPayr4400Print.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//Print버튼 클릭시 처리 
//	    			Payr4400Print("PAYRT4400") ;
	    			Payr4400Print("PAYRT4400", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
	    		}
	    	});
		  	
		    btnPayr4400Excel = new Button("급여이체내역"); 
		    btnPayr4400Excel.setIcon(MSFMainApp.ICONS.excel16()); 
		  	topPayr4400Bar.add(btnPayr4400Excel); 
		  	btnPayr4400Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			exportExcel(); 
	    		}
	    	});
		  	
		    btnPayr4400Excel = new Button("연계파일생성"); 
		    btnPayr4400Excel.setIcon(MSFMainApp.ICONS.excel16()); 
		  	topPayr4400Bar.add(btnPayr4400Excel); 
		  	btnPayr4400Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	  	...
	  	plFrmPayr4400.add(topPayr4400Bar);    
	  	
		//파일업로드 처리
		plFrmPayr4400.setAction("bizform");
		plFrmPayr4400.setEncoding(Encoding.MULTIPART);
		plFrmPayr4400.setMethod(Method.POST);
		
	
		vp.add(plFrmPayr4400);
		//엣지변환
		//plFrmPayr4400.setSize("990px", "680px");
		plFrmPayr4400.setSize("990px", "730px");
	  }
 ...
 **/ 
 ...
 private void createSearchForm() {   
   ...
   */
  		...
  		plFrmPayr4400.add(payYrGrid);  
	  
	   plFrmPayr4400.setLayout(new FlowLayout());
		    	   ...
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkPayr4400Auth("srhPayYr", lsPayYrStore); 
              	 }
       ...
       // srhPayMonth.setEmptyText("--월선택--");
            ...
            public void handleEvent(StoreEvent<BaseModel> be) {  
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
		              ...
		              checkPayr4400Auth("srhPayMonth", lsPayMonth); 
              	 }
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkPayr4400Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
              	 }
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkPayr4400Auth("srhEmymtDivCd", lsEmymtDivCd); 
              	 }
	    ...
	    layoutContainer_12.add(srhEmymtDivCd, new FormData("100%"));
	    layoutContainer_14.setBorders(false); 
	    layoutContainer_14.add(layoutContainer_12, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.7));
...
//	    lcSchCol.add(lcSchLeft,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.18));  		// 지급년도
//	    lcSchCol.add(layoutContainer,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.22));	// 단위기관
//	    lcSchCol.add(layoutContainer_14,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.38));	// 고용구분
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	checkPayr4400Auth("srhDeptCd", lsDeptCd); 
              	 }
            	 ...
            	 EventType type = be.getType();
		    	   if (type == Store.Add) { 
              		 ...
              		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		                     ...
		                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
		                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
		                	//checkPayr4400Auth("srhTypOccuCd", lsTypOccuCd); 
              	 }
		   ...
		   LayoutContainer layoutContainer_1 = new LayoutContainer();
		    ...
		    srhPayCd.getStore().addStoreListener( new StoreListener<BaseModel>() {   
	            public void handleEvent(StoreEvent<BaseModel> be) {  
	            	...
	            	checkPayr4400Auth("srhPayCd",lsPayCd);   
	            }
	        });  
 ...
 private void createCheckBoxGrid() {   
	  ...
	  //  frmlytStd.setLabelWidth(85); 
	   // frmlytStd.setLabelAlign(LabelAlign.RIGHT);   
	    ...
	    //payr0301GridPanel.setSize(935, 455);  
   ...
   **/ 	  
	...
	public void reload() {
			...
			payr0301GridPanel.getTableDef().addColumnFilter("pymtYrMnth", (MSFSharedUtils.getSelectedComboValue(srhPayYr,"payYr")+ MSFSharedUtils.getSelectedComboValue(srhPayMonth,"month")), SimpleColumnFilter.OPERATOR_EQUALS); 
			...
			//payr0301GridPanel.getTableDef().addColumnFilter("deptCd", MSFSharedUtils.allowNulls(srhDeptCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	public void exportExcel() {
       ...
       // param.put("pyspGrdeCd", PayrUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","pyspGrdeCd")); 
        ...
        //넘어온메서드 값을 가지고 처리 한다.  
        GWTUtils.xlsExportData(GWT.getHostPageBaseURL() + "exp/XlsPayr4400Export.do","extgwtFrame", JsUtil.toJavaScriptObject(param)); 
	}
	
	//엑셀저장함수 
	public void exportHoJoExcel() {
		// TODO Auto-generated method stub
		// payr0301GridPanel.callExportServlet("");
        HashMap<String, String> param = new HashMap<String, String>(); 
       ...
       // param.put("pyspGrdeCd", PayrUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","pyspGrdeCd")); 
        ...
        //넘어온메서드 값을 가지고 처리 한다.  
        GWTUtils.xlsExportData(GWT.getHostPageBaseURL() + "exp/XlsHoJoPayr4400Export.do","extgwtFrame", JsUtil.toJavaScriptObject(param)); 
	}
...
//               srhBusinCd.setValue(bmInit);    //사업
         ...
         srhPayCd.setValue(lsPayCd.getAt(0));     //급여구분
