...
@Controller
public class Payr5400ExcelController {


    private static final Logger logger = LoggerFactory.getLogger(Payr5400ExcelController.class);  
    private static final String calledClass = Payr5400ExcelController.class.getName();

    
    @Resource(name = "payr5400ExcelService")
    private Payr5400ExcelService payr5400ExcelService;
     ...
     * @throws Exception
     */
    @RequestMapping(value="/exp/XlsPayr5400TempExport.do")
    public ModelAndView xlsPayr5400TotExport(@RequestParam HashMap<String, String> param, HttpServletRequest request ,ModelMap model) 
                    throws Exception { 
    	
    	String method = calledClass + ".xlsPayr5400TotExport";
    	
    	String windowNm = "개별수당관리";
		String windowId = "PAYR5400";
		...
		try{
	        ...
	        payr0520Vo.setPayExtpyCd(MSFSharedUtils.allowNulls(param.get("payItemCd")));		// 수당공제항목코드
	        
	        
	        List listPayr5400Vo = payr5400ExcelService.selectXlsPayr0520List(payr0520Vo);
	        
	        List<EgovMap> excelList = new ArrayList<EgovMap>();

	    	for(int i=0; i<listPayr5400Vo.size(); i++){
	    		EgovMap eMap  = new EgovMap();
	    		eMap = (EgovMap) listPayr5400Vo.get(i);
	        ...
	        map.put("listPayr5400", excelList);  
        ...
        return new ModelAndView("payr5400ExcelView", map);
     ...
     */
    @RequestMapping(value = "/imp/XlsReadPayr5400Import.do", method = RequestMethod.POST)
    public String handleFormUpload(@RequestParam("payr5400flUp") MultipartFile file,
    	...
    	String xmlConfig = "/upXmlConfig/PAYR/Payr5400Mapping.xml";
		...
		try {
			...
			if(xlsReadStatus.isStatusOK()) {
				...
				if( payr0520List == null || payr0520List.size() <= 0 ){
					...
					result = payr5400ExcelService.insertXlsPayr5400ToPayr0520(payr0520List, request);
...
//				// 엑셀 업로드 서비스 호출
//				// 배치 처리
//				payr5400ExcelService.insertXlsPayr5400ToPayr0520(payr0520List, request);
//				sResult =  "success";
