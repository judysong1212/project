...
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Pyyt0302Def;
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Pyyt0304Def;
import com.app.exterms.yearendtax.client.languages.YetaConstants;
import com.app.exterms.yearendtax.client.service.yeta2016.Yeta1900Service;
import com.app.exterms.yearendtax.client.service.yeta2016.Yeta1900ServiceAsync;
...
=====================================================**/
public class Yeta1900  extends MSFPanel { 
...
//	// private MSFCustomForm msfCustomForm; 
	private Pyyt0304Def pyyt0304Def  = new Pyyt0304Def("YETA1900");   //그리드 테이블 컬럼 define  
	// private MSFCustomForm msfCustomForm; 
	private MSFGridPanel pyyt0304GridPanel;
	  
	//수당 
	private Pyyt0302Def pyyt0302ExtpyDef  = new Pyyt0302Def("PYYT0302EXTPY");   //그리드 테이블 컬럼 define  
	// private MSFCustomForm msfCustomForm; 
	private MSFGridPanel pyyt0302ExtpyGridPanel; 
	  
	//공제 
	private Pyyt0302Def pyyt0302DducDef  = new Pyyt0302Def("PYYT0302DDUC");   //그리드 테이블 컬럼 define  
	// private MSFCustomForm msfCustomForm; 
...
//      private TextField<String> srhMangeDeptNm; //관리부서 수정  
		...
		private FileUploadField yeta1900flUp; // 파일업로드처리 부분
 ...
 **/
	//입력수정삭제처리 rpc 처리부     
	private Yeta1900ServiceAsync yeta1900Service = Yeta1900Service.Util.getInstance();
...
//                            pyyt0302Dto.setRevnAddr(MSFSharedUtils.allowNulls(bmMapModel.get("revnAddr")));    /** column 수정주소 : revnAddr */                           
            		...
            		yeta1900Service.saveYeta1900ToPyyt0302(listPyyt0302Dto, flag, new AsyncCallback<Integer>() {
                        public void onFailure(Throwable caught) {
                            MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
                                    MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta1900ToPyyt0302(save) : " + caught), null);
                        }
 ...
 **/ 	
	...
	public Yeta1900(String txtForm) {
...
//                        pyyt0302Dto.setRevnAddr(MSFSharedUtils.allowNulls(bmMapModel.get("revnAddr")));    /** column 수정주소 : revnAddr */                           
				...
				MessageBox.confirm("경고", "해당 대상자를 정말로 삭제하겠습니까?(기등록된 수당공제항목도 함께 삭제됩니다.)</BR> 삭제된 데이터는 복구되지 않습니다.", new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						//if("Yes".equals(be.getButtonClicked().getText())) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
			        		yeta1900Service.deleteYeta1900List(listPyyt0302Dto, new AsyncCallback<Integer>() {
			                    public void onFailure(Throwable caught) {
			                        MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		...
		creYetaDpcd.addSelectionChangedListener(new SelectionChangedListener<BaseModel>() {
			public void selectionChanged(SelectionChangedEvent<BaseModel> se) {
				...
				if (bmClutYr != null) {
					...
					creBusoprRgstnum.removeAllListeners();
...
//		payDdlneYn = new HiddenField<String>();			/** column 급여마감여부 : payDdlneYn */
		...
		fldstNewFieldset.setSize("340px", "440px");
		...
		layoutContainer1.add(layoutContainer_1, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.35));
...
//		layoutContainer_10.setLayout(new FitLayout());
		...
		fldstNewFieldset_1.setSize(622, 440);
		...
		layoutContainer_2.setBorders(false);
		layoutContainer1.add(layoutContainer_2, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.65));
...
//        });  
	...
	private LayoutContainer payrLeft01() {
		...
		pyyt0302ExtpyGridPanel.getBottomComponent().setVisible(false);
 	...
 	private LayoutContainer payrLeft02() {
		...
		Pyyt0302DducGridPanel.getBottomComponent().setVisible(false);
...
//		fldstNewFieldset.setStyleAttribute("marginTop", "10px");
	    ...
	    yeta1900flUp = new FileUploadField();
		yeta1900flUp.setFieldLabel("급여업로드");
		yeta1900flUp.setName("yeta1900flUp");
		yeta1900flUp.setValidator(new Validator() {  
			@Override 
		...
		yeta1900flUp.addListener(Events.Change, new Listener<FieldEvent>() {
	...
	//			excelGrid.getStore().removeAll();
		...
		layoutContainer_21.add(yeta1900flUp, new FormData("100%"));
...
//			pyyt0304GridPanel.getTableDef().addColumnFilter("payCd", MSFSharedUtils.getSelectedComboValue(srhPayCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
         ...
         pyyt0304GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
         pyyt0304GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
         pyyt0304GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   
			
         pyyt0304GridPanel.getTableDef().addColumnFilter("yetaDpcd", MSFSharedUtils.getSelectedComboValue(creYetaDpcd, "yetaDpcd"), SimpleColumnFilter.OPERATOR_EQUALS);
         pyyt0304GridPanel.getTableDef().addColumnFilter("busoprRgstnum", MSFSharedUtils.getSelectedComboValue(creBusoprRgstnum, "busoprRgstnum"), SimpleColumnFilter.OPERATOR_EQUALS);
	...
	public void payrInfoReload() {
		...
		pyyt0304Dto.setHanNm((String) bmData.get("hanNm"));
         ...
         yeta1900Service.Yeta1900InfoList(pyyt0304Dto, new AsyncCallback<Pyyt0304BM>() {
...
//		payDdlneYn.setValue("") ;  /** column 급여마감여부 : payDdlneYn */
	...
	public void exportExcel01() {
		...
		// payr0301GridPanel.callExportServlet("");
	       ...
	       // dlgn0250GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsDlgn0250Export.do","extgwtFrame" ,param);  
	        GWTUtils.xlsExportData(GWT.getHostPageBaseURL() + "exp/XlsYeta1900Export01.do","extgwtFrame", JsUtil.toJavaScriptObject(param)); 
	...
	public void exportExcel02() {
		...
		// payr0301GridPanel.callExportServlet("");
	        ...
	        DateTimeFormat dateTimeFormat = DateTimeFormat.getFormat("yyyyMMdd");
	        param.put("year", (MSFSharedUtils.getSelectedComboValue(srhPayYr,"year"))); 
	       ...
	       // dlgn0250GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsDlgn0250Export.do","extgwtFrame" ,param);  
	        ...
	        pyyt0304GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta1900Export02.do","extgwtFrame" ,param);
	...
	public void exportTotExcel() {
	        ...
	        //넘어온메서드 값을 가지고 처리 한다.  
	       // dlgn0250GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsDlgn0250Export.do","extgwtFrame" ,param);  
	        GWTUtils.xlsExportData(GWT.getHostPageBaseURL() + "exp/XlsYeta1900Export03.do","extgwtFrame", JsUtil.toJavaScriptObject(param)); 
	...
	private void xlsExportData(String strTyp) {
        ...
        if("typ01".equals(strTyp)) {	 	// 가변
        	 pyyt0304GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsTemYeta190001Export.do","extgwtFrame", param);  
        }else {								// 고정
		    	...
		    	public void handleEvent(MessageBoxEvent be) {
		      	//	if("Yes".equals(be.getButtonClicked().getText())){
		      		...
		      		if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
	              		...
	              		String filename = MSFSharedUtils.allowNulls(yeta1900flUp.getValue());
	              		if (filename.length() == 0) {
	                		...
	                		// 가변
	                		if("typ01".equals(MSFSharedUtils.getSelectedComboValue(exlType,"commCd"))) {
	                			plFrmYeta1900.setAction(GWT.getHostPageBaseURL() + "imp/XlsReadYeta190001Import.do");
	                		}else {
	                		// 고정
	                			plFrmYeta1900.setAction(GWT.getHostPageBaseURL() + "imp/XlsReadYeta190002Import.do");
	                		}
