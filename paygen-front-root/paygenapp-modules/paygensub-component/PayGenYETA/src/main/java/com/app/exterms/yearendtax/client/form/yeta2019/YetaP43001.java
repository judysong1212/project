...
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye165030BM;
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye165030DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye165030Def;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43001Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43001ServiceAsync;
...
=====================================================**/
public  class YetaP43001   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP43001GridPanel;
//	private Ye16Ta3170Def YetaP43001ToYe5030Def  = new Ye16Ta3170Def("YetaP43001");	//이월기부금 그리드 테이블 컬럼 define 
	private Ye165030Def YetaP43001ToYe5030Def  = new Ye165030Def("YetaP43001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
// 	private TextField<String> srhClutSeptCd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP43001ServiceAsync yetaP43001Service = YetaP43001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP43001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		lsdacRvyyStore = SysComComboUtils.getSysComComboYearData(msfCoCalendarDto);		//급여년도
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent be) {
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 YetaP43001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = YetaP43001GridPanel.getTableDef().getDefaultValues();  
		         ...
		         YetaP43001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         YetaP43001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         YetaP43001GridPanel.getMsfGrid().getGrid().startEditing(YetaP43001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 이월기부금 내용을 삭제하시겠습니까?" , new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){	
							...
							Iterator<BaseModel> itBm = YetaP43001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP43001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	private void createMsfGridForm() {
		...
		YetaP43001GridPanel = new MSFGridPanel(YetaP43001ToYe5030Def, false, false, false, false, false);
		YetaP43001GridPanel.setBorders(false);
		YetaP43001GridPanel.setBodyBorder(false);
		YetaP43001GridPanel.setHeaderVisible(false); 
		YetaP43001GridPanel.getBottomComponent().setVisible(false);
		
		GWTUtils.findColumnConfig(YetaP43001GridPanel, Ye165030BM.ATTR_THCYRDDCAMT).setStyle("background-color:#F5F6CE;");
		
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		GWTUtils.findColumnConfig(YetaP43001GridPanel, Yeta3170BM.ATTR_CTRBAMNT).setStyle("Height:20px;");
		...
		YetaP43001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		
		cpYeta23001.add(YetaP43001GridPanel);
		cp01.add(YetaP43001GridPanel);
	...
	private void YetaP43001FormSave() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			if(dataChk) {
				...
				yetaP43001Service.activityOnYetaP43001ToYe165030(listYe165030Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP43001ToYe165030(" + actionDatabase.name() + ") : " + caught), null);
					}
	...
	public void reload() {
		...
		YetaP43001GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP43001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		
		YetaP43001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		
		YetaP43001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		
		YetaP43001GridPanel.reload();
