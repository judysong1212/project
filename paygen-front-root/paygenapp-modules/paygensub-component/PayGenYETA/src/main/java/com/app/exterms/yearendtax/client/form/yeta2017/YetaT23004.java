...
=====================================================**/
public  class YetaT23004   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT23004; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaT23004GridPanel;
	private MSFGridPanel ye162530GridPanel; //임대차계약그리드
	private MSFGridPanel ye162520GridPanel; // 금전소비대차그리드
	private Ye162520Def ye162520Def = new Ye162520Def("YetaT23004");   
	private Ye162530Def ye162530Def = new Ye162530Def("YetaT23004");   
...
//		 }
	...
	public YetaT23004(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT23004 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT23004.setBodyBorder(false);
		cpYetaT23004.setBorders(false);
		cpYetaT23004.setHeaderVisible(false);
		cpYetaT23004.setSize("790px", "640px");
		   
		this.add(cpYetaT23004);
...
//		     });     
		...
		cpYetaT23004.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				yetaT23004GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = yetaT23004GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        yetaT23004GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        yetaT23004GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        yetaT23004GridPanel.getMsfGrid().getGrid().startEditing(yetaT23004GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = yetaT23004GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(yetaT23004GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//				
//		cpYetaT23004.add(topYetaBtnBar);
...
//		
//		yetaT23004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		yetaT23004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT23004.add(yetaT23004GridPanel);
		...
		cpYetaT23004.add(fldstNewFieldset);
		cpYetaT23004.add(fldstNewFieldset1);
		cpYetaT23004.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			yetaT23004GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaT23004GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaT23004GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaT23004GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaT23004GridPanel.reload();
