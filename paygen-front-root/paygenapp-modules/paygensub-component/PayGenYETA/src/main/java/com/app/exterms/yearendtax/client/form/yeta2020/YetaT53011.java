...
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53011Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53011ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT53011Def;
...
=====================================================**/
public  class YetaT53011   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT53011; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT53011GridPanel;
//	private YetaT53011Def YetaT53011Def = new YetaT53011Def("YetaT53011");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT53011ServiceAsync yetaT53011Service = YetaT53011Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT53011Service.activityOnYetaT53011(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT53011(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT53011 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT53011.setBodyBorder(false);
		cpYetaT53011.setBorders(false);
		cpYetaT53011.setHeaderVisible(false);
		cpYetaT53011.setSize("790px", "640px");
		   
		this.add(cpYetaT53011);
...
//		     });     
		...
		cpYetaT53011.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT53011GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT53011GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT53011GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT53011GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT53011GridPanel.getMsfGrid().getGrid().startEditing(YetaT53011GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT53011GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT53011GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT53011.add(topYetaBtnBar);
...
//		YetaT53011GridPanel = new MSFGridPanel(YetaT53011Def, false, false, false, false, false);
//		YetaT53011GridPanel.setSize("790px", "420px");
//		YetaT53011GridPanel.setBorders(true);
//		YetaT53011GridPanel.setHeaderVisible(false);
...
//		
//		YetaT53011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT53011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT53011.add(YetaT53011GridPanel);
...
//		
		...
		cpYetaT53011.add(layoutContainer);
		cpYetaT53011.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT53011GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT53011GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT53011GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT53011GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT53011GridPanel.reload();
