...
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8100Service;
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8100ServiceAsync;
...
=====================================================**/
public class Yeta8100   extends MSFPanel {
	/**
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	 * 	전역변수 선언부  시작
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	...
	private FormPanel plFrmYeta8100;
	private String txtForm = "";
	private XTemplate detailTp;

	private Ye161010Def Yeta8100def = new Ye161010Def("Yeta8100");
	private MSFGridPanel Yeta8100GridPanel;
	...
	private ComboBox<BaseModel> srhMangDeptCd; 				// 관리부서 수정 / 04.27
	...
	private TextField<String> utDpobNm; // 부서명
	...
	private Yeta8100ServiceAsync yeta8100Service = Yeta8100Service.Util.getInstance(); 
	...
	private ListStore<BaseModel> lsMangeDeptCd 	= new ListStore<BaseModel>();			//관리부서 수정 / 05.04
	...
	private ComboBox<BaseModel> srhMangeDeptCd; // 관리부서 추가 / 05.03
	...
	private void authExecEnabled() { 
		 ...
		 * 값순서 : 단위기관 ,고용구분, 부서, 직종, 직종세 ,사업 순으로 없으면 ""
				 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
		//----------------------------------------------------
		Field<?>[] fldArrField = {srhMangDeptCd,srhEmymtDivCd,srhDeptCd,srhTypOccuCd,srhDtilOccuInttnCd,srhBusinCd}; // 04.27
		// Field<?>[] fldArrField = {srhPayrMangDeptCd,srhEmymtDivCd,srhDeptCd,srhTypOccuCd,srhDtilOccuInttnCd,srhBusinCd};
		gwtAuthorization.formAuthFieldConfig(fldArrField);

		srhClutYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2023")));
	...
	private void initLoad() {
		...
		authMapDef.put("srhMangDeptCd",Boolean.FALSE); // 관리부서 수정 / 05.03
...
//		authMapDef.put("creDeptCd",Boolean.FALSE); 
	 ...
	 * 데이타를 삭제한다.
	 */
	private void Yeta8100_menuDelete() {
		...
		MessageBox.confirm("연말정산 대상자 삭제", "선택된 데이타를 삭제 하시겠습니까?<br>(연말정산 대상자를 삭제합니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = Yeta8100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
					...
					yeta8100Service.Yeta8100ToYe160401_Delete(listYe161010Dto, new AsyncCallback<Long>() {
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
									MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta8100ToYe160401_Delete(삭제) : " + caught), null);
						}
...
//
//		//Iterator<BaseModel> itBm = Yeta8100GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
//
//		Iterator<BaseModel> itBm = Yeta8100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator();
//
//		List<Record> lsRec = new ArrayList<Record>();
...
//			Yeta8100Service.UpDpobExec_Yeta_Insert_2017(listYe161010Dto, new AsyncCallback<Long>() {
//				public void onFailure(Throwable caught) {
//					tracker.setStatus(true);
//					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
...
//	}
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			while (records.hasNext()) {
				...
				if ((Boolean) bmMapModel.get("select")) {
					...
					ye161010Dto.setSystemkey(MSFSharedUtils.allowNulls(bmMapModel.get("systemkey")));    					/** column SYSTEMKEY : systemkey */
					ye161010Dto.setPayrMangDeptCd(MSFSharedUtils.allowNulls(bmMapModel.get("payrMangDeptCd")) );
					ye161010Dto.setMangeDeptCd(MSFSharedUtils.allowNulls(bmMapModel.get("mangeDeptCd"))  ); // 관리부서 수정 / 05.03
					ye161010Dto.setDeptCd(MSFSharedUtils.allowNulls(bmMapModel.get("deptCd"))  );
					ye161010Dto.setEmymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("emymtDivCd")) );
					ye161010Dto.setTypOccuCd(MSFSharedUtils.allowNulls(bmMapModel.get("typOccuCd")) );
					ye161010Dto.setDtilOccuInttnCd(MSFSharedUtils.allowNulls(bmMapModel.get("dtilOccuInttnCd")));
			...
			t.scheduleRepeating(500);
			tracker.setStatus(true);
			yeta8100Service.PayrExec_Yeta_2023_Insert(listYe161010Dto, new AsyncCallback<Long>() {
				public void onFailure(Throwable caught) {
					tracker.setStatus(true);
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("PayrExec_Yeta_Insert(급여자료이관) : " + caught), null);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 **/
	private void Yeta8100Print(String fileName, String repType) {
...
//			RdaPrint(fileName);
	...
	private void RexPrint(String fileName) {
		...
		String mangdeptcd = MSFSharedUtils.getSelectedComboValue(srhMangDeptCd, "mangeDeptCd") ; // 05.03
...
//			serarchParam += "" + YetaUtils.getPrintStrToMultiData(lsDeptCd, MSFSharedUtils.allowNulls(srhDeptCd.getValue()),"deptNmRtchnt","deptCd") +"⊥";		
     		//20151212-추가 시작 
		    if (mangdeptcd.equals(deptCdAuth)) { 
		    	serarchParam += "⊥"; ////$9 부서
            } else {
			...
			List<BaseModel> list = Yeta8100GridPanel.getGrid().getSelectionModel().getSelectedItems();
			...
			//20151212-추가 끝 
			serarchParam += "" + mangdeptcd+"⊥";  		//$18 관리부서	/ 05.03 추가
	...
	public Yeta8100(String txtForm) {
	...
	private void createYeta0100Form() {
		...
		plFrmYeta8100 = new FormPanel();
		plFrmYeta8100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산대상자관리"));
		plFrmYeta8100.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta8100.setPadding(2);
		plFrmYeta8100.setFrame(true);
		...
		plFrmYeta8100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta8100.getHeader().addTool(btnlogView);
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		// 멀티콤보박스 닫기
		plFrmYeta8100.addListener(Events.OnClick, new Listener<ComponentEvent>() {
			@Override
		...
		btnYeta0100Init.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Yeta8100GridPanel.getMsfGrid().clearData();
			}
		});
		...
		btnYeta0100Cre.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP810001 yetaP810001Form = new YetaP810001(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta810001 = new MSFFormWindows("연말정산대상자검색",yetaP810001Form, "닫기", "920px", "710px", false);
				msFwYeta810001.show();
				yetaP810001Form.setMSFFormWindows(msFwYeta810001);
				...
				ppRecord.set("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangDeptCd, "mangeDeptCd")); // 관리부서 수정 / 05.03
		...
		plFrmYeta8100.add(topYeta0100Bar);
		...
		btnYeta0100Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				Yeta8100Print("YETAT8100", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
			}
		});
		plFrmYeta8100.add(topYeta0100Bar);	
		...
		// 파일업로드 처리
		plFrmYeta8100.setAction("myurl");
		plFrmYeta8100.setEncoding(Encoding.MULTIPART);
		plFrmYeta8100.setMethod(Method.POST);
		vp.add(plFrmYeta8100);

		plFrmYeta8100.setSize("990px", "680px");
		plFrmYeta8100.setLayout(new FlowLayout());
	}
	...
	private void createSearchForm() {   		
		...
		srhPayrMangDeptCd.addSelectionChangedListener(new SelectionChangedListener<BaseModel>() {
			public void selectionChanged(SelectionChangedEvent<BaseModel> se) {
				...
				sysComBass0400Dto.setMangeDeptCd(MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd")); // 관리부서 수정 / 05.03
...
// 관리부서로 변경 / 05.03
		...
		srhMangeDeptCd.setFieldLabel("관리부서"); // 04.20 -> 부서에서 관리부서로 변경 
...
// 부서
		...
		layoutContainer5.add(srhMangeDeptCd, new FormData("100%"));
...
//		srhDtilOccuInttnCd.setEnabled(gwtExtAuth.getEnableDtilOccuInttnCd());
		...
		utDpobNm = new TextField<String>(); // 04.20 / -> 원천징수부서 색상 변경
		...
		btnUpDpobTrans.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP810002 yetaP810002Form = new YetaP810002(ActionDatabase.READ, getThis());

				//final MSFFormWindows msFwYeta810002 = new MSFFormWindows("원천신고부서",yetaP810002Form, "닫기", "920px", "710px", false);
				final MSFFormWindows msFwYeta810002 = new MSFFormWindows("원천신고부서",yetaP810002Form, "닫기", "1050px", "800px", false);
				msFwYeta810002.show();
				yetaP810002Form.setMSFFormWindows(msFwYeta810002);
				...
				bmRecord.set("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd")); // 관리부서
		...
		plFrmYeta8100.add(fieldSet1);
		plFrmYeta8100.add(fieldSet2);	
	...
	private void createMsfGridForm() {
		...
		Yeta8100GridPanel = new MSFGridPanel(Yeta8100def, false, false, false,false, false);
		// Yeta8100GridPanel.setSize(960, 400);
		Yeta8100GridPanel.setHeaderVisible(false);
		Yeta8100GridPanel.setBorders(true);
		// Yeta8100GridPanel.getBottomComponent().setVisible(false);
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = Yeta8100GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					Yeta8100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					Yeta8100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				Yeta8100GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					Yeta8100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					Yeta8100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		//툴바 넣어주는것
		Yeta8100GridPanel.setBottomComponent(bottomToolbar);

		final Grid<BaseModel> grid = Yeta8100GridPanel.getMsfGrid().getGrid();
		...
		cp01.add(Yeta8100GridPanel); 

		fieldSetGrd.add(cp01);
		plFrmYeta8100.add(fieldSetGrd);
	}
...
//	}	
	...
	public void reload() {
		...
		Yeta8100GridPanel.getTableDef().setTableColumnFilters(filters);

		Yeta8100GridPanel.getTableDef().addColumnFilter("yrtxBlggYr",MSFSharedUtils.getSelectedComboValue(srhClutYr, "year"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta8100GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd"), SimpleColumnFilter.OPERATOR_EQUALS);	

		Yeta8100GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta8100GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		

		// 관리부서 수정 / 05.03
		Yeta8100GridPanel.getTableDef().addColumnFilter("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta8100GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta8100GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta8100GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta8100GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta8100GridPanel.getTableDef().addColumnFilter("fnm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta8100GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta8100GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(utDpobCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta8100GridPanel.getTableDef().addColumnFilter("utDpobChk", MSFSharedUtils.convertBooleanToString(utDpobChk.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		

		Yeta8100GridPanel.reload();
	...
	private void xlsExportData() {
		...
		param.put("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd")); // 관리부서 수정
		...
		if ("".equals(srhClutYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				Yeta8100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta8100Export.do", "extgwtFrame", param);
			}
		} else {
			Yeta8100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta8100Export.do", "extgwtFrame", param);
		}
