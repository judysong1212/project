...
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23008Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23008ServiceAsync;
...
=====================================================**/
public  class YetaT23008   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT23008; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaT23008GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT23008ServiceAsync yetaT23008Service = YetaT23008Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT23008Service.activityOnYetaT23008(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT23008(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT23008 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT23008.setBodyBorder(false);
		cpYetaT23008.setBorders(false);
		cpYetaT23008.setHeaderVisible(false);
		cpYetaT23008.setSize("790px", "640px");
		   
		this.add(cpYetaT23008);
...
//		     });     
		...
		cpYetaT23008.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				yetaT23008GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = yetaT23008GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        yetaT23008GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        yetaT23008GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        yetaT23008GridPanel.getMsfGrid().getGrid().startEditing(yetaT23008GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = yetaT23008GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaT23008GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT23008.add(topYetaBtnBar);
...
//		cpYetaT23008.add(topYetaBtnBar);
//		
//		yetaT23008GridPanel = new MSFGridPanel(yetaT23008Def, false, false, false, false, false);
//		yetaT23008GridPanel.setSize("790px", "420px");
//		yetaT23008GridPanel.setBorders(true);
//		yetaT23008GridPanel.setHeaderVisible(false);
//		
...
//		
//		yetaT23008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		yetaT23008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT23008.add(yetaT23008GridPanel);
		...
		cpYetaT23008.add(fldstNewFieldset);
		cpYetaT23008.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			yetaT23008GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaT23008GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaT23008GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaT23008GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaT23008GridPanel.reload();
