...
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73008Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73008ServiceAsync;
...
=====================================================**/
public  class YetaT73008   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73008; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73008GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT73008ServiceAsync yetaT73008Service = YetaT73008Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT73008Service.activityOnYetaT73008(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT73008(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73008 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73008.setBodyBorder(false);
		cpYetaT73008.setBorders(false);
		cpYetaT73008.setHeaderVisible(false);
		cpYetaT73008.setSize("790px", "640px");
		   
		this.add(cpYetaT73008);
...
//		     });     
		...
		cpYetaT73008.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT73008GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT73008GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT73008GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT73008GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT73008GridPanel.getMsfGrid().getGrid().startEditing(YetaT73008GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT73008GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT73008GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT73008.add(topYetaBtnBar);
...
//		cpYetaT73008.add(topYetaBtnBar);
//		
//		YetaT73008GridPanel = new MSFGridPanel(YetaT73008Def, false, false, false, false, false);
//		YetaT73008GridPanel.setSize("790px", "420px");
//		YetaT73008GridPanel.setBorders(true);
//		YetaT73008GridPanel.setHeaderVisible(false);
//		
...
//		
//		YetaT73008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT73008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT73008.add(YetaT73008GridPanel);
		...
		cpYetaT73008.add(fldstNewFieldset);
		cpYetaT73008.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT73008GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73008GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73008GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73008GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73008GridPanel.reload();
