...
import com.app.exterms.yearendtax.client.service.yeta2016.Yeta1500Service;
import com.app.exterms.yearendtax.client.service.yeta2016.Yeta1500ServiceAsync;
...
=====================================================**/
public class Yeta1500   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmYeta1500; 
    private String txtForm = "";
    private XTemplate detailTp;
 
    private Ye16Ta1000Def yeta1000def = new Ye16Ta1000Def("YETA1500");
...
//		private MSFGridPanel typOccuCdGridPanel;
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
   private ButtonBar topYeta1500Bar;
   private Button btnYeta1500Init; 
//   private Button btnYeta0100Save;
//   private Button btnYeta0100Del;
   private Button btnYeta1500Sreach;
//   private Button btnYeta0100Print;
   private Button btnYeta1500Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
      ...
      **/
		// -------------- 검색조건 생성 폼 시작 --------------
    	private MSFComboBox<BaseModel> srhEdacRvyy;				//정산년
...
//	private Yeta1100ServiceAsync yeta1100Service = Yeta1100Service.Util.getInstance();
	private Yeta1500ServiceAsync yeta1500Service = Yeta1500Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 
   ...
   // -------------- stroe [검색부]선언 시작 -------------- 
	   ...
	   **/
	 	 ...
	 	 */
	  	private void checkYeta1500Auth( String authAction, ListStore<BaseModel>bm) {
		 ...
		 private void authExecEnabled() { 
		   //------------------
			 ...
			 srhSettGbcd.setValue(lsCalcSevePayPsnDivCd.getAt(0));
...
//       	               }    
//	                       
//       	            yeta1100Service.Payr06420_Payr400_Delete(listYeta2000Dto ,
...
//       	               }    
//	                       
//       	            yeta1100Service.PayrExec_Yeta_Insert(listYeta2000Dto ,
//	                               new AsyncCallback<Long>() {
//	                       public void onFailure(Throwable caught) {
//	                       MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
...
//	    
	 ...
	 **/ 	 
		...
		public Yeta1500(String txtForm) {
	 ...
	 **/		  
		private void createYeta0100Form() {
			...
			plFrmYeta1500 = new FormPanel(); 
			plFrmYeta1500.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산급여반영"));
			plFrmYeta1500.setIcon(MSFMainApp.ICONS.text());
			plFrmYeta1500.setPadding(2);
			plFrmYeta1500.setFrame(true); 
			...
			btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 //로그 뷰화면 호출 메서드 
					  funcLogMessage("연말정산급여반영","YETA1500");
					}
				});
			plFrmYeta1500.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
			plFrmYeta1500.getHeader().addTool(btnlogView); 
			/************************************************************************
			 * 화면 에러 처리시 필요한 부분으로 추가 
			 ************************************************************************/
 
		     //멀티콤보박스 닫기 
			plFrmYeta1500.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
	            @Override
...
//	                        }
			...
			/** 상단 버튼 ButtonBar**/
			topYeta1500Bar = new ButtonBar();    
			topYeta1500Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
			btnYeta1500Init = new Button("초기화");  
			btnYeta1500Init.setIcon(MSFMainApp.ICONS.new16());
			topYeta1500Bar.add(btnYeta1500Init); 
			btnYeta1500Init.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//초기화 
	    			formInit();
	    		}
			...
			btnYeta1500Sreach = new Button("조 회");  
			btnYeta1500Sreach.setIcon(MSFMainApp.ICONS.search());
			topYeta1500Bar.add(btnYeta1500Sreach);
			btnYeta1500Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//조회버튼 클릭시 처리 
	    			reloadYeta1500();  
	    		}
...
//			topYeta1500Bar.add(btnYeta0100Print);	
			
//			btnYeta0100Print = new Button("인 쇄");  
//			btnYeta0100Print.setIcon(MSFMainApp.ICONS.print16());
//			topYeta1500Bar.add(btnYeta0100Print);	
			
			btnYeta1500Excel = new Button("엑 셀");  
			btnYeta1500Excel.setIcon(MSFMainApp.ICONS.excel16());
			topYeta1500Bar.add(btnYeta1500Excel);
			btnYeta1500Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//엑셀 저장
	    			xlsExportData();  
	    		}
			...
			plFrmYeta1500.add(topYeta1500Bar);
		      
			//파일업로드 처리
			plFrmYeta1500.setAction("myurl");
			plFrmYeta1500.setEncoding(Encoding.MULTIPART);
			plFrmYeta1500.setMethod(Method.POST);		         
			vp.add(plFrmYeta1500);	
			
			plFrmYeta1500.setSize("990px", "680px");	      
			plFrmYeta1500.setLayout(new FlowLayout());
		}
    ...
    */
    private void createSearchForm() {   		
		  ...
		  //--------------------단위기관 불러 오는 함수 ------------------------------------------------
		    ...
		    /**
		     * Y0110001	Y011	0001	전체
		     * Y0110002	Y011	0002	10만원 이상
		     * Y0110003	Y011	0003	10만원 이하
		     * Y0110004	Y0110	0004	환급금
			 * Y0110005	Y011	0005	환수금
		     * **/
		    sysComBass0300Dto.setRpsttvCd("Y011");
...
//		  	plFrmYeta1500.add(typOccuCdGrid);
			...
			plFrmYeta1500.setLayout(new FlowLayout());
			  ...
			  layoutContainer1.setLayout(frmlytSch);				  
			    ...
			    srhEdacRvyy = new MSFComboBox<BaseModel>();
...
//														 searchBusinCd.setValue(lsBusinCd.getAt(0));
			    ...
			    lsPayrMangDeptCd.addStoreListener( new StoreListener<BaseModel>() {   
			        public void handleEvent(StoreEvent<BaseModel> be) {  
			        	checkYeta1500Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
			        }
			    });    
				    	   ...
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta1500Auth("srhEmymtDivCd", lsEmymtDivCd); 
			           	 }
			        }
			    });    
...
//			    frmlytSch.setLabelWidth(0); 
			    ...
			    srhSettGbcd.setStore(lsCalcSevePayPsnDivCd);
			    ...
			    layoutContainer7.add(srhSettGbcd, new FormData("100%"));
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta1500Auth("srhDeptCd", lsDeptCd); 
			           	 }
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		 //  checkYeta1500Auth("srhTypOccuCd", lsTypOccuCd); 
			           	 }
			        }
			    }); 
...
//		GWTUtils.findColumnConfig(msfGridPanel, Yeta2000BM.ATTR_DIVDPYMTDIVCD).setStyle("background-color:#F5F6CE;");
			...
			public void componentSelected(ButtonEvent ce) { 
				...
				if (MSFSharedUtils.paramNotNull(msfGridPanel.getCurrentlySelectedItem())) { 
					...
					while (records.hasNext()) {
		                ...
		                yeta2000Dto.setDpobCd(MSFSharedUtils.allowNulls(bmMapModel.get("dpobCd")));						/** 사업장코드 : dpobCd */
		                yeta2000Dto.setSystemkey(MSFSharedUtils.allowNulls(bmMapModel.get("systemkey")));   			/** SYSTEMKEY : systemkey */
		                yeta2000Dto.setEdacRvyy(MSFSharedUtils.allowNulls(bmMapModel.get("edacRvyy")));    				/** column 연말정산귀속년도 : edacRvyy */
		                yeta2000Dto.setSettGbcd(MSFSharedUtils.allowNulls(bmMapModel.get("settGbcd")));    				/** column 정산구분코드 : settGbcd */
		                
		                yeta2000Dto.setDivdPymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("divdPymtDivCd$commCd")));/** column 분할납부구분코드 : divdPymtDivCd */
		                yeta2000Dto.setDivdPymt("0");    																/** column 분할납부횟수 : divdPymt */
		                yeta2000Dto.setYrtxPrcsYn((Boolean) bmMapModel.get("yrtxPrcsYn"));								/** column 연말정산처리여부 : yrtxPrcsYn */
					...
					yeta1500Service.saveYeta1500Grid(listYeta2000dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta1500Grid(" + "저장" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
		...
		cp01.addButton(new Button("전월급여반영", new SelectionListener<ButtonEvent>() {   
			@Override  
			public void componentSelected(ButtonEvent ce) {   
...
//					ntime = format.format(c.getTime());
					...
					cal.add(cal.MONTH,-1);
					String beforeMonth  = fmt2.format(cal.getTime());
...
//					ntime += fmt2.format(today);
					...
					while (records.hasNext()) {
		                ...
		                yeta2000Dto.setYrtxApptnYrMnth(ntime);												/** column 연말정산적용년월 : yrtxApptnYrMnth */
		                yeta2000Dto.setYrtxPrcsDt("");														/** column 연말정산처리일자 : yrtxPrcsDt */
		                yeta2000Dto.setYrtxPrcsYn((Boolean) bmMapModel.get("yrtxPrcsYn"));					/** column 연말정산처리여부 : yrtxPrcsYn */
		                yeta2000Dto.setDivdPymtDivCd("B0300010");											/** column 분할납부구분코드 : divdPymtDivCd */
		                yeta2000Dto.setDivdPymt("0");    													/** column 분할납부횟수 : divdPymt */
					...
					yeta1500Service.activityOnYeta1500List(listYeta2000dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta1500List(" + "당월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
		...
		cp01.addButton(new Button("당월급여반영", new SelectionListener<ButtonEvent>() {   
			@Override  
			public void componentSelected(ButtonEvent ce) {   
...
//					ntime = format.format(c.getTime());
					...
					while (records.hasNext()) {
		                ...
		                yeta2000Dto.setYrtxApptnYrMnth("");			/** column 연말정산적용년월 : yrtxApptnYrMnth */
		                yeta2000Dto.setYrtxPrcsDt("");				/** column 연말정산처리일자 : yrtxPrcsDt */
		                yeta2000Dto.setYrtxPrcsYn(false);			/** column 연말정산처리여부 : yrtxPrcsYn */
		                yeta2000Dto.setDivdPymtDivCd("B0300010");	/** column 분할납부구분코드 : divdPymtDivCd */
		                yeta2000Dto.setDivdPymt("0");    			/** column 분할납부횟수 : divdPymt */
	...
	private Yeta1500 getThis() {
		return this;
	}
		   
	private void reloadYeta1500() {
	...
	private void xlsExportData() {
		...
		param.put("edacRvyy", MSFSharedUtils.getSelectedComboValue(srhEdacRvyy, "year"));//귀속년도
		param.put("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd"));//정산구분
		...
		if ("".equals(srhEdacRvyy)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta1500Export.do", "extgwtFrame", param);
			}
		} else {
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta1500Export.do", "extgwtFrame", param);
		}
