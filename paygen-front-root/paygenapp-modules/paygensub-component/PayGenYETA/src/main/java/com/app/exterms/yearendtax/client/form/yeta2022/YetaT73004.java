...
=====================================================**/
public  class YetaT73004   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73004; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73004GridPanel;
	private MSFGridPanel ye162530GridPanel; //임대차계약그리드
	private MSFGridPanel ye162520GridPanel; // 금전소비대차그리드
	private Ye162520Def ye162520Def = new Ye162520Def("YetaT73004");   
	private Ye162530Def ye162530Def = new Ye162530Def("YetaT73004");   
...
//		 }
	...
	public YetaT73004(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73004 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73004.setBodyBorder(false);
		cpYetaT73004.setBorders(false);
		cpYetaT73004.setHeaderVisible(false);
		cpYetaT73004.setSize("790px", "640px");
		   
		this.add(cpYetaT73004);
...
//		     });     
		...
		cpYetaT73004.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT73004GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT73004GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT73004GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT73004GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT73004GridPanel.getMsfGrid().getGrid().startEditing(YetaT73004GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT73004GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaT73004GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//				
//		cpYetaT73004.add(topYetaBtnBar);
...
//		
//		YetaT73004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT73004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT73004.add(YetaT73004GridPanel);
		...
		cpYetaT73004.add(fldstNewFieldset);
		cpYetaT73004.add(fldstNewFieldset1);
		cpYetaT73004.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT73004GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73004GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73004GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73004GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73004GridPanel.reload();
