...
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Ye162040Def;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53010Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53010ServiceAsync;
...
=====================================================**/
public  class YetaT53010   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT53010; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT53010GridPanel;
	private Ye162040Def ye162040Def = new Ye162040Def("YetaT53010");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT53010ServiceAsync yetaT53010Service = YetaT53010Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT53010Service.activityOnYetaT53010(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT53010(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT53010 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT53010.setBodyBorder(false);
		cpYetaT53010.setBorders(false);
		cpYetaT53010.setHeaderVisible(false);
		cpYetaT53010.setSize("790px", "640px");
		   
		this.add(cpYetaT53010);
...
//		     });     
		...
		cpYetaT53010.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT53010GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT53010GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT53010GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT53010GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT53010GridPanel.getMsfGrid().getGrid().startEditing(YetaT53010GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT53010GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT53010GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT53010.add(topYetaBtnBar);
...
//		cpYetaT53010.add(topYetaBtnBar);
		...
		YetaT53010GridPanel = new MSFGridPanel(ye162040Def, false, false, false, false, false);
		YetaT53010GridPanel.setSize("790px", "200px");
		YetaT53010GridPanel.setBorders(true);
		YetaT53010GridPanel.setHeaderVisible(false);
...
//		
//		YetaT53010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT53010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT53010.add(YetaT53010GridPanel);
		...
		fldstNewFieldset.add(YetaT53010GridPanel);
		cpYetaT53010.add(fldstNewFieldset);
		cpYetaT53010.add(layoutContainer10);
		cpYetaT53010.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT53010GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT53010GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT53010GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT53010GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT53010GridPanel.reload();
