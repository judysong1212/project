...
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23006Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23006ServiceAsync;
...
=====================================================**/
public  class YetaT23006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT23006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaT23006GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT23006");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT23006ServiceAsync yetaT23006Service = YetaT23006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
   ...
   */
	// -------------- 검색 조건 시작 --------------
 	...
 	private MSFNumberField srhSum1; //개인연금저축 변수명 다시 해줘야 함.
 	private MSFNumberField srhSum2; //합계 변수명 다시 해줘야 함.
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT23006Service.activityOnYetaT23006(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT23006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT23006 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT23006.setBodyBorder(false);
		cpYetaT23006.setBorders(false);
		cpYetaT23006.setHeaderVisible(false);
		cpYetaT23006.setSize("790px", "640px");
		   
		this.add(cpYetaT23006);
...
//		     });     
		...
		cpYetaT23006.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				yetaT23006GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = yetaT23006GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        yetaT23006GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        yetaT23006GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        yetaT23006GridPanel.getMsfGrid().getGrid().startEditing(yetaT23006GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaT23006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = yetaT23006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT23006.add(topYetaBtnBar);
...
//		cpYetaT23006.add(topYetaBtnBar);
		...
		yetaT23006GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		yetaT23006GridPanel.setSize("790px", "200px");
		yetaT23006GridPanel.setBorders(true);
		yetaT23006GridPanel.setHeaderVisible(false);
...
//		
//		yetaT23006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		yetaT23006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT23006.add(yetaT23006GridPanel);
	
		FieldSet fldstNewFieldset = new FieldSet();
		fldstNewFieldset.setHeadingHtml("개인연금저출불입액");
		...
		layoutContainer10.setStyleAttribute("marginTop", "7px");
		...
		srhSum2.setFieldLabel("합계");
		layoutContainer2.add(srhSum2, new FormData("95%"));
		...
		fldstNewFieldset.add(yetaT23006GridPanel);
		cpYetaT23006.add(fldstNewFieldset);
		cpYetaT23006.add(layoutContainer10);
		cpYetaT23006.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			yetaT23006GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaT23006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaT23006GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaT23006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaT23006GridPanel.reload();
