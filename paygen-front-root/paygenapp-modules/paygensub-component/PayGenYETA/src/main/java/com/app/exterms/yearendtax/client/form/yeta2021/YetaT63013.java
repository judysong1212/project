...
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye162010Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63013Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63013ServiceAsync;
...
=====================================================**/
public  class YetaT63013   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63013; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63013GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT63013");   
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63013ServiceAsync yetaT63013Service = YetaT63013Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63013Service.activityOnYetaT63013(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63013(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63013 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
	 	cpYetaT63013.setBodyBorder(false);
	 	cpYetaT63013.setBorders(false);
	 	cpYetaT63013.setHeaderVisible(false);
	 	cpYetaT63013.setSize("790px", "640px");
		   
		this.add(cpYetaT63013);
...
//		     });     
		...
		cpYetaT63013.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63013GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63013GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63013GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63013GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63013GridPanel.getMsfGrid().getGrid().startEditing(YetaT63013GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT63013GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaT63013GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//		cpYetaT63013.add(topYetaBtnBar);
...
//		cpYetaT63013.add(topYetaBtnBar);
		...
		YetaT63013GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT63013GridPanel.setSize("790px", "200px");
		YetaT63013GridPanel.setBorders(true);
		YetaT63013GridPanel.setHeaderVisible(false);
...
//		
//		YetaT63013GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63013GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63013.add(YetaT63013GridPanel);
		...
		fldstNewFieldset.add(YetaT63013GridPanel);
		cpYetaT63013.add(fldstNewFieldset);
		cpYetaT63013.add(layoutContainer10);
		cpYetaT63013.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63013GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63013GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63013GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63013GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63013GridPanel.reload();
