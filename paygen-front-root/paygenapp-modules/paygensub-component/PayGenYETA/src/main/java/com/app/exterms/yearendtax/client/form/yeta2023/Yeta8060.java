...
import com.app.exterms.yearendtax.client.form.defs.yeta2023.Yeta8060Def;
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8060Service;
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8060ServiceAsync;
 ...
 * =========================================================================
 **/
public class Yeta8060 extends MSFPanel {
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta8060Bar;
	private Button btnYeta8060Dpob; 
	private Button btnYeta8060New; 
	private Button btnYeta8060Save;
	private Button btnYeta8060Del;
	private Button btnYeta8060Sreach;
	// -------------- 상단 버튼 종료 --------------
...
//	private MSFComboBox<BaseModel> hhrkUntDpobCd; 					// 상위 단위사업장 코드
	...
	private CheckBoxGroup chkgrpYeta8060;   						// 체크 그룹
	...
	// -------------- 서비스 호출 시작 -------------- 
	//private InfcYeta8060ServiceAsync infcYeta8060Service = InfcYeta8060Service.Util.getInstance();
	private Yeta8060ServiceAsync yeta8060Service = Yeta8060Service.Util.getInstance();
	private PrgmComBass0300ServiceAsync sysComBass0300Service = PrgmComBass0300Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 


	// -------------- grid 시작 -------------- 
	private Yeta8060Def Yeta8060Def = new Yeta8060Def(); // 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel;


	private Yeta8060Def mYeta8060Def = new Yeta8060Def("Yeta8060_01"); // 주 그리드 테이블 컬럼 define
	...
	// private PrgmComYeta8060DTO sysComYeta8060Dto;
	// -------------- DTO 선언 종료  -------------- 

	private VerticalPanel vp;
	private FormPanel plFrmYeta8060;
	...
	private SelectionListener<ButtonEvent> selectionListener = new SelectionListener<ButtonEvent>() {
		public void componentSelected(ButtonEvent ce) {
			if (ActionDatabase.INSERT.equals(actionDatabase)) {
				actionDatabase = null;
				if (ce.getButton() == btnYeta8060Save) {
					doAction(ActionDatabase.INSERT);
				}
			} else if (ce.getButton() == btnYeta8060Save) {
				doAction(ActionDatabase.UPDATE);
				actionDatabase = null;
			} else if (ce.getButton() == btnYeta8060Del) {
	...
	public void save(final ActionDatabase actionDatabase) {
		...
		final Yeta8060DTO dto = new Yeta8060DTO();
...
//		}
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					if("DELETE".equals(actionDatabase.name())) {
						...
						yeta8060Service.delChkYeta8060(dto, actionDatabase, new AsyncCallback<Integer>() {
							@Override
							public void onSuccess(Integer result) {
								...
								if (result == 1) {
									...
									// 저장 처리
									saveYeta8060(dto, actionDatabase, fileName);
								}
							}
							...
							public void onFailure(Throwable caught) {
								...
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
										MSFMainApp.ADMINMESSAGES.ExceptionMessageService("delChkYeta8060("+ actionDatabase.name()+ ") : "+ caught),null);
							}
						});
					}else {
						// 저장 처리
	...
	public static native void closeFunction() /*-{
			   $wnd.closeFunction =
			      $entry(@com.app.exterms.yearendtax.client.form.yeta2023.Yeta8060::onCloseGwtFunction(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;));
	  }-*/;
	...
	public ContentPanel getViewPanel() {
		...
		if (panel == null) {
			...
			// 상위 사업장
			yeta8060Service.getParentDpobCd(new AsyncCallback<List<BaseModel>>() {
				@Override
			...
			vp.setSpacing(10);
			createYeta8060Form(); 		// 화면 기본정보를 설정
			 ...
			 */
			// 폼데이터를 바인딩
			formBinding = new FormBinding(this.plFrmYeta8060, true);
	...
	// 생성자 함수에 변수을 받아 처리
	public Yeta8060(String txtForm) {
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createYeta8060Form() {

		plFrmYeta8060 = new FormPanel();
		plFrmYeta8060.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 단위사업장등록관리"));
		plFrmYeta8060.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta8060.setBodyStyleName("pad-text");
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//로그 뷰화면 호출 메서드 
				funcLogMessage("단위사업장등록관리","Yeta8060");
			}
		});
		plFrmYeta8060.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta8060.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		plFrmYeta8060.setPadding(2);
		plFrmYeta8060.setFrame(true);
		plFrmYeta8060.setLayout(new FormLayout());

		degtrSecRegnNum = new HiddenField<String>();
		imgKeyCode = new HiddenField<String>();
		imgKeyCode.setName("imgKeyCode");
		plFrmYeta8060.add(imgKeyCode);
		frmKey = new HiddenField<String>();
		frmKey.setName("frmKey");
		plFrmYeta8060.add(frmKey);
		whdgTxRegrstDeptCd = new HiddenField<String>();
		whdgTxRegrstDeptCd.setName("whdgTxRegrstDeptCd");
		plFrmYeta8060.add(whdgTxRegrstDeptCd);

		topYeta8060Bar = new ButtonBar();
		topYeta8060Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		btnYeta8060Dpob = new Button("주(총괄납부)사업장관리");
		topYeta8060Bar.add(btnYeta8060Dpob);
		btnYeta8060Dpob.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				YetaP806001 yetaP806001Form = new YetaP806001(ActionDatabase.READ, getThis());

				MSFFormWindows msFwYeta806001 = new MSFFormWindows("주(총괄납부)사업장 관리",yetaP806001Form, "닫기", "820px", "710px", false);
				msFwYeta806001.show();
				yetaP806001Form.setMSFFormWindows(msFwYeta806001);
			}
		});

		btnYeta8060New = new Button("신규");
		btnYeta8060New.setIcon(MSFMainApp.ICONS.new16());
		topYeta8060Bar.add(btnYeta8060New);
		btnYeta8060New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 신규 버튼 클릭시 처리
				formYeta8060Init();
		...
		btnYeta8060Save = new Button("저장");
		btnYeta8060Save.setIcon(MSFMainApp.ICONS.save16());
		//		btnYeta8060Save.addSelectionListener(selectionListener);
		btnYeta8060Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
		...
		topYeta8060Bar.add(btnYeta8060Save);

		btnYeta8060Del = new Button("삭제");
		btnYeta8060Del.setIcon(MSFMainApp.ICONS.delete16());
		btnYeta8060Del.addSelectionListener(selectionListener);
		topYeta8060Bar.add(btnYeta8060Del);

		btnYeta8060Sreach = new Button("조회");
		btnYeta8060Sreach.setIcon(MSFMainApp.ICONS.search16());
		topYeta8060Bar.add(btnYeta8060Sreach);
		btnYeta8060Sreach.addListener(Events.Select,
		...
		plFrmYeta8060.add(topYeta8060Bar);

		// 파일업로드 처리 - 사업장 직인 정보
		plFrmYeta8060.setAction("myurl"); // url 정보 추가 할것.
		plFrmYeta8060.setEncoding(Encoding.MULTIPART);
		plFrmYeta8060.setMethod(Method.POST);

		vp.add(plFrmYeta8060);
		plFrmYeta8060.setSize("990px", "680px");
	}
	...
	private void createStandardForm() {
		...
		plFrmYeta8060.add(layoutContainer);
	...
	private LayoutContainer createMasterGrid() {
		...
		msfGridPanel01 = new MSFGridPanel(mYeta8060Def, false, false, false, false,false);
		msfGridPanel01.setHeaderVisible(false);
		msfGridPanel01.setBodyBorder(true);
		msfGridPanel01.setBorders(true);

		final Grid Yeta8060MGrid = msfGridPanel01.getMsfGrid().getGrid();
		Yeta8060MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (msfGridPanel01.getCurrentlySelectedItem() != null) { 
					formYeta8060Init();
					//선택된 값을 우측 그리드에 넣기 위한 메서드 호출 
					leftYeta8060Detail(msfGridPanel01.getCurrentlySelectedItem());
				}
	...
	private void createListGrid() {
		...
		msfGridPanel = new MSFGridPanel(Yeta8060Def, false, false, false, true, true);
		...
		plFrmYeta8060.add(FieldSetGrid);
	...
	private Yeta8060 getThis() {
		return this;
	}

	private void leftYeta8060Detail(BaseModel selRecord) {
	 ...
	 * 저장
	 */
	private void saveYeta8060(final Yeta8060DTO dto, final ActionDatabase actionDatabase, final String fileName) {
			
		yeta8060Service.saveYeta8060(dto, actionDatabase, new AsyncCallback<String>() {
			@Override
			public void onSuccess(String result) {
				...
				if ("DELETE".equals(actionDatabase.name())) {
					...
					if(!"".equals(fileName) && fileName != null){
					    ...
					    plFrmYeta8060.mask();  
					    plFrmYeta8060.setAction(GWT.getHostPageBaseURL() + "upload/ImageFileUpload.do"); 
	                    plFrmYeta8060.submit();
	                    plFrmYeta8060.onFrameLoad(); 
	                    picBoolFile = true;
						//사진 등록처리 servlet 끝 
	                    
						// reset and unmask the form
						// after file upload
	                    plFrmYeta8060.addListener(Events.Submit, new Listener<FormEvent>() {
				              public void handleEvent(FormEvent evt) {  
				                  plFrmYeta8060.unmask(); 
			...
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
						MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta8060("+ actionDatabase.name()+ ") : "+ caught),null);
			}
		});
	}
