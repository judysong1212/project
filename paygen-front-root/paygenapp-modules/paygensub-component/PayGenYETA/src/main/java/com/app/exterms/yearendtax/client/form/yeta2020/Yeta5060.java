...
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Yeta5060Def;
import com.app.exterms.yearendtax.client.service.yeta2020.Yeta5060Service;
import com.app.exterms.yearendtax.client.service.yeta2020.Yeta5060ServiceAsync;
 ...
 * =========================================================================
 **/
public class Yeta5060 extends MSFPanel {
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta5060Bar;
	private Button btnYeta5060Dpob; 
	private Button btnYeta5060New; 
	private Button btnYeta5060Save;
	private Button btnYeta5060Del;
	private Button btnYeta5060Sreach;
	// -------------- 상단 버튼 종료 --------------
...
//	private MSFComboBox<BaseModel> hhrkUntDpobCd; 					// 상위 단위사업장 코드
	...
	private CheckBoxGroup chkgrpYeta5060;   						// 체크 그룹
	...
	// -------------- 서비스 호출 시작 -------------- 
	//private InfcYeta5060ServiceAsync infcYeta5060Service = InfcYeta5060Service.Util.getInstance();
	private Yeta5060ServiceAsync yeta5060Service = Yeta5060Service.Util.getInstance();
	private PrgmComBass0300ServiceAsync sysComBass0300Service = PrgmComBass0300Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 


	// -------------- grid 시작 -------------- 
	private Yeta5060Def Yeta5060Def = new Yeta5060Def(); // 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel;


	private Yeta5060Def mYeta5060Def = new Yeta5060Def("Yeta5060_01"); // 주 그리드 테이블 컬럼 define
	...
	// private PrgmComYeta5060DTO sysComYeta5060Dto;
	// -------------- DTO 선언 종료  -------------- 

	private VerticalPanel vp;
	private FormPanel plFrmYeta5060;
	...
	private SelectionListener<ButtonEvent> selectionListener = new SelectionListener<ButtonEvent>() {
		public void componentSelected(ButtonEvent ce) {
			if (ActionDatabase.INSERT.equals(actionDatabase)) {
				actionDatabase = null;
				if (ce.getButton() == btnYeta5060Save) {
					doAction(ActionDatabase.INSERT);
				}
			} else if (ce.getButton() == btnYeta5060Save) {
				doAction(ActionDatabase.UPDATE);
				actionDatabase = null;
			} else if (ce.getButton() == btnYeta5060Del) {
	...
	public void save(final ActionDatabase actionDatabase) {
		...
		final Yeta5060DTO dto = new Yeta5060DTO();
...
//		}
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					if("DELETE".equals(actionDatabase.name())) {
						...
						yeta5060Service.delChkYeta5060(dto, actionDatabase, new AsyncCallback<Integer>() {
							@Override
							public void onSuccess(Integer result) {
								...
								if (result == 1) {
									...
									// 저장 처리
									saveYeta5060(dto, actionDatabase, fileName);
								}
							}
							...
							public void onFailure(Throwable caught) {
								...
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
										MSFMainApp.ADMINMESSAGES.ExceptionMessageService("delChkYeta5060("+ actionDatabase.name()+ ") : "+ caught),null);
							}
						});
					}else {
						// 저장 처리
	...
	public static native void closeFunction() /*-{
			   $wnd.closeFunction =
			      $entry(@com.app.exterms.yearendtax.client.form.yeta2020.Yeta5060::onCloseGwtFunction(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;));
	  }-*/;
	...
	public ContentPanel getViewPanel() {
		...
		if (panel == null) {
			...
			// 상위 사업장
			yeta5060Service.getParentDpobCd(new AsyncCallback<List<BaseModel>>() {
				@Override
			...
			vp.setSpacing(10);
			createYeta5060Form(); 		// 화면 기본정보를 설정
			 ...
			 */
			// 폼데이터를 바인딩
			formBinding = new FormBinding(this.plFrmYeta5060, true);
	...
	// 생성자 함수에 변수을 받아 처리
	public Yeta5060(String txtForm) {
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createYeta5060Form() {

		plFrmYeta5060 = new FormPanel();
		plFrmYeta5060.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 단위사업장등록관리"));
		plFrmYeta5060.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta5060.setBodyStyleName("pad-text");
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//로그 뷰화면 호출 메서드 
				funcLogMessage("단위사업장등록관리","Yeta5060");
			}
		});
		plFrmYeta5060.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta5060.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		plFrmYeta5060.setPadding(2);
		plFrmYeta5060.setFrame(true);
		plFrmYeta5060.setLayout(new FormLayout());

		degtrSecRegnNum = new HiddenField<String>();
		imgKeyCode = new HiddenField<String>();
		imgKeyCode.setName("imgKeyCode");
		plFrmYeta5060.add(imgKeyCode);
		frmKey = new HiddenField<String>();
		frmKey.setName("frmKey");
		plFrmYeta5060.add(frmKey);
		whdgTxRegrstDeptCd = new HiddenField<String>();
		whdgTxRegrstDeptCd.setName("whdgTxRegrstDeptCd");
		plFrmYeta5060.add(whdgTxRegrstDeptCd);

		topYeta5060Bar = new ButtonBar();
		topYeta5060Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		btnYeta5060Dpob = new Button("주(총괄납부)사업장관리");
		topYeta5060Bar.add(btnYeta5060Dpob);
		btnYeta5060Dpob.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				YetaP506001 yetaP506001Form = new YetaP506001(ActionDatabase.READ, getThis());

				MSFFormWindows msFwYeta506001 = new MSFFormWindows("주(총괄납부)사업장 관리",yetaP506001Form, "닫기", "820px", "710px", false);
				msFwYeta506001.show();
				yetaP506001Form.setMSFFormWindows(msFwYeta506001);
			}
		});

		btnYeta5060New = new Button("신규");
		btnYeta5060New.setIcon(MSFMainApp.ICONS.new16());
		topYeta5060Bar.add(btnYeta5060New);
		btnYeta5060New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 신규 버튼 클릭시 처리
				formYeta5060Init();
		...
		btnYeta5060Save = new Button("저장");
		btnYeta5060Save.setIcon(MSFMainApp.ICONS.save16());
		//		btnYeta5060Save.addSelectionListener(selectionListener);
		btnYeta5060Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
		...
		topYeta5060Bar.add(btnYeta5060Save);

		btnYeta5060Del = new Button("삭제");
		btnYeta5060Del.setIcon(MSFMainApp.ICONS.delete16());
		btnYeta5060Del.addSelectionListener(selectionListener);
		topYeta5060Bar.add(btnYeta5060Del);

		btnYeta5060Sreach = new Button("조회");
		btnYeta5060Sreach.setIcon(MSFMainApp.ICONS.search16());
		topYeta5060Bar.add(btnYeta5060Sreach);
		btnYeta5060Sreach.addListener(Events.Select,
		...
		plFrmYeta5060.add(topYeta5060Bar);

		// 파일업로드 처리 - 사업장 직인 정보
		plFrmYeta5060.setAction("myurl"); // url 정보 추가 할것.
		plFrmYeta5060.setEncoding(Encoding.MULTIPART);
		plFrmYeta5060.setMethod(Method.POST);

		vp.add(plFrmYeta5060);
		plFrmYeta5060.setSize("990px", "680px");
	}
	...
	private void createStandardForm() {
		...
		plFrmYeta5060.add(layoutContainer);
	...
	private LayoutContainer createMasterGrid() {
		...
		msfGridPanel01 = new MSFGridPanel(mYeta5060Def, false, false, false, false,false);
		msfGridPanel01.setHeaderVisible(false);
		msfGridPanel01.setBodyBorder(true);
		msfGridPanel01.setBorders(true);

		final Grid Yeta5060MGrid = msfGridPanel01.getMsfGrid().getGrid();
		Yeta5060MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (msfGridPanel01.getCurrentlySelectedItem() != null) { 
					formYeta5060Init();
					//선택된 값을 우측 그리드에 넣기 위한 메서드 호출 
					leftYeta5060Detail(msfGridPanel01.getCurrentlySelectedItem());
				}
	...
	private void createListGrid() {
		...
		msfGridPanel = new MSFGridPanel(Yeta5060Def, false, false, false, true, true);
		...
		plFrmYeta5060.add(FieldSetGrid);
	...
	private Yeta5060 getThis() {
		return this;
	}

	private void leftYeta5060Detail(BaseModel selRecord) {
	 ...
	 * 저장
	 */
	private void saveYeta5060(final Yeta5060DTO dto, final ActionDatabase actionDatabase, final String fileName) {
			
		yeta5060Service.saveYeta5060(dto, actionDatabase, new AsyncCallback<String>() {
			@Override
			public void onSuccess(String result) {
				...
				if ("DELETE".equals(actionDatabase.name())) {
					...
					if(!"".equals(fileName) && fileName != null){
					    ...
					    plFrmYeta5060.mask();  
					    plFrmYeta5060.setAction(GWT.getHostPageBaseURL() + "upload/ImageFileUpload.do"); 
	                    plFrmYeta5060.submit();
	                    plFrmYeta5060.onFrameLoad(); 
	                    picBoolFile = true;
						//사진 등록처리 servlet 끝 
	                    
						// reset and unmask the form
						// after file upload
	                    plFrmYeta5060.addListener(Events.Submit, new Listener<FormEvent>() {
				              public void handleEvent(FormEvent evt) {  
				                  plFrmYeta5060.unmask(); 
			...
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
						MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta5060("+ actionDatabase.name()+ ") : "+ caught),null);
			}
		});
	}
