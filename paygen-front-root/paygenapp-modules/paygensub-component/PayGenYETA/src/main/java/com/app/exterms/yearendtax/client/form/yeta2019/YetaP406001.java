 ...
 * =====================================================
 **/
public class YetaP406001 extends MSFFormPanel {
	...
	private ContentPanel cpYetaP406001;

	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP406001_01GridPanel;
	private MSFGridPanel YetaP406001_02GridPanel;

	private Yeta4060Def Yeta406001MtDef = new Yeta4060Def("Yeta406001_Mt");
	private Yeta4060Def Yeta406001StDef = new Yeta4060Def("Yeta406001_St");
	// -------------- grid 선언 종료 ---------------
	...
	//	private YetaP2060ServiceAsync YetaP406001Service = YetaP406001Service.Util.getInstance();
	 ...
	 * ++++++++++++++
	 **/
	public YetaP406001(final ActionDatabase actionDatabase,	final MSFPanel caller) {
		...
		cpYetaP406001 = new ContentPanel();

		createGridForm();

		mReload();
		cpYetaP406001.setBodyBorder(false);
		cpYetaP406001.setBorders(false);
		cpYetaP406001.setHeaderVisible(false);
		cpYetaP406001.setSize("790px", "640px");

		this.add(cpYetaP406001);
	...
	private void createGridForm() {
		...
		YetaP406001_01GridPanel = new MSFGridPanel(Yeta406001MtDef, false, false, false, false, false);
		YetaP406001_01GridPanel.setHeaderVisible(false);
		YetaP406001_01GridPanel.setSize(305, 570);
		YetaP406001_01GridPanel.setBorders(true);

		final Grid Yeta206001MGrid = YetaP406001_01GridPanel.getMsfGrid().getGrid();
		Yeta206001MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (YetaP406001_01GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = YetaP406001_01GridPanel.getCurrentlySelectedItem();
					sReload(bmData);
				}
			}
		}); 	

		fieldMtSet.add(YetaP406001_01GridPanel);
		...
		YetaP406001_02GridPanel = new MSFGridPanel(Yeta406001StDef, false, false, false, false, false);
		YetaP406001_02GridPanel.setHeaderVisible(false);
		YetaP406001_02GridPanel.setSize(460, 570);
		YetaP406001_02GridPanel.setBorders(true);
		
		fieldStSet.add(YetaP406001_02GridPanel);
		...
		btnBassSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (YetaP406001_02GridPanel.getCurrentlySelectedItem() == null) { 
		...
		cpYetaP406001.add(layoutContainer1, new FormData("100%"));
		cpYetaP406001.add(btnBar, new FormData("100%"));
	...
	public void mReload() {
		...
		YetaP406001_01GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP406001_01GridPanel.reload();
	...
	public void sReload(BaseModel bmData) {
		...
		YetaP406001_02GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP406001_02GridPanel.getTableDef().addColumnFilter("utDpobCd",MSFSharedUtils.allowNulls(bmData.get("utDpobCd")), SimpleColumnFilter.OPERATOR_LIKE);
		YetaP406001_02GridPanel.reload();
							...
							MessageBox.confirm("종사업장 등록", "선택된 사업장의 사업자번호가 주 사업자 번호로 변경됩니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
								...
								public void handleEvent(MessageBoxEvent be) {
									if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
										BaseModel bmData = YetaP406001_01GridPanel.getCurrentlySelectedItem();
										Yeta4060DTO dto = new Yeta4060DTO();
										...
										yeta4060Service.insertYetaS406001(dto, actionDatabase, new AsyncCallback<Integer>() {
											@Override
											public void onSuccess(Integer result) {	
												...
												//상세정보 초기화
												YetaP406001_01GridPanel.reload();
												YetaP406001_02GridPanel.getMsfGrid().clearData();
											}
		...
		MessageBox.confirm("사업장 삭제", "선택된 사업장을 삭제 하시겠습니까?<br>",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = YetaP406001_02GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator(); 
					List<Record>  lsRec  = new ArrayList<Record>();
					List<Yeta4060DTO> listYeta4060Dto = new ArrayList<Yeta4060DTO>();       
					...
					if (MSFSharedUtils.paramNotNull(records)) {   
						...
						while (records.hasNext()) {
							...
							listYeta4060Dto.add(dto);
						}
					}   


					if(listYeta4060Dto.size() <= 0 ) {
						MessageBox.info("경고", "삭제할 사업장을 선택해 주세요.", null);
						return;
					}

					yeta4060Service.Yeta406001_Delete(listYeta4060Dto, actionDatabase, new AsyncCallback<Long>() {
						@Override
			...
			MessageBox.confirm("주사업자 수정", "선택된 사업장의 일련번호를 변경합니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
				...
				public void handleEvent(MessageBoxEvent be) {
					if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
						...
						BaseModel bmData1 = YetaP406001_01GridPanel.getCurrentlySelectedItem();
						...
						yeta4060Service.Yeta406001NumUpdate(listYeta4060Dto, actionDatabase, new AsyncCallback<Long>() {
							@Override
							public void onSuccess(Long result) {	
								MessageBox.info("","저장 되었습니다.", null);
