 ...
 * =====================================================
 **/
public class YetaP806001 extends MSFFormPanel {
	...
	private ContentPanel cpYetaP806001;

	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP806001_01GridPanel;
	private MSFGridPanel YetaP806001_02GridPanel;

	private Yeta8060Def Yeta806001MtDef = new Yeta8060Def("Yeta806001_Mt");
	private Yeta8060Def Yeta806001StDef = new Yeta8060Def("Yeta806001_St");
	// -------------- grid 선언 종료 ---------------
	...
	//	private YetaP2060ServiceAsync YetaP806001Service = YetaP806001Service.Util.getInstance();
	 ...
	 * ++++++++++++++
	 **/
	public YetaP806001(final ActionDatabase actionDatabase,	final MSFPanel caller) {
		...
		cpYetaP806001 = new ContentPanel();

		createGridForm();

		mReload();
		cpYetaP806001.setBodyBorder(false);
		cpYetaP806001.setBorders(false);
		cpYetaP806001.setHeaderVisible(false);
		cpYetaP806001.setSize("790px", "640px");

		this.add(cpYetaP806001);
	...
	private void createGridForm() {
		...
		YetaP806001_01GridPanel = new MSFGridPanel(Yeta806001MtDef, false, false, false, false, false);
		YetaP806001_01GridPanel.setHeaderVisible(false);
		YetaP806001_01GridPanel.setSize(305, 570);
		YetaP806001_01GridPanel.setBorders(true);

		final Grid Yeta206001MGrid = YetaP806001_01GridPanel.getMsfGrid().getGrid();
		Yeta206001MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (YetaP806001_01GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = YetaP806001_01GridPanel.getCurrentlySelectedItem();
					sReload(bmData);
				}
			}
		}); 	

		fieldMtSet.add(YetaP806001_01GridPanel);
		...
		YetaP806001_02GridPanel = new MSFGridPanel(Yeta806001StDef, false, false, false, false, false);
		YetaP806001_02GridPanel.setHeaderVisible(false);
		YetaP806001_02GridPanel.setSize(460, 570);
		YetaP806001_02GridPanel.setBorders(true);
		
		fieldStSet.add(YetaP806001_02GridPanel);
		...
		btnBassSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (YetaP806001_02GridPanel.getCurrentlySelectedItem() == null) { 
		...
		cpYetaP806001.add(layoutContainer1, new FormData("100%"));
		cpYetaP806001.add(btnBar, new FormData("100%"));
	...
	public void mReload() {
		...
		YetaP806001_01GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP806001_01GridPanel.reload();
	...
	public void sReload(BaseModel bmData) {
		...
		YetaP806001_02GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP806001_02GridPanel.getTableDef().addColumnFilter("utDpobCd",MSFSharedUtils.allowNulls(bmData.get("utDpobCd")), SimpleColumnFilter.OPERATOR_LIKE);
		YetaP806001_02GridPanel.reload();
							...
							MessageBox.confirm("종사업장 등록", "선택된 사업장의 사업자번호가 주 사업자 번호로 변경됩니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
								...
								public void handleEvent(MessageBoxEvent be) {
									if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
										BaseModel bmData = YetaP806001_01GridPanel.getCurrentlySelectedItem();
										Yeta8060DTO dto = new Yeta8060DTO();
										...
										yeta8060Service.insertYetaS806001(dto, actionDatabase, new AsyncCallback<Integer>() {
											@Override
											public void onSuccess(Integer result) {	
												...
												//상세정보 초기화
												YetaP806001_01GridPanel.reload();
												YetaP806001_02GridPanel.getMsfGrid().clearData();
											}
		...
		MessageBox.confirm("사업장 삭제", "선택된 사업장을 삭제 하시겠습니까?<br>",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = YetaP806001_02GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator(); 
					List<Record>  lsRec  = new ArrayList<Record>();
					List<Yeta8060DTO> listYeta8060Dto = new ArrayList<Yeta8060DTO>();       
					...
					if (MSFSharedUtils.paramNotNull(records)) {   
						...
						while (records.hasNext()) {
							...
							listYeta8060Dto.add(dto);
						}
					}   


					if(listYeta8060Dto.size() <= 0 ) {
						MessageBox.info("경고", "삭제할 사업장을 선택해 주세요.", null);
						return;
					}

					yeta8060Service.Yeta806001_Delete(listYeta8060Dto, actionDatabase, new AsyncCallback<Long>() {
						@Override
			...
			MessageBox.confirm("주사업자 수정", "선택된 사업장의 일련번호를 변경합니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
				...
				public void handleEvent(MessageBoxEvent be) {
					if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
						...
						BaseModel bmData1 = YetaP806001_01GridPanel.getCurrentlySelectedItem();
						...
						yeta8060Service.Yeta806001NumUpdate(listYeta8060Dto, actionDatabase, new AsyncCallback<Long>() {
							@Override
							public void onSuccess(Long result) {	
								MessageBox.info("","저장 되었습니다.", null);
