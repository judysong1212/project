...
import com.app.exterms.yearendtax.client.form.defs.yeta2018.Ye162010Def;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33013Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33013ServiceAsync;
...
=====================================================**/
public  class YetaT33013   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33013; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33013GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT33013");   
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33013ServiceAsync yetaT33013Service = YetaT33013Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33013Service.activityOnYetaT33013(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT33013(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33013 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
	 	cpYetaT33013.setBodyBorder(false);
	 	cpYetaT33013.setBorders(false);
	 	cpYetaT33013.setHeaderVisible(false);
	 	cpYetaT33013.setSize("790px", "640px");
		   
		this.add(cpYetaT33013);
...
//		     });     
		...
		cpYetaT33013.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT33013GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT33013GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT33013GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT33013GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT33013GridPanel.getMsfGrid().getGrid().startEditing(YetaT33013GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT33013GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaT33013GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//		cpYetaT33013.add(topYetaBtnBar);
...
//		cpYetaT33013.add(topYetaBtnBar);
		...
		YetaT33013GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT33013GridPanel.setSize("790px", "200px");
		YetaT33013GridPanel.setBorders(true);
		YetaT33013GridPanel.setHeaderVisible(false);
...
//		
//		YetaT33013GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT33013GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT33013.add(YetaT33013GridPanel);
		...
		fldstNewFieldset.add(YetaT33013GridPanel);
		cpYetaT33013.add(fldstNewFieldset);
		cpYetaT33013.add(layoutContainer10);
		cpYetaT33013.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33013GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33013GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33013GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33013GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33013GridPanel.reload();
