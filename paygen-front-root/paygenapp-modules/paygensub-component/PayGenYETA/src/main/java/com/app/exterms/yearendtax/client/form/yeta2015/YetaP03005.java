...
import com.app.exterms.yearendtax.client.form.defs.yeta2015.Yeta3130Def;
import com.app.exterms.yearendtax.client.service.yeta2015.YetaP03005Service;
import com.app.exterms.yearendtax.client.service.yeta2015.YetaP03005ServiceAsync;
...
=====================================================**/
public  class YetaP03005   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta03005; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP03005GridPanel;
//	private Yeta0300Def yeta0300Def_010  = new Yeta0300Def("YETA030010");	//가족교육비 그리드 테이블 컬럼 define 
	private Yeta3130Def yetaP03004ToYeta3130Def  = new Yeta3130Def("YETAP03005");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP03005ServiceAsync yetaP03005Service = YetaP03005Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
 ...
 **/ 
     ...
     */ 
		...
		private void yetaP03005FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
		              
				List<Yeta3130DTO> listYeta3130dto = new ArrayList<Yeta3130DTO>();  
...
//					yeta3130Dto.setSettGbcd(MSFSharedUtils.allowNulls(bmMapModel.get("settGbcd$commCd")));  /** column 정산구분코드 : settGbcd */
				...
				yetaP03005Service.activityOnYetaP03005ToYeta3130(listYeta3130dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("yetaP03005FormSave(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	...
	public YetaP03005(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYeta03005 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYeta03005.setBodyBorder(false);
		cpYeta03005.setBorders(false);
		cpYeta03005.setHeaderVisible(false);
		cpYeta03005.setSize("790px", "640px");
		   
		this.add(cpYeta03005);
...
//		     });     
		...
		cpYeta03005.setLayout(new FlowLayout());
...
//				systemkey.setValue("20080013");
				...
				yetaP03005GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = yetaP03005GridPanel.getTableDef().getDefaultValues();  
...
//		         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
		        ...
		        yetaP03005GridPanel.getMsfGrid().getGrid().stopEditing();      
		        yetaP03005GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        yetaP03005GridPanel.getMsfGrid().getGrid().startEditing(yetaP03005GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = yetaP03005GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP03005GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//		cpYeta03005.add(topYetaBtnBar);
		...
		yetaP03005GridPanel = new MSFGridPanel(yetaP03004ToYeta3130Def, false, false, false, false, false);
		yetaP03005GridPanel.setSize("790px", "420px");
		yetaP03005GridPanel.setBorders(true);
		yetaP03005GridPanel.setHeaderVisible(false);
		
		
		/** --- 그리드 합계 부분  시작 --- **/
//		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		 
//		payrSummaryRow.setCellStyle(Yeta3130BM.ATTR_MNRNAMNT,"summary_color");	 /** column 월세_연간월세금액 : mnrnAmnt */
//							    
//		payrSummaryRow.setSummaryType(Yeta3130BM.ATTR_MNRNAMNT, SummaryType.SUM);   
//		payrSummaryRow.setSummaryFormat(Yeta3130BM.ATTR_MNRNAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
//		
//		yetaP03005GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
		
			
		cpYeta03005.add(yetaP03005GridPanel);
	}	
	...
	public void reload() {
			...
			yetaP03005GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaP03005GridPanel.getTableDef().addColumnFilter("edacRvyy", MSFSharedUtils.allowNulls(srhEdacRvyy.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaP03005GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaP03005GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaP03005GridPanel.reload();
