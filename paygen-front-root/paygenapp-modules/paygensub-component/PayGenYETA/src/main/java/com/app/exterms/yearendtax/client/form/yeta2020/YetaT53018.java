...
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53018Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53018ServiceAsync;
...
=====================================================**/
public  class YetaT53018   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT53018; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT53018GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT53018ServiceAsync yetaT53018Service = YetaT53018Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT53018Service.activityOnYetaT53018(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT53018(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT53018 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT53018.setBodyBorder(false);
		cpYetaT53018.setBorders(false);
		cpYetaT53018.setHeaderVisible(false);
		cpYetaT53018.setSize("790px", "640px");
		   
		this.add(cpYetaT53018);
...
//		     });     
		...
		cpYetaT53018.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT53018GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT53018GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT53018GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT53018GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT53018GridPanel.getMsfGrid().getGrid().startEditing(YetaT53018GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT53018GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//				
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT53018GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//				
		cpYetaT53018.add(topYetaBtnBar);
...
//		YetaT53018GridPanel = new MSFGridPanel(YetaT53018Def, false, false, false, false, false);
//		YetaT53018GridPanel.setSize("790px", "420px");
//		YetaT53018GridPanel.setBorders(true);
//		YetaT53018GridPanel.setHeaderVisible(false);
...
//		
//		YetaT53018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT53018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT53018.add(YetaT53018GridPanel);
		...
		cpYetaT53018.add(layoutContainer);
	...
	public void reload() {
			...
			YetaT53018GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT53018GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT53018GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT53018GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT53018GridPanel.reload();
