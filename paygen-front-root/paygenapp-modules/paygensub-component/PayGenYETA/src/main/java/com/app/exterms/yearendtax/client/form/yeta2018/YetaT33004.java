...
=====================================================**/
public  class YetaT33004   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33004; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33004GridPanel;
	private MSFGridPanel ye162530GridPanel; //임대차계약그리드
	private MSFGridPanel ye162520GridPanel; // 금전소비대차그리드
	private Ye162520Def ye162520Def = new Ye162520Def("YetaT33004");   
	private Ye162530Def ye162530Def = new Ye162530Def("YetaT33004");   
...
//		 }
	...
	public YetaT33004(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33004 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33004.setBodyBorder(false);
		cpYetaT33004.setBorders(false);
		cpYetaT33004.setHeaderVisible(false);
		cpYetaT33004.setSize("790px", "640px");
		   
		this.add(cpYetaT33004);
...
//		     });     
		...
		cpYetaT33004.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT33004GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT33004GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT33004GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT33004GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT33004GridPanel.getMsfGrid().getGrid().startEditing(YetaT33004GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT33004GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaT33004GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//				
//		cpYetaT33004.add(topYetaBtnBar);
...
//		
//		YetaT33004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT33004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT33004.add(YetaT33004GridPanel);
		...
		cpYetaT33004.add(fldstNewFieldset);
		cpYetaT33004.add(fldstNewFieldset1);
		cpYetaT33004.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33004GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33004GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33004GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33004GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33004GridPanel.reload();
