...
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye162010BM;
...
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73007ServiceAsync;
...
=====================================================**/
public  class YetaP73007   extends MSFFormPanel {  
 ...
 **/
	...
	private ContentPanel cpYeta73007;
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP73007GridPanel;
//	private Ye16Ta3100Def YetaP73007ToYe162010Def  = new Ye16Ta3100Def("YETAP13007");	//종전근무지내역 그리드 테이블 컬럼 define 
	private Ye162010Def ye162010Def  = new Ye162010Def("YetaP73007");	//종전근무지내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP73007ServiceAsync yetaP73007Service = YetaP73007Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------		
     ...
     */  
		...
		private void YetaP73007FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				if(dataChk) {
					...
					yetaP73007Service.activityOnYetaP73007ToYe162010(listYe162010Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
		 ...
		 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 **/		  
	  public YetaP73007(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			initLoad(); 
	       ...
	       // this.setLayout(layout);   
	        ...
	        cpYeta73007 = new ContentPanel();  
	        
	        createSearchForm();			//검색필드를 적용
	        createMsfGridForm();		//그리드필드
		   
	           
		   cpYeta73007.setBodyBorder(false);
		   cpYeta73007.setBorders(false);
		   cpYeta73007.setHeaderVisible(false);
		   cpYeta73007.setSize("790px", "530px");
		   
		   this.add(cpYeta73007);
...
//			     });     
			...
			cpYeta73007.setLayout(new FlowLayout());
			...
			btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 ...
					 YetaP73007GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = YetaP73007GridPanel.getTableDef().getDefaultValues();  
			         ...
			         YetaP73007GridPanel.getMsfGrid().getGrid().stopEditing();      
			         YetaP73007GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         YetaP73007GridPanel.getMsfGrid().getGrid().startEditing(YetaP73007GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
					...
					MessageBox.confirm("삭제", "선택하신 연금저축 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
						...
						public void handleEvent(MessageBoxEvent be) {
							if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
								...
								Iterator<BaseModel> itBm = YetaP73007GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					...
					setListRecord(YetaP73007GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
					doAction(actionDatabase);
...
//	            	reload();
		...
		private void createMsfGridForm() {
			...
			YetaP73007GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
//			YetaP73007GridPanel.setSize("790px", "300px");
			YetaP73007GridPanel.setBorders(false);
			YetaP73007GridPanel.setHeaderVisible(false);
			...
			YetaP73007GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			/** --- 그리드 합계 부분  시작 --- **/
			
			cpYeta73007.add(YetaP73007GridPanel);
...
//			layoutContainer.setBorders(false);
			...
			cp01.add(YetaP73007GridPanel);
			
			cpYeta73007.add(cp01);
			
			
//			cpYeta73007.add(layoutContainer);
	}
	...
	public void reload() {
		...
		YetaP73007GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP73007GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP73007GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
															, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP73007GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP73007GridPanel.reload();
