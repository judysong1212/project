...
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63005Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63005ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT63005Def;
...
=====================================================**/
public  class YetaT63005   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63005; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63005GridPanel;
//	private YetaT63005Def YetaT63005Def = new YetaT63005Def("YetaT63005");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63005ServiceAsync yetaT63005Service = YetaT63005Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63005Service.activityOnYetaT63005(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63005(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63005 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63005.setBodyBorder(false);
		cpYetaT63005.setBorders(false);
		cpYetaT63005.setHeaderVisible(false);
		cpYetaT63005.setSize("790px", "640px");
		   
		this.add(cpYetaT63005);
...
//		     });     
		...
		cpYetaT63005.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63005GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63005GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63005GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63005GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63005GridPanel.getMsfGrid().getGrid().startEditing(YetaT63005GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT63005GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT63005GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT63005.add(topYetaBtnBar);
...
//		YetaT63005GridPanel = new MSFGridPanel(YetaT63005Def, false, false, false, false, false);
//		YetaT63005GridPanel.setSize("790px", "420px");
//		YetaT63005GridPanel.setBorders(true);
//		YetaT63005GridPanel.setHeaderVisible(false);
//		
...
//		
//		YetaT63005GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63005GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//      cpYetaT63005.add(YetaT63005GridPanel);
		...
		cpYetaT63005.add(fldstNewFieldset);
		cpYetaT63005.add(layoutContainer1_1);
		cpYetaT63005.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63005GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63005GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63005GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63005GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63005GridPanel.reload();
