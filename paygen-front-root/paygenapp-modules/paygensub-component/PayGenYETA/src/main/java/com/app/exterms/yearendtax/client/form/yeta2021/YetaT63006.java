...
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63006Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63006ServiceAsync;
...
=====================================================**/
public  class YetaT63006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63006GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT63006");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63006ServiceAsync yetaT63006Service = YetaT63006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63006Service.activityOnYetaT63006(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63006 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63006.setBodyBorder(false);
		cpYetaT63006.setBorders(false);
		cpYetaT63006.setHeaderVisible(false);
		cpYetaT63006.setSize("790px", "640px");
		   
		this.add(cpYetaT63006);
...
//		     });     
		...
		cpYetaT63006.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63006GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63006GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63006GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63006GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63006GridPanel.getMsfGrid().getGrid().startEditing(YetaT63006GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT63006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT63006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT63006.add(topYetaBtnBar);
...
//		cpYetaT63006.add(topYetaBtnBar);
		...
		YetaT63006GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT63006GridPanel.setSize("790px", "200px");
		YetaT63006GridPanel.setBorders(true);
		YetaT63006GridPanel.setHeaderVisible(false);
...
//		
//		YetaT63006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63006.add(YetaT63006GridPanel);
		...
		fldstNewFieldset.add(YetaT63006GridPanel);
		cpYetaT63006.add(fldstNewFieldset);
		cpYetaT63006.add(layoutContainer10);
		cpYetaT63006.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63006GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63006GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63006GridPanel.reload();
