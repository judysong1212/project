...
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33018Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33018ServiceAsync;
...
=====================================================**/
public  class YetaT43018   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33018; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33018GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33018ServiceAsync yetaT33018Service = YetaT33018Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33018Service.activityOnYetaT33018(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT43018(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33018 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33018.setBodyBorder(false);
		cpYetaT33018.setBorders(false);
		cpYetaT33018.setHeaderVisible(false);
		cpYetaT33018.setSize("790px", "640px");
		   
		this.add(cpYetaT33018);
...
//		     });     
		...
		cpYetaT33018.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT33018GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT33018GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT33018GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT33018GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT33018GridPanel.getMsfGrid().getGrid().startEditing(YetaT33018GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT33018GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//				
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT33018GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//				
		cpYetaT33018.add(topYetaBtnBar);
...
//		YetaT33018GridPanel = new MSFGridPanel(YetaT33018Def, false, false, false, false, false);
//		YetaT33018GridPanel.setSize("790px", "420px");
//		YetaT33018GridPanel.setBorders(true);
//		YetaT33018GridPanel.setHeaderVisible(false);
...
//		
//		YetaT33018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT33018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT33018.add(YetaT33018GridPanel);
		...
		cpYetaT33018.add(layoutContainer);
	...
	public void reload() {
			...
			YetaT33018GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33018GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33018GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33018GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33018GridPanel.reload();
