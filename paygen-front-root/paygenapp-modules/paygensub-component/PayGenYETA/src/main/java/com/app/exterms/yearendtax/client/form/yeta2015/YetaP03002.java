...
import com.app.exterms.yearendtax.client.form.defs.yeta2015.Yeta3140Def;
import com.app.exterms.yearendtax.client.service.yeta2015.YetaP03002Service;
import com.app.exterms.yearendtax.client.service.yeta2015.YetaP03002ServiceAsync;
...
=====================================================**/
public  class YetaP03002   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta03002; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP03002GridPanel;
//	private Yeta0300Def yeta0300Def_07  = new Yeta0300Def("YETA03007");	//의료비내역 그리드 테이블 컬럼 define 
	private Yeta3140Def yetaP03002ToYeta3140Def  = new Yeta3140Def("YETAP03002");//의료비내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
 ...
 **/ 
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP03002ServiceAsync yetaP03002Service = YetaP03002Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 	
     ...
     */  
		...
		private void yetaP03002FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
	              
				List<Yeta3140DTO> listYeta3140Dto = new ArrayList<Yeta3140DTO>();  
				...
				while (records.hasNext()) {
	                ...
	                Yeta3140DTO yeta3140Dto= new Yeta3140DTO();  
...
//	                bmData.set("ctifTrea", false);
				...
				yetaP03002Service.activityOnYetaP03002ToYeta3140(listYeta3140Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP03002ToYeta3140(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	  ...
	  public YetaP03002(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			initLoad();		    
		   ...
		   cpYeta03002.setBodyBorder(false);
		   cpYeta03002.setBorders(false);
		   cpYeta03002.setHeaderVisible(false);
		   cpYeta03002.setSize("790px", "640px");
		   
		   this.add(cpYeta03002);
...
//			     });     
			...
			cpYeta03002.setLayout(new FlowLayout());
...
//					systemkey.setValue("20080013");
					...
					yetaP03002GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = yetaP03002GridPanel.getTableDef().getDefaultValues();  
...
//			         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
			         ...
			         yetaP03002GridPanel.getMsfGrid().getGrid().stopEditing();      
			         yetaP03002GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         yetaP03002GridPanel.getMsfGrid().getGrid().startEditing(yetaP03002GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//					doAction(actionDatabase);
			...
			btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					Iterator<BaseModel> itBm = yetaP03002GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					setListRecord(yetaP03002GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//			BaseModel bmData = yetaP03002GridPanel.getMsfGrid().getStore().getModels().get(0);  
//			if (MSFSharedUtils.paramNotNull(bmData)){
				// TODO Auto-generated method stub
				IColumnFilter filters = null;
				
				yetaP03002GridPanel.getTableDef().setTableColumnFilters(filters);
				yetaP03002GridPanel.getTableDef().addColumnFilter("edacRvyy", MSFSharedUtils.allowNulls(srhEdacRvyy.getValue())
	...
	//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
				yetaP03002GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd")
						, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
				yetaP03002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
						, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
				yetaP03002GridPanel.reload();
...
//			cp01.setSize(790, 390);
		    ...
		    //연말정산대상자 및 내역
			yetaP03002GridPanel = new MSFGridPanel(yetaP03002ToYeta3140Def, false, false, false, false, false);
			yetaP03002GridPanel.setSize("790px", "390px");
			yetaP03002GridPanel.setBorders(false);
			yetaP03002GridPanel.setBodyBorder(false);
			yetaP03002GridPanel.setHeaderVisible(false); 
			yetaP03002GridPanel.getBottomComponent().setVisible(false);
			
			
			
//			AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//			 
//			payrSummaryRow.setCellStyle(Yeta3140BM.ATTR_CASHUECT,"summary_color");	/** set 지급건수 : cashUect */
//			payrSummaryRow.setCellStyle(Yeta3140BM.ATTR_CASHUEAM,"summary_color");	/** set 지급금액 : cashUeam */
//			
//								    
//			payrSummaryRow.setSummaryType(Yeta3140BM.ATTR_CASHUECT, SummaryType.SUM);   
//			payrSummaryRow.setSummaryFormat(Yeta3140BM.ATTR_CASHUECT, NumberFormat.getFormat("#,##0;(#,##0)"));  
//								    
//			payrSummaryRow.setSummaryType(Yeta3140BM.ATTR_CASHUEAM, SummaryType.SUM);   
//			payrSummaryRow.setSummaryFormat(Yeta3140BM.ATTR_CASHUEAM, NumberFormat.getFormat("#,##0;(#,##0)"));  
//								      
//			
//			yetaP03002GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			
			
			gridFieldSet.add(yetaP03002GridPanel);
//			
//			cp01.add(gridFieldSet);
//			
//			cpYeta03002.add(cp01);
			
			cpYeta03002.add(gridFieldSet);
			
			//초기 조회
//			reloadYetaP03002Grid();
		}
