...
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4200Service;
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4200ServiceAsync;
 ...
 * </pre>
 */
public class Yeta4200 extends MSFPanel {
	...
	private FormPanel plFrmYeta4200;
	private String txtForm = "";


	private MSFGridPanel Yeta4200GridPanel;
	private MSFGridPanel Yeta4200XmlGridPanel;
	private Ye161010Def Yeta4200def = new Ye161010Def("Yeta4200");
	private Ye161010Def Yeta4200defXml = new Ye161010Def("Yeta4200_RESULT");
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta4200Bar;
	private Button btnYeta4200Sec; 	// 초기화
	private Button btnYeta4200Reset; 	// 초기화
	private Button btnYeta4200New; 		// 신규
	private Button btnYeta4200Save; 	// 저장
	private Button btnYeta4200Del; 		// 삭제
	private Button btnYeta4200Sreach; 	// 조회
	private Button btnYeta4200Print; 	// 인쇄
	private Button btnYetaP420001; 	// 상세정보 팝업
	private Button btnYetaP420002; 	// 상세정보 팝업
	 ...
	 * ++++++++++++++++++++++
	 **/
	private Yeta4200ServiceAsync yeta4200Service = Yeta4200Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2019")));
	...
	public Yeta4200(String txtForm) {
	...
	private void createYeta0200Form() {
		...
		plFrmYeta4200 = new FormPanel();
		plFrmYeta4200.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산파일업로드"));
		plFrmYeta4200.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta4200.setPadding(2);
		plFrmYeta4200.setFrame(true);
		...
		plFrmYeta4200.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta4200.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		//멀티콤보박스 닫기 
		plFrmYeta4200.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar **/
		topYeta4200Bar = new ButtonBar();
		topYeta4200Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		if ("admin".equals(MSFMainApp.get().getUser().getUsrId())) {	
			btnYeta4200Sec = new Button("암호화패치");
			btnYeta4200Sec.setIcon(MSFMainApp.ICONS.new16());
			topYeta4200Bar.add(btnYeta4200Sec);
			btnYeta4200Sec.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
				 
					Ye161010DTO ye161010Dto = new Ye161010DTO(); 
						   
					yeta4200Service.yetaChangeConvert(ye161010Dto,  new AsyncCallback<String>(){
				                 public void onFailure(Throwable caught) {
				                     MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
				                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta4200Service : " + caught), null);
				                 }
		...
		btnYeta4200Reset = new Button("초기화");
		btnYeta4200Reset.setIcon(MSFMainApp.ICONS.new16());
		topYeta4200Bar.add(btnYeta4200Reset);
		btnYeta4200Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//초기화 
				formInit();
				Yeta4200GridPanel.getMsfGrid().clearData(); 
			}
		});  
		...
		btnYeta4200Sreach = new Button("조 회");
		btnYeta4200Sreach.setIcon(MSFMainApp.ICONS.search());
		topYeta4200Bar.add(btnYeta4200Sreach);
		btnYeta4200Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//조회버튼 클릭시 처리 
				reloadYeta4200();  
			}
		...
		plFrmYeta4200.add(topYeta4200Bar);

		// 파일업로드 처리
		plFrmYeta4200.setAction("myurl");
		plFrmYeta4200.setEncoding(Encoding.MULTIPART);
		plFrmYeta4200.setMethod(Method.POST);

		vp.add(plFrmYeta4200);
		plFrmYeta4200.setSize("990px", "830px");
		plFrmYeta4200.setLayout(new FlowLayout());
	...
	private void createMsfGridForm() {
		...
		Yeta4200GridPanel = new MSFGridPanel(Yeta4200def, false, false, false, false,false);
		Yeta4200GridPanel.setHeaderVisible(false);
		Yeta4200GridPanel.setSize(470, 640);
		Yeta4200GridPanel.setBorders(true);
		final Grid yeta4000DefGrid = Yeta4200GridPanel.getMsfGrid().getGrid();
		yeta4000DefGrid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (Yeta4200GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = new BaseModel();
					formInit() ;
					bmData = Yeta4200GridPanel.getCurrentlySelectedItem();
		...
		Yeta4200GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 4, new HeaderGroupConfig("소득공제자료", 1, 2));

		fsGird.add(Yeta4200GridPanel);
		...
		Yeta4200XmlGridPanel = new MSFGridPanel(Yeta4200defXml, false, false,false, false);
		Yeta4200XmlGridPanel.setSize(250, 305);
		Yeta4200XmlGridPanel.setBorders(true);
		Yeta4200XmlGridPanel.setStyleAttribute("marginTop", "5px");
		layoutContainer17.add(Yeta4200XmlGridPanel,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.8));

		btnYetaP420001 = new Button("개별간소화파일");
		btnYetaP420001.setStyleAttribute("marginTop", "5px");
		btnYetaP420001.setStyleAttribute("marginLeft", "5px");
		btnYetaP420001.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					...
					final MSFFormWindows msFwYeta420001 = new MSFFormWindows("자료추출상세내역",yetaP420001Form, "닫기", "920px", "710px", false);
					msFwYeta420001.show();
					yetaP420001Form.setMSFFormWindows(msFwYeta420001);

					//					funcSetPopUpRecord();

					yetaP420001Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP420001,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));

		btnYetaP420002 = new Button("일괄간소화파일");
		btnYetaP420002.setStyleAttribute("marginTop", "5px");
		btnYetaP420002.setStyleAttribute("marginLeft", "5px");
		btnYetaP420002.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					Window.alert("정산대상자를 선택해주세요.");
				}
					...
					yetaP420002Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP420002,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));

		layoutContainer1_4.add(layoutContainer17, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.495));

		plFrmYeta4200.add(layoutContainer1_4);
	}
	...
	private Yeta4200 getThis(){
		return this;
	}

	private void reloadYeta4200(){

		IColumnFilter filters = null;
		Yeta4200GridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta4200GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 

		//정산구분
		Yeta4200GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta4200GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta4200GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd,srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta4200GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta4200GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta4200GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta4200GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);


		Yeta4200GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta4200GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
		Yeta4200GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   

		Yeta4200GridPanel.reload();  

	}

	private void reloadYeta4200Result(){

		IColumnFilter filters = null;
		Yeta4200XmlGridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta4200XmlGridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 
		//정산구분
		Yeta4200XmlGridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta4200XmlGridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta4200XmlGridPanel.reload();  
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//submit the form
							plFrmYeta4200.mask("PDF 전자문서 업데이트 중입니다. 업로드 중..."); 

							//파일업로드 처리
							plFrmYeta4200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta4200Import.do");
							plFrmYeta4200.setEncoding(Encoding.MULTIPART);
							plFrmYeta4200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta4200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
							plFrmYeta4200.submit();  
							plFrmYeta4200.onFrameLoad();  
							picBoolFile = true;
						...
						// after file upload 
						plFrmYeta4200.addListener(Events.Submit, new Listener<FormEvent>() {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//파일업로드 처리
							plFrmYeta4200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta4200DdcImport.do");
							plFrmYeta4200.setEncoding(Encoding.MULTIPART);
							plFrmYeta4200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta4200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
