...
import com.app.exterms.yearendtax.client.dto.yeta2023.Ye165010BM;
import com.app.exterms.yearendtax.client.dto.yeta2023.Ye165010DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2023.Ye165010Def;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP83003Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP83003ServiceAsync;
...
=====================================================**/
public  class YetaP83003   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta83003; 
	...
	*#######################################################*/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP83003GridPanel;
//	private Yeta0300Def yeta0300Def_08  = new Yeta0300Def("YETA03008");	//가족교육비 그리드 테이블 컬럼 define 
//	private Ye16Ta3180Def YetaP83003ToYe165010  = new Ye16Ta3180Def("YetaP83003");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye165010Def YetaP83003ToYe165010  = new Ye165010Def("YetaP83003");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP83003ServiceAsync yetaP83003Service = YetaP83003Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 	
	...
	private void YetaP83003FormSave() {
...
//				}
			...
			if (dataChk) {
				...
				yetaP83003Service.activityOnYetaP83003ToYe165010(listYe165010Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP83003ToYe165010(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	...
	public YetaP83003(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		this.actionDatabase = actionDatabase;
		...
		cpYeta83003 = new ContentPanel();

		createSearchForm(); // 기본정보 필드를 적용
		createMsfGridForm(); // 그리드필드
		createStandardForm(); // 기본정보필드

		cpYeta83003.setBodyBorder(false);
		cpYeta83003.setBorders(false);
		cpYeta83003.setHeaderVisible(false);
		cpYeta83003.setSize("790px", "490px");

		this.add(cpYeta83003);
	...
	private void createSearchForm() {
		...
		cpYeta83003.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP83003GridPanel.getMsfGrid().clearData();
					 
				BaseModel bmData = YetaP83003GridPanel.getTableDef().getDefaultValues();  
				...
				YetaP83003GridPanel.getMsfGrid().getGrid().stopEditing();
				YetaP83003GridPanel.getMsfGrid().getGrid().getStore().insert(bmData, 0);
				YetaP83003GridPanel.getMsfGrid().getGrid().startEditing(YetaP83003GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);
...
//				actionDatabase = ActionDatabase.INSERT;
				...
				MessageBox.confirm("삭제", "선택하신 기부금 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
							...
							Iterator<BaseModel> itBm = YetaP83003GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP83003GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	      layoutContainer_2.setBorders(false);
	...
	private void createMsfGridForm() {
		...
		YetaP83003GridPanel = new MSFGridPanel(YetaP83003ToYe165010, false, false, false, false, false);
		YetaP83003GridPanel.setHeaderVisible(false);  
		YetaP83003GridPanel.setBorders(false);
		YetaP83003GridPanel.setHeaderVisible(false);
		YetaP83003GridPanel.getBottomComponent().setVisible(true);
		...
		YetaP83003GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			

		cp01.add(YetaP83003GridPanel);

		cpYeta83003.add(cp01);
	...
	public void reload() {
		...
		YetaP83003GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP83003GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP83003GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP83003GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP83003GridPanel.reload();
