...
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63008Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63008ServiceAsync;
...
=====================================================**/
public  class YetaT63008   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63008; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63008GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63008ServiceAsync yetaT63008Service = YetaT63008Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63008Service.activityOnYetaT63008(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63008(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63008 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63008.setBodyBorder(false);
		cpYetaT63008.setBorders(false);
		cpYetaT63008.setHeaderVisible(false);
		cpYetaT63008.setSize("790px", "640px");
		   
		this.add(cpYetaT63008);
...
//		     });     
		...
		cpYetaT63008.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63008GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63008GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63008GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63008GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63008GridPanel.getMsfGrid().getGrid().startEditing(YetaT63008GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT63008GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT63008GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT63008.add(topYetaBtnBar);
...
//		cpYetaT63008.add(topYetaBtnBar);
//		
//		YetaT63008GridPanel = new MSFGridPanel(YetaT63008Def, false, false, false, false, false);
//		YetaT63008GridPanel.setSize("790px", "420px");
//		YetaT63008GridPanel.setBorders(true);
//		YetaT63008GridPanel.setHeaderVisible(false);
//		
...
//		
//		YetaT63008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63008.add(YetaT63008GridPanel);
		...
		cpYetaT63008.add(fldstNewFieldset);
		cpYetaT63008.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63008GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63008GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63008GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63008GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63008GridPanel.reload();
