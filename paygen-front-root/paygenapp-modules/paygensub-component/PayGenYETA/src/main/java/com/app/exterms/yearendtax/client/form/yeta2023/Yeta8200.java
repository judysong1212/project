...
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8200ServiceAsync;
 ...
 * </pre>
 */
public class Yeta8200 extends MSFPanel {
	...
	private FormPanel plFrmYeta8200;
	private String txtForm = "";


	private MSFGridPanel Yeta8200GridPanel;
	private MSFGridPanel Yeta8200XmlGridPanel;
	private Ye161010Def Yeta8200def = new Ye161010Def("Yeta8200");
	private Ye161010Def Yeta8200defXml = new Ye161010Def("Yeta8200_RESULT");
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta8200Bar;
	private Button btnYeta8200Sec; 	// 초기화
	private Button btnYeta8200Reset; 	// 초기화
	private Button btnYeta8200New; 		// 신규
	private Button btnYeta8200Save; 	// 저장
	private Button btnYeta8200Del; 		// 삭제
	private Button btnYeta8200Sreach; 	// 조회
	private Button btnYeta8200Print; 	// 인쇄
	private Button btnYetaP820001; 	// 상세정보 팝업
	private Button btnYetaP820002; 	// 상세정보 팝업
	 ...
	 * ++++++++++++++++++++++
	 **/
	private Yeta8200ServiceAsync yeta8200Service = Yeta8200Service.Util.getInstance(); 
	 ...
	 */
	/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
	private HiddenField<String> mangeDeptCd;/** set 정산구분코드 : settGbcd */
	private ListStore<BaseModel> lsMangeDeptCd 	= new ListStore<BaseModel>();
	private ComboBox<BaseModel> srhMangeDeptCd; 
	...
	private void authExecEnabled() { 
		 ...
		 */
		// ----------------------------------------------------
		Field<?>[] fldArrField = { srhPayrMangDeptCd, srhEmymtDivCd,srhDeptCd, srhTypOccuCd, srhDtilOccuInttnCd, srhBusinCd }; //2305
		gwtAuthorization.formAuthFieldConfig(fldArrField);

		srhYrtxBlggYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2023")));
		 ...
		 * ++++++++++++++++++++++++++++++++++++++++++
		 */
		srhDeptCd.getListView().fireEvent(Events.CheckChanged); //2305
	}
	...
	private void initLoad() {
		...
		authMapDef.put("srhMangDeptCd",Boolean.FALSE); //2305
	...
	public Yeta8200(String txtForm) {
	...
	private void createYeta0200Form() {
		...
		plFrmYeta8200 = new FormPanel();
		plFrmYeta8200.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산파일업로드"));
		plFrmYeta8200.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta8200.setPadding(2);
		plFrmYeta8200.setFrame(true);
		...
		plFrmYeta8200.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta8200.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		//멀티콤보박스 닫기 
		plFrmYeta8200.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar **/
		topYeta8200Bar = new ButtonBar();
		topYeta8200Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		if ("admin".equals(MSFMainApp.get().getUser().getUsrId())) {	
			btnYeta8200Sec = new Button("암호화패치");
			btnYeta8200Sec.setIcon(MSFMainApp.ICONS.new16());
			topYeta8200Bar.add(btnYeta8200Sec);
			btnYeta8200Sec.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
				 
					Ye161010DTO ye161010Dto = new Ye161010DTO(); 
						   
					yeta8200Service.yetaChangeConvert(ye161010Dto,  new AsyncCallback<String>(){
				                 public void onFailure(Throwable caught) {
				                     MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
				                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta8200Service : " + caught), null);
				                 }
		...
		btnYeta8200Reset = new Button("초기화");
		btnYeta8200Reset.setIcon(MSFMainApp.ICONS.new16());
		topYeta8200Bar.add(btnYeta8200Reset);
		btnYeta8200Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//초기화 
				formInit();
				Yeta8200GridPanel.getMsfGrid().clearData(); 
			}
		});  
		...
		btnYeta8200Sreach = new Button("조 회");
		btnYeta8200Sreach.setIcon(MSFMainApp.ICONS.search());
		topYeta8200Bar.add(btnYeta8200Sreach);
		btnYeta8200Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//조회버튼 클릭시 처리 
				formInit();
				reloadYeta8200();  
		...
		plFrmYeta8200.add(topYeta8200Bar);

		// 파일업로드 처리
		plFrmYeta8200.setAction("myurl");
		plFrmYeta8200.setEncoding(Encoding.MULTIPART);
		plFrmYeta8200.setMethod(Method.POST);

		vp.add(plFrmYeta8200);
		plFrmYeta8200.setSize("990px", "830px");
		plFrmYeta8200.setLayout(new FlowLayout());
		...
		srhPayrMangDeptCd.addSelectionChangedListener(new SelectionChangedListener<BaseModel>() {
			public void selectionChanged(SelectionChangedEvent<BaseModel> se) {
				...
				sysComBass0400Dto.setMangeDeptCd(MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd")); // 관리부서 수정
				lsDeptCd = PrgmComComboUtils.getPayrBass0400ComboData(sysComBass0400Dto);
				// --------------------부서 불러 오는 함수------------------------------------------------
				lsDeptCd.addStoreListener(new StoreListener<BaseModel>() {
					public void handleEvent(StoreEvent<BaseModel> be) {
		...
		srhResnRegnNum.setVisible(false); // 화면 숨기기
		        ...
		        srhMangeDeptCd.getStore().addStoreListener( new StoreListener<BaseModel>() {   
		            public void handleEvent(StoreEvent<BaseModel> be) {  
		            	...
		            	if (type == Store.Add) { 
		            		 ...
		            		 * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				     			++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
//		            			checkYeta0200Auth("srhMangeDeptCd", lsMangeDeptCd);
...
//		            } 
				...
				srhDeptCd.setWidth(60);
				srhDeptCd.setFieldLabel("");
				srhDeptCd.setReadOnly(gwtExtAuth.getReadOnlyDeptCd());
				srhDeptCd.setEnabled(gwtExtAuth.getEnableDeptCd());
				srhDeptCd.setVisible(false); // 화면 숨기기
				...
				layoutContainer8.add(srhMangeDeptCd, new FormData("100%"));
	...
	private void createMsfGridForm() {
		...
		Yeta8200GridPanel = new MSFGridPanel(Yeta8200def, false, false, false, false,false);
		Yeta8200GridPanel.setHeaderVisible(false);
		Yeta8200GridPanel.setSize(470, 640);
		Yeta8200GridPanel.setBorders(true);
		final Grid yeta8000DefGrid = Yeta8200GridPanel.getMsfGrid().getGrid();
		yeta8000DefGrid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (Yeta8200GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = new BaseModel();
					formInit() ;
					bmData = Yeta8200GridPanel.getCurrentlySelectedItem();
		...
		Yeta8200GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 4, new HeaderGroupConfig("소득공제자료", 1, 2));

		fsGird.add(Yeta8200GridPanel);
		...
		Yeta8200XmlGridPanel = new MSFGridPanel(Yeta8200defXml, false, false,false, false);
		Yeta8200XmlGridPanel.setSize(250, 305);
		Yeta8200XmlGridPanel.setBorders(true);
		Yeta8200XmlGridPanel.setStyleAttribute("marginTop", "5px");
		layoutContainer17.add(Yeta8200XmlGridPanel,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.8));

		btnYetaP820001 = new Button("개별간소화파일");
		btnYetaP820001.setStyleAttribute("marginTop", "5px");
		btnYetaP820001.setStyleAttribute("marginLeft", "5px");
		btnYetaP820001.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					...
					final MSFFormWindows msFwYeta820001 = new MSFFormWindows("자료추출상세내역",yetaP820001Form, "닫기", "920px", "710px", false);
					msFwYeta820001.show();
					yetaP820001Form.setMSFFormWindows(msFwYeta820001);

					//					funcSetPopUpRecord();

					yetaP820001Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP820001,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));


		layoutContainer1_4.add(layoutContainer17, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.495));

		plFrmYeta8200.add(layoutContainer1_4);
	}
	...
	private void funcSetPopUpRecord() {
		...
		// 관리부서 추가 / 
		ppRecord.set("mangeDeptCd", clutSeptCd.getValue());
	...
	private Yeta8200 getThis(){
		return this;
	}

	private void reloadYeta8200(){

		IColumnFilter filters = null;
		Yeta8200GridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta8200GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 

		//정산구분
		Yeta8200GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta8200GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta8200GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		
		// 관리부서 추가 / 
		Yeta8200GridPanel.getTableDef().addColumnFilter("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
			
		Yeta8200GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta8200GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		
		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta8200GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta8200GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta8200GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta8200GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta8200GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
		Yeta8200GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   

		Yeta8200GridPanel.reload();  

	}

	private void reloadYeta8200Result(){

		IColumnFilter filters = null;
		Yeta8200XmlGridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta8200XmlGridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 
		//정산구분
		Yeta8200XmlGridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta8200XmlGridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		// 관리부서 추가/ 
		Yeta8200GridPanel.getTableDef().addColumnFilter("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta8200XmlGridPanel.reload();  
		...
		MessageBox.confirm("전자문서", "전자문서를 업로드 하시겠습니까?<br>(등록되어있는 전자문서가 있을시 삭제 처리 됩니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					//PDF 변경 
					if(systemkey.getValue()=="" || systemkey.getValue() == null){
						//MessageBox.alert("", "PDF 전자문서 업로드 대상자를 선택하십시요.", null);
						Window.alert("PDF 전자문서 업로드 대상자를 더블클릭 해주세요.");
					}else{
						if(clutSeptCd.getValue() == "" || clutSeptCd.getValue() == null){
							MessageBox.alert("", "정산구분은 필수 선택조건입니다.", null);
								...
								if (filename.length() == 0) {
									...
									//submit the form
									plFrmYeta8200.mask("PDF 전자문서 업데이트 중입니다. 업로드 중..."); 
		
									//파일업로드 처리
									plFrmYeta8200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta8200Import.do");
									plFrmYeta8200.setEncoding(Encoding.MULTIPART);
									plFrmYeta8200.setMethod(Method.POST);   
									//	                               FormElement frmEl=FormElement.as(plFrmYeta7200.getElement()); 
									//	                               frmEl.setAcceptCharset("UTF-8"); 
									plFrmYeta8200.submit();  
									plFrmYeta8200.onFrameLoad();  
									picBoolFile = true;
						...
						// after file upload 
						plFrmYeta8200.addListener(Events.Submit, new Listener<FormEvent>() {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//파일업로드 처리
							plFrmYeta8200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta8200DdcImport.do");
							plFrmYeta8200.setEncoding(Encoding.MULTIPART);
							plFrmYeta8200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta8200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
							plFrmYeta8200.submit();  
							plFrmYeta8200.onFrameLoad();  
							picBoolFile = true;
