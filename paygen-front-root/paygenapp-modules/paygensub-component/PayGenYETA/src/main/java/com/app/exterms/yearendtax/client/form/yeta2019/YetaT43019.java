...
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43019Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43019ServiceAsync;
...
=====================================================**/
public  class YetaT43019   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT43019; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT43019GridPanel;
	private Ye162510Def YetaT43019ToYe162510Def = new Ye162510Def("YETAT13019");   //월세새액공제명세 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT43019ServiceAsync yetaT43019Service = YetaT43019Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
 ...
 **/ 
     ...
     */ 
		...
		private void YetaT43019FormSave() {  
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT43019Service.activityOnYetaT43019ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
					
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("YetaT43019FormSave(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	...
	public YetaT43019(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT43019 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT43019.setBodyBorder(false);
		cpYetaT43019.setBorders(false);
		cpYetaT43019.setHeaderVisible(false);
		cpYetaT43019.setSize("790px", "640px");
		   
		this.add(cpYetaT43019);
...
//		     });     
		...
		cpYetaT43019.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaT43019GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = YetaT43019GridPanel.getTableDef().getDefaultValues();  
		        ...
		        YetaT43019GridPanel.getMsfGrid().getGrid().stopEditing();      
		        YetaT43019GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        YetaT43019GridPanel.getMsfGrid().getGrid().startEditing(YetaT43019GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT43019GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT43019GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT43019.add(topYetaBtnBar);
...
//		cpYetaT43019.add(topYetaBtnBar);
		...
		YetaT43019GridPanel = new MSFGridPanel(YetaT43019ToYe162510Def, false, false, false, false, false);
		YetaT43019GridPanel.setSize("790px", "200px");
		YetaT43019GridPanel.setBorders(true);
		YetaT43019GridPanel.setHeaderVisible(false);
...
//		
//		YetaT43019GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		/** --- 그리드 합계 부분  종료 --- **/
		
//   	cpYetaT43019.add(YetaT43019GridPanel);
		...
		fldstNewFieldset.add(YetaT43019GridPanel);
		cpYetaT43019.add(fldstNewFieldset);
		cpYetaT43019.add(layoutContainer1);
		cpYetaT43019.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT43019GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT43019GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT43019GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT43019GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT43019GridPanel.reload();
