...
import com.app.exterms.yearendtax.client.service.yeta2021.Yeta6100Service;
import com.app.exterms.yearendtax.client.service.yeta2021.Yeta6100ServiceAsync;
...
=====================================================**/
public class Yeta6100   extends MSFPanel {
	/**
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	 * 	전역변수 선언부  시작
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	...
	private FormPanel plFrmYeta6100;
	private String txtForm = "";
	private XTemplate detailTp;

	private Ye161010Def Yeta6100def = new Ye161010Def("Yeta6100");
	private MSFGridPanel Yeta6100GridPanel;
	...
	private Yeta6100ServiceAsync yeta6100Service = Yeta6100Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhClutYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2021")));
...
//		authMapDef.put("creDeptCd",Boolean.FALSE); 
	 ...
	 * 데이타를 삭제한다.
	 */
	private void Yeta6100_menuDelete() {
		...
		MessageBox.confirm("연말정산 대상자 삭제", "선택된 데이타를 삭제 하시겠습니까?<br>(연말정산 대상자를 삭제합니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = Yeta6100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
					...
					yeta6100Service.Yeta6100ToYe160401_Delete(listYe161010Dto, new AsyncCallback<Long>() {
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
									MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta6100ToYe160401_Delete(삭제) : " + caught), null);
						}
...
//
//		//Iterator<BaseModel> itBm = Yeta6100GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
//
//		Iterator<BaseModel> itBm = Yeta6100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator();
//
//		List<Record> lsRec = new ArrayList<Record>();
...
//			Yeta6100Service.UpDpobExec_Yeta_Insert_2017(listYe161010Dto, new AsyncCallback<Long>() {
//				public void onFailure(Throwable caught) {
//					tracker.setStatus(true);
//					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
...
//	}
	...
	private void PayrExec_Yeta_Insert() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			t.scheduleRepeating(500);
			tracker.setStatus(true);
			yeta6100Service.PayrExec_Yeta_2021_Insert(listYe161010Dto, new AsyncCallback<Long>() {
				public void onFailure(Throwable caught) {
					tracker.setStatus(true);
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("PayrExec_Yeta_Insert(급여자료이관) : " + caught), null);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 **/
	private void Yeta6100Print(String fileName, String repType) {
...
//			serarchParam += "" + YetaUtils.getPrintStrToMultiData(lsDeptCd, MSFSharedUtils.allowNulls(srhDeptCd.getValue()),"deptNmRtchnt","deptCd") +"⊥";		
			...
			List<BaseModel> list = Yeta6100GridPanel.getGrid().getSelectionModel().getSelectedItems();
	...
	public Yeta6100(String txtForm) {
	...
	private void createYeta0100Form() {
		...
		plFrmYeta6100 = new FormPanel();
		plFrmYeta6100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산대상자관리"));
		plFrmYeta6100.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta6100.setPadding(2);
		plFrmYeta6100.setFrame(true);
		...
		plFrmYeta6100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta6100.getHeader().addTool(btnlogView);
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		// 멀티콤보박스 닫기
		plFrmYeta6100.addListener(Events.OnClick, new Listener<ComponentEvent>() {
			@Override
		...
		btnYeta0100Init.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Yeta6100GridPanel.getMsfGrid().clearData();
			}
		});
		...
		btnYeta0100Cre.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP610001 yetaP610001Form = new YetaP610001(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta610001 = new MSFFormWindows("연말정산대상자검색",yetaP610001Form, "닫기", "920px", "710px", false);
				msFwYeta610001.show();
				yetaP610001Form.setMSFFormWindows(msFwYeta610001);
		...
		plFrmYeta6100.add(topYeta0100Bar);
		...
		btnYeta0100Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				Yeta6100Print("YETAT6100", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
			}
		});
		plFrmYeta6100.add(topYeta0100Bar);	
		...
		// 파일업로드 처리
		plFrmYeta6100.setAction("myurl");
		plFrmYeta6100.setEncoding(Encoding.MULTIPART);
		plFrmYeta6100.setMethod(Method.POST);
		vp.add(plFrmYeta6100);

		plFrmYeta6100.setSize("990px", "680px");
		plFrmYeta6100.setLayout(new FlowLayout());
	}
...
//		srhDtilOccuInttnCd.setEnabled(gwtExtAuth.getEnableDtilOccuInttnCd());
		...
		btnUpDpobTrans.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP610002 yetaP610002Form = new YetaP610002(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta610002 = new MSFFormWindows("원천신고부서",yetaP610002Form, "닫기", "920px", "710px", false);
				msFwYeta610002.show();
				yetaP610002Form.setMSFFormWindows(msFwYeta610002);
		...
		plFrmYeta6100.add(fieldSet1);
		plFrmYeta6100.add(fieldSet2);	
	...
	private void createMsfGridForm() {
		...
		Yeta6100GridPanel = new MSFGridPanel(Yeta6100def, false, false, false,false, false);
		// Yeta6100GridPanel.setSize(960, 400);
		Yeta6100GridPanel.setHeaderVisible(false);
		Yeta6100GridPanel.setBorders(true);
		// Yeta6100GridPanel.getBottomComponent().setVisible(false);
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = Yeta6100GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					Yeta6100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					Yeta6100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				Yeta6100GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					Yeta6100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					Yeta6100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		//툴바 넣어주는것
		Yeta6100GridPanel.setBottomComponent(bottomToolbar);

		final Grid<BaseModel> grid = Yeta6100GridPanel.getMsfGrid().getGrid();
		...
		cp01.add(Yeta6100GridPanel); 

		fieldSetGrd.add(cp01);
		plFrmYeta6100.add(fieldSetGrd);
	}
...
//	}	
	...
	public void reload() {
		...
		Yeta6100GridPanel.getTableDef().setTableColumnFilters(filters);

		Yeta6100GridPanel.getTableDef().addColumnFilter("yrtxBlggYr",MSFSharedUtils.getSelectedComboValue(srhClutYr, "year"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta6100GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd"), SimpleColumnFilter.OPERATOR_EQUALS);	

		Yeta6100GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta6100GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta6100GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta6100GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta6100GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta6100GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta6100GridPanel.getTableDef().addColumnFilter("fnm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta6100GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta6100GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(utDpobCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta6100GridPanel.getTableDef().addColumnFilter("utDpobChk", MSFSharedUtils.convertBooleanToString(utDpobChk.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		

		Yeta6100GridPanel.reload();
	...
	private void xlsExportData() {
		...
		if ("".equals(srhClutYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				Yeta6100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta6100Export.do", "extgwtFrame", param);
			}
		} else {
			Yeta6100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta6100Export.do", "extgwtFrame", param);
		}
