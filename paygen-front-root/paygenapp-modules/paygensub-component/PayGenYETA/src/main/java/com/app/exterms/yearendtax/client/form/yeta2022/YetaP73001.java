...
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye165030BM;
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye165030DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2022.Ye165030Def;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73001Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73001ServiceAsync;
...
=====================================================**/
public  class YetaP73001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta73001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP73001GridPanel;
//	private Ye16Ta3170Def YetaP73001ToYe5030Def  = new Ye16Ta3170Def("YetaP73001");	//이월기부금 그리드 테이블 컬럼 define 
	private Ye165030Def YetaP73001ToYe5030Def  = new Ye165030Def("YetaP73001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
// 	private TextField<String> srhClutSeptCd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP73001ServiceAsync yetaP73001Service = YetaP73001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP73001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta73001 = new ContentPanel();

		createSearchForm(); // 기본정보 필드를 적용
		// createMsfGridForm(); //그리드필드
		createStandardForm(); // 기본정보필드

		cpYeta73001.setBodyBorder(false);
		cpYeta73001.setBorders(false);
		cpYeta73001.setHeaderVisible(false);
		cpYeta73001.setSize("790px", "640px");

		this.add(cpYeta73001);
...
//		lsdacRvyyStore = SysComComboUtils.getSysComComboYearData(msfCoCalendarDto);		//급여년도
		...
		cpYeta73001.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent be) {
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 YetaP73001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = YetaP73001GridPanel.getTableDef().getDefaultValues();  
		         ...
		         YetaP73001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         YetaP73001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         YetaP73001GridPanel.getMsfGrid().getGrid().startEditing(YetaP73001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 이월기부금 내용을 삭제하시겠습니까?" , new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){	
							...
							Iterator<BaseModel> itBm = YetaP73001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP73001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//				srhClutSeptCd.setValue(lsclutSeptCd.getAt(0));
		...
		cpYeta73001.add(fieldSet);
		cpYeta73001.add(createMsfGridForm(), new FormData("100%"));
	}  
...
//	private void createMsfGridForm() {
		...
		YetaP73001GridPanel = new MSFGridPanel(YetaP73001ToYe5030Def, false, false, false, false, false);
		YetaP73001GridPanel.setBorders(false);
		YetaP73001GridPanel.setBodyBorder(false);
		YetaP73001GridPanel.setHeaderVisible(false); 
		YetaP73001GridPanel.getBottomComponent().setVisible(false);
		
		GWTUtils.findColumnConfig(YetaP73001GridPanel, Ye165030BM.ATTR_THCYRDDCAMT).setStyle("background-color:#F5F6CE;");
		
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		GWTUtils.findColumnConfig(YetaP73001GridPanel, Yeta3170BM.ATTR_CTRBAMNT).setStyle("Height:20px;");
		...
		YetaP73001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		
		cpYeta73001.add(YetaP73001GridPanel);
		cp01.add(YetaP73001GridPanel);
	...
	private void YetaP73001FormSave() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			if(dataChk) {
				...
				yetaP73001Service.activityOnYetaP73001ToYe165030(listYe165030Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP73001ToYe165030(" + actionDatabase.name() + ") : " + caught), null);
					}
	...
	public void reload() {
		...
		YetaP73001GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP73001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		
		YetaP73001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		
		YetaP73001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		
		YetaP73001GridPanel.reload();
