...
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye162520BM;
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye162520DTO;
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye162530BM;
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye162530DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye162520Def;
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye162530Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23004Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23004ServiceAsync;
...
=====================================================**/
public  class YetaP23004   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP23004GridPanel01;
	private Ye162520Def yetaP23004ToYeta3120Def  = new Ye162520Def("YetaP23004");	//금전소비대차계약 그리드 테이블 컬럼 define 
 ...
 **/ 
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP23004ServiceAsync yetaP23004Service = YetaP23004Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 	
	...
	private void doAction(ActionDatabase actionDatabase, String doActionFlag) {
		...
		case INSERT:
			...
			if ("YE162520".equals(doActionFlag)) {
				yetaP23004ToYe162520FormSave();
			} else if ("YE162530".equals(doActionFlag)) {
				yetaP23004ToYe162530FormSave();
			} else {
	...
	private void yetaP23004ToYe162520FormSave() {
...
//				ye162520Dto.setEctnFlTrnrCtlYn( ((Boolean) bmMapModel.get("ectnFlTrnrCtlYn")) ? "Y" : "N"  );								/** column 전자파일이관여부 : ectnFlTrnrCtlYn */
			...
			yetaP23004Service.activityOnYetaP23004ToYe162520(listYe162520Dto, actionDatabase, new AsyncCallback<Long>(){
				public void onFailure(Throwable caught) {
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP23004ToYe162520(" + actionDatabase.name() + ") : " + caught), null);
				}
	...
	private void yetaP23004ToYe162530FormSave() {  
		if (MSFSharedUtils.paramNotNull(records)) { 
...
//				}
			...
			yetaP23004Service.activityOnYetaP23004ToYe162530(listYe162530Dto, actionDatabase, new AsyncCallback<Long>(){
				public void onFailure(Throwable caught) {
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("yetaP23004ToYe162530FormSave(" + actionDatabase.name() + ") : " + caught), null);
				}
				public void onSuccess(Long result) { 
					if (result == 0) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), actionDatabase + "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
...
//		 }
	  ...
	  public YetaP23004(final ActionDatabase actionDatabase, final MSFPanel caller) {
	...
	private void createMsfGridForm1() {
		...
		btnYe162520New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				yetaP23004GridPanel01.getMsfGrid().clearData();

				BaseModel bmData = yetaP23004GridPanel01.getTableDef().getDefaultValues();
...
//			    bmData.set("ectnFlTrnrCtlYn"		, false);    									/** column 전자파일이관여부 : ectnFlTrnrCtlYn */			    
			    ...
			    yetaP23004GridPanel01.getMsfGrid().getGrid().stopEditing();      
			    yetaP23004GridPanel01.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			    yetaP23004GridPanel01.getMsfGrid().getGrid().startEditing(yetaP23004GridPanel01.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 금전소비대차계약 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm  = yetaP23004GridPanel01.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYe162520Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP23004GridPanel01.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		yetaP23004GridPanel01 = new MSFGridPanel(yetaP23004ToYeta3120Def,false, false, false, false, false);
		yetaP23004GridPanel01.setSize("790px", "190px");
		yetaP23004GridPanel01.setBorders(false);
		yetaP23004GridPanel01.setHeaderVisible(false);
		yetaP23004GridPanel01.getBottomComponent().setVisible(true);
...
//		payrSummaryRow.setSummaryFormat(Ye162520BM.ATTR_CNCLITRO, NumberFormat.getFormat("#,##0;(#,##0)"));  
		...
		yetaP23004GridPanel01.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			
		gridFieldSet.add(yetaP23004GridPanel01);
...
//			    bmData.set("ectnFlTrnrCtlYn"		, false);    									/** column 전자파일이관여부 : ectnFlTrnrCtlYn */			    
	...
	public void reloadYeta3120() {
		...
		yetaP23004GridPanel01.getTableDef().setTableColumnFilters(filters);
		yetaP23004GridPanel01.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		yetaP23004GridPanel01.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		yetaP23004GridPanel01.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
		yetaP23004GridPanel01.reload();
	}
