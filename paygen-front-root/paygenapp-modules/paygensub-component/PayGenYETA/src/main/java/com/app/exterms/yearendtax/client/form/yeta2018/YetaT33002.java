...
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33002Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33002ServiceAsync;
...
=====================================================**/
public  class YetaT33002   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33002; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33002GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33002ServiceAsync yetaT33002Service = YetaT33002Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33002Service.activityOnYetaT33002(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT33002(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33002 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33002.setBodyBorder(false);
		cpYetaT33002.setBorders(false);
		cpYetaT33002.setHeaderVisible(false);
		cpYetaT33002.setSize("790px", "640px");
		   
		this.add(cpYetaT33002);
...
//		     });     
		...
		cpYetaT33002.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT33002GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT33002GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT33002GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT33002GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT33002GridPanel.getMsfGrid().getGrid().startEditing(YetaT33002GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT33002GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT33002GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT33002.add(topYetaBtnBar);
...
//		YetaT33002GridPanel = new MSFGridPanel(YetaT33002Def, false, false, false, false, false);
//		YetaT33002GridPanel.setSize("790px", "420px");
//		YetaT33002GridPanel.setBorders(true);
//		YetaT33002GridPanel.setHeaderVisible(false);
//		
//      cpYetaT33002.add(YetaT33002GridPanel);
...
//		fldstNewFieldset1.add(layoutContainer13);
		...
		cpYetaT33002.add(fldstNewFieldset10);
		cpYetaT33002.add(layoutContainer14);
		cpYetaT33002.add(layoutContainer15);
		cpYetaT33002.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33002GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33002GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33002GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33002GridPanel.reload();
