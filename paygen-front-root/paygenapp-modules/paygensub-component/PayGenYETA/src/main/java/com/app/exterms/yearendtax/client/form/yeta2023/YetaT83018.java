...
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83018Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83018ServiceAsync;
...
=====================================================**/
public  class YetaT83018   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT83018; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT83018GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT83018ServiceAsync yetaT83018Service = YetaT83018Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT83018Service.activityOnYetaT83018(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT83018(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT83018 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT83018.setBodyBorder(false);
		cpYetaT83018.setBorders(false);
		cpYetaT83018.setHeaderVisible(false);
		cpYetaT83018.setSize("790px", "640px");
		   
		this.add(cpYetaT83018);
...
//		     });     
		...
		cpYetaT83018.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT83018GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT83018GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT83018GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT83018GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT83018GridPanel.getMsfGrid().getGrid().startEditing(YetaT83018GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT83018GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//				
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT83018GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//				
		cpYetaT83018.add(topYetaBtnBar);
...
//		YetaT83018GridPanel = new MSFGridPanel(YetaT83018Def, false, false, false, false, false);
//		YetaT83018GridPanel.setSize("790px", "420px");
//		YetaT83018GridPanel.setBorders(true);
//		YetaT83018GridPanel.setHeaderVisible(false);
...
//		
//		YetaT83018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT83018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT83018.add(YetaT83018GridPanel);
		...
		cpYetaT83018.add(layoutContainer);
	...
	public void reload() {
			...
			YetaT83018GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT83018GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT83018GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT83018GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT83018GridPanel.reload();
