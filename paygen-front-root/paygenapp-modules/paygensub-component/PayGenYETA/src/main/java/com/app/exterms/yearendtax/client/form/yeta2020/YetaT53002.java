...
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53002Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53002ServiceAsync;
...
=====================================================**/
public  class YetaT53002   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT53002; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT53002GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT53002ServiceAsync yetaT53002Service = YetaT53002Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT53002Service.activityOnYetaT53002(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT53002(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT53002 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT53002.setBodyBorder(false);
		cpYetaT53002.setBorders(false);
		cpYetaT53002.setHeaderVisible(false);
		cpYetaT53002.setSize("790px", "640px");
		   
		this.add(cpYetaT53002);
...
//		     });     
		...
		cpYetaT53002.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT53002GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT53002GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT53002GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT53002GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT53002GridPanel.getMsfGrid().getGrid().startEditing(YetaT53002GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT53002GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT53002GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT53002.add(topYetaBtnBar);
...
//		YetaT53002GridPanel = new MSFGridPanel(YetaT53002Def, false, false, false, false, false);
//		YetaT53002GridPanel.setSize("790px", "420px");
//		YetaT53002GridPanel.setBorders(true);
//		YetaT53002GridPanel.setHeaderVisible(false);
//		
//      cpYetaT53002.add(YetaT53002GridPanel);
...
//		fldstNewFieldset1.add(layoutContainer13);
		...
		cpYetaT53002.add(fldstNewFieldset10);
		cpYetaT53002.add(layoutContainer14);
		cpYetaT53002.add(layoutContainer15);
		cpYetaT53002.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT53002GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT53002GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT53002GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT53002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT53002GridPanel.reload();
