...
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4500Service;
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4500ServiceAsync;
...
=====================================================**/
public class Yeta4500   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmYeta4500; 
    private String txtForm = "";
    private XTemplate detailTp;
 
    private Yeta1000Def yeta1000def = new Yeta1000Def("Yeta4500");
...
//		private MSFGridPanel typOccuCdGridPanel;
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
   private ButtonBar topYeta4500Bar;
   private Button btnYeta4500Init; 
//   private Button btnYeta0100Save;
//   private Button btnYeta0100Del;
   private Button btnYeta4500Sreach;
//   private Button btnYeta0100Print;
   private Button btnYeta4500Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 **/
	...
	private Yeta4500ServiceAsync yeta4500Service = Yeta4500Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 
   ...
   // -------------- stroe [검색부]선언 시작 -------------- 
	   ...
	   **/
	 	 ...
	 	 */
	  	private void checkYeta4500Auth( String authAction, ListStore<BaseModel>bm) {
...
//	    
	 ...
	 **/ 	 
		...
		public Yeta4500(String txtForm) {
	 ...
	 **/		  
		private void createYeta0100Form() {
			...
			plFrmYeta4500 = new FormPanel(); 
			plFrmYeta4500.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산급여반영"));
			plFrmYeta4500.setIcon(MSFMainApp.ICONS.text());
			plFrmYeta4500.setPadding(2);
			plFrmYeta4500.setFrame(true); 
			...
			btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 //로그 뷰화면 호출 메서드 
					  funcLogMessage("연말정산급여반영","Yeta4500");
					}
				});
			plFrmYeta4500.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
			plFrmYeta4500.getHeader().addTool(btnlogView); 
			/************************************************************************
			 * 화면 에러 처리시 필요한 부분으로 추가 
			 ************************************************************************/
 
		     //멀티콤보박스 닫기 
			plFrmYeta4500.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
	            @Override
...
//	                        }
			...
			/** 상단 버튼 ButtonBar**/
			topYeta4500Bar = new ButtonBar();    
			topYeta4500Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
			btnYeta4500Init = new Button("초기화");  
			btnYeta4500Init.setIcon(MSFMainApp.ICONS.new16());
			topYeta4500Bar.add(btnYeta4500Init); 
			btnYeta4500Init.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//초기화 
	    			formInit();
	    		}
			...
			btnYeta4500Sreach = new Button("조 회");  
			btnYeta4500Sreach.setIcon(MSFMainApp.ICONS.search());
			topYeta4500Bar.add(btnYeta4500Sreach);
			btnYeta4500Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//조회버튼 클릭시 처리 
	    			reloadYeta4500();  
	    		}
...
//			topYeta4500Bar.add(btnYeta0100Print);	
			
//			btnYeta0100Print = new Button("인 쇄");  
//			btnYeta0100Print.setIcon(MSFMainApp.ICONS.print16());
//			topYeta4500Bar.add(btnYeta0100Print);	
			
			btnYeta4500Excel = new Button("엑 셀");  
			btnYeta4500Excel.setIcon(MSFMainApp.ICONS.excel16());
			topYeta4500Bar.add(btnYeta4500Excel);
			btnYeta4500Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//엑셀 저장
	    			xlsExportData();  
	    		}
			...
			plFrmYeta4500.add(topYeta4500Bar);
		      
			//파일업로드 처리
			plFrmYeta4500.setAction("myurl");
			plFrmYeta4500.setEncoding(Encoding.MULTIPART);
			plFrmYeta4500.setMethod(Method.POST);		         
			vp.add(plFrmYeta4500);	
			
			plFrmYeta4500.setSize("990px", "680px");	      
			plFrmYeta4500.setLayout(new FlowLayout());
		}
...
//		  	plFrmYeta4500.add(typOccuCdGrid);
...
//														 searchBusinCd.setValue(lsBusinCd.getAt(0));
			    ...
			    lsPayrMangDeptCd.addStoreListener( new StoreListener<BaseModel>() {   
			        public void handleEvent(StoreEvent<BaseModel> be) {  
			        	checkYeta4500Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
			        }
			    });    
				    	   ...
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta4500Auth("srhEmymtDivCd", lsEmymtDivCd); 
			           	 }
			        }
			    });    
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta4500Auth("srhDeptCd", lsDeptCd); 
			           	 }
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		 //  checkYeta4500Auth("srhTypOccuCd", lsTypOccuCd); 
			           	 }
			        }
			    });     
...
//		                BaseModel bmMapModel = (BaseModel)record.getModel();
					...
					yeta4500Service.saveYeta4500Grid(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta4500Grid(" + "저장" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
...
//					ntime = format.format(c.getTime());
					...
					while (records.hasNext()) {
		                ...
		                ye161010Dto.setYrtxApptnYrMnth("202002");		/** column 연말정산적용년월 : yrtxApptnYrMnth */
...
//		                ye161010Dto.setDivdPymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("divdPymtDivCd$commCd")));/** column 분할납부구분코드 : divdPymtDivCd */
					...
					yeta4500Service.activityOnYeta4500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta4500List(" + "2월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
...
//					ntime += fmt2.format(today);
					...
					while (records.hasNext()) {
		                ...
		                ye161010Dto.setYrtxApptnYrMnth("202003");		/** column 연말정산적용년월 : yrtxApptnYrMnth */
		                ye161010Dto.setYrtxPrcsDt("");	/** column 연말정산처리일자 : yrtxPrcsDt */
		                ye161010Dto.setYrtxPrcsYn((Boolean) bmMapModel.get("yrtxPrcsYn"));		/** column 연말정산처리여부 : yrtxPrcsYn */
					...
					yeta4500Service.activityOnYeta4500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta4500List(" + "3월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
...
//					ntime = format.format(c.getTime());
					...
					yeta4500Service.activityOnYeta4500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta4500List(" + "당월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
	...
	private Yeta4500 getThis() {
		return this;
	}
		   
	private void reloadYeta4500() {
	...
	private void xlsExportData() {
		...
		if ("".equals(srhYrtxBlggYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta4500Export.do", "extgwtFrame", param);
			}
		} else {
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta4500Export.do", "extgwtFrame", param);
		}
