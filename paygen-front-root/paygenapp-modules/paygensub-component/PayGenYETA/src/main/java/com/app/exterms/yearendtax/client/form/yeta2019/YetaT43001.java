...
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye160510Def;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43001Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43001ServiceAsync;
...
=====================================================**/
public  class YetaT43001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT43001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT43001GridPanel;
	private Ye160510Def ye160510Def = new Ye160510Def("YetaT43001");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT43001ServiceAsync yetaT43001Service = YetaT43001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT43001Service.activityOnYetaT43001(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT43001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT43001 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT43001.setBodyBorder(false);
		cpYetaT43001.setBorders(false);
		cpYetaT43001.setHeaderVisible(false);
		cpYetaT43001.setSize("790px", "640px");
		   
		this.add(cpYetaT43001);
...
//		     });     
		...
		cpYetaT43001.setLayout(new FlowLayout());
		...
		btnYetaAdd.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT43001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT43001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
				List<Record> lsRec = new ArrayList<Record>();

				
				if((ActionDatabase.INSERT).equals(actionDatabase)) {
					YetaT43001GridPanel.getMsfGrid().clearData();
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT43001.add(topYetaBtnBar);
...
//		cpYetaT43001.add(topYetaBtnBar);
		...
		YetaT43001GridPanel = new MSFGridPanel(ye160510Def, false, false, false, false, false);
		YetaT43001GridPanel.setSize("790px", "200px");
		YetaT43001GridPanel.setBorders(true);
		YetaT43001GridPanel.setHeaderVisible(false);
...
//		
//		YetaT43001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT43001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT43001.add(YetaT43001GridPanel);
		...
		fldstNewFieldset.add(YetaT43001GridPanel);
		
		cpYetaT43001.add(fldstNewFieldset);
		cpYetaT43001.add(layoutContainer10);
		cpYetaT43001.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT43001GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT43001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT43001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT43001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT43001GridPanel.reload();
