...
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33011Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33011ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT33011Def;
...
=====================================================**/
public  class YetaT33011   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33011; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33011GridPanel;
//	private YetaT33011Def YetaT33011Def = new YetaT33011Def("YetaT33011");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33011ServiceAsync yetaT33011Service = YetaT33011Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33011Service.activityOnYetaT33011(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT33011(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33011 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33011.setBodyBorder(false);
		cpYetaT33011.setBorders(false);
		cpYetaT33011.setHeaderVisible(false);
		cpYetaT33011.setSize("790px", "640px");
		   
		this.add(cpYetaT33011);
...
//		     });     
		...
		cpYetaT33011.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT33011GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT33011GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT33011GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT33011GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT33011GridPanel.getMsfGrid().getGrid().startEditing(YetaT33011GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT33011GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT33011GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT33011.add(topYetaBtnBar);
...
//		YetaT33011GridPanel = new MSFGridPanel(YetaT33011Def, false, false, false, false, false);
//		YetaT33011GridPanel.setSize("790px", "420px");
//		YetaT33011GridPanel.setBorders(true);
//		YetaT33011GridPanel.setHeaderVisible(false);
...
//		
//		YetaT33011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT33011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT33011.add(YetaT33011GridPanel);
...
//		
		...
		cpYetaT33011.add(layoutContainer);
		cpYetaT33011.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33011GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33011GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33011GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33011GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33011GridPanel.reload();
