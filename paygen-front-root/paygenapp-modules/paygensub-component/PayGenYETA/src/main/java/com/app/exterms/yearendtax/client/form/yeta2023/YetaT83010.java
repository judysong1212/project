...
import com.app.exterms.yearendtax.client.form.defs.yeta2023.Ye162040Def;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83010Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83010ServiceAsync;
...
=====================================================**/
public  class YetaT83010   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT83010; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT83010GridPanel;
	private Ye162040Def ye162040Def = new Ye162040Def("YetaT83010");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT83010ServiceAsync yetaT83010Service = YetaT83010Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT83010Service.activityOnYetaT83010(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT83010(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT83010 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT83010.setBodyBorder(false);
		cpYetaT83010.setBorders(false);
		cpYetaT83010.setHeaderVisible(false);
		cpYetaT83010.setSize("790px", "640px");
		   
		this.add(cpYetaT83010);
...
//		     });     
		...
		cpYetaT83010.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT83010GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT83010GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT83010GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT83010GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT83010GridPanel.getMsfGrid().getGrid().startEditing(YetaT83010GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT83010GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT83010GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT83010.add(topYetaBtnBar);
...
//		cpYetaT83010.add(topYetaBtnBar);
		...
		YetaT83010GridPanel = new MSFGridPanel(ye162040Def, false, false, false, false, false);
		YetaT83010GridPanel.setSize("790px", "200px");
		YetaT83010GridPanel.setBorders(true);
		YetaT83010GridPanel.setHeaderVisible(false);
...
//		
//		YetaT83010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT83010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT83010.add(YetaT83010GridPanel);
		...
		fldstNewFieldset.add(YetaT83010GridPanel);
		cpYetaT83010.add(fldstNewFieldset);
		cpYetaT83010.add(layoutContainer10);
		cpYetaT83010.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT83010GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT83010GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT83010GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT83010GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT83010GridPanel.reload();
