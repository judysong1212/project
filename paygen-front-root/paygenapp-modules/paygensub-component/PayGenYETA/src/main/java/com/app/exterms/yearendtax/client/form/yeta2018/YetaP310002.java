...
import com.app.exterms.yearendtax.client.form.defs.yeta2018.Ye161020Def;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP310002Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP310002ServiceAsync;
...
=====================================================**/
public  class YetaP310002   extends MSFFormPanel {  
 ...
 **/
	...
	private ContentPanel cpYeta2100;

	private Ye161020Def ye161020Def = new Ye161020Def("YetaP310002");
	private MSFGridPanel YetaP310002GridPanel;
 ...
 **/ 
	...
	private YetaP310002ServiceAsync yetaP310002Service = YetaP310002Service.Util.getInstance();
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP310002(ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad();
		...
		cpYeta2100 = new ContentPanel();

		createSearchForm(); 	// 검색필드를 적용
		createStandardForm(); 	// 기본정보필드

		cpYeta2100.setBodyBorder(false);
		cpYeta2100.setBorders(false);
		cpYeta2100.setHeaderVisible(false);
		cpYeta2100.setSize("890px", "640px");
		...
		btnSrh.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				if(creUtDpobCd.getValue() != null && !"".equals(creUtDpobCd.getValue()) ) {
					...
					yetaP310002Service.getYe161005Data(ye161005Dto , new AsyncCallback<Ye161005DTO>() {
...
//				
//				
//				int  lsCnt = YetaP310002GridPanel.getMsfGrid().getGrid().getStore().getCount();
//          	  
//          	  	for(int iCnt=0;iCnt < lsCnt;iCnt++) {
//          	  		YetaP310002GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
//          	  		YetaP310002GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
//          	  	} 
//          	  	YetaP310002GridPanel.getMsfGrid().getGrid().getView().refresh(true);
//          	  	
//			}
//		});
//		btnBar.add(btnAllOn);
...
//          	  
//          	  	for(int iCnt=0;iCnt < lsCnt;iCnt++) {
//          	  		YetaP310002GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
//          	  		YetaP310002GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
//          	  	} 
//          	  	YetaP310002GridPanel.getMsfGrid().getGrid().getView().refresh(true);
//          	  	
...
//		lcSchCol3.add(layoutContainer27,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.34)); 	// 공백
	...
	private void createStandardForm() {
		...
		cpYeta2100.add(layoutContainer);
	...
	private LayoutContainer payrModiListInfo() {
		...
		YetaP310002GridPanel = new MSFGridPanel(ye161020Def, false, false, false, false);
		YetaP310002GridPanel.setHeaderVisible(false);
		YetaP310002GridPanel.setBodyBorder(false);
		YetaP310002GridPanel.setBorders(true);
		YetaP310002GridPanel.getBottomComponent().setVisible(true);

		
        final Grid<BaseModel> grid = YetaP310002GridPanel.getMsfGrid().getGrid();
        ...
        btnOnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {   
        	...
        	public void componentSelected(ButtonEvent ce) {  
        		...
        		if("".equals(creUtDpobCd.getValue()) || creUtDpobCd.getValue() == null) {
            		...
            		Iterator<BaseModel> itBm  = YetaP310002GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
        ...
        //툴바 넣어주는것
        YetaP310002GridPanel.setBottomComponent(bottomToolbar);
        
		

		cp01.add(YetaP310002GridPanel);
	...
	public void reload() {
		...
		// 그리드 초기화
		YetaP310002GridPanel.getMsfGrid().clearData();
		...
		YetaP310002GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP310002GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", srhClutYr.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP310002GridPanel.getTableDef().addColumnFilter("clutSeptCd", clutSeptCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
				 
		YetaP310002GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", payrMangDeptCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP310002GridPanel.getTableDef().addColumnFilter("emymtDivCd", emymtDivCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 		
		
		
		YetaP310002GridPanel.getTableDef().addColumnFilter("deptCd", deptCd.getValue() , SimpleColumnFilter.OPERATOR_EQUALS);
		YetaP310002GridPanel.getTableDef().addColumnFilter("typOccuCd", typOccuCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP310002GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", dtilOccuInttnCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		
		YetaP310002GridPanel.getTableDef().addColumnFilter("businCd", businCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP310002GridPanel.getTableDef().addColumnFilter("hanNm", srhHanNm.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP310002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP310002GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   
		
		
		YetaP310002GridPanel.getTableDef().addColumnFilter("utDpobCd", creUtDpobCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
		YetaP310002GridPanel.getTableDef().addColumnFilter("utDpobChk", utDpobChk.getValue()? "1" : "0", SimpleColumnFilter.OPERATOR_EQUALS);
		
		YetaP310002GridPanel.reload();
...
//				creEdacRvyy.setValue(lscreClutYrStore.findModel("year","2016"));
	...
	private void saveYeta161010(List<Ye161010DTO> lsYe161010Dto) {
		...
		yetaP310002Service.saveYetaP310002ToYe161010(lsYe161010Dto , new AsyncCallback<Integer>() {
			public void onFailure(Throwable caught) {
				MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
...
//		}
	...
	//		return;
		...
		yetaP310002Service.saveYetaP310002ToYeta161005(ye161005Dto , actionDatabase ,  new AsyncCallback<Integer>() {
			@Override
               public void onFailure(Throwable caught) {
            	   MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
