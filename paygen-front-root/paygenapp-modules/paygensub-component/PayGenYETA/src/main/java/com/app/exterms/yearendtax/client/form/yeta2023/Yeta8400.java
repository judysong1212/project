...
import com.app.exterms.yearendtax.client.form.defs.yeta2023.Ye18Ta1000Def;
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8400Service;
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8400ServiceAsync;
 ...
 * =====================================================
 **/
public class Yeta8400 extends MSFPanel {
	...
	private FormPanel plFrmYeta8400;
...
//	private MSFGridPanel typOccuCdGridPanel;
	 ...
	 **/
	// ---------- 상단 버튼 시작 ----------
	private ButtonBar topYeta8400Bar;
	private Button btnYeta8400Reset;	//초기화
	private Button btnYeta8400Save;		//저장
	private Button btnYeta8400Search;	//조회
	// ---------- 상단 버튼 종료 ----------
	
	// ---------- 엑셀 버튼 시작 ----------
	private ButtonBar exYeta8400Bar;	
...
//	private Yeta0100ServiceAsync yeta0100Service = Yeta0100Service.Util.getInstance();
	private Yeta8400ServiceAsync yeta8400Service = Yeta8400Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
	 ...
	 */
	private void checkYeta8400Auth(String authAction, ListStore<BaseModel> bm) {
...
//			formData = new FormData("-650");
			...
			createYeta8400Form();	// 화면 기본정보를 설정
	...
	public Yeta8400(String txtForm) {
...
//
		...
		plFrmYeta8400.setLayout(new FlowLayout());
		...
		lscreEmymtDivCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta8400Auth("creEmymtDivCd", lscreEmymtDivCd);
				}
			}
		});
		...
		creDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta8400Auth("creDeptCd", lscreDeptCd);
				}
			}
		});
		...
		creTypOccuCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					//checkYeta8400Auth("creTypOccuCd", lsTypOccuCd);
				}
			}
		});
...
//		PayrMangDeptCd = new HiddenField<String>(); //급여관리부서코드 
		...
		// 파일업로드 처리
		plFrmYeta8400.setAction("myurl");
		plFrmYeta8400.setEncoding(Encoding.MULTIPART);
		plFrmYeta8400.setMethod(Method.POST);

		plFrmYeta8400.setSize("990px", "820px");
		plFrmYeta8400.setLayout(new FlowLayout());
...
//		layoutContainer1_4.setStyleAttribute("marginBottom", "10px");
		...
		exYeta8400Bar = new ButtonBar();
		exYeta8400Bar.setAlignment(HorizontalAlignment.RIGHT);

		/*
		btnYetaVerifiction= new Button("신고사전검증");
		exYeta8400Bar.add(btnYetaVerifiction);
		...
		exYeta8400Bar.add(btnYetaExImport);

		btnYetaExSave= new Button("신고파일생성");
		exYeta8400Bar.add(btnYetaExSave);
		...
		layoutContainer11.add(exYeta8400Bar);
		...
		plFrmYeta8400.add(layoutContainer1_4,new com.extjs.gxt.ui.client.widget.layout.ColumnData(1));	// 텍스트와 버튼
...
//		});
	...
	private void createYeta8400Form() {

		plFrmYeta8400 = new FormPanel();
		plFrmYeta8400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산신고파일생성"));
		plFrmYeta8400.setIcon(Resources.APP_ICONS.text());
		plFrmYeta8400.setPadding(2);
		plFrmYeta8400.setFrame(true);
		...
		plFrmYeta8400.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta8400.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/
		
		   //멀티콤보박스 닫기 
		plFrmYeta8400.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
		            @Override
		            public void handleEvent(ComponentEvent ce) { 
...
//						                        }
	...
	private void createSearchForm() {
		...
		topYeta8400Bar = new ButtonBar();
		topYeta8400Bar.setAlignment(HorizontalAlignment.RIGHT);

		btnYeta8400Reset = new Button("초기화");
		btnYeta8400Reset.setIcon(MSFMainApp.ICONS.new16());
		btnYeta8400Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				resetStandardForm();	//전자신고기본정보 초기화
			}
		});
		topYeta8400Bar.add(btnYeta8400Reset);

//		topBtn = new Button("신 규");
//		topBtn.setIcon(MSFMainApp.ICONS.new16());
//		topYeta8400Bar.add(topBtn);
...
//		buttonBar.add(topBtn);
		...
		btnYeta8400Save = new Button("저 장");
		btnYeta8400Save.setIcon(MSFMainApp.ICONS.save16());
		btnYeta8400Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			@Override
		...
		topYeta8400Bar.add(btnYeta8400Save);

		btnYeta8400Search = new Button("조 회");
		btnYeta8400Search.setIcon(MSFMainApp.ICONS.search());
		btnYeta8400Search.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
...
//				ye161005Bm.setSrhBusinCd(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"));
				...
				yeta8400Service.getYeta8400List(ye161005Dto, new AsyncCallback<Ye161005BM>(){
		...
		topYeta8400Bar.add(btnYeta8400Search);

//		topBtn = new Button("인 쇄");
//		topBtn.setIcon(MSFMainApp.ICONS.print16());
//		buttonBar.add(topBtn);
		
		plFrmYeta8400.add(topYeta8400Bar);
		...
		lsEmymtDivCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta8400Auth("srhEmymtDivCd", lsEmymtDivCd);
				}
			}
		});
		...
		srhDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta8400Auth("srhDeptCd", lsDeptCd);
				}
			}
		});
		...
		srhTypOccuCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					//checkYeta8400Auth("srhTypOccuCd", lsTypOccuCd);
				}
			}
		});
...
//        cpGrid.add(gridFieldSet);
		...
		plFrmYeta8400.add(gridFieldSet);
//		plFrmYeta8400.add(msfGridPanel);
	}
...
//		msfGridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);//사업
	...
	/** 정산 마감 및 풀기 **/
	private void finishYeta8400(final String flag) { 
...
//	    				ye161005Bm.setBusinCd(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"));
	    			...
	    			//	ye17Ta1010Dto.setBusoprRgstnum(MSFSharedUtils.getSelectedComboValue(srhBusoprRgstnum, "busoprRgstnum"));
	      				...
	      				yeta8400Service.finishOnYe161005(ye17Ta1010Dto, new AsyncCallback<Long>(){
	                         public void onFailure(Throwable caught) {
...
//			} else { 
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta8400Export.do","extgwtFrame" ,param);   
			//}
		}else{ 
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta8400Export.do","extgwtFrame" ,param);  
		}
...
//		ye161005ExDto.setDpobCd(dpobCd.getValue());    														/** column 사업장코드 : dpobCd */
		...
		yeta8400Service.updateYe161005(ye161005ExDto, new AsyncCallback<Long>(){
			@Override
			public void onFailure(Throwable caught) {
...
//		param.put("businCd", MSFSharedUtils.getSelectedComboValue(creBusinCd,"businCd"));
		...
		if ("Y0090003".equals(MSFSharedUtils.allowNulls(param.get("creFile")))) {
			...
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/FileYeta8400Export.do","extgwtFrame" ,param);   
			//}
		}else{ 
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/FileYeta8400Export.do","extgwtFrame" ,param);  
		}	
...
//		param.put("businCd", MSFSharedUtils.getSelectedComboValue(creBusinCd,"businCd"));
		...
		if(MSFSharedUtils.paramNull(param.get("resuNumb"))){
			...
			MessageBox.info("", "중간 생성부 부서 또는 원천징수부서는 필수 입니다.(택1)", null);
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/Yeta8400Verifiction.do","extgwtFrame" ,param);   
