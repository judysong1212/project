...
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye161020Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP210002Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP210002ServiceAsync;
...
=====================================================**/
public  class YetaP210002   extends MSFFormPanel {  
 ...
 **/
	...
	private Ye161020Def ye161020Def = new Ye161020Def("YetaP210002");
	private MSFGridPanel yetaP210002GridPanel;
 ...
 **/ 
	...
	private YetaP210002ServiceAsync yetaP210002Service = YetaP210002Service.Util.getInstance();
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP210002(ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		btnSrh.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				if(creUtDpobCd.getValue() != null && !"".equals(creUtDpobCd.getValue()) ) {
					...
					yetaP210002Service.getYe161005Data(ye161005Dto , new AsyncCallback<Ye161005DTO>() {
...
//				
//				
//				int  lsCnt = yetaP210002GridPanel.getMsfGrid().getGrid().getStore().getCount();
//          	  
//          	  	for(int iCnt=0;iCnt < lsCnt;iCnt++) {
//          	  		yetaP210002GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
//          	  		yetaP210002GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
//          	  	} 
//          	  	yetaP210002GridPanel.getMsfGrid().getGrid().getView().refresh(true);
//          	  	
//			}
//		});
//		btnBar.add(btnAllOn);
...
//          	  
//          	  	for(int iCnt=0;iCnt < lsCnt;iCnt++) {
//          	  		yetaP210002GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
//          	  		yetaP210002GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
//          	  	} 
//          	  	yetaP210002GridPanel.getMsfGrid().getGrid().getView().refresh(true);
//          	  	
...
//		lcSchCol3.add(layoutContainer27,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.34)); 	// 공백
	...
	private LayoutContainer payrModiListInfo() {
		...
		yetaP210002GridPanel = new MSFGridPanel(ye161020Def, false, false, false, false);
		yetaP210002GridPanel.setHeaderVisible(false);
		yetaP210002GridPanel.setBodyBorder(false);
		yetaP210002GridPanel.setBorders(true);
		yetaP210002GridPanel.getBottomComponent().setVisible(true);

		
        final Grid<BaseModel> grid = yetaP210002GridPanel.getMsfGrid().getGrid();
        ...
        btnOnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {   
        	...
        	public void componentSelected(ButtonEvent ce) {  
        		...
        		if("".equals(creUtDpobCd.getValue()) || creUtDpobCd.getValue() == null) {
            		...
            		Iterator<BaseModel> itBm  = yetaP210002GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
        ...
        //툴바 넣어주는것
        yetaP210002GridPanel.setBottomComponent(bottomToolbar);
        
		

		cp01.add(yetaP210002GridPanel);
	...
	public void reload() {
		...
		// 그리드 초기화
		yetaP210002GridPanel.getMsfGrid().clearData();
		...
		yetaP210002GridPanel.getTableDef().setTableColumnFilters(filters);
		
		yetaP210002GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", srhClutYr.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		yetaP210002GridPanel.getTableDef().addColumnFilter("clutSeptCd", clutSeptCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
				 
		yetaP210002GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", payrMangDeptCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		yetaP210002GridPanel.getTableDef().addColumnFilter("emymtDivCd", emymtDivCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 		
		
		
		yetaP210002GridPanel.getTableDef().addColumnFilter("deptCd", deptCd.getValue() , SimpleColumnFilter.OPERATOR_EQUALS);
		yetaP210002GridPanel.getTableDef().addColumnFilter("typOccuCd", typOccuCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		yetaP210002GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", dtilOccuInttnCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		
		yetaP210002GridPanel.getTableDef().addColumnFilter("businCd", businCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		yetaP210002GridPanel.getTableDef().addColumnFilter("hanNm", srhHanNm.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		yetaP210002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		yetaP210002GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   
		
		
		yetaP210002GridPanel.getTableDef().addColumnFilter("utDpobCd", creUtDpobCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
		yetaP210002GridPanel.getTableDef().addColumnFilter("utDpobChk", utDpobChk.getValue()? "1" : "0", SimpleColumnFilter.OPERATOR_EQUALS);
		
		
		
		yetaP210002GridPanel.reload();
...
//				creEdacRvyy.setValue(lscreClutYrStore.findModel("year","2016"));
	...
	private void saveYeta161010(List<Ye161010DTO> lsYe161010Dto) {
		...
		yetaP210002Service.saveYetaP210002ToYe161010(lsYe161010Dto , new AsyncCallback<Integer>() {
			public void onFailure(Throwable caught) {
				MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
...
//		}
	...
	//		return;
		...
		yetaP210002Service.saveYetaP210002ToYeta161005(ye161005Dto , actionDatabase ,  new AsyncCallback<Integer>() {
			@Override
               public void onFailure(Throwable caught) {
            	   MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
