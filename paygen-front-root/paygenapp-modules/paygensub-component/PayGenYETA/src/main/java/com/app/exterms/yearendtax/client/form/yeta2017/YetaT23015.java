...
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23015Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23015ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT23015Def;
...
=====================================================**/
public  class YetaT23015   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT23015; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaT23015GridPanel;
//	private YetaT23015Def yetaT23015Def = new YetaT23015Def("YetaT23015");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT23015ServiceAsync yetaT23015Service = YetaT23015Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT23015Service.activityOnYetaT23015(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT23015(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT23015 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT23015.setBodyBorder(false);
		cpYetaT23015.setBorders(false);
		cpYetaT23015.setHeaderVisible(false);
		cpYetaT23015.setSize("790px", "640px");
		   
		this.add(cpYetaT23015);
...
//		     });     
		...
		cpYetaT23015.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				yetaT23015GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = yetaT23015GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        yetaT23015GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        yetaT23015GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        yetaT23015GridPanel.getMsfGrid().getGrid().startEditing(yetaT23015GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = yetaT23015GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(yetaT23015GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT23015.add(topYetaBtnBar);
...
//		cpYetaT23015.add(topYetaBtnBar);
//		
//		yetaT23015GridPanel = new MSFGridPanel(yetaT23015Def, false, false, false, false, false);
//		yetaT23015GridPanel.setSize("790px", "420px");
//		yetaT23015GridPanel.setBorders(true);
//		yetaT23015GridPanel.setHeaderVisible(false);
...
//		
//		yetaT23015GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		yetaT23015GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
 //       cpYetaT23015.add(yetaT23015GridPanel);
		FieldSet fieldSet = new FieldSet();
		...
		cpYetaT23015.add(fieldSet);
		cpYetaT23015.add(layoutContainer6);
		cpYetaT23015.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			yetaT23015GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaT23015GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaT23015GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaT23015GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaT23015GridPanel.reload();
