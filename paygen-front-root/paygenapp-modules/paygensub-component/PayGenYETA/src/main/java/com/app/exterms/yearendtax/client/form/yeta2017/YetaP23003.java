...
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye165010BM;
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye165010DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye165010Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23003Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23003ServiceAsync;
...
=====================================================**/
public  class YetaP23003   extends MSFFormPanel {  
 ...
 **/
	...
	*#######################################################*/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP23003GridPanel;
//	private Yeta0300Def yeta0300Def_08  = new Yeta0300Def("YETA03008");	//가족교육비 그리드 테이블 컬럼 define 
//	private Ye16Ta3180Def yetaP23003ToYe165010  = new Ye16Ta3180Def("YETAP23003");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye165010Def yetaP23003ToYe165010  = new Ye165010Def("YETAP23003");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP23003ServiceAsync yetaP23003Service = YetaP23003Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 	
	...
	private void yetaP23003FormSave() {
...
//				}
			...
			if (dataChk) {
				...
				yetaP23003Service.activityOnYetaP23003ToYe165010(listYe165010Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP23003ToYe165010(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	...
	public YetaP23003(final ActionDatabase actionDatabase, final MSFPanel caller) {
	...
	private void createSearchForm() {
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				yetaP23003GridPanel.getMsfGrid().clearData();
					 
				BaseModel bmData = yetaP23003GridPanel.getTableDef().getDefaultValues();  
				...
				yetaP23003GridPanel.getMsfGrid().getGrid().stopEditing();
				yetaP23003GridPanel.getMsfGrid().getGrid().getStore().insert(bmData, 0);
				yetaP23003GridPanel.getMsfGrid().getGrid().startEditing(yetaP23003GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);
...
//				actionDatabase = ActionDatabase.INSERT;
				...
				MessageBox.confirm("삭제", "선택하신 기부금 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
							...
							Iterator<BaseModel> itBm = yetaP23003GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP23003GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	      layoutContainer_2.setBorders(false);
	...
	private void createMsfGridForm() {
		...
		yetaP23003GridPanel = new MSFGridPanel(yetaP23003ToYe165010, false, false, false, false, false);
		yetaP23003GridPanel.setHeaderVisible(false);  
		yetaP23003GridPanel.setBorders(false);
		yetaP23003GridPanel.setHeaderVisible(false);
		yetaP23003GridPanel.getBottomComponent().setVisible(true);
		...
		yetaP23003GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			

		cp01.add(yetaP23003GridPanel);
	...
	public void reload() {
		...
		yetaP23003GridPanel.getTableDef().setTableColumnFilters(filters);
		yetaP23003GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		yetaP23003GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		yetaP23003GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		yetaP23003GridPanel.reload();
