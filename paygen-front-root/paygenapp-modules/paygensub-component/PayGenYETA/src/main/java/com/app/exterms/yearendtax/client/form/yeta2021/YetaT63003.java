...
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63002Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63002ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT63002Def;
...
=====================================================**/
public  class YetaT63003   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63002; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaT63002GridPanel;
//	private YetaT63002Def yetaT63002Def = new YetaT63002Def("YetaT63002");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63002ServiceAsync yetaT63002Service = YetaT63002Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63002Service.activityOnYetaT63002(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63003(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63002 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63002.setBodyBorder(false);
		cpYetaT63002.setBorders(false);
		cpYetaT63002.setHeaderVisible(false);
		cpYetaT63002.setSize("790px", "640px");
		   
		this.add(cpYetaT63002);
...
//		     });     
		...
		cpYetaT63002.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				yetaT63002GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = yetaT63002GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        yetaT63002GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        yetaT63002GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        yetaT63002GridPanel.getMsfGrid().getGrid().startEditing(yetaT63002GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = yetaT63002GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaT63002GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT63002.add(topYetaBtnBar);
...
//		yetaT63002GridPanel = new MSFGridPanel(yetaT63002Def, false, false, false, false, false);
//		yetaT63002GridPanel.setSize("790px", "420px");
//		yetaT63002GridPanel.setBorders(true);
//		yetaT63002GridPanel.setHeaderVisible(false);
//		
//      cpYetaT63002.add(yetaT63002GridPanel);
...
//		cpYetaT63002.add(fldstNewFieldset10);
		cpYetaT63002.add(fldstNewFieldset);
		cpYetaT63002.add(fldstNewFieldset1);
		cpYetaT63002.add(layoutContainer14);
		cpYetaT63002.add(layoutContainer15);
	...
	public void reload() {
			...
			yetaT63002GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaT63002GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaT63002GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaT63002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaT63002GridPanel.reload();
