...
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye160404BM;
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye160404DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye160404Def;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43006Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43006ServiceAsync;
...
=====================================================**/
public  class YetaP43006   extends MSFFormPanel {  
 ...
 **/
	...
	private ContentPanel cpYeta23006;
	...
	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP43006GridPanel;
	private Ye160404Def YetaP43006ToYe160404Def = new Ye160404Def("YetaP43006"); // 종전근무지내역 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료 ---------------
   ...
   */
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP43006ServiceAsync yetaP43006Service = YetaP43006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP43006(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta23006 = new ContentPanel();

		createSearchForm(); 	// 기본정보 필드를 적용
		createMsfGridForm(); 	// 그리드필드
		createStandardForm(); 	// 기본정보필드

		cpYeta23006.setBodyBorder(false);
		cpYeta23006.setBorders(false);
		cpYeta23006.setHeaderVisible(false);
//		cpYeta23006.setSize("790px", "640px");
		cpYeta23006.setSize("930px", "640px");

		this.add(cpYeta23006);
...
//		this.setSize("810px", "710px");
	...
	private void createSearchForm() {
		...
		cpYeta23006.setLayout(new FlowLayout());
				...
				MessageBox.confirm("삭제", "선택하신 종전근무지 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP43006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaP43006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
	...
	private void createStandardForm() {
		...
		btnNewButton.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				BaseModel bmData = YetaP43006GridPanel.getCurrentlySelectedItem();
...
//				setPPRecord(bmData); //팝업에 넘길값 
				...
				// 전체내역
				YetaP430010 yetaP230010Form = new YetaP430010(ActionDatabase.UPDATE,getThis()); 
						    
				MSFFormWindows msFwYeta230010 = new MSFFormWindows("비과세 및 감면소득", yetaP230010Form, "닫기", "820px", "750px", true);
				msFwYeta230010.show();
				yetaP230010Form.setMSFFormWindows(msFwYeta230010);

				yetaP230010Form.bind(ppRecord);
			}
		});
		btnNewButton.hide();
	...
	private void createMsfGridForm() {
		...
		YetaP43006GridPanel = new MSFGridPanel(YetaP43006ToYe160404Def, false,false, false, false, false);
		YetaP43006GridPanel.setHeaderVisible(false);
		YetaP43006GridPanel.setSize("915px", "200px");
		YetaP43006GridPanel.setBorders(false);
		YetaP43006GridPanel.setHeadingText("");
		...
		YetaP43006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  시작 --- **/
		
		final Grid YetaP43006Grid = YetaP43006GridPanel.getMsfGrid().getGrid();
		YetaP43006Grid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				// TODO Auto-generated method stub
				//그리드 선택시
				actionDatabase = ActionDatabase.UPDATE;
				yetaP13006ToYeta3200Detail(YetaP43006GridPanel.getCurrentlySelectedItem());
				btnNewButton.show();
			}
		});
		
		gridFieldSet.add(YetaP43006GridPanel);

		cpYeta23006.add(gridFieldSet);
	}

	private YetaP43006 getThis() {
		return this;
	}
	...
	private void YetaP43006FormSave() {
...
//    				ye160404Dto.setPrcspWhdgReipSumtYn(MSFSharedUtils.allowNulls(bmMapModel.get("prcspWhdgReipSumtYn")));    	/** column 종전근무지원천징수영수증제출여부 : prcspWhdgReipSumtYn */				
    			...
    			yetaP43006Service.activityOnYetaP43006ToYe160404(listYe160404Dto, actionDatabase, new AsyncCallback<Long>(){
    				public void onFailure(Throwable caught) {
    					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
    							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP43006ToYe160404(" + actionDatabase.name() + ") : " + caught)
    							, null);
    					}
...
//		hanNm01.setValue("");				// 성명   
	...
	public void reload() {
		...
		YetaP43006GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP43006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 연말정산귀속년도 
		
		YetaP43006GridPanel.getTableDef().addColumnFilter("clutSeptCd",clutSeptCd.getValue()
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 정산구분
		
		YetaP43006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 시스템키
		
//		YetaP43006GridPanel.getTableDef().addColumnFilter("whdgDebrBusoprRgstnum", MSFSharedUtils.allowNulls(whdgDebrBusoprRgstnum.getValue())
//				, SimpleColumnFilter.OPERATOR_EQUALS); // 원천징수의무자사업자등록번호
		
		YetaP43006GridPanel.reload();
