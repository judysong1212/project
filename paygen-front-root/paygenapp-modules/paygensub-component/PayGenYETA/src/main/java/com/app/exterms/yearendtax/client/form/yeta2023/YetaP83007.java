...
import com.app.exterms.yearendtax.client.dto.yeta2023.Ye162010BM;
...
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP83007Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP83007ServiceAsync;
...
=====================================================**/
public  class YetaP83007   extends MSFFormPanel {  
 ...
 **/
	...
	private ContentPanel cpYeta83007;
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP83007GridPanel;
//	private Ye16Ta3100Def YetaP83007ToYe162010Def  = new Ye16Ta3100Def("YETAP13007");	//종전근무지내역 그리드 테이블 컬럼 define 
	private Ye162010Def ye162010Def  = new Ye162010Def("YetaP83007");	//종전근무지내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP83007ServiceAsync yetaP83007Service = YetaP83007Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------		
     ...
     */  
		...
		private void YetaP83007FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				if(dataChk) {
					...
					yetaP83007Service.activityOnYetaP83007ToYe162010(listYe162010Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
		 ...
		 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 **/		  
	  public YetaP83007(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			initLoad(); 
	       ...
	       // this.setLayout(layout);   
	        ...
	        cpYeta83007 = new ContentPanel();  
	        
	        createSearchForm();			//검색필드를 적용
	        createMsfGridForm();		//그리드필드
		   
	           
		   cpYeta83007.setBodyBorder(false);
		   cpYeta83007.setBorders(false);
		   cpYeta83007.setHeaderVisible(false);
		   cpYeta83007.setSize("790px", "530px");
		   
		   this.add(cpYeta83007);
...
//			     });     
			...
			cpYeta83007.setLayout(new FlowLayout());
			...
			btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 ...
					 YetaP83007GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = YetaP83007GridPanel.getTableDef().getDefaultValues();  
			         ...
			         YetaP83007GridPanel.getMsfGrid().getGrid().stopEditing();      
			         YetaP83007GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         YetaP83007GridPanel.getMsfGrid().getGrid().startEditing(YetaP83007GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
					...
					MessageBox.confirm("삭제", "선택하신 연금저축 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
						...
						public void handleEvent(MessageBoxEvent be) {
							if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
								...
								Iterator<BaseModel> itBm = YetaP83007GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					...
					setListRecord(YetaP83007GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
					doAction(actionDatabase);
...
//	            	reload();
		...
		private void createMsfGridForm() {
			...
			YetaP83007GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
//			YetaP83007GridPanel.setSize("790px", "300px");
			YetaP83007GridPanel.setBorders(false);
			YetaP83007GridPanel.setHeaderVisible(false);
			...
			YetaP83007GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			/** --- 그리드 합계 부분  시작 --- **/
			
			cpYeta83007.add(YetaP83007GridPanel);
...
//			layoutContainer.setBorders(false);
			...
			cp01.add(YetaP83007GridPanel);
			
			cpYeta83007.add(cp01);
			
			
//			cpYeta83007.add(layoutContainer);
	}
	...
	public void reload() {
		...
		YetaP83007GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP83007GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP83007GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
															, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP83007GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP83007GridPanel.reload();
