...
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73018Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73018ServiceAsync;
...
=====================================================**/
public  class YetaT73018   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73018; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73018GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT73018ServiceAsync yetaT73018Service = YetaT73018Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT73018Service.activityOnYetaT73018(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT73018(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73018 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73018.setBodyBorder(false);
		cpYetaT73018.setBorders(false);
		cpYetaT73018.setHeaderVisible(false);
		cpYetaT73018.setSize("790px", "640px");
		   
		this.add(cpYetaT73018);
...
//		     });     
		...
		cpYetaT73018.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT73018GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT73018GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT73018GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT73018GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT73018GridPanel.getMsfGrid().getGrid().startEditing(YetaT73018GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT73018GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//				
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT73018GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//				
		cpYetaT73018.add(topYetaBtnBar);
...
//		YetaT73018GridPanel = new MSFGridPanel(YetaT73018Def, false, false, false, false, false);
//		YetaT73018GridPanel.setSize("790px", "420px");
//		YetaT73018GridPanel.setBorders(true);
//		YetaT73018GridPanel.setHeaderVisible(false);
...
//		
//		YetaT73018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT73018GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT73018.add(YetaT73018GridPanel);
		...
		cpYetaT73018.add(layoutContainer);
	...
	public void reload() {
			...
			YetaT73018GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73018GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73018GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73018GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73018GridPanel.reload();
