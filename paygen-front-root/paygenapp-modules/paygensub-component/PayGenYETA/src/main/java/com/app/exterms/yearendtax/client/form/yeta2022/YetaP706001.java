 ...
 * =====================================================
 **/
public class YetaP706001 extends MSFFormPanel {
	...
	private ContentPanel cpYetaP706001;

	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP706001_01GridPanel;
	private MSFGridPanel YetaP706001_02GridPanel;

	private Yeta7060Def Yeta706001MtDef = new Yeta7060Def("Yeta706001_Mt");
	private Yeta7060Def Yeta706001StDef = new Yeta7060Def("Yeta706001_St");
	// -------------- grid 선언 종료 ---------------
	...
	//	private YetaP2060ServiceAsync YetaP706001Service = YetaP706001Service.Util.getInstance();
	 ...
	 * ++++++++++++++
	 **/
	public YetaP706001(final ActionDatabase actionDatabase,	final MSFPanel caller) {
		...
		cpYetaP706001 = new ContentPanel();

		createGridForm();

		mReload();
		cpYetaP706001.setBodyBorder(false);
		cpYetaP706001.setBorders(false);
		cpYetaP706001.setHeaderVisible(false);
		cpYetaP706001.setSize("790px", "640px");

		this.add(cpYetaP706001);
	...
	private void createGridForm() {
		...
		YetaP706001_01GridPanel = new MSFGridPanel(Yeta706001MtDef, false, false, false, false, false);
		YetaP706001_01GridPanel.setHeaderVisible(false);
		YetaP706001_01GridPanel.setSize(305, 570);
		YetaP706001_01GridPanel.setBorders(true);

		final Grid Yeta206001MGrid = YetaP706001_01GridPanel.getMsfGrid().getGrid();
		Yeta206001MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (YetaP706001_01GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = YetaP706001_01GridPanel.getCurrentlySelectedItem();
					sReload(bmData);
				}
			}
		}); 	

		fieldMtSet.add(YetaP706001_01GridPanel);
		...
		YetaP706001_02GridPanel = new MSFGridPanel(Yeta706001StDef, false, false, false, false, false);
		YetaP706001_02GridPanel.setHeaderVisible(false);
		YetaP706001_02GridPanel.setSize(460, 570);
		YetaP706001_02GridPanel.setBorders(true);
		
		fieldStSet.add(YetaP706001_02GridPanel);
		...
		btnBassSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (YetaP706001_02GridPanel.getCurrentlySelectedItem() == null) { 
		...
		cpYetaP706001.add(layoutContainer1, new FormData("100%"));
		cpYetaP706001.add(btnBar, new FormData("100%"));
	...
	public void mReload() {
		...
		YetaP706001_01GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP706001_01GridPanel.reload();
	...
	public void sReload(BaseModel bmData) {
		...
		YetaP706001_02GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP706001_02GridPanel.getTableDef().addColumnFilter("utDpobCd",MSFSharedUtils.allowNulls(bmData.get("utDpobCd")), SimpleColumnFilter.OPERATOR_LIKE);
		YetaP706001_02GridPanel.reload();
							...
							MessageBox.confirm("종사업장 등록", "선택된 사업장의 사업자번호가 주 사업자 번호로 변경됩니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
								...
								public void handleEvent(MessageBoxEvent be) {
									if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
										BaseModel bmData = YetaP706001_01GridPanel.getCurrentlySelectedItem();
										Yeta7060DTO dto = new Yeta7060DTO();
										...
										yeta7060Service.insertYetaS706001(dto, actionDatabase, new AsyncCallback<Integer>() {
											@Override
											public void onSuccess(Integer result) {	
												...
												//상세정보 초기화
												YetaP706001_01GridPanel.reload();
												YetaP706001_02GridPanel.getMsfGrid().clearData();
											}
		...
		MessageBox.confirm("사업장 삭제", "선택된 사업장을 삭제 하시겠습니까?<br>",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = YetaP706001_02GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator(); 
					List<Record>  lsRec  = new ArrayList<Record>();
					List<Yeta7060DTO> listYeta7060Dto = new ArrayList<Yeta7060DTO>();       
					...
					if (MSFSharedUtils.paramNotNull(records)) {   
						...
						while (records.hasNext()) {
							...
							listYeta7060Dto.add(dto);
						}
					}   


					if(listYeta7060Dto.size() <= 0 ) {
						MessageBox.info("경고", "삭제할 사업장을 선택해 주세요.", null);
						return;
					}

					yeta7060Service.Yeta706001_Delete(listYeta7060Dto, actionDatabase, new AsyncCallback<Long>() {
						@Override
			...
			MessageBox.confirm("주사업자 수정", "선택된 사업장의 일련번호를 변경합니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
				...
				public void handleEvent(MessageBoxEvent be) {
					if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
						...
						BaseModel bmData1 = YetaP706001_01GridPanel.getCurrentlySelectedItem();
						...
						yeta7060Service.Yeta706001NumUpdate(listYeta7060Dto, actionDatabase, new AsyncCallback<Long>() {
							@Override
							public void onSuccess(Long result) {	
								MessageBox.info("","저장 되었습니다.", null);
