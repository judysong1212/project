...
=====================================================**/
public  class YetaT83019   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT83019; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT83019GridPanel;
	private Ye162510Def YetaT83019ToYe162510Def = new Ye162510Def("YETAT13019");   //월세새액공제명세 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료  ---------------
...
//		 }
	...
	public YetaT83019(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT83019 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT83019.setBodyBorder(false);
		cpYetaT83019.setBorders(false);
		cpYetaT83019.setHeaderVisible(false);
		cpYetaT83019.setSize("790px", "640px");
		   
		this.add(cpYetaT83019);
...
//		     });     
		...
		cpYetaT83019.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaT83019GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = YetaT83019GridPanel.getTableDef().getDefaultValues();  
		        ...
		        YetaT83019GridPanel.getMsfGrid().getGrid().stopEditing();      
		        YetaT83019GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        YetaT83019GridPanel.getMsfGrid().getGrid().startEditing(YetaT83019GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT83019GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT83019GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT83019.add(topYetaBtnBar);
...
//		cpYetaT83019.add(topYetaBtnBar);
		...
		YetaT83019GridPanel = new MSFGridPanel(YetaT83019ToYe162510Def, false, false, false, false, false);
		YetaT83019GridPanel.setSize("790px", "200px");
		YetaT83019GridPanel.setBorders(true);
		YetaT83019GridPanel.setHeaderVisible(false);
...
//		
//		YetaT83019GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		/** --- 그리드 합계 부분  종료 --- **/
		
//   	cpYetaT83019.add(YetaT83019GridPanel);
		...
		fldstNewFieldset.add(YetaT83019GridPanel);
		cpYetaT83019.add(fldstNewFieldset);
		cpYetaT83019.add(layoutContainer1);
		cpYetaT83019.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT83019GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT83019GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT83019GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT83019GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT83019GridPanel.reload();
