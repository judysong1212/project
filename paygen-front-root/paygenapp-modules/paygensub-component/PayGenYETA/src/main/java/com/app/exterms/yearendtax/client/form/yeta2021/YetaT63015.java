...
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63015Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63015ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT63015Def;
...
=====================================================**/
public  class YetaT63015   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63015; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63015GridPanel;
//	private YetaT63015Def YetaT63015Def = new YetaT63015Def("YetaT63015");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63015ServiceAsync yetaT63015Service = YetaT63015Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63015Service.activityOnYetaT63015(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63015(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63015 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63015.setBodyBorder(false);
		cpYetaT63015.setBorders(false);
		cpYetaT63015.setHeaderVisible(false);
		cpYetaT63015.setSize("790px", "640px");
		   
		this.add(cpYetaT63015);
...
//		     });     
		...
		cpYetaT63015.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63015GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63015GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63015GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63015GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63015GridPanel.getMsfGrid().getGrid().startEditing(YetaT63015GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT63015GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaT63015GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT63015.add(topYetaBtnBar);
...
//		cpYetaT63015.add(topYetaBtnBar);
//		
//		YetaT63015GridPanel = new MSFGridPanel(YetaT63015Def, false, false, false, false, false);
//		YetaT63015GridPanel.setSize("790px", "420px");
//		YetaT63015GridPanel.setBorders(true);
//		YetaT63015GridPanel.setHeaderVisible(false);
...
//		
//		YetaT63015GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63015GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
 //       cpYetaT63015.add(YetaT63015GridPanel);
		FieldSet fieldSet = new FieldSet();
		...
		cpYetaT63015.add(fieldSet);
		cpYetaT63015.add(layoutContainer6);
		cpYetaT63015.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63015GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63015GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63015GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63015GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63015GridPanel.reload();
