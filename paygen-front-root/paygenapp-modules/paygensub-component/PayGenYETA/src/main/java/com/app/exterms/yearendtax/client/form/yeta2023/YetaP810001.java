...
import com.app.exterms.yearendtax.client.form.defs.yeta2023.InfcPsnl0100Def;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP810001Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP810001ServiceAsync;
...
=====================================================**/
public  class YetaP810001   extends MSFFormPanel {  
	...
	private InfcPsnl0100Def YetaP810001def = new InfcPsnl0100Def("YetaP810001");
	private MSFGridPanel YetaP810001GridPanel;
	...
	private YetaP810001ServiceAsync yetaP810001Service = YetaP810001Service.Util.getInstance();
	...
	private ListStore<BaseModel> lsMangeDeptCd = new ListStore<BaseModel>(); 			// 관리부서콤보 / 04.27
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP810001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta81001.setSize("890px", "640px");
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lscrPayYr.findModel("year",String.valueOf("2023")));
	...
	private void createSearchForm() {
		...
		//---------------------멀티콤보박스 닫기 -------------------------------------------------
		cpYeta81001.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
...
// 부서
		...
		lcCol0201.add(srhMangeDeptCd, new FormData("100%"));	
	...
	private void createStandardForm() {
		...
		cpYeta81001.add(layoutContainer);
	...
	private LayoutContainer payrModiListInfo() {
		...
		YetaP810001GridPanel = new MSFGridPanel(YetaP810001def, false, false, false, false,false);
		YetaP810001GridPanel.setHeaderVisible(false);
		YetaP810001GridPanel.setBodyBorder(false);
		YetaP810001GridPanel.setBorders(true);
		YetaP810001GridPanel.getBottomComponent().setVisible(false);

		final Grid<BaseModel> grid = YetaP810001GridPanel.getMsfGrid().getGrid();
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = YetaP810001GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					YetaP810001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					YetaP810001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				YetaP810001GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					YetaP810001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					YetaP810001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		btnSelected.addSelectionListener(new SelectionListener<ButtonEvent>() {   
			...
			public void componentSelected(ButtonEvent ce) {  
				...
				Iterator<BaseModel> itBm = YetaP810001GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); // .getSelectionModel().getSelectedItems().listIterator();
				...
				while (iterRecords.hasNext()) {
					...
					if ((Boolean) bmMapModel.get("select")) {
						...
						ye161010Dto.setMangeDeptCd(MSFSharedUtils.allowNulls(bmMapModel.get("mangeDeptCd")) );					// 관리부서 수정 05.08
				...
				yetaP810001Service.saveYetaP810001ToYe161010(listYe161010Dto,listYe161020Dto, actionDatabase, new AsyncCallback<PagingLoadResult<ShowMessageBM>>() {
					...
					public void onFailure(Throwable caught) {
						...
						smForm.setWindowId("YetaP810001");
		...
		refresh.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				YetaP810001GridPanel.reload();	                	
			}  
		});		            
		bottomToolbar.add(refresh);

		//툴바 넣어주는것
		YetaP810001GridPanel.setBottomComponent(bottomToolbar);

		cp01.add(YetaP810001GridPanel);
	...
	public void reload() {
		...
		YetaP810001GridPanel.getTableDef().setTableColumnFilters(filters);

		YetaP810001GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP810001GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		
		...
		YetaP810001GridPanel.getTableDef().addColumnFilter("hdofcCodtnCd", strHdofcCodtnCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		// 관리부서 수정 05.08
		YetaP810001GridPanel.getTableDef().addColumnFilter("mangeDeptCd", MSFSharedUtils.getSelectedComboValue(srhMangeDeptCd,"mangeDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		YetaP810001GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		YetaP810001GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDtilOccuInttnCd = YetaUtils.getStrValToBMMultiCombo(lsDtilOccuInttnCd,srhDtilOccuInttnCd.getValue(),"dtilOccuClsNm","dtilOccuInttnCd");  
		YetaP810001GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP810001GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP810001GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP810001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP810001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr, "year")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP810001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP810001GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(srhUtDpobCd.getValue())  , SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	private void formInit() {
		...
		YetaP810001GridPanel.getMsfGrid().clearData();
