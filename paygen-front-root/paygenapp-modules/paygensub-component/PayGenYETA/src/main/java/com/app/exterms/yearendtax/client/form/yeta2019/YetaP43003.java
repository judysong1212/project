...
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye165010BM;
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye165010DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye165010Def;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43003Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43003ServiceAsync;
...
=====================================================**/
public  class YetaP43003   extends MSFFormPanel {  
 ...
 **/
	...
	*#######################################################*/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP43003GridPanel;
//	private Yeta0300Def yeta0300Def_08  = new Yeta0300Def("YETA03008");	//가족교육비 그리드 테이블 컬럼 define 
//	private Ye16Ta3180Def YetaP43003ToYe165010  = new Ye16Ta3180Def("YetaP43003");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye165010Def YetaP43003ToYe165010  = new Ye165010Def("YetaP43003");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP43003ServiceAsync yetaP43003Service = YetaP43003Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 	
	...
	private void YetaP43003FormSave() {
...
//				}
			...
			if (dataChk) {
				...
				yetaP43003Service.activityOnYetaP43003ToYe165010(listYe165010Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP43003ToYe165010(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	...
	public YetaP43003(final ActionDatabase actionDatabase, final MSFPanel caller) {
	...
	private void createSearchForm() {
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP43003GridPanel.getMsfGrid().clearData();
					 
				BaseModel bmData = YetaP43003GridPanel.getTableDef().getDefaultValues();  
				...
				YetaP43003GridPanel.getMsfGrid().getGrid().stopEditing();
				YetaP43003GridPanel.getMsfGrid().getGrid().getStore().insert(bmData, 0);
				YetaP43003GridPanel.getMsfGrid().getGrid().startEditing(YetaP43003GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);
...
//				actionDatabase = ActionDatabase.INSERT;
				...
				MessageBox.confirm("삭제", "선택하신 기부금 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
							...
							Iterator<BaseModel> itBm = YetaP43003GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP43003GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	      layoutContainer_2.setBorders(false);
	...
	private void createMsfGridForm() {
		...
		YetaP43003GridPanel = new MSFGridPanel(YetaP43003ToYe165010, false, false, false, false, false);
		YetaP43003GridPanel.setHeaderVisible(false);  
		YetaP43003GridPanel.setBorders(false);
		YetaP43003GridPanel.setHeaderVisible(false);
		YetaP43003GridPanel.getBottomComponent().setVisible(true);
		...
		YetaP43003GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			

		cp01.add(YetaP43003GridPanel);
	...
	public void reload() {
		...
		YetaP43003GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP43003GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP43003GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP43003GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP43003GridPanel.reload();
