...
import com.app.exterms.yearendtax.client.dto.yeta2020.Ye160404BM;
import com.app.exterms.yearendtax.client.dto.yeta2020.Ye160404DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Ye160404Def;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP53006Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP53006ServiceAsync;
...
=====================================================**/
public  class YetaP53006   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP53006GridPanel;
	private Ye160404Def YetaP53006ToYe160404Def = new Ye160404Def("YetaP53006"); // 종전근무지내역 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료 ---------------
   ...
   */
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP53006ServiceAsync yetaP53006Service = YetaP53006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP53006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		this.setSize("810px", "710px");
	...
	private void createSearchForm() {
				...
				MessageBox.confirm("삭제", "선택하신 종전근무지 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP53006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaP53006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
	...
	private void createStandardForm() {
		...
		btnNewButton.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				BaseModel bmData = YetaP53006GridPanel.getCurrentlySelectedItem();
...
//				setPPRecord(bmData); //팝업에 넘길값 
				...
				// 전체내역
				YetaP530010 yetaP230010Form = new YetaP530010(ActionDatabase.UPDATE,getThis()); 
	...
	private void createMsfGridForm() {
		...
		YetaP53006GridPanel = new MSFGridPanel(YetaP53006ToYe160404Def, false,false, false, false, false);
		YetaP53006GridPanel.setHeaderVisible(false);
		YetaP53006GridPanel.setSize("915px", "200px");
		YetaP53006GridPanel.setBorders(false);
		YetaP53006GridPanel.setHeadingText("");
		...
		YetaP53006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  시작 --- **/
		
		final Grid YetaP53006Grid = YetaP53006GridPanel.getMsfGrid().getGrid();
		YetaP53006Grid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				// TODO Auto-generated method stub
				//그리드 선택시
				actionDatabase = ActionDatabase.UPDATE;
				yetaP13006ToYeta3200Detail(YetaP53006GridPanel.getCurrentlySelectedItem());
				btnNewButton.show();
			}
		});
		
		gridFieldSet.add(YetaP53006GridPanel);

		cpYeta23006.add(gridFieldSet);
	}

	private YetaP53006 getThis() {
		return this;
	}
	...
	private void YetaP53006FormSave() {
...
//    				ye160404Dto.setPrcspWhdgReipSumtYn(MSFSharedUtils.allowNulls(bmMapModel.get("prcspWhdgReipSumtYn")));    	/** column 종전근무지원천징수영수증제출여부 : prcspWhdgReipSumtYn */				
    			...
    			yetaP53006Service.activityOnYetaP53006ToYe160404(listYe160404Dto, actionDatabase, new AsyncCallback<Long>(){
    				public void onFailure(Throwable caught) {
    					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
    							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP53006ToYe160404(" + actionDatabase.name() + ") : " + caught)
    							, null);
    					}
...
//		hanNm01.setValue("");				// 성명   
	...
	public void reload() {
		...
		YetaP53006GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP53006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 연말정산귀속년도 
		
		YetaP53006GridPanel.getTableDef().addColumnFilter("clutSeptCd",clutSeptCd.getValue()
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 정산구분
		
		YetaP53006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 시스템키
		
//		YetaP53006GridPanel.getTableDef().addColumnFilter("whdgDebrBusoprRgstnum", MSFSharedUtils.allowNulls(whdgDebrBusoprRgstnum.getValue())
//				, SimpleColumnFilter.OPERATOR_EQUALS); // 원천징수의무자사업자등록번호
		
		YetaP53006GridPanel.reload();
