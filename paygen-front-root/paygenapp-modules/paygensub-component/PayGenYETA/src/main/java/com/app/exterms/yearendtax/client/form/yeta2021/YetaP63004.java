...
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye162520BM;
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye162520DTO;
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye162530BM;
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye162530DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye162520Def;
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye162530Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63004Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63004ServiceAsync;
...
=====================================================**/
public  class YetaP63004   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta63004; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP63004GridPanel01;
	private Ye162520Def YetaP63004ToYeta3120Def  = new Ye162520Def("YetaP23004");	//금전소비대차계약 그리드 테이블 컬럼 define 
 ...
 **/ 
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP63004ServiceAsync yetaP63004Service = YetaP63004Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 	
	...
	private void doAction(ActionDatabase actionDatabase, String doActionFlag) {
		...
		case INSERT:
			...
			if ("YE162520".equals(doActionFlag)) {
				YetaP63004ToYe162520FormSave();
			} else if ("YE162530".equals(doActionFlag)) {
				YetaP63004ToYe162530FormSave();
			} else {
	...
	private void YetaP63004ToYe162520FormSave() {
...
//				ye162520Dto.setEctnFlTrnrCtlYn( ((Boolean) bmMapModel.get("ectnFlTrnrCtlYn")) ? "Y" : "N"  );								/** column 전자파일이관여부 : ectnFlTrnrCtlYn */
			...
			yetaP63004Service.activityOnYetaP63004ToYe162520(listYe162520Dto, actionDatabase, new AsyncCallback<Long>(){
				public void onFailure(Throwable caught) {
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP63004ToYe162520(" + actionDatabase.name() + ") : " + caught), null);
				}
	...
	private void YetaP63004ToYe162530FormSave() {  
		if (MSFSharedUtils.paramNotNull(records)) { 
...
//				}
			...
			yetaP63004Service.activityOnYetaP63004ToYe162530(listYe162530Dto, actionDatabase, new AsyncCallback<Long>(){
				public void onFailure(Throwable caught) {
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("YetaP63004ToYe162530FormSave(" + actionDatabase.name() + ") : " + caught), null);
				}
				public void onSuccess(Long result) { 
					if (result == 0) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), actionDatabase + "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
...
//		 }
	  ...
	  public YetaP63004(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			initLoad(); 
	       ...
	       // this.setLayout(layout);   
	        ...
	        cpYeta63004 = new ContentPanel();  

		    createSearchForm();			//검색필드를 적용
	        createMsfGridForm1();		//금전소비대차계약
	        createMsfGridForm2();		//임대차계약
	           
	        cpYeta63004.setBodyBorder(false);
	        cpYeta63004.setBorders(false);
	        cpYeta63004.setHeaderVisible(false);
	        cpYeta63004.setSize("790px", "570px");
		   
	        this.add(cpYeta63004);
	...
	private void createSearchForm() {
		...
		cpYeta63004.setLayout(new FlowLayout());
	...
	private void createMsfGridForm1() {
		...
		btnYe162520New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP63004GridPanel01.getMsfGrid().clearData();

				BaseModel bmData = YetaP63004GridPanel01.getTableDef().getDefaultValues();
...
//			    bmData.set("ectnFlTrnrCtlYn"		, false);    									/** column 전자파일이관여부 : ectnFlTrnrCtlYn */			    
			    ...
			    YetaP63004GridPanel01.getMsfGrid().getGrid().stopEditing();      
			    YetaP63004GridPanel01.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			    YetaP63004GridPanel01.getMsfGrid().getGrid().startEditing(YetaP63004GridPanel01.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 금전소비대차계약 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm  = YetaP63004GridPanel01.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYe162520Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP63004GridPanel01.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		cpYeta63004.add(topBtnYe162520Bar);

		FieldSet gridFieldSet = new FieldSet();
		gridFieldSet.setHeadingHtml("금전소비대차계약");
		gridFieldSet.setBorders(true);

		YetaP63004GridPanel01 = new MSFGridPanel(YetaP63004ToYeta3120Def,false, false, false, false, false);
		YetaP63004GridPanel01.setSize("790px", "190px");
		YetaP63004GridPanel01.setBorders(false);
		YetaP63004GridPanel01.setHeaderVisible(false);
		YetaP63004GridPanel01.getBottomComponent().setVisible(true);
...
//		payrSummaryRow.setSummaryFormat(Ye162520BM.ATTR_CNCLITRO, NumberFormat.getFormat("#,##0;(#,##0)"));  
		...
		YetaP63004GridPanel01.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			
		gridFieldSet.add(YetaP63004GridPanel01);
		cpYeta63004.add(gridFieldSet);
	}
...
//			    bmData.set("ectnFlTrnrCtlYn"		, false);    									/** column 전자파일이관여부 : ectnFlTrnrCtlYn */			    
		...
		cpYeta63004.add(topBtnYe162530Bar);
	...
	public void reloadYeta3120() {
		...
		YetaP63004GridPanel01.getTableDef().setTableColumnFilters(filters);
		YetaP63004GridPanel01.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP63004GridPanel01.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP63004GridPanel01.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
		YetaP63004GridPanel01.reload();
	}
