...
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye162040Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23010Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23010ServiceAsync;
...
=====================================================**/
public  class YetaT23010   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT23010; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaT23010GridPanel;
	private Ye162040Def ye162040Def = new Ye162040Def("YetaT23010");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT23010ServiceAsync yetaT23010Service = YetaT23010Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT23010Service.activityOnYetaT23010(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT23010(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT23010 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT23010.setBodyBorder(false);
		cpYetaT23010.setBorders(false);
		cpYetaT23010.setHeaderVisible(false);
		cpYetaT23010.setSize("790px", "640px");
		   
		this.add(cpYetaT23010);
...
//		     });     
		...
		cpYetaT23010.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				yetaT23010GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = yetaT23010GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        yetaT23010GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        yetaT23010GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        yetaT23010GridPanel.getMsfGrid().getGrid().startEditing(yetaT23010GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaT23010GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = yetaT23010GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT23010.add(topYetaBtnBar);
...
//		cpYetaT23010.add(topYetaBtnBar);
		...
		yetaT23010GridPanel = new MSFGridPanel(ye162040Def, false, false, false, false, false);
		yetaT23010GridPanel.setSize("790px", "200px");
		yetaT23010GridPanel.setBorders(true);
		yetaT23010GridPanel.setHeaderVisible(false);
...
//		
//		yetaT23010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		yetaT23010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT23010.add(yetaT23010GridPanel);
		...
		fldstNewFieldset.add(yetaT23010GridPanel);
		cpYetaT23010.add(fldstNewFieldset);
		cpYetaT23010.add(layoutContainer10);
		cpYetaT23010.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			yetaT23010GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaT23010GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaT23010GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaT23010GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaT23010GridPanel.reload();
