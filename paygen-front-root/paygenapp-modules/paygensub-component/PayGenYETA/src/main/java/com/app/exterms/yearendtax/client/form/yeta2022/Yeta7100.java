...
import com.app.exterms.yearendtax.client.service.yeta2022.Yeta7100ServiceAsync;
...
=====================================================**/
public class Yeta7100   extends MSFPanel {
	/**
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	 * 	전역변수 선언부  시작
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	...
	private FormPanel plFrmYeta7100;
	private String txtForm = "";
	private XTemplate detailTp;

	private Ye161010Def Yeta7100def = new Ye161010Def("Yeta7100");
	private MSFGridPanel Yeta7100GridPanel;
	...
	private Yeta7100ServiceAsync yeta7100Service = Yeta7100Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhClutYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2022")));
...
//		authMapDef.put("creDeptCd",Boolean.FALSE); 
	 ...
	 * 데이타를 삭제한다.
	 */
	private void Yeta7100_menuDelete() {
		...
		MessageBox.confirm("연말정산 대상자 삭제", "선택된 데이타를 삭제 하시겠습니까?<br>(연말정산 대상자를 삭제합니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = Yeta7100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
					...
					yeta7100Service.Yeta7100ToYe160401_Delete(listYe161010Dto, new AsyncCallback<Long>() {
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
									MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta7100ToYe160401_Delete(삭제) : " + caught), null);
						}
...
//
//		//Iterator<BaseModel> itBm = Yeta7100GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
//
//		Iterator<BaseModel> itBm = Yeta7100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator();
//
//		List<Record> lsRec = new ArrayList<Record>();
...
//			Yeta7100Service.UpDpobExec_Yeta_Insert_2017(listYe161010Dto, new AsyncCallback<Long>() {
//				public void onFailure(Throwable caught) {
//					tracker.setStatus(true);
//					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
...
//	}
	...
	private void PayrExec_Yeta_Insert() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			t.scheduleRepeating(500);
			tracker.setStatus(true);
			yeta7100Service.PayrExec_Yeta_2022_Insert(listYe161010Dto, new AsyncCallback<Long>() {
				public void onFailure(Throwable caught) {
					tracker.setStatus(true);
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("PayrExec_Yeta_Insert(급여자료이관) : " + caught), null);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 **/
	private void Yeta7100Print(String fileName, String repType) {
...
//			serarchParam += "" + YetaUtils.getPrintStrToMultiData(lsDeptCd, MSFSharedUtils.allowNulls(srhDeptCd.getValue()),"deptNmRtchnt","deptCd") +"⊥";		
			...
			List<BaseModel> list = Yeta7100GridPanel.getGrid().getSelectionModel().getSelectedItems();
	...
	public Yeta7100(String txtForm) {
	...
	private void createYeta0100Form() {
		...
		plFrmYeta7100 = new FormPanel();
		plFrmYeta7100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산대상자관리"));
		plFrmYeta7100.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta7100.setPadding(2);
		plFrmYeta7100.setFrame(true);
		...
		plFrmYeta7100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta7100.getHeader().addTool(btnlogView);
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		// 멀티콤보박스 닫기
		plFrmYeta7100.addListener(Events.OnClick, new Listener<ComponentEvent>() {
			@Override
		...
		btnYeta0100Init.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Yeta7100GridPanel.getMsfGrid().clearData();
			}
		});
		...
		btnYeta0100Cre.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP710001 yetaP710001Form = new YetaP710001(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta710001 = new MSFFormWindows("연말정산대상자검색",yetaP710001Form, "닫기", "920px", "710px", false);
				msFwYeta710001.show();
				yetaP710001Form.setMSFFormWindows(msFwYeta710001);
		...
		plFrmYeta7100.add(topYeta0100Bar);
		...
		btnYeta0100Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				Yeta7100Print("YETAT7100", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
			}
		});
		plFrmYeta7100.add(topYeta0100Bar);	
		...
		// 파일업로드 처리
		plFrmYeta7100.setAction("myurl");
		plFrmYeta7100.setEncoding(Encoding.MULTIPART);
		plFrmYeta7100.setMethod(Method.POST);
		vp.add(plFrmYeta7100);

		plFrmYeta7100.setSize("990px", "680px");
		plFrmYeta7100.setLayout(new FlowLayout());
	}
...
//		srhDtilOccuInttnCd.setEnabled(gwtExtAuth.getEnableDtilOccuInttnCd());
		...
		btnUpDpobTrans.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP710002 yetaP710002Form = new YetaP710002(ActionDatabase.READ, getThis());

				//final MSFFormWindows msFwYeta710002 = new MSFFormWindows("원천신고부서",yetaP710002Form, "닫기", "920px", "710px", false);
				final MSFFormWindows msFwYeta710002 = new MSFFormWindows("원천신고부서",yetaP710002Form, "닫기", "1050px", "800px", false);
				msFwYeta710002.show();
				yetaP710002Form.setMSFFormWindows(msFwYeta710002);
		...
		plFrmYeta7100.add(fieldSet1);
		plFrmYeta7100.add(fieldSet2);	
	...
	private void createMsfGridForm() {
		...
		Yeta7100GridPanel = new MSFGridPanel(Yeta7100def, false, false, false,false, false);
		// Yeta7100GridPanel.setSize(960, 400);
		Yeta7100GridPanel.setHeaderVisible(false);
		Yeta7100GridPanel.setBorders(true);
		// Yeta7100GridPanel.getBottomComponent().setVisible(false);
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = Yeta7100GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					Yeta7100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					Yeta7100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				Yeta7100GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					Yeta7100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					Yeta7100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		//툴바 넣어주는것
		Yeta7100GridPanel.setBottomComponent(bottomToolbar);

		final Grid<BaseModel> grid = Yeta7100GridPanel.getMsfGrid().getGrid();
		...
		cp01.add(Yeta7100GridPanel); 

		fieldSetGrd.add(cp01);
		plFrmYeta7100.add(fieldSetGrd);
	}
...
//	}	
	...
	public void reload() {
		...
		Yeta7100GridPanel.getTableDef().setTableColumnFilters(filters);

		Yeta7100GridPanel.getTableDef().addColumnFilter("yrtxBlggYr",MSFSharedUtils.getSelectedComboValue(srhClutYr, "year"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta7100GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd"), SimpleColumnFilter.OPERATOR_EQUALS);	

		Yeta7100GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta7100GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta7100GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta7100GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta7100GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta7100GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta7100GridPanel.getTableDef().addColumnFilter("fnm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta7100GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta7100GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(utDpobCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta7100GridPanel.getTableDef().addColumnFilter("utDpobChk", MSFSharedUtils.convertBooleanToString(utDpobChk.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		

		Yeta7100GridPanel.reload();
	...
	private void xlsExportData() {
		...
		if ("".equals(srhClutYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				Yeta7100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta7100Export.do", "extgwtFrame", param);
			}
		} else {
			Yeta7100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta7100Export.do", "extgwtFrame", param);
		}
