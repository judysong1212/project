...
import com.app.exterms.yearendtax.client.form.defs.yeta2022.InfcPsnl0100Def;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP710001Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP710001ServiceAsync;
...
=====================================================**/
public  class YetaP710001   extends MSFFormPanel {  
	...
	private InfcPsnl0100Def YetaP710001def = new InfcPsnl0100Def("YetaP710001");
	private MSFGridPanel YetaP710001GridPanel;
	...
	private YetaP710001ServiceAsync yetaP710001Service = YetaP710001Service.Util.getInstance();
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP710001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta71001.setSize("890px", "640px");
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lscrPayYr.findModel("year",String.valueOf("2022")));
	...
	private void createSearchForm() {
		...
		//---------------------멀티콤보박스 닫기 -------------------------------------------------
		cpYeta71001.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
	...
	private void createStandardForm() {
		...
		cpYeta71001.add(layoutContainer);
	...
	private LayoutContainer payrModiListInfo() {
		...
		YetaP710001GridPanel = new MSFGridPanel(YetaP710001def, false, false, false, false,false);
		YetaP710001GridPanel.setHeaderVisible(false);
		YetaP710001GridPanel.setBodyBorder(false);
		YetaP710001GridPanel.setBorders(true);
		YetaP710001GridPanel.getBottomComponent().setVisible(false);

		final Grid<BaseModel> grid = YetaP710001GridPanel.getMsfGrid().getGrid();
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = YetaP710001GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					YetaP710001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					YetaP710001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				YetaP710001GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					YetaP710001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					YetaP710001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		btnSelected.addSelectionListener(new SelectionListener<ButtonEvent>() {   
			...
			public void componentSelected(ButtonEvent ce) {  
				...
				Iterator<BaseModel> itBm = YetaP710001GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); // .getSelectionModel().getSelectedItems().listIterator();
				...
				yetaP710001Service.saveYetaP710001ToYe161010(listYe161010Dto,listYe161020Dto, actionDatabase, new AsyncCallback<PagingLoadResult<ShowMessageBM>>() {
					...
					public void onFailure(Throwable caught) {
						...
						smForm.setWindowId("YetaP710001");
		...
		refresh.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				YetaP710001GridPanel.reload();	                	
			}  
		});		            
		bottomToolbar.add(refresh);

		//툴바 넣어주는것
		YetaP710001GridPanel.setBottomComponent(bottomToolbar);

		cp01.add(YetaP710001GridPanel);
	...
	public void reload() {
		...
		YetaP710001GridPanel.getTableDef().setTableColumnFilters(filters);

		YetaP710001GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP710001GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		
		...
		YetaP710001GridPanel.getTableDef().addColumnFilter("hdofcCodtnCd", strHdofcCodtnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		YetaP710001GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		YetaP710001GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDtilOccuInttnCd = YetaUtils.getStrValToBMMultiCombo(lsDtilOccuInttnCd,srhDtilOccuInttnCd.getValue(),"dtilOccuClsNm","dtilOccuInttnCd");  
		YetaP710001GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP710001GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP710001GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP710001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP710001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr, "year")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP710001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP710001GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(srhUtDpobCd.getValue())  , SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	private void formInit() {
		...
		YetaP710001GridPanel.getMsfGrid().clearData();
