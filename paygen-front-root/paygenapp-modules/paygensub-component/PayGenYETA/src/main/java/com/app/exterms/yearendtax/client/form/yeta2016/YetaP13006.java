...
import com.app.exterms.yearendtax.client.dto.yeta2016.Ye16Ta3110DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2016.Ye16Ta3110Def;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13006Service;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13006ServiceAsync;
...
=====================================================**/
public  class YetaP13006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta13006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP13006GridPanel;
	private Ye16Ta3110Def yetaP13006ToYeta03110Def  = new Ye16Ta3110Def("YETAP13006");	 //종전근무지내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
   ...
   */
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP13006ServiceAsync yetaP13006Service = YetaP13006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP13006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 
		...
		cpYeta13006 = new ContentPanel();  

		createSearchForm();		//기본정보 필드를 적용
		createMsfGridForm();	//그리드필드
		createStandardForm(); 	//기본정보필드   
		   
		cpYeta13006.setBodyBorder(false);
		cpYeta13006.setBorders(false);
		cpYeta13006.setHeaderVisible(false);
		cpYeta13006.setSize("790px", "640px");
		   
		this.add(cpYeta13006);
...
//		     });     
		...
		cpYeta13006.setLayout(new FlowLayout());
...
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = yetaP13006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
				List<Record> lsRec = new ArrayList<Record>();

				if((ActionDatabase.INSERT).equals(actionDatabase)) {
					yetaP13006GridPanel.getMsfGrid().clearData();
					resetYetaP13006();	//종전근무지상세 초기화
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP13006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//	      wthdSbyn.setHideLabel(true);
		  ...
		  btnNewButton.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				//TODO 수정하기..
				BaseModel bmData = yetaP13006GridPanel.getCurrentlySelectedItem(); 
				...
				//전체내역  
				YetaP13008 yetaP13008Form = new YetaP13008(ActionDatabase.READ, getThis());   
						    
				MSFFormWindows msFwYeta03008 = new MSFFormWindows("비과세 및 감면소득", yetaP13008Form, "닫기", "820px", "710px", true);
				msFwYeta03008.show();
				yetaP13008Form.setMSFFormWindows(msFwYeta03008);
				
				//TODO 추후 주석해제 
				yetaP13008Form.bind(ppRecord);
...
//	      return lcFormLayer;	  
	...
	private void createMsfGridForm() {			
		...
		yetaP13006GridPanel = new MSFGridPanel(yetaP13006ToYeta03110Def, false, false, false, false, false);
		yetaP13006GridPanel.setHeaderVisible(false);  
		yetaP13006GridPanel.setSize("780px", "200px");
		yetaP13006GridPanel.setBorders(true);
		yetaP13006GridPanel.setHeadingText("");
		
		
		/** --- 그리드 합계 부분  시작 --- **/
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
		
		payrSummaryRow.setHtml(Ye16Ta3110BM.ATTR_EDACRVYY, "합 계"); 
		 
		payrSummaryRow.setCellStyle(Ye16Ta3110BM.ATTR_PAYRTOTL,"summary_color");	/** set 급여합계금액 : payrTotl */
		payrSummaryRow.setCellStyle(Ye16Ta3110BM.ATTR_BONSAMNT,"summary_color");	/** set 상여합계금액 : bonsAmnt */
		payrSummaryRow.setCellStyle(Ye16Ta3110BM.ATTR_EARNTAXN,"summary_color");	/** set 소득세 : earnTaxn */
		payrSummaryRow.setCellStyle(Ye16Ta3110BM.ATTR_IHTXTAXN,"summary_color");	/** set 지방소득세 : ihtxTaxn */
		
							    
		payrSummaryRow.setSummaryType(Ye16Ta3110BM.ATTR_PAYRTOTL, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3110BM.ATTR_PAYRTOTL, NumberFormat.getFormat("#,##0;(#,##0)"));  
							    
		payrSummaryRow.setSummaryType(Ye16Ta3110BM.ATTR_BONSAMNT, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3110BM.ATTR_BONSAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
		
		payrSummaryRow.setSummaryType(Ye16Ta3110BM.ATTR_EARNTAXN, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3110BM.ATTR_EARNTAXN, NumberFormat.getFormat("#,##0;(#,##0)"));  
		
		payrSummaryRow.setSummaryType(Ye16Ta3110BM.ATTR_IHTXTAXN, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3110BM.ATTR_IHTXTAXN, NumberFormat.getFormat("#,##0;(#,##0)"));  
							      
		
		yetaP13006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  시작 --- **/
		
		final Grid yetaP13006Grid = yetaP13006GridPanel.getMsfGrid().getGrid();
		yetaP13006Grid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				// TODO Auto-generated method stub
				//그리드 선택시
				yetaP13006ToYeta3200Detail(yetaP13006GridPanel.getCurrentlySelectedItem());
			}
		});
		
		gridFieldSet.add(yetaP13006GridPanel);
			
		cpYeta13006.add(gridFieldSet);
	}		
		
	private YetaP13006 getThis(){
	       return this;
	}
	...
	//버튼에 따른 Action
		...
		private void yetaP13006FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				
				Ye16Ta3110DTO yeta3110Dto= new Ye16Ta3110DTO();  
				List<Ye16Ta3110DTO> listYeta3110dto = new ArrayList<Ye16Ta3110DTO>();  
				
				if((ActionDatabase.INSERT).equals(actionDatabase) || (ActionDatabase.UPDATE).equals(actionDatabase)) {
					
					yeta3110Dto= new Ye16Ta3110DTO();
					listYeta3110dto = new ArrayList<Ye16Ta3110DTO>();  
...
//		                yeta3110Dto.set.((Long)bmMapModel.get("reseSmam"));	/** column 연구보조비합계금액 : reseSmam */
				...
				yetaP13006Service.activityOnYetaP13006ToYeta3110(listYeta3110dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP13006ToYeta3110(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//        }
	...
	public void reload() {
			...
			yetaP13006GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaP13006GridPanel.getTableDef().addColumnFilter("edacRvyy", MSFSharedUtils.allowNulls(srhEdacRvyy.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaP13006GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaP13006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaP13006GridPanel.reload();
