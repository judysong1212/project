...
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye18Ta1000Def;
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4400Service;
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4400ServiceAsync;
 ...
 * =====================================================
 **/
public class Yeta4400 extends MSFPanel {
	...
	private FormPanel plFrmYeta4400;
...
//	private MSFGridPanel typOccuCdGridPanel;
	 ...
	 **/
	// ---------- 상단 버튼 시작 ----------
	private ButtonBar topYeta4400Bar;
	private Button btnYeta4400Reset;	//초기화
	private Button btnYeta4400Save;		//저장
	private Button btnYeta4400Search;	//조회
	// ---------- 상단 버튼 종료 ----------
	
	// ---------- 엑셀 버튼 시작 ----------
	private ButtonBar exYeta4400Bar;	
...
//	private Yeta0100ServiceAsync yeta0100Service = Yeta0100Service.Util.getInstance();
	private Yeta4400ServiceAsync yeta4400Service = Yeta4400Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
	 ...
	 */
	private void checkYeta4400Auth(String authAction, ListStore<BaseModel> bm) {
	 ...
	 private void authExecEnabled() { 
		 ...
		 /**
			...
			creYrtxBlggYr.setValue(lsClutYrStore.findModel("year", "2019"));
			yrtxBlggYr.setValue(lscreClutYrStore.findModel("year", "2019"));
...
//			formData = new FormData("-650");
			...
			createYeta4400Form();	// 화면 기본정보를 설정
	...
	public Yeta4400(String txtForm) {
...
//
		...
		plFrmYeta4400.setLayout(new FlowLayout());
		...
		lscreEmymtDivCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta4400Auth("creEmymtDivCd", lscreEmymtDivCd);
				}
			}
		});
		...
		creDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta4400Auth("creDeptCd", lscreDeptCd);
				}
			}
		});
		...
		creTypOccuCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					//checkYeta4400Auth("creTypOccuCd", lsTypOccuCd);
				}
			}
		});
...
//		PayrMangDeptCd = new HiddenField<String>(); //급여관리부서코드 
		...
		// 파일업로드 처리
		plFrmYeta4400.setAction("myurl");
		plFrmYeta4400.setEncoding(Encoding.MULTIPART);
		plFrmYeta4400.setMethod(Method.POST);

		plFrmYeta4400.setSize("990px", "820px");
		plFrmYeta4400.setLayout(new FlowLayout());
...
//		layoutContainer1_4.setStyleAttribute("marginBottom", "10px");
		...
		exYeta4400Bar = new ButtonBar();
		exYeta4400Bar.setAlignment(HorizontalAlignment.RIGHT);

		/*
		btnYetaVerifiction= new Button("신고사전검증");
		exYeta4400Bar.add(btnYetaVerifiction);
		...
		exYeta4400Bar.add(btnYetaExImport);

		btnYetaExSave= new Button("신고파일생성");
		exYeta4400Bar.add(btnYetaExSave);
		...
		layoutContainer11.add(exYeta4400Bar);
		...
		plFrmYeta4400.add(layoutContainer1_4,new com.extjs.gxt.ui.client.widget.layout.ColumnData(1));	// 텍스트와 버튼
...
//		});
	...
	private void createYeta4400Form() {

		plFrmYeta4400 = new FormPanel();
		plFrmYeta4400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산신고파일생성"));
		plFrmYeta4400.setIcon(Resources.APP_ICONS.text());
		plFrmYeta4400.setPadding(2);
		plFrmYeta4400.setFrame(true);
		...
		plFrmYeta4400.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta4400.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/
		
		   //멀티콤보박스 닫기 
		plFrmYeta4400.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
		            @Override
		            public void handleEvent(ComponentEvent ce) { 
...
//						                        }
	...
	private void createSearchForm() {
		...
		topYeta4400Bar = new ButtonBar();
		topYeta4400Bar.setAlignment(HorizontalAlignment.RIGHT);

		btnYeta4400Reset = new Button("초기화");
		btnYeta4400Reset.setIcon(MSFMainApp.ICONS.new16());
		btnYeta4400Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				resetStandardForm();	//전자신고기본정보 초기화
			}
		});
		topYeta4400Bar.add(btnYeta4400Reset);

//		topBtn = new Button("신 규");
//		topBtn.setIcon(MSFMainApp.ICONS.new16());
//		topYeta4400Bar.add(topBtn);
...
//		buttonBar.add(topBtn);
		...
		btnYeta4400Save = new Button("저 장");
		btnYeta4400Save.setIcon(MSFMainApp.ICONS.save16());
		btnYeta4400Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			@Override
		...
		topYeta4400Bar.add(btnYeta4400Save);

		btnYeta4400Search = new Button("조 회");
		btnYeta4400Search.setIcon(MSFMainApp.ICONS.search());
		btnYeta4400Search.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
...
//				ye161005Bm.setSrhBusinCd(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"));
				...
				yeta4400Service.getYeta4400List(ye161005Dto, new AsyncCallback<Ye161005BM>(){
		...
		topYeta4400Bar.add(btnYeta4400Search);

//		topBtn = new Button("인 쇄");
//		topBtn.setIcon(MSFMainApp.ICONS.print16());
//		buttonBar.add(topBtn);
		
		plFrmYeta4400.add(topYeta4400Bar);
		...
		lsEmymtDivCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta4400Auth("srhEmymtDivCd", lsEmymtDivCd);
				}
			}
		});
		...
		srhDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta4400Auth("srhDeptCd", lsDeptCd);
				}
			}
		});
		...
		srhTypOccuCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					//checkYeta4400Auth("srhTypOccuCd", lsTypOccuCd);
				}
			}
		});
...
//        cpGrid.add(gridFieldSet);
		...
		plFrmYeta4400.add(gridFieldSet);
//		plFrmYeta4400.add(msfGridPanel);
	}
...
//		msfGridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);//사업
	...
	/** 정산 마감 및 풀기 **/
	private void finishYeta4400(final String flag) { 
...
//	    				ye161005Bm.setBusinCd(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"));
	    			...
	    			//	ye17Ta1010Dto.setBusoprRgstnum(MSFSharedUtils.getSelectedComboValue(srhBusoprRgstnum, "busoprRgstnum"));
	      				...
	      				yeta4400Service.finishOnYe161005(ye17Ta1010Dto, new AsyncCallback<Long>(){
	                         public void onFailure(Throwable caught) {
...
//			} else { 
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta4400Export.do","extgwtFrame" ,param);   
			//}
		}else{ 
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta4400Export.do","extgwtFrame" ,param);  
		}
...
//		ye161005ExDto.setDpobCd(dpobCd.getValue());    														/** column 사업장코드 : dpobCd */
		...
		yeta4400Service.updateYe161005(ye161005ExDto, new AsyncCallback<Long>(){
			@Override
			public void onFailure(Throwable caught) {
...
//		param.put("businCd", MSFSharedUtils.getSelectedComboValue(creBusinCd,"businCd"));
		...
		if ("Y0090003".equals(MSFSharedUtils.allowNulls(param.get("creFile")))) {
			...
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/FileYeta4400Export.do","extgwtFrame" ,param);   
			//}
		}else{ 
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/FileYeta4400Export.do","extgwtFrame" ,param);  
		}	
	...
	/*-----------------------------------------------------------------------------------------------------------------------------------
	 * 검증 추가 
	 *  @RequestMapping(value = "/exp/Yeta4400Verifiction.do") 
	 -------------------------------------------------------------------------------------------------------------------------------------*/
...
//		param.put("businCd", MSFSharedUtils.getSelectedComboValue(creBusinCd,"businCd"));
		...
		if(MSFSharedUtils.paramNull(param.get("resuNumb"))){
			...
			MessageBox.info("", "중간 생성부 부서 또는 원천징수부서는 필수 입니다.(택1)", null);
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/Yeta4400Verifiction.do","extgwtFrame" ,param);   
