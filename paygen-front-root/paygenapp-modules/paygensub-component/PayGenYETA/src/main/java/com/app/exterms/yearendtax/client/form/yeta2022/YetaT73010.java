...
import com.app.exterms.yearendtax.client.form.defs.yeta2022.Ye162040Def;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73010Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73010ServiceAsync;
...
=====================================================**/
public  class YetaT73010   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73010; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73010GridPanel;
	private Ye162040Def ye162040Def = new Ye162040Def("YetaT73010");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT73010ServiceAsync yetaT73010Service = YetaT73010Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT73010Service.activityOnYetaT73010(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT73010(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73010 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73010.setBodyBorder(false);
		cpYetaT73010.setBorders(false);
		cpYetaT73010.setHeaderVisible(false);
		cpYetaT73010.setSize("790px", "640px");
		   
		this.add(cpYetaT73010);
...
//		     });     
		...
		cpYetaT73010.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT73010GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT73010GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT73010GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT73010GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT73010GridPanel.getMsfGrid().getGrid().startEditing(YetaT73010GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT73010GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT73010GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT73010.add(topYetaBtnBar);
...
//		cpYetaT73010.add(topYetaBtnBar);
		...
		YetaT73010GridPanel = new MSFGridPanel(ye162040Def, false, false, false, false, false);
		YetaT73010GridPanel.setSize("790px", "200px");
		YetaT73010GridPanel.setBorders(true);
		YetaT73010GridPanel.setHeaderVisible(false);
...
//		
//		YetaT73010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT73010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT73010.add(YetaT73010GridPanel);
		...
		fldstNewFieldset.add(YetaT73010GridPanel);
		cpYetaT73010.add(fldstNewFieldset);
		cpYetaT73010.add(layoutContainer10);
		cpYetaT73010.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT73010GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73010GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73010GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73010GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73010GridPanel.reload();
