...
import com.app.exterms.yearendtax.client.form.defs.yeta2016.Ye16Ta3140Def;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13002Service;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13002ServiceAsync;
...
=====================================================**/
public  class YetaP13002   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta13002; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP13002GridPanel;
//	private Yeta0300Def yeta0300Def_07  = new Yeta0300Def("YETA03007");	//의료비내역 그리드 테이블 컬럼 define 
	private Ye16Ta3140Def yetaP13002ToYe16Ta3140Def  = new Ye16Ta3140Def("YETAP13002");//의료비내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
 ...
 **/ 
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP13002ServiceAsync yetaP13002Service = YetaP13002Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 	
     ...
     */  
		...
		private void yetaP13002FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				
				Boolean dataChk = true;
	              
				List<Ye16Ta3140DTO> listYeta3140Dto = new ArrayList<Ye16Ta3140DTO>();  
				...
				while (records.hasNext()) {
	                ...
	                yeta3140Dto.setDpobCd(MSFSharedUtils.allowNulls(bmMapModel.get("dpobCd")));				/** 사업장코드 : dpobCd */
	                yeta3140Dto.setSystemkey(MSFSharedUtils.allowNulls(bmMapModel.get("systemkey")));   	/** SYSTEMKEY : systemkey */
	                yeta3140Dto.setEdacRvyy(MSFSharedUtils.allowNulls(bmMapModel.get("edacRvyy")));    		/** column 연말정산귀속년도 : edacRvyy */
	                yeta3140Dto.setSettGbcd(MSFSharedUtils.allowNulls(bmMapModel.get("settGbcd")));    		/** column 정산구분코드 : settGbcd */
	               
	                yeta3140Dto.setMediSeqn((Long) bmMapModel.get("mediSeqn")); 							/** column 의료비지급일련번호 : mediSeqn */
	                yeta3140Dto.setRsnoNumb(MSFSharedUtils.allowNulls(bmMapModel.get("rsnoNumb")));    		/** column 주민등록번호 : rsnoNumb */
	                yeta3140Dto.setPsclHpyn(MSFSharedUtils.allowNulls(bmMapModel.get("psclHpyn")));    		/** column 본인등해당여부 : psclHpyn */
	                yeta3140Dto.setVendNumb(MSFSharedUtils.allowNulls(bmMapModel.get("vendNumb")));    		/** column 거래처번호 : vendNumb */
	                yeta3140Dto.setVendName(MSFSharedUtils.allowNulls(bmMapModel.get("vendName")));    		/** column 거래처명 : vendName */
	                yeta3140Dto.setMediSeqn((Long) bmMapModel.get("mediSeqn"));    							/** column 의료비지급일련번호 : mediSeqn */
	                
	                yeta3140Dto.setSystemkey(MSFSharedUtils.allowNulls(bmMapModel.get("systemkey")));		/** column SYSTEMKEY : systemkey */
	                yeta3140Dto.setMediCfcd(MSFSharedUtils.allowNulls(bmMapModel.get("mediCfcd$commCd")));	/** column 의료증빙코드 : mediCfcd */
	                yeta3140Dto.setCashUect((Long) bmMapModel.get("cashUect"));    							/** column 지급건수 : cashUect */
	                yeta3140Dto.setCashUeam((Long) bmMapModel.get("cashUeam"));    							/** column 지급금액 : cashUeam */
	                yeta3140Dto.setCtifTrea((String) bmMapModel.get("ctifTrea")); 							/** column 난임시술비해당여부 : ctifTrea */
	                
	                
	                
	                /** 그리드에 있는 항목에 대해서 입력 체크 **/
	                if(MSFSharedUtils.paramNull( yeta3140Dto.getRsnoNumb().replace("-", ""))) {
	                	...
	                	dataChk = false;
	                	return; 
	                }else if(MSFSharedUtils.paramNull( yeta3140Dto.getVendNumb().replace("-", ""))) {
	                	MessageBox.alert("경고", "사업자번호를 입력해 주세요.", null);
	                	dataChk = false;
	                	return; 
	                }else if(MSFSharedUtils.paramNull( yeta3140Dto.getVendName())) {
	                	MessageBox.alert("경고", "거래처명을 입력해주세요.", null);
	                	dataChk = false;
	                	return; 
	                }else if(MSFSharedUtils.paramNull( yeta3140Dto.getMediCfcd())) {
	                	MessageBox.alert("경고", "의료증빙구분을 입력해주세요.", null);
	                	dataChk = false;
	                	return; 
	                }else if(MSFSharedUtils.paramNull( yeta3140Dto.getCashUect())) {
	                	MessageBox.alert("경고", "지급건수를 입력해 주세요.", null);
	                	dataChk = false;
	                	return; 
	                }else if(MSFSharedUtils.paramNull( yeta3140Dto.getCashUeam())) {
	                	MessageBox.alert("경고", "지급금액을 입력해 주세요.", null);
	                	dataChk = false;
	                	return; 
	                }
	                
//	                bmData.set("ctifTrea", false);
	                
	                listYeta3140Dto.add(yeta3140Dto);
				}  
				
				
				if(dataChk) {
					
					if (listYeta3140Dto.size() <= 0) {
						MessageBox.alert(actionDatabase.name() + "저장", actionDatabase.name() +  "처리할 데이타가 존재하지 않습니다.", null);
						return;
					}
					
					yetaP13002Service.activityOnYetaP13002ToYeta3140(listYeta3140Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP13002ToYeta3140(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	  ...
	  public YetaP13002(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			initLoad();		    
		   ...
		   cpYeta13002.setBodyBorder(false);
		   cpYeta13002.setBorders(false);
		   cpYeta13002.setHeaderVisible(false);
		   cpYeta13002.setSize("790px", "640px");
		   
		   this.add(cpYeta13002);
...
//			     });     
			...
			cpYeta13002.setLayout(new FlowLayout());
...
//					systemkey.setValue("20080013");
					...
					yetaP13002GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = yetaP13002GridPanel.getTableDef().getDefaultValues();  
...
//			         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
			         ...
			         bmData.set("mediSeqn", 0L);    /** column 의료비지급일련번호 : mediSeqn */
			         bmData.set("rsnoNumb", "");    /** column 주민등록번호 : rsnoNumb */
			         bmData.set("psclHpyn", false); /** column 본인등해당여부 : psclHpyn */
			         bmData.set("vendNumb", "");    /** column 거래처번호 : vendNumb */
			         bmData.set("vendName", "");    /** column 거래처명 : vendName */
			         bmData.set("mediCfcd", "");    /** column 의료증빙코드 : mediCfcd */
			         bmData.set("cashUect", 0L);    /** column 지급건수 : cashUect */
			         bmData.set("cashUeam", 0L);    /** column 지급금액 : cashUeam */
			         bmData.set("ctifTrea", "0"); /** column 난임시술비해당여부 : ctifTrea */
			         
			         yetaP13002GridPanel.getMsfGrid().getGrid().stopEditing();      
			         yetaP13002GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         yetaP13002GridPanel.getMsfGrid().getGrid().startEditing(yetaP13002GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//					doAction(actionDatabase);
			...
			btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					Iterator<BaseModel> itBm = yetaP13002GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					setListRecord(yetaP13002GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//			BaseModel bmData = yetaP13002GridPanel.getMsfGrid().getStore().getModels().get(0);  
//			if (MSFSharedUtils.paramNotNull(bmData)){
				// TODO Auto-generated method stub
				IColumnFilter filters = null;
				
				yetaP13002GridPanel.getTableDef().setTableColumnFilters(filters);
				yetaP13002GridPanel.getTableDef().addColumnFilter("edacRvyy", MSFSharedUtils.allowNulls(srhEdacRvyy.getValue())
	...
	//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
				yetaP13002GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd")
						, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
				yetaP13002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
						, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
				yetaP13002GridPanel.reload();
...
//			cp01.setSize(790, 390);
		    ...
		    //연말정산대상자 및 내역
			yetaP13002GridPanel = new MSFGridPanel(yetaP13002ToYe16Ta3140Def, false, false, false, false, false);
			yetaP13002GridPanel.setSize("790px", "390px");
			yetaP13002GridPanel.setBorders(false);
			yetaP13002GridPanel.setBodyBorder(false);
			yetaP13002GridPanel.setHeaderVisible(false); 
			yetaP13002GridPanel.getBottomComponent().setVisible(false);
			
			
			/** --- 그리드 합계 부분  시작 --- **/
			AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
			
			payrSummaryRow.setHtml(Ye16Ta3140BM.ATTR_RSNONUMB, "합 계"); 
			 
			payrSummaryRow.setCellStyle(Ye16Ta3140BM.ATTR_CASHUECT,"summary_color");	/** set 지급건수 : cashUect */
			payrSummaryRow.setCellStyle(Ye16Ta3140BM.ATTR_CASHUEAM,"summary_color");	/** set 지급금액 : cashUeam */
			
								    
			payrSummaryRow.setSummaryType(Ye16Ta3140BM.ATTR_CASHUECT, SummaryType.SUM);   
			payrSummaryRow.setSummaryFormat(Ye16Ta3140BM.ATTR_CASHUECT, NumberFormat.getFormat("#,##0;(#,##0)"));  
								    
			payrSummaryRow.setSummaryType(Ye16Ta3140BM.ATTR_CASHUEAM, SummaryType.SUM);   
			payrSummaryRow.setSummaryFormat(Ye16Ta3140BM.ATTR_CASHUEAM, NumberFormat.getFormat("#,##0;(#,##0)"));  
								      
			
			yetaP13002GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			/** --- 그리드 합계 부분  시작 --- **/
			
			gridFieldSet.add(yetaP13002GridPanel);
//			
//			cp01.add(gridFieldSet);
//			
//			cpYeta13002.add(cp01);
			
			cpYeta13002.add(gridFieldSet);
