...
import com.app.exterms.yearendtax.client.service.yeta2021.Yeta6200Service;
import com.app.exterms.yearendtax.client.service.yeta2021.Yeta6200ServiceAsync;
 ...
 * </pre>
 */
public class Yeta6200 extends MSFPanel {
	...
	private FormPanel plFrmYeta6200;
	private String txtForm = "";


	private MSFGridPanel Yeta6200GridPanel;
	private MSFGridPanel Yeta6200XmlGridPanel;
	private Ye161010Def Yeta6200def = new Ye161010Def("Yeta6200");
	private Ye161010Def Yeta6200defXml = new Ye161010Def("Yeta6200_RESULT");
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta6200Bar;
	private Button btnYeta6200Sec; 	// 초기화
	private Button btnYeta6200Reset; 	// 초기화
	private Button btnYeta6200New; 		// 신규
	private Button btnYeta6200Save; 	// 저장
	private Button btnYeta6200Del; 		// 삭제
	private Button btnYeta6200Sreach; 	// 조회
	private Button btnYeta6200Print; 	// 인쇄
	private Button btnYetaP620001; 	// 상세정보 팝업
	private Button btnYetaP620002; 	// 상세정보 팝업
	 ...
	 * ++++++++++++++++++++++
	 **/
	private Yeta6200ServiceAsync yeta6200Service = Yeta6200Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2021")));
	...
	public Yeta6200(String txtForm) {
	...
	private void createYeta0200Form() {
		...
		plFrmYeta6200 = new FormPanel();
		plFrmYeta6200.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산파일업로드"));
		plFrmYeta6200.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta6200.setPadding(2);
		plFrmYeta6200.setFrame(true);
		...
		plFrmYeta6200.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta6200.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		//멀티콤보박스 닫기 
		plFrmYeta6200.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar **/
		topYeta6200Bar = new ButtonBar();
		topYeta6200Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		if ("admin".equals(MSFMainApp.get().getUser().getUsrId())) {	
			btnYeta6200Sec = new Button("암호화패치");
			btnYeta6200Sec.setIcon(MSFMainApp.ICONS.new16());
			topYeta6200Bar.add(btnYeta6200Sec);
			btnYeta6200Sec.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
				 
					Ye161010DTO ye161010Dto = new Ye161010DTO(); 
						   
					yeta6200Service.yetaChangeConvert(ye161010Dto,  new AsyncCallback<String>(){
				                 public void onFailure(Throwable caught) {
				                     MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
				                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta6200Service : " + caught), null);
				                 }
		...
		btnYeta6200Reset = new Button("초기화");
		btnYeta6200Reset.setIcon(MSFMainApp.ICONS.new16());
		topYeta6200Bar.add(btnYeta6200Reset);
		btnYeta6200Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//초기화 
				formInit();
				Yeta6200GridPanel.getMsfGrid().clearData(); 
			}
		});  
		...
		btnYeta6200Sreach = new Button("조 회");
		btnYeta6200Sreach.setIcon(MSFMainApp.ICONS.search());
		topYeta6200Bar.add(btnYeta6200Sreach);
		btnYeta6200Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//조회버튼 클릭시 처리 
				reloadYeta6200();  
			}
		...
		plFrmYeta6200.add(topYeta6200Bar);

		// 파일업로드 처리
		plFrmYeta6200.setAction("myurl");
		plFrmYeta6200.setEncoding(Encoding.MULTIPART);
		plFrmYeta6200.setMethod(Method.POST);

		vp.add(plFrmYeta6200);
		plFrmYeta6200.setSize("990px", "830px");
		plFrmYeta6200.setLayout(new FlowLayout());
	...
	private void createMsfGridForm() {
		...
		Yeta6200GridPanel = new MSFGridPanel(Yeta6200def, false, false, false, false,false);
		Yeta6200GridPanel.setHeaderVisible(false);
		Yeta6200GridPanel.setSize(470, 640);
		Yeta6200GridPanel.setBorders(true);
		final Grid yeta6000DefGrid = Yeta6200GridPanel.getMsfGrid().getGrid();
		yeta6000DefGrid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (Yeta6200GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = new BaseModel();
					formInit() ;
					bmData = Yeta6200GridPanel.getCurrentlySelectedItem();
		...
		Yeta6200GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 4, new HeaderGroupConfig("소득공제자료", 1, 2));

		fsGird.add(Yeta6200GridPanel);
		...
		Yeta6200XmlGridPanel = new MSFGridPanel(Yeta6200defXml, false, false,false, false);
		Yeta6200XmlGridPanel.setSize(250, 305);
		Yeta6200XmlGridPanel.setBorders(true);
		Yeta6200XmlGridPanel.setStyleAttribute("marginTop", "5px");
		layoutContainer17.add(Yeta6200XmlGridPanel,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.8));

		btnYetaP620001 = new Button("개별간소화파일");
		btnYetaP620001.setStyleAttribute("marginTop", "5px");
		btnYetaP620001.setStyleAttribute("marginLeft", "5px");
		btnYetaP620001.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					...
					final MSFFormWindows msFwYeta620001 = new MSFFormWindows("자료추출상세내역",yetaP620001Form, "닫기", "920px", "710px", false);
					msFwYeta620001.show();
					yetaP620001Form.setMSFFormWindows(msFwYeta620001);

					//					funcSetPopUpRecord();

					yetaP620001Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP620001,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));
		//일괄간소화파일은 데이터스킵문제로 화면에 안보이게 주석처리함 연말정산2021
		/*btnYetaP620002 = new Button("일괄간소화파일");
		btnYetaP620002.setStyleAttribute("marginTop", "5px");
		btnYetaP620002.setStyleAttribute("marginLeft", "5px");
		btnYetaP620002.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					Window.alert("정산대상자를 선택해주세요.");
				}
					...
					yetaP620002Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP620002,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));*/

		layoutContainer1_4.add(layoutContainer17, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.495));

		plFrmYeta6200.add(layoutContainer1_4);
	}
	...
	private Yeta6200 getThis(){
		return this;
	}

	private void reloadYeta6200(){

		IColumnFilter filters = null;
		Yeta6200GridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta6200GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 

		//정산구분
		Yeta6200GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta6200GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta6200GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd,srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta6200GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta6200GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta6200GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta6200GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);


		Yeta6200GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta6200GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
		Yeta6200GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   

		Yeta6200GridPanel.reload();  

	}

	private void reloadYeta6200Result(){

		IColumnFilter filters = null;
		Yeta6200XmlGridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta6200XmlGridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 
		//정산구분
		Yeta6200XmlGridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta6200XmlGridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta6200XmlGridPanel.reload();  
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//submit the form
							plFrmYeta6200.mask("PDF 전자문서 업데이트 중입니다. 업로드 중..."); 

							//파일업로드 처리
							plFrmYeta6200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta6200Import.do");
							plFrmYeta6200.setEncoding(Encoding.MULTIPART);
							plFrmYeta6200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta6200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
							plFrmYeta6200.submit();  
							plFrmYeta6200.onFrameLoad();  
							picBoolFile = true;
						...
						// after file upload 
						plFrmYeta6200.addListener(Events.Submit, new Listener<FormEvent>() {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//파일업로드 처리
							plFrmYeta6200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta6200DdcImport.do");
							plFrmYeta6200.setEncoding(Encoding.MULTIPART);
							plFrmYeta6200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta6200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
