...
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43007ServiceAsync;
...
=====================================================**/
public  class YetaP43007   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP43007GridPanel;
//	private Ye16Ta3100Def YetaP43007ToYe162010Def  = new Ye16Ta3100Def("YETAP13007");	//종전근무지내역 그리드 테이블 컬럼 define 
	private Ye162010Def ye162010Def  = new Ye162010Def("YetaP43007");	//종전근무지내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP43007ServiceAsync yetaP43007Service = YetaP43007Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------		
     ...
     */  
		...
		private void YetaP43007FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				if(dataChk) {
					...
					yetaP43007Service.activityOnYetaP43007ToYe162010(listYe162010Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
		 ...
		 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 **/		  
	  public YetaP43007(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			     });     
			...
			btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 ...
					 YetaP43007GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = YetaP43007GridPanel.getTableDef().getDefaultValues();  
			         ...
			         YetaP43007GridPanel.getMsfGrid().getGrid().stopEditing();      
			         YetaP43007GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         YetaP43007GridPanel.getMsfGrid().getGrid().startEditing(YetaP43007GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
					...
					MessageBox.confirm("삭제", "선택하신 연금저축 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
						...
						public void handleEvent(MessageBoxEvent be) {
							if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
								...
								Iterator<BaseModel> itBm = YetaP43007GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					...
					setListRecord(YetaP43007GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
					doAction(actionDatabase);
...
//	            	reload();
		...
		private void createMsfGridForm() {
			...
			YetaP43007GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
//			YetaP43007GridPanel.setSize("790px", "300px");
			YetaP43007GridPanel.setBorders(false);
			YetaP43007GridPanel.setHeaderVisible(false);
			...
			YetaP43007GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			/** --- 그리드 합계 부분  시작 --- **/
			
			cpYeta23007.add(YetaP43007GridPanel);
...
//			cpYeta23007.add(layoutContainer);
	...
	public void reload() {
		...
		YetaP43007GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP43007GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP43007GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
															, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP43007GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP43007GridPanel.reload();
