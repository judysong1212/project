...
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye160510Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63001Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63001ServiceAsync;
...
=====================================================**/
public  class YetaT63001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63001GridPanel;
	private Ye160510Def ye160510Def = new Ye160510Def("YetaT63001");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63001ServiceAsync yetaT63001Service = YetaT63001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63001Service.activityOnYetaT63001(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63001 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63001.setBodyBorder(false);
		cpYetaT63001.setBorders(false);
		cpYetaT63001.setHeaderVisible(false);
		cpYetaT63001.setSize("790px", "640px");
		   
		this.add(cpYetaT63001);
...
//		     });     
		...
		cpYetaT63001.setLayout(new FlowLayout());
		...
		btnYetaAdd.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT63001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT63001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
				List<Record> lsRec = new ArrayList<Record>();

				
				if((ActionDatabase.INSERT).equals(actionDatabase)) {
					YetaT63001GridPanel.getMsfGrid().clearData();
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT63001.add(topYetaBtnBar);
...
//		cpYetaT63001.add(topYetaBtnBar);
		...
		YetaT63001GridPanel = new MSFGridPanel(ye160510Def, false, false, false, false, false);
		YetaT63001GridPanel.setSize("790px", "200px");
		YetaT63001GridPanel.setBorders(true);
		YetaT63001GridPanel.setHeaderVisible(false);
...
//		
//		YetaT63001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63001.add(YetaT63001GridPanel);
		...
		fldstNewFieldset.add(YetaT63001GridPanel);
		
		cpYetaT63001.add(fldstNewFieldset);
		cpYetaT63001.add(layoutContainer10);
		cpYetaT63001.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63001GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63001GridPanel.reload();
