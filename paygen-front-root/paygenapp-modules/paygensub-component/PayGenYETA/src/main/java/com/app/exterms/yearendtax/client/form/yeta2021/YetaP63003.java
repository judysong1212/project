...
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye165010Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63003Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63003ServiceAsync;
...
=====================================================**/
public  class YetaP63003   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta63003; 
	...
	*#######################################################*/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP63003GridPanel;
//	private Yeta0300Def yeta0300Def_08  = new Yeta0300Def("YETA03008");	//가족교육비 그리드 테이블 컬럼 define 
//	private Ye16Ta3180Def YetaP63003ToYe165010  = new Ye16Ta3180Def("YetaP63003");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye165010Def YetaP63003ToYe165010  = new Ye165010Def("YetaP63003");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP63003ServiceAsync yetaP63003Service = YetaP63003Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 	
	...
	private void YetaP63003FormSave() {
...
//				}
			...
			if (dataChk) {
				...
				yetaP63003Service.activityOnYetaP63003ToYe165010(listYe165010Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP63003ToYe165010(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	...
	public YetaP63003(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		this.actionDatabase = actionDatabase;
		...
		cpYeta63003 = new ContentPanel();

		createSearchForm(); // 기본정보 필드를 적용
		createMsfGridForm(); // 그리드필드
		createStandardForm(); // 기본정보필드

		cpYeta63003.setBodyBorder(false);
		cpYeta63003.setBorders(false);
		cpYeta63003.setHeaderVisible(false);
		cpYeta63003.setSize("790px", "490px");

		this.add(cpYeta63003);
	...
	private void createSearchForm() {
		...
		cpYeta63003.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP63003GridPanel.getMsfGrid().clearData();
					 
				BaseModel bmData = YetaP63003GridPanel.getTableDef().getDefaultValues();  
				...
				YetaP63003GridPanel.getMsfGrid().getGrid().stopEditing();
				YetaP63003GridPanel.getMsfGrid().getGrid().getStore().insert(bmData, 0);
				YetaP63003GridPanel.getMsfGrid().getGrid().startEditing(YetaP63003GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);
...
//				actionDatabase = ActionDatabase.INSERT;
				...
				MessageBox.confirm("삭제", "선택하신 기부금 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
							...
							Iterator<BaseModel> itBm = YetaP63003GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP63003GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	      layoutContainer_2.setBorders(false);
	...
	private void createMsfGridForm() {
		...
		YetaP63003GridPanel = new MSFGridPanel(YetaP63003ToYe165010, false, false, false, false, false);
		YetaP63003GridPanel.setHeaderVisible(false);  
		YetaP63003GridPanel.setBorders(false);
		YetaP63003GridPanel.setHeaderVisible(false);
		YetaP63003GridPanel.getBottomComponent().setVisible(true);
		...
		YetaP63003GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			

		cp01.add(YetaP63003GridPanel);

		cpYeta63003.add(cp01);
	...
	public void reload() {
		...
		YetaP63003GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP63003GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP63003GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP63003GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP63003GridPanel.reload();
