...
import com.app.exterms.yearendtax.client.form.defs.yeta2018.Yeta3060Def;
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3060Service;
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3060ServiceAsync;
 ...
 * =========================================================================
 **/
public class Yeta3060 extends MSFPanel {
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta3060Bar;
	private Button btnYeta3060Dpob; 
	private Button btnYeta3060New; 
	private Button btnYeta3060Save;
	private Button btnYeta3060Del;
	private Button btnYeta3060Sreach;
	// -------------- 상단 버튼 종료 --------------
...
//	private MSFComboBox<BaseModel> hhrkUntDpobCd; 					// 상위 단위사업장 코드
	...
	private CheckBoxGroup chkgrpYeta3060;   						// 체크 그룹
	...
	// -------------- 서비스 호출 시작 -------------- 
	//private InfcYeta3060ServiceAsync infcYeta3060Service = InfcYeta3060Service.Util.getInstance();
	private Yeta3060ServiceAsync yeta3060Service = Yeta3060Service.Util.getInstance();
	private PrgmComBass0300ServiceAsync sysComBass0300Service = PrgmComBass0300Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 


	// -------------- grid 시작 -------------- 
	private Yeta3060Def Yeta3060Def = new Yeta3060Def(); // 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel;


	private Yeta3060Def mYeta3060Def = new Yeta3060Def("Yeta3060_01"); // 주 그리드 테이블 컬럼 define
	...
	// private PrgmComYeta3060DTO sysComYeta3060Dto;
	// -------------- DTO 선언 종료  -------------- 

	private VerticalPanel vp;
	private FormPanel plFrmYeta3060;
	...
	private SelectionListener<ButtonEvent> selectionListener = new SelectionListener<ButtonEvent>() {
		public void componentSelected(ButtonEvent ce) {
			if (ActionDatabase.INSERT.equals(actionDatabase)) {
				actionDatabase = null;
				if (ce.getButton() == btnYeta3060Save) {
					doAction(ActionDatabase.INSERT);
				}
			} else if (ce.getButton() == btnYeta3060Save) {
				doAction(ActionDatabase.UPDATE);
				actionDatabase = null;
			} else if (ce.getButton() == btnYeta3060Del) {
...
//		if("C0030020" == MSFSharedUtils.allowNulls(YetaUtils.getSelectedComboValue(mtstBusinDivCd, "commCd"))
//				|| "C0030020".equals(YetaUtils.getSelectedComboValue(mtstBusinDivCd, "commCd"))
//				|| MSFSharedUtils.paramNull(YetaUtils.getSelectedComboValue(mtstBusinDivCd, "commCd"))
//				) {
//			MessageBox.alert("오류", "주종사업장구분은 현재 주사업장만 가능합니다. <br> 주사업장으로 변경 후 저장해주세요.", null);
//			return;
//		}

		final Yeta3060DTO dto = new Yeta3060DTO();
...
//		}
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					if("DELETE".equals(actionDatabase.name())) {
						...
						yeta3060Service.delChkYeta3060(dto, actionDatabase, new AsyncCallback<Integer>() {
							@Override
							public void onSuccess(Integer result) {
								...
								if (result == 1) {
									...
									// 저장 처리
									saveYeta3060(dto, actionDatabase, fileName);
								}
							}
							...
							public void onFailure(Throwable caught) {
								...
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
										MSFMainApp.ADMINMESSAGES.ExceptionMessageService("delChkYeta3060("+ actionDatabase.name()+ ") : "+ caught),null);
							}
						});
					}else {
						// 저장 처리
	...
	public static native void closeFunction() /*-{
			   $wnd.closeFunction =
			      $entry(@com.app.exterms.yearendtax.client.form.yeta2018.Yeta3060::onCloseGwtFunction(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;));
	  }-*/;
	...
	public ContentPanel getViewPanel() {
		...
		if (panel == null) {
			...
			// 상위 사업장
			yeta3060Service.getParentDpobCd(new AsyncCallback<List<BaseModel>>() {
				@Override
			...
			vp.setSpacing(10);
			createYeta3060Form(); 		// 화면 기본정보를 설정
			 ...
			 */
			// 폼데이터를 바인딩
			formBinding = new FormBinding(this.plFrmYeta3060, true);
	...
	// 생성자 함수에 변수을 받아 처리
	public Yeta3060(String txtForm) {
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createYeta3060Form() {

		plFrmYeta3060 = new FormPanel();
		plFrmYeta3060.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 단위사업장등록관리"));
		plFrmYeta3060.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta3060.setBodyStyleName("pad-text");
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//로그 뷰화면 호출 메서드 
				funcLogMessage("단위사업장등록관리","Yeta3060");
			}
		});
		plFrmYeta3060.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta3060.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		plFrmYeta3060.setPadding(2);
		plFrmYeta3060.setFrame(true);
		plFrmYeta3060.setLayout(new FormLayout());

		degtrSecRegnNum = new HiddenField<String>();
		imgKeyCode = new HiddenField<String>();
		imgKeyCode.setName("imgKeyCode");
		plFrmYeta3060.add(imgKeyCode);
		frmKey = new HiddenField<String>();
		frmKey.setName("frmKey");
		plFrmYeta3060.add(frmKey);
		whdgTxRegrstDeptCd = new HiddenField<String>();
		whdgTxRegrstDeptCd.setName("whdgTxRegrstDeptCd");
		plFrmYeta3060.add(whdgTxRegrstDeptCd);

		topYeta3060Bar = new ButtonBar();
		topYeta3060Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		btnYeta3060Dpob = new Button("주(총괄납부)사업장관리");
		topYeta3060Bar.add(btnYeta3060Dpob);
		btnYeta3060Dpob.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				YetaP306001 yetaP306001Form = new YetaP306001(ActionDatabase.READ, getThis());

				MSFFormWindows msFwYeta306001 = new MSFFormWindows("주(총괄납부)사업장 관리",yetaP306001Form, "닫기", "820px", "710px", false);
				msFwYeta306001.show();
				yetaP306001Form.setMSFFormWindows(msFwYeta306001);
			}
		});

		btnYeta3060New = new Button("신규");
		btnYeta3060New.setIcon(MSFMainApp.ICONS.new16());
		topYeta3060Bar.add(btnYeta3060New);
		btnYeta3060New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 신규 버튼 클릭시 처리
				formYeta3060Init();
		...
		btnYeta3060Save = new Button("저장");
		btnYeta3060Save.setIcon(MSFMainApp.ICONS.save16());
		//		btnYeta3060Save.addSelectionListener(selectionListener);
		btnYeta3060Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
		...
		topYeta3060Bar.add(btnYeta3060Save);

		btnYeta3060Del = new Button("삭제");
		btnYeta3060Del.setIcon(MSFMainApp.ICONS.delete16());
		btnYeta3060Del.addSelectionListener(selectionListener);
		topYeta3060Bar.add(btnYeta3060Del);

		btnYeta3060Sreach = new Button("조회");
		btnYeta3060Sreach.setIcon(MSFMainApp.ICONS.search16());
		topYeta3060Bar.add(btnYeta3060Sreach);
		btnYeta3060Sreach.addListener(Events.Select,
		...
		plFrmYeta3060.add(topYeta3060Bar);

		// 파일업로드 처리 - 사업장 직인 정보
		plFrmYeta3060.setAction("myurl"); // url 정보 추가 할것.
		plFrmYeta3060.setEncoding(Encoding.MULTIPART);
		plFrmYeta3060.setMethod(Method.POST);

		vp.add(plFrmYeta3060);
		plFrmYeta3060.setSize("990px", "680px");
	}
	...
	private void createStandardForm() {
		...
		plFrmYeta3060.add(layoutContainer);
	...
	private LayoutContainer createMasterGrid() {
		...
		msfGridPanel01 = new MSFGridPanel(mYeta3060Def, false, false, false, false,false);
		msfGridPanel01.setHeaderVisible(false);
		msfGridPanel01.setBodyBorder(true);
		msfGridPanel01.setBorders(true);

		final Grid Yeta3060MGrid = msfGridPanel01.getMsfGrid().getGrid();
		Yeta3060MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (msfGridPanel01.getCurrentlySelectedItem() != null) { 
					formYeta3060Init();
					//선택된 값을 우측 그리드에 넣기 위한 메서드 호출 
					leftYeta3060Detail(msfGridPanel01.getCurrentlySelectedItem());
				}
	...
	private void createListGrid() {
		...
		msfGridPanel = new MSFGridPanel(Yeta3060Def, false, false, false, true, true);
		...
		plFrmYeta3060.add(FieldSetGrid);
	...
	private Yeta3060 getThis() {
		return this;
	}

	private void leftYeta3060Detail(BaseModel selRecord) {
	 ...
	 * 저장
	 */
	private void saveYeta3060(final Yeta3060DTO dto, final ActionDatabase actionDatabase, final String fileName) {
			
		yeta3060Service.saveYeta3060(dto, actionDatabase, new AsyncCallback<String>() {
			@Override
			public void onSuccess(String result) {
				...
				if ("DELETE".equals(actionDatabase.name())) {
					...
					if(!"".equals(fileName) && fileName != null){
					    ...
					    plFrmYeta3060.mask();  
					    plFrmYeta3060.setAction(GWT.getHostPageBaseURL() + "upload/ImageFileUpload.do"); 
	                    plFrmYeta3060.submit();
	                    plFrmYeta3060.onFrameLoad(); 
	                    picBoolFile = true;
						//사진 등록처리 servlet 끝 
	                    
						// reset and unmask the form
						// after file upload
	                    plFrmYeta3060.addListener(Events.Submit, new Listener<FormEvent>() {
				              public void handleEvent(FormEvent evt) {  
				                  plFrmYeta3060.unmask(); 
			...
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
						MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta3060("+ actionDatabase.name()+ ") : "+ caught),null);
			}
		});
	}
