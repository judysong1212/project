...
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Ye160510Def;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53001Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53001ServiceAsync;
...
=====================================================**/
public  class YetaT53001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT53001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT53001GridPanel;
	private Ye160510Def ye160510Def = new Ye160510Def("YetaT53001");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT53001ServiceAsync yetaT53001Service = YetaT53001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT53001Service.activityOnYetaT53001(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT53001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT53001 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT53001.setBodyBorder(false);
		cpYetaT53001.setBorders(false);
		cpYetaT53001.setHeaderVisible(false);
		cpYetaT53001.setSize("790px", "640px");
		   
		this.add(cpYetaT53001);
...
//		     });     
		...
		cpYetaT53001.setLayout(new FlowLayout());
		...
		btnYetaAdd.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT53001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT53001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
				List<Record> lsRec = new ArrayList<Record>();

				
				if((ActionDatabase.INSERT).equals(actionDatabase)) {
					YetaT53001GridPanel.getMsfGrid().clearData();
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT53001.add(topYetaBtnBar);
...
//		cpYetaT53001.add(topYetaBtnBar);
		...
		YetaT53001GridPanel = new MSFGridPanel(ye160510Def, false, false, false, false, false);
		YetaT53001GridPanel.setSize("790px", "200px");
		YetaT53001GridPanel.setBorders(true);
		YetaT53001GridPanel.setHeaderVisible(false);
...
//		
//		YetaT53001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT53001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT53001.add(YetaT53001GridPanel);
		...
		fldstNewFieldset.add(YetaT53001GridPanel);
		
		cpYetaT53001.add(fldstNewFieldset);
		cpYetaT53001.add(layoutContainer10);
		cpYetaT53001.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT53001GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT53001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT53001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT53001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT53001GridPanel.reload();
