...
=====================================================**/
public  class YetaT63004   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63004; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63004GridPanel;
	private MSFGridPanel ye162530GridPanel; //임대차계약그리드
	private MSFGridPanel ye162520GridPanel; // 금전소비대차그리드
	private Ye162520Def ye162520Def = new Ye162520Def("YetaT63004");   
	private Ye162530Def ye162530Def = new Ye162530Def("YetaT63004");   
...
//		 }
	...
	public YetaT63004(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63004 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63004.setBodyBorder(false);
		cpYetaT63004.setBorders(false);
		cpYetaT63004.setHeaderVisible(false);
		cpYetaT63004.setSize("790px", "640px");
		   
		this.add(cpYetaT63004);
...
//		     });     
		...
		cpYetaT63004.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63004GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63004GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63004GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63004GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63004GridPanel.getMsfGrid().getGrid().startEditing(YetaT63004GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT63004GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaT63004GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//				
//		cpYetaT63004.add(topYetaBtnBar);
...
//		
//		YetaT63004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63004GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63004.add(YetaT63004GridPanel);
		...
		cpYetaT63004.add(fldstNewFieldset);
		cpYetaT63004.add(fldstNewFieldset1);
		cpYetaT63004.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63004GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63004GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63004GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63004GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63004GridPanel.reload();
