...
import com.app.exterms.yearendtax.client.service.yeta2015.Yeta0500ServiceAsync;
...
=====================================================**/
public class Yeta0500   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmYeta0500; 
    private String txtForm = "";
    private XTemplate detailTp;
 
    private Yeta1000Def yeta1000def = new Yeta1000Def("YETA0500");
...
//		private MSFGridPanel typOccuCdGridPanel;
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
   private ButtonBar topYeta0500Bar;
   private Button btnYeta0500Init; 
//   private Button btnYeta0100Save;
//   private Button btnYeta0100Del;
   private Button btnYeta0500Sreach;
//   private Button btnYeta0100Print;
   private Button btnYeta0500Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 **/
	...
	private Yeta0500ServiceAsync yeta0500Service = Yeta0500Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 
   ...
   // -------------- stroe [검색부]선언 시작 -------------- 
	   ...
	   **/
	 	 ...
	 	 */
	  	private void checkYeta0500Auth( String authAction, ListStore<BaseModel>bm) {
...
//	    
	 ...
	 **/ 	 
		...
		public Yeta0500(String txtForm) {
	 ...
	 **/		  
		private void createYeta0100Form() {
			...
			plFrmYeta0500 = new FormPanel(); 
			plFrmYeta0500.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산대상자관리"));
			plFrmYeta0500.setIcon(MSFMainApp.ICONS.text());
			plFrmYeta0500.setPadding(2);
			plFrmYeta0500.setFrame(true); 
			...
			btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 //로그 뷰화면 호출 메서드 
					  funcLogMessage("연말정산대상자관리","YETA0500");
					}
				});
			plFrmYeta0500.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
			plFrmYeta0500.getHeader().addTool(btnlogView); 
			/************************************************************************
			 * 화면 에러 처리시 필요한 부분으로 추가 
			 ************************************************************************/
 
		     //멀티콤보박스 닫기 
			plFrmYeta0500.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
	            @Override
...
//	                        }
			...
			/** 상단 버튼 ButtonBar**/
			topYeta0500Bar = new ButtonBar();    
			topYeta0500Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
			btnYeta0500Init = new Button("초기화");  
			btnYeta0500Init.setIcon(MSFMainApp.ICONS.new16());
			topYeta0500Bar.add(btnYeta0500Init); 
			btnYeta0500Init.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//초기화 
	    			formInit();
	    		}
			...
			btnYeta0500Sreach = new Button("조 회");  
			btnYeta0500Sreach.setIcon(MSFMainApp.ICONS.search());
			topYeta0500Bar.add(btnYeta0500Sreach);
			btnYeta0500Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//조회버튼 클릭시 처리 
	    			reloadYeta0500();  
	    		}
...
//			topYeta0500Bar.add(btnYeta0100Print);	
			
//			btnYeta0100Print = new Button("인 쇄");  
//			btnYeta0100Print.setIcon(MSFMainApp.ICONS.print16());
//			topYeta0500Bar.add(btnYeta0100Print);	
			
			btnYeta0500Excel = new Button("엑 셀");  
			btnYeta0500Excel.setIcon(MSFMainApp.ICONS.excel16());
			topYeta0500Bar.add(btnYeta0500Excel);
			btnYeta0500Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//엑셀 저장
	    			xlsExportData();  
	    		}
			...
			plFrmYeta0500.add(topYeta0500Bar);
		      
			//파일업로드 처리
			plFrmYeta0500.setAction("myurl");
			plFrmYeta0500.setEncoding(Encoding.MULTIPART);
			plFrmYeta0500.setMethod(Method.POST);		         
			vp.add(plFrmYeta0500);	
			
			plFrmYeta0500.setSize("990px", "680px");	      
			plFrmYeta0500.setLayout(new FlowLayout());
		}
...
//		  	plFrmYeta0500.add(typOccuCdGrid);
...
//														 searchBusinCd.setValue(lsBusinCd.getAt(0));
			    ...
			    lsPayrMangDeptCd.addStoreListener( new StoreListener<BaseModel>() {   
			        public void handleEvent(StoreEvent<BaseModel> be) {  
			        	checkYeta0500Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
			        }
			    });    
				    	   ...
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta0500Auth("srhEmymtDivCd", lsEmymtDivCd); 
			           	 }
			        }
			    });    
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta0500Auth("srhDeptCd", lsDeptCd); 
			           	 }
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		 //  checkYeta0500Auth("srhTypOccuCd", lsTypOccuCd); 
			           	 }
			        }
			    });     
		...
		cp01.addButton(new Button("저장", new SelectionListener<ButtonEvent>() {   
			...
			public void componentSelected(ButtonEvent ce) { 
				...
				if (MSFSharedUtils.paramNotNull(msfGridPanel.getCurrentlySelectedItem())) { 
					...
					yeta0500Service.saveYeta0500Grid(listYeta2000dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta0500Grid(" + "저장" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
...
//					ntime = format.format(c.getTime());
					...
					yeta0500Service.activityOnYeta0500List(listYeta2000dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta0500List(" + "당월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
		...
		cp01.addButton(new Button("당월급여반영취소", new SelectionListener<ButtonEvent>() {   
			@Override  
			public void componentSelected(ButtonEvent ce) {   
...
//					ntime = format.format(c.getTime());
	...
	private Yeta0500 getThis() {
		return this;
	}
		   
	private void reloadYeta0500() {
	...
	private void xlsExportData() {
		...
		if ("".equals(srhEdacRvyy)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta0500Export.do", "extgwtFrame", param);
			}
		} else {
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta0500Export.do", "extgwtFrame", param);
		}
