...
import com.app.exterms.yearendtax.client.form.defs.yeta2016.Ye16Ta3170Def;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13001Service;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13001ServiceAsync;
...
=====================================================**/
public  class YetaP13001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta13001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP13001GridPanel;
	private Ye16Ta3170Def yetaP13001ToYeta3170Def  = new Ye16Ta3170Def("YETAP13001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
   ...
   */
	// -------------- 검색 조건 시작 --------------
 	private TextField<String> srhEdacRvyy;		// 귀속년도
 	private ComboBox<BaseModel> srhSettGbcd; 	// 정산구분
// 	private TextField<String> srhSettGbcd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP13001ServiceAsync yetaP13001Service = YetaP13001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	  ...
	  public YetaP13001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//	        createMsfGridForm();		//그리드필드
		   ...
		   cpYeta13001.setBodyBorder(false);
		   cpYeta13001.setBorders(false);
		   cpYeta13001.setHeaderVisible(false);
		   cpYeta13001.setSize("790px", "640px");
		   
		   this.add(cpYeta13001);
...
//		systemkey.addListener(Events.Add, new Listener<BaseEvent>() {
...
//		     });     
		...
		cpYeta13001.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent be) {
				...
				//테스트용
//				dpobCd.setValue("20144050000001");
//				systemkey.setValue("20080013");
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 yetaP13001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = yetaP13001GridPanel.getTableDef().getDefaultValues();  
...
//		         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
		         ...
		         yetaP13001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         yetaP13001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         yetaP13001GridPanel.getMsfGrid().getGrid().startEditing(yetaP13001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = yetaP13001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP13001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//				srhSettGbcd.setValue(lsSettGbcd.getAt(0));
		...
		cpYeta13001.add(fieldSet);
		cpYeta13001.add(createMsfGridForm(), new FormData("100%"));
	}  
...
//	private void createMsfGridForm() {
		...
		yetaP13001GridPanel = new MSFGridPanel(yetaP13001ToYeta3170Def, false, false, false, false, false);
//		yetaP13001GridPanel.setSize(790, 440);
//		yetaP13001GridPanel.setHeight(400);
		yetaP13001GridPanel.setBorders(false);
		yetaP13001GridPanel.setBodyBorder(false);
		yetaP13001GridPanel.setHeaderVisible(false); 
		yetaP13001GridPanel.getBottomComponent().setVisible(false);
		
		GWTUtils.findColumnConfig(yetaP13001GridPanel, Ye16Ta3170BM.ATTR_DYYRDEAM).setStyle("background-color:#F5F6CE;");
		
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		GWTUtils.findColumnConfig(yetaP13001GridPanel, Yeta3170BM.ATTR_CTRBAMNT).setStyle("Height:20px;");
		
		payrSummaryRow.setHtml(Ye16Ta3170BM.ATTR_CTRBYRMN, "합 계"); 
		
		payrSummaryRow.setCellStyle(Ye16Ta3170BM.ATTR_CTRBAMNT,"summary_color");	/** set 기부조정명세기부금액 : ctrbAmnt */
		payrSummaryRow.setCellStyle(Ye16Ta3170BM.ATTR_BFYRDEAM,"summary_color");	/** set 전년도_공제금액 : bfyrDeam */
		payrSummaryRow.setCellStyle(Ye16Ta3170BM.ATTR_DETRAMNT,"summary_color");	/** set 공제대상금액 : detrAmnt */
		
		payrSummaryRow.setCellStyle(Ye16Ta3170BM.ATTR_DYYRDEAM,"summary_color");	/** set 해당연도공제금액 : dyyrDeam */
		payrSummaryRow.setCellStyle(Ye16Ta3170BM.ATTR_DYYROTAM,"summary_color");	/** set 해당년도_미공제소멸금액 : dyyrOtam */
		payrSummaryRow.setCellStyle(Ye16Ta3170BM.ATTR_AYYRNTAM,"summary_color");	/** set 해당년도_미공제이월금액 : ayyrNtam */
		
							    
		
		
		
		payrSummaryRow.setSummaryType(Ye16Ta3170BM.ATTR_CTRBAMNT, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3170BM.ATTR_CTRBAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
							    
		payrSummaryRow.setSummaryType(Ye16Ta3170BM.ATTR_BFYRDEAM, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3170BM.ATTR_BFYRDEAM, NumberFormat.getFormat("#,##0;(#,##0)"));  
							      
		payrSummaryRow.setSummaryType(Ye16Ta3170BM.ATTR_DETRAMNT, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3170BM.ATTR_DETRAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));
		
		payrSummaryRow.setSummaryType(Ye16Ta3170BM.ATTR_DYYRDEAM, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3170BM.ATTR_DYYRDEAM, NumberFormat.getFormat("#,##0;(#,##0)"));
		
		payrSummaryRow.setSummaryType(Ye16Ta3170BM.ATTR_DYYROTAM, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3170BM.ATTR_DYYROTAM, NumberFormat.getFormat("#,##0;(#,##0)"));
		
		payrSummaryRow.setSummaryType(Ye16Ta3170BM.ATTR_AYYRNTAM, SummaryType.SUM);   
		payrSummaryRow.setSummaryFormat(Ye16Ta3170BM.ATTR_AYYRNTAM, NumberFormat.getFormat("#,##0;(#,##0)"));
		
		
		yetaP13001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		//초기 조회
//		BaseModel bmData = yetaP13001GridPanel.getMsfGrid().getStore().getModels().get(0);  
//		if (MSFSharedUtils.paramNotNull(bmData)){
//			reloadYetaP13001Grid();
//		}
		
//		cp01.add(yetaP13001GridPanel);
//		lcTabFormLayer.add(cp01);
//		
		cpYeta13001.add(yetaP13001GridPanel);
		cp01.add(yetaP13001GridPanel);
		lcTabFormLayer.add(cp01);
//		cpYeta13001.add(lcTabFormLayer);
...
//		return lcTabFormLayer;
	...
	private void yetaP13001FormSave() {  
		if (MSFSharedUtils.paramNotNull(records)) { 
              
			List<Ye16Ta3170DTO> listYeta3170dto = new ArrayList<Ye16Ta3170DTO>();  
			...
			while (records.hasNext()) {
                ...
                if(MSFSharedUtils.paramNull( yeta3170Dto.getEdacRvyy())) {
                	MessageBox.alert("경고", "년도를 입력해 주세요.", null);
                	dataChk = false;
					return; 
                }else if(MSFSharedUtils.paramNull( yeta3170Dto.getCtrbDdcd())) {
                	MessageBox.alert("경고", "유형을 선택해 주세요.", null);
                	dataChk = false;
					return; 
                }else if(MSFSharedUtils.paramNull( yeta3170Dto.getCtrbAmnt())) {
                	MessageBox.alert("경고", "기부금액을 입력해 주세요.", null);
                	dataChk = false;
					return; 
                }else if(MSFSharedUtils.paramNull( yeta3170Dto.getDetrAmnt())) {
                	MessageBox.alert("경고", "공제대상금액을 입력해 주세요.", null);
                	dataChk = false;
					return; 
                }else if(MSFSharedUtils.paramNull( yeta3170Dto.getDyyrDeam())) {
                	MessageBox.alert("경고", "해당연도공제금액을 입력해 주세요.", null);
                	dataChk = false;
					return; 
                }else if(MSFSharedUtils.paramNull( yeta3170Dto.getDyyrOtam())) {
                	MessageBox.alert("경고", "소멸금액을 입력해 주세요.", null);
                	dataChk = false;
					return; 
                }else if(MSFSharedUtils.paramNull( yeta3170Dto.getAyyrNtam())) {
			...
			if(dataChk) {
				...
				yetaP13001Service.activityOnYetaP13001ToYeta3170(listYeta3170dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP13001ToYeta3170(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		if (MSFSharedUtils.paramNotNull(bmData)){
			...
			yetaP13001GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaP13001GridPanel.getTableDef().addColumnFilter("edacRvyy", MSFSharedUtils.allowNulls(srhEdacRvyy.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaP13001GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			
//			yetaP13001GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.allowNulls(srhSettGbcd.getValue())
//					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaP13001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaP13001GridPanel.reload();
