...
import com.app.exterms.yearendtax.client.dto.yeta2020.Ye165010BM;
import com.app.exterms.yearendtax.client.dto.yeta2020.Ye165010DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Ye165010Def;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP53003Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP53003ServiceAsync;
...
=====================================================**/
public  class YetaP53003   extends MSFFormPanel {  
 ...
 **/
	...
	*#######################################################*/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP53003GridPanel;
//	private Yeta0300Def yeta0300Def_08  = new Yeta0300Def("YETA03008");	//가족교육비 그리드 테이블 컬럼 define 
//	private Ye16Ta3180Def YetaP53003ToYe165010  = new Ye16Ta3180Def("YetaP53003");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye165010Def YetaP53003ToYe165010  = new Ye165010Def("YetaP53003");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP53003ServiceAsync yetaP53003Service = YetaP53003Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 	
	...
	private void YetaP53003FormSave() {
...
//				}
			...
			if (dataChk) {
				...
				yetaP53003Service.activityOnYetaP53003ToYe165010(listYe165010Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP53003ToYe165010(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	...
	public YetaP53003(final ActionDatabase actionDatabase, final MSFPanel caller) {
	...
	private void createSearchForm() {
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP53003GridPanel.getMsfGrid().clearData();
					 
				BaseModel bmData = YetaP53003GridPanel.getTableDef().getDefaultValues();  
				...
				YetaP53003GridPanel.getMsfGrid().getGrid().stopEditing();
				YetaP53003GridPanel.getMsfGrid().getGrid().getStore().insert(bmData, 0);
				YetaP53003GridPanel.getMsfGrid().getGrid().startEditing(YetaP53003GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);
...
//				actionDatabase = ActionDatabase.INSERT;
				...
				MessageBox.confirm("삭제", "선택하신 기부금 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
							...
							Iterator<BaseModel> itBm = YetaP53003GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP53003GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	      layoutContainer_2.setBorders(false);
	...
	private void createMsfGridForm() {
		...
		YetaP53003GridPanel = new MSFGridPanel(YetaP53003ToYe165010, false, false, false, false, false);
		YetaP53003GridPanel.setHeaderVisible(false);  
		YetaP53003GridPanel.setBorders(false);
		YetaP53003GridPanel.setHeaderVisible(false);
		YetaP53003GridPanel.getBottomComponent().setVisible(true);
		...
		YetaP53003GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			

		cp01.add(YetaP53003GridPanel);
	...
	public void reload() {
		...
		YetaP53003GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP53003GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP53003GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP53003GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP53003GridPanel.reload();
