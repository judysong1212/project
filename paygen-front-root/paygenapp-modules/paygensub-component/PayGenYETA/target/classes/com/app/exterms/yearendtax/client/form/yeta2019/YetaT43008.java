...
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43008Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43008ServiceAsync;
...
=====================================================**/
public  class YetaT43008   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT43008; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT43008GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT43008ServiceAsync yetaT43008Service = YetaT43008Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT43008Service.activityOnYetaT43008(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT43008(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT43008 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT43008.setBodyBorder(false);
		cpYetaT43008.setBorders(false);
		cpYetaT43008.setHeaderVisible(false);
		cpYetaT43008.setSize("790px", "640px");
		   
		this.add(cpYetaT43008);
...
//		     });     
		...
		cpYetaT43008.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT43008GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT43008GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT43008GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT43008GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT43008GridPanel.getMsfGrid().getGrid().startEditing(YetaT43008GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT43008GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT43008GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT43008.add(topYetaBtnBar);
...
//		cpYetaT43008.add(topYetaBtnBar);
//		
//		YetaT43008GridPanel = new MSFGridPanel(YetaT43008Def, false, false, false, false, false);
//		YetaT43008GridPanel.setSize("790px", "420px");
//		YetaT43008GridPanel.setBorders(true);
//		YetaT43008GridPanel.setHeaderVisible(false);
//		
...
//		
//		YetaT43008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT43008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT43008.add(YetaT43008GridPanel);
		...
		cpYetaT43008.add(fldstNewFieldset);
		cpYetaT43008.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT43008GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT43008GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT43008GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT43008GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT43008GridPanel.reload();
