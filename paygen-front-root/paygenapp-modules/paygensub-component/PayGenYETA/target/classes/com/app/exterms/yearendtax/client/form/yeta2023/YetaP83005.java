...
import com.app.exterms.yearendtax.client.form.defs.yeta2023.Ye162510Def;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP83005Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaP83005ServiceAsync;
...
=====================================================**/
public  class YetaP83005   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta83005; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP83005GridPanel;
//	private Ye16Ta3130Def yetaP83004ToYe162510Def  = new Ye16Ta3130Def("YETAP13005");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye162510Def yetaP83004ToYe162510Def  = new Ye162510Def("YetaP83005");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP83005ServiceAsync yetaP83005Service = YetaP83005Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
 ...
 **/ 
     ...
     */ 
		...
		private void YetaP83005FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				yetaP83005Service.activityOnYetaP83005ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("YetaP83005FormSave(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	...
	public YetaP83005(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYeta83005 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYeta83005.setBodyBorder(false);
		cpYeta83005.setBorders(false);
		cpYeta83005.setHeaderVisible(false);
		cpYeta83005.setSize("790px", "480px");
		   
		this.add(cpYeta83005);
...
//		     });     
		...
		cpYeta83005.setLayout(new FlowLayout());
...
//				systemkey.setValue("20080013");
				...
				YetaP83005GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = YetaP83005GridPanel.getTableDef().getDefaultValues();  
...
//		         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
		        ...
		        YetaP83005GridPanel.getMsfGrid().getGrid().stopEditing();      
		        YetaP83005GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        YetaP83005GridPanel.getMsfGrid().getGrid().startEditing(YetaP83005GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 월세액소득공제 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP83005GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP83005GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//		});
	...
	private void createMsfGridForm() {
		...
		YetaP83005GridPanel = new MSFGridPanel(yetaP83004ToYe162510Def, false, false, false, false, false);
		YetaP83005GridPanel.setSize("790px", "400px");
		YetaP83005GridPanel.setBorders(false);
		YetaP83005GridPanel.setHeaderVisible(false);
		...
		YetaP83005GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
		
			
		cpYeta83005.add(YetaP83005GridPanel);
	}	
	...
	public void reload() {
			...
			YetaP83005GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaP83005GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
			YetaP83005GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaP83005GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaP83005GridPanel.reload();
