...
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Yeta6060Def;
import com.app.exterms.yearendtax.client.service.yeta2021.Yeta6060Service;
import com.app.exterms.yearendtax.client.service.yeta2021.Yeta6060ServiceAsync;
 ...
 * =========================================================================
 **/
public class Yeta6060 extends MSFPanel {
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta6060Bar;
	private Button btnYeta6060Dpob; 
	private Button btnYeta6060New; 
	private Button btnYeta6060Save;
	private Button btnYeta6060Del;
	private Button btnYeta6060Sreach;
	// -------------- 상단 버튼 종료 --------------
...
//	private MSFComboBox<BaseModel> hhrkUntDpobCd; 					// 상위 단위사업장 코드
	...
	private CheckBoxGroup chkgrpYeta6060;   						// 체크 그룹
	...
	// -------------- 서비스 호출 시작 -------------- 
	//private InfcYeta6060ServiceAsync infcYeta6060Service = InfcYeta6060Service.Util.getInstance();
	private Yeta6060ServiceAsync yeta6060Service = Yeta6060Service.Util.getInstance();
	private PrgmComBass0300ServiceAsync sysComBass0300Service = PrgmComBass0300Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 


	// -------------- grid 시작 -------------- 
	private Yeta6060Def Yeta6060Def = new Yeta6060Def(); // 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel;


	private Yeta6060Def mYeta6060Def = new Yeta6060Def("Yeta6060_01"); // 주 그리드 테이블 컬럼 define
	...
	// private PrgmComYeta6060DTO sysComYeta6060Dto;
	// -------------- DTO 선언 종료  -------------- 

	private VerticalPanel vp;
	private FormPanel plFrmYeta6060;
	...
	private SelectionListener<ButtonEvent> selectionListener = new SelectionListener<ButtonEvent>() {
		public void componentSelected(ButtonEvent ce) {
			if (ActionDatabase.INSERT.equals(actionDatabase)) {
				actionDatabase = null;
				if (ce.getButton() == btnYeta6060Save) {
					doAction(ActionDatabase.INSERT);
				}
			} else if (ce.getButton() == btnYeta6060Save) {
				doAction(ActionDatabase.UPDATE);
				actionDatabase = null;
			} else if (ce.getButton() == btnYeta6060Del) {
	...
	public void save(final ActionDatabase actionDatabase) {
		...
		final Yeta6060DTO dto = new Yeta6060DTO();
...
//		}
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					if("DELETE".equals(actionDatabase.name())) {
						...
						yeta6060Service.delChkYeta6060(dto, actionDatabase, new AsyncCallback<Integer>() {
							@Override
							public void onSuccess(Integer result) {
								...
								if (result == 1) {
									...
									// 저장 처리
									saveYeta6060(dto, actionDatabase, fileName);
								}
							}
							...
							public void onFailure(Throwable caught) {
								...
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
										MSFMainApp.ADMINMESSAGES.ExceptionMessageService("delChkYeta6060("+ actionDatabase.name()+ ") : "+ caught),null);
							}
						});
					}else {
						// 저장 처리
	...
	public static native void closeFunction() /*-{
			   $wnd.closeFunction =
			      $entry(@com.app.exterms.yearendtax.client.form.yeta2021.Yeta6060::onCloseGwtFunction(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;));
	  }-*/;
	...
	public ContentPanel getViewPanel() {
		...
		if (panel == null) {
			...
			// 상위 사업장
			yeta6060Service.getParentDpobCd(new AsyncCallback<List<BaseModel>>() {
				@Override
			...
			vp.setSpacing(10);
			createYeta6060Form(); 		// 화면 기본정보를 설정
			 ...
			 */
			// 폼데이터를 바인딩
			formBinding = new FormBinding(this.plFrmYeta6060, true);
	...
	// 생성자 함수에 변수을 받아 처리
	public Yeta6060(String txtForm) {
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createYeta6060Form() {

		plFrmYeta6060 = new FormPanel();
		plFrmYeta6060.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 단위사업장등록관리"));
		plFrmYeta6060.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta6060.setBodyStyleName("pad-text");
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//로그 뷰화면 호출 메서드 
				funcLogMessage("단위사업장등록관리","Yeta6060");
			}
		});
		plFrmYeta6060.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta6060.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		plFrmYeta6060.setPadding(2);
		plFrmYeta6060.setFrame(true);
		plFrmYeta6060.setLayout(new FormLayout());

		degtrSecRegnNum = new HiddenField<String>();
		imgKeyCode = new HiddenField<String>();
		imgKeyCode.setName("imgKeyCode");
		plFrmYeta6060.add(imgKeyCode);
		frmKey = new HiddenField<String>();
		frmKey.setName("frmKey");
		plFrmYeta6060.add(frmKey);
		whdgTxRegrstDeptCd = new HiddenField<String>();
		whdgTxRegrstDeptCd.setName("whdgTxRegrstDeptCd");
		plFrmYeta6060.add(whdgTxRegrstDeptCd);

		topYeta6060Bar = new ButtonBar();
		topYeta6060Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		btnYeta6060Dpob = new Button("주(총괄납부)사업장관리");
		topYeta6060Bar.add(btnYeta6060Dpob);
		btnYeta6060Dpob.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				YetaP606001 yetaP606001Form = new YetaP606001(ActionDatabase.READ, getThis());

				MSFFormWindows msFwYeta606001 = new MSFFormWindows("주(총괄납부)사업장 관리",yetaP606001Form, "닫기", "820px", "710px", false);
				msFwYeta606001.show();
				yetaP606001Form.setMSFFormWindows(msFwYeta606001);
			}
		});

		btnYeta6060New = new Button("신규");
		btnYeta6060New.setIcon(MSFMainApp.ICONS.new16());
		topYeta6060Bar.add(btnYeta6060New);
		btnYeta6060New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 신규 버튼 클릭시 처리
				formYeta6060Init();
		...
		btnYeta6060Save = new Button("저장");
		btnYeta6060Save.setIcon(MSFMainApp.ICONS.save16());
		//		btnYeta6060Save.addSelectionListener(selectionListener);
		btnYeta6060Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
		...
		topYeta6060Bar.add(btnYeta6060Save);

		btnYeta6060Del = new Button("삭제");
		btnYeta6060Del.setIcon(MSFMainApp.ICONS.delete16());
		btnYeta6060Del.addSelectionListener(selectionListener);
		topYeta6060Bar.add(btnYeta6060Del);

		btnYeta6060Sreach = new Button("조회");
		btnYeta6060Sreach.setIcon(MSFMainApp.ICONS.search16());
		topYeta6060Bar.add(btnYeta6060Sreach);
		btnYeta6060Sreach.addListener(Events.Select,
		...
		plFrmYeta6060.add(topYeta6060Bar);

		// 파일업로드 처리 - 사업장 직인 정보
		plFrmYeta6060.setAction("myurl"); // url 정보 추가 할것.
		plFrmYeta6060.setEncoding(Encoding.MULTIPART);
		plFrmYeta6060.setMethod(Method.POST);

		vp.add(plFrmYeta6060);
		plFrmYeta6060.setSize("990px", "680px");
	}
	...
	private void createStandardForm() {
		...
		plFrmYeta6060.add(layoutContainer);
	...
	private LayoutContainer createMasterGrid() {
		...
		msfGridPanel01 = new MSFGridPanel(mYeta6060Def, false, false, false, false,false);
		msfGridPanel01.setHeaderVisible(false);
		msfGridPanel01.setBodyBorder(true);
		msfGridPanel01.setBorders(true);

		final Grid Yeta6060MGrid = msfGridPanel01.getMsfGrid().getGrid();
		Yeta6060MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (msfGridPanel01.getCurrentlySelectedItem() != null) { 
					formYeta6060Init();
					//선택된 값을 우측 그리드에 넣기 위한 메서드 호출 
					leftYeta6060Detail(msfGridPanel01.getCurrentlySelectedItem());
				}
	...
	private void createListGrid() {
		...
		msfGridPanel = new MSFGridPanel(Yeta6060Def, false, false, false, true, true);
		...
		plFrmYeta6060.add(FieldSetGrid);
	...
	private Yeta6060 getThis() {
		return this;
	}

	private void leftYeta6060Detail(BaseModel selRecord) {
	 ...
	 * 저장
	 */
	private void saveYeta6060(final Yeta6060DTO dto, final ActionDatabase actionDatabase, final String fileName) {
			
		yeta6060Service.saveYeta6060(dto, actionDatabase, new AsyncCallback<String>() {
			@Override
			public void onSuccess(String result) {
				...
				if ("DELETE".equals(actionDatabase.name())) {
					...
					if(!"".equals(fileName) && fileName != null){
					    ...
					    plFrmYeta6060.mask();  
					    plFrmYeta6060.setAction(GWT.getHostPageBaseURL() + "upload/ImageFileUpload.do"); 
	                    plFrmYeta6060.submit();
	                    plFrmYeta6060.onFrameLoad(); 
	                    picBoolFile = true;
						//사진 등록처리 servlet 끝 
	                    
						// reset and unmask the form
						// after file upload
	                    plFrmYeta6060.addListener(Events.Submit, new Listener<FormEvent>() {
				              public void handleEvent(FormEvent evt) {  
				                  plFrmYeta6060.unmask(); 
			...
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
						MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta6060("+ actionDatabase.name()+ ") : "+ caught),null);
			}
		});
	}
