...
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43011Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43011ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT43011Def;
...
=====================================================**/
public  class YetaT43011   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT43011; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT43011GridPanel;
//	private YetaT43011Def YetaT43011Def = new YetaT43011Def("YetaT43011");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT43011ServiceAsync yetaT43011Service = YetaT43011Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT43011Service.activityOnYetaT43011(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT43011(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT43011 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT43011.setBodyBorder(false);
		cpYetaT43011.setBorders(false);
		cpYetaT43011.setHeaderVisible(false);
		cpYetaT43011.setSize("790px", "640px");
		   
		this.add(cpYetaT43011);
...
//		     });     
		...
		cpYetaT43011.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT43011GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT43011GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT43011GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT43011GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT43011GridPanel.getMsfGrid().getGrid().startEditing(YetaT43011GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT43011GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT43011GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT43011.add(topYetaBtnBar);
...
//		YetaT43011GridPanel = new MSFGridPanel(YetaT43011Def, false, false, false, false, false);
//		YetaT43011GridPanel.setSize("790px", "420px");
//		YetaT43011GridPanel.setBorders(true);
//		YetaT43011GridPanel.setHeaderVisible(false);
...
//		
//		YetaT43011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT43011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT43011.add(YetaT43011GridPanel);
...
//		
		...
		cpYetaT43011.add(layoutContainer);
		cpYetaT43011.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT43011GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT43011GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT43011GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT43011GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT43011GridPanel.reload();
