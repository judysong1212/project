...
import com.app.exterms.yearendtax.client.service.yeta2017.Yeta2200Service;
import com.app.exterms.yearendtax.client.service.yeta2017.Yeta2200ServiceAsync;
 ...
 * 
 * <pre>
 * 1. 패키지명 : com.app.exterms.yearendtax.client.form.yeta2017
 * 2. 타입명 : Yeta2200.java
 ...
 * </pre>
 */
public class Yeta2200 extends MSFPanel {
	...
	private FormPanel plFrmYeta2200;
	private String txtForm = "";


	private MSFGridPanel yeta2200GridPanel;
	private MSFGridPanel yeta2200XmlGridPanel;
	private Ye161010Def yeta2200def = new Ye161010Def("YETA2200");
	private Ye161010Def yeta2200defXml = new Ye161010Def("YETA2200_RESULT");
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta2200Bar;
	private Button btnYeta2200Sec; 	// 초기화
	private Button btnYeta2200Reset; 	// 초기화
	private Button btnYeta2200New; 		// 신규
	private Button btnYeta2200Save; 	// 저장
	private Button btnYeta2200Del; 		// 삭제
	private Button btnYeta2200Sreach; 	// 조회
	private Button btnYeta2200Print; 	// 인쇄
	private Button btnYetaP220001; 	// 상세정보 팝업
	private Button btnYetaP220002; 	// 상세정보 팝업
	 ...
	 * ++++++++++++++++++++++
	 **/
	private Yeta2200ServiceAsync yeta2200Service = Yeta2200Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2017")));
	...
	public Yeta2200(String txtForm) {
	...
	private void createYeta0200Form() {
		...
		plFrmYeta2200 = new FormPanel();
		plFrmYeta2200.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산파일업로드"));
		plFrmYeta2200.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta2200.setPadding(2);
		plFrmYeta2200.setFrame(true);
		...
		plFrmYeta2200.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta2200.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		//멀티콤보박스 닫기 
		plFrmYeta2200.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar **/
		topYeta2200Bar = new ButtonBar();
		topYeta2200Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		if ("admin".equals(MSFMainApp.get().getUser().getUsrId())) {	
			btnYeta2200Sec = new Button("암호화패치");
			btnYeta2200Sec.setIcon(MSFMainApp.ICONS.new16());
			topYeta2200Bar.add(btnYeta2200Sec);
			btnYeta2200Sec.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
				 
					Ye161010DTO ye161010Dto = new Ye161010DTO(); 
						   
					yeta2200Service.yetaChangeConvert(ye161010Dto,  new AsyncCallback<String>(){
				                 public void onFailure(Throwable caught) {
				                     MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
				                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("yeta2200Service : " + caught), null);
				                 }
		...
		btnYeta2200Reset = new Button("초기화");
		btnYeta2200Reset.setIcon(MSFMainApp.ICONS.new16());
		topYeta2200Bar.add(btnYeta2200Reset);
		btnYeta2200Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//초기화 
				formInit();
				yeta2200GridPanel.getMsfGrid().clearData(); 
			}
		});  
		...
		btnYeta2200Sreach = new Button("조 회");
		btnYeta2200Sreach.setIcon(MSFMainApp.ICONS.search());
		topYeta2200Bar.add(btnYeta2200Sreach);
		btnYeta2200Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//조회버튼 클릭시 처리 
				reloadYeta2200();  
			}
		...
		plFrmYeta2200.add(topYeta2200Bar);

		// 파일업로드 처리
		plFrmYeta2200.setAction("myurl");
		plFrmYeta2200.setEncoding(Encoding.MULTIPART);
		plFrmYeta2200.setMethod(Method.POST);

		vp.add(plFrmYeta2200);
		plFrmYeta2200.setSize("990px", "830px");
		plFrmYeta2200.setLayout(new FlowLayout());
...
//		});
	...
	private void createMsfGridForm() {
		...
		yeta2200GridPanel = new MSFGridPanel(yeta2200def, false, false, false, false,false);
		yeta2200GridPanel.setHeaderVisible(false);
		yeta2200GridPanel.setSize(470, 640);
		yeta2200GridPanel.setBorders(true);
		final Grid yeta2000DefGrid = yeta2200GridPanel.getMsfGrid().getGrid();
		yeta2000DefGrid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (yeta2200GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = new BaseModel();
					formInit() ;
					bmData = yeta2200GridPanel.getCurrentlySelectedItem();
		...
		yeta2200GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 4, new HeaderGroupConfig("소득공제자료", 1, 2));

		fsGird.add(yeta2200GridPanel);
		...
		yeta2200XmlGridPanel = new MSFGridPanel(yeta2200defXml, false, false,false, false);
		yeta2200XmlGridPanel.setSize(250, 305);
		yeta2200XmlGridPanel.setBorders(true);
		yeta2200XmlGridPanel.setStyleAttribute("marginTop", "5px");
		layoutContainer17.add(yeta2200XmlGridPanel,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.8));

		btnYetaP220001 = new Button("개별간소화파일");
		btnYetaP220001.setStyleAttribute("marginTop", "5px");
		btnYetaP220001.setStyleAttribute("marginLeft", "5px");
		btnYetaP220001.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					...
					final MSFFormWindows msFwYeta220001 = new MSFFormWindows("자료추출상세내역",yetaP220001Form, "닫기", "920px", "710px", false);
					msFwYeta220001.show();
					yetaP220001Form.setMSFFormWindows(msFwYeta220001);

					//					funcSetPopUpRecord();

					yetaP220001Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP220001,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));

		btnYetaP220002 = new Button("일괄간소화파일");
		btnYetaP220002.setStyleAttribute("marginTop", "5px");
		btnYetaP220002.setStyleAttribute("marginLeft", "5px");
		btnYetaP220002.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					Window.alert("정산대상자를 선택해주세요.");
				}
					...
					yetaP220002Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP220002,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));

		layoutContainer1_4.add(layoutContainer17, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.495));

		plFrmYeta2200.add(layoutContainer1_4);
	}
	...
	private Yeta2200 getThis(){
		return this;
	}

	private void reloadYeta2200(){

		IColumnFilter filters = null;
		yeta2200GridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		yeta2200GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 

		//정산구분
		yeta2200GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 

		yeta2200GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		yeta2200GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd,srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		yeta2200GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		yeta2200GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		yeta2200GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		yeta2200GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);


		yeta2200GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		yeta2200GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
		yeta2200GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   

		yeta2200GridPanel.reload();  

	}

	private void reloadYeta2200Result(){

		IColumnFilter filters = null;
		yeta2200XmlGridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		yeta2200XmlGridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 
		//정산구분
		yeta2200XmlGridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		yeta2200XmlGridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		yeta2200XmlGridPanel.reload();  
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//submit the form
							plFrmYeta2200.mask("PDF 전자문서 업데이트 중입니다. 업로드 중..."); 

							//파일업로드 처리
							plFrmYeta2200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta2200Import.do");
							plFrmYeta2200.setEncoding(Encoding.MULTIPART);
							plFrmYeta2200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta2200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
							plFrmYeta2200.submit();  
							plFrmYeta2200.onFrameLoad();  
							picBoolFile = true;
						...
						// after file upload 
						plFrmYeta2200.addListener(Events.Submit, new Listener<FormEvent>() {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//파일업로드 처리
							plFrmYeta2200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta2200DdcImport.do");
							plFrmYeta2200.setEncoding(Encoding.MULTIPART);
							plFrmYeta2200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta2200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
