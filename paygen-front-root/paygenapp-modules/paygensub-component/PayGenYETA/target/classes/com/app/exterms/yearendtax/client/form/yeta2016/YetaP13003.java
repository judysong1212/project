...
import com.app.exterms.yearendtax.client.dto.yeta2016.Ye16Ta3180DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2016.Ye16Ta3180Def;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13003Service;
import com.app.exterms.yearendtax.client.service.yeta2016.YetaP13003ServiceAsync;
...
=====================================================**/
public  class YetaP13003   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta13003; 
	...
	*#######################################################*/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP13003GridPanel;
//	private Yeta0300Def yeta0300Def_08  = new Yeta0300Def("YETA03008");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye16Ta3180Def yetaP13003ToYeta3180  = new Ye16Ta3180Def("YETAP13003");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
 ...
 **/ 
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP13003ServiceAsync yetaP13003Service = YetaP13003Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 	
     ...
     */  
		...
		private void yetaP13003FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				
				Boolean dataChk = true;
	              
				List<Ye16Ta3180DTO> listYeta3180dto = new ArrayList<Ye16Ta3180DTO>();  
				...
				while (records.hasNext()) {
	                ...
	                Ye16Ta3180DTO yeta3180Dto= new Ye16Ta3180DTO();  
	                ...
	                yeta3180Dto.setCtrbSeqn((Long) bmMapModel.get("ctrbSeqn"));    							/** column 기부내역일련번호 : ctrbSeqn */
	                yeta3180Dto.setCtrbCtnt(MSFSharedUtils.allowNulls(bmMapModel.get("ctrbCtnt")));    		/** column 기부금유형콤보 : ctrbCtnt */
	                yeta3180Dto.setCtrbAmnt((Long) bmMapModel.get("ctrbAmnt")); 							/** column 기부금액 : ctrbAmnt */
	                yeta3180Dto.setCtrbNcnt((Long)bmMapModel.get("ctrbNcnt"));   ///건수
	                yeta3180Dto.setCtrbTgam((Long) bmMapModel.get("ctrbTgam")); 							/** column 기부금액 : ctrbAmnt */
	                yeta3180Dto.setCtjaCham((Long) bmMapModel.get("ctjaCham")); 							/** column 기부장려신청금액 : ctjaCham */
	                
	                
	                /** 그리드에 있는 항목에 대해서 입력 체크 **/
	                if(MSFSharedUtils.paramNull( yeta3180Dto.getEdacRvyy())) {
	                	MessageBox.alert("경고", "연도를 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtrbDdcd())) {
	                	MessageBox.alert("경고", "유형을 선택해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtrbCont())) {
	                	MessageBox.alert("경고", "기부내용을 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtrbBsnu())) {
	                	MessageBox.alert("경고", "기부처명을 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtrbBsnu().replace("-", ""))) {
	                	MessageBox.alert("경고", "사업자번호를 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getCrtbCncd())) {
	                	MessageBox.alert("경고", "기부자관계를 선택해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getCbtgName())) {
	                	MessageBox.alert("경고", "기부자성명을 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getRsnoNumb().replace("-", ""))) {
	                	MessageBox.alert("경고", "주민번호를 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                }else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtrbNcnt())) {
	                	MessageBox.alert("경고", "건수를 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                } else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtrbAmnt())) {
	                	MessageBox.alert("경고", "기부금액을 입력해 주세요.", null);
	                	dataChk = false;
						return; 
	                }
	                
//	                else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtrbTgam())) {
//	                	MessageBox.alert("경고", "기부금액을 입력해 주세요.", null);
//	                	dataChk = false;
//						return; 
//	                }
	                else if(MSFSharedUtils.paramNull( yeta3180Dto.getCtjaCham())) {
				...
				if(dataChk) {
					...
					yetaP13003Service.activityOnYetaP13003ToYeta3180(listYeta3180dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP13001ToYeta3180(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	  ...
	  public YetaP13003(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			initLoad(); 
		   ...
		   cpYeta13003.setBodyBorder(false);
		   cpYeta13003.setBorders(false);
		   cpYeta13003.setHeaderVisible(false);
		   cpYeta13003.setSize("790px", "640px");
		   
		   this.add(cpYeta13003);
...
//			     });     
			...
			cpYeta13003.setLayout(new FlowLayout());
...
//					systemkey.setValue("20080013");
					...
					yetaP13003GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = yetaP13003GridPanel.getTableDef().getDefaultValues();  
...
//			         bmData.set("ctrbCtnt", "");    /** column 기부금유형콤보TEMP : ctrbCtnt */
			         ...
			         yetaP13003GridPanel.getMsfGrid().getGrid().stopEditing();      
			         yetaP13003GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         yetaP13003GridPanel.getMsfGrid().getGrid().startEditing(yetaP13003GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//					doAction(actionDatabase);
			...
			btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					Iterator<BaseModel> itBm = yetaP13003GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					setListRecord(yetaP13003GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
					doAction(actionDatabase);
...
//		private LayoutContainer createMsfGridForm() {
		private void createMsfGridForm() {
			...
			yetaP13003GridPanel = new MSFGridPanel(yetaP13003ToYeta3180, false, false, false, false, false);
			yetaP13003GridPanel.setHeaderVisible(false);  
			yetaP13003GridPanel.setSize("790px", "420px");
			yetaP13003GridPanel.setBorders(true);
			yetaP13003GridPanel.setHeaderVisible(false);
			yetaP13003GridPanel.getBottomComponent().setVisible(false);
			
			
			/** --- 그리드 합계 부분  시작 --- **/
			AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();
			
			payrSummaryRow.setHtml(Ye16Ta3180BM.ATTR_CTRBYRMN, "합 계");
			 
			payrSummaryRow.setCellStyle(Ye16Ta3180BM.ATTR_CTRBCTNT,"summary_color");	/** set 기부금유형콤보TEMP : ctrbCtnt */
			payrSummaryRow.setCellStyle(Ye16Ta3180BM.ATTR_CTRBAMNT,"summary_color");	/** set 기부금액 : ctrbAmnt */
								    
			payrSummaryRow.setSummaryType(Ye16Ta3180BM.ATTR_CTRBCTNT, SummaryType.SUM);   
			payrSummaryRow.setSummaryFormat(Ye16Ta3180BM.ATTR_CTRBCTNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
								    
			payrSummaryRow.setSummaryType(Ye16Ta3180BM.ATTR_CTRBAMNT, SummaryType.SUM);   
			payrSummaryRow.setSummaryFormat(Ye16Ta3180BM.ATTR_CTRBAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
			
			yetaP13003GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			/** --- 그리드 합계 부분  종료 --- **/
			

			cp01.add(yetaP13003GridPanel);
			
			cpYeta13003.add(cp01);
		...
		public void reload() {
				...
				yetaP13003GridPanel.getTableDef().setTableColumnFilters(filters);
				yetaP13003GridPanel.getTableDef().addColumnFilter("edacRvyy", MSFSharedUtils.allowNulls(srhEdacRvyy.getValue())
	...
	//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
				yetaP13003GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd")
						, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
				yetaP13003GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
						, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
				yetaP13003GridPanel.reload();
