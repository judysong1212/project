...
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3500Service;
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3500ServiceAsync;
...
=====================================================**/
public class Yeta3500   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmYeta3500; 
    private String txtForm = "";
    private XTemplate detailTp;
 
    private Yeta1000Def yeta1000def = new Yeta1000Def("Yeta3500");
...
//		private MSFGridPanel typOccuCdGridPanel;
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
   private ButtonBar topYeta3500Bar;
   private Button btnYeta3500Init; 
//   private Button btnYeta0100Save;
//   private Button btnYeta0100Del;
   private Button btnYeta3500Sreach;
//   private Button btnYeta0100Print;
   private Button btnYeta3500Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 **/
	...
	private Yeta3500ServiceAsync yeta3500Service = Yeta3500Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 
   ...
   // -------------- stroe [검색부]선언 시작 -------------- 
	   ...
	   **/
	 	 ...
	 	 */
	  	private void checkYeta3500Auth( String authAction, ListStore<BaseModel>bm) {
...
//	    
	 ...
	 **/ 	 
		...
		public Yeta3500(String txtForm) {
	 ...
	 **/		  
		private void createYeta0100Form() {
			...
			plFrmYeta3500 = new FormPanel(); 
			plFrmYeta3500.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산급여반영"));
			plFrmYeta3500.setIcon(MSFMainApp.ICONS.text());
			plFrmYeta3500.setPadding(2);
			plFrmYeta3500.setFrame(true); 
			...
			btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 //로그 뷰화면 호출 메서드 
					  funcLogMessage("연말정산급여반영","Yeta3500");
					}
				});
			plFrmYeta3500.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
			plFrmYeta3500.getHeader().addTool(btnlogView); 
			/************************************************************************
			 * 화면 에러 처리시 필요한 부분으로 추가 
			 ************************************************************************/
 
		     //멀티콤보박스 닫기 
			plFrmYeta3500.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
	            @Override
...
//	                        }
			...
			/** 상단 버튼 ButtonBar**/
			topYeta3500Bar = new ButtonBar();    
			topYeta3500Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
			btnYeta3500Init = new Button("초기화");  
			btnYeta3500Init.setIcon(MSFMainApp.ICONS.new16());
			topYeta3500Bar.add(btnYeta3500Init); 
			btnYeta3500Init.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//초기화 
	    			formInit();
	    		}
			...
			btnYeta3500Sreach = new Button("조 회");  
			btnYeta3500Sreach.setIcon(MSFMainApp.ICONS.search());
			topYeta3500Bar.add(btnYeta3500Sreach);
			btnYeta3500Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//조회버튼 클릭시 처리 
	    			reloadYeta3500();  
	    		}
...
//			topYeta3500Bar.add(btnYeta0100Print);	
			
//			btnYeta0100Print = new Button("인 쇄");  
//			btnYeta0100Print.setIcon(MSFMainApp.ICONS.print16());
//			topYeta3500Bar.add(btnYeta0100Print);	
			
			btnYeta3500Excel = new Button("엑 셀");  
			btnYeta3500Excel.setIcon(MSFMainApp.ICONS.excel16());
			topYeta3500Bar.add(btnYeta3500Excel);
			btnYeta3500Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//엑셀 저장
	    			xlsExportData();  
	    		}
			...
			plFrmYeta3500.add(topYeta3500Bar);
		      
			//파일업로드 처리
			plFrmYeta3500.setAction("myurl");
			plFrmYeta3500.setEncoding(Encoding.MULTIPART);
			plFrmYeta3500.setMethod(Method.POST);		         
			vp.add(plFrmYeta3500);	
			
			plFrmYeta3500.setSize("990px", "680px");	      
			plFrmYeta3500.setLayout(new FlowLayout());
		}
...
//		  	plFrmYeta3500.add(typOccuCdGrid);
...
//														 searchBusinCd.setValue(lsBusinCd.getAt(0));
			    ...
			    lsPayrMangDeptCd.addStoreListener( new StoreListener<BaseModel>() {   
			        public void handleEvent(StoreEvent<BaseModel> be) {  
			        	checkYeta3500Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
			        }
			    });    
				    	   ...
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta3500Auth("srhEmymtDivCd", lsEmymtDivCd); 
			           	 }
			        }
			    });    
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta3500Auth("srhDeptCd", lsDeptCd); 
			           	 }
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		 //  checkYeta3500Auth("srhTypOccuCd", lsTypOccuCd); 
			           	 }
			        }
			    });     
...
//		                BaseModel bmMapModel = (BaseModel)record.getModel();
					...
					yeta3500Service.saveYeta3500Grid(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta3500Grid(" + "저장" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
...
//		                ye161010Dto.setDivdPymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("divdPymtDivCd$commCd")));/** column 분할납부구분코드 : divdPymtDivCd */
					...
					yeta3500Service.activityOnYeta3500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta3500List(" + "2월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
...
//					ntime += fmt2.format(today);
					...
					yeta3500Service.activityOnYeta3500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta3500List(" + "3월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
...
//					ntime = format.format(c.getTime());
					...
					yeta3500Service.activityOnYeta3500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta3500List(" + "당월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
	...
	private Yeta3500 getThis() {
		return this;
	}
		   
	private void reloadYeta3500() {
	...
	private void xlsExportData() {
		...
		if ("".equals(srhYrtxBlggYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta3500Export.do", "extgwtFrame", param);
			}
		} else {
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta3500Export.do", "extgwtFrame", param);
		}
