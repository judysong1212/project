...
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83008Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83008ServiceAsync;
...
=====================================================**/
public  class YetaT83008   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT83008; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT83008GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT83008ServiceAsync yetaT83008Service = YetaT83008Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT83008Service.activityOnYetaT83008(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT83008(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT83008 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT83008.setBodyBorder(false);
		cpYetaT83008.setBorders(false);
		cpYetaT83008.setHeaderVisible(false);
		cpYetaT83008.setSize("790px", "640px");
		   
		this.add(cpYetaT83008);
...
//		     });     
		...
		cpYetaT83008.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT83008GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT83008GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT83008GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT83008GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT83008GridPanel.getMsfGrid().getGrid().startEditing(YetaT83008GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT83008GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT83008GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT83008.add(topYetaBtnBar);
...
//		cpYetaT83008.add(topYetaBtnBar);
//		
//		YetaT83008GridPanel = new MSFGridPanel(YetaT83008Def, false, false, false, false, false);
//		YetaT83008GridPanel.setSize("790px", "420px");
//		YetaT83008GridPanel.setBorders(true);
//		YetaT83008GridPanel.setHeaderVisible(false);
//		
...
//		
//		YetaT83008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT83008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT83008.add(YetaT83008GridPanel);
		...
		cpYetaT83008.add(fldstNewFieldset);
		cpYetaT83008.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT83008GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT83008GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT83008GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT83008GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT83008GridPanel.reload();
