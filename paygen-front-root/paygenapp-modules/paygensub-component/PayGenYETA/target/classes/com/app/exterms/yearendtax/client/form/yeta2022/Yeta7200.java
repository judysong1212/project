...
import com.app.exterms.yearendtax.client.service.yeta2022.Yeta7200Service;
import com.app.exterms.yearendtax.client.service.yeta2022.Yeta7200ServiceAsync;
 ...
 * </pre>
 */
public class Yeta7200 extends MSFPanel {
	...
	private FormPanel plFrmYeta7200;
	private String txtForm = "";


	private MSFGridPanel Yeta7200GridPanel;
	private MSFGridPanel Yeta7200XmlGridPanel;
	private Ye161010Def Yeta7200def = new Ye161010Def("Yeta7200");
	private Ye161010Def Yeta7200defXml = new Ye161010Def("Yeta7200_RESULT");
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta7200Bar;
	private Button btnYeta7200Sec; 	// 초기화
	private Button btnYeta7200Reset; 	// 초기화
	private Button btnYeta7200New; 		// 신규
	private Button btnYeta7200Save; 	// 저장
	private Button btnYeta7200Del; 		// 삭제
	private Button btnYeta7200Sreach; 	// 조회
	private Button btnYeta7200Print; 	// 인쇄
	private Button btnYetaP720001; 	// 상세정보 팝업
	private Button btnYetaP720002; 	// 상세정보 팝업
	 ...
	 * ++++++++++++++++++++++
	 **/
	private Yeta7200ServiceAsync yeta7200Service = Yeta7200Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2022")));
	...
	public Yeta7200(String txtForm) {
	...
	private void createYeta0200Form() {
		...
		plFrmYeta7200 = new FormPanel();
		plFrmYeta7200.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산파일업로드"));
		plFrmYeta7200.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta7200.setPadding(2);
		plFrmYeta7200.setFrame(true);
		...
		plFrmYeta7200.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta7200.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		//멀티콤보박스 닫기 
		plFrmYeta7200.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar **/
		topYeta7200Bar = new ButtonBar();
		topYeta7200Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		if ("admin".equals(MSFMainApp.get().getUser().getUsrId())) {	
			btnYeta7200Sec = new Button("암호화패치");
			btnYeta7200Sec.setIcon(MSFMainApp.ICONS.new16());
			topYeta7200Bar.add(btnYeta7200Sec);
			btnYeta7200Sec.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
				 
					Ye161010DTO ye161010Dto = new Ye161010DTO(); 
						   
					yeta7200Service.yetaChangeConvert(ye161010Dto,  new AsyncCallback<String>(){
				                 public void onFailure(Throwable caught) {
				                     MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
				                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta7200Service : " + caught), null);
				                 }
		...
		btnYeta7200Reset = new Button("초기화");
		btnYeta7200Reset.setIcon(MSFMainApp.ICONS.new16());
		topYeta7200Bar.add(btnYeta7200Reset);
		btnYeta7200Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//초기화 
				formInit();
				Yeta7200GridPanel.getMsfGrid().clearData(); 
			}
		});  
		...
		btnYeta7200Sreach = new Button("조 회");
		btnYeta7200Sreach.setIcon(MSFMainApp.ICONS.search());
		topYeta7200Bar.add(btnYeta7200Sreach);
		btnYeta7200Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//조회버튼 클릭시 처리 
				reloadYeta7200();  
			}
		...
		plFrmYeta7200.add(topYeta7200Bar);

		// 파일업로드 처리
		plFrmYeta7200.setAction("myurl");
		plFrmYeta7200.setEncoding(Encoding.MULTIPART);
		plFrmYeta7200.setMethod(Method.POST);

		vp.add(plFrmYeta7200);
		plFrmYeta7200.setSize("990px", "830px");
		plFrmYeta7200.setLayout(new FlowLayout());
	...
	private void createMsfGridForm() {
		...
		Yeta7200GridPanel = new MSFGridPanel(Yeta7200def, false, false, false, false,false);
		Yeta7200GridPanel.setHeaderVisible(false);
		Yeta7200GridPanel.setSize(470, 640);
		Yeta7200GridPanel.setBorders(true);
		final Grid yeta7000DefGrid = Yeta7200GridPanel.getMsfGrid().getGrid();
		yeta7000DefGrid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (Yeta7200GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = new BaseModel();
					formInit() ;
					bmData = Yeta7200GridPanel.getCurrentlySelectedItem();
		...
		Yeta7200GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 4, new HeaderGroupConfig("소득공제자료", 1, 2));

		fsGird.add(Yeta7200GridPanel);
		...
		Yeta7200XmlGridPanel = new MSFGridPanel(Yeta7200defXml, false, false,false, false);
		Yeta7200XmlGridPanel.setSize(250, 305);
		Yeta7200XmlGridPanel.setBorders(true);
		Yeta7200XmlGridPanel.setStyleAttribute("marginTop", "5px");
		layoutContainer17.add(Yeta7200XmlGridPanel,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.8));

		btnYetaP720001 = new Button("개별간소화파일");
		btnYetaP720001.setStyleAttribute("marginTop", "5px");
		btnYetaP720001.setStyleAttribute("marginLeft", "5px");
		btnYetaP720001.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					...
					final MSFFormWindows msFwYeta720001 = new MSFFormWindows("자료추출상세내역",yetaP720001Form, "닫기", "920px", "710px", false);
					msFwYeta720001.show();
					yetaP720001Form.setMSFFormWindows(msFwYeta720001);

					//					funcSetPopUpRecord();

					yetaP720001Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP720001,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));
		//일괄간소화파일은 데이터스킵문제로 화면에 안보이게 주석처리함 연말정산2022
		/*btnYetaP720002 = new Button("일괄간소화파일");
		btnYetaP720002.setStyleAttribute("marginTop", "5px");
		btnYetaP720002.setStyleAttribute("marginLeft", "5px");
		btnYetaP720002.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					Window.alert("정산대상자를 선택해주세요.");
				}
					...
					yetaP720002Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP720002,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));*/

		layoutContainer1_4.add(layoutContainer17, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.495));

		plFrmYeta7200.add(layoutContainer1_4);
	}
	...
	private Yeta7200 getThis(){
		return this;
	}

	private void reloadYeta7200(){

		IColumnFilter filters = null;
		Yeta7200GridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta7200GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 

		//정산구분
		Yeta7200GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta7200GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta7200GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd,srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta7200GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta7200GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta7200GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta7200GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);


		Yeta7200GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta7200GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
		Yeta7200GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   

		Yeta7200GridPanel.reload();  

	}

	private void reloadYeta7200Result(){

		IColumnFilter filters = null;
		Yeta7200XmlGridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta7200XmlGridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 
		//정산구분
		Yeta7200XmlGridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta7200XmlGridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta7200XmlGridPanel.reload();  
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//submit the form
							plFrmYeta7200.mask("PDF 전자문서 업데이트 중입니다. 업로드 중..."); 

							//파일업로드 처리
							plFrmYeta7200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta7200Import.do");
							plFrmYeta7200.setEncoding(Encoding.MULTIPART);
							plFrmYeta7200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta7200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
							plFrmYeta7200.submit();  
							plFrmYeta7200.onFrameLoad();  
							picBoolFile = true;
						...
						// after file upload 
						plFrmYeta7200.addListener(Events.Submit, new Listener<FormEvent>() {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//파일업로드 처리
							plFrmYeta7200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta7200DdcImport.do");
							plFrmYeta7200.setEncoding(Encoding.MULTIPART);
							plFrmYeta7200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta7200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
