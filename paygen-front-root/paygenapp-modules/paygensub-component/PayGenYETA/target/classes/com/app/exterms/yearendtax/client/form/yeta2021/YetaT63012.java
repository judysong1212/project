...
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63012Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63012ServiceAsync;
...
=====================================================**/
public  class YetaT63012   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63012; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63012GridPanel;
//	private YetaT63012Def YetaT63012Def = new YetaT63012Def("YetaT63012");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63012ServiceAsync yetaT63012Service = YetaT63012Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63012Service.activityOnYetaT63012(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63012(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63012 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63012.setBodyBorder(false);
		cpYetaT63012.setBorders(false);
		cpYetaT63012.setHeaderVisible(false);
		cpYetaT63012.setSize("790px", "640px");
		   
		this.add(cpYetaT63012);
...
//		     });     
		...
		cpYetaT63012.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63012GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63012GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63012GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63012GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63012GridPanel.getMsfGrid().getGrid().startEditing(YetaT63012GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT63012GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//				
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT63012GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//				
		cpYetaT63012.add(topYetaBtnBar);
...
//		YetaT63012GridPanel = new MSFGridPanel(YetaT63012Def, false, false, false, false, false);
//		YetaT63012GridPanel.setSize("790px", "420px");
//		YetaT63012GridPanel.setBorders(true);
//		YetaT63012GridPanel.setHeaderVisible(false);
...
//		
//		YetaT63012GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63012GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63012.add(YetaT63012GridPanel);
...
//		layoutContainer7.setStyleAttribute("marginBottom", "5px")
		...
		cpYetaT63012.add(fldstNewFieldset_1);
		cpYetaT63012.add(fldstNewFieldset_4);
	...
	public void reload() {
			...
			YetaT63012GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63012GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63012GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63012GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63012GridPanel.reload();
