...
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33006Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33006ServiceAsync;
...
=====================================================**/
public  class YetaT33006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33006GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT33006");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33006ServiceAsync yetaT33006Service = YetaT33006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33006Service.activityOnYetaT33006(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT33006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33006 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33006.setBodyBorder(false);
		cpYetaT33006.setBorders(false);
		cpYetaT33006.setHeaderVisible(false);
		cpYetaT33006.setSize("790px", "640px");
		   
		this.add(cpYetaT33006);
...
//		     });     
		...
		cpYetaT33006.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT33006GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT33006GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT33006GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT33006GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT33006GridPanel.getMsfGrid().getGrid().startEditing(YetaT33006GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT33006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT33006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT33006.add(topYetaBtnBar);
...
//		cpYetaT33006.add(topYetaBtnBar);
		...
		YetaT33006GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT33006GridPanel.setSize("790px", "200px");
		YetaT33006GridPanel.setBorders(true);
		YetaT33006GridPanel.setHeaderVisible(false);
...
//		
//		YetaT33006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT33006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT33006.add(YetaT33006GridPanel);
		...
		fldstNewFieldset.add(YetaT33006GridPanel);
		cpYetaT33006.add(fldstNewFieldset);
		cpYetaT33006.add(layoutContainer10);
		cpYetaT33006.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33006GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33006GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33006GridPanel.reload();
