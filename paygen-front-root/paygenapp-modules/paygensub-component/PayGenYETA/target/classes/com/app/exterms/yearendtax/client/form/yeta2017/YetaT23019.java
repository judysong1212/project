...
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye162510Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23019Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaT23019ServiceAsync;
...
=====================================================**/
public  class YetaT23019   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT23019; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaT23019GridPanel;
	private Ye162510Def yetaT23019ToYe162510Def = new Ye162510Def("YETAT13019");   //월세새액공제명세 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT23019ServiceAsync yetaT23019Service = YetaT23019Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT23019Service.activityOnYetaT23019ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT23019(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT23019 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT23019.setBodyBorder(false);
		cpYetaT23019.setBorders(false);
		cpYetaT23019.setHeaderVisible(false);
		cpYetaT23019.setSize("790px", "640px");
		   
		this.add(cpYetaT23019);
...
//		     });     
		...
		cpYetaT23019.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				yetaT23019GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = yetaT23019GridPanel.getTableDef().getDefaultValues();  
		        ...
		        yetaT23019GridPanel.getMsfGrid().getGrid().stopEditing();      
		        yetaT23019GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        yetaT23019GridPanel.getMsfGrid().getGrid().startEditing(yetaT23019GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = yetaT23019GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaT23019GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT23019.add(topYetaBtnBar);
...
//		cpYetaT23019.add(topYetaBtnBar);
		...
		yetaT23019GridPanel = new MSFGridPanel(yetaT23019ToYe162510Def, false, false, false, false, false);
		yetaT23019GridPanel.setSize("790px", "200px");
		yetaT23019GridPanel.setBorders(true);
		yetaT23019GridPanel.setHeaderVisible(false);
...
//		
//		yetaT23019GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		/** --- 그리드 합계 부분  종료 --- **/
		
//   	cpYetaT23019.add(yetaT23019GridPanel);
		...
		fldstNewFieldset.add(yetaT23019GridPanel);
		cpYetaT23019.add(fldstNewFieldset);
		cpYetaT23019.add(layoutContainer1);
		cpYetaT23019.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			yetaT23019GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaT23019GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaT23019GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaT23019GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaT23019GridPanel.reload();
