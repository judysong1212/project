...
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43006Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43006ServiceAsync;
...
=====================================================**/
public  class YetaT43006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT43006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT43006GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT43006");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT43006ServiceAsync yetaT43006Service = YetaT43006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT43006Service.activityOnYetaT43006(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT43006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT43006 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT43006.setBodyBorder(false);
		cpYetaT43006.setBorders(false);
		cpYetaT43006.setHeaderVisible(false);
		cpYetaT43006.setSize("790px", "640px");
		   
		this.add(cpYetaT43006);
...
//		     });     
		...
		cpYetaT43006.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT43006GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT43006GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT43006GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT43006GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT43006GridPanel.getMsfGrid().getGrid().startEditing(YetaT43006GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT43006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT43006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT43006.add(topYetaBtnBar);
...
//		cpYetaT43006.add(topYetaBtnBar);
		...
		YetaT43006GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT43006GridPanel.setSize("790px", "200px");
		YetaT43006GridPanel.setBorders(true);
		YetaT43006GridPanel.setHeaderVisible(false);
...
//		
//		YetaT43006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT43006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT43006.add(YetaT43006GridPanel);
		...
		fldstNewFieldset.add(YetaT43006GridPanel);
		cpYetaT43006.add(fldstNewFieldset);
		cpYetaT43006.add(layoutContainer10);
		cpYetaT43006.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT43006GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT43006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT43006GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT43006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT43006GridPanel.reload();
