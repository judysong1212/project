...
import com.app.exterms.yearendtax.client.form.defs.yeta2018.InfcPsnl0100Def;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP310001Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP310001ServiceAsync;
...
=====================================================**/
public  class YetaP310001   extends MSFFormPanel {  
	...
	private  ContentPanel cpYeta31001; 

	private InfcPsnl0100Def YetaP310001def = new InfcPsnl0100Def("YetaP310001");
	private MSFGridPanel YetaP310001GridPanel;
	...
	private YetaP310001ServiceAsync yetaP310001Service = YetaP310001Service.Util.getInstance();
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP310001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta31001 = new ContentPanel();

		createSearchForm(); 	// 검색필드를 적용
		createStandardForm(); 	// 기본정보필드

		cpYeta31001.setBodyBorder(false);
		cpYeta31001.setBorders(false);
		cpYeta31001.setHeaderVisible(false);
		cpYeta31001.setSize("890px", "640px");

		this.add(cpYeta31001);
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lscrPayYr.findModel("year",String.valueOf("2018")));
	...
	private void createSearchForm() {
		...
		//---------------------멀티콤보박스 닫기 -------------------------------------------------
		cpYeta31001.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
	...
	private void createStandardForm() {
		...
		cpYeta31001.add(layoutContainer);
	...
	private LayoutContainer payrModiListInfo() {
		...
		YetaP310001GridPanel = new MSFGridPanel(YetaP310001def, false, false, false, false,false);
		YetaP310001GridPanel.setHeaderVisible(false);
		YetaP310001GridPanel.setBodyBorder(false);
		YetaP310001GridPanel.setBorders(true);
		YetaP310001GridPanel.getBottomComponent().setVisible(false);

		final Grid<BaseModel> grid = YetaP310001GridPanel.getMsfGrid().getGrid();
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = YetaP310001GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					YetaP310001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					YetaP310001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				YetaP310001GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					YetaP310001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					YetaP310001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		btnSelected.addSelectionListener(new SelectionListener<ButtonEvent>() {   
			...
			public void componentSelected(ButtonEvent ce) {  
				...
				Iterator<BaseModel> itBm = YetaP310001GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); // .getSelectionModel().getSelectedItems().listIterator();
				...
				yetaP310001Service.saveYetaP310001ToYe161010(listYe161010Dto,listYe161020Dto, actionDatabase, new AsyncCallback<PagingLoadResult<ShowMessageBM>>() {
					...
					public void onFailure(Throwable caught) {
						...
						smForm.setWindowId("YetaP310001");
		...
		refresh.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				YetaP310001GridPanel.reload();	                	
			}  
		});		            
		bottomToolbar.add(refresh);

		//툴바 넣어주는것
		YetaP310001GridPanel.setBottomComponent(bottomToolbar);

		cp01.add(YetaP310001GridPanel);
	...
	public void reload() {
		...
		YetaP310001GridPanel.getTableDef().setTableColumnFilters(filters);

		YetaP310001GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP310001GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		
		...
		YetaP310001GridPanel.getTableDef().addColumnFilter("hdofcCodtnCd", strHdofcCodtnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		YetaP310001GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		YetaP310001GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDtilOccuInttnCd = YetaUtils.getStrValToBMMultiCombo(lsDtilOccuInttnCd,srhDtilOccuInttnCd.getValue(),"dtilOccuClsNm","dtilOccuInttnCd");  
		YetaP310001GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP310001GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP310001GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP310001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP310001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr, "year")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP310001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP310001GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(srhUtDpobCd.getValue())  , SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	private void formInit() {
		...
		YetaP310001GridPanel.getMsfGrid().clearData();
