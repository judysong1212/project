...
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23005ServiceAsync;
...
=====================================================**/
public  class YetaP23005   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP23005GridPanel;
//	private Ye16Ta3130Def yetaP23004ToYe162510Def  = new Ye16Ta3130Def("YETAP13005");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye162510Def yetaP23004ToYe162510Def  = new Ye162510Def("YETAP23005");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP23005ServiceAsync yetaP23005Service = YetaP23005Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
 ...
 **/ 
     ...
     */ 
		...
		private void yetaP23005FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				yetaP23005Service.activityOnYetaP23005ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("yetaP23005FormSave(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	...
	public YetaP23005(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//				systemkey.setValue("20080013");
				...
				yetaP23005GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = yetaP23005GridPanel.getTableDef().getDefaultValues();  
...
//		         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
		        ...
		        yetaP23005GridPanel.getMsfGrid().getGrid().stopEditing();      
		        yetaP23005GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        yetaP23005GridPanel.getMsfGrid().getGrid().startEditing(yetaP23005GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 월세액소득공제 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = yetaP23005GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP23005GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//		});
	...
	private void createMsfGridForm() {
		...
		yetaP23005GridPanel = new MSFGridPanel(yetaP23004ToYe162510Def, false, false, false, false, false);
		yetaP23005GridPanel.setSize("790px", "400px");
		yetaP23005GridPanel.setBorders(false);
		yetaP23005GridPanel.setHeaderVisible(false);
		...
		yetaP23005GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
		
			
		cpYeta23005.add(yetaP23005GridPanel);
	}	
	...
	public void reload() {
			...
			yetaP23005GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaP23005GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
			yetaP23005GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaP23005GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaP23005GridPanel.reload();
