...
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye160404Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23006Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23006ServiceAsync;
...
=====================================================**/
public  class YetaP23006   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel yetaP23006GridPanel;
	private Ye160404Def yetaP23006ToYe160404Def = new Ye160404Def("YETAP23006"); // 종전근무지내역 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료 ---------------
   ...
   */
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP23006ServiceAsync yetaP23006Service = YetaP23006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP23006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		this.setSize("810px", "710px");
	...
	private void createSearchForm() {
				...
				MessageBox.confirm("삭제", "선택하신 종전근무지 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = yetaP23006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
//				setListRecord(yetaP23006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
	...
	private void createStandardForm() {
		...
		btnNewButton.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				BaseModel bmData = yetaP23006GridPanel.getCurrentlySelectedItem();
...
//				setPPRecord(bmData); //팝업에 넘길값 
				...
				// 전체내역
				YetaP230010 yetaP230010Form = new YetaP230010(ActionDatabase.UPDATE,getThis()); 
	...
	private void createMsfGridForm() {
		...
		yetaP23006GridPanel = new MSFGridPanel(yetaP23006ToYe160404Def, false,false, false, false, false);
		yetaP23006GridPanel.setHeaderVisible(false);
		yetaP23006GridPanel.setSize("915px", "200px");
		yetaP23006GridPanel.setBorders(false);
		yetaP23006GridPanel.setHeadingText("");
		...
		yetaP23006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  시작 --- **/
		
		final Grid yetaP23006Grid = yetaP23006GridPanel.getMsfGrid().getGrid();
		yetaP23006Grid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				// TODO Auto-generated method stub
				//그리드 선택시
				actionDatabase = ActionDatabase.UPDATE;
				yetaP13006ToYeta3200Detail(yetaP23006GridPanel.getCurrentlySelectedItem());
				btnNewButton.show();
			}
		});
		
		gridFieldSet.add(yetaP23006GridPanel);

		cpYeta23006.add(gridFieldSet);
	}

	private YetaP23006 getThis() {
		return this;
	}
	...
	private void yetaP23006FormSave() {
...
//    				ye160404Dto.setPrcspWhdgReipSumtYn(MSFSharedUtils.allowNulls(bmMapModel.get("prcspWhdgReipSumtYn")));    	/** column 종전근무지원천징수영수증제출여부 : prcspWhdgReipSumtYn */				
    			...
    			yetaP23006Service.activityOnYetaP23006ToYe160404(listYe160404Dto, actionDatabase, new AsyncCallback<Long>(){
    				public void onFailure(Throwable caught) {
    					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
    							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP23006ToYe160404(" + actionDatabase.name() + ") : " + caught)
    							, null);
    					}
...
//		hanNm01.setValue("");				// 성명   
	...
	public void reload() {
		...
		yetaP23006GridPanel.getTableDef().setTableColumnFilters(filters);
		yetaP23006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 연말정산귀속년도 
		
		yetaP23006GridPanel.getTableDef().addColumnFilter("clutSeptCd",clutSeptCd.getValue()
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 정산구분
		
		yetaP23006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 시스템키
		
//		yetaP23006GridPanel.getTableDef().addColumnFilter("whdgDebrBusoprRgstnum", MSFSharedUtils.allowNulls(whdgDebrBusoprRgstnum.getValue())
//				, SimpleColumnFilter.OPERATOR_EQUALS); // 원천징수의무자사업자등록번호
		
		yetaP23006GridPanel.reload();
