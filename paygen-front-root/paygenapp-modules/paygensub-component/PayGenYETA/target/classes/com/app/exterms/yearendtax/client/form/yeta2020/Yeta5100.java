...
import com.app.exterms.yearendtax.client.service.yeta2020.Yeta5100Service;
import com.app.exterms.yearendtax.client.service.yeta2020.Yeta5100ServiceAsync;
...
=====================================================**/
public class Yeta5100   extends MSFPanel {
	/**
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	 * 	전역변수 선언부  시작
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	...
	private FormPanel plFrmYeta5100;
	private String txtForm = "";
	private XTemplate detailTp;

	private Ye161010Def Yeta5100def = new Ye161010Def("Yeta5100");
	private MSFGridPanel Yeta5100GridPanel;
	...
	private Yeta5100ServiceAsync yeta5100Service = Yeta5100Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhClutYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2020")));
...
//		authMapDef.put("creDeptCd",Boolean.FALSE); 
	 ...
	 * 데이타를 삭제한다.
	 */
	private void Yeta5100_menuDelete() {
		...
		MessageBox.confirm("연말정산 대상자 삭제", "선택된 데이타를 삭제 하시겠습니까?<br>(연말정산 대상자를 삭제합니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = Yeta5100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
					...
					yeta5100Service.Yeta5100ToYe160401_Delete(listYe161010Dto, new AsyncCallback<Long>() {
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
									MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta5100ToYe160401_Delete(삭제) : " + caught), null);
						}
...
//
//		//Iterator<BaseModel> itBm = Yeta5100GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
//
//		Iterator<BaseModel> itBm = Yeta5100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator();
//
//		List<Record> lsRec = new ArrayList<Record>();
...
//			Yeta5100Service.UpDpobExec_Yeta_Insert_2017(listYe161010Dto, new AsyncCallback<Long>() {
//				public void onFailure(Throwable caught) {
//					tracker.setStatus(true);
//					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
...
//	}
	...
	private void PayrExec_Yeta_Insert() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			t.scheduleRepeating(500);
			tracker.setStatus(true);
			yeta5100Service.PayrExec_Yeta_2020_Insert(listYe161010Dto, new AsyncCallback<Long>() {
				public void onFailure(Throwable caught) {
					tracker.setStatus(true);
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("PayrExec_Yeta_Insert(급여자료이관) : " + caught), null);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 **/
	private void Yeta5100Print(String fileName, String repType) {
...
//			serarchParam += "" + YetaUtils.getPrintStrToMultiData(lsDeptCd, MSFSharedUtils.allowNulls(srhDeptCd.getValue()),"deptNmRtchnt","deptCd") +"⊥";		
			...
			List<BaseModel> list = Yeta5100GridPanel.getGrid().getSelectionModel().getSelectedItems();
	...
	public Yeta5100(String txtForm) {
	...
	private void createYeta0100Form() {
		...
		plFrmYeta5100 = new FormPanel();
		plFrmYeta5100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산대상자관리"));
		plFrmYeta5100.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta5100.setPadding(2);
		plFrmYeta5100.setFrame(true);
		...
		plFrmYeta5100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta5100.getHeader().addTool(btnlogView);
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		// 멀티콤보박스 닫기
		plFrmYeta5100.addListener(Events.OnClick, new Listener<ComponentEvent>() {
			@Override
		...
		btnYeta0100Init.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Yeta5100GridPanel.getMsfGrid().clearData();
			}
		});
		...
		btnYeta0100Cre.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP510001 yetaP510001Form = new YetaP510001(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta510001 = new MSFFormWindows("연말정산대상자검색",yetaP510001Form, "닫기", "920px", "710px", false);
				msFwYeta510001.show();
				yetaP510001Form.setMSFFormWindows(msFwYeta510001);
		...
		plFrmYeta5100.add(topYeta0100Bar);
		...
		btnYeta0100Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				Yeta5100Print("YETAT5100", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
			}
		});
		plFrmYeta5100.add(topYeta0100Bar);	
		...
		// 파일업로드 처리
		plFrmYeta5100.setAction("myurl");
		plFrmYeta5100.setEncoding(Encoding.MULTIPART);
		plFrmYeta5100.setMethod(Method.POST);
		vp.add(plFrmYeta5100);

		plFrmYeta5100.setSize("990px", "680px");
		plFrmYeta5100.setLayout(new FlowLayout());
	}
...
//		srhDtilOccuInttnCd.setEnabled(gwtExtAuth.getEnableDtilOccuInttnCd());
		...
		btnUpDpobTrans.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP510002 yetaP510002Form = new YetaP510002(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta510002 = new MSFFormWindows("원천신고부서",yetaP510002Form, "닫기", "920px", "710px", false);
				msFwYeta510002.show();
				yetaP510002Form.setMSFFormWindows(msFwYeta510002);
		...
		plFrmYeta5100.add(fieldSet1);
		plFrmYeta5100.add(fieldSet2);	
	...
	private void createMsfGridForm() {
		...
		Yeta5100GridPanel = new MSFGridPanel(Yeta5100def, false, false, false,false, false);
		// Yeta5100GridPanel.setSize(960, 400);
		Yeta5100GridPanel.setHeaderVisible(false);
		Yeta5100GridPanel.setBorders(true);
		// Yeta5100GridPanel.getBottomComponent().setVisible(false);
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = Yeta5100GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					Yeta5100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					Yeta5100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				Yeta5100GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					Yeta5100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					Yeta5100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		//툴바 넣어주는것
		Yeta5100GridPanel.setBottomComponent(bottomToolbar);

		final Grid<BaseModel> grid = Yeta5100GridPanel.getMsfGrid().getGrid();
		...
		cp01.add(Yeta5100GridPanel); 

		fieldSetGrd.add(cp01);
		plFrmYeta5100.add(fieldSetGrd);
	}
...
//	}	
	...
	public void reload() {
		...
		Yeta5100GridPanel.getTableDef().setTableColumnFilters(filters);

		Yeta5100GridPanel.getTableDef().addColumnFilter("yrtxBlggYr",MSFSharedUtils.getSelectedComboValue(srhClutYr, "year"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta5100GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd"), SimpleColumnFilter.OPERATOR_EQUALS);	

		Yeta5100GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta5100GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta5100GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta5100GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta5100GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta5100GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta5100GridPanel.getTableDef().addColumnFilter("fnm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta5100GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta5100GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(utDpobCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta5100GridPanel.getTableDef().addColumnFilter("utDpobChk", MSFSharedUtils.convertBooleanToString(utDpobChk.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		

		Yeta5100GridPanel.reload();
	...
	private void xlsExportData() {
		...
		if ("".equals(srhClutYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				Yeta5100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta5100Export.do", "extgwtFrame", param);
			}
		} else {
			Yeta5100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta5100Export.do", "extgwtFrame", param);
		}
