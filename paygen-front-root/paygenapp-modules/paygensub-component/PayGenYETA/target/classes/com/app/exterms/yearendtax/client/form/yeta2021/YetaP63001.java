...
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye165030BM;
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye165030DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye165030Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63001Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63001ServiceAsync;
...
=====================================================**/
public  class YetaP63001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta63001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP63001GridPanel;
//	private Ye16Ta3170Def YetaP63001ToYe5030Def  = new Ye16Ta3170Def("YetaP63001");	//이월기부금 그리드 테이블 컬럼 define 
	private Ye165030Def YetaP63001ToYe5030Def  = new Ye165030Def("YetaP63001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
// 	private TextField<String> srhClutSeptCd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP63001ServiceAsync yetaP63001Service = YetaP63001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP63001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta63001 = new ContentPanel();

		createSearchForm(); // 기본정보 필드를 적용
		// createMsfGridForm(); //그리드필드
		createStandardForm(); // 기본정보필드

		cpYeta63001.setBodyBorder(false);
		cpYeta63001.setBorders(false);
		cpYeta63001.setHeaderVisible(false);
		cpYeta63001.setSize("790px", "640px");

		this.add(cpYeta63001);
...
//		lsdacRvyyStore = SysComComboUtils.getSysComComboYearData(msfCoCalendarDto);		//급여년도
		...
		cpYeta63001.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent be) {
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 YetaP63001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = YetaP63001GridPanel.getTableDef().getDefaultValues();  
		         ...
		         YetaP63001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         YetaP63001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         YetaP63001GridPanel.getMsfGrid().getGrid().startEditing(YetaP63001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 이월기부금 내용을 삭제하시겠습니까?" , new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){	
							...
							Iterator<BaseModel> itBm = YetaP63001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP63001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//				srhClutSeptCd.setValue(lsclutSeptCd.getAt(0));
		...
		cpYeta63001.add(fieldSet);
		cpYeta63001.add(createMsfGridForm(), new FormData("100%"));
	}  
...
//	private void createMsfGridForm() {
		...
		YetaP63001GridPanel = new MSFGridPanel(YetaP63001ToYe5030Def, false, false, false, false, false);
		YetaP63001GridPanel.setBorders(false);
		YetaP63001GridPanel.setBodyBorder(false);
		YetaP63001GridPanel.setHeaderVisible(false); 
		YetaP63001GridPanel.getBottomComponent().setVisible(false);
		
		GWTUtils.findColumnConfig(YetaP63001GridPanel, Ye165030BM.ATTR_THCYRDDCAMT).setStyle("background-color:#F5F6CE;");
		
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		GWTUtils.findColumnConfig(YetaP63001GridPanel, Yeta3170BM.ATTR_CTRBAMNT).setStyle("Height:20px;");
		...
		YetaP63001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		
		cpYeta63001.add(YetaP63001GridPanel);
		cp01.add(YetaP63001GridPanel);
	...
	private void YetaP63001FormSave() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			if(dataChk) {
				...
				yetaP63001Service.activityOnYetaP63001ToYe165030(listYe165030Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP63001ToYe165030(" + actionDatabase.name() + ") : " + caught), null);
					}
	...
	public void reload() {
		...
		YetaP63001GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP63001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		
		YetaP63001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		
		YetaP63001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		
		YetaP63001GridPanel.reload();
