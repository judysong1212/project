...
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83006Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83006ServiceAsync;
...
=====================================================**/
public  class YetaT83006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT83006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT83006GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT83006");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT83006ServiceAsync yetaT83006Service = YetaT83006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT83006Service.activityOnYetaT83006(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT83006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT83006 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT83006.setBodyBorder(false);
		cpYetaT83006.setBorders(false);
		cpYetaT83006.setHeaderVisible(false);
		cpYetaT83006.setSize("790px", "640px");
		   
		this.add(cpYetaT83006);
...
//		     });     
		...
		cpYetaT83006.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT83006GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT83006GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT83006GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT83006GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT83006GridPanel.getMsfGrid().getGrid().startEditing(YetaT83006GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT83006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT83006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT83006.add(topYetaBtnBar);
...
//		cpYetaT83006.add(topYetaBtnBar);
		...
		YetaT83006GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT83006GridPanel.setSize("790px", "200px");
		YetaT83006GridPanel.setBorders(true);
		YetaT83006GridPanel.setHeaderVisible(false);
...
//		
//		YetaT83006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT83006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT83006.add(YetaT83006GridPanel);
		...
		fldstNewFieldset.add(YetaT83006GridPanel);
		cpYetaT83006.add(fldstNewFieldset);
		cpYetaT83006.add(layoutContainer10);
		cpYetaT83006.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT83006GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT83006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT83006GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT83006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT83006GridPanel.reload();
