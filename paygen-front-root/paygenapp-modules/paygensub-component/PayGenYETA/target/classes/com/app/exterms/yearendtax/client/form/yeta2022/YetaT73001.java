...
import com.app.exterms.yearendtax.client.form.defs.yeta2022.Ye160510Def;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73001Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73001ServiceAsync;
...
=====================================================**/
public  class YetaT73001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73001GridPanel;
	private Ye160510Def ye160510Def = new Ye160510Def("YetaT73001");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT73001ServiceAsync yetaT73001Service = YetaT73001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT73001Service.activityOnYetaT73001(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT73001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73001 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73001.setBodyBorder(false);
		cpYetaT73001.setBorders(false);
		cpYetaT73001.setHeaderVisible(false);
		cpYetaT73001.setSize("790px", "640px");
		   
		this.add(cpYetaT73001);
...
//		     });     
		...
		cpYetaT73001.setLayout(new FlowLayout());
		...
		btnYetaAdd.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT73001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT73001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
				List<Record> lsRec = new ArrayList<Record>();

				
				if((ActionDatabase.INSERT).equals(actionDatabase)) {
					YetaT73001GridPanel.getMsfGrid().clearData();
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT73001.add(topYetaBtnBar);
...
//		cpYetaT73001.add(topYetaBtnBar);
		...
		YetaT73001GridPanel = new MSFGridPanel(ye160510Def, false, false, false, false, false);
		YetaT73001GridPanel.setSize("790px", "200px");
		YetaT73001GridPanel.setBorders(true);
		YetaT73001GridPanel.setHeaderVisible(false);
...
//		
//		YetaT73001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT73001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT73001.add(YetaT73001GridPanel);
		...
		fldstNewFieldset.add(YetaT73001GridPanel);
		
		cpYetaT73001.add(fldstNewFieldset);
		cpYetaT73001.add(layoutContainer10);
		cpYetaT73001.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT73001GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73001GridPanel.reload();
