...
import com.app.exterms.yearendtax.client.form.defs.yeta2023.Ye160510Def;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83001Service;
import com.app.exterms.yearendtax.client.service.yeta2023.YetaT83001ServiceAsync;
...
=====================================================**/
public  class YetaT83001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT83001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT83001GridPanel;
	private Ye160510Def ye160510Def = new Ye160510Def("YetaT83001");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT83001ServiceAsync yetaT83001Service = YetaT83001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT83001Service.activityOnYetaT83001(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT83001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT83001 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT83001.setBodyBorder(false);
		cpYetaT83001.setBorders(false);
		cpYetaT83001.setHeaderVisible(false);
		cpYetaT83001.setSize("790px", "640px");
		   
		this.add(cpYetaT83001);
...
//		     });     
		...
		cpYetaT83001.setLayout(new FlowLayout());
		...
		btnYetaAdd.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT83001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT83001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
				List<Record> lsRec = new ArrayList<Record>();

				
				if((ActionDatabase.INSERT).equals(actionDatabase)) {
					YetaT83001GridPanel.getMsfGrid().clearData();
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT83001.add(topYetaBtnBar);
...
//		cpYetaT83001.add(topYetaBtnBar);
		...
		YetaT83001GridPanel = new MSFGridPanel(ye160510Def, false, false, false, false, false);
		YetaT83001GridPanel.setSize("790px", "200px");
		YetaT83001GridPanel.setBorders(true);
		YetaT83001GridPanel.setHeaderVisible(false);
...
//		
//		YetaT83001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT83001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT83001.add(YetaT83001GridPanel);
		...
		fldstNewFieldset.add(YetaT83001GridPanel);
		
		cpYetaT83001.add(fldstNewFieldset);
		cpYetaT83001.add(layoutContainer10);
		cpYetaT83001.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT83001GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT83001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT83001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT83001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT83001GridPanel.reload();
