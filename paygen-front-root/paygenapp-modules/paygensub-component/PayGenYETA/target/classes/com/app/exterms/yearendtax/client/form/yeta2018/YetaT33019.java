...
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33019ServiceAsync;
...
=====================================================**/
public  class YetaT33019   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33019; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33019GridPanel;
	private Ye162510Def YetaT33019ToYe162510Def = new Ye162510Def("YETAT13019");   //월세새액공제명세 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33019ServiceAsync yetaT33019Service = YetaT33019Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
 ...
 **/ 
     ...
     */ 
		...
		private void YetaT33019FormSave() {  
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33019Service.activityOnYetaT33019ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
					
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("YetaT33019FormSave(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	...
	public YetaT33019(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33019 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33019.setBodyBorder(false);
		cpYetaT33019.setBorders(false);
		cpYetaT33019.setHeaderVisible(false);
		cpYetaT33019.setSize("790px", "640px");
		   
		this.add(cpYetaT33019);
...
//		     });     
		...
		cpYetaT33019.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaT33019GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = YetaT33019GridPanel.getTableDef().getDefaultValues();  
		        ...
		        YetaT33019GridPanel.getMsfGrid().getGrid().stopEditing();      
		        YetaT33019GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        YetaT33019GridPanel.getMsfGrid().getGrid().startEditing(YetaT33019GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT33019GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT33019GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT33019.add(topYetaBtnBar);
...
//		cpYetaT33019.add(topYetaBtnBar);
		...
		YetaT33019GridPanel = new MSFGridPanel(YetaT33019ToYe162510Def, false, false, false, false, false);
		YetaT33019GridPanel.setSize("790px", "200px");
		YetaT33019GridPanel.setBorders(true);
		YetaT33019GridPanel.setHeaderVisible(false);
...
//		
//		YetaT33019GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		/** --- 그리드 합계 부분  종료 --- **/
		
//   	cpYetaT33019.add(YetaT33019GridPanel);
		...
		fldstNewFieldset.add(YetaT33019GridPanel);
		cpYetaT33019.add(fldstNewFieldset);
		cpYetaT33019.add(layoutContainer1);
		cpYetaT33019.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33019GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33019GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33019GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33019GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33019GridPanel.reload();
