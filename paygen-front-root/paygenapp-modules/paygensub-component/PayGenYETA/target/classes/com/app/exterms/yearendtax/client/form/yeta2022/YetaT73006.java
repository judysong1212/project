...
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73006Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73006ServiceAsync;
...
=====================================================**/
public  class YetaT73006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73006GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT73006");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT73006ServiceAsync yetaT73006Service = YetaT73006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT73006Service.activityOnYetaT73006(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT73006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73006 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73006.setBodyBorder(false);
		cpYetaT73006.setBorders(false);
		cpYetaT73006.setHeaderVisible(false);
		cpYetaT73006.setSize("790px", "640px");
		   
		this.add(cpYetaT73006);
...
//		     });     
		...
		cpYetaT73006.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT73006GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT73006GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT73006GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT73006GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT73006GridPanel.getMsfGrid().getGrid().startEditing(YetaT73006GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT73006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT73006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT73006.add(topYetaBtnBar);
...
//		cpYetaT73006.add(topYetaBtnBar);
		...
		YetaT73006GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT73006GridPanel.setSize("790px", "200px");
		YetaT73006GridPanel.setBorders(true);
		YetaT73006GridPanel.setHeaderVisible(false);
...
//		
//		YetaT73006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT73006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT73006.add(YetaT73006GridPanel);
		...
		fldstNewFieldset.add(YetaT73006GridPanel);
		cpYetaT73006.add(fldstNewFieldset);
		cpYetaT73006.add(layoutContainer10);
		cpYetaT73006.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT73006GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73006GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73006GridPanel.reload();
