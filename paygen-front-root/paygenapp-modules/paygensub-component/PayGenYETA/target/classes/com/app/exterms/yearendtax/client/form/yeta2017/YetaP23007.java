...
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye162010BM;
...
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23007ServiceAsync;
...
=====================================================**/
public  class YetaP23007   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP23007GridPanel;
//	private Ye16Ta3100Def yetaP23007ToYe162010Def  = new Ye16Ta3100Def("YETAP13007");	//종전근무지내역 그리드 테이블 컬럼 define 
	private Ye162010Def ye162010Def  = new Ye162010Def("YETAP23007");	//종전근무지내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP23007ServiceAsync yetaP23007Service = YetaP23007Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------		
     ...
     */  
		...
		private void yetaP23007FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				if(dataChk) {
					...
					yetaP23007Service.activityOnYetaP23007ToYe162010(listYe162010Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
		 ...
		 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 **/		  
	  public YetaP23007(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			     });     
			...
			btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 ...
					 yetaP23007GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = yetaP23007GridPanel.getTableDef().getDefaultValues();  
			         ...
			         yetaP23007GridPanel.getMsfGrid().getGrid().stopEditing();      
			         yetaP23007GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         yetaP23007GridPanel.getMsfGrid().getGrid().startEditing(yetaP23007GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
					...
					MessageBox.confirm("삭제", "선택하신 연금저축 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
						...
						public void handleEvent(MessageBoxEvent be) {
							if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
								...
								Iterator<BaseModel> itBm = yetaP23007GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					...
					setListRecord(yetaP23007GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
					doAction(actionDatabase);
...
//	            	reload();
		...
		private void createMsfGridForm() {
			...
			yetaP23007GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
//			yetaP23007GridPanel.setSize("790px", "300px");
			yetaP23007GridPanel.setBorders(false);
			yetaP23007GridPanel.setHeaderVisible(false);
			...
			yetaP23007GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			/** --- 그리드 합계 부분  시작 --- **/
			
			cpYeta23007.add(yetaP23007GridPanel);
...
//			cpYeta23007.add(layoutContainer);
	...
	public void reload() {
		...
		yetaP23007GridPanel.getTableDef().setTableColumnFilters(filters);
		
		yetaP23007GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		yetaP23007GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
															, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		yetaP23007GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		yetaP23007GridPanel.reload();
