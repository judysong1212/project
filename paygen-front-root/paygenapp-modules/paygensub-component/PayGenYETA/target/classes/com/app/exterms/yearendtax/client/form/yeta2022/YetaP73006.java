...
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye160404BM;
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye160404DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2022.Ye160404Def;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73006Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73006ServiceAsync;
...
=====================================================**/
public  class YetaP73006   extends MSFFormPanel {  
 ...
 **/
	...
	private ContentPanel cpYeta73006;
	...
	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP73006GridPanel;
	private Ye160404Def YetaP73006ToYe160404Def = new Ye160404Def("YetaP73006"); // 종전근무지내역 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료 ---------------
   ...
   */
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP73006ServiceAsync yetaP73006Service = YetaP73006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP73006(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta73006 = new ContentPanel();

		createSearchForm(); 	// 기본정보 필드를 적용
		createMsfGridForm(); 	// 그리드필드
		createStandardForm(); 	// 기본정보필드

		cpYeta73006.setBodyBorder(false);
		cpYeta73006.setBorders(false);
		cpYeta73006.setHeaderVisible(false);
//		cpYeta73006.setSize("790px", "640px");
		cpYeta73006.setSize("950px", "720px");

		this.add(cpYeta73006);
...
//		this.setSize("810px", "710px");
	...
	private void createSearchForm() {
		...
		cpYeta73006.setLayout(new FlowLayout());
				...
				MessageBox.confirm("삭제", "선택하신 종전근무지 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP73006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaP73006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
	...
	private void createStandardForm() {
		...
		btnNewButton.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				BaseModel bmData = YetaP73006GridPanel.getCurrentlySelectedItem();
...
//				setPPRecord(bmData); //팝업에 넘길값 
				...
				// 전체내역
				YetaP730010 yetaP730010Form = new YetaP730010(ActionDatabase.UPDATE,getThis()); 
						    
				MSFFormWindows msFwYeta730010 = new MSFFormWindows("비과세 및 감면소득", yetaP730010Form, "닫기", "820px", "750px", true);
				msFwYeta730010.show();
				yetaP730010Form.setMSFFormWindows(msFwYeta730010);

				yetaP730010Form.bind(ppRecord);
			}
		});
		btnNewButton.hide();
	...
	private void createMsfGridForm() {
		...
		YetaP73006GridPanel = new MSFGridPanel(YetaP73006ToYe160404Def, false,false, false, false, false);
		YetaP73006GridPanel.setHeaderVisible(false);
		YetaP73006GridPanel.setSize("915px", "200px");
		YetaP73006GridPanel.setBorders(false);
		YetaP73006GridPanel.setHeadingText("");
		...
		YetaP73006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  시작 --- **/
		
		final Grid YetaP73006Grid = YetaP73006GridPanel.getMsfGrid().getGrid();
		YetaP73006Grid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				// TODO Auto-generated method stub
				//그리드 선택시
				actionDatabase = ActionDatabase.UPDATE;
				yetaP13006ToYeta3200Detail(YetaP73006GridPanel.getCurrentlySelectedItem());
				btnNewButton.show();
			}
		});
		
		gridFieldSet.add(YetaP73006GridPanel);

		cpYeta73006.add(gridFieldSet);
	}

	private YetaP73006 getThis() {
		return this;
	}
	...
	private void YetaP73006FormSave() {
...
//    				ye160404Dto.setPrcspWhdgReipSumtYn(MSFSharedUtils.allowNulls(bmMapModel.get("prcspWhdgReipSumtYn")));    	/** column 종전근무지원천징수영수증제출여부 : prcspWhdgReipSumtYn */				
    			...
    			yetaP73006Service.activityOnYetaP73006ToYe160404(listYe160404Dto, actionDatabase, new AsyncCallback<Long>(){
    				public void onFailure(Throwable caught) {
    					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
    							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP73006ToYe160404(" + actionDatabase.name() + ") : " + caught)
    							, null);
    					}
...
//		hanNm01.setValue("");				// 성명   
	...
	public void reload() {
		...
		YetaP73006GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP73006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 연말정산귀속년도 
		
		YetaP73006GridPanel.getTableDef().addColumnFilter("clutSeptCd",clutSeptCd.getValue()
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 정산구분
		
		YetaP73006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 시스템키
		
//		YetaP73006GridPanel.getTableDef().addColumnFilter("whdgDebrBusoprRgstnum", MSFSharedUtils.allowNulls(whdgDebrBusoprRgstnum.getValue())
//				, SimpleColumnFilter.OPERATOR_EQUALS); // 원천징수의무자사업자등록번호
		
		YetaP73006GridPanel.reload();
