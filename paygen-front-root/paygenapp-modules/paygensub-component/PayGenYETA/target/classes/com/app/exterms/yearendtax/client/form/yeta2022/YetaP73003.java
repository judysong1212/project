...
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye165010BM;
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye165010DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2022.Ye165010Def;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73003Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73003ServiceAsync;
...
=====================================================**/
public  class YetaP73003   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta73003; 
	...
	*#######################################################*/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP73003GridPanel;
//	private Yeta0300Def yeta0300Def_08  = new Yeta0300Def("YETA03008");	//가족교육비 그리드 테이블 컬럼 define 
//	private Ye16Ta3180Def YetaP73003ToYe165010  = new Ye16Ta3180Def("YetaP73003");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye165010Def YetaP73003ToYe165010  = new Ye165010Def("YetaP73003");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP73003ServiceAsync yetaP73003Service = YetaP73003Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 	
	...
	private void YetaP73003FormSave() {
...
//				}
			...
			if (dataChk) {
				...
				yetaP73003Service.activityOnYetaP73003ToYe165010(listYe165010Dto, actionDatabase, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP73003ToYe165010(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	...
	public YetaP73003(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		this.actionDatabase = actionDatabase;
		...
		cpYeta73003 = new ContentPanel();

		createSearchForm(); // 기본정보 필드를 적용
		createMsfGridForm(); // 그리드필드
		createStandardForm(); // 기본정보필드

		cpYeta73003.setBodyBorder(false);
		cpYeta73003.setBorders(false);
		cpYeta73003.setHeaderVisible(false);
		cpYeta73003.setSize("790px", "490px");

		this.add(cpYeta73003);
	...
	private void createSearchForm() {
		...
		cpYeta73003.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP73003GridPanel.getMsfGrid().clearData();
					 
				BaseModel bmData = YetaP73003GridPanel.getTableDef().getDefaultValues();  
				...
				YetaP73003GridPanel.getMsfGrid().getGrid().stopEditing();
				YetaP73003GridPanel.getMsfGrid().getGrid().getStore().insert(bmData, 0);
				YetaP73003GridPanel.getMsfGrid().getGrid().startEditing(YetaP73003GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);
...
//				actionDatabase = ActionDatabase.INSERT;
				...
				MessageBox.confirm("삭제", "선택하신 기부금 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
							...
							Iterator<BaseModel> itBm = YetaP73003GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP73003GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	      layoutContainer_2.setBorders(false);
	...
	private void createMsfGridForm() {
		...
		YetaP73003GridPanel = new MSFGridPanel(YetaP73003ToYe165010, false, false, false, false, false);
		YetaP73003GridPanel.setHeaderVisible(false);  
		YetaP73003GridPanel.setBorders(false);
		YetaP73003GridPanel.setHeaderVisible(false);
		YetaP73003GridPanel.getBottomComponent().setVisible(true);
		...
		YetaP73003GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
			

		cp01.add(YetaP73003GridPanel);

		cpYeta73003.add(cp01);
	...
	public void reload() {
		...
		YetaP73003GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP73003GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP73003GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP73003GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP73003GridPanel.reload();
