...
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye162510BM;
import com.app.exterms.yearendtax.client.dto.yeta2022.Ye162510DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2022.Ye162510Def;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73005Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaP73005ServiceAsync;
...
=====================================================**/
public  class YetaP73005   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta73005; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP73005GridPanel;
//	private Ye16Ta3130Def yetaP73004ToYe162510Def  = new Ye16Ta3130Def("YETAP13005");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye162510Def yetaP73004ToYe162510Def  = new Ye162510Def("YetaP73005");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP73005ServiceAsync yetaP73005Service = YetaP73005Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
 ...
 **/ 
     ...
     */ 
		...
		private void YetaP73005FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				yetaP73005Service.activityOnYetaP73005ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("YetaP73005FormSave(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	...
	public YetaP73005(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYeta73005 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYeta73005.setBodyBorder(false);
		cpYeta73005.setBorders(false);
		cpYeta73005.setHeaderVisible(false);
		cpYeta73005.setSize("790px", "480px");
		   
		this.add(cpYeta73005);
...
//		     });     
		...
		cpYeta73005.setLayout(new FlowLayout());
...
//				systemkey.setValue("20080013");
				...
				YetaP73005GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = YetaP73005GridPanel.getTableDef().getDefaultValues();  
...
//		         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
		        ...
		        YetaP73005GridPanel.getMsfGrid().getGrid().stopEditing();      
		        YetaP73005GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        YetaP73005GridPanel.getMsfGrid().getGrid().startEditing(YetaP73005GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 월세액소득공제 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP73005GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP73005GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//		});
	...
	private void createMsfGridForm() {
		...
		YetaP73005GridPanel = new MSFGridPanel(yetaP73004ToYe162510Def, false, false, false, false, false);
		YetaP73005GridPanel.setSize("790px", "400px");
		YetaP73005GridPanel.setBorders(false);
		YetaP73005GridPanel.setHeaderVisible(false);
		...
		YetaP73005GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
		
			
		cpYeta73005.add(YetaP73005GridPanel);
	}	
	...
	public void reload() {
			...
			YetaP73005GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaP73005GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
			YetaP73005GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaP73005GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaP73005GridPanel.reload();
