...
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4100Service;
import com.app.exterms.yearendtax.client.service.yeta2019.Yeta4100ServiceAsync;
...
=====================================================**/
public class Yeta4100   extends MSFPanel {
	/**
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	 * 	전역변수 선언부  시작
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	...
	private FormPanel plFrmYeta4100;
	private String txtForm = "";
	private XTemplate detailTp;

	private Ye161010Def Yeta4100def = new Ye161010Def("Yeta4100");
	private MSFGridPanel Yeta4100GridPanel;
	...
	private Yeta4100ServiceAsync yeta4100Service = Yeta4100Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhClutYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2019")));
...
//		authMapDef.put("creDeptCd",Boolean.FALSE); 
	 ...
	 * 데이타를 삭제한다.
	 */
	private void Yeta4100_menuDelete() {
		...
		MessageBox.confirm("연말정산 대상자 삭제", "선택된 데이타를 삭제 하시겠습니까?<br>(연말정산 대상자를 삭제합니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = Yeta4100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
					...
					yeta4100Service.Yeta4100ToYe160401_Delete(listYe161010Dto, new AsyncCallback<Long>() {
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
									MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta4100ToYe160401_Delete(삭제) : " + caught), null);
						}
...
//
//		//Iterator<BaseModel> itBm = Yeta4100GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
//
//		Iterator<BaseModel> itBm = Yeta4100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator();
//
//		List<Record> lsRec = new ArrayList<Record>();
...
//			Yeta4100Service.UpDpobExec_Yeta_Insert_2017(listYe161010Dto, new AsyncCallback<Long>() {
//				public void onFailure(Throwable caught) {
//					tracker.setStatus(true);
//					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
...
//	}
	...
	private void PayrExec_Yeta_Insert() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			t.scheduleRepeating(500);
			tracker.setStatus(true);
			yeta4100Service.PayrExec_Yeta_2019_Insert(listYe161010Dto, new AsyncCallback<Long>() {
				public void onFailure(Throwable caught) {
					tracker.setStatus(true);
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("PayrExec_Yeta_Insert(급여자료이관) : " + caught), null);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 **/
	private void Yeta4100Print(String fileName, String repType) {
...
//			serarchParam += "" + YetaUtils.getPrintStrToMultiData(lsDeptCd, MSFSharedUtils.allowNulls(srhDeptCd.getValue()),"deptNmRtchnt","deptCd") +"⊥";		
			...
			List<BaseModel> list = Yeta4100GridPanel.getGrid().getSelectionModel().getSelectedItems();
	...
	public Yeta4100(String txtForm) {
	...
	private void createYeta0100Form() {
		...
		plFrmYeta4100 = new FormPanel();
		plFrmYeta4100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산대상자관리"));
		plFrmYeta4100.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta4100.setPadding(2);
		plFrmYeta4100.setFrame(true);
		...
		plFrmYeta4100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta4100.getHeader().addTool(btnlogView);
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		// 멀티콤보박스 닫기
		plFrmYeta4100.addListener(Events.OnClick, new Listener<ComponentEvent>() {
			@Override
		...
		btnYeta0100Init.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Yeta4100GridPanel.getMsfGrid().clearData();
			}
		});
		...
		btnYeta0100Cre.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP410001 yetaP410001Form = new YetaP410001(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta410001 = new MSFFormWindows("연말정산대상자검색",yetaP410001Form, "닫기", "920px", "710px", false);
				msFwYeta410001.show();
				yetaP410001Form.setMSFFormWindows(msFwYeta410001);
		...
		plFrmYeta4100.add(topYeta0100Bar);
		...
		btnYeta0100Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				Yeta4100Print("YETAT4100", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
			}
		});
		plFrmYeta4100.add(topYeta0100Bar);	
		...
		// 파일업로드 처리
		plFrmYeta4100.setAction("myurl");
		plFrmYeta4100.setEncoding(Encoding.MULTIPART);
		plFrmYeta4100.setMethod(Method.POST);
		vp.add(plFrmYeta4100);

		plFrmYeta4100.setSize("990px", "680px");
		plFrmYeta4100.setLayout(new FlowLayout());
	}
...
//		srhDtilOccuInttnCd.setEnabled(gwtExtAuth.getEnableDtilOccuInttnCd());
		...
		btnUpDpobTrans.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP410002 yetaP410002Form = new YetaP410002(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta410002 = new MSFFormWindows("원천신고부서",yetaP410002Form, "닫기", "920px", "710px", false);
				msFwYeta410002.show();
				yetaP410002Form.setMSFFormWindows(msFwYeta410002);
		...
		plFrmYeta4100.add(fieldSet1);
		plFrmYeta4100.add(fieldSet2);	
	...
	private void createMsfGridForm() {
		...
		Yeta4100GridPanel = new MSFGridPanel(Yeta4100def, false, false, false,false, false);
		// Yeta4100GridPanel.setSize(960, 400);
		Yeta4100GridPanel.setHeaderVisible(false);
		Yeta4100GridPanel.setBorders(true);
		// Yeta4100GridPanel.getBottomComponent().setVisible(false);
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = Yeta4100GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					Yeta4100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					Yeta4100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				Yeta4100GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					Yeta4100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					Yeta4100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		//툴바 넣어주는것
		Yeta4100GridPanel.setBottomComponent(bottomToolbar);

		final Grid<BaseModel> grid = Yeta4100GridPanel.getMsfGrid().getGrid();
		...
		cp01.add(Yeta4100GridPanel); 

		fieldSetGrd.add(cp01);
		plFrmYeta4100.add(fieldSetGrd);
	}
...
//	}	
	...
	public void reload() {
		...
		Yeta4100GridPanel.getTableDef().setTableColumnFilters(filters);

		Yeta4100GridPanel.getTableDef().addColumnFilter("yrtxBlggYr",MSFSharedUtils.getSelectedComboValue(srhClutYr, "year"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta4100GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd"), SimpleColumnFilter.OPERATOR_EQUALS);	

		Yeta4100GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta4100GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta4100GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta4100GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta4100GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta4100GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta4100GridPanel.getTableDef().addColumnFilter("fnm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta4100GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta4100GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(utDpobCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta4100GridPanel.getTableDef().addColumnFilter("utDpobChk", MSFSharedUtils.convertBooleanToString(utDpobChk.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		

		Yeta4100GridPanel.reload();
	...
	private void xlsExportData() {
		...
		if ("".equals(srhClutYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				Yeta4100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta4100Export.do", "extgwtFrame", param);
			}
		} else {
			Yeta4100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta4100Export.do", "extgwtFrame", param);
		}
