...
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53006Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53006ServiceAsync;
...
=====================================================**/
public  class YetaT53006   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT53006; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT53006GridPanel;
	private Ye162010Def ye162010Def = new Ye162010Def("YetaT53006");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT53006ServiceAsync yetaT53006Service = YetaT53006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT53006Service.activityOnYetaT53006(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT53006(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT53006 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT53006.setBodyBorder(false);
		cpYetaT53006.setBorders(false);
		cpYetaT53006.setHeaderVisible(false);
		cpYetaT53006.setSize("790px", "640px");
		   
		this.add(cpYetaT53006);
...
//		     });     
		...
		cpYetaT53006.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT53006GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT53006GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT53006GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT53006GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT53006GridPanel.getMsfGrid().getGrid().startEditing(YetaT53006GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT53006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT53006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT53006.add(topYetaBtnBar);
...
//		cpYetaT53006.add(topYetaBtnBar);
		...
		YetaT53006GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
		YetaT53006GridPanel.setSize("790px", "200px");
		YetaT53006GridPanel.setBorders(true);
		YetaT53006GridPanel.setHeaderVisible(false);
...
//		
//		YetaT53006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT53006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT53006.add(YetaT53006GridPanel);
		...
		fldstNewFieldset.add(YetaT53006GridPanel);
		cpYetaT53006.add(fldstNewFieldset);
		cpYetaT53006.add(layoutContainer10);
		cpYetaT53006.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT53006GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT53006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT53006GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT53006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT53006GridPanel.reload();
