...
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3100Service;
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3100ServiceAsync;
...
=====================================================**/
public class Yeta3100   extends MSFPanel {
	/**
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	 * 	전역변수 선언부  시작
	 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
	...
	private FormPanel plFrmYeta3100;
	private String txtForm = "";
	private XTemplate detailTp;

	private Ye161010Def Yeta3100def = new Ye161010Def("Yeta3100");
	private MSFGridPanel Yeta3100GridPanel;
	...
	private Yeta3100ServiceAsync yeta3100Service = Yeta3100Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhClutYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2018")));
...
//		authMapDef.put("creDeptCd",Boolean.FALSE); 
	 ...
	 * 데이타를 삭제한다.
	 */
	private void Yeta3100_menuDelete() {
		...
		MessageBox.confirm("연말정산 대상자 삭제", "선택된 데이타를 삭제 하시겠습니까?<br>(연말정산 대상자를 삭제합니다.)",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = Yeta3100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
					...
					yeta3100Service.Yeta3100ToYe160401_Delete(listYe161010Dto , new AsyncCallback<Long>() {
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
									MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta3100ToYe160401_Delete(삭제) : " + caught), null);
						}
...
//
//		//Iterator<BaseModel> itBm = Yeta3100GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
//
//		Iterator<BaseModel> itBm = Yeta3100GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator();
//
//		List<Record> lsRec = new ArrayList<Record>();
...
//			Yeta3100Service.UpDpobExec_Yeta_Insert_2017(listYe161010Dto, new AsyncCallback<Long>() {
//				public void onFailure(Throwable caught) {
//					tracker.setStatus(true);
//					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
...
//	}
	...
	private void PayrExec_Yeta_Insert() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			t.scheduleRepeating(500);
			tracker.setStatus(true);
			yeta3100Service.PayrExec_Yeta_2018_Insert(listYe161010Dto, new AsyncCallback<Long>() {
				public void onFailure(Throwable caught) {
					tracker.setStatus(true);
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("PayrExec_Yeta_Insert(급여자료이관) : " + caught), null);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 **/
	private void Yeta3100Print(String fileName, String repType) {
...
//			serarchParam += "" + YetaUtils.getPrintStrToMultiData(lsDeptCd, MSFSharedUtils.allowNulls(srhDeptCd.getValue()),"deptNmRtchnt","deptCd") +"⊥";		
			...
			List<BaseModel> list = Yeta3100GridPanel.getGrid().getSelectionModel().getSelectedItems();
	...
	public Yeta3100(String txtForm) {
	...
	private void createYeta0100Form() {
		...
		plFrmYeta3100 = new FormPanel();
		plFrmYeta3100.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산대상자관리"));
		plFrmYeta3100.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta3100.setPadding(2);
		plFrmYeta3100.setFrame(true);
		...
		plFrmYeta3100.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta3100.getHeader().addTool(btnlogView);
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		// 멀티콤보박스 닫기
		plFrmYeta3100.addListener(Events.OnClick, new Listener<ComponentEvent>() {
			@Override
		...
		btnYeta0100Init.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Yeta3100GridPanel.getMsfGrid().clearData();
			}
		});
		...
		btnYeta0100Cre.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				////				}
				//				
				YetaP310001 yetaP310001Form = new YetaP310001(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta310001 = new MSFFormWindows("연말정산대상자검색",yetaP310001Form, "닫기", "920px", "710px", false);
				msFwYeta310001.show();
				yetaP310001Form.setMSFFormWindows(msFwYeta310001);
		...
		plFrmYeta3100.add(topYeta0100Bar);
		...
		btnYeta0100Print.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				Yeta3100Print("YETAT3100", MSFMainApp.getMsg("PayGen.ReportDivisionType"));
			}
		});
		plFrmYeta3100.add(topYeta0100Bar);	
		...
		// 파일업로드 처리
		plFrmYeta3100.setAction("myurl");
		plFrmYeta3100.setEncoding(Encoding.MULTIPART);
		plFrmYeta3100.setMethod(Method.POST);
		vp.add(plFrmYeta3100);

		plFrmYeta3100.setSize("990px", "680px");
		plFrmYeta3100.setLayout(new FlowLayout());
	}
...
//		srhDtilOccuInttnCd.setEnabled(gwtExtAuth.getEnableDtilOccuInttnCd());
		...
		btnUpDpobTrans.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaP310002 yetaP310002Form = new YetaP310002(ActionDatabase.READ, getThis());

				final MSFFormWindows msFwYeta310002 = new MSFFormWindows("원천신고부서",yetaP310002Form, "닫기", "920px", "710px", false);
				msFwYeta310002.show();
				yetaP310002Form.setMSFFormWindows(msFwYeta310002);
		...
		plFrmYeta3100.add(fieldSet1);
		plFrmYeta3100.add(fieldSet2);	
	...
	private void createMsfGridForm() {
		...
		Yeta3100GridPanel = new MSFGridPanel(Yeta3100def, false, false, false,false, false);
		// Yeta3100GridPanel.setSize(960, 400);
		Yeta3100GridPanel.setHeaderVisible(false);
		Yeta3100GridPanel.setBorders(true);
		// Yeta3100GridPanel.getBottomComponent().setVisible(false);
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = Yeta3100GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					Yeta3100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					Yeta3100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				Yeta3100GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					Yeta3100GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					Yeta3100GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		//툴바 넣어주는것
		Yeta3100GridPanel.setBottomComponent(bottomToolbar);

		final Grid<BaseModel> grid = Yeta3100GridPanel.getMsfGrid().getGrid();
		...
		cp01.add(Yeta3100GridPanel); 

		fieldSetGrd.add(cp01);
		plFrmYeta3100.add(fieldSetGrd);
	}
...
//	}	
	...
	public void reload() {
		...
		Yeta3100GridPanel.getTableDef().setTableColumnFilters(filters);

		Yeta3100GridPanel.getTableDef().addColumnFilter("yrtxBlggYr",MSFSharedUtils.getSelectedComboValue(srhClutYr, "year"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta3100GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd"), SimpleColumnFilter.OPERATOR_EQUALS);	

		Yeta3100GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta3100GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta3100GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta3100GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta3100GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta3100GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta3100GridPanel.getTableDef().addColumnFilter("fnm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		Yeta3100GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta3100GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(utDpobCd.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta3100GridPanel.getTableDef().addColumnFilter("utDpobChk", MSFSharedUtils.convertBooleanToString(utDpobChk.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		

		Yeta3100GridPanel.reload();
	...
	private void xlsExportData() {
		...
		if ("".equals(srhClutYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				Yeta3100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta3100Export.do", "extgwtFrame", param);
			}
		} else {
			Yeta3100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta3100Export.do", "extgwtFrame", param);
		}
