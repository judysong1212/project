...
import com.app.exterms.yearendtax.client.form.defs.yeta2022.Yeta7060Def;
import com.app.exterms.yearendtax.client.service.yeta2022.Yeta7060Service;
import com.app.exterms.yearendtax.client.service.yeta2022.Yeta7060ServiceAsync;
 ...
 * =========================================================================
 **/
public class Yeta7060 extends MSFPanel {
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta7060Bar;
	private Button btnYeta7060Dpob; 
	private Button btnYeta7060New; 
	private Button btnYeta7060Save;
	private Button btnYeta7060Del;
	private Button btnYeta7060Sreach;
	// -------------- 상단 버튼 종료 --------------
...
//	private MSFComboBox<BaseModel> hhrkUntDpobCd; 					// 상위 단위사업장 코드
	...
	private CheckBoxGroup chkgrpYeta7060;   						// 체크 그룹
	...
	// -------------- 서비스 호출 시작 -------------- 
	//private InfcYeta7060ServiceAsync infcYeta7060Service = InfcYeta7060Service.Util.getInstance();
	private Yeta7060ServiceAsync yeta7060Service = Yeta7060Service.Util.getInstance();
	private PrgmComBass0300ServiceAsync sysComBass0300Service = PrgmComBass0300Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 


	// -------------- grid 시작 -------------- 
	private Yeta7060Def Yeta7060Def = new Yeta7060Def(); // 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel;


	private Yeta7060Def mYeta7060Def = new Yeta7060Def("Yeta7060_01"); // 주 그리드 테이블 컬럼 define
	...
	// private PrgmComYeta7060DTO sysComYeta7060Dto;
	// -------------- DTO 선언 종료  -------------- 

	private VerticalPanel vp;
	private FormPanel plFrmYeta7060;
	...
	private SelectionListener<ButtonEvent> selectionListener = new SelectionListener<ButtonEvent>() {
		public void componentSelected(ButtonEvent ce) {
			if (ActionDatabase.INSERT.equals(actionDatabase)) {
				actionDatabase = null;
				if (ce.getButton() == btnYeta7060Save) {
					doAction(ActionDatabase.INSERT);
				}
			} else if (ce.getButton() == btnYeta7060Save) {
				doAction(ActionDatabase.UPDATE);
				actionDatabase = null;
			} else if (ce.getButton() == btnYeta7060Del) {
	...
	public void save(final ActionDatabase actionDatabase) {
		...
		final Yeta7060DTO dto = new Yeta7060DTO();
...
//		}
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					if("DELETE".equals(actionDatabase.name())) {
						...
						yeta7060Service.delChkYeta7060(dto, actionDatabase, new AsyncCallback<Integer>() {
							@Override
							public void onSuccess(Integer result) {
								...
								if (result == 1) {
									...
									// 저장 처리
									saveYeta7060(dto, actionDatabase, fileName);
								}
							}
							...
							public void onFailure(Throwable caught) {
								...
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
										MSFMainApp.ADMINMESSAGES.ExceptionMessageService("delChkYeta7060("+ actionDatabase.name()+ ") : "+ caught),null);
							}
						});
					}else {
						// 저장 처리
	...
	public static native void closeFunction() /*-{
			   $wnd.closeFunction =
			      $entry(@com.app.exterms.yearendtax.client.form.yeta2022.Yeta7060::onCloseGwtFunction(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;));
	  }-*/;
	...
	public ContentPanel getViewPanel() {
		...
		if (panel == null) {
			...
			// 상위 사업장
			yeta7060Service.getParentDpobCd(new AsyncCallback<List<BaseModel>>() {
				@Override
			...
			vp.setSpacing(10);
			createYeta7060Form(); 		// 화면 기본정보를 설정
			 ...
			 */
			// 폼데이터를 바인딩
			formBinding = new FormBinding(this.plFrmYeta7060, true);
	...
	// 생성자 함수에 변수을 받아 처리
	public Yeta7060(String txtForm) {
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createYeta7060Form() {

		plFrmYeta7060 = new FormPanel();
		plFrmYeta7060.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 단위사업장등록관리"));
		plFrmYeta7060.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta7060.setBodyStyleName("pad-text");
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//로그 뷰화면 호출 메서드 
				funcLogMessage("단위사업장등록관리","Yeta7060");
			}
		});
		plFrmYeta7060.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta7060.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		plFrmYeta7060.setPadding(2);
		plFrmYeta7060.setFrame(true);
		plFrmYeta7060.setLayout(new FormLayout());

		degtrSecRegnNum = new HiddenField<String>();
		imgKeyCode = new HiddenField<String>();
		imgKeyCode.setName("imgKeyCode");
		plFrmYeta7060.add(imgKeyCode);
		frmKey = new HiddenField<String>();
		frmKey.setName("frmKey");
		plFrmYeta7060.add(frmKey);
		whdgTxRegrstDeptCd = new HiddenField<String>();
		whdgTxRegrstDeptCd.setName("whdgTxRegrstDeptCd");
		plFrmYeta7060.add(whdgTxRegrstDeptCd);

		topYeta7060Bar = new ButtonBar();
		topYeta7060Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		btnYeta7060Dpob = new Button("주(총괄납부)사업장관리");
		topYeta7060Bar.add(btnYeta7060Dpob);
		btnYeta7060Dpob.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				YetaP706001 yetaP706001Form = new YetaP706001(ActionDatabase.READ, getThis());

				MSFFormWindows msFwYeta706001 = new MSFFormWindows("주(총괄납부)사업장 관리",yetaP706001Form, "닫기", "820px", "710px", false);
				msFwYeta706001.show();
				yetaP706001Form.setMSFFormWindows(msFwYeta706001);
			}
		});

		btnYeta7060New = new Button("신규");
		btnYeta7060New.setIcon(MSFMainApp.ICONS.new16());
		topYeta7060Bar.add(btnYeta7060New);
		btnYeta7060New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 신규 버튼 클릭시 처리
				formYeta7060Init();
		...
		btnYeta7060Save = new Button("저장");
		btnYeta7060Save.setIcon(MSFMainApp.ICONS.save16());
		//		btnYeta7060Save.addSelectionListener(selectionListener);
		btnYeta7060Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
		...
		topYeta7060Bar.add(btnYeta7060Save);

		btnYeta7060Del = new Button("삭제");
		btnYeta7060Del.setIcon(MSFMainApp.ICONS.delete16());
		btnYeta7060Del.addSelectionListener(selectionListener);
		topYeta7060Bar.add(btnYeta7060Del);

		btnYeta7060Sreach = new Button("조회");
		btnYeta7060Sreach.setIcon(MSFMainApp.ICONS.search16());
		topYeta7060Bar.add(btnYeta7060Sreach);
		btnYeta7060Sreach.addListener(Events.Select,
		...
		plFrmYeta7060.add(topYeta7060Bar);

		// 파일업로드 처리 - 사업장 직인 정보
		plFrmYeta7060.setAction("myurl"); // url 정보 추가 할것.
		plFrmYeta7060.setEncoding(Encoding.MULTIPART);
		plFrmYeta7060.setMethod(Method.POST);

		vp.add(plFrmYeta7060);
		plFrmYeta7060.setSize("990px", "680px");
	}
	...
	private void createStandardForm() {
		...
		plFrmYeta7060.add(layoutContainer);
	...
	private LayoutContainer createMasterGrid() {
		...
		msfGridPanel01 = new MSFGridPanel(mYeta7060Def, false, false, false, false,false);
		msfGridPanel01.setHeaderVisible(false);
		msfGridPanel01.setBodyBorder(true);
		msfGridPanel01.setBorders(true);

		final Grid Yeta7060MGrid = msfGridPanel01.getMsfGrid().getGrid();
		Yeta7060MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (msfGridPanel01.getCurrentlySelectedItem() != null) { 
					formYeta7060Init();
					//선택된 값을 우측 그리드에 넣기 위한 메서드 호출 
					leftYeta7060Detail(msfGridPanel01.getCurrentlySelectedItem());
				}
	...
	private void createListGrid() {
		...
		msfGridPanel = new MSFGridPanel(Yeta7060Def, false, false, false, true, true);
		...
		plFrmYeta7060.add(FieldSetGrid);
	...
	private Yeta7060 getThis() {
		return this;
	}

	private void leftYeta7060Detail(BaseModel selRecord) {
	 ...
	 * 저장
	 */
	private void saveYeta7060(final Yeta7060DTO dto, final ActionDatabase actionDatabase, final String fileName) {
			
		yeta7060Service.saveYeta7060(dto, actionDatabase, new AsyncCallback<String>() {
			@Override
			public void onSuccess(String result) {
				...
				if ("DELETE".equals(actionDatabase.name())) {
					...
					if(!"".equals(fileName) && fileName != null){
					    ...
					    plFrmYeta7060.mask();  
					    plFrmYeta7060.setAction(GWT.getHostPageBaseURL() + "upload/ImageFileUpload.do"); 
	                    plFrmYeta7060.submit();
	                    plFrmYeta7060.onFrameLoad(); 
	                    picBoolFile = true;
						//사진 등록처리 servlet 끝 
	                    
						// reset and unmask the form
						// after file upload
	                    plFrmYeta7060.addListener(Events.Submit, new Listener<FormEvent>() {
				              public void handleEvent(FormEvent evt) {  
				                  plFrmYeta7060.unmask(); 
			...
			public void onFailure(Throwable caught) {
				caught.printStackTrace();
					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
						MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta7060("+ actionDatabase.name()+ ") : "+ caught),null);
			}
		});
	}
