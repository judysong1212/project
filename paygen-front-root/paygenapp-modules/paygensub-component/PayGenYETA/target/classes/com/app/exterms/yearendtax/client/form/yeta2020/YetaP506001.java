 ...
 * =====================================================
 **/
public class YetaP506001 extends MSFFormPanel {
	...
	private ContentPanel cpYetaP506001;

	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP506001_01GridPanel;
	private MSFGridPanel YetaP506001_02GridPanel;

	private Yeta5060Def Yeta506001MtDef = new Yeta5060Def("Yeta506001_Mt");
	private Yeta5060Def Yeta506001StDef = new Yeta5060Def("Yeta506001_St");
	// -------------- grid 선언 종료 ---------------
	...
	//	private YetaP2060ServiceAsync YetaP506001Service = YetaP506001Service.Util.getInstance();
	 ...
	 * ++++++++++++++
	 **/
	public YetaP506001(final ActionDatabase actionDatabase,	final MSFPanel caller) {
		...
		cpYetaP506001 = new ContentPanel();

		createGridForm();

		mReload();
		cpYetaP506001.setBodyBorder(false);
		cpYetaP506001.setBorders(false);
		cpYetaP506001.setHeaderVisible(false);
		cpYetaP506001.setSize("790px", "640px");

		this.add(cpYetaP506001);
	...
	private void createGridForm() {
		...
		YetaP506001_01GridPanel = new MSFGridPanel(Yeta506001MtDef, false, false, false, false, false);
		YetaP506001_01GridPanel.setHeaderVisible(false);
		YetaP506001_01GridPanel.setSize(305, 570);
		YetaP506001_01GridPanel.setBorders(true);

		final Grid Yeta206001MGrid = YetaP506001_01GridPanel.getMsfGrid().getGrid();
		Yeta206001MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (YetaP506001_01GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = YetaP506001_01GridPanel.getCurrentlySelectedItem();
					sReload(bmData);
				}
			}
		}); 	

		fieldMtSet.add(YetaP506001_01GridPanel);
		...
		YetaP506001_02GridPanel = new MSFGridPanel(Yeta506001StDef, false, false, false, false, false);
		YetaP506001_02GridPanel.setHeaderVisible(false);
		YetaP506001_02GridPanel.setSize(460, 570);
		YetaP506001_02GridPanel.setBorders(true);
		
		fieldStSet.add(YetaP506001_02GridPanel);
		...
		btnBassSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (YetaP506001_02GridPanel.getCurrentlySelectedItem() == null) { 
		...
		cpYetaP506001.add(layoutContainer1, new FormData("100%"));
		cpYetaP506001.add(btnBar, new FormData("100%"));
	...
	public void mReload() {
		...
		YetaP506001_01GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP506001_01GridPanel.reload();
	...
	public void sReload(BaseModel bmData) {
		...
		YetaP506001_02GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP506001_02GridPanel.getTableDef().addColumnFilter("utDpobCd",MSFSharedUtils.allowNulls(bmData.get("utDpobCd")), SimpleColumnFilter.OPERATOR_LIKE);
		YetaP506001_02GridPanel.reload();
							...
							MessageBox.confirm("종사업장 등록", "선택된 사업장의 사업자번호가 주 사업자 번호로 변경됩니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
								...
								public void handleEvent(MessageBoxEvent be) {
									if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
										BaseModel bmData = YetaP506001_01GridPanel.getCurrentlySelectedItem();
										Yeta5060DTO dto = new Yeta5060DTO();
										...
										yeta5060Service.insertYetaS506001(dto, actionDatabase, new AsyncCallback<Integer>() {
											@Override
											public void onSuccess(Integer result) {	
												...
												//상세정보 초기화
												YetaP506001_01GridPanel.reload();
												YetaP506001_02GridPanel.getMsfGrid().clearData();
											}
		...
		MessageBox.confirm("사업장 삭제", "선택된 사업장을 삭제 하시겠습니까?<br>",new Listener<MessageBoxEvent>(){
			...
			public void handleEvent(MessageBoxEvent be) {
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					Iterator<BaseModel> itBm = YetaP506001_02GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator(); 
					List<Record>  lsRec  = new ArrayList<Record>();
					List<Yeta5060DTO> listYeta5060Dto = new ArrayList<Yeta5060DTO>();       
					...
					if (MSFSharedUtils.paramNotNull(records)) {   
						...
						while (records.hasNext()) {
							...
							listYeta5060Dto.add(dto);
						}
					}   


					if(listYeta5060Dto.size() <= 0 ) {
						MessageBox.info("경고", "삭제할 사업장을 선택해 주세요.", null);
						return;
					}

					yeta5060Service.Yeta506001_Delete(listYeta5060Dto, actionDatabase, new AsyncCallback<Long>() {
						@Override
			...
			MessageBox.confirm("주사업자 수정", "선택된 사업장의 일련번호를 변경합니다.<br> 진행 하시겠습니까?",new Listener<MessageBoxEvent>(){
				...
				public void handleEvent(MessageBoxEvent be) {
					if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
						...
						BaseModel bmData1 = YetaP506001_01GridPanel.getCurrentlySelectedItem();
						...
						yeta5060Service.Yeta506001NumUpdate(listYeta5060Dto, actionDatabase, new AsyncCallback<Long>() {
							@Override
							public void onSuccess(Long result) {	
								MessageBox.info("","저장 되었습니다.", null);
