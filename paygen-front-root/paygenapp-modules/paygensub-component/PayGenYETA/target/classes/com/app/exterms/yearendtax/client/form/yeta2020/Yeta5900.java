...
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Pyyt0302Def;
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Pyyt0304Def;
...
import com.app.exterms.yearendtax.client.service.yeta2020.Yeta5900ServiceAsync;
...
=====================================================**/
public class Yeta5900  extends MSFPanel { 
...
//	// private MSFCustomForm msfCustomForm; 
	private Pyyt0304Def pyyt0304Def  = new Pyyt0304Def("Yeta5900");   //그리드 테이블 컬럼 define  
	// private MSFCustomForm msfCustomForm; 
	private MSFGridPanel pyyt0304GridPanel;
	  
	//수당 
	private Pyyt0302Def pyyt0302ExtpyDef  = new Pyyt0302Def("Yeta5900_PYYT0302EXTPY");   //그리드 테이블 컬럼 define  
	// private MSFCustomForm msfCustomForm; 
	private MSFGridPanel pyyt0302ExtpyGridPanel; 
	  
	//공제 
	private Pyyt0302Def pyyt0302DducDef  = new Pyyt0302Def("Yeta5900_PYYT0302DDUC");   //그리드 테이블 컬럼 define  
	// private MSFCustomForm msfCustomForm; 
...
//      private TextField<String> srhMangeDeptNm; //관리부서 수정  
		...
		private FileUploadField Yeta5900flUp; // 파일업로드처리 부분
 ...
 **/	  
	 ...
	 */
      	...
      	private void authExecEnabled() {
      		...
      		srhPayYr.setValue(lsPayYrStore.findModel("year", "2020"));
 ...
 **/
	//입력수정삭제처리 rpc 처리부     
	private Yeta5900ServiceAsync yeta5900Service = Yeta5900Service.Util.getInstance();
...
//                            pyyt0302Dto.setRevnAddr(MSFSharedUtils.allowNulls(bmMapModel.get("revnAddr")));    /** column 수정주소 : revnAddr */                           
            		...
            		yeta5900Service.saveYeta5900ToPyyt0302(listPyyt0302Dto, flag, new AsyncCallback<Integer>() {
                        public void onFailure(Throwable caught) {
                            MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
                                    MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta5900ToPyyt0302(save) : " + caught), null);
                        }
 ...
 **/ 	
	...
	public Yeta5900(String txtForm) {
...
//                        pyyt0302Dto.setRevnAddr(MSFSharedUtils.allowNulls(bmMapModel.get("revnAddr")));    /** column 수정주소 : revnAddr */                           
				...
				MessageBox.confirm("경고", "해당 대상자를 정말로 삭제하겠습니까?(기등록된 수당공제항목도 함께 삭제됩니다.)</BR> 삭제된 데이터는 복구되지 않습니다.", new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						//if("Yes".equals(be.getButtonClicked().getText())) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
			        		yeta5900Service.deleteYeta5900List(listPyyt0302Dto, new AsyncCallback<Integer>() {
			                    public void onFailure(Throwable caught) {
			                        MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
...
//		fldstNewFieldset.setStyleAttribute("marginTop", "10px");
	    ...
	    Yeta5900flUp = new FileUploadField();
	    Yeta5900flUp.setFieldLabel("급여업로드");
		Yeta5900flUp.setName("Yeta5900flUp");
		Yeta5900flUp.setValidator(new Validator() {  
			@Override 
		...
		Yeta5900flUp.addListener(Events.Change, new Listener<FieldEvent>() {
	...
	//			excelGrid.getStore().removeAll();
		...
		layoutContainer_21.add(Yeta5900flUp, new FormData("100%"));
...
//    	 payDdlneYn.setValue((String)selRecord.get("payDdlneYn")) ;  /** column 급여마감여부 : payDdlneYn */
	...
	public void payrInfoReload() {
		...
		pyyt0304Dto.setPayCd((String) bmData.get("payCd"));
         ...
         yeta5900Service.Yeta5900InfoList(pyyt0304Dto, new AsyncCallback<Pyyt0304BM>() {
...
//		payDdlneYn.setValue("") ;  /** column 급여마감여부 : payDdlneYn */
	...
	public void exportExcel01() {
		...
		// payr0301GridPanel.callExportServlet("");
	       ...
	       // dlgn0250GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsDlgn0250Export.do","extgwtFrame" ,param);  
	        GWTUtils.xlsExportData(GWT.getHostPageBaseURL() + "exp/XlsYeta5900Export01.do","extgwtFrame", JsUtil.toJavaScriptObject(param)); 
	...
	public void exportExcel02() {
		...
		// payr0301GridPanel.callExportServlet("");
	       ...
	       // dlgn0250GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsDlgn0250Export.do","extgwtFrame" ,param);  
	        ...
	        pyyt0304GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta5900Export02.do","extgwtFrame" ,param);
	...
	public void exportTotExcel() {
	        ...
	        //넘어온메서드 값을 가지고 처리 한다.  
	       // dlgn0250GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsDlgn0250Export.do","extgwtFrame" ,param);  
	        GWTUtils.xlsExportData(GWT.getHostPageBaseURL() + "exp/XlsYeta5900Export03.do","extgwtFrame", JsUtil.toJavaScriptObject(param)); 
	...
	private void xlsExportData(String strTyp) {
        ...
        if("typ01".equals(strTyp)) {	 	// 가변
        	 pyyt0304GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsTemYeta590001Export.do","extgwtFrame", param);  
        }else {								// 고정
        	 pyyt0304GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsTemYeta590002Export.do","extgwtFrame", param);  
        }
		    	...
		    	public void handleEvent(MessageBoxEvent be) {
		      	//	if("Yes".equals(be.getButtonClicked().getText())){
		      		...
		      		if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
	              		...
	              		String filename = MSFSharedUtils.allowNulls(Yeta5900flUp.getValue());
	              		if (filename.length() == 0) {
	                		...
	                		// 가변
	                		if("typ01".equals(MSFSharedUtils.getSelectedComboValue(exlType,"commCd"))) {
	                			plFrmYeta1900.setAction(GWT.getHostPageBaseURL() + "imp/XlsReadYeta590001Import.do");
	                		}else {
	                		// 고정
	                			plFrmYeta1900.setAction(GWT.getHostPageBaseURL() + "imp/XlsReadYeta590002Import.do");
	                		}
