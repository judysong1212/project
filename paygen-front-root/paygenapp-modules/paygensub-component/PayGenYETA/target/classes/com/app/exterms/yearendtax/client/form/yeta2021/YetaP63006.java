...
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye160404BM;
import com.app.exterms.yearendtax.client.dto.yeta2021.Ye160404DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye160404Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63006Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaP63006ServiceAsync;
...
=====================================================**/
public  class YetaP63006   extends MSFFormPanel {  
 ...
 **/
	...
	private ContentPanel cpYeta63006;
	...
	// -------------- grid 선언 시작 ---------------
	private MSFGridPanel YetaP63006GridPanel;
	private Ye160404Def YetaP63006ToYe160404Def = new Ye160404Def("YetaP63006"); // 종전근무지내역 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료 ---------------
   ...
   */
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP63006ServiceAsync yetaP63006Service = YetaP63006Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP63006(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta63006 = new ContentPanel();

		createSearchForm(); 	// 기본정보 필드를 적용
		createMsfGridForm(); 	// 그리드필드
		createStandardForm(); 	// 기본정보필드

		cpYeta63006.setBodyBorder(false);
		cpYeta63006.setBorders(false);
		cpYeta63006.setHeaderVisible(false);
//		cpYeta63006.setSize("790px", "640px");
		cpYeta63006.setSize("930px", "640px");

		this.add(cpYeta63006);
		formBinding = new FormBinding(this, true);

//		this.setSize("810px", "710px");
		this.setSize("950px", "710px");
	}
	...
	private void createSearchForm() {
		...
		cpYeta63006.setLayout(new FlowLayout());
				...
				MessageBox.confirm("삭제", "선택하신 종전근무지 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP63006GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaP63006GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
	...
	private void createStandardForm() {
		...
		btnNewButton.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				BaseModel bmData = YetaP63006GridPanel.getCurrentlySelectedItem();
...
//				setPPRecord(bmData); //팝업에 넘길값 
				...
				// 전체내역
				YetaP630010 yetaP630010Form = new YetaP630010(ActionDatabase.UPDATE,getThis()); 
						    
				MSFFormWindows msFwYeta630010 = new MSFFormWindows("비과세 및 감면소득", yetaP630010Form, "닫기", "820px", "750px", true);
				msFwYeta630010.show();
				yetaP630010Form.setMSFFormWindows(msFwYeta630010);

				yetaP630010Form.bind(ppRecord);
			}
		});
		btnNewButton.hide();
	...
	private void createMsfGridForm() {
		...
		YetaP63006GridPanel = new MSFGridPanel(YetaP63006ToYe160404Def, false,false, false, false, false);
		YetaP63006GridPanel.setHeaderVisible(false);
		YetaP63006GridPanel.setSize("915px", "200px");
		YetaP63006GridPanel.setBorders(false);
		YetaP63006GridPanel.setHeadingText("");
		...
		YetaP63006GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  시작 --- **/
		
		final Grid YetaP63006Grid = YetaP63006GridPanel.getMsfGrid().getGrid();
		YetaP63006Grid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			@Override
			public void handleEvent(BaseEvent be) {
				// TODO Auto-generated method stub
				//그리드 선택시
				actionDatabase = ActionDatabase.UPDATE;
				yetaP13006ToYeta3200Detail(YetaP63006GridPanel.getCurrentlySelectedItem());
				btnNewButton.show();
			}
		});
		
		gridFieldSet.add(YetaP63006GridPanel);

		cpYeta63006.add(gridFieldSet);
	}

	private YetaP63006 getThis() {
		return this;
	}
	...
	private void YetaP63006FormSave() {
...
//    				ye160404Dto.setPrcspWhdgReipSumtYn(MSFSharedUtils.allowNulls(bmMapModel.get("prcspWhdgReipSumtYn")));    	/** column 종전근무지원천징수영수증제출여부 : prcspWhdgReipSumtYn */				
    			...
    			yetaP63006Service.activityOnYetaP63006ToYe160404(listYe160404Dto, actionDatabase, new AsyncCallback<Long>(){
    				public void onFailure(Throwable caught) {
    					MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
    							, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP63006ToYe160404(" + actionDatabase.name() + ") : " + caught)
    							, null);
    					}
...
//		hanNm01.setValue("");				// 성명   
	...
	public void reload() {
		...
		YetaP63006GridPanel.getTableDef().setTableColumnFilters(filters);
		YetaP63006GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 연말정산귀속년도 
		
		YetaP63006GridPanel.getTableDef().addColumnFilter("clutSeptCd",clutSeptCd.getValue()
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 정산구분
		
		YetaP63006GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);	// 시스템키
		
//		YetaP63006GridPanel.getTableDef().addColumnFilter("whdgDebrBusoprRgstnum", MSFSharedUtils.allowNulls(whdgDebrBusoprRgstnum.getValue())
//				, SimpleColumnFilter.OPERATOR_EQUALS); // 원천징수의무자사업자등록번호
		
		YetaP63006GridPanel.reload();
