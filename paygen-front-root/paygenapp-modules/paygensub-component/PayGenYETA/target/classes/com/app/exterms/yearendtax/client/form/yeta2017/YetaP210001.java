...
import com.app.exterms.yearendtax.client.form.defs.yeta2017.InfcPsnl0100Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP210001Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP210001ServiceAsync;
...
=====================================================**/
public  class YetaP210001   extends MSFFormPanel {  
	...
	private InfcPsnl0100Def yetaP210001def = new InfcPsnl0100Def("YETAP210001");
	private MSFGridPanel yetaP210001GridPanel;
	...
	private YetaP210001ServiceAsync yetaP210001Service = YetaP210001Service.Util.getInstance();
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP210001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta21001.setSize("890px", "640px");
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lscrPayYr.findModel("year",String.valueOf("2017")));
	...
	private void createSearchForm() {
		...
		//---------------------멀티콤보박스 닫기 -------------------------------------------------
		cpYeta21001.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
	...
	private void createStandardForm() {
		...
		cpYeta21001.add(layoutContainer);
	...
	private LayoutContainer payrModiListInfo() {
		...
		yetaP210001GridPanel = new MSFGridPanel(yetaP210001def, false, false, false, false,false);
		yetaP210001GridPanel.setHeaderVisible(false);
		yetaP210001GridPanel.setBodyBorder(false);
		yetaP210001GridPanel.setBorders(true);
		yetaP210001GridPanel.getBottomComponent().setVisible(false);

		final Grid<BaseModel> grid = yetaP210001GridPanel.getMsfGrid().getGrid();
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = yetaP210001GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					yetaP210001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					yetaP210001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				yetaP210001GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					yetaP210001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					yetaP210001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		btnSelected.addSelectionListener(new SelectionListener<ButtonEvent>() {   
			...
			public void componentSelected(ButtonEvent ce) {  
				...
				Iterator<BaseModel> itBm = yetaP210001GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); // .getSelectionModel().getSelectedItems().listIterator();
				...
				yetaP210001Service.saveYetaP210001ToYe161010(listYe161010Dto,listYe161020Dto, actionDatabase, new AsyncCallback<PagingLoadResult<ShowMessageBM>>() {
					...
					public void onFailure(Throwable caught) {
						...
						smForm.setWindowId("YETAP210001");
		...
		refresh.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				yetaP210001GridPanel.reload();	                	
			}  
		});		            
		bottomToolbar.add(refresh);

		//툴바 넣어주는것
		yetaP210001GridPanel.setBottomComponent(bottomToolbar);

		cp01.add(yetaP210001GridPanel);
	...
	public void reload() {
		...
		yetaP210001GridPanel.getTableDef().setTableColumnFilters(filters);

		yetaP210001GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		yetaP210001GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		
		...
		yetaP210001GridPanel.getTableDef().addColumnFilter("hdofcCodtnCd", strHdofcCodtnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		yetaP210001GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		yetaP210001GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDtilOccuInttnCd = YetaUtils.getStrValToBMMultiCombo(lsDtilOccuInttnCd,srhDtilOccuInttnCd.getValue(),"dtilOccuClsNm","dtilOccuInttnCd");  
		yetaP210001GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		yetaP210001GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		yetaP210001GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		yetaP210001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		yetaP210001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr, "year")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		yetaP210001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		yetaP210001GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(srhUtDpobCd.getValue())  , SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	private void formInit() {
		...
		yetaP210001GridPanel.getMsfGrid().clearData();
