...
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye162510BM;
import com.app.exterms.yearendtax.client.dto.yeta2019.Ye162510DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye162510Def;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43005Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP43005ServiceAsync;
...
=====================================================**/
public  class YetaP43005   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP43005GridPanel;
//	private Ye16Ta3130Def yetaP23004ToYe162510Def  = new Ye16Ta3130Def("YETAP13005");	//가족교육비 그리드 테이블 컬럼 define 
	private Ye162510Def yetaP23004ToYe162510Def  = new Ye162510Def("YetaP43005");	//가족교육비 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP43005ServiceAsync yetaP43005Service = YetaP43005Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
 ...
 **/ 
     ...
     */ 
		...
		private void YetaP43005FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				yetaP43005Service.activityOnYetaP43005ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("YetaP43005FormSave(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//		 }
	...
	public YetaP43005(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//				systemkey.setValue("20080013");
				...
				YetaP43005GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = YetaP43005GridPanel.getTableDef().getDefaultValues();  
...
//		         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
		        ...
		        YetaP43005GridPanel.getMsfGrid().getGrid().stopEditing();      
		        YetaP43005GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        YetaP43005GridPanel.getMsfGrid().getGrid().startEditing(YetaP43005GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 월세액소득공제 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP43005GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP43005GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//		});
	...
	private void createMsfGridForm() {
		...
		YetaP43005GridPanel = new MSFGridPanel(yetaP23004ToYe162510Def, false, false, false, false, false);
		YetaP43005GridPanel.setSize("790px", "400px");
		YetaP43005GridPanel.setBorders(false);
		YetaP43005GridPanel.setHeaderVisible(false);
		...
		YetaP43005GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  종료 --- **/
		
			
		cpYeta23005.add(YetaP43005GridPanel);
	}	
	...
	public void reload() {
			...
			YetaP43005GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaP43005GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
			YetaP43005GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaP43005GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaP43005GridPanel.reload();
