...
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73019Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73019ServiceAsync;
...
=====================================================**/
public  class YetaT73019   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73019; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73019GridPanel;
	private Ye162510Def YetaT73019ToYe162510Def = new Ye162510Def("YETAT13019");   //월세새액공제명세 그리드 테이블 컬럼 define
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT73019ServiceAsync yetaT73019Service = YetaT73019Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT73019Service.activityOnYetaT73019ToYe162510(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT73019(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73019 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73019.setBodyBorder(false);
		cpYetaT73019.setBorders(false);
		cpYetaT73019.setHeaderVisible(false);
		cpYetaT73019.setSize("790px", "640px");
		   
		this.add(cpYetaT73019);
...
//		     });     
		...
		cpYetaT73019.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				YetaT73019GridPanel.getMsfGrid().clearData();
				 
				BaseModel bmData = YetaT73019GridPanel.getTableDef().getDefaultValues();  
		        ...
		        YetaT73019GridPanel.getMsfGrid().getGrid().stopEditing();      
		        YetaT73019GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		        YetaT73019GridPanel.getMsfGrid().getGrid().startEditing(YetaT73019GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
...
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT73019GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT73019GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT73019.add(topYetaBtnBar);
...
//		cpYetaT73019.add(topYetaBtnBar);
		...
		YetaT73019GridPanel = new MSFGridPanel(YetaT73019ToYe162510Def, false, false, false, false, false);
		YetaT73019GridPanel.setSize("790px", "200px");
		YetaT73019GridPanel.setBorders(true);
		YetaT73019GridPanel.setHeaderVisible(false);
...
//		
//		YetaT73019GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		/** --- 그리드 합계 부분  종료 --- **/
		
//   	cpYetaT73019.add(YetaT73019GridPanel);
		...
		fldstNewFieldset.add(YetaT73019GridPanel);
		cpYetaT73019.add(fldstNewFieldset);
		cpYetaT73019.add(layoutContainer1);
		cpYetaT73019.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT73019GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73019GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73019GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73019GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73019GridPanel.reload();
