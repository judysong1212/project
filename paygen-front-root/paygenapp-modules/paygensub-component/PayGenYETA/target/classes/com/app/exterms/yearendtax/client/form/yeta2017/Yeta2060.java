 ...
 * =========================================================================
 **/
public class Yeta2060 extends MSFPanel {
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta2060Bar;
	private Button btnYeta2060Dpob; 
	private Button btnYeta2060New; 
	private Button btnYeta2060Save;
	private Button btnYeta2060Del;
	private Button btnYeta2060Sreach;
	// -------------- 상단 버튼 종료 --------------
...
//	private MSFComboBox<BaseModel> hhrkUntDpobCd; 					// 상위 단위사업장 코드
	...
	private CheckBoxGroup chkgrpYeta2060;   						// 체크 그룹
	...
	// -------------- 서비스 호출 시작 -------------- 
	//private InfcYeta2060ServiceAsync infcYeta2060Service = InfcYeta2060Service.Util.getInstance();
	...
	// -------------- grid 시작 -------------- 
	private Yeta2060Def Yeta2060Def = new Yeta2060Def(); // 그리드 테이블 컬럼 define
	private MSFGridPanel msfGridPanel;


	private Yeta2060Def mYeta2060Def = new Yeta2060Def("Yeta2060_01"); // 주 그리드 테이블 컬럼 define
	...
	// private PrgmComYeta2060DTO sysComYeta2060Dto;
	// -------------- DTO 선언 종료  -------------- 

	private VerticalPanel vp;
	private FormPanel plFrmYeta2060;
	...
	private SelectionListener<ButtonEvent> selectionListener = new SelectionListener<ButtonEvent>() {
		public void componentSelected(ButtonEvent ce) {
			if (ActionDatabase.INSERT.equals(actionDatabase)) {
				actionDatabase = null;
				if (ce.getButton() == btnYeta2060Save) {
					doAction(ActionDatabase.INSERT);
				}
			} else if (ce.getButton() == btnYeta2060Save) {
				doAction(ActionDatabase.UPDATE);
				actionDatabase = null;
			} else if (ce.getButton() == btnYeta2060Del) {
...
//		}
		...
		final Yeta2060DTO dto = new Yeta2060DTO();
...
//		}
			...
			public void handleEvent(MessageBoxEvent be) {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					if("DELETE".equals(actionDatabase.name())) {
						...
						yeta2060Service.delChkYeta2060(dto, actionDatabase, new AsyncCallback<Integer>() {
							@Override
							public void onSuccess(Integer result) {
								...
								if (result == 1) {
									...
									// 저장 처리
									saveYeta2060(dto, actionDatabase, fileName);
								}
							}
							...
							public void onFailure(Throwable caught) {
								...
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(),
										MSFMainApp.ADMINMESSAGES.ExceptionMessageService("delChkYeta2060("+ actionDatabase.name()+ ") : "+ caught),null);
							}
						});
					}else {
						// 저장 처리
	...
	public static native void closeFunction() /*-{
			   $wnd.closeFunction =
			      $entry(@com.app.exterms.yearendtax.client.form.yeta2017.Yeta2060::onCloseGwtFunction(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;));
	  }-*/;
	...
	public ContentPanel getViewPanel() {
		...
		if (panel == null) {
			...
			// 상위 사업장
			yeta2060Service.getParentDpobCd(new AsyncCallback<List<BaseModel>>() {
				@Override
			...
			vp.setSpacing(10);
			createYeta2060Form(); 		// 화면 기본정보를 설정
			 ...
			 */
			// 폼데이터를 바인딩
			formBinding = new FormBinding(this.plFrmYeta2060, true);
	...
	// 생성자 함수에 변수을 받아 처리
	public Yeta2060(String txtForm) {
	 ...
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 **/
	private void createYeta2060Form() {

		plFrmYeta2060 = new FormPanel();
		plFrmYeta2060.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 단위사업장등록관리"));
		plFrmYeta2060.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta2060.setBodyStyleName("pad-text");
		...
		btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//로그 뷰화면 호출 메서드 
				funcLogMessage("단위사업장등록관리","Yeta2060");
			}
		});
		plFrmYeta2060.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta2060.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		plFrmYeta2060.setPadding(2);
		plFrmYeta2060.setFrame(true);
		plFrmYeta2060.setLayout(new FormLayout());

		degtrSecRegnNum = new HiddenField<String>();
		imgKeyCode = new HiddenField<String>();
		imgKeyCode.setName("imgKeyCode");
		plFrmYeta2060.add(imgKeyCode);
		frmKey = new HiddenField<String>();
		frmKey.setName("frmKey");
		plFrmYeta2060.add(frmKey);
		whdgTxRegrstDeptCd = new HiddenField<String>();
		whdgTxRegrstDeptCd.setName("whdgTxRegrstDeptCd");
		plFrmYeta2060.add(whdgTxRegrstDeptCd);

		topYeta2060Bar = new ButtonBar();
		topYeta2060Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		btnYeta2060Dpob = new Button("주(총괄납부)사업장관리");
		topYeta2060Bar.add(btnYeta2060Dpob);
		btnYeta2060Dpob.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				YetaP206001 yetaP206001Form = new YetaP206001(ActionDatabase.READ, getThis());

				MSFFormWindows msFwYeta206001 = new MSFFormWindows("주(총괄납부)사업장 관리",yetaP206001Form, "닫기", "820px", "710px", false);
				msFwYeta206001.show();
				yetaP206001Form.setMSFFormWindows(msFwYeta206001);
			}
		});

		btnYeta2060New = new Button("신규");
		btnYeta2060New.setIcon(MSFMainApp.ICONS.new16());
		topYeta2060Bar.add(btnYeta2060New);
		btnYeta2060New.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				// 신규 버튼 클릭시 처리
				formYeta2060Init();
		...
		btnYeta2060Save = new Button("저장");
		btnYeta2060Save.setIcon(MSFMainApp.ICONS.save16());
		//		btnYeta2060Save.addSelectionListener(selectionListener);
		btnYeta2060Save.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
		...
		topYeta2060Bar.add(btnYeta2060Save);

		btnYeta2060Del = new Button("삭제");
		btnYeta2060Del.setIcon(MSFMainApp.ICONS.delete16());
		btnYeta2060Del.addSelectionListener(selectionListener);
		topYeta2060Bar.add(btnYeta2060Del);

		btnYeta2060Sreach = new Button("조회");
		btnYeta2060Sreach.setIcon(MSFMainApp.ICONS.search16());
		topYeta2060Bar.add(btnYeta2060Sreach);
		btnYeta2060Sreach.addListener(Events.Select,
		...
		plFrmYeta2060.add(topYeta2060Bar);

		// 파일업로드 처리 - 사업장 직인 정보
		plFrmYeta2060.setAction("myurl"); // url 정보 추가 할것.
		plFrmYeta2060.setEncoding(Encoding.MULTIPART);
		plFrmYeta2060.setMethod(Method.POST);

		vp.add(plFrmYeta2060);
		plFrmYeta2060.setSize("990px", "680px");
	}
	...
	private void createStandardForm() {
		...
		plFrmYeta2060.add(layoutContainer);
	...
	private LayoutContainer createMasterGrid() {
		...
		msfGridPanel01 = new MSFGridPanel(mYeta2060Def, false, false, false, false,false);
		msfGridPanel01.setHeaderVisible(false);
		msfGridPanel01.setBodyBorder(true);
		msfGridPanel01.setBorders(true);

		final Grid Yeta2060MGrid = msfGridPanel01.getMsfGrid().getGrid();
		Yeta2060MGrid.addListener(Events.RowDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (msfGridPanel01.getCurrentlySelectedItem() != null) { 
					formYeta2060Init();
					//선택된 값을 우측 그리드에 넣기 위한 메서드 호출 
					leftYeta2060Detail(msfGridPanel01.getCurrentlySelectedItem());
				}
	...
	private void createListGrid() {
		...
		msfGridPanel = new MSFGridPanel(Yeta2060Def, false, false, false, true, true);
		...
		plFrmYeta2060.add(FieldSetGrid);
	...
	private Yeta2060 getThis() {
		return this;
	}

	private void leftYeta2060Detail(BaseModel selRecord) {
	 ...
	 * 저장
	 */
	private void saveYeta2060(final Yeta2060DTO dto, final ActionDatabase actionDatabase, final String fileName) {
			
		yeta2060Service.saveYeta2060(dto, actionDatabase, new AsyncCallback<String>() {
			@Override
			public void onSuccess(String result) {
				...
				if ("DELETE".equals(actionDatabase.name())) {
					...
					if(!"".equals(fileName) && fileName != null){
					    ...
					    plFrmYeta2060.mask();  
					    plFrmYeta2060.setAction(GWT.getHostPageBaseURL() + "upload/ImageFileUpload.do"); 
	                    plFrmYeta2060.submit();
	                    plFrmYeta2060.onFrameLoad(); 
	                    picBoolFile = true;
						//사진 등록처리 servlet 끝 
	                    
						// reset and unmask the form
						// after file upload
	                    plFrmYeta2060.addListener(Events.Submit, new Listener<FormEvent>() {
				              public void handleEvent(FormEvent evt) {  
				                  plFrmYeta2060.unmask(); 
