...
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8500Service;
import com.app.exterms.yearendtax.client.service.yeta2023.Yeta8500ServiceAsync;
...
=====================================================**/
public class Yeta8500   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmYeta8500; 
    private String txtForm = "";
    private XTemplate detailTp;
 
    private Yeta1000Def yeta1000def = new Yeta1000Def("Yeta8500");
...
//		private MSFGridPanel typOccuCdGridPanel;
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
   private ButtonBar topYeta8500Bar;
   private Button btnYeta8500Init; 
//   private Button btnYeta0100Save;
//   private Button btnYeta0100Del;
   private Button btnYeta8500Sreach;
//   private Button btnYeta0100Print;
   private Button btnYeta8500Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 **/
	...
	private Yeta8500ServiceAsync yeta8500Service = Yeta8500Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 
   ...
   // -------------- stroe [검색부]선언 시작 -------------- 
	   ...
	   **/
	 	 ...
	 	 */
	  	private void checkYeta8500Auth( String authAction, ListStore<BaseModel>bm) {
...
//	    
	 ...
	 **/ 	 
		 ...
		 */
		public Yeta8500() {
			setSize("1670px", "770px");  
		} 

		public Yeta8500(String txtForm) {
	 ...
	 **/		  
		private void createYeta0100Form() {
			...
			plFrmYeta8500 = new FormPanel(); 
			plFrmYeta8500.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산급여반영"));
			plFrmYeta8500.setIcon(MSFMainApp.ICONS.text());
			plFrmYeta8500.setPadding(2);
			plFrmYeta8500.setFrame(true); 
			...
			btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 //로그 뷰화면 호출 메서드 
					  funcLogMessage("연말정산급여반영","Yeta8500");
					}
				});
			plFrmYeta8500.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
			plFrmYeta8500.getHeader().addTool(btnlogView); 
			/************************************************************************
			 * 화면 에러 처리시 필요한 부분으로 추가 
			 ************************************************************************/
 
		     //멀티콤보박스 닫기 
			plFrmYeta8500.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
	            @Override
...
//	                        }
			...
			/** 상단 버튼 ButtonBar**/
			topYeta8500Bar = new ButtonBar();    
			topYeta8500Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
			btnYeta8500Init = new Button("초기화");  
			btnYeta8500Init.setIcon(MSFMainApp.ICONS.new16());
			topYeta8500Bar.add(btnYeta8500Init); 
			btnYeta8500Init.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//초기화 
	    			formInit();
	    		}
			...
			btnYeta8500Sreach = new Button("조 회");  
			btnYeta8500Sreach.setIcon(MSFMainApp.ICONS.search());
			topYeta8500Bar.add(btnYeta8500Sreach);
			btnYeta8500Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//조회버튼 클릭시 처리 
	    			reloadYeta8500();  
	    		}
...
//			topYeta8500Bar.add(btnYeta0100Print);	
			
//			btnYeta0100Print = new Button("인 쇄");  
//			btnYeta0100Print.setIcon(MSFMainApp.ICONS.print16());
//			topYeta8500Bar.add(btnYeta0100Print);	
			
			btnYeta8500Excel = new Button("엑 셀");  
			btnYeta8500Excel.setIcon(MSFMainApp.ICONS.excel16());
			topYeta8500Bar.add(btnYeta8500Excel);
			btnYeta8500Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//엑셀 저장
	    			xlsExportData();  
	    		}
			...
			plFrmYeta8500.add(topYeta8500Bar);
		      
			//파일업로드 처리
			plFrmYeta8500.setAction("myurl");
			plFrmYeta8500.setEncoding(Encoding.MULTIPART);
			plFrmYeta8500.setMethod(Method.POST);		         
			vp.add(plFrmYeta8500);	
			
			plFrmYeta8500.setSize("1060px", "750px");	      
			plFrmYeta8500.setLayout(new FlowLayout());
		}
...
//		  	plFrmYeta8500.add(typOccuCdGrid);
...
//														 searchBusinCd.setValue(lsBusinCd.getAt(0));
			    ...
			    lsPayrMangDeptCd.addStoreListener( new StoreListener<BaseModel>() {   
			        public void handleEvent(StoreEvent<BaseModel> be) {  
			        	checkYeta8500Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
			        }
			    });    
				    	   ...
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta8500Auth("srhEmymtDivCd", lsEmymtDivCd); 
			           	 }
			        }
			    });    
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta8500Auth("srhDeptCd", lsDeptCd); 
			           	 }
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		 //  checkYeta8500Auth("srhTypOccuCd", lsTypOccuCd); 
			           	 }
			        }
			    });     
...
//		                BaseModel bmMapModel = (BaseModel)record.getModel();
					...
					yeta8500Service.saveYeta8500Grid(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta8500Grid(" + "저장" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
		...
		cp01.addButton(new Button("급여반영", new SelectionListener<ButtonEvent>() {   
			@Override  
			public void componentSelected(ButtonEvent ce) {   
...
//					ntime = format.format(c.getTime());
					...
					while (records.hasNext()) {
		                ...
		                ye161010Dto.setYrtxApptnYrMnth("202402");		/** column 연말정산적용년월 : yrtxApptnYrMnth */
...
//		                ye161010Dto.setDivdPymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("divdPymtDivCd$commCd")));/** column 분할납부구분코드 : divdPymtDivCd */
					...
					yeta8500Service.activityOnYeta8500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta8500List(" + "급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
		...
		//2022연말정산_추가 : 3월급여반영버튼 삭제 후 2월급여반영버튼을 통합으로 쓰기
		/////
...
//					ntime = format.format(c.getTime());
					...
					yeta8500Service.activityOnYeta8500_D_List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta8500List(" + "당월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
	...
	private Yeta8500 getThis() {
		return this;
	}
		   
	private void reloadYeta8500() {
	...
	private void xlsExportData() {
		...
		if ("".equals(srhYrtxBlggYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta8500Export.do", "extgwtFrame", param);
			}
		} else {
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta8500Export.do", "extgwtFrame", param);
		}
