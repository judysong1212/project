...
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye165030BM;
import com.app.exterms.yearendtax.client.dto.yeta2017.Ye165030DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2017.Ye165030Def;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23001Service;
import com.app.exterms.yearendtax.client.service.yeta2017.YetaP23001ServiceAsync;
...
=====================================================**/
public  class YetaP23001   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP23001GridPanel;
//	private Ye16Ta3170Def yetaP23001ToYe5030Def  = new Ye16Ta3170Def("YETAP23001");	//이월기부금 그리드 테이블 컬럼 define 
	private Ye165030Def yetaP23001ToYe5030Def  = new Ye165030Def("YETAP23001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
// 	private TextField<String> srhClutSeptCd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP23001ServiceAsync yetaP23001Service = YetaP23001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP23001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		lsdacRvyyStore = SysComComboUtils.getSysComComboYearData(msfCoCalendarDto);		//급여년도
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent be) {
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 yetaP23001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = yetaP23001GridPanel.getTableDef().getDefaultValues();  
		         ...
		         yetaP23001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         yetaP23001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         yetaP23001GridPanel.getMsfGrid().getGrid().startEditing(yetaP23001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 이월기부금 내용을 삭제하시겠습니까?" , new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){	
							...
							Iterator<BaseModel> itBm = yetaP23001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP23001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	private void createMsfGridForm() {
		...
		yetaP23001GridPanel = new MSFGridPanel(yetaP23001ToYe5030Def, false, false, false, false, false);
		yetaP23001GridPanel.setBorders(false);
		yetaP23001GridPanel.setBodyBorder(false);
		yetaP23001GridPanel.setHeaderVisible(false); 
		yetaP23001GridPanel.getBottomComponent().setVisible(false);
		
		GWTUtils.findColumnConfig(yetaP23001GridPanel, Ye165030BM.ATTR_THCYRDDCAMT).setStyle("background-color:#F5F6CE;");
		
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		GWTUtils.findColumnConfig(yetaP23001GridPanel, Yeta3170BM.ATTR_CTRBAMNT).setStyle("Height:20px;");
		...
		yetaP23001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		
		cpYeta23001.add(yetaP23001GridPanel);
		cp01.add(yetaP23001GridPanel);
	...
	private void yetaP23001FormSave() {
...
//                    }
			...
			if(dataChk) {
				...
				yetaP23001Service.activityOnYetaP23001ToYe165030(listYe165030Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP23001ToYe165030(" + actionDatabase.name() + ") : " + caught), null);
					}
	...
	public void reload() {
		...
		yetaP23001GridPanel.getTableDef().setTableColumnFilters(filters);
		
		yetaP23001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		
		yetaP23001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		
		yetaP23001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		
		yetaP23001GridPanel.reload();
