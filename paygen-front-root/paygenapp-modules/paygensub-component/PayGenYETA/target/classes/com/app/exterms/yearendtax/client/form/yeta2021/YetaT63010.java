...
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye162040Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63010Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63010ServiceAsync;
...
=====================================================**/
public  class YetaT63010   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63010; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63010GridPanel;
	private Ye162040Def ye162040Def = new Ye162040Def("YetaT63010");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63010ServiceAsync yetaT63010Service = YetaT63010Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63010Service.activityOnYetaT63010(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63010(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63010 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63010.setBodyBorder(false);
		cpYetaT63010.setBorders(false);
		cpYetaT63010.setHeaderVisible(false);
		cpYetaT63010.setSize("790px", "640px");
		   
		this.add(cpYetaT63010);
...
//		     });     
		...
		cpYetaT63010.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63010GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63010GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63010GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63010GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63010GridPanel.getMsfGrid().getGrid().startEditing(YetaT63010GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT63010GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT63010GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT63010.add(topYetaBtnBar);
...
//		cpYetaT63010.add(topYetaBtnBar);
		...
		YetaT63010GridPanel = new MSFGridPanel(ye162040Def, false, false, false, false, false);
		YetaT63010GridPanel.setSize("790px", "200px");
		YetaT63010GridPanel.setBorders(true);
		YetaT63010GridPanel.setHeaderVisible(false);
...
//		
//		YetaT63010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63010.add(YetaT63010GridPanel);
		...
		fldstNewFieldset.add(YetaT63010GridPanel);
		cpYetaT63010.add(fldstNewFieldset);
		cpYetaT63010.add(layoutContainer10);
		cpYetaT63010.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63010GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63010GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63010GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63010GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63010GridPanel.reload();
