...
import com.app.exterms.yearendtax.client.form.defs.yeta2018.Ye162040Def;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33010Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33010ServiceAsync;
...
=====================================================**/
public  class YetaT33010   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33010; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33010GridPanel;
	private Ye162040Def ye162040Def = new Ye162040Def("YetaT33010");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33010ServiceAsync yetaT33010Service = YetaT33010Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33010Service.activityOnYetaT33010(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT33010(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33010 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33010.setBodyBorder(false);
		cpYetaT33010.setBorders(false);
		cpYetaT33010.setHeaderVisible(false);
		cpYetaT33010.setSize("790px", "640px");
		   
		this.add(cpYetaT33010);
...
//		     });     
		...
		cpYetaT33010.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT33010GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT33010GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT33010GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT33010GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT33010GridPanel.getMsfGrid().getGrid().startEditing(YetaT33010GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT33010GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT33010GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT33010.add(topYetaBtnBar);
...
//		cpYetaT33010.add(topYetaBtnBar);
		...
		YetaT33010GridPanel = new MSFGridPanel(ye162040Def, false, false, false, false, false);
		YetaT33010GridPanel.setSize("790px", "200px");
		YetaT33010GridPanel.setBorders(true);
		YetaT33010GridPanel.setHeaderVisible(false);
...
//		
//		YetaT33010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT33010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT33010.add(YetaT33010GridPanel);
		...
		fldstNewFieldset.add(YetaT33010GridPanel);
		cpYetaT33010.add(fldstNewFieldset);
		cpYetaT33010.add(layoutContainer10);
		cpYetaT33010.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33010GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33010GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33010GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33010GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33010GridPanel.reload();
