...
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye161020Def;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP410002Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaP410002ServiceAsync;
...
=====================================================**/
public  class YetaP410002   extends MSFFormPanel {  
 ...
 **/
	...
	private ContentPanel cpYeta4100;

	private Ye161020Def ye161020Def = new Ye161020Def("YetaP410002");
	private MSFGridPanel YetaP410002GridPanel;
 ...
 **/ 
	...
	private YetaP410002ServiceAsync yetaP410002Service = YetaP410002Service.Util.getInstance();
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP410002(ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad();
		...
		cpYeta4100 = new ContentPanel();

		createSearchForm(); 	// 검색필드를 적용
		createStandardForm(); 	// 기본정보필드

		cpYeta4100.setBodyBorder(false);
		cpYeta4100.setBorders(false);
		cpYeta4100.setHeaderVisible(false);
		cpYeta4100.setSize("890px", "640px");
		...
		btnSrh.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				if(creUtDpobCd.getValue() != null && !"".equals(creUtDpobCd.getValue()) ) {
					...
					yetaP410002Service.getYe161005Data(ye161005Dto , new AsyncCallback<Ye161005DTO>() {
...
//				
//				
//				int  lsCnt = YetaP410002GridPanel.getMsfGrid().getGrid().getStore().getCount();
//          	  
//          	  	for(int iCnt=0;iCnt < lsCnt;iCnt++) {
//          	  		YetaP410002GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
//          	  		YetaP410002GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
//          	  	} 
//          	  	YetaP410002GridPanel.getMsfGrid().getGrid().getView().refresh(true);
//          	  	
//			}
//		});
//		btnBar.add(btnAllOn);
...
//          	  
//          	  	for(int iCnt=0;iCnt < lsCnt;iCnt++) {
//          	  		YetaP410002GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
//          	  		YetaP410002GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
//          	  	} 
//          	  	YetaP410002GridPanel.getMsfGrid().getGrid().getView().refresh(true);
//          	  	
...
//		lcSchCol3.add(layoutContainer27,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.34)); 	// 공백
	...
	private void createStandardForm() {
		...
		cpYeta4100.add(layoutContainer);
	...
	private LayoutContainer payrModiListInfo() {
		...
		YetaP410002GridPanel = new MSFGridPanel(ye161020Def, false, false, false, false);
		YetaP410002GridPanel.setHeaderVisible(false);
		YetaP410002GridPanel.setBodyBorder(false);
		YetaP410002GridPanel.setBorders(true);
		YetaP410002GridPanel.getBottomComponent().setVisible(true);

		
        final Grid<BaseModel> grid = YetaP410002GridPanel.getMsfGrid().getGrid();
        ...
        btnOnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {   
        	...
        	public void componentSelected(ButtonEvent ce) {  
        		...
        		if("".equals(creUtDpobCd.getValue()) || creUtDpobCd.getValue() == null) {
            		...
            		Iterator<BaseModel> itBm  = YetaP410002GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); 
        ...
        //툴바 넣어주는것
        YetaP410002GridPanel.setBottomComponent(bottomToolbar);
        
		

		cp01.add(YetaP410002GridPanel);
	...
	public void reload() {
		...
		// 그리드 초기화
		YetaP410002GridPanel.getMsfGrid().clearData();
		...
		YetaP410002GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP410002GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", srhClutYr.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP410002GridPanel.getTableDef().addColumnFilter("clutSeptCd", clutSeptCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
				 
		YetaP410002GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", payrMangDeptCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP410002GridPanel.getTableDef().addColumnFilter("emymtDivCd", emymtDivCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 		
		
		
		YetaP410002GridPanel.getTableDef().addColumnFilter("deptCd", deptCd.getValue() , SimpleColumnFilter.OPERATOR_EQUALS);
		YetaP410002GridPanel.getTableDef().addColumnFilter("typOccuCd", typOccuCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP410002GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", dtilOccuInttnCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS); 
		
		YetaP410002GridPanel.getTableDef().addColumnFilter("businCd", businCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP410002GridPanel.getTableDef().addColumnFilter("hanNm", srhHanNm.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP410002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP410002GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   
		
		
		YetaP410002GridPanel.getTableDef().addColumnFilter("utDpobCd", creUtDpobCd.getValue(), SimpleColumnFilter.OPERATOR_EQUALS);
		YetaP410002GridPanel.getTableDef().addColumnFilter("utDpobChk", utDpobChk.getValue()? "1" : "0", SimpleColumnFilter.OPERATOR_EQUALS);
		
		YetaP410002GridPanel.reload();
...
//				creEdacRvyy.setValue(lscreClutYrStore.findModel("year","2016"));
	...
	private void saveYeta161010(List<Ye161010DTO> lsYe161010Dto) {
		...
		yetaP410002Service.saveYetaP410002ToYe161010(lsYe161010Dto , new AsyncCallback<Integer>() {
			public void onFailure(Throwable caught) {
				MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
...
//		}
	...
	//		return;
		...
		yetaP410002Service.saveYetaP410002ToYeta161005(ye161005Dto , actionDatabase ,  new AsyncCallback<Integer>() {
			@Override
               public void onFailure(Throwable caught) {
            	   MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
