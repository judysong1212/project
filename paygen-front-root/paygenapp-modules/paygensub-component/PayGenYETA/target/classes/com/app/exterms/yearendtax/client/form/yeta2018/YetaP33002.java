...
import com.app.exterms.yearendtax.client.dto.yeta2018.Ye164010DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2018.Ye164010Def;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP33002Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP33002ServiceAsync;
...
=====================================================**/
public  class YetaP33002   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP33002GridPanel;
	private Ye164010Def YetaP33002ToYe164010Def  = new Ye164010Def("YetaP33002");//의료비내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
 ...
 **/ 
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP33002ServiceAsync yetaP33002Service = YetaP33002Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 	
	...
	private void YetaP33002FormSave() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			if (dataChk) {
				...
				yetaP33002Service.activityOnYetaP33002ToYe164010(listYe164010Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService()
								, MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP33002ToYe164010(" + actionDatabase.name() + ") : " + caught), null);
						}
...
//		 }
	...
	public YetaP33002(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//				}
//
//				YetaP33002GridPanel.getMsfGrid().clearData();
//
//				BaseModel bmData = YetaP33002GridPanel.getTableDef().getDefaultValues();
//					 
...
//				bmData.set("mdxpsSeilNum", 0L);     	 			/** column 의료비지급일련번호 : mdxpsSeilNum */
//				bmData.set("mdxpsVvalKrnNm", "기타자료");     	 		/** column 의료비국세자료구분코드 : mdxpsVvalKrnCd */
//				bmData.set("mdxpsVvalKrnCd", "Y0140001");     	 	/** column 의료비국세자료구분코드 : mdxpsVvalKrnCd */
//				
//				bmData.set("resnoEncCntn", "");   					/** column 의료비공제대상자주민등록번호 : resnoEncCntn */
//				bmData.set("yn", "1");    							/** column 본인등해당여부 : yn */
//				bmData.set("bsnoEncCntn", "");    					/** column 지급처사업자등록번호 : bsnoEncCntn */
//				bmData.set("plymNm", "");    						/** column 의료비_상호명 : plymNm */
//				bmData.set("mdxpsPrfClCd", "");   					/** column 의료비_의료증빙코드 : mdxpsPrfClCd */
//					 
//				bmData.set("scnt", 0L);   							/** column 의료비_건수 : scnt */
//				bmData.set("useAmt", 0L);    						/** column 의료비_금액 : useAmt */
//				bmData.set("yn2", "0");    							/** column 의료비_난임시술비해당여부 : yn2 */
//			         
//			    YetaP33002GridPanel.getMsfGrid().getGrid().stopEditing();      
//			    YetaP33002GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//			    YetaP33002GridPanel.getMsfGrid().getGrid().startEditing(YetaP33002GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//			         
//				actionDatabase = ActionDatabase.INSERT;
				...
				MessageBox.confirm("삭제", "선택하신 의료비 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							...
							Iterator<BaseModel> itBm = YetaP33002GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP33002GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//			BaseModel bmData = YetaP33002GridPanel.getMsfGrid().getStore().getModels().get(0);  
//			if (MSFSharedUtils.paramNotNull(bmData)){
				// TODO Auto-generated method stub
				IColumnFilter filters = null;
				
				YetaP33002GridPanel.getTableDef().setTableColumnFilters(filters);
				YetaP33002GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
	...
	//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
				YetaP33002GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
						, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
				YetaP33002GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
						, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
				YetaP33002GridPanel.reload();
...
//			actionDatabase = ActionDatabase.READ;
	...
	private void createMsfGridForm() {
		...
		YetaP33002GridPanel = new MSFGridPanel(YetaP33002ToYe164010Def, false,false, false, false, false);
		YetaP33002GridPanel.setSize("780px", "390px");
		YetaP33002GridPanel.setBorders(true);
		YetaP33002GridPanel.setBodyBorder(false);
		YetaP33002GridPanel.setHeaderVisible(false);
		YetaP33002GridPanel.getBottomComponent().setVisible(true);
		...
		payrSummaryRow.setCellStyle(Ye164010BM.ATTR_SCNT,"summary_color");	
		payrSummaryRow.setCellStyle(Ye164010BM.ATTR_USEAMT,"summary_color");	
		...
		YetaP33002GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		/** --- 그리드 합계 부분  시작 --- **/
			
		gridFieldSet.add(YetaP33002GridPanel);
