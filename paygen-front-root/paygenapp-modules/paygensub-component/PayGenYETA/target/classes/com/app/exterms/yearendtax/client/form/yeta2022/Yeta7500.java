...
import com.app.exterms.yearendtax.client.service.yeta2022.Yeta7500Service;
import com.app.exterms.yearendtax.client.service.yeta2022.Yeta7500ServiceAsync;
...
=====================================================**/
public class Yeta7500   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmYeta7500; 
    private String txtForm = "";
    private XTemplate detailTp;
 
    private Yeta1000Def yeta1000def = new Yeta1000Def("Yeta7500");
...
//		private MSFGridPanel typOccuCdGridPanel;
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
   private ButtonBar topYeta7500Bar;
   private Button btnYeta7500Init; 
//   private Button btnYeta0100Save;
//   private Button btnYeta0100Del;
   private Button btnYeta7500Sreach;
//   private Button btnYeta0100Print;
   private Button btnYeta7500Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 **/
	...
	private Yeta7500ServiceAsync yeta7500Service = Yeta7500Service.Util.getInstance();
	// -------------- 서비스 호출 종료 -------------- 
   ...
   // -------------- stroe [검색부]선언 시작 -------------- 
	   ...
	   **/
	 	 ...
	 	 */
	  	private void checkYeta7500Auth( String authAction, ListStore<BaseModel>bm) {
...
//	    
	 ...
	 **/ 	 
		...
		public Yeta7500(String txtForm) {
	 ...
	 **/		  
		private void createYeta0100Form() {
			...
			plFrmYeta7500 = new FormPanel(); 
			plFrmYeta7500.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산급여반영"));
			plFrmYeta7500.setIcon(MSFMainApp.ICONS.text());
			plFrmYeta7500.setPadding(2);
			plFrmYeta7500.setFrame(true); 
			...
			btnlogView.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 //로그 뷰화면 호출 메서드 
					  funcLogMessage("연말정산급여반영","Yeta7500");
					}
				});
			plFrmYeta7500.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
			plFrmYeta7500.getHeader().addTool(btnlogView); 
			/************************************************************************
			 * 화면 에러 처리시 필요한 부분으로 추가 
			 ************************************************************************/
 
		     //멀티콤보박스 닫기 
			plFrmYeta7500.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
	            @Override
...
//	                        }
			...
			/** 상단 버튼 ButtonBar**/
			topYeta7500Bar = new ButtonBar();    
			topYeta7500Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
			btnYeta7500Init = new Button("초기화");  
			btnYeta7500Init.setIcon(MSFMainApp.ICONS.new16());
			topYeta7500Bar.add(btnYeta7500Init); 
			btnYeta7500Init.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//초기화 
	    			formInit();
	    		}
			...
			btnYeta7500Sreach = new Button("조 회");  
			btnYeta7500Sreach.setIcon(MSFMainApp.ICONS.search());
			topYeta7500Bar.add(btnYeta7500Sreach);
			btnYeta7500Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//조회버튼 클릭시 처리 
	    			reloadYeta7500();  
	    		}
...
//			topYeta7500Bar.add(btnYeta0100Print);	
			
//			btnYeta0100Print = new Button("인 쇄");  
//			btnYeta0100Print.setIcon(MSFMainApp.ICONS.print16());
//			topYeta7500Bar.add(btnYeta0100Print);	
			
			btnYeta7500Excel = new Button("엑 셀");  
			btnYeta7500Excel.setIcon(MSFMainApp.ICONS.excel16());
			topYeta7500Bar.add(btnYeta7500Excel);
			btnYeta7500Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    		public void handleEvent(ButtonEvent e) {
	    			//엑셀 저장
	    			xlsExportData();  
	    		}
			...
			plFrmYeta7500.add(topYeta7500Bar);
		      
			//파일업로드 처리
			plFrmYeta7500.setAction("myurl");
			plFrmYeta7500.setEncoding(Encoding.MULTIPART);
			plFrmYeta7500.setMethod(Method.POST);		         
			vp.add(plFrmYeta7500);	
			
			plFrmYeta7500.setSize("1060px", "750px");	      
			plFrmYeta7500.setLayout(new FlowLayout());
		}
...
//		  	plFrmYeta7500.add(typOccuCdGrid);
...
//														 searchBusinCd.setValue(lsBusinCd.getAt(0));
			    ...
			    lsPayrMangDeptCd.addStoreListener( new StoreListener<BaseModel>() {   
			        public void handleEvent(StoreEvent<BaseModel> be) {  
			        	checkYeta7500Auth("srhPayrMangDeptCd", lsPayrMangDeptCd); 
			        }
			    });    
				    	   ...
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta7500Auth("srhEmymtDivCd", lsEmymtDivCd); 
			           	 }
			        }
			    });    
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		   checkYeta7500Auth("srhDeptCd", lsDeptCd); 
			           	 }
			        	  ...
			        	  EventType type = be.getType();
				    	   if (type == Store.Add) { 
			           		 /**++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				                     ...
				                     * "srhPayrMangDeptCd","srhEmymtDivCd","srhDeptCd","srhTypOccuCd","srhBusinCd"
				                     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
				    		 //  checkYeta7500Auth("srhTypOccuCd", lsTypOccuCd); 
			           	 }
			        }
			    });     
...
//		                BaseModel bmMapModel = (BaseModel)record.getModel();
					...
					yeta7500Service.saveYeta7500Grid(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("saveYeta7500Grid(" + "저장" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
...
//					ntime = format.format(c.getTime());
					...
					while (records.hasNext()) {
		                ...
		                ye161010Dto.setYrtxApptnYrMnth("202202");		/** column 연말정산적용년월 : yrtxApptnYrMnth */
...
//		                ye161010Dto.setDivdPymtDivCd(MSFSharedUtils.allowNulls(bmMapModel.get("divdPymtDivCd$commCd")));/** column 분할납부구분코드 : divdPymtDivCd */
					...
					yeta7500Service.activityOnYeta7500List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta7500List(" + "급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
...
//					ntime = format.format(c.getTime());
					...
					yeta7500Service.activityOnYeta7500_D_List(listYe161010dto, new AsyncCallback<Long>(){
						public void onFailure(Throwable caught) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
		                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYeta7500List(" + "당월급여반영" + ") : " + caught), null);
						}
						public void onSuccess(Long result) { 
							if (result == 0) {
								MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
	...
	private Yeta7500 getThis() {
		return this;
	}
		   
	private void reloadYeta7500() {
	...
	private void xlsExportData() {
		...
		if ("".equals(srhYrtxBlggYr)) {
			...
			if ("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))) {
				...
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta7500Export.do", "extgwtFrame", param);
			}
		} else {
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL()+ "exp/XlsYeta7500Export.do", "extgwtFrame", param);
		}
