...
import com.app.exterms.yearendtax.client.form.defs.yeta2021.Ye162030Def;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63007Service;
import com.app.exterms.yearendtax.client.service.yeta2021.YetaT63007ServiceAsync;
...
=====================================================**/
public  class YetaT63007   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT63007; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT63007GridPanel;
	private Ye162030Def ye162030Def = new Ye162030Def("YetaT63007");   
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT63007ServiceAsync yetaT63007Service = YetaT63007Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT63007Service.activityOnYetaT63007(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT63007(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT63007 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT63007.setBodyBorder(false);
		cpYetaT63007.setBorders(false);
		cpYetaT63007.setHeaderVisible(false);
		cpYetaT63007.setSize("790px", "640px");
		   
		this.add(cpYetaT63007);
...
//		     });     
		...
		cpYetaT63007.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT63007GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT63007GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT63007GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT63007GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT63007GridPanel.getMsfGrid().getGrid().startEditing(YetaT63007GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT63007GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//			public void handleEvent(ButtonEvent e) {
//				setListRecord(YetaT63007GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
//				doAction(actionDatabase);
//
//				if ((ActionDatabase.INSERT).equals(actionDatabase)) {
//					actionDatabase = ActionDatabase.INSERT;
...
//				
//		cpYetaT63007.add(topYetaBtnBar);
...
//		cpYetaT63007.add(topYetaBtnBar);
		...
		YetaT63007GridPanel = new MSFGridPanel(ye162030Def, false, false, false, false, false);
		YetaT63007GridPanel.setSize("790px", "200px");
		YetaT63007GridPanel.setBorders(true);
		YetaT63007GridPanel.setHeaderVisible(false);
...
//		
//		YetaT63007GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT63007GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT63007.add(YetaT63007GridPanel);
...
//		cpYetaT63007.add(layoutContainer1);
//		cpYetaT63007.add(layoutContainer2);
		fldstNewFieldset.add(YetaT63007GridPanel);
		cpYetaT63007.add(fldstNewFieldset);
		cpYetaT63007.add(layoutContainer10);
		cpYetaT63007.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT63007GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT63007GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT63007GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT63007GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT63007GridPanel.reload();
