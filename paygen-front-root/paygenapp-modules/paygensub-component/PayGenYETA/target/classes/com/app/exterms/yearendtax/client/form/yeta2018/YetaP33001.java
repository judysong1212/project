...
import com.app.exterms.yearendtax.client.dto.yeta2018.Ye165030BM;
import com.app.exterms.yearendtax.client.dto.yeta2018.Ye165030DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2018.Ye165030Def;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP33001Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP33001ServiceAsync;
...
=====================================================**/
public  class YetaP33001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta23001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP33001GridPanel;
//	private Ye16Ta3170Def YetaP33001ToYe5030Def  = new Ye16Ta3170Def("YetaP33001");	//이월기부금 그리드 테이블 컬럼 define 
	private Ye165030Def YetaP33001ToYe5030Def  = new Ye165030Def("YetaP33001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
// 	private TextField<String> srhClutSeptCd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP33001ServiceAsync yetaP33001Service = YetaP33001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP33001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta23001 = new ContentPanel();

		createSearchForm(); // 기본정보 필드를 적용
		// createMsfGridForm(); //그리드필드
		createStandardForm(); // 기본정보필드

		cpYeta23001.setBodyBorder(false);
		cpYeta23001.setBorders(false);
		cpYeta23001.setHeaderVisible(false);
		cpYeta23001.setSize("790px", "640px");

		this.add(cpYeta23001);
...
//		lsdacRvyyStore = SysComComboUtils.getSysComComboYearData(msfCoCalendarDto);		//급여년도
		...
		cpYeta23001.setLayout(new FlowLayout());
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent be) {
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 YetaP33001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = YetaP33001GridPanel.getTableDef().getDefaultValues();  
		         ...
		         YetaP33001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         YetaP33001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         YetaP33001GridPanel.getMsfGrid().getGrid().startEditing(YetaP33001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 이월기부금 내용을 삭제하시겠습니까?" , new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){	
							...
							Iterator<BaseModel> itBm = YetaP33001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP33001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//				srhClutSeptCd.setValue(lsclutSeptCd.getAt(0));
		...
		cpYeta23001.add(fieldSet);
		cpYeta23001.add(createMsfGridForm(), new FormData("100%"));
	}  
...
//	private void createMsfGridForm() {
		...
		YetaP33001GridPanel = new MSFGridPanel(YetaP33001ToYe5030Def, false, false, false, false, false);
		YetaP33001GridPanel.setBorders(false);
		YetaP33001GridPanel.setBodyBorder(false);
		YetaP33001GridPanel.setHeaderVisible(false); 
		YetaP33001GridPanel.getBottomComponent().setVisible(false);
		
		GWTUtils.findColumnConfig(YetaP33001GridPanel, Ye165030BM.ATTR_THCYRDDCAMT).setStyle("background-color:#F5F6CE;");
		
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		GWTUtils.findColumnConfig(YetaP33001GridPanel, Yeta3170BM.ATTR_CTRBAMNT).setStyle("Height:20px;");
		...
		YetaP33001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		
		cpYeta23001.add(YetaP33001GridPanel);
		cp01.add(YetaP33001GridPanel);
	...
	private void YetaP33001FormSave() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			while (records.hasNext()) {
                ...
                if(!"DELETE".equals(actionDatabase.name())) {
    				...
    				}else if(  Long.parseLong("2013") > Long.parseLong(ye165030Dto.getAttrYr())
    							||  Long.parseLong("2016") < Long.parseLong(ye165030Dto.getAttrYr())) {
                    	MessageBox.alert("경고", "2013~2016년도 데이터만 입력 가능합니다.", null);
                    	dataChk = false;
    					return; 
    				}
...
//                	// 2017년도 미만 데이터만 삭제 처리 가능하도록 
//                	if( Long.parseLong("2017") < Long.parseLong(ye165030Dto.getAttrYr()) ) {
//                    	MessageBox.alert("경고", "2017년도 미만 데이터만 삭제 가능합니다. ", null);
//                    	dataChk = false;
//    					return; 
//                    }
			...
			if(dataChk) {
				...
				yetaP33001Service.activityOnYetaP33001ToYe165030(listYe165030Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP33001ToYe165030(" + actionDatabase.name() + ") : " + caught), null);
					}
	...
	public void reload() {
		...
		YetaP33001GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP33001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		
		YetaP33001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		
		YetaP33001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		
		YetaP33001GridPanel.reload();
