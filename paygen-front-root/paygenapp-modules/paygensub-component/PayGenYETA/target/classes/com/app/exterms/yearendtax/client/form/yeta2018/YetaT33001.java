...
import com.app.exterms.yearendtax.client.form.defs.yeta2018.Ye160510Def;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33001Service;
import com.app.exterms.yearendtax.client.service.yeta2018.YetaT33001ServiceAsync;
...
=====================================================**/
public  class YetaT33001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT33001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT33001GridPanel;
	private Ye160510Def ye160510Def = new Ye160510Def("YetaT33001");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT33001ServiceAsync yetaT33001Service = YetaT33001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT33001Service.activityOnYetaT33001(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT33001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT33001 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT33001.setBodyBorder(false);
		cpYetaT33001.setBorders(false);
		cpYetaT33001.setHeaderVisible(false);
		cpYetaT33001.setSize("790px", "640px");
		   
		this.add(cpYetaT33001);
...
//		     });     
		...
		cpYetaT33001.setLayout(new FlowLayout());
		...
		btnYetaAdd.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT33001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT33001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
				List<Record> lsRec = new ArrayList<Record>();

				
				if((ActionDatabase.INSERT).equals(actionDatabase)) {
					YetaT33001GridPanel.getMsfGrid().clearData();
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT33001.add(topYetaBtnBar);
...
//		cpYetaT33001.add(topYetaBtnBar);
		...
		YetaT33001GridPanel = new MSFGridPanel(ye160510Def, false, false, false, false, false);
		YetaT33001GridPanel.setSize("790px", "200px");
		YetaT33001GridPanel.setBorders(true);
		YetaT33001GridPanel.setHeaderVisible(false);
...
//		
//		YetaT33001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT33001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT33001.add(YetaT33001GridPanel);
		...
		fldstNewFieldset.add(YetaT33001GridPanel);
		
		cpYetaT33001.add(fldstNewFieldset);
		cpYetaT33001.add(layoutContainer10);
		cpYetaT33001.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT33001GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT33001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT33001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT33001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT33001GridPanel.reload();
