...
import com.app.exterms.yearendtax.client.form.defs.yeta2019.Ye162040Def;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43010Service;
import com.app.exterms.yearendtax.client.service.yeta2019.YetaT43010ServiceAsync;
...
=====================================================**/
public  class YetaT43010   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT43010; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT43010GridPanel;
	private Ye162040Def ye162040Def = new Ye162040Def("YetaT43010");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT43010ServiceAsync yetaT43010Service = YetaT43010Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
   ...
   */
	// -------------- 검색 조건 시작 --------------
 	...
 	private MSFNumberField srhSum1; //장기집합투자증권저축 변수명 다시 해줘야 함.
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT43010Service.activityOnYetaT43010(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT43010(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT43010 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT43010.setBodyBorder(false);
		cpYetaT43010.setBorders(false);
		cpYetaT43010.setHeaderVisible(false);
		cpYetaT43010.setSize("790px", "640px");
		   
		this.add(cpYetaT43010);
...
//		     });     
		...
		cpYetaT43010.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT43010GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT43010GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT43010GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT43010GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT43010GridPanel.getMsfGrid().getGrid().startEditing(YetaT43010GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//		topYetaBtnBar.add(btnYetaNew);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT43010GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = YetaT43010GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		cpYetaT43010.add(topYetaBtnBar);
...
//		cpYetaT43010.add(topYetaBtnBar);
		...
		YetaT43010GridPanel = new MSFGridPanel(ye162040Def, false, false, false, false, false);
		YetaT43010GridPanel.setSize("790px", "200px");
		YetaT43010GridPanel.setBorders(true);
		YetaT43010GridPanel.setHeaderVisible(false);
...
//		
//		YetaT43010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT43010GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT43010.add(YetaT43010GridPanel);
		
        FieldSet fldstNewFieldset = new FieldSet();
		fldstNewFieldset.setHeadingHtml("장기집합투자증권저축");
		...
		srhSum1.setFieldLabel("장기집합투자증권저축");
		...
		fldstNewFieldset.add(YetaT43010GridPanel);
		cpYetaT43010.add(fldstNewFieldset);
		cpYetaT43010.add(layoutContainer10);
		cpYetaT43010.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT43010GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT43010GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT43010GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT43010GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT43010GridPanel.reload();
