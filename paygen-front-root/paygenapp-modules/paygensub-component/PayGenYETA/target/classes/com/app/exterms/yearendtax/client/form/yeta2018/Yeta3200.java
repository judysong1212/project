...
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3200Service;
import com.app.exterms.yearendtax.client.service.yeta2018.Yeta3200ServiceAsync;
 ...
 * </pre>
 */
public class Yeta3200 extends MSFPanel {
	...
	private FormPanel plFrmYeta3200;
	private String txtForm = "";


	private MSFGridPanel Yeta3200GridPanel;
	private MSFGridPanel Yeta3200XmlGridPanel;
	private Ye161010Def Yeta3200def = new Ye161010Def("Yeta3200");
	private Ye161010Def Yeta3200defXml = new Ye161010Def("Yeta3200_RESULT");
	...
	// -------------- 상단 버튼 시작 --------------
	private ButtonBar topYeta3200Bar;
	private Button btnYeta3200Sec; 	// 초기화
	private Button btnYeta3200Reset; 	// 초기화
	private Button btnYeta3200New; 		// 신규
	private Button btnYeta3200Save; 	// 저장
	private Button btnYeta3200Del; 		// 삭제
	private Button btnYeta3200Sreach; 	// 조회
	private Button btnYeta3200Print; 	// 인쇄
	private Button btnYetaP320001; 	// 상세정보 팝업
	private Button btnYetaP320002; 	// 상세정보 팝업
	 ...
	 * ++++++++++++++++++++++
	 **/
	private Yeta3200ServiceAsync yeta3200Service = Yeta3200Service.Util.getInstance(); 
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lsClutYrStore.findModel("year",String.valueOf("2018")));
	...
	public Yeta3200(String txtForm) {
	...
	private void createYeta0200Form() {
		...
		plFrmYeta3200 = new FormPanel();
		plFrmYeta3200.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산파일업로드"));
		plFrmYeta3200.setIcon(MSFMainApp.ICONS.text());
		plFrmYeta3200.setPadding(2);
		plFrmYeta3200.setFrame(true);
		...
		plFrmYeta3200.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta3200.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/

		//멀티콤보박스 닫기 
		plFrmYeta3200.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar **/
		topYeta3200Bar = new ButtonBar();
		topYeta3200Bar.setAlignment(HorizontalAlignment.RIGHT);
		
		if ("admin".equals(MSFMainApp.get().getUser().getUsrId())) {	
			btnYeta3200Sec = new Button("암호화패치");
			btnYeta3200Sec.setIcon(MSFMainApp.ICONS.new16());
			topYeta3200Bar.add(btnYeta3200Sec);
			btnYeta3200Sec.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
				 
					Ye161010DTO ye161010Dto = new Ye161010DTO(); 
						   
					yeta3200Service.yetaChangeConvert(ye161010Dto,  new AsyncCallback<String>(){
				                 public void onFailure(Throwable caught) {
				                     MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
				                             MSFMainApp.ADMINMESSAGES.ExceptionMessageService("Yeta3200Service : " + caught), null);
				                 }
		...
		btnYeta3200Reset = new Button("초기화");
		btnYeta3200Reset.setIcon(MSFMainApp.ICONS.new16());
		topYeta3200Bar.add(btnYeta3200Reset);
		btnYeta3200Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//초기화 
				formInit();
				Yeta3200GridPanel.getMsfGrid().clearData(); 
			}
		});  
		...
		btnYeta3200Sreach = new Button("조 회");
		btnYeta3200Sreach.setIcon(MSFMainApp.ICONS.search());
		topYeta3200Bar.add(btnYeta3200Sreach);
		btnYeta3200Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				//조회버튼 클릭시 처리 
				reloadYeta3200();  
			}
		...
		plFrmYeta3200.add(topYeta3200Bar);

		// 파일업로드 처리
		plFrmYeta3200.setAction("myurl");
		plFrmYeta3200.setEncoding(Encoding.MULTIPART);
		plFrmYeta3200.setMethod(Method.POST);

		vp.add(plFrmYeta3200);
		plFrmYeta3200.setSize("990px", "830px");
		plFrmYeta3200.setLayout(new FlowLayout());
...
//		});
	...
	private void createMsfGridForm() {
		...
		Yeta3200GridPanel = new MSFGridPanel(Yeta3200def, false, false, false, false,false);
		Yeta3200GridPanel.setHeaderVisible(false);
		Yeta3200GridPanel.setSize(470, 640);
		Yeta3200GridPanel.setBorders(true);
		final Grid yeta3000DefGrid = Yeta3200GridPanel.getMsfGrid().getGrid();
		yeta3000DefGrid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (Yeta3200GridPanel.getCurrentlySelectedItem() != null) { 
					BaseModel bmData = new BaseModel();
					formInit() ;
					bmData = Yeta3200GridPanel.getCurrentlySelectedItem();
		...
		Yeta3200GridPanel.getGrid().getColumnModel().addHeaderGroup(0, 4, new HeaderGroupConfig("소득공제자료", 1, 2));

		fsGird.add(Yeta3200GridPanel);
		...
		Yeta3200XmlGridPanel = new MSFGridPanel(Yeta3200defXml, false, false,false, false);
		Yeta3200XmlGridPanel.setSize(250, 305);
		Yeta3200XmlGridPanel.setBorders(true);
		Yeta3200XmlGridPanel.setStyleAttribute("marginTop", "5px");
		layoutContainer17.add(Yeta3200XmlGridPanel,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.8));

		btnYetaP320001 = new Button("개별간소화파일");
		btnYetaP320001.setStyleAttribute("marginTop", "5px");
		btnYetaP320001.setStyleAttribute("marginLeft", "5px");
		btnYetaP320001.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					...
					final MSFFormWindows msFwYeta320001 = new MSFFormWindows("자료추출상세내역",yetaP320001Form, "닫기", "920px", "710px", false);
					msFwYeta320001.show();
					yetaP320001Form.setMSFFormWindows(msFwYeta320001);

					//					funcSetPopUpRecord();

					yetaP320001Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP320001,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));

		btnYetaP320002 = new Button("일괄간소화파일");
		btnYetaP320002.setStyleAttribute("marginTop", "5px");
		btnYetaP320002.setStyleAttribute("marginLeft", "5px");
		btnYetaP320002.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				if (systemkey.getValue() == null) {
					Window.alert("정산대상자를 선택해주세요.");
				}
					...
					final MSFFormWindows msFwYeta320002 = new MSFFormWindows("자료추출상세내역",yetaP320002Form, "닫기", "945px", "710px", false);
					msFwYeta320002.show();
					yetaP320002Form.setMSFFormWindows(msFwYeta320002);

					//					funcSetPopUpRecord();

					yetaP320002Form.bind(ppRecord);
				}
			}
		});

		layoutContainer17.add(btnYetaP320002,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.2));

		layoutContainer1_4.add(layoutContainer17, new com.extjs.gxt.ui.client.widget.layout.ColumnData(.495));

		plFrmYeta3200.add(layoutContainer1_4);
	}
	...
	private Yeta3200 getThis(){
		return this;
	}

	private void reloadYeta3200(){

		IColumnFilter filters = null;
		Yeta3200GridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta3200GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 

		//정산구분
		Yeta3200GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta3200GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS);
		Yeta3200GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 


		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd,srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		Yeta3200GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		Yeta3200GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 
		...
		Yeta3200GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta3200GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);


		Yeta3200GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(srhSystemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta3200GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls(srhHanNm.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);    
		Yeta3200GridPanel.getTableDef().addColumnFilter("resnRegnNum", MSFSharedUtils.allowNulls(srhResnRegnNum.getValue()), SimpleColumnFilter.OPERATOR_EQUALS);   

		Yeta3200GridPanel.reload();  

	}

	private void reloadYeta3200Result(){

		IColumnFilter filters = null;
		Yeta3200XmlGridPanel.getTableDef().setTableColumnFilters(filters);

		//정산년
		Yeta3200XmlGridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr,"year") , SimpleColumnFilter.OPERATOR_EQUALS); 
		//정산구분
		Yeta3200XmlGridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		Yeta3200XmlGridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		Yeta3200XmlGridPanel.reload();  
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//submit the form
							plFrmYeta3200.mask("PDF 전자문서 업데이트 중입니다. 업로드 중..."); 

							//파일업로드 처리
							plFrmYeta3200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta3200Import.do");
							plFrmYeta3200.setEncoding(Encoding.MULTIPART);
							plFrmYeta3200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta3200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
							plFrmYeta3200.submit();  
							plFrmYeta3200.onFrameLoad();  
							picBoolFile = true;
						...
						// after file upload 
						plFrmYeta3200.addListener(Events.Submit, new Listener<FormEvent>() {
				...
				if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
					...
					if("".equals(systemkey.getValue())){
						...
						if (filename.length() == 0) {
							...
							//파일업로드 처리
							plFrmYeta3200.setAction(GWT.getHostPageBaseURL() + "/imp/XmlYeta3200DdcImport.do");
							plFrmYeta3200.setEncoding(Encoding.MULTIPART);
							plFrmYeta3200.setMethod(Method.POST);   
							//	                               FormElement frmEl=FormElement.as(plFrmYeta3200.getElement()); 
							//	                               frmEl.setAcceptCharset("UTF-8"); 
