...
import com.app.exterms.yearendtax.client.dto.yeta2020.Ye165030BM;
import com.app.exterms.yearendtax.client.dto.yeta2020.Ye165030DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2020.Ye165030Def;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP53001Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP53001ServiceAsync;
...
=====================================================**/
public  class YetaP53001   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP53001GridPanel;
//	private Ye16Ta3170Def YetaP53001ToYe5030Def  = new Ye16Ta3170Def("YetaP53001");	//이월기부금 그리드 테이블 컬럼 define 
	private Ye165030Def YetaP53001ToYe5030Def  = new Ye165030Def("YetaP53001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
// 	private TextField<String> srhClutSeptCd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP53001ServiceAsync yetaP53001Service = YetaP53001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	...
	public YetaP53001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		lsdacRvyyStore = SysComComboUtils.getSysComComboYearData(msfCoCalendarDto);		//급여년도
		...
		btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
			...
			public void handleEvent(ButtonEvent be) {
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 YetaP53001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = YetaP53001GridPanel.getTableDef().getDefaultValues();  
		         ...
		         YetaP53001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         YetaP53001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         YetaP53001GridPanel.getMsfGrid().getGrid().startEditing(YetaP53001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
				...
				MessageBox.confirm("삭제", "선택하신 이월기부금 내용을 삭제하시겠습니까?" , new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){	
							...
							Iterator<BaseModel> itBm = YetaP53001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaP53001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//	private void createMsfGridForm() {
		...
		YetaP53001GridPanel = new MSFGridPanel(YetaP53001ToYe5030Def, false, false, false, false, false);
		YetaP53001GridPanel.setBorders(false);
		YetaP53001GridPanel.setBodyBorder(false);
		YetaP53001GridPanel.setHeaderVisible(false); 
		YetaP53001GridPanel.getBottomComponent().setVisible(false);
		
		GWTUtils.findColumnConfig(YetaP53001GridPanel, Ye165030BM.ATTR_THCYRDDCAMT).setStyle("background-color:#F5F6CE;");
		
		AggregationRowConfig<BaseModel> payrSummaryRow = new AggregationRowConfig<BaseModel>();  
//		GWTUtils.findColumnConfig(YetaP53001GridPanel, Yeta3170BM.ATTR_CTRBAMNT).setStyle("Height:20px;");
		...
		YetaP53001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		
		cpYeta23001.add(YetaP53001GridPanel);
		cp01.add(YetaP53001GridPanel);
	...
	private void YetaP53001FormSave() {
		...
		if (MSFSharedUtils.paramNotNull(records)) {
			...
			if(dataChk) {
				...
				yetaP53001Service.activityOnYetaP53001ToYe165030(listYe165030Dto, actionDatabase, new AsyncCallback<Long>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("activityOnYetaP53001ToYe165030(" + actionDatabase.name() + ") : " + caught), null);
					}
	...
	public void reload() {
		...
		YetaP53001GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP53001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		
		YetaP53001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
				, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		
		YetaP53001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
				, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		
		YetaP53001GridPanel.reload();
