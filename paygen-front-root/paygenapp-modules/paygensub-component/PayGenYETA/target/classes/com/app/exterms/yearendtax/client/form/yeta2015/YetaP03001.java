...
import com.app.exterms.yearendtax.client.dto.yeta2015.Yeta3170DTO;
import com.app.exterms.yearendtax.client.form.defs.yeta2015.Yeta3170Def;
import com.app.exterms.yearendtax.client.service.yeta2015.YetaP03001Service;
import com.app.exterms.yearendtax.client.service.yeta2015.YetaP03001ServiceAsync;
...
=====================================================**/
public  class YetaP03001   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYeta03001; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel yetaP03001GridPanel;
	private Yeta3170Def yetaP03001ToYeta3170Def  = new Yeta3170Def("YETAP03001");	//이월기부금 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------	
...
// 	private TextField<String> srhSettGbcd;
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP03001ServiceAsync yetaP03001Service = YetaP03001Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
...
//		 }
	  ...
	  public YetaP03001(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//	        createMsfGridForm();		//그리드필드
		   ...
		   cpYeta03001.setBodyBorder(false);
		   cpYeta03001.setBorders(false);
		   cpYeta03001.setHeaderVisible(false);
		   cpYeta03001.setSize("790px", "640px");
		   
		   this.add(cpYeta03001);
...
//		     });     
		...
		cpYeta03001.setLayout(new FlowLayout());
...
//				systemkey.setValue("20080013");
				...
				if (MSFSharedUtils.paramNull(systemkey.getValue())) {
				 ...
				 yetaP03001GridPanel.getMsfGrid().clearData();
				 
				 BaseModel bmData = yetaP03001GridPanel.getTableDef().getDefaultValues();  
...
//		         bmData.set("dpobCd",  MSFMainApp.get().getUser().getDpobCd()); /** column 사업장코드 : dpobCd */
		         ...
		         yetaP03001GridPanel.getMsfGrid().getGrid().stopEditing();      
		         yetaP03001GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
		         yetaP03001GridPanel.getMsfGrid().getGrid().startEditing(yetaP03001GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//				doAction(actionDatabase);
		...
		btnYetaDel.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Iterator<BaseModel> itBm = yetaP03001GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(yetaP03001GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
...
//				srhSettGbcd.setValue(lsSettGbcd.getAt(0));
		...
		cpYeta03001.add(fieldSet);
		cpYeta03001.add(createMsfGridForm(), new FormData("100%"));
	}  
...
//	private void createMsfGridForm() {
		...
		yetaP03001GridPanel = new MSFGridPanel(yetaP03001ToYeta3170Def, false, false, false, false, false);
//		yetaP03001GridPanel.setSize(790, 440);
//		yetaP03001GridPanel.setHeight(400);
		yetaP03001GridPanel.setBorders(false);
		yetaP03001GridPanel.setBodyBorder(false);
		yetaP03001GridPanel.setHeaderVisible(false); 
		yetaP03001GridPanel.getBottomComponent().setVisible(false);
		
//		GWTUtils.findColumnConfig(yetaP03001GridPanel, Yeta3170BM.ATTR_PYMTDDUCSUM).setStyle("background-color:#F5F6CE;");
...
//		
//		 
//		payrSummaryRow.setCellStyle(Yeta3170BM.ATTR_CTRBAMNT,"summary_color");	/** set 기부조정명세기부금액 : ctrbAmnt */
//		payrSummaryRow.setCellStyle(Yeta3170BM.ATTR_BFYRDEAM,"summary_color");	/** set 전년도_공제금액 : bfyrDeam */
//		payrSummaryRow.setCellStyle(Yeta3170BM.ATTR_DETRAMNT,"summary_color");	/** set 공제대상금액 : detrAmnt */
//		
//		payrSummaryRow.setCellStyle(Yeta3170BM.ATTR_DYYRDEAM,"summary_color");	/** set 해당연도공제금액 : dyyrDeam */
//		payrSummaryRow.setCellStyle(Yeta3170BM.ATTR_DYYROTAM,"summary_color");	/** set 해당년도_미공제소멸금액 : dyyrOtam */
//		payrSummaryRow.setCellStyle(Yeta3170BM.ATTR_AYYRNTAM,"summary_color");	/** set 해당년도_미공제이월금액 : ayyrNtam */
//		
//							    
//		payrSummaryRow.setSummaryType(Yeta3170BM.ATTR_CTRBAMNT, SummaryType.SUM);   
//		payrSummaryRow.setSummaryFormat(Yeta3170BM.ATTR_CTRBAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));  
//							    
//		payrSummaryRow.setSummaryType(Yeta3170BM.ATTR_BFYRDEAM, SummaryType.SUM);   
//		payrSummaryRow.setSummaryFormat(Yeta3170BM.ATTR_BFYRDEAM, NumberFormat.getFormat("#,##0;(#,##0)"));  
//							      
//		payrSummaryRow.setSummaryType(Yeta3170BM.ATTR_DETRAMNT, SummaryType.SUM);   
//		payrSummaryRow.setSummaryFormat(Yeta3170BM.ATTR_DETRAMNT, NumberFormat.getFormat("#,##0;(#,##0)"));
//		
//		payrSummaryRow.setSummaryType(Yeta3170BM.ATTR_DYYRDEAM, SummaryType.SUM);   
//		payrSummaryRow.setSummaryFormat(Yeta3170BM.ATTR_DYYRDEAM, NumberFormat.getFormat("#,##0;(#,##0)"));
//		
//		payrSummaryRow.setSummaryType(Yeta3170BM.ATTR_DYYROTAM, SummaryType.SUM);   
//		payrSummaryRow.setSummaryFormat(Yeta3170BM.ATTR_DYYROTAM, NumberFormat.getFormat("#,##0;(#,##0)"));
//		
//		payrSummaryRow.setSummaryType(Yeta3170BM.ATTR_AYYRNTAM, SummaryType.SUM);   
//		payrSummaryRow.setSummaryFormat(Yeta3170BM.ATTR_AYYRNTAM, NumberFormat.getFormat("#,##0;(#,##0)"));
//		
//		
//		yetaP03001GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
		//초기 조회
//		BaseModel bmData = yetaP03001GridPanel.getMsfGrid().getStore().getModels().get(0);  
//		if (MSFSharedUtils.paramNotNull(bmData)){
//			reloadYetaP03001Grid();
//		}
		
//		cp01.add(yetaP03001GridPanel);
//		lcTabFormLayer.add(cp01);
//		
		cpYeta03001.add(yetaP03001GridPanel);
		cp01.add(yetaP03001GridPanel);
		lcTabFormLayer.add(cp01);
//		cpYeta03001.add(lcTabFormLayer);
...
//		if (MSFSharedUtils.paramNotNull(bmData)){
			...
			yetaP03001GridPanel.getTableDef().setTableColumnFilters(filters);
			yetaP03001GridPanel.getTableDef().addColumnFilter("edacRvyy", MSFSharedUtils.allowNulls(srhEdacRvyy.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			yetaP03001GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.getSelectedComboValue(srhSettGbcd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			
//			yetaP03001GridPanel.getTableDef().addColumnFilter("settGbcd", MSFSharedUtils.allowNulls(srhSettGbcd.getValue())
//					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			yetaP03001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			yetaP03001GridPanel.reload();
