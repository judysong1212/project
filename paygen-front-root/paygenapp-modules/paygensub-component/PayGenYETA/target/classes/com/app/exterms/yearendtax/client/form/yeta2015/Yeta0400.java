...
import com.app.exterms.yearendtax.client.service.yeta2015.Yeta0400ServiceAsync;
 ...
 * =====================================================
 **/
public class Yeta0400 extends MSFPanel {
	...
	private FormPanel plFrmYeta0400;
	private FormData formData;
	private String txtForm = "";
	private XTemplate detailTp;
	private MSFGridPanel msfGridPanel;
	private Yeta1000Def yeta1000def = new Yeta1000Def("YETA0400");
...
//	private MSFGridPanel typOccuCdGridPanel;
	 ...
	 **/
	// ---------- 상단 버튼 시작 ----------
	private ButtonBar topYeta0400Bar;
	private Button btnYeta0400Reset;	//초기화
	private Button btnYeta0400Save;		//저장
	private Button btnYeta0400Search;	//조회
	// ---------- 상단 버튼 종료 ----------
	
	// ---------- 엑셀 버튼 시작 ----------
	private ButtonBar exYeta0400Bar;	
...
//	private List<ModelData> mDtalistHdofcCodtnCd;
	...
	private FileUploadField yeta0400flUp; // 파일업로드처리 부분
	...
	private Yeta1000DTO yeta1000Dto;
	// -------------- DTO 선언 종료 --------------
	private Yeta1000BM yeta1000Bm;
	
	
	// -------------- 서비스 호출 시작 --------------
//	private Yeta0100ServiceAsync yeta0100Service = Yeta0100Service.Util.getInstance();
	private Yeta0400ServiceAsync yeta0400Service = Yeta0400Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------
	 ...
	 */
	private void checkYeta0400Auth(String authAction, ListStore<BaseModel> bm) {
	 ...
	 private void authExecEnabled() { 
		 ...
		 /**
			...
			creEdacRvyy.setValue(lsClutYrStore.findModel("year", "2015"));
...
//			formData = new FormData("-650");
			...
			createYeta0400Form();	// 화면 기본정보를 설정
	 ...
	 */
	public Yeta0400() {
		setSize("1010px", "760px");
	}

	public Yeta0400(String txtForm) {
...
//
		...
		plFrmYeta0400.setLayout(new FlowLayout());
		...
		lscreEmymtDivCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta0400Auth("creEmymtDivCd", lscreEmymtDivCd);
				}
			}
		});
		...
		creDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta0400Auth("creDeptCd", lscreDeptCd);
				}
			}
		});
		...
		creTypOccuCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					//checkYeta0400Auth("creTypOccuCd", lsTypOccuCd);
				}
			}
		});
...
//		creBusinCd.getListView().setWidth("200px");
...
//		lcSchCol1.add(layoutContainer1_2,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.3)); 	// 성명, 주민등록번호
	...
	private void createStandardForm() {
		...
		// 파일업로드 처리
		plFrmYeta0400.setAction("myurl");
		plFrmYeta0400.setEncoding(Encoding.MULTIPART);
		plFrmYeta0400.setMethod(Method.POST);

		plFrmYeta0400.setSize("990px", "750px");
		plFrmYeta0400.setLayout(new FlowLayout());
...
//		layoutContainer1_4.setStyleAttribute("marginBottom", "10px");
		...
		exYeta0400Bar = new ButtonBar();
		exYeta0400Bar.setAlignment(HorizontalAlignment.RIGHT);
		...
		exYeta0400Bar.add(btnYetaExImport);

		btnYetaExSave= new Button("신고파일생성");
		exYeta0400Bar.add(btnYetaExSave);
		...
		layoutContainer11.add(exYeta0400Bar);
		...
		plFrmYeta0400.add(layoutContainer1_4,new com.extjs.gxt.ui.client.widget.layout.ColumnData(1));	// 텍스트와 버튼
...
//		});
	...
	private void createYeta0400Form() {

		plFrmYeta0400 = new FormPanel();
		plFrmYeta0400.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 연말정산신고파일생성"));
		plFrmYeta0400.setIcon(Resources.APP_ICONS.text());
		plFrmYeta0400.setPadding(2);
		plFrmYeta0400.setFrame(true);
		...
		plFrmYeta0400.getHeader().setStyleAttribute("padding", "5px 0 2px 0");
		plFrmYeta0400.getHeader().addTool(btnlogView); 
		/************************************************************************
		 * 화면 에러 처리시 필요한 부분으로 추가 
		 ************************************************************************/
		
		   //멀티콤보박스 닫기 
		plFrmYeta0400.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
		            @Override
		            public void handleEvent(ComponentEvent ce) { 
...
//						                        }
	...
	private void createSearchForm() {
		...
		topYeta0400Bar = new ButtonBar();
		topYeta0400Bar.setAlignment(HorizontalAlignment.RIGHT);

		btnYeta0400Reset = new Button("초기화");
		btnYeta0400Reset.setIcon(MSFMainApp.ICONS.new16());
		btnYeta0400Reset.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				resetStandardForm();	//전자신고기본정보 초기화
			}
		});
		topYeta0400Bar.add(btnYeta0400Reset);

//		topBtn = new Button("신 규");
//		topBtn.setIcon(MSFMainApp.ICONS.new16());
//		topYeta0400Bar.add(topBtn);
...
//		buttonBar.add(topBtn);
		...
		btnYeta0400Save = new Button("저 장");
		btnYeta0400Save.setIcon(MSFMainApp.ICONS.save16());
		btnYeta0400Save.addListener(Events.Select, new Listener<ButtonEvent>() {
		...
		topYeta0400Bar.add(btnYeta0400Save);

		btnYeta0400Search = new Button("조 회");
		btnYeta0400Search.setIcon(MSFMainApp.ICONS.search());
		btnYeta0400Search.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				
				yeta1000Dto = new Yeta1000DTO();
				yeta1000Bm = new Yeta1000BM();
...
//				yeta1000Bm.setBusinCd(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"));
				...
				yeta0400Service.getYeta0400List(yeta1000Bm, new AsyncCallback<Yeta1000BM>(){
					...
					@Override
					public void onSuccess(Yeta1000BM result) {
						// TODO Auto-generated method stub
		...
		topYeta0400Bar.add(btnYeta0400Search);

//		topBtn = new Button("인 쇄");
//		topBtn.setIcon(MSFMainApp.ICONS.print16());
//		buttonBar.add(topBtn);
		
		plFrmYeta0400.add(topYeta0400Bar);
		...
		lsEmymtDivCd.addStoreListener(new StoreListener<BaseModel>() {
			public void handleEvent(StoreEvent<BaseModel> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta0400Auth("srhEmymtDivCd", lsEmymtDivCd);
				}
			}
		});
		...
		srhDeptCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					checkYeta0400Auth("srhDeptCd", lsDeptCd);
				}
			}
		});
		...
		srhTypOccuCd.getStore().addStoreListener(new StoreListener<ModelData>() {
			public void handleEvent(StoreEvent<ModelData> be) {
				...
				if (type == Store.Add) {
					 ...
					 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					 */
					//checkYeta0400Auth("srhTypOccuCd", lsTypOccuCd);
				}
			}
		});
...
//        cpGrid.add(gridFieldSet);
		...
		plFrmYeta0400.add(gridFieldSet);
//		plFrmYeta0400.add(msfGridPanel);
	}
...
//		layoutContainer1_5.setBorders(false);
//
//		plFrmYeta0400.add(layoutContainer1_5, new com.extjs.gxt.ui.client.widget.layout.ColumnData(1)); // 텍스트와 버튼
...
//		msfGridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);//사업
	...
	private void yeta0300Dtail(Yeta1000BM result) {
...
//		String strEdacSeilNum = String.valueOf(result.getEdacSeilNum());
		...
		reprName.setValue(MSFSharedUtils.allowNulls(result.getJuriName()));
		juriName.setValue(MSFSharedUtils.allowNulls(result.getJuriName()));
		busiNumb.setValue(MSFSharedUtils.allowNulls(result.getBusiNumb()));
		resuNumb.setValue(MSFSharedUtils.allowNulls(result.getResuNumb()));
		juriNumb.setValue(MSFSharedUtils.allowNulls(result.getJuriNumb()));
		...
		taxaCode.setValue(MSFSharedUtils.defaultNulls(result.getTaxaCode(),"142"));
		incmGbcd.setValue(MSFSharedUtils.defaultNulls(result.getIncmGbcd(),"3"));
		taxaDnum.setValue(MSFSharedUtils.allowNulls(result.getTaxaDnum()));

		charDept.setValue(MSFSharedUtils.allowNulls(result.getCharDept()));
		taxaPrcd.setValue(MSFSharedUtils.defaultNulls(result.getTaxaPrcd(),"9000"));
		homeTxid.setValue(MSFSharedUtils.allowNulls(result.getHomeTxid()));
		charTele.setValue(MSFSharedUtils.allowNulls(result.getCharTele()));
		charName.setValue(MSFSharedUtils.allowNulls(result.getCharName()));
...
//		retireCount.setValue(value);
	...
	/** 정산 마감 및 풀기 **/
	private void finishYeta0400(final String flag) { 
...
//	      					yeta1000Dto.setClosDate("");//마감일자
	      				...
	      				yeta0400Service.finishOnYeta1000(yeta1000Bm, new AsyncCallback<Long>(){
	                         public void onFailure(Throwable caught) {
	                         ...
	                         public void onSuccess(Long result) { 
	                            ...
	                            if (result == 0) {
	                                MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), 
	                                       ( "Y".equals(flag) ? "마감풀기" : "마감") + "(처리)가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
	                            } else {
	                                MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(), 
	                                        ( "Y".equals(flag) ? "마감풀기" : "마감") + "(처리)가 완료되었습니다.", null);
...
//				MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(), "사업명을 선택하십시요.", null);
//			} else { 
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta0400Export.do","extgwtFrame" ,param);   
			//}
		}else{ 
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsYeta0400Export.do","extgwtFrame" ,param);  
		}
...
//		
		...
		yeta0400Service.updateYeta1000(yeta1000Bm, new AsyncCallback<Long>(){
...
//		param.put("businCd", MSFSharedUtils.getSelectedComboValue(creBusinCd,"businCd"));
		...
		if(MSFSharedUtils.paramNull(param.get("resuNumb"))){
			...
			MessageBox.info("", "년도는 필수 입니다.", null);
		}else if(MSFSharedUtils.paramNull(param.get("deptCd"))){
...
//			if("".equals(MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"))){ 
			...
			//	MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(), "사업명을 선택하십시요.", null);
			//} else { 
				msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/FileYeta0400Export.do","extgwtFrame" ,param);   
			//}
		}else{ 
			msfGridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/FileYeta0400Export.do","extgwtFrame" ,param);  
		}	
