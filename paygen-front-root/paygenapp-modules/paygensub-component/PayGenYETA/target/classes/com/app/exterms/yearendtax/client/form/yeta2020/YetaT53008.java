...
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53008Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaT53008ServiceAsync;
...
=====================================================**/
public  class YetaT53008   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT53008; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT53008GridPanel;
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT53008ServiceAsync yetaT53008Service = YetaT53008Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT53008Service.activityOnYetaT53008(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT53008(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT53008 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT53008.setBodyBorder(false);
		cpYetaT53008.setBorders(false);
		cpYetaT53008.setHeaderVisible(false);
		cpYetaT53008.setSize("790px", "640px");
		   
		this.add(cpYetaT53008);
...
//		     });     
		...
		cpYetaT53008.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT53008GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT53008GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT53008GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT53008GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT53008GridPanel.getMsfGrid().getGrid().startEditing(YetaT53008GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT53008GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT53008GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT53008.add(topYetaBtnBar);
...
//		cpYetaT53008.add(topYetaBtnBar);
//		
//		YetaT53008GridPanel = new MSFGridPanel(YetaT53008Def, false, false, false, false, false);
//		YetaT53008GridPanel.setSize("790px", "420px");
//		YetaT53008GridPanel.setBorders(true);
//		YetaT53008GridPanel.setHeaderVisible(false);
//		
...
//		
//		YetaT53008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT53008GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT53008.add(YetaT53008GridPanel);
		...
		cpYetaT53008.add(fldstNewFieldset);
		cpYetaT53008.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT53008GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT53008GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT53008GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT53008GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT53008GridPanel.reload();
