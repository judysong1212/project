...
import com.app.exterms.yearendtax.client.dto.yeta2018.Ye162010BM;
...
import com.app.exterms.yearendtax.client.service.yeta2018.YetaP33007ServiceAsync;
...
=====================================================**/
public  class YetaP33007   extends MSFFormPanel {  
 ...
 **/
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaP33007GridPanel;
//	private Ye16Ta3100Def YetaP33007ToYe162010Def  = new Ye16Ta3100Def("YETAP13007");	//종전근무지내역 그리드 테이블 컬럼 define 
	private Ye162010Def ye162010Def  = new Ye162010Def("YetaP33007");	//종전근무지내역 그리드 테이블 컬럼 define 
	// -------------- grid 선언 종료  ---------------
...
//	private  ListStore<BaseModel> lsdacRvyyStore = new ListStore<BaseModel>(); 	// 년도
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaP33007ServiceAsync yetaP33007Service = YetaP33007Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------		
     ...
     */  
		...
		private void YetaP33007FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
				...
				if(dataChk) {
					...
					yetaP33007Service.activityOnYetaP33007ToYe162010(listYe162010Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
		 ...
		 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		 **/		  
	  public YetaP33007(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//			     });     
			...
			btnYetaNew.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					 ...
					 YetaP33007GridPanel.getMsfGrid().clearData();
					 
					 BaseModel bmData = YetaP33007GridPanel.getTableDef().getDefaultValues();  
			         ...
			         YetaP33007GridPanel.getMsfGrid().getGrid().stopEditing();      
			         YetaP33007GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
			         YetaP33007GridPanel.getMsfGrid().getGrid().startEditing(YetaP33007GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
					...
					MessageBox.confirm("삭제", "선택하신 연금저축 내용을 삭제하시겠습니까?",new Listener<MessageBoxEvent>(){
						...
						public void handleEvent(MessageBoxEvent be) {
							if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
								...
								Iterator<BaseModel> itBm = YetaP33007GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
			...
			btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
				public void handleEvent(ButtonEvent e) {
					...
					setListRecord(YetaP33007GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
					doAction(actionDatabase);
...
//	            	reload();
		...
		private void createMsfGridForm() {
			...
			YetaP33007GridPanel = new MSFGridPanel(ye162010Def, false, false, false, false, false);
//			YetaP33007GridPanel.setSize("790px", "300px");
			YetaP33007GridPanel.setBorders(false);
			YetaP33007GridPanel.setHeaderVisible(false);
			...
			YetaP33007GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
			/** --- 그리드 합계 부분  시작 --- **/
			
			cpYeta23007.add(YetaP33007GridPanel);
...
//			cpYeta23007.add(layoutContainer);
	...
	public void reload() {
		...
		YetaP33007GridPanel.getTableDef().setTableColumnFilters(filters);
		
		YetaP33007GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도 
		YetaP33007GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
															, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
		YetaP33007GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
															, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
			 
		YetaP33007GridPanel.reload();
