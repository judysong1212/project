...
import com.app.exterms.yearendtax.client.form.defs.yeta2020.InfcPsnl0100Def;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP510001Service;
import com.app.exterms.yearendtax.client.service.yeta2020.YetaP510001ServiceAsync;
...
=====================================================**/
public  class YetaP510001   extends MSFFormPanel {  
	...
	private  ContentPanel cpYeta51001; 

	private InfcPsnl0100Def YetaP510001def = new InfcPsnl0100Def("YetaP510001");
	private MSFGridPanel YetaP510001GridPanel;
	...
	private YetaP510001ServiceAsync yetaP510001Service = YetaP510001Service.Util.getInstance();
	 ...
	 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 		
	 **/ 		    
	public YetaP510001(final ActionDatabase actionDatabase, final MSFPanel caller) {
		...
		cpYeta51001 = new ContentPanel();

		createSearchForm(); 	// 검색필드를 적용
		createStandardForm(); 	// 기본정보필드

		cpYeta51001.setBodyBorder(false);
		cpYeta51001.setBorders(false);
		cpYeta51001.setHeaderVisible(false);
		cpYeta51001.setSize("890px", "640px");

		this.add(cpYeta51001);
	...
	private void authExecEnabled() { 
		...
		srhYrtxBlggYr.setValue(lscrPayYr.findModel("year",String.valueOf("2020")));
	...
	private void createSearchForm() {
		...
		//---------------------멀티콤보박스 닫기 -------------------------------------------------
		cpYeta51001.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
	...
	private void createStandardForm() {
		...
		cpYeta51001.add(layoutContainer);
	...
	private LayoutContainer payrModiListInfo() {
		...
		YetaP510001GridPanel = new MSFGridPanel(YetaP510001def, false, false, false, false,false);
		YetaP510001GridPanel.setHeaderVisible(false);
		YetaP510001GridPanel.setBodyBorder(false);
		YetaP510001GridPanel.setBorders(true);
		YetaP510001GridPanel.getBottomComponent().setVisible(false);

		final Grid<BaseModel> grid = YetaP510001GridPanel.getMsfGrid().getGrid();
		...
		btnAllUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {		        
			public void componentSelected(ButtonEvent ce) { 
				...
				int  lsCnt = YetaP510001GridPanel.getMsfGrid().getGrid().getStore().getCount();

				for(int iCnt=0;iCnt < lsCnt;iCnt++) {

					YetaP510001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.TRUE);
					YetaP510001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();

				} 
				YetaP510001GridPanel.getMsfGrid().getGrid().getView().refresh(true);
			}
		});
		...
		btnUnUsed.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				...
				for(int iCnt=0;iCnt < lsCnt;iCnt++) {
					...
					YetaP510001GridPanel.getMsfGrid().getGrid().getStore().getAt(iCnt).set("select", Boolean.FALSE);
					YetaP510001GridPanel.getMsfGrid().getGrid().getStore().commitChanges();
		...
		btnSelected.addSelectionListener(new SelectionListener<ButtonEvent>() {   
			...
			public void componentSelected(ButtonEvent ce) {  
				...
				Iterator<BaseModel> itBm = YetaP510001GridPanel.getMsfGrid().getGrid().getStore().getModels().listIterator(); // .getSelectionModel().getSelectedItems().listIterator();
				...
				yetaP510001Service.saveYetaP510001ToYe161010(listYe161010Dto,listYe161020Dto, actionDatabase, new AsyncCallback<PagingLoadResult<ShowMessageBM>>() {
					...
					public void onFailure(Throwable caught) {
						...
						smForm.setWindowId("YetaP510001");
		...
		refresh.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {  
				YetaP510001GridPanel.reload();	                	
			}  
		});		            
		bottomToolbar.add(refresh);

		//툴바 넣어주는것
		YetaP510001GridPanel.setBottomComponent(bottomToolbar);

		cp01.add(YetaP510001GridPanel);
	...
	public void reload() {
		...
		YetaP510001GridPanel.getTableDef().setTableColumnFilters(filters);

		YetaP510001GridPanel.getTableDef().addColumnFilter("payrMangDeptCd", MSFSharedUtils.getSelectedComboValue(srhPayrMangDeptCd,"payrMangDeptCd"), SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP510001GridPanel.getTableDef().addColumnFilter("emymtDivCd", MSFSharedUtils.getSelectedComboValue(srhEmymtDivCd,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS); 		
		...
		YetaP510001GridPanel.getTableDef().addColumnFilter("hdofcCodtnCd", strHdofcCodtnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDeptCd = YetaUtils.getStrValToBMMultiCombo(lsDeptCd, srhDeptCd.getValue(),"deptNmRtchnt","deptCd");  
		YetaP510001GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);

		String strTypOccuCd = YetaUtils.getStrValToBMMultiCombo(lsTypOccuCd,srhTypOccuCd.getValue(),"typOccuNm","typOccuCd"); 
		YetaP510001GridPanel.getTableDef().addColumnFilter("typOccuCd", strTypOccuCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		String strDtilOccuInttnCd = YetaUtils.getStrValToBMMultiCombo(lsDtilOccuInttnCd,srhDtilOccuInttnCd.getValue(),"dtilOccuClsNm","dtilOccuInttnCd");  
		YetaP510001GridPanel.getTableDef().addColumnFilter("dtilOccuInttnCd", strDtilOccuInttnCd, SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP510001GridPanel.getTableDef().addColumnFilter("businCd", MSFSharedUtils.getSelectedComboValue(srhBusinCd,"businCd"), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP510001GridPanel.getTableDef().addColumnFilter("hanNm", MSFSharedUtils.allowNulls( srhHanNm.getValue() ), SimpleColumnFilter.OPERATOR_EQUALS);	
		YetaP510001GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue()), SimpleColumnFilter.OPERATOR_EQUALS); 

		YetaP510001GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.getSelectedComboValue(srhYrtxBlggYr, "year")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP510001GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd, "commCd")  , SimpleColumnFilter.OPERATOR_EQUALS); 
		YetaP510001GridPanel.getTableDef().addColumnFilter("utDpobCd", MSFSharedUtils.allowNulls(srhUtDpobCd.getValue())  , SimpleColumnFilter.OPERATOR_EQUALS); 
	...
	private void formInit() {
		...
		YetaP510001GridPanel.getMsfGrid().clearData();
