...
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73011Service;
import com.app.exterms.yearendtax.client.service.yeta2022.YetaT73011ServiceAsync;
...
//import com.app.exterms.yearendtax.client.form.defs.yeta2016.YetaT73011Def;
...
=====================================================**/
public  class YetaT73011   extends MSFFormPanel {  
 ...
 **/
	...
	private  ContentPanel cpYetaT73011; 
	...
	// -------------- grid 선언 시작  ---------------	  
	private MSFGridPanel YetaT73011GridPanel;
//	private YetaT73011Def YetaT73011Def = new YetaT73011Def("YetaT73011");   //TODO 일단 화면번호로 Def
	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 서비스 호출 시작 --------------
	private YetaT73011ServiceAsync yetaT73011Service = YetaT73011Service.Util.getInstance();
	// -------------- 서비스 호출 종료 --------------	 
...
//					ye162510Dto.setMnthRnttyHusCnttArea((MSFSharedUtils.allowNulls(bmMapModel.get("mnthRnttyHusCnttArea"))));    		/** column 월세주택계약면적 : mnthRnttyHusCnttArea */
				...
				yetaT73011Service.activityOnYetaT73011(listYe162510Dto, actionDatabase, new AsyncCallback<Long>(){
...
//		 }
	...
	public YetaT73011(final ActionDatabase actionDatabase, final MSFPanel caller) {
...
//		initLoad(); 	
		...
		this.actionDatabase = actionDatabase;
	 	...
	 	cpYetaT73011 = new ContentPanel();  

		createSearchForm();			//기본정보 필드를 적용
	 	createMsfGridForm();		//그리드필드
		   
		cpYetaT73011.setBodyBorder(false);
		cpYetaT73011.setBorders(false);
		cpYetaT73011.setHeaderVisible(false);
		cpYetaT73011.setSize("790px", "640px");
		   
		this.add(cpYetaT73011);
...
//		     });     
		...
		cpYetaT73011.setLayout(new FlowLayout());
...
//	      	     }
//				 
//				YetaT73011GridPanel.getMsfGrid().clearData();
//				 
//				BaseModel bmData = YetaT73011GridPanel.getTableDef().getDefaultValues();  
//		              
...
//		         
//		        YetaT73011GridPanel.getMsfGrid().getGrid().stopEditing();      
//		        YetaT73011GridPanel.getMsfGrid().getGrid().getStore().insert(bmData,0);   
//		        YetaT73011GridPanel.getMsfGrid().getGrid().startEditing(YetaT73011GridPanel.getMsfGrid().getGrid().getStore().indexOf(bmData), 0);   
//
...
//			public void handleEvent(ButtonEvent e) {
//				Iterator<BaseModel> itBm = YetaT73011GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
...
//		topYetaBtnBar.add(btnYetaDel);
		...
		btnYetaSave.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				setListRecord(YetaT73011GridPanel.getMsfGrid().getStore().getModifiedRecords().iterator());
				doAction(actionDatabase);
...
//			topYetaBtnBar.add(btnYetaClose);
		...
		cpYetaT73011.add(topYetaBtnBar);
...
//		YetaT73011GridPanel = new MSFGridPanel(YetaT73011Def, false, false, false, false, false);
//		YetaT73011GridPanel.setSize("790px", "420px");
//		YetaT73011GridPanel.setBorders(true);
//		YetaT73011GridPanel.setHeaderVisible(false);
...
//		
//		YetaT73011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow);  
//		
...
//		
//		YetaT73011GridPanel.getGrid().getColumnModel().addAggregationRow(payrSummaryRow1);
//		/** --- 그리드 합계 부분  종료 --- **/
		
//        cpYetaT73011.add(YetaT73011GridPanel);
...
//		
		...
		cpYetaT73011.add(layoutContainer);
		cpYetaT73011.add(fldstNewFieldset_1);
	...
	public void reload() {
			...
			YetaT73011GridPanel.getTableDef().setTableColumnFilters(filters);
			YetaT73011GridPanel.getTableDef().addColumnFilter("yrtxBlggYr", MSFSharedUtils.allowNulls(srhYrtxBlggYr.getValue())
...
//					, SimpleColumnFilter.OPERATOR_EQUALS);//연말정산귀속년도
			YetaT73011GridPanel.getTableDef().addColumnFilter("clutSeptCd", MSFSharedUtils.getSelectedComboValue(srhClutSeptCd,"commCd")
					, SimpleColumnFilter.OPERATOR_EQUALS);//정산구분
			YetaT73011GridPanel.getTableDef().addColumnFilter("systemkey", MSFSharedUtils.allowNulls(systemkey.getValue())
					, SimpleColumnFilter.OPERATOR_EQUALS);//시스템키
		 
			YetaT73011GridPanel.reload();
