...
=====================================================**/
public class PrgmPayrP405001  extends Lov implements PrgmComDaoConstants { 
	...
	private static ListStore<BaseModel> lsPayYrStore = new ListStore<BaseModel>();		//년도 
	private static ListStore<BaseModel> lsPayMonth = new ListStore<BaseModel>();  		//급여월   
	private static ListStore<BaseModel> lsPayCd = new ListStore<BaseModel>(); 			//급여구분
	...
	private static ComboBox<BaseModel> payYr;						//지급년
	private static ComboBox<BaseModel> payMonth;					//지급월
	private static ComboBox<BaseModel> payCd;     				//급여구분
	private static ComboBox<BaseModel> typOccuCd; //직종  
	private static ComboBox<BaseModel> emymtDivCd; //고용구분  

	public static ComboBox<BaseModel> getPayYr() {
		return payYr;
	}

	public static void setPayYr(ComboBox<BaseModel> payYr) {
		PrgmPayrP405001.payYr = payYr;
	}

	public static ComboBox<BaseModel> getPayMonth() {
		return payMonth;
	}

	public static void setPayMonth(ComboBox<BaseModel> payMonth) {
		PrgmPayrP405001.payMonth = payMonth;
	}

	public static ComboBox<BaseModel> getPayCd() {
		return payCd;
	}

	public static void setPayCd(ComboBox<BaseModel> payCd) {
		PrgmPayrP405001.payCd = payCd;
	}
	...
	public static void setTypOccuCd(ComboBox<BaseModel> typOccuCd) {
		PrgmPayrP405001.typOccuCd = typOccuCd;
	}
	...
	public static void setEmymtDivCd(ComboBox<BaseModel> emymtDivCd) {
		PrgmPayrP405001.emymtDivCd = emymtDivCd;
	}
	...
	public static void setBusinCd(ComboBox<BaseModel> businCd) {
		PrgmPayrP405001.businCd = businCd;
	}
	...
	public static void setResnRegnNum(TextField<String> resnRegnNum) {
		PrgmPayrP405001.resnRegnNum = resnRegnNum;
	}
	...
	public static void setSystemkey(HiddenField<String> systemkey) {
		PrgmPayrP405001.systemkey = systemkey;
	}
	...
	public static void setHdofcCodtnCd(ComboBox<BaseModel> hdofcCodtnCd) {
		PrgmPayrP405001.hdofcCodtnCd = hdofcCodtnCd;
	}

	public PrgmPayrP405001(int width, int height) {
		this();
		this.width  = width;
		this.height = height;
	}

	public PrgmPayrP405001() { 
		// 권한처리 부분 추가 전역이 아니 각 화면별로 객체 생성시 처리를 위해 변경함.
		gwtAuthorization =  GWTAuthorization.getInstance();
		gwtExtAuth =  GWTExtAuth.getInstance();

		lovDef = new LovDef();
		lovDef.setTitle(SysComLabel.titleSysComPopUpPayr405001());  //팝업상단의 타이틀 
		lovDef.setDaoClass(CLASS_SYSCOM_PSNL0100VO);
		lovDef.setAutoFillGrid(false);   
		lovDef.showFilterForm(); 
		lovDef.setCustomListMethod(PRGMCOM_METHOD_POPUP_PAYR405001_PAYR0304_List);
	...
	public static final LayoutContainer getMsfComP0100Filter() {
		...
		LayoutContainer layoutContainer0 = new LayoutContainer(new ColumnLayout());
		
		LayoutContainer layoutContainer0_1= new LayoutContainer();
		FormLayout frmLyout = new FormLayout(); 
		frmLyout.setLabelWidth(70);
		frmLyout.setLabelAlign(LabelAlign.RIGHT);
		layoutContainer0_1.setLayout(frmLyout);

		payYr = new ComboBox<BaseModel>();
		payYr.setName("payYr");
		payYr.setForceSelection(true);
		payYr.setMinChars(1);
		payYr.setDisplayField("yearDisp");
		payYr.setValueField("year");
		payYr.setTriggerAction(TriggerAction.ALL);
		payYr.setSelectOnFocus(true); 
		payYr.setReadOnly(false);
		payYr.setEnabled(true); 
		payYr.setStore(lsPayYrStore);
		payYr.setFieldLabel("지급년월"); 
		payYr.getStore().addStoreListener( new StoreListener<BaseModel>() {   
			public void handleEvent(StoreEvent<BaseModel> be) {   
				payYr.setValue(lsPayYrStore.findModel("year",DateTimeFormat.getFormat("yyyy").format(new Date())));
			}
		});
		layoutContainer0_1.add(payYr, new FormData("100%"));
		
		LayoutContainer layoutContainer0_2 = new LayoutContainer();
		frmLyout = new FormLayout();  
		frmLyout.setLabelWidth(0); 
		frmLyout.setLabelAlign(LabelAlign.RIGHT); 
		layoutContainer0_2.setLayout(frmLyout);
		layoutContainer0_2.setBorders(false);
        
        /** column 지급년월 : Mnth */ 
        payMonth = new ComboBox<BaseModel>();
        payMonth.setName("payMonth");
        payMonth.setForceSelection(true);
        payMonth.setMinChars(1);
        payMonth.setDisplayField("monthDisp");
        payMonth.setValueField("month");
        payMonth.setTriggerAction(TriggerAction.ALL);
        payMonth.setSelectOnFocus(true); 
        payMonth.setReadOnly(false);
        payMonth.setEnabled(true); 
        payMonth.setStore(lsPayMonth);
        payMonth.setHideLabel(true);
        payMonth.getStore().addStoreListener( new StoreListener<BaseModel>() {   
			public void handleEvent(StoreEvent<BaseModel> be) {   
				payMonth.setValue(lsPayMonth.findModel("month",DateTimeFormat.getFormat("MM").format(new Date())));
			}
		});
        layoutContainer0_2.add(payMonth, new FormData("100%"));
        
        LayoutContainer layoutContainer_1 = new LayoutContainer();
		frmLyout = new FormLayout();  
		frmLyout.setLabelWidth(80); 
		frmLyout.setLabelAlign(LabelAlign.RIGHT); 
		layoutContainer_1.setLayout(frmLyout);
		...
		layoutContainer_1.add(payCd, new FormData("100%"));
			...
			public void selectionChanged(SelectionChangedEvent<BaseModel> se) {
				...
				if (bmDeptCd != null && MSFConfiguration.EMYMT_DIVCD02.equals(emymtDivCd.getValue().get("commCd"))) {
					if (bmDeptCd != null) {
						...
						sysComBass0500Dto.setBusinApptnYr(MSFSharedUtils.getSelectedComboValue(payYr,"year")); 
					}      
					//--------------------사업 불러 오는 함수 -------------------------------------------------
		...
		layoutContainer.add(layoutContainer_1,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.3)); 
		
		layoutContainer0.add(layoutContainer0_1,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.72)); 
		layoutContainer0.add(layoutContainer0_2,new com.extjs.gxt.ui.client.widget.layout.ColumnData(.25)); 
