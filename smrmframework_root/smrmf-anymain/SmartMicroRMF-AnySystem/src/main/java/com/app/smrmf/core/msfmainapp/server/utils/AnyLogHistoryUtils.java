...
public class AnyLogHistoryUtils {
 
    private static Logger logger = LoggerFactory.getLogger(AnyLogHistoryUtils.class);
    
    public enum AnyAtDatabase  { 
	...
	public static void tracerLoginUserActivity(
    	...
    	if (SmrmfUtils.defaultNulls(user.getAppWebConn(),"").equals(PropConfs.SYSM_CONN_WEB)) {
    		SysmConf.setSysmConfigFile(PropConfs.GLOBALS_CONFIG_FILE);
    	} else {
    		SysmConf.setSysmConfigFile(PropConfs.GLOBALS_CONFIG_FILE);
    	} 
    	 
    	HashMap<String, String> msfGlobals = SysmConf.getPayGenConfigInfo(RemoteServiceUtil.getThreadLocalConfig(), RemoteServiceUtil.getThreadLocalRequest()); 

     	logger.info("넘어온 값 확인 : " +  user.getAppWebConn());
    	
        boolean traceEnabled = "true".equals(msfGlobals.get("Globals.TraceUserActivity"));   //Globals.TRACE_USER_ACTIVITY
     
        logger.info("넘어온 값 확인 : " +  traceEnabled);
...
//             }  
        	  ...
        	  logHisServDAO = (LogHistoryServiceDAO)BeanFinder.getBean("LogHistoryServiceDAO");
        	  ...
        	  hist0100Vo.setUsrConnectDivCd("WEB");    			/** column 사용자접속구분코드 : usrConnectDivCd */
        	 ...
        	 // date = new Date(httpSession.getLastAccessedTime());
        	  ...
        	  hist0200Vo.setUsrConnectDivCd("WEB");    			/** column 사용자접속구분코드 : usrConnectDivCd */
...
//    	  String strJobSeptDivCd = "XXXXXXXX";
    	  ...
    	  if(typeActivity.contains(":")) {
    		  ...
    		  for(int i = 0; i < TypeActivity.length; i ++) {
            	  ...
            	  if ("C".equals(strTypeActivity)  || (AnyAtDatabase.INSERT.toString()).equals(typeActivity)) {
            		  
            		  sbJobSeptDivCd.replace(1, 2, "C");
            		  
            		  hist0200Vo.setUsrJobSeptDivCd(sbJobSeptDivCd.toString());    								/** column 사용자작업구분코드 : usrJobSeptDivCd */
            		  
            	  } else if("R".equals(strTypeActivity) || (AnyAtDatabase.READ.toString()).equals(typeActivity)){
            		  
            		  sbJobSeptDivCd.replace(2, 3, "R");
            		
            		  hist0200Vo.setUsrJobSeptDivCd(sbJobSeptDivCd.toString());    								/** column 사용자작업구분코드 : usrJobSeptDivCd */
            	 
            	  } else if("U".equals(strTypeActivity) || (AnyAtDatabase.UPDATE.toString()).equals(typeActivity)){
            		  
            		  sbJobSeptDivCd.replace(3, 4, "U");
            		
            		  hist0200Vo.setUsrJobSeptDivCd(sbJobSeptDivCd.toString());    								/** column 사용자작업구분코드 : usrJobSeptDivCd */
            	
            	  } else if("D".equals(strTypeActivity) || (AnyAtDatabase.DELETE.toString()).equals(typeActivity)){
            		  
            		  sbJobSeptDivCd.replace(4, 5, "D");
            		
            		  hist0200Vo.setUsrJobSeptDivCd(sbJobSeptDivCd.toString());    								/** column 사용자작업구분코드 : usrJobSeptDivCd */
            	 
            	  } else if("S".equals(strTypeActivity) || (AnyAtDatabase.LIST.toString()).equals(typeActivity) || (AnyAtDatabase.CUSTOM.toString()).equals(typeActivity) ){
        	  ...
        	  if ("C".equals(typeActivity) || (AnyAtDatabase.INSERT.toString()).equals(typeActivity)) {
        		  hist0200Vo.setUsrJobSeptDivCd("CXXXXXXX");    												/** column 사용자작업구분코드 : usrJobSeptDivCd */
        	 
        	  } else if("R".equals(typeActivity) || (AnyAtDatabase.READ.toString()).equals(typeActivity)){
        		
        		  hist0200Vo.setUsrJobSeptDivCd("XRXXXXXX");    											/** column 사용자작업구분코드 : usrJobSeptDivCd */
        	 
        	  } else if("U".equals(typeActivity) || (AnyAtDatabase.UPDATE.toString()).equals(typeActivity)){
        		
        		  hist0200Vo.setUsrJobSeptDivCd("XXUXXXXX");    											/** column 사용자작업구분코드 : usrJobSeptDivCd */
        	
        	  } else if("D".equals(typeActivity) || (AnyAtDatabase.DELETE.toString()).equals(typeActivity)){
        		
        		  hist0200Vo.setUsrJobSeptDivCd("XXXDXXXX");    											/** column 사용자작업구분코드 : usrJobSeptDivCd */
        	 
        	  } else if("S".equals(typeActivity) || (AnyAtDatabase.LIST.toString()).equals(typeActivity) || (AnyAtDatabase.CUSTOM.toString()).equals(typeActivity) ){
...
//                if ( isDaoDetached ) {
//                    detachedDao = dao;
//                } else if ( actionDatabase==AnyAtDatabase.INSERT ) {
...
//                   ,idValue);
          ...
          //  throw SmrmfUtils.getOperationException("insertHistoryUserActivity()", ex, logger);
        }
        finally {
...
//        HttpSession httpSession =  RemoteServiceUtil.getThreadLocalSession(); 
        ...
        try {
		       ...
		       if (SmrmfUtils.dataNotNull(user)) {
					...
					hist0300Vo.setUsrConnectDivCd(PropConfs.SYSM_CONN_WEB);	/** column 사용자접속구분코드 : usrConnectDivCd */
		...
		//	    	  } 
					...
					hist0300Vo.setErrCtnt(SmrmfUtils.getStackTraceString(caught));    					    	/** column 에러내용 : errCtnt */
