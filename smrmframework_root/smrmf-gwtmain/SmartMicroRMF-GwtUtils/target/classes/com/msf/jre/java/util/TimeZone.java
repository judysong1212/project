	 ...
	 private static TimeZone getDefaultTimeZone()
	 {
		 ...
		 for( String value : getAvailableIDs())
		 {
			...
			if( tz.timeZoneContainer.getTimeZone().getOffset(new Date()) == offset)
			{
				...
				{
                    //Asia/Seoul
					//Give priority to NY TZ for now, use googles geolocation later America/New_York
					if(tz.getID().equals("Asia/Seoul"))
					{
						return tz;
					}
		...
		{
			return getTimeZone("Etc/GMT+9");    //return getTimeZone("Etc/GMT+0");
		}
