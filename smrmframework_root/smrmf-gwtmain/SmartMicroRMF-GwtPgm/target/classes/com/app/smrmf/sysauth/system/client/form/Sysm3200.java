...
import com.app.smrmf.sysauth.system.client.form.defs.SysmHist0200Def;
import com.app.smrmf.sysauth.system.client.service.Sysm3200Service;
import com.app.smrmf.sysauth.system.client.service.Sysm3200ServiceAsync;
...
=====================================================**/
public class Sysm3200   extends MSFPanel {
...
//	private HiddenField<String> systemkey;	/** column SYSTEMKEY : systemkey */
	...
	// -------------- grid 선언 시작  ---------------
 	private SysmHist0200Def hist0200Def  = new SysmHist0200Def("");   //그리드 테이블 컬럼 define  
	...
	// -------------- 코드 호출 시작 --------------
	private Sysm3200ServiceAsync hist0200Service = Sysm3200Service.Util.getInstance();										//공통코드 
	// -------------- 코드 호출 종료 --------------
/**
 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
 * 	전역변수 선언부  종료
...
//	  		detailTp = XTemplate.create(getDetailTemplate());
	 ...
	 **/ 	 
		...
		public Sysm3200(String txtForm) {
...
//		plFrmSysm3200.setMethod(Method.POST);
		...
		// 오라클 용량 체크_hieju 08.07************************************************************************************************************
		btnSysm3200Excel = new Button("점검내역");  
		...
		btnSysm3200Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
    		public void handleEvent(ButtonEvent e) {
    			//메세지 추가
                final MessageBox box = MessageBox.wait("용량점검", "데이터베이스 용량체크 중입니다...", "용량 체크 중...");
                ...
                hist0200Service.getrequestdataList(new AsyncCallback<List<BaseModel>>(){

					@Override
					public void onFailure(Throwable arg0) {
						 timer.cancel();
		                 box.close();
						 MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(),"용량체크가 정상적으로 처리되지 않았습니다.", null);
					}
					...
					public void onSuccess(List<BaseModel> arg0) {
		                tracker.setStatus(true);
						 ...
						 //엑셀저장  
						 requestExcelFileExport();
						 // MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(),"용량체크가 정상적으로 처리되었습니다.", null);
					}
...
//			_MethodAction := 'insert';
	...
	// 오라클 디비 엑셀 저장
	private void requestExcelFileExport() {
		...
		if("".equals(srhDataType)){
			...
			hist0200GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "/exp/XlsSysm3200Export.do","extgwtFrame", param);  
		}
	}
