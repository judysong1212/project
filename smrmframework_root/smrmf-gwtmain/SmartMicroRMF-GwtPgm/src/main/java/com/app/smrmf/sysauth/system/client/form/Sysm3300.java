...
import com.app.smrmf.sysauth.system.client.form.defs.SysmHist0300Def;
import com.app.smrmf.sysauth.system.client.service.Sysm3300Service;
import com.app.smrmf.sysauth.system.client.service.Sysm3300ServiceAsync;
...
=====================================================**/
public class Sysm3300   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmSysm3300; 
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
	private ButtonBar topSysm3300Bar;
//	private Button btnStts0100Init;
	private Button btnSysm3300Sreach;
	private Button btnSysm3300Del;
//	private Button btnStts0100Print;
	private Button btnSysm3300Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
//	private TextField<String> srhHourS; 							// 성명
//	private TextField<String> srhMinuteS;							// 주민등록번호
//	private TextField<String> srhHourE; 							// 성명
//	private TextField<String> srhMinuteE;							// 주민등록번호
...
//	private HiddenField<String> systemkey;	/** column SYSTEMKEY : systemkey */
	...
	// -------------- grid 선언 시작  ---------------
 	private SysmHist0300Def hist0300Def  = new SysmHist0300Def("");   //그리드 테이블 컬럼 define  
	...
	// -------------- 코드 호출 시작 --------------
	private Sysm3300ServiceAsync hist0300Service = Sysm3300Service.Util.getInstance();						
	// -------------- 코드 호출 종료 --------------
/**
 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
 * 	전역변수 선언부  종료
...
//	 			 권한 검색조건처리를 위해 추가된 부분
		...
		private void hist0300FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
	              
				List<SysIfHist0300DTO> listHist0300dto = new ArrayList<SysIfHist0300DTO>();  
				...
				while (records.hasNext()) {
	                ...
	                Hist0300Dto.setDpobCd(MSFSharedUtils.allowNulls(bmMapModel.get("dpobCd")));    				/** column 사업장코드 : dpobCd */
	                Hist0300Dto.setErrOccrrncYr(MSFSharedUtils.allowNulls(bmMapModel.get("errOccrrncYr")));    	/** column 에러발생년도 : errOccrrncYr */
	                Hist0300Dto.setUsrId(MSFSharedUtils.allowNulls(bmMapModel.get("usrId")));   				/** column 사용자아이디 : usrId */
	                Hist0300Dto.setErrSeilNum((Long) bmMapModel.get("errSeilNum"));    							/** column 에러일련번호 : errSeilNum */
	                
	                listHist0300dto.add(Hist0300Dto);
				}  
	                
				if (listHist0300dto.size() <= 0) {
					MessageBox.alert(actionDatabase.name() + "삭제", actionDatabase.name() +  "처리할 데이타가 존재하지 않습니다.", null);
					return;
				}
	                
				hist0300Service.deleteHist0300(listHist0300dto, new AsyncCallback<Integer>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("deleteHist0300(" + actionDatabase.name() + ") : " + caught), null);
					}
...
//	  		detailTp = XTemplate.create(getDetailTemplate());
	 ...
	 **/ 	 
		...
		public Sysm3300(String txtForm) {
	...
	private void createStts0100Form() {
		...
		plFrmSysm3300 = new FormPanel();
//		plFrmSysm3300.setHeadingText(PayGenConst.PAYGEN_VERSION.concat(" - 프로그램에러로그"));
		plFrmSysm3300.setHeadingText("ExTerms Ver1.0 - " + " - 프로그램에러로그");
		plFrmSysm3300.setIcon(MSFMainApp.ICONS.text());
		plFrmSysm3300.setPadding(2);
		plFrmSysm3300.setFrame(true);
		
		plFrmSysm3300.setSize("990px", "680px");
		plFrmSysm3300.setLayout(new FlowLayout());
		
 
		//멀티콤보박스 닫기 
		plFrmSysm3300.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar**/
		topSysm3300Bar = new ButtonBar();    
		topSysm3300Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
//			btnStts0100Init = new Button("초기화");  
//			btnStts0100Init.setIcon(MSFMainApp.ICONS.new16());
//			topSysm3300Bar.add(btnStts0100Init);
			    
		      
		btnSysm3300Sreach = new Button("조 회", Stts0100ButtonListener);
		btnSysm3300Sreach.setIcon(MSFMainApp.ICONS.search());
		topSysm3300Bar.add(btnSysm3300Sreach);
		btnSysm3300Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
		...
		btnSysm3300Del = new Button("삭 제", Stts0100ButtonListener);
		btnSysm3300Del.setIcon(MSFMainApp.ICONS.delete16());
		topSysm3300Bar.add(btnSysm3300Del);
		btnSysm3300Del.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				MessageBox.confirm("경고", "해당 데이터를 삭제하시겠습니까?", new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							Iterator<BaseModel> itBm  = hist0300GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnSysm3300Excel = new Button("엑 셀");  
		btnSysm3300Excel.setIcon(MSFMainApp.ICONS.excel16());
		topSysm3300Bar.add(btnSysm3300Excel); 
		btnSysm3300Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
    		public void handleEvent(ButtonEvent e) {
    			//엑셀저장  
    			excelFileExport();
    		}
		...
		plFrmSysm3300.add(topSysm3300Bar);
		      
//		// 파일업로드 처리
//		plFrmSysm3300.setAction("myurl");
//		plFrmSysm3300.setEncoding(Encoding.MULTIPART);
//		plFrmSysm3300.setMethod(Method.POST);
		vp.add(plFrmSysm3300);
    ...
    private void createSearchForm() { 
        ...
        //--------------------시간 구분 코드 불러 오는 함수 --------------------------------------------------
		//시간
		hist0300Service.getHourList(new AsyncCallback<List<BaseModel>>() {
			@Override
		...
		//분
		hist0300Service.getMinuteList(new AsyncCallback<List<BaseModel>>() {
			@Override
			public void onSuccess(List<BaseModel> result) {
				lsMinute.add(result);
			}
...
//		cpGrid.setSize(940, 430);
		...
		final Grid hist0300Grid = hist0300GridPanel.getMsfGrid().getGrid();
		hist0300Grid.addListener(Events.CellDoubleClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				SysmP330001 histP030001Form = new SysmP330001(ActionDatabase.READ, getThis());   
			    
//				MSFFormWindows msFwHistP030001= new MSFFormWindows("사용자에러상세정보", histP030001Form, "닫기", "820px", "400px", true);
				MSFFormWindows msFwHistP030001= new MSFFormWindows("사용자에러상세정보", histP030001Form, "닫기", "890px", "4850px", true);
				
				msFwHistP030001.show();
				histP030001Form.setMSFFormWindows(msFwHistP030001);
				
		 	   	BaseModel bmData = hist0300GridPanel.getCurrentlySelectedItem();  
		 	   
			 	bmData.set("dpobCd", (String) bmData.get("dpobCd"));
			 	bmData.set("deptCd", (String) bmData.get("deptCd"));
			 	bmData.set("deptNm", (String) bmData.get("deptNm"));
			 	bmData.set("usrId", (String) bmData.get("usrId"));
			 	bmData.set("usrNm", (String) bmData.get("usrNm"));
			 	bmData.set("errOccrrncDtntm",(String) bmData.get("errOccrrncDtntm"));
			 	bmData.set("errEthdNm", (String) bmData.get("errEthdNm"));
			 	bmData.set("errNm", (String) bmData.get("errNm"));
			 	bmData.set("errCtnt", (String) bmData.get("errCtnt"));
			 	bmData.set("systemkey", (String) bmData.get("systemkey"));

				setPPRecord(bmData);
				
				histP030001Form.bind(ppRecord); 
			}
		});
		cpGrid.add(hist0300GridPanel); 
	
		lcStdGrid.add(cpGrid);   
		fieldSetGrd.add(lcStdGrid);   
		
//		LabelField lblfldNewLabelfield = new LabelField(" * 작업코드분류 :  [C] [R][U][D][S][B] , 구분 : [ APP : 업무페이지 ] [ WEB : 마이페이지 ] ");
//		fieldSetGrd.add(lblfldNewLabelfield, new FormData("100%"));
			      
		plFrmSysm3300.add(fieldSetGrd, new FormData("100%"));
	}
...
//			_MethodAction := 'insert';
	...
	public void reload() {
		...
		//그리드 초기화
		hist0300GridPanel.getMsfGrid().clearData();
				    
		IColumnFilter filters = null;
		hist0300GridPanel.getTableDef().setTableColumnFilters(filters);
		
		hist0300GridPanel.getTableDef().addColumnFilter("usrConnectDivCd"
				, MSFSharedUtils.getSelectedComboValue(srhDataType,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS);								// 조회구분 
		
		hist0300GridPanel.getTableDef().addColumnFilter("errOccrrncDtntmS"
		...
		hist0300GridPanel.getTableDef().addColumnFilter("errOccrrncDtntmE"
		...
		hist0300GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);							// 부서

		
		hist0300GridPanel.getTableDef().addColumnFilter("systemkey", srhSystemkey.getValue() , SimpleColumnFilter.OPERATOR_EQUALS);	// 시스템키
		
		hist0300GridPanel.reload();
	...
	private void excelFileExport() {
		...
		param.put("errOccrrncDtntmS", ( MSFSharedUtils.getConvertDateToString(srhDayS, "yyyyMMdd") 
				+ MSFSharedUtils.getSelectedComboValue(srhHourS,"commCd") + MSFSharedUtils.getSelectedComboValue(srhMinuteS,"commCd")));		// 에러발생시간
		param.put("errOccrrncDtntmE", ( MSFSharedUtils.getConvertDateToString(srhDayE, "yyyyMMdd") 
				+ MSFSharedUtils.getSelectedComboValue(srhHourE,"commCd") + MSFSharedUtils.getSelectedComboValue(srhMinuteE,"commCd")));		// 에러발생시간
		...
		if("".equals(srhDataType)){
			...
			hist0300GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsFileSysm3300Export.do","extgwtFrame" ,param);  
		}
	}	
	...
	**/
	...
	private void fnPopupPsnl0100()  {
        MSFFormPanel popCom0100 = SysIfComPopupUtils.lovPopUpMsfCom0100Form(srhHanNm.getValue());  //인사 
