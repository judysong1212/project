...
import com.app.smrmf.infc.sysif.msfpackage.client.form.SysIfP0910;
...
public class Sysm0100Def extends TableDef implements SysmDaoConstants {
    private int row = 0;
    ...
    public Sysm0100Def(String chkMenu){
...
//        }); 
    	...
    	listColumnDefs.add(new ColumnDef("성명", SysIfSysm0100BM.ATTR_USRNM, ColumnDef.TYPE_STRING , 90, true, true, true){
    		{
    			 if (Constants.SSO_USE_TYPE.equals(MSFMainApp.getMsg("PayGen.SsoUseType"))) { 
    				 if (Constants.SSO_LINK_TYPE.equals(MSFMainApp.getMsg("PayGen.SsoLinkType"))) { 
    					 setLov(new SysIfP0910(600, 500));
    				 } else {
    	...
    	listColumnDefs.add(new ColumnDef("연말정산", SysIfSysm0100BM.ATTR_YRTXINPTPRCSYN, ColumnDef.TYPE_BOOLEAN , 90, false, true, true){
    		{
...
//  });
    ...
    protected void setEditor(CheckBox checkBoxField) {
