...
import com.app.smrmf.sysauth.systemusr.client.form.defs.SysUsrHist0100Def;
import com.app.smrmf.sysauth.systemusr.client.service.SysmUsr3100Service;
import com.app.smrmf.sysauth.systemusr.client.service.SysmUsr3100ServiceAsync;
...
=====================================================**/
public class SysmUsr3100   extends MSFPanel {
 ...
 **/
    ...
    private FormPanel plFrmSysm3100; 
    ...
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    */
	private ButtonBar topSysm3100Bar;
//	private Button btnStts0100Init;
	private Button btnSysm3100Sreach;
	private Button btnSysm3100Del;
//	private Button btnStts0100Print;
	private Button btnSysm3100Excel;
   /**
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    * 검색변수 선언 
    * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
//	private HiddenField<String> systemkey;	/** column SYSTEMKEY : systemkey */
	...
	// -------------- grid 선언 시작  ---------------
 	private SysUsrHist0100Def hist0100Def  = new SysUsrHist0100Def("");   //그리드 테이블 컬럼 define  
 	private MSFGridPanel hist0100GridPanel;
 	// -------------- grid 선언 종료  ---------------
	...
	// -------------- 코드 호출 시작 --------------
	private SysmUsr3100ServiceAsync hist0100Service = SysmUsr3100Service.Util.getInstance();										//공통코드 
	// -------------- 코드 호출 종료 --------------
/**
 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
 * 	전역변수 선언부  종료
...
//	 			 권한 검색조건처리를 위해 추가된 부분
		...
		private void hist0100FormSave() {  
			if (MSFSharedUtils.paramNotNull(records)) { 
	              
				List<SysIfHist0100DTO> listHist0100dto = new ArrayList<SysIfHist0100DTO>();  
				...
				while (records.hasNext()) {
	                ...
	                Hist0100Dto.setDpobCd(MSFSharedUtils.allowNulls(bmMapModel.get("dpobCd")));    				/** column 사업장코드 : dpobCd */
	                Hist0100Dto.setUsrConnectYr(MSFSharedUtils.allowNulls(bmMapModel.get("usrConnectYr")));    	/** column 사용자접속년도 : usrConnectYr */
	                Hist0100Dto.setUsrId(MSFSharedUtils.allowNulls(bmMapModel.get("usrId")));   						/** column 사용자아이디 : usrId */
	                Hist0100Dto.setUsrConnectSeilNum((Long) bmMapModel.get("usrConnectSeilNum"));    			/** column 사용자접속일련번호 : usrConnectSeilNum */
	                
	                listHist0100dto.add(Hist0100Dto);
				}  
	                
				if (listHist0100dto.size() <= 0) {
					MessageBox.alert(actionDatabase.name() + "삭제", actionDatabase.name() +  "처리할 데이타가 존재하지 않습니다.", null);
					return;
				}
	                
				hist0100Service.deleteHist0100(listHist0100dto, new AsyncCallback<Integer>(){
					public void onFailure(Throwable caught) {
						MessageBox.alert(MSFMainApp.MSFCONSTANTS.ExceptionTitleService(), 
	                              MSFMainApp.ADMINMESSAGES.ExceptionMessageService("deleteHist0100(" + actionDatabase.name() + ") : " + caught), null);
					}
					public void onSuccess(Integer result) { 
						if (result == 0) {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.Errore(), actionDatabase + "처리가 완료되지 않았습니다. 데이타를 확인하십시요.", null);
						} else {
							MessageBox.alert(MSFMainApp.MSFCONSTANTS.Alert(), "총 " + result + "건의 " + actionDatabase + "처리가 완료되었습니다.", null);
							reload();
						} 
					} 
				}); 
...
//	  		detailTp = XTemplate.create(getDetailTemplate());
	 ...
	 **/ 	 
		...
		public SysmUsr3100(String txtForm) {
	...
	private void createStts0100Form() {
		...
		plFrmSysm3100 = new FormPanel();
		plFrmSysm3100.setHeadingText("ExTerms Ver1.0 - " + " - 사용자접속로그");
		plFrmSysm3100.setIcon(MSFMainApp.ICONS.text());
		plFrmSysm3100.setPadding(2);
		plFrmSysm3100.setFrame(true);
		
		plFrmSysm3100.setSize("990px", "680px");
		plFrmSysm3100.setLayout(new FlowLayout());
		
 
		//멀티콤보박스 닫기 
		plFrmSysm3100.addListener(Events.OnClick,new Listener<ComponentEvent>() { 
			@Override
		...
		/** 상단 버튼 ButtonBar**/
		topSysm3100Bar = new ButtonBar();    
		topSysm3100Bar.setAlignment(HorizontalAlignment.RIGHT);
			    
//			btnStts0100Init = new Button("초기화");  
//			btnStts0100Init.setIcon(MSFMainApp.ICONS.new16());
//			topSysm3100Bar.add(btnStts0100Init);
			    
		      
		btnSysm3100Sreach = new Button("조 회", Stts0100ButtonListener);
		btnSysm3100Sreach.setIcon(MSFMainApp.ICONS.search());
		topSysm3100Bar.add(btnSysm3100Sreach);
		btnSysm3100Sreach.addListener(Events.Select, new Listener<ButtonEvent>() {
		...
		btnSysm3100Del = new Button("삭 제", Stts0100ButtonListener);
		btnSysm3100Del.setIcon(MSFMainApp.ICONS.delete16());
		topSysm3100Bar.add(btnSysm3100Del);
		btnSysm3100Del.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				MessageBox.confirm("경고", "해당 데이터를 삭제하시겠습니까?", new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if(Dialog.YES.equals(be.getButtonClicked().getItemId())){
							Iterator<BaseModel> itBm  = hist0100GridPanel.getMsfGrid().getGrid().getSelectionModel().getSelectedItems().listIterator();
		...
		btnSysm3100Excel = new Button("엑 셀");  
		btnSysm3100Excel.setIcon(MSFMainApp.ICONS.excel16());
		topSysm3100Bar.add(btnSysm3100Excel); 
		btnSysm3100Excel.addListener(Events.Select, new Listener<ButtonEvent>() {
	    	public void handleEvent(ButtonEvent e) {
	    		excelFileExport();
			}
		}); 
		...
		plFrmSysm3100.add(topSysm3100Bar);
		      
//		// 파일업로드 처리
//		plFrmSysm3100.setAction("myurl");
//		plFrmSysm3100.setEncoding(Encoding.MULTIPART);
//		plFrmSysm3100.setMethod(Method.POST);
		vp.add(plFrmSysm3100);
    ...
    private void createSearchForm() { 
		...
		//--------------------시간 구분 코드 불러 오는 함수 --------------------------------------------------
		//시간
		hist0100Service.getHourList(new AsyncCallback<List<BaseModel>>() {
			@Override
		...
		//분
		hist0100Service.getMinuteList(new AsyncCallback<List<BaseModel>>() {
			@Override
			public void onSuccess(List<BaseModel> result) {
				lsMinute.add(result);
			}
...
//        lcSchCol1.add(lcSchCol1_5,  new com.extjs.gxt.ui.client.widget.layout.ColumnData(.225));		// 부서
	...
	private void createMsfGridForm() {
		...
		cpGrid.setSize(955, 510);
//		cpGrid.setSize(940, 430);
		    
		hist0100GridPanel = new MSFGridPanel(hist0100Def, false, false, false, false);
		hist0100GridPanel.setHeaderVisible(false);  
		hist0100GridPanel.setBodyBorder(true);
		hist0100GridPanel.setBorders(true);
		
		final Grid hist0100Grid = hist0100GridPanel.getMsfGrid().getGrid();
		hist0100Grid.addListener(Events.RowClick, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {  
				if (hist0100GridPanel.getCurrentlySelectedItem() != null) {    
//					setRecord(hist0100GridPanel.getCurrentlySelectedItem());
	   			} 
	   		}
		}); 
		cpGrid.add(hist0100GridPanel); 
	
		lcStdGrid.add(cpGrid);   
		fieldSetGrd.add(lcStdGrid);   
			      
		plFrmSysm3100.add(fieldSetGrd, new FormData("100%"));
	}
...
//			_MethodAction := 'insert';
	...
	public void reload() {
		...
		//그리드 초기화
		hist0100GridPanel.getMsfGrid().clearData();
				    
		IColumnFilter filters = null;
		hist0100GridPanel.getTableDef().setTableColumnFilters(filters);
		
		hist0100GridPanel.getTableDef().addColumnFilter("usrConnectDivCd"
				, MSFSharedUtils.getSelectedComboValue(srhDataType,"commCd"), SimpleColumnFilter.OPERATOR_EQUALS);							// 조회구분 
		
		hist0100GridPanel.getTableDef().addColumnFilter("usrConnectBgnnDtntm"
		...
		hist0100GridPanel.getTableDef().addColumnFilter("usrConnectEndDtntm"
		...
		hist0100GridPanel.getTableDef().addColumnFilter("deptCd", strDeptCd , SimpleColumnFilter.OPERATOR_EQUALS);							// 부서

		
		hist0100GridPanel.getTableDef().addColumnFilter("systemkey", srhSystemkey.getValue() , SimpleColumnFilter.OPERATOR_EQUALS);			// 시스템키
		
		hist0100GridPanel.reload();
	...
	private void excelFileExport() {
		...
		if("".equals(srhDataType)){
			...
			hist0100GridPanel.callXlsExportServlet(GWT.getHostPageBaseURL() + "exp/XlsFileSysm3100Export.do","extgwtFrame" ,param);  
		}
	}	
