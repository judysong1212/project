...
import com.app.smrmf.constc.jinan.ConstJinAn;
 ...
 */
@Controller 
public class PayrJinAnProcessContainer  extends AbstractCustomServiceImpl {
	

	private static final Logger logger = LoggerFactory.getLogger(PayrJinAnProcessContainer.class);
            ...
            //                                  payr0301Vo.setRevnAddr(payr0250Vo.getRevnAddr());    /** column 수정주소 : revnAddr */ 
                                                ...
                                                while (iterpayr0530Set.hasNext() ) {
                                                    ...
                                                    if ("Y".equals(payr0530.getFamyAllwPymtYn())) {
                                                    	if(("B0260030".equals(payr0530.getFamyExtpyDivCd())) && (!PayrServerUtilsJA.getAgeYn(MSFSharedUtils.allowNulls(payr0530.getPayFamyResnRegnNum()).replaceAll("-", ""))) && ("N".equals(payr0306Vo.getDabpnDducYn()))){
                                                    		payr0306Vo.setFamyExtpySum(new BigDecimal("0"));
                                                    	}
                                                    	else if(("B0260045".equals(payr0530.getFamyExtpyDivCd())) && (!PayrServerUtilsJA.getAgeYn(MSFSharedUtils.allowNulls(payr0530.getPayFamyResnRegnNum()).replaceAll("-", ""))) && ("N".equals(payr0306Vo.getDabpnDducYn()))){
                                                    		payr0306Vo.setFamyExtpySum(new BigDecimal("0"));
                                                    	}
                                                    	else if(("B0260060".equals(payr0530.getFamyExtpyDivCd())) && (!PayrServerUtilsJA.getAgeYn(MSFSharedUtils.allowNulls(payr0530.getPayFamyResnRegnNum()).replaceAll("-", ""))) && ("N".equals(payr0306Vo.getDabpnDducYn()))){
                                                    		payr0306Vo.setFamyExtpySum(new BigDecimal("0"));
                                                    	}
                                                    	else{
                                                                ...
                                                                if ((new BigDecimal(payCalculationVo.getDilnlazAbnceNumDys())).compareTo(BigDecimal.ZERO) > 0) {
	                                                               ...
	                                                               //  payCalculationVo.getDilnlazAbnceNumDys()   /** column 근태_결근일수 : dilnlazAbnceNumDys */
	                                                                abnceNumDysAmt = (new BigDecimal(payCalculationVo.getDilnlazAbnceNumDys()))
	                                                                                .multiply(payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal(ConstJinAn.PAYR_BASS_WKTM_YG_01)));  // USALY_AMNT_HRWG_QNTY 시급통상임금 
            ...
            //                                                   payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                        	   ...
                                                        	   //정근수당 -호봉제 
                                                           	   ...
                                                           	   if (MSFSharedUtils.paramNotNull(payr0450Vo)) {  
                                                        		   ...
                                                        		   //상여금인경우 처리를 위한 로직수정 작업  환경미화원 인경우 통상임금으로 처리 함. 
                                                        		   baseBnusAmnt = payr0410.getMnthUsalyAmntWag(); //.getMnthUsalyAmntAllwBnusSum();
              ...
              //                                                   payr0302Vo.setRevnAddr(payr0302Vo.getRevnAddr());    /** column 수정주소 : revnAddr */   
                                                               ...
                                                               //  edPymtAmnt = (basePymtAmnt.add((tpPayr0302vo.getPymtDducSum().compareTo(BigDecimal.ZERO) > 0 ? tpPayr0302vo.getPymtDducSum() 
                                                                //		                    : payr0410.getPymtDducSum()))).divide(new BigDecimal("3"), -1, BigDecimal.ROUND_DOWN)  ;
                                                       	    	 ...
                                                       	    	 edPymtAmnt = (payr0410.getMnthUsalyAmntWag().multiply((payr0410.getPymtDducRate().divide(new BigDecimal("100"))))).setScale(-1,BigDecimal.ROUND_DOWN) ;
	                                                                 ...
	                                                                 // if ((payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum()).compareTo(BigDecimal.ZERO) > 0) {
	                                                                      if (ExtermsProps.getProps("PAY_EMYMT_PYMT_07").equals(payr0410.getPayrImcd())) {
	                                                                          ...
	                                                                          if (new BigDecimal(payCalculationVo.getDilnlazTotTmRstDutyTm()).compareTo(BigDecimal.ZERO) > 0) {
	                                                                         	  ...
	                                                                         	  BigDecimal ucstSum =  BigDecimal.ZERO;
	                                                                           	  //도로보수원의 경우 고정금액으로 처리 하는걸로 간주 함 todo 통상임금 및 승급에 따른 이전 시간외 통상임금 구하는 로직 으로 전면 수정 
	                                                                              if ("00900010A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) ) {    
	                                                                       ...
	                                                                       // 야간수당       -- 시간산정C9410300 MNTH_USALY_AMNT_WAG
	                                                                          if (new BigDecimal(MSFSharedUtils.defaultNulls(payCalculationVo.getDilnlazTotNtotTm(),"0")).compareTo(BigDecimal.ZERO) > 0) {
	                                                                        		 ...
	                                                                        		 BigDecimal ucstSum =  BigDecimal.ZERO;
		                                                                           	  //도로보수원의 경우 고정금액으로 처리 하는걸로 간주 함 todo 통상임금 및 승급에 따른 이전 시간외 통상임금 구하는 로직 으로 전면 수정 
		                                                                              if ("00900010A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) ) {    
                                                                                           //TODO 차후 야간 근무시 확인 필요 
		                                                                                 //ucstSum =  payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum());
		                                                                               
		                                                                                 //if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
		                                                                               	  // ucstSum =   payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum();
		                                                                                // }
		                                                                            	  ucstSum = (payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal("0.5"))).setScale(5,BigDecimal.ROUND_DOWN);  //payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum());
		                                                                                  
		                                                                                    if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
		                                                                                  	  ucstSum =  (payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal("0.5"))).setScale(5,BigDecimal.ROUND_DOWN);   //payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum();
		                                                                                    }  
		                                                                                 
		                                                                              } else if ("70100010A0060000".equals(payCalculationVo.getDtilOccuInttnCd()) ) {    
 //환경미화원  무조건 150프로
			              		                                                            //시간외근무  -USALY_AMNT_HRWG_QNTY 계산된 시급으로 정리한다. 통상시급 
		                                                                                    ucstSum = (payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal("1.5"))).setScale(5,BigDecimal.ROUND_DOWN);  //payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum());
		                                                                                    ...
		                                                                                    if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
			              		                                                            ...
			              		                                                            //시간외근무  -getUsalyAmntHrwgQnty 계산된 시급으로 정리한다. 통상시급  계산 후 통상시금에 0.5 을 추가 한다 
		                                                                                    ucstSum = (payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal("0.5"))).setScale(5,BigDecimal.ROUND_DOWN);  //payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum());
		                                                                                    ...
		                                                                                    // 야간근무 추가 하는 부분 처리 
		                                                                                  //  ucstSum = ucstSum.add((payr0410.getUsalyAmntHrwgQnty().multiply(new BigDecimal("0.5"))).setScale(5,BigDecimal.ROUND_DOWN)); 
...
//도로보수원
		                                                                                 ...
		                                                                                 if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
		                                                                                    ...
		                                                                                    if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
			              		                                                             ...
			              		                                                             //TODO 차후 확인 통상임금 / 26일을 한값으로 처리 한다. 
		                                                                                    //ucstSum = ((payr0410.getMnthUsalyAmntWag().divide((new BigDecimal("209")),5,BigDecimal.ROUND_HALF_UP)).multiply(new BigDecimal("1.5"))).setScale(5,BigDecimal.ROUND_DOWN);  //payr0410.getBefUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getBefUcstSum() : (payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum());
		                                                                                  
		                                                                                    //if (payr0250Vo.getJobYrMnth().substring(4,6).equals("12")) { 
		                                                                                  	 // ucstSum =  ((payr0410.getMnthUsalyAmntWag().divide((new BigDecimal("209")),5,BigDecimal.ROUND_HALF_UP)).multiply(new BigDecimal("1.5"))).setScale(5,BigDecimal.ROUND_DOWN);   //payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum();
		                                                                                    //}
		              		                                                        	  
		              		                                                        	    ucstSum = (payr0410.getUsalyAmntPdyQnty().multiply(new BigDecimal("1.5"))).setScale(5,BigDecimal.ROUND_DOWN);  
	                                                                      ...
	                                                                      } else   if (ExtermsProps.getProps("PAY_EMYMT_PYMT_38").equals(payr0410.getPayrImcd())) {
	                                                                            //PAY_EMYMT_PYMT_38 = "C9080200";    // 급식비 일단위 게산 처리 로직 추가 로 인해 변경 
	                                                                    	    BigDecimal dayPaySum = BigDecimal.ZERO; 
                                                                                BigDecimal ucstSum = payr0410.getUcstSum().compareTo(BigDecimal.ZERO) > 0 ? payr0410.getUcstSum() : payr0410.getPymtDducSum();
                                                                              
                                                                                //도로보수원인 경우 처리 로직 추가 
                                                                                /**
                                                                                 * 매월 15일 이상근무자은 전액 지급함. 미만인 근로자는 소정근로일수로 전체금액/30일을 한금액으로 산정한다. payCalculationVo.getDilnlazWkdDutyNumDys()
                                                                                 */
                                                                                if ((new BigDecimal(payCalculationVo.getDilnlazLnchDys())).compareTo((new BigDecimal("15"))) >= 0) {                                                                                
	                                                                                ...
	                                                                                if (  (ExtermsProps.getProps("PAY_EMYMT_DYMNTH_01").equals(payr0410.getDayMnthAmntDivCd())) 
	                                                                                    ...
	                                                                                    //총 근무일수  * 본봉
	                                                                                	dayPaySum = ((new BigDecimal(payCalculationVo.getDilnlazLnchDys())).multiply(ucstSum.divide((new BigDecimal("30")),-1, BigDecimal.ROUND_HALF_DOWN))).setScale(-1,BigDecimal.ROUND_DOWN); 
	                                                                      ...
	                                                                      } else   if (ExtermsProps.getProps("PAY_EMYMT_PYMT_06").equals(payr0410.getPayrImcd())) {
...
//	                                                                             
	                                                                      ...
	                                                                      } else   if (ExtermsProps.getProps("PAY_EMYMT_PYMT_45").equals(payr0410.getPayrImcd())) {
	                                                                           //PAY_EMYMT_PYMT_45 = "C9100150";    // 교통비 
	                                                                    	  //도로보수원 만 해당됨 
                                                                              ...
                                                                              if (  (ExtermsProps.getProps("PAY_EMYMT_DYMNTH_01").equals(payr0410.getDayMnthAmntDivCd())) 
                                                                                  ...
                                                                                  //총 근무일수  * 본봉
                                                                              	dayPaySum = ((new BigDecimal(payCalculationVo.getDilnlazTfcAssCstNumDys())).multiply(ucstSum)).setScale(-1,BigDecimal.ROUND_DOWN); 
...
//	                                                                             
	                                                                      ...
	                                                                      } else  if (ExtermsProps.getProps("PAY_EMYMT_PYMT_20").equals(payr0410.getPayrImcd())) {
	                                                                          // PAY_EMYMT_PYMT_20 = "C9410600";    // 연월차로 처리 하는 경우 연차수당 
	                                                                          if (new BigDecimal(payCalculationVo.getDilnlazLvsgNumDys()).compareTo(BigDecimal.ZERO) > 0) {
	                                                                              ...
	                                                                              totTmSum = ((new BigDecimal(payCalculationVo.getDilnlazLvsgNumDys())).multiply(ucstSum)).setScale(-1, BigDecimal.ROUND_DOWN);  
	                                                                    	    ...
	                                                                    	    /* 도로보수원 처리   */
	                                                                    	     if ("00900010A0060000".equals(payCalculationVo.getDtilOccuInttnCd())) {    
	                                                                                   ...
	                                                                                   //   BigDecimal dducSumRate =  payr0410.getPymtDducRate() ; 
	                                                                                    ...
	                                                                                    totTmSum = ucstSum ; //ucstSum.multiply((dducSumRate.divide(new BigDecimal("100"), 5, BigDecimal.ROUND_DOWN))).setScale(-1,BigDecimal.ROUND_DOWN);
	                                                                    	    	     ...
	                                                                    	    	     ucstSum =    payr0410.getMnthUsalyAmntWag();
